...
export function documentTheme(company = {}, accentOverride) {
  ...
  // Computed before the object literal because the "on wash" values below need
  // it as a background to measure against.
  const accentWash = tint(accent, 0.96, 0.1);
  const accentWashStrong = tint(accent, 0.9, 0.16);
  ...
  return {
    ...
    // Body text sitting ON a wash rather than on paper.
    //
    // inkMuted is measured against paper. A wash is a shade darker, which was
    // enough to take the hero subhead to 4.43:1 — under the bar, on the second
    // line of the page, on every site. Small enough that nobody would ever
    // report it and it would never get fixed.
    inkOnWash: ensureContrast(ink, accentWash, 4.5),
    inkMutedOnWash: ensureContrast(inkMuted, accentWash, 4.5),
...
export function fillPair(theme) {
  ...
  const fg = theme.accentOn;
  if (contrastRatio(fg, theme.accentFill) >= 4.5) {
    return { bg: theme.accentFill, fg };
  }

  // ── Mid-tones ─────────────────────────────────────────────────────────────
  //
  // A brand colour around 50% lightness — mid grey, mid blue, olive — reaches
  // roughly 4.3:1 against BOTH black and white. readableForeground picks the
  // better of the two and the better of the two is still not good enough, so
  // no choice of foreground fixes it. The file header warns that mid-tones are
  // where the naive rule fails; this is that case, and it was unhandled.
  //
  // So move the FILL instead of the text. Stepping away from the foreground in
  // small increments keeps it recognisably their colour — a slightly deeper
  // navy on the totals bar, not a substituted one. Capped at 12 steps, after
  // which ink is the honest answer.
  // Move the fill AWAY from the foreground. A light foreground wants a darker
  // fill and vice versa — getting this backwards walks the background towards
  // the text, never reaches 4.5, and silently falls through to ink for every
  // grey brand in the product.
  const fgIsLight = contrastRatio(fg, "#000000") > contrastRatio(fg, "#ffffff");
  let bg = theme.accentFill;
  for (let i = 0; i < 12; i++) {
    bg = adjustLightness(bg, fgIsLight ? -0.05 : 0.05);
...
/**
 * A neutral chip — grey background, legible grey text.
 *
 * Exists because `{ fg: theme.inkMuted, bg: "#f3f4f6" }` was being written by
 * hand in several places and is 4.39:1, not 4.5:1. inkMuted is contrast-checked
 * against PAPER; putting it on a grey chip costs the ~0.1 that took it under.
 *
 * Small, easy to miss, and exactly the sort of thing that survives forever
 * because nobody measures a "Draft" badge. Measured here once instead.
 */
export function neutralPair(theme) {
  const bg = "#f3f4f6";
  return { fg: ensureContrast(theme.inkMuted, bg, 4.5), bg };
}

/**
 * A washed surface — a card, a panel, a bookable day on a calendar — together
 * with text measured against IT rather than against paper.
 *
 * Three decisions live here, and each one was a bug first:
 *
 *  • A near-white brand tints to something indistinguishable from the card, so
 *    the affordance disappears while the text stays perfectly legible. That
 *    branch substitutes the neutral chip, which is visible whatever the brand.
 *  • theme.ink and theme.inkMuted are measured against PAPER. A wash is a shade
 *    darker, which was enough to take a subhead to 4.43:1 — under the bar, on
 *    the second line of the page, on every site.
 *  • theme.accentText is measured against paper too, and costs about 0.2 on a
 *    wash: enough to put lime, mid-grey and pale-grey brands under 4.5:1 on the
 *    one number that says what something costs.
 *
 * app/book/[companySlug]/BookingFlow.js still computes this inline; it is the
 * original and should be folded in here next time it is touched.
 */
export function washPair(theme) {
  const base = accentIsWashedOut(theme)
  ...
  return { ...base, accent: ensureContrast(theme.accentText, base.bg, 4.5) };
}
...
export function statusStyle(status, theme) {
  switch (status) {
    ...
    case "sent": {
      // Measured, not assumed: accentText on accentWash fell under 4.5:1 for
      // seven brand colours including plain red (#dc2626 → 4.41). washPair
      // already decides, per brand, whether the wash can carry ink at all and
      // falls back to the neutral pair when it cannot; the pill uses the
      // same decision the rest of the document uses. Re-executed against the
      // hostile brand set in check:contrast.
      const pair = washPair(theme);
      return { fg: pair.ink, bg: pair.bg };
    }
    default:
      return neutralPair(theme);
  }
}
