...
const CONTENT = {
  ...
  installation_services: { accent: PALETTE.slate, steps: MEASURE_SUPPLY_INSTALL },
  ...
  general_contracting_reno: { accent: PALETTE.slate, steps: PLAN_BUILD_HANDOVER },
  ...
  pressure_washing_house: { accent: PALETTE.denim, steps: VISIT_SERVICE_VERIFY },
  pressure_washing_driveway: { accent: PALETTE.denim, steps: VISIT_SERVICE_VERIFY },
...
export function resolveServiceContent(categoryKey, override) {
  ...
  const included =
    firstArray(override?.includedItems) ||
    base.included ||
    GENERIC.included;
...
export function dominantProcessSteps(groups = []) {
  if (!groups.length) return GENERIC.steps.map((s, i) => ({ ...s, num: i + 1 }));
