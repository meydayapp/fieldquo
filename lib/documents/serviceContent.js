...
export function resolveServiceContent(categoryKey, override, takeoff, language) {
  ...
  // creation), never the viewer's. The French catalogue is merged over the
  // English one field by field, so a trade not yet translated — or one field
  // of it — renders the English sentence rather than nothing, and the gap is
  // a gap in serviceContent.fr.js rather than a blank on a client's quote.
  // A company's own override wins over both, in whichever language they
  // wrote it: that is their text, and this file has no business translating it.
  const french = isFrench(language);
  const en = ownProp(CONTENT, categoryKey) || {};
  const base = french ? { ...en, ...(ownProp(CONTENT_FR, categoryKey) || {}) } : en;
  const generic = french ? { ...GENERIC, ...GENERIC_FR } : GENERIC;
...
export function dominantProcessSteps(groups = [], language) {
  const generic = isFrench(language) ? { ...GENERIC, ...GENERIC_FR } : GENERIC;
  if (!groups.length)
