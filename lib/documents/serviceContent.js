...
import { CONTENT_FR, GENERIC_FR } from "@/lib/documents/serviceContent.fr";
import { CONTENT_ES, GENERIC_ES } from "@/lib/documents/serviceContent.es";
import { CONTENT_IT, GENERIC_IT } from "@/lib/documents/serviceContent.it";
import { CONTENT_DE, GENERIC_DE } from "@/lib/documents/serviceContent.de";
import { CONTENT_UK, GENERIC_UK } from "@/lib/documents/serviceContent.uk";
import { CONTENT_PA, GENERIC_PA } from "@/lib/documents/serviceContent.pa";
import { CONTENT_TL, GENERIC_TL } from "@/lib/documents/serviceContent.tl";

// ── One catalogue per document language ─────────────────────────────────────
//
// Until 2026-09-25 this file knew one other language, and asked `isFrench()`.
// A Spanish, German or Punjabi quote therefore printed English prose under a
// translated frame — the owner's question was "are the presets already
// translated in the job process on your quotes?", and for six of the eight
// document languages the answer was no. The lookup is now per language, the
// same shape for every one: a static catalogue keyed like CONTENT below,
// merged over the English FIELD BY FIELD, so a field a catalogue lacks prints
// the English sentence rather than nothing. scripts/check-service-content-
// languages.mjs proves every catalogue carries every preset string.
//
// Static, not machine-translated at send time (AGENTS.md non-negotiable 6):
// a document picks its catalogue by its own fixed `language` and renders the
// same sentences on every day after it was created.
const CATALOGUES = {
  fr: { content: CONTENT_FR, generic: GENERIC_FR },
  es: { content: CONTENT_ES, generic: GENERIC_ES },
  it: { content: CONTENT_IT, generic: GENERIC_IT },
  de: { content: CONTENT_DE, generic: GENERIC_DE },
  uk: { content: CONTENT_UK, generic: GENERIC_UK },
  pa: { content: CONTENT_PA, generic: GENERIC_PA },
  tl: { content: CONTENT_TL, generic: GENERIC_TL },
};

/**
 * The catalogue language for a document language: "fr-CA" → "fr", "ES" →
 * "es"; anything without a catalogue (null, "", "pt", "french") → "en".
 */
export function presetLanguage(language) {
  const l = String(language || "").trim().toLowerCase().split(/[-_]/)[0];
  return Object.prototype.hasOwnProperty.call(CATALOGUES, l) ? l : "en";
}

/** "fr", "fr-CA", "FR" — the document's language, not the viewer's. */
export function isFrench(language) {
  return presetLanguage(language) === "fr";
}

function catalogueFor(language) {
  const l = presetLanguage(language);
  return l === "en" ? null : CATALOGUES[l];
}

/** The generic block in `language`, English underneath any gap. */
function genericFor(language) {
  const c = catalogueFor(language);
  return c ? { ...GENERIC, ...(c.generic || {}) } : GENERIC;
}
...
// Gutters. The takeoff asks one question first — cleaning, new installation,
// replacement, repair or guard only — and it changes the WORK, not the rate,
// so the scope paragraph has to change with it. Keyed on the same values as
// GUTTER_WORK_TYPES in lib/pricing/tradeScope.js.
//
// Each replaces the paragraph rather than adding to it, and each ends by
// naming what is NOT in it, because the commonest gutter dispute is a client
// who thought the fascia, the roof or the second elevation was included.
const GUTTER_WORK_VARIANTS = {
  cleaning:
    "We clear the gutters and downspouts you have, flush them to prove they drain, and inspect the runs while they are empty — hangers, seams, joints and the fascia behind them — sealing the small faults we find as we go. What is priced here is the work listed above: replacing a run, re-pitching it, or fixing rotten fascia is separate work and appears only where you see it priced.",
  install:
    "We supply and install new eavestrough on the runs priced above, formed on site to the length of your fascia and hung to a fall that carries water to the outlets, with the downspouts listed above taken to where you want the water to go. Only the runs and downspouts priced above are installed — guards, heated cable, soffit, fascia and any roof work are separate lines and are included only where you see them priced.",
  replacement:
    "We take down the eavestrough you have, take it away, and hang new run in its place along the runs priced above, formed on site and set to a fall that carries water to the outlets, with the downspouts listed above. Removing and disposing of the old gutters is part of what you are paying for here — you will see it either inside the per-foot rate or on a line of its own. Fascia found rotten once the old run is off is reported to you with photographs and priced before any of it is repaired. Only the runs and downspouts priced above are replaced.",
  repair:
    "We repair the faults listed above — resealing seams and joints, refastening hangers, and re-pitching short sections so the run drains instead of holding water — and run water through the repaired sections before we leave, so you can see them flow. Repairs are priced by the section on this quote. Replacing a full run, re-pitching the whole system or making good rotten fascia is separate work and appears only where you see it priced.",
  guard_only:
    "We fit the gutter guard priced above over the runs listed above. A guard has to go onto a clear gutter, or it simply traps the debris underneath it, so any run that is not clear is cleaned first — that cleaning is separate work and is included only where you see it priced. Only the runs priced above are guarded, and the downspouts, the roof and anything above the gutter line are separate lines on the same terms.",
};
...
const CONTENT = {
  ...
  // Gutters. The owner's own five-step process, in their order, because the
  // ORDER is the argument: you cannot tell whether a downspout drains until the
  // troughs are clear, and you cannot see a loose hanger or a leaking seam
  // until the debris is out of the way. A quote that lists "clean gutters" and
  // stops is describing one of the five.
  gutter_services: {
    accent: PALETTE.slate,
    description: GUTTER_WORK_VARIANTS.cleaning,
    // QuoteScopeGroup.takeoff carries `workType` for this trade — see
    // createTradeConfig in lib/pricing/tradeScope.js — so a replacement quote
    // has no excuse to describe a cleaning visit. The trade-level paragraph
    // above IS the cleaning one, because an unrecognised value falls back to
    // it and cleaning is both the commonest job and the takeoff's default.
    variantOn: "workType",
    variantLabel: "the type of gutter work",
    variants: GUTTER_WORK_VARIANTS,
    included: [
      "Gutters cleared by hand and the debris taken away, not blown onto the grounds",
      "Every downspout flushed and confirmed draining",
      "Runs, hangers and seams inspected while they are empty",
      "Minor sealing where a seam or joint needs it",
      "Anything found that needs more than sealing is reported with photographs before it is done",
    ],
    steps: [
      {
        title: "Clear the gutters",
        body: "Every run cleared by hand of leaves, grit and built-up debris, and the debris carried away rather than flushed into the downspouts or left on the grounds.",
      },
      {
        title: "Flush the downspouts",
        body: "Each downspout flushed through and watched, so what leaves the gutter is confirmed to reach the ground. A clear gutter over a blocked downspout still overflows.",
      },
      {
        title: "Inspect and seal",
        body: "With the runs empty we check the hangers, seams, joints and the fascia behind them for looseness, damage and leaks, and seal the minor faults we find. Anything larger is reported before it is done, not added to the bill.",
      },
      {
        title: "Gutter guard — optional",
        body: "Where it is on this quote, Smart Screen aluminium gutter guard is fitted over the cleared runs. Only appears above when it was actually sold; a guard fitted over an unclean gutter traps the debris underneath it.",
        // Not given a timeline. The other four are a visit; this one depends on
        // how many feet of guard, and inventing a duration here would put a
        // commitment in a contractor's mouth.
      },
      {
        title: "Hand over",
        body: "Guard installation carries a [warranty period] warranty covering [what it covers]. The gutters are left draining and the work area cleared.",
      },
    ],
    mayChange: [
      {
        title: "What the runs look like once they are empty",
        body: "A loose hanger, a split seam or rotten fascia behind the gutter cannot be seen through a full trough. Minor sealing is in the price above; anything structural is reported with photographs and priced separately before any of it is done.",
      },
      {
        title: "How the roof is reached",
        body: "A run that needs staging, a ladder standoff over a conservatory, or a second person for safety takes longer than one reached off a ladder on level ground.",
      },
      // The questions the pricing research kept answering, in the two slots
      // this file already has for them. No `faq` field was added: "what could
      // change this price" and a glossary are the same two questions a FAQ
      // asks, and a third array would be the one nobody keeps current.
      {
        title: "How many downspouts the run actually needs",
        body: "The downspouts on this quote are the ones the house has now. A run that was under-drained before will be under-drained again with new metal on it, so where we think another outlet is needed we say so and price it separately rather than assuming you want one.",
      },
    ],
    glossary: [
      {
        term: "Downspout",
        body: "The vertical pipe carrying water from the gutter to the ground. Clearing the gutter without proving the downspout drains fixes half the problem.",
      },
      {
        term: "Hanger",
        body: "The bracket holding the gutter to the fascia. A loose one lets the run sag, and a sagging run holds water instead of moving it.",
      },
      {
        term: "Fascia",
        body: "The board behind the gutter that the hangers screw into. Gutter work stops where rotten fascia starts, because nothing holds in soft wood — which is why a replacement quote can only be sure of the fascia once the old run is off it.",
      },
      {
        term: "Seamless eavestrough",
        body: "Trough rolled on site from one continuous coil to the exact length of your run, so the only joints are at the corners and the outlets. Sectional gutter is joined every few feet, and every joint is somewhere that can eventually leak.",
      },
      {
        term: "Five-inch and six-inch",
        body: "The width of the trough. Six-inch trough with a larger outlet carries considerably more water than five, which is what a large roof, a steep roof, or a valley emptying into one corner needs.",
      },
      {
        term: "Micro-mesh guard",
        body: "A fine screen that keeps out shingle grit, seed and pine needles as well as leaves. A basic screen keeps out leaves and lets the small debris through, which is the difference the two prices describe.",
      },
      {
        term: "Gutter guard",
        body: "A screen over the gutter that keeps leaves out while letting water through. It reduces cleaning; it does not end it, and it has to go on a clean gutter.",
      },
      {
        term: "Minimum service charge",
        body: "A short run costs almost as much to reach as a long one — the same truck, the same ladders, the same trip. Small visits are therefore charged at a minimum rather than by the foot, and where that applies the quote shows the top-up on its own line instead of quietly thickening the rate.",
      },
    ],
  },
...
export function resolveServiceContent(categoryKey, rawOverride, takeoff, language) {
  // The document's OWN language (Quote.language / Invoice.language, fixed at
  // creation), never the viewer's. That language's catalogue is merged over
  // the English one field by field, so a field a catalogue lacks renders the
  // English sentence rather than nothing.
  //
  // A company's own wording wins over the catalogue — in the document's
  // language when a translation of THAT wording was drafted on save
  // (overrideIn below), otherwise in whichever language they typed it.
  const override = overrideIn(rawOverride, language);
  const catalogue = catalogueFor(language);
  const en = ownProp(CONTENT, categoryKey) || {};
  const base = catalogue ? { ...en, ...(ownProp(catalogue.content, categoryKey) || {}) } : en;
  const generic = genericFor(language);
  ...
  const description =
    cleanString(override?.scopeDescription) ||
    variantDescription(base, takeoff);
...
// ── A company's edited wording, in the document's language ──────────────────
//
// The owner, 2026-09-25: "Do any changes and additions get translated?" They
// did not. A company that rewrote its process steps in Settings › Services
// had them print in the language typed on every quote, whatever the quote's
// language. Since then the save drafts each edited field into the other
// document languages (lib/i18n/autoTranslate.js, model "serviceContent"),
// stored on the override row as translations[lang][field].
//
// The same contract as CompanyTextTranslation: a draft is used only for the
// wording it was drafted from — its hash must match the company's CURRENT
// field — so an edit made a moment ago prints the company's own words until
// the fresh draft lands, never a translation of what they used to say. An
// entry in the language the wording was detected in (`from`) is never used:
// the company's own text is. Pending entries carry no text and are skipped.
...
function overrideIn(override, language) {
  if (!override || typeof override !== "object") return override;
  const lang = String(language || "").trim().toLowerCase().split(/[-_]/)[0];
  const tr = lang && override.translations && typeof override.translations === "object" ? override.translations[lang] : null;
  if (!tr || typeof tr !== "object") return override;
  let out = override;
  for (const [field, key] of OVERRIDE_FIELDS) {
    const entry = tr[field];
    if (!entry || typeof entry !== "object" || entry.pending) continue;
    if (entry.from && entry.from === lang) continue;
    const source = override[field];
    const hasSource = field === "scopeDescription" ? Boolean(cleanString(source)) : Boolean(firstArray(source));
    if (!hasSource) continue;
    if (entry.sourceHash !== serviceContentHash(field, source)) continue;
    const value = entry[key];
    const usable = field === "scopeDescription" ? Boolean(cleanString(value)) : Boolean(firstArray(value));
    if (!usable) continue;
    out = { ...out, [field]: value };
  }
  return out;
...
function finishedEntry(entry) {
  ...
  const parts = [entry.title, entry.term, entry.body, entry.timeline]
    ...
    .join("\n");
  return (
    Boolean(entry.title || entry.term) && !unfilledPlaceholders(parts).length
  );
}
 ...
 */
export function dominantProcessSteps(groups = [], language) {
  const generic = genericFor(language);
  if (!groups.length)
  ...
  return resolveServiceContent(biggest?.categoryKey, biggest?.override, undefined, language).steps;
}
 ...
 */
export function dominantGlossary(groups = [], language) {
  if (!groups.length) return [];
  const biggest = [...groups].sort(
    (a, b) => Number(b.subtotal || 0) - Number(a.subtotal || 0),
  )[0];
  // `language` since 2026-09-25: before it, a French roofing quote printed
  // its glossary in English because this call was never told the language.
  return resolveServiceContent(biggest?.categoryKey, biggest?.override, undefined, language)
    .glossary;
}

/**
 * The raw catalogues, for scripts/check-service-content-languages.mjs only —
 * which walks the English and proves every other language carries every
 * string. Not for rendering: resolveServiceContent is the reader.
 */
export function presetCatalogues() {
  return { en: { content: CONTENT, generic: GENERIC }, ...CATALOGUES };
}
