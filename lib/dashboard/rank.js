...
function conversionMetric(overview) {
  ...
  // ── The comparison, floored on BOTH sides ────────────────────────────────
  //
  // /api/analytics/overview sends `priorConversionRate` honestly — null when
  // last month had no sent quotes, so "up from 0%" off no activity can never
  // appear — and now sends `priorQuotesSent`, the denominator behind it.
  //
  // That second number is what lets the floor apply to the prior as well as to
  // the current month. Without it this side could only check that a prior
  // EXISTED, so "up from 25%" could be a comparison against four quotes of
  // which one was accepted — a rate that a single decision moves by 25 points,
  // printed beside a properly floored current rate as though the two were the
  // same kind of thing. The percent sign made them look alike; the sample sizes
  // did not.
  //
  // An ABSENT prior count fails the floor rather than bypassing it. A payload
  // that carries a rate but not its denominator is one this file cannot judge,
  // and AGENTS.md's rule is that absence of a statement is not a statement.
  // Read before coercion, deliberately. `num()` above is Number()-based, and
  // Number(null) is 0 while Number(undefined) is NaN — so num(null) returns a
  // ZERO, not a null. Routed through it, "last month is unknown" would arrive
  // here indistinguishable from "last month sent none", and the floor would
  // then refuse it for the wrong reason. It happens to refuse either way while
  // RATE_FLOOR is 10; that is an accident of the constant, not a guarantee, and
  // it would silently become a bug the day the floor moved to 0.
  const priorSent =
    overview.priorQuotesSent == null ? null : num(overview.priorQuotesSent);
  const priorBelowFloor = priorSent == null || priorSent < RATE_FLOOR;
  const delta =
    belowFloor || priorBelowFloor || rate == null
      ? null
  ...
  return {
    ...
    floor: RATE_FLOOR,
    belowFloor,
    // Last month's denominator: a number when known, null when the payload did
    // not carry one. Surfaced rather than kept local so that the difference
    // between "no prior month" and "a prior month of zero" can be asserted —
    // a distinction that is invisible from the delta alone, since both refuse.
    priorSample: priorSent,
