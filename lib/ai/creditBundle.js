...
export function bundleAvailability(companyCurrency) {
  ...
  return {
    ok: false,
    reason:
      `AI credit plans are priced in US dollars and your FieldQuo plan bills in ${currency}. ` +
      "Stripe can't run both on one account, so plans aren't available to you yet — one-time top-ups still are.",
  };
}
