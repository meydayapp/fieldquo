 ...
 */
/**
 * Whether this company can start a bundle at all.
 *
 * The bundle is a USD subscription (its economics are USD — lib/ai/
 * imageEconomics.js). Stripe locks a customer to ONE currency across
 * subscriptions, so a company whose FieldQuo plan bills in CAD cannot add a
 * USD subscription to the same customer: Stripe refuses the checkout. The
 * route used to swallow that refusal as "couldn't reach the payment
 * provider" — a sentence that was false and sent the owner looking at Stripe's
 * status page — and the page kept offering a button that could not work.
 *
 * Top-ups are one-time payments and are not locked, which is why they kept
 * working on the same customer while bundles 502'd. Pricing the bundle in the
 * plan's currency is the real fix and is a PRICE decision (what a CAD bundle
 * costs), which is the owner's; until then the button says why it is off.
 */
export function bundleAvailability(companyCurrency) {
  ...
  return {
    ok: false,
    reason:
      `AI credit plans are billed in US dollars — the AI is bought in US dollars, and exchange rates move — ` +
      `and your FieldQuo plan bills in ${currency}. Stripe can't run both on one account, so plans aren't ` +
      "available on a non-USD account. One-time top-ups are.",
  };
}
