...
/**
 * The BEST model — for the calls a company pays for by the token and that
 * act in its name.
 *
 * The owner's words (2026-09-19): "for this we need to be able to use the
 * best model — and again it's part of the paid services based on the tokens
 * that the company has purchased." The AI employee holds a conversation with
 * a stranger and can put a slot on the calendar; the difference between a
 * good and a mediocre model there is a booking nobody wanted. So the employee
 * runs on this, and every tool call that commits or is proposed does too.
 * Drafts and captions elsewhere stay on MODEL.
 *
 * Named by TIER at the call site (`tier: "best"`), never by model id, for the
 * same reason `quality: "writing"` is: swapping the vendor's strongest model
 * is one edit here. The default is the strongest current OpenAI reasoning
 * model; it MUST have a row in lib/ai/usage.js's PRICING, and
 * scripts/check-ai-employee.mjs fails when it does not, because a missing
 * rate would meter the company's most expensive calls at the fallback.
 */
const BEST_MODEL = process.env.OPENAI_MODEL_BEST || "gpt-5.5";

export const AI_MODEL = MODEL;
export const AI_WRITING_MODEL = WRITING_MODEL;
export const AI_BEST_MODEL = BEST_MODEL;

/** The two tiers a caller may name. Anything else is `standard`. */
export const AI_TIERS = Object.freeze(["standard", "best"]);

/** The model a tier resolves to. */
export function modelForTier(tier) {
  return tier === "best" ? BEST_MODEL : MODEL;
}

/**
 * The tier a recorded model id belongs to, for the platform cost split.
 *
 * By id rather than by a stored column: AiUsage rows already carry `model`,
 * and a tier written at record time would disagree with this function the day
 * OPENAI_MODEL_BEST changes. "best" is the best model's id; everything else —
 * the mini, the writing model, an old id — is standard.
 */
export function tierForModel(model) {
  return model === BEST_MODEL ? "best" : "standard";
}
 ...
 */
function userContent(prompt, images, maxImages, detail = "low", files = []) {
  ...
  // A document the vendor reads itself — a PDF receipt, every page. Unlike an
  // image this cannot be a URL on chat completions: the vendor takes the bytes
  // inline (`file_data`), so the CALLER fetches them (lib/receipts/pdf.js,
  // which also refuses anything not on our own Cloudinary) and hands this
  // function base64. Only PDFs, because that is the one document type the
  // vendor renders page by page; anything else is dropped rather than sent.
  const docs = (Array.isArray(files) ? files : [])
    .filter((f) => f && f.mimeType === "application/pdf" && typeof f.base64 === "string" && f.base64)
    .slice(0, 1);

  if (!urls.length && !docs.length) return prompt;
  ...
  return [
    ...
    ...docs.map((f) => ({
      type: "file",
      file: {
        filename: String(f.filename || "receipt.pdf").slice(0, 120),
...
export async function complete({
  ...
  quality,
  // "best" opts into BEST_MODEL — see its header. `quality` wins when both
  // are given, because a literal model id is the more specific request.
  tier = "standard",
  reasoningEffort = "low",
  ...
  imageDetail = "low",
  // At most one PDF, as { filename, mimeType: "application/pdf", base64 } —
  // see userContent. Empty for every caller that predates it, so their
  ...
  const content = userContent(prompt, images, maxImages, imageDetail, files);
  // A PDF counts as a picture for metering: the vendor renders each page to an
  // image and bills it as one, so a PDF read belongs in the `_photos` feature
  // row beside the photo reads it is compared with.
  const imagesSent = Array.isArray(content)
    ? content.filter((c) => c.type === "image_url" || c.type === "file").length
    : 0;
...
export async function runToolLoop({
  ...
  maxImages = 4,
  imageDetail = "low",
  // "standard" (MODEL) or "best" (BEST_MODEL). The copilot passes nothing and
  // keeps the mini; the AI employee passes "best".
  tier = "standard",
  reasoningEffort = "low",
}) {
  ...
  for (let round = 0; round < maxRounds; round++) {
    ...
    if (calls.length === 0) {
      if (onUsage) {
        await onUsage({ model, promptTokens, completionTokens, imageCount: imagesSent });
      }
...
// ── Speech to text ────────────────────────────────────────────────────────
//
// For lib/sales/calls/transcribe.js and nothing else yet. Whisper rather than
// the newer transcribe models because it is the one that returns SEGMENTS with
// timestamps (`verbose_json`), and a sales-call transcript with no times cannot
// be merged across the rep's track and the contractor's track. $0.006 per
// audio minute, billed per track.

const TRANSCRIBE_MODEL = process.env.OPENAI_TRANSCRIBE_MODEL || "whisper-1";
export const AI_TRANSCRIBE_MODEL = TRANSCRIBE_MODEL;

/**
 * One audio file in, timestamped segments out.
 *
 * @param {{ audio: Buffer, filename?: string, language?: string|null,
 *           prompt?: string|null, onUsage?: Function }} args
 * @returns {Promise<{ ok: true, text: string, segments: {start:number,end:number,text:string}[], seconds: number|null }
 *                 | { ok: false, reason: string, message: string }>}
 */
export async function transcribeAudio({ audio, filename = "audio.wav", language = null, prompt = null, onUsage }) {
  if (!isAiConfigured()) return { ok: false, reason: AI_FAILURE.UNCONFIGURED, message: "OPENAI_API_KEY is not set" };
  if (!Buffer.isBuffer(audio) || audio.length === 0) {
    return { ok: false, reason: AI_FAILURE.EMPTY, message: "No audio to transcribe" };
  }
  try {
    const res = await client.audio.transcriptions.create({
      ...
      // A two-letter hint stops the model transcribing a French call as
      // English phonetics. Null lets it detect.
      ...(language ? { language } : {}),
      ...(prompt ? { prompt } : {}),
    });
    const segments = (Array.isArray(res?.segments) ? res.segments : [])
      ...
      .filter((s) => s.text);
    const seconds = Number.isFinite(Number(res?.duration)) ? Number(res.duration) : null;
    if (onUsage) await onUsage({ model: TRANSCRIBE_MODEL, seconds });
    return { ok: true, text: typeof res?.text === "string" ? res.text.trim() : "", segments, seconds };
  } catch (err) {
    console.error("[ai] transcription failed:", err?.message || err);
    return { ok: false, reason: AI_FAILURE.VENDOR_ERROR, message: err?.message || "transcription failed" };
  }
}
