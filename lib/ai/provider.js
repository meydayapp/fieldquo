 ...
 *
 * `detail: "low"` is not a quality setting to tune later. A site photo is being
 * read for what is IN it — water damage, a peeling door, an occupied kitchen —
 * not for fine text, and low detail costs a flat, predictable number of tokens
 * per image instead of scaling with resolution. An estimator's phone camera
 * would otherwise make the price of a review depend on which phone they own.
 */
function userContent(prompt, images, maxImages) {
...
export async function complete({
  ...
  const content = userContent(prompt, images, maxImages);
