...
const TOOL_ACCESS = {
  ...
  //
  // ── It is the COMPANY's calendar, not the caller's own ──────────────────
  //
  // This comment used to say "a Worker's own week is the least restricted
  // thing in the product", which was a claim about a filter that does not
  // exist: the query below is `job: { companyId }` with no assignee condition
  // at all, so it returns every visit in the company to anybody holding
  // jobs:view_only.
  //
  // That is why the level stayed at view_only when Crew moved to jobs:none —
  // Crew loses this tool rather than being handed the whole company's
  // schedule through it. Giving them back their OWN week means adding the
  // assignee filter (JobVisit.assignedToId), which is the same scoping work
  // the jobs list needs and is deliberately not guessed at here.
  getUpcomingWork: {
