...
export function buildSystemPrompt(definitions) {
  ...
  return [HEADER, whatYouDo, REFUSAL, CRISIS_RULE, howYouAnswer, DRAFTING].join("\n\n");
}
 ...
 */
export async function askCopilot({ companyId, member, messages, language, onUsage }) {
  ...
  const { text, messages: conversation } = await runToolLoop({
    ...
    system: withLanguage(buildSystemPrompt(definitions), language),
    messages,
    tools: definitions,
    onUsage,
