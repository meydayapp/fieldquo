 ...
 *
 * Since 2026-09-25 the same ONE call also returns a per-photo trade reading
 * and trade evidence (lib/ai/deepReadEvidence.js). That adds a larger schema
 * in (~2.3k chars base, ~1.2k per trade family, three families at most) and
 * one short line per photo plus one evidence object out — roughly +$0.01 at
 * the eight-photo cap, ~$0.145 against the unchanged 25¢. The owner's
 * condition was one call and the same meter; both hold.
 */
...
/**
 * One AI-built material list for a job (lib/materials/build.js).
 *
 * Text only, on the standard model: the approved lines, the takeoff numbers,
 * the recipe rates and the stock list go in (~4k tokens), a grouped list of
 * up to eighty lines with a reason each comes out (~2k tokens). At the mini
 * model's rates that is ~$0.005 a run. Priced flat at 10¢ — the same 2x
 * margin rule as the two above would say 1¢, but a price under a dime is a
 * price nobody can reason about on a statement, and the mockup's 25¢ was
 * marked a placeholder. THIS NUMBER IS THE OWNER'S TO SET: the report that
 * shipped it names it, and changing it is one edit here — the banner over
 * the list, the spend gate and the credit page all read this constant.
 */
export const MATERIAL_LIST_CENTS = 10;
...
// ── Chat, charged by the call ──────────────────────────────────────────────
//
// The owner, 2026-09-25: the AI employee — the company's own chat bot on its
// website, Meta and SMS — is "theirs to pay … from their paid AI
// subscription". Unlike a picture it has no flat price: a reply is one to four
// model rounds on the best model, and its cost moves with the conversation. So
// it is charged from its own token counts, at the SAME rule as everything
// above: the ~50% gross margin approved on 2026-08-30, which this file already
// states as "the price is 2x the cost". One multiplier, named once, so a
// picture and a sentence cannot drift onto two different margins.
export const PAY_AS_YOU_GO_MULTIPLIER = 2;

/**
 * Cents the AI credit is charged for one metered chat call, from its vendor
 * cost in micros (lib/ai/usage.js's estimateCostMicros).
 *
 * Rounded UP to the cent — the direction AiEmployeeReply.costCents and
 * typicalConversationCostCents already round — so the balance never reads
 * higher than what was spent. The consequence, stated rather than hidden: a
 * call that costs a small fraction of a cent (the front desk's four-label
 * triage on the mini model, ~0.03¢) is charged 1¢. Zero for anything that is
 * not a positive finite number: a NaN cost must never become a NaN debit.
 */
export function chatChargeCents(costMicros) {
  const micros = Number(costMicros);
  if (!Number.isFinite(micros) || micros <= 0) return 0;
  return Math.ceil((micros * PAY_AS_YOU_GO_MULTIPLIER) / 10_000);
}
...
export const CREDIT_COST = {
  ...
  material_list: MATERIAL_LIST_CENTS * CREDITS_PER_CENT,
};
