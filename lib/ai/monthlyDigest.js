...
export async function generateMonthlyDigest({
  ...
  const metricsForPrompt = {
    ...
    // Present ONLY when a row in another currency was converted into that
    // figure (lib/analytics/spendCurrency.js) — the model is told to use
    // only given numbers, and a converted total handed over bare would be
    // read as exact. An absent key can't become a false claim.
    ...(marketing.totals.approximate
      ? {
          marketingSpendIsApproximate: true,
          marketingSpendConvertedFrom: marketing.totals.currencyConversions.map(
            (c) => `${c.currency} ${c.amount} at ${c.rate} (rate ${c.rateAgeDays} days old)`,
          ),
        }
      : {}),
