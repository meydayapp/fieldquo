    ...
    include: {
      ...
      quote: {
        select: {
          ...
          total: true,
          acceptedTotal: true,
          // `category.key`, `takeoff` and `intakeValues` are for the paid deep
          // read: which trade the photos should show, which trade evidence to
          // ask for, and the measured figures its estimates sit beside
          // (lib/ai/deepReadView.js). The free review reads label and lines.
          scopeGroups: {
            select: {
              label: true,
              lineItems: true,
