...
async function writingPass({
  ...
  const payload = {
    ...
    //
    // That paragraph is a WORSE version of one already on the quote. The
    // document has carried a per-trade scope description and a "what's
    // included" list from lib/documents/serviceContent.js since it was
    // written; the model had never been shown either, so the only thing it
    // could find to improve was the six-word name at the top of them.
    //
    // Sent per service rather than as one flat list, because that is how the
    // client reads it: a card per trade, with its scope, its inclusions and
    // its lines together. A model asked to judge wording off a list of names
    // is being asked a different question from the one that matters.
    services: (quote.scopeGroups || []).map((g) => {
  ...
  // Photos only. `clientPhotos` also carries videos and documents — a PDF
  // handed to a vision model is a wasted image slot and a confusing answer.
  const photos = (Array.isArray(quote.clientPhotos) ? quote.clientPhotos : [])
