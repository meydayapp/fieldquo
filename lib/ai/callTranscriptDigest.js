...
You return a JSON object: "calls", a list of { "callIndex", "notes" }.
...
/**
 * The shape asked of the vendor, enforced there with `strict: true`.
 *
 * ── What this schema deliberately does NOT contain ────────────────────────
 *
 * A number that means anything. `callIndex` is an INDEX — a pointer back into
 * the array we sent — not a measurement, and it is range-checked below against
 * `candidates.length` anyway. There is no "confidence", no "sentiment score",
 * no "how many callers mentioned price". Every count this feature reports is
 * an array length computed in assembleInsights(); see its header, and see
 * lib/ai/jsonSchema.js's closing note on why a schema is the easiest place to
 * quietly undo that. Adding one numeric field here would turn a model's guess
 * into a figure an owner reads over coffee as a fact.
 *
 * ── And what it CANNOT contain ────────────────────────────────────────────
 *
 * Every real limit this feature has. MAX_NOTES_PER_CALL is `maxItems`,
 * MAX_NOTE_CHARS is `maxLength`, "not blank" is `minLength` — all three are
 * on the strict subset's unsupported list, so none of them can be pushed to
 * the vendor. They stay in parseModelOutput() below, along with the
 * looksLikeInstruction gate, which no schema in any format could express.
 */
const DIGEST_SCHEMA = {
  type: "object",
  properties: {
    calls: {
      ...
      items: {
        type: "object",
        properties: {
          callIndex: { type: "integer", description: "The number given for that call." },
          notes: {
            type: "array",
            description: "Things actually said. Empty is the common, correct answer.",
            items: { type: "string" },
          },
        },
 ...
 *
 * Returns `{ notesByQuoteId, droppedCount }` — never throws; a malformed shape
 * is treated exactly like "the model had nothing to say", which is the same
 * posture lib/ai/quoteReview.js takes.
 *
 * @param parsed  the object provider.js validated against DIGEST_SCHEMA. Every
 *   `Array.isArray` and `typeof` below is KEPT rather than trimmed on the
 *   strength of that guarantee: this function is also called by
 *   scripts/check-digest-transcripts.mjs with hand-written hostile input, it
 *   is the last thing between a phone recording and a page an owner reads,
 *   and the vendor's promise is one vendor's promise. Defence that costs a
 *   `typeof` is not defence worth deleting.
 */
export function parseModelOutput(parsed, candidates) {
...
export async function buildCallInsights({
  ...
  const result = await completeFn({
    ...
    schemaName: "call_transcript_digest",
  });

  // MODEL_EMPTY still covers every unhappy path, because from the owner's side
  // of the screen they are the same sentence: nothing was read. The difference
  // is that provider.js now logs which one it was — before this, a retired
  // model ID, a rate limit and a model with genuinely nothing to say all
  // arrived here as the same falsy value and left as the same REASON.
  //
  // Not split into more REASONS deliberately. The vocabulary is closed so the
  // page can render a real sentence for each, and "the vendor refused our
  // schema" is not a sentence a contractor can act on — it is a sentence for
  // whoever reads the logs, which is where it now goes.
  if (!result.ok) {
    return assembleInsights({
      totalCandidates,
      read,
  ...
  const { notesByQuoteId, droppedCount } = parseModelOutput(result.data, read);
