...
{ "notes": ["<something an estimator may have missed>"] }
 ...
 *          failed, or its answer couldn't be parsed. The caller's job on null
...
export async function runVisionPass({ quote, onUsage }) {
  ...
  try {
    ...
    const notes = Array.isArray(parsed.notes)
      ? parsed.notes.map((n) => (typeof n === "string" ? n.trim() : "")).filter(Boolean)
      : [];
    return { notes, photosRead: photos.length };
  } catch {
    console.error("[visionPass] model returned unparseable JSON");
    return null;
  }
}
