...
// The injection guard now lives beside the fence in lib/voice/transcript.js:
// this file and lib/ai/callQuoteDraft.js both need it, neither owns it, and a
// second copy would be the one that stopped matching the day a pattern was
// added. Re-exported because scripts/check-voice-recovery.mjs hammers it here,
// where the lesson that produced it is written down.
...
export async function recoverLeadFromCall({
  ...
  //
  // The live receptionist rule (lib/ai/crisisRule.js) now says the emergency
  // line once and CONTINUES the call — takes the address, books the time,
  // finishes what it was doing. So a transcript that trips mentionsCrisis()
  // today is not, the way it used to be, a transcript with nothing usable in
  // it: it is very likely a real caller who got the 911 line and then kept
  // talking. Refusing to recover a lead from it would silently throw away
  // exactly the thing this file exists to save — see lib/ai/crisisRule.js's
  // "why these two no longer refuse" for the full reasoning, this file's half
  // of the same decision as lib/ai/callQuoteDraft.js.
  //
  // needsReview is the SAME flag save-caller sets for a property emergency
  // (app/api/voice/tools/[tool]/route.js) and the SAME queue the receptionist
  // screen already shows flagged calls in (app/app/receptionist/page.js) — not
  // a new mechanism. The transcript was never deleted and never will be; what
  // this adds is a human being pointed at the call rather than finding out by
  // accident, on TOP of the lead still being recovered normally below.
  if (mentionsCrisis(said)) {
    await prisma.voiceCall
      .update({ where: { id: call.id }, data: { needsReview: true } })
      .catch((err) => console.error("[voice/recover] needsReview not set:", err?.message));
  ...
  const lead = await createLead({
    ...
    // The address the recovery already evidence-checked against the caller's
    // own words (validateRecoveredLead keeps it only when they gave the address
    // of the JOB). It went into the message prose and stopped there, so a
    // recovered call converted into a client with no address while the transcript
    // it came from had one in it. Same shape convertLead reads — no city or
    // province: a spoken street is not a parsed jurisdiction.
    intake: buildLeadIntake({ address: found.address }),
