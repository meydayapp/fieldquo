...
export const RECOVERY_REASONS = {
  ...
  NOT_FOUND: "not_found",
  // The caller described danger to themselves. See the matching reason and
  // comment in lib/ai/callQuoteDraft.js's DRAFT_REASONS — same rule, same
  // reasoning, this file's half of it. A call like this earns a human's
  // attention, not a row in the leads list scored on how ready it is to buy.
...
export async function recoverLeadFromCall({
  ...
  // ── A crisis call is not raw material for a lead ──────────────────────────
  //
  // This whole file exists because a webhook outage threw calls away and this
  // is how they get their lead back — but "recovered" means "scored and put in
  // front of a salesperson", and a caller who described being a danger to
  // themselves is not a missed sale. Checked before the model is asked
  // anything; see mentionsCrisis() in lib/ai/crisisRule.js.
  //
  ...
  // a new mechanism, because silently doing nothing with this call is its own
  // failure. The transcript was never deleted and never will be; what changes
  // is that nobody automated turns it into a sales artifact, and a human sees
  // it was flagged rather than finding out by accident.
  if (mentionsCrisis(said)) {
    ...
    return { ok: false, reason: RECOVERY_REASONS.CRISIS_DETECTED };
  }
