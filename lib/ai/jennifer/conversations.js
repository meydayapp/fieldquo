...
export async function markEscalated({ conversationId, reason }) {
  const row = await db.jenniferConversation.update({
    where: { id: conversationId },
    data: { status: "escalated", escalationReason: reason || "handed off" },
  });
  // The push copy — a visitor Jennifer could not help is waiting for a
  // human, and the queue at /platform/jennifer is not polled. Every active
  // admin and superadmin; fire-and-forget (lib/notify/push.js).
  void pushToPlatformRoles({
    roles: ["superadmin", "admin"],
    payload: {
      title: "Jennifer escalated a conversation",
      body: reason ? `Reason: ${String(reason).replace(/_/g, " ")}` : "Handed off to a human.",
      tag: `jennifer-escalation:${conversationId}`,
      url: "/platform/jennifer",
    },
  });
  return row;
}
