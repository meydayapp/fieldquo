...
//
// Every account-status tool below is written from a positive list of fields —
...
//
// ── The company's own numbers: the copilot's tools, not a second set ────────
//
// The owner asked Jennifer "how many quotes are pending" and "how much money
// is owed" and was pointed at a page both times (2026-09-24). The numbers
// exist one file over — lib/ai/copilotTools.js computes them for FieldQuo AI
// — so company mode now hands Jennifer THAT list, filtered by the same grid
// (copilotToolsFor) and wrapped in the same fence as the account tools.
// Imported rather than re-implemented: two copies of "what counts as owed"
// is how the two assistants come to disagree about the same invoice.
//
// This widens constraint 2 on purpose, and only this far: those tools return
// a client's NAME (through redactClient, which is the one field every
// clientsProperties level permits) and a quote's own notes. They never
// select an email, a phone, an address beyond the job site, a photo or a
// transcript — see the selects in copilotTools.js — and every result still
// passes through fenceCompanyData before the model reads it.
//
...
import { copilotToolsFor } from "@/lib/ai/copilotTools";
...
const ACCOUNT_TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    description:
      "This company's PAID AI wallet balance status (image generation, the deep photo " +
      "read, the AI material list, and — since 2026-09-25 — every AI employee reply on the " +
      "website chat, Facebook/Instagram/WhatsApp and SMS). 'zero' (or 'low', below the cost of " +
      "one reply) explains an AI employee " +
      "that has gone quiet and hands every conversation to a person. This is a SEPARATE " +
      "balance from voice/phone credit — see getReceptionistStatus for that one.",
    input_schema: { type: "object", properties: {} },
  },
  {
...
function companyTools({ companyId, member, dataMember }) {
  ...
  // The company's own numbers — see this file's header. `dataMember` is the
  // enforceable row (role + grid + userId) the route loaded, or null when it
  // couldn't; copilotToolsFor(null) yields nothing, which is the direction to
  // fail in: a support chat that can't identify the asker's access level
  // answers support questions and no data questions.
  const data = copilotToolsFor(dataMember || null);
  for (const def of data.definitions) {
    definitions.push(def);
    const impl = data.implementations[def.name];
    implementations[def.name] = async (args) =>
      // companyId LAST, so the model's own is overwritten, exactly as below;
      // copilotToolsFor binds `member` the same way one layer in.
      fenceCompanyData(await impl({ ...args, companyId }));
  }
 ...
 * @param member      { role } — required when mode === "company"; the role
 *                    decides which account tools exist
 * @param dataMember  the row from loadEnforceableMember ({ role, permissions,
 *                    userId }) or null — decides which of the copilot's data
 *                    tools exist, through copilotToolsFor. Null means none.
 */
export function jenniferToolsFor({ mode, companyId, member, dataMember = null } = {}) {
  if (mode === "company") return companyTools({ companyId, member, dataMember });
