...
// ── Why this file is NOT on complete()'s schema mode ───────────────────────
//
// Everything else that asked a model for JSON moved to
// `complete({ schema })` (see lib/ai/jsonSchema.js). This one deliberately
// did not, and the reason is structural rather than effort.
//
// The reply's shape is COMPANY-DEFINED: the intake fields below come from
// that company's own catalogue, so a schema for this call would have to be
// built per request from their field list. Strict structured outputs have no
// optional properties — every declared property must appear in `required` —
// so such a schema would compel the model to emit a value for every intake
// field on every call, including the ones the caller never mentioned. That is
// the exact opposite of what coerceIntakeValue() exists to do: it returns
// `{ ok: false }` rather than a default, because there is no sensible default
// for "how many doors", and NOT KNOWING has to survive as not knowing all the
// way onto a quote somebody prices.
//
// Expressing each field as nullable would technically work and would swap one
// guarantee for a worse one — a model that has to write `null` explicitly for
// forty fields is a model being asked forty leading questions. The
// hand-coercion below already handles absence correctly and is executed
// against hostile input by scripts/check-call-draft.mjs. It stays.
//
// This is also the file the receipt project audited in
// docs/construction/AUDIT-port-candidates.md gets wrong in the other
// direction: it passes totals through an LLM twice. Nothing here reads a price
// out of the model — the draft names services and answers intake questions,
// and lib/quotes/ prices it from the company's own rows afterwards.
...
export async function draftQuoteFromCall({
  ...
  // ── A crisis mention flags the call for a person. It does not stop this ────
  //
  // lib/voice/prompt.js's crisis rule is what actually protects the caller, in
  // the moment, on the call — and that rule now says the line once and
  // CONTINUES, rather than stopping the call outright the way the old rule
  // did. So a transcript that trips mentionsCrisis() today is likely to have a
  // real quote sitting right after the crisis line, not nothing: the caller
  // got the 911 line and kept describing the job. Refusing to draft from it
  // would silently drop exactly the lead this file exists to capture. Full
  // reasoning in lib/ai/crisisRule.js, "why these two no longer refuse" — this
  // is the same decision lib/ai/callLeadRecovery.js makes for its half.
  //
  // What this DOES still do: flag the call so a human sees it, the same
  // needsReview flag save-caller sets for a property emergency, before falling
  // through to draft normally.
  if (mentionsCrisis(said)) {
    await db.voiceCall
      .updateMany({ where: { id: call.id, companyId }, data: { needsReview: true } })
      .catch(() => {});
    // Falls through on purpose — see the comment above.
  }
