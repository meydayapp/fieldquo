...
export async function recordPlatformAiUsage(
  db,
  { area, model, promptTokens = 0, completionTokens = 0, prospectId = null, campaignId = null, salesRepId = null, ref = null, meta = null } = {},
) {
  ...
  const data = {
    ...
    salesRepId,
    ...(ref ? { ref } : {}),
    // What the spend was for beyond the area — for call_script, which
    // language and whether a rep's switch asked for it. Only a plain object
    // is stored; a string or a number here is a caller's mistake and is
    // dropped rather than written as a JSON scalar the reader would trip on.
