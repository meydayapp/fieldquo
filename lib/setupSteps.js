...
export const SETUP_STEPS = [
  ...
  },
  {
    ...
    minutes: [3, 10],
    // 2026-09-25, the owner: "it is loaded by default, so if it is loaded by
    // default it should say so." A company whose signup seeded services
    ...
    // you (N)"; one with nothing seeded keeps the plain title, because there
    // is nothing "we added" to review. The count sits in brackets rather than
    // in the sentence so no language has to decline "service" for it.
    titleWhen: (s) => {
    ...
    // Only for a company whose trade FieldQuo cannot already list: a trade
    // with no seed file, fewer than QUOTE_COVERAGE_MIN seeded services
    // installed, or no catalogue trade at all. A plumber who signed up and
    // got 101 plumbing rows has nothing to confirm that "Add missing
    // services" does not already do. An absent signal APPLIES.
    appliesWhen: (s) => quoteCoverageThin(s.quoteCoverage),
  },
  {
    key: "overhead",
