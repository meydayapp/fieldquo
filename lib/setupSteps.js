// lib/setupSteps.js
//
// The dashboard's "Additional set-up steps" — the fifteen things a company
...
//
// A third way off the card, for one row only: a step that does not APPLY.
// The company has said it works alone, or the plan has no seat left to put
// anybody in — a step nobody can finish is worse than no step, and a step
...
/**
 * The team row first, then the ten in the order the owner listed them. `href`
 ...
 *
 * `appliesWhen` is optional and true by default: only the team row has a
 * reason not to apply (see the header). A step that does not apply is not on
