...
// a customer.
//
// `on_the_way` was the one hole. It shipped with `labelKey: null` and English
// only, because no catalogue key for it existed anywhere in the product — the
// honest thing to do rather than borrow one that means something else ("In
// progress" is not "On the way", and a crew member reading a badge is entitled
// to the difference). `app.status.onTheWay` landed in all nine languages and
// it is wired up; check:appointment-status now asserts there is NO hole at
// all, so the next status added without a key fails the build rather than
// reaching a French office in English.
//
// The English in `en` is not dead weight either way: it is the fallback t()
// takes when a language is missing the key, and it is what the check scripts
// and any non-React caller read.
...
export const APPOINTMENT_STATUS_PRESENTATION = {
  ...
  // ── JobVisit ─────────────────────────────────────────────────────────────
  // The same key lib/jobs/visitStatus.js reads, on purpose: the job page's
  // visit badge and this calendar's row show the same visit, and one fact must
  // not have two vocabularies.
  on_the_way: { tone: "booked", labelKey: "app.status.onTheWay", en: "On the way" },
...
/**
 * The moves the OFFICE can make on an Appointment, from a given status.
 *
 * Same shape as visitActions in lib/jobs/visitStatus.js — `{ to, labelKey,
 * label, tone }` — so one control (app/components/schedule/EntryActions.js)
 * renders both kinds of row. The vocabulary is AppointmentStatus's, not
 * JobVisit's: there is no "on my way" here because an appointment has no crew
 * route texting from a van, and `needs_supervisor` is left for the assignee
 * picker to resolve (assigning a supervisor is what clears it) rather than
 * offered as a button that would claim to.
 *
 * Cancelling is a status, never a delete, and reopening a cancelled row goes
 * back to `scheduled` — the same two decisions the visit list already made.
 */
export function appointmentActions(status) {
  switch (status) {
    case "completed":
      return [{ to: "scheduled", labelKey: "app.visitAction.reopen", label: "Reopen", tone: "quiet", office: true }];
    case "cancelled":
      return [{ to: "scheduled", labelKey: "app.visitAction.putBackOn", label: "Put it back on", tone: "quiet", office: true }];
    case "scheduled":
    case "needs_supervisor":
      return [
        { to: "completed", labelKey: "app.visitAction.markComplete", label: "Mark complete", tone: "quiet", office: true },
        { to: "cancelled", labelKey: "app.visitAction.cancelVisit", label: "Cancel visit", tone: "quiet", office: true, cancels: true },
      ];
    default:
      // A Booking-shaped row (confirmed, pending_payment) or an unknown value:
      // this list cannot act on it, and offering a button that would 404 on
      // /api/appointments/[id] is the dead control the calendar's own comment
      // warns about.
      return [];
  }
}
