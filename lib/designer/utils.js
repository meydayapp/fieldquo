 ...
 *
 * `filename` is optional and additive — every existing caller (the toolbar's
 * savePng/saveJpg/saveSvg/saveJson) omits it and keeps getting a random name,
 * because a single export has nothing more useful to be called. The campaign
 * editor's "download all" is the one caller that passes it: five files all
 * named by crypto.randomUUID() is exactly the "design-1.png through
 * design-5.png nobody can use" failure lib/marketing/ratios.js's own header
 * describes — assetFilename() from that module is what names them instead.
 */
export function downloadFile(file, type, filename) {
  ...
  anchorElement.download = filename || `${crypto.randomUUID()}.${type}`;
