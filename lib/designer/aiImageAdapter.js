...
export const AI_IMAGE_VENDOR_READY = false;
 ...
 * }>}
 */
export async function statusForCompany(companyId) {
 ...
 */
export async function requestAiImage({ companyId, action, payload, note }) {
  const featureAvailable = await featureAllowsSpend(companyId, "marketing_designer");
  if (!featureAvailable) {
  ...
  try {
    // TODO(image-adapter-seam): call the real vendor once lib/ai/images.js
    // lands, e.g.:
    //   const { url } = action === "generate"
    //     ? await generateImage(payload)
    //     : await removeBackground(payload);
    //   return { ok: true, url };
    throw new Error("AI image vendor is not wired yet");
  } catch (err) {
