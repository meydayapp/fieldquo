 ...
 *   |"stroke-width"|"font"|"opacity"|"filter"|"settings"} ActiveTool
 *
 * Which single sidebar panel is open. "select" means none — the default,
 * inert state. Unlike the source clone, there is no "templates" or "ai" tool:
 * both sidebars were dropped (see app/components/designer/README below) along
 * with the "remove-bg" tool, which depended on the same AI backend as "ai"
 * and would otherwise have been a button that opened a panel with nothing
 * behind it.
 */
