 ...
 *
 * `scopes` defaults to the calendar set; the Business Profile connection
 * (lib/reviews/googleBusiness/client.js) passes its own through the SAME
 * OAuth client — one Cloud project, one client id, one verification queue,
 * a second scope rather than a second app.
 */
export function buildGoogleAuthorizeUrl({ clientId = googleOAuthClientId(), redirectUri, state, scopes = GOOGLE_CALENDAR_SCOPES }) {
