 ...
 */
// TODO(booking-modes): use the mode label helper from lib/booking once it lands
function whatIs(entry, copy) {
  if (entry.kind === "visit") {
    return entry.title ? `${copy.jobVisit}: ${entry.title}` : copy.jobVisit;
  }
  const mode = entry.booking?.mode || null;
  if (mode === "call") return copy.phoneCall;
  if (mode === "video") return copy.videoCall;
  if (mode === "visit") return copy.siteVisit;
...
export function feedEventsFor({ entries, full, company, origin, now = new Date() }) {
  const copy = feedCopy(company?.defaultLanguage);
