...
export function selectInvoiceBanners({
  ...
  // ── Entered as a past job ─────────────────────────────────────────────────
  //
  // Paid before it was typed in, and no client was ever written to about it.
  // Whatever the columns below say — a $0 hand-typed amount, a due date
  // somebody added later — this document must never offer to be sent or
  // chased: an email about a 2024 job would be the first this client ever got
  // from the software. The paid banner still shows, from the same money rule
  // as everything else; only the two client-facing actions are withheld.
  ...
  // ── A bank debit on its way, or bounced ─────────────────────────────────
  //
  // Pre-authorized debit / ACH completes Checkout unpaid and clears in 3–5
  // business days (lib/stripe/settleCheckoutSession.js writes these
  // columns). Without this the invoice reads "overdue" or "balance owing"
  // for a client who paid four days ago, and a bounced debit reads as a
  // client who never tried. Pending sits ABOVE the balance banners; a
  // failure carries Stripe's own reason.
  if (invoice.pendingPaymentAt && !invoice.pendingPaymentFailedAt && money.due > PAID_EPSILON) {
    banners.push({
      id: "bankPending",
      tone: "info",
      action: null,
      data: { method: invoice.pendingPaymentMethod || null, since: invoice.pendingPaymentAt },
    });
  } else if (invoice.pendingPaymentFailedAt && money.due > PAID_EPSILON) {
    banners.push({
      id: "bankFailed",
      tone: "warning",
      action: null,
      data: { method: invoice.pendingPaymentMethod || null, reason: invoice.pendingPaymentFailure || null },
    });
  }
  ...
  if (invoice.status !== "disputed" && money.settled && money.paid > PAID_EPSILON) {
    ...
    if (daysLate !== null && daysLate > 0 && !historical) {
      banners.push({
        id: "overdue",
        tone: "critical",
        action: "chase",
    ...
    if (money.partiallyPaid) {
      banners.push({
        ...
        action: isDraft || historical ? null : "chase",
  ...
  if (!money.settled && !historical && !String(invoice.client?.email || "").trim()) {
    banners.push({
      id: "noClientEmail",
      tone: "warning",
