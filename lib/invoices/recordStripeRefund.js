// lib/invoices/recordStripeRefund.js
//
// charge.refunded: money that was collected has (partly or fully) gone back
// out. Neither Stripe webhook handled this before — a refund issued straight
// from the contractor's own Stripe Express dashboard (or any refund not
// raised through FieldQuo's own visit-cancel-refund flow, which already
// tracks its own booking fee separately via Booking.feeRefundedAt) left
// Invoice.status reading "paid" forever, after the money had already left.
//
// ── Idempotency ──────────────────────────────────────────────────────────
//
// The natural key here is different from recordStripePayment.js's. Stripe
// delivers charge.refunded again for EVERY additional partial refund on the
// same charge, not just as a retry — and the Charge object's own
// `amount_refunded` is already the CUMULATIVE total refunded on that charge.
// Writing it as an absolute value (never an increment) is what makes a
// duplicate delivery a genuine no-op instead of double-deducting: replaying
// the identical event writes the identical number twice.
//
// ── Scope ────────────────────────────────────────────────────────────────
//
// Only refunds that land on an Invoice Payment are handled here — the CONNECT
// side, a homeowner paying a contractor. A booking visit fee has no Payment row
// at all (settleBookingFee.js records it straight onto
// Booking.feeStripePaymentIntentId/feePaidCents) — that one is still out of
// scope; see docs/MONEY-FIXES.md for why.
//
// A charge with no matching Payment is NOT automatically "not ours" any more,
// and the sentence that used to say so was hiding a real bug. A FieldQuo
// SUBSCRIPTION invoice has no Payment row either, so a contractor charging back
// their own subscription landed on this decline and vanished. That case now has
// its own recognition — lib/billing/subscriptionChargeEvent.js — which
// lib/stripe/settleChargeEvent.js offers the event to AFTER this function
// declines it. Nothing about this function changed; the decline simply stopped
// being the end of the road.
//
// A charge neither path claims (an AI/voice top-up, a paid migration) is still
// genuinely not ours, the same "not one of ours" contract
// settleCheckoutSession.js uses for checkout sessions.

import { computeInvoiceState } from "@/lib/invoices/computeInvoiceState";
import { latestInFamily, familyPayments } from "@/lib/invoices/family";

/**
 * @param db      the Prisma client (or an interactive transaction client)
 * @param charge  a Stripe Charge object — event.data.object for charge.refunded
 */
export async function recordStripeRefund(db, charge) {
  const paymentIntentId =
    typeof charge?.payment_intent === "string"
      ? charge.payment_intent
      : charge?.payment_intent?.id || null;
  if (!paymentIntentId) return { recorded: false, reason: "no_payment_intent" };

  const payment = await db.payment.findFirst({
    where: { stripePaymentIntentId: paymentIntentId },
    select: { id: true, invoiceId: true, amount: true, refundedAmount: true, disputeStatus: true },
  });
  if (!payment) return { recorded: false, reason: "no_matching_payment" };

  // Stripe's cumulative amount_refunded includes refunds FieldQuo itself
  // issued (lib/invoices/refund.js), which already exist as their own
  // negative rows and are counted by computeInvoiceState from those rows.
  // Subtract them here so refundedAmount holds only what was refunded
  // OUTSIDE the app (the Stripe dashboard), and the same money is never
  // counted twice. Filtered in JS as well as in the query so a naive test
  // double that ignores `where` still gets the right answer.
  const own = (
    await db.payment.findMany({
      where: { refundOfPaymentId: payment.id, kind: "refund", stripeRefundId: { not: null } },
    })
  ).filter((r) => r.kind === "refund" && r.refundOfPaymentId === payment.id && r.stripeRefundId);
  const ownRefunded = own.reduce((s, r) => s + Math.max(0, -Number(r.amount || 0)), 0);
  const refundedAmount = Math.max(0, Number(charge.amount_refunded || 0) / 100 - ownRefunded);
  const now = new Date();

  await db.payment.update({
    where: { id: payment.id },
    data: { refundedAmount, refundedAt: now },
  });

  // The latest version and the family's payments — the refunded row may sit on
  // an older version of an invoice that has since been amended. See
  // lib/invoices/family.js.
  const invoice = await latestInFamily(db, payment.invoiceId);
  // invoiceId rides along on every `recorded` return so lib/stripe/settleChargeEvent.js
  // can tell the contractor money left, without re-running this lookup. See the
  // notification call there.
  if (!invoice) return { recorded: true, invoiceUpdated: false, invoiceId: payment.invoiceId };

  const state = computeInvoiceState({
    total: invoice.total,
    payments: (await familyPayments(db, invoice.id)).map((p) =>
      p.id === payment.id ? { ...p, refundedAmount, refundedAt: now } : p,
    ),
    priorStatus: invoice.status,
  });

  await db.invoice.update({
    where: { id: invoice.id },
    data: {
      amountPaid: state.amountPaid,
      amountDue: state.amountDue,
      amountRefunded: state.amountRefunded,
      status: state.status,
      // Stamped once, the same convention Booking.feeRefundedAt already uses
      // ("written only after Stripe confirms the refund... a record that
      // money moved, not that somebody asked for it to"). A replayed delivery
      // that changes nothing about the refunded total leaves this untouched.
      refundedAt: state.amountRefunded > 0 ? invoice.refundedAt || now : invoice.refundedAt,
    },
  });

  return {
    recorded: true,
    invoiceUpdated: true,
    invoiceId: invoice.id,
    status: state.status,
    amountRefunded: state.amountRefunded,
  };
}
