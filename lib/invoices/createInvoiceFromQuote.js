...
import { offlineDiscountAmount } from "@/lib/payments/offlineDiscount";
...
export async function ensureInvoiceForQuote(quoteId, { createdById = null, db: deps } = {}) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    // ── The e-transfer / cheque discount the client chose ─────────────────
    //
    // Folded INTO `discount` so every reader of subtotal − discount + tax
    // stays right, and recorded beside it so the document can say what part
    // of the discount it is and "How to pay" can leave the card link off
    // (lib/payments/offlineDiscount.js). The amount is derived from the same
    // base the approval route used — the accepted subtotal less the flat
    // discount — and only when the client's choice was recorded.
    const offlinePct = Number(quote.offlineDiscountPct);
    const offlineAmount =
      quote.offlineDiscountChosen === true && useAccepted && Number.isFinite(offlinePct) && offlinePct > 0
        ? offlineDiscountAmount(Number(quote.acceptedSubtotal) - Number(quote.discount), offlinePct)
        : 0;
    const invoiceDiscount = round2(Number(quote.discount) + offlineAmount);
    ...
    const invoice = await tx.invoice.create({
      data: {
        ...
        offlineDiscountAmount: offlineAmount > 0 ? offlineAmount : null,
        ...
        // And what the tax line SAID — rate, what it applied to, the rates
        // month — copied verbatim, so the invoice explains its tax the way
        // the quote did rather than re-resolving against a newer table.
        // Prisma refuses a bare null on a Json column; DbNull is the value.
