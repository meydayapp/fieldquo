...
// plus one new pairing. Kept in ONE place so the pages cannot drift again — and
// that place is now lib/status/tone.js, because subscription status and payout
// status render the same chips on the same kind of screen and a second copy of
// these five strings would be the drift this module was written to stop. The
// name is kept so every existing importer, and check-invoice-status.mjs, are
// untouched.
export { STATUS_TONE_CLASSES as INVOICE_TONE_CLASSES } from "@/lib/status/tone";
...
export function invoiceStatusClasses(status) {
  return toneClasses(invoiceStatusPresentation(status).tone);
}
