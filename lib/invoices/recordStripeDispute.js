...
export async function recordStripeDispute(db, dispute) {
  ...
  const payment = await db.payment.findFirst({
    ...
    select: {
      ...
      disputeStatus: true,
      disputedAt: true,
      // Read by lib/stripe/disputeRecovery.js, which settleChargeEvent runs
      // after this returns — the three columns are its idempotency.
      disputeHeldCents: true,
      disputeFeeCents: true,
      disputeReturnedCents: true,
    },
  });
  ...
  // The latest version and the family's payments — see recordStripeRefund.js
  // and lib/invoices/family.js for why neither is the row the payment names.
  ...
  if (!invoice) return { recorded: true, invoiceUpdated: false, invoiceId: payment.invoiceId, payment };
