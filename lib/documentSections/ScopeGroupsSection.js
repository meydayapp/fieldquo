...
import { documentTheme, washPair, ruleColor } from "@/lib/documents/theme";
import {
...
// The one projection of a takeoff a client may see: the captured satellite
// still and "Lawn measured: 1,850 sq ft". Nothing else on the takeoff is
// read here (see the comment at resolveServiceContent below).
import { measureEvidence } from "@/lib/measure/measureImages";
// The traced shape drawn from its own vertices — prints even when the still
// above it was never captured. Pure; the public quote page draws the same
// points (lib/documentSections/traceOutline.js).
import { traceOutline } from "@/lib/documentSections/traceOutline";
// A line's scope paragraph may carry the library's rich text (bold, lists,
// links — lib/quotes/richText.js); a plain paragraph comes through the same
// parser unchanged. A text block priced "none" prints no amount at all.
import { RichTextPdf } from "@/lib/documentSections/richTextPdf";
...
export function PdfSection({ data, company = {}, language }) {
  ...
  const glossary = dominantGlossary(groups, language);
    ...
    <View style={{ marginTop: 4 }}>
      ...
      {groups.map((g, gi) => {
        ...
        const content = resolveServiceContent(
          ...
          g.override,
          g.takeoff,
          // The document's language, so a French quote's prose is French —
          // see lib/documents/serviceContent.fr.js.
          language,
        );
        ...
        return (
          ...
          >
            ...
            <View style={{ paddingVertical: 8, paddingHorizontal: 10 }}>
              ...
              {/* The measurement the price rests on: the satellite still
                  with the traced lawn (or the roof, or the eaves) and one
                  caption — "Lawn measured: 1,850 sq ft". A client comparing
                  quotes sees WHAT was priced, not only how much. Only a
                  captured copy prints (lib/measure/measureImages.js); a
                  caption can print alone when the still was never captured. */}
              {(() => {
                const evidence = measureEvidence(g.takeoff, g.categoryKey, language);
                const outline = traceOutline(g.takeoff);
                if (!evidence && !outline) return null;
                return (
                  <View
                    style={{
                      marginBottom: 8,
                  ...
                  >
                    {evidence?.imageUrl ? (
                      <Image
                        ...
                        style={{ width: 240, height: 150, objectFit: "cover", borderRadius: 3, marginBottom: 4 }}
                      />
                    ) : null}
                    {/* The outline the homeowner (or the estimator) drew,
                        beside the area it came to. ruleColor, not the raw
                        accent: a white or pale brand would draw an invisible
                        shape, and ruleColor already falls back to ink for
                        exactly that case (the page's hairlines use it). The
                        fill is the same colour at low opacity. */}
                    <View style={{ flexDirection: "row", alignItems: "center" }}>
                      {outline ? (
                        ...
                        >
                          <Polygon
                            points={outline.pointsAttr}
                            fill={ruleColor(theme)}
                            fillOpacity={0.18}
                            stroke={ruleColor(theme)}
                            strokeWidth={1.2}
                      ...
                      {evidence?.caption ? (
                        <Text style={{ fontSize: 8.5, fontFamily: PDF_FONT_BOLD, color: theme.ink, flex: 1 }}>
                          {evidence.caption}
                        </Text>
                      ) : null}
                    </View>
                    {/* Where it was measured — the job site, which is often
                        not the billing address in the header. inkMuted, the
                        pairing theme.js already holds at 4.5:1. */}
                    {evidence?.measuredAt ? (
                      <Text style={{ fontSize: 8, color: theme.inkMuted, marginTop: 1 }}>
                        {evidence.measuredAt}
                      </Text>
                    ) : null}
                  </View>
              ...
              {g.items.map((item, i) => (
                // The name-and-price row, then the scope underneath it. A View
                // per item rather than a row per item, because a line can now
                // carry a paragraph and the amount must stay level with the
                // NAME, not drift down beside the last sentence of the scope.
                <View key={i} style={{ marginBottom: item.detail ? 6 : 3 }}>
                  <View
                    style={{
                      flexDirection: "row",
                      justifyContent: "space-between",
                  ...
                  >
                    ...
                    {/* The exclusions do not cost $0.00 — an unpriced text
                        block has no amount column, not a zero in it. */}
                  ...
                  {/* What the line actually covers. Indented and quieter than
                      the name, so the page still scans as a list of prices —
                      a homeowner comparing three quotes reads the amounts
                      first and the scope second, and this must not get in the
                      way of the first pass.

                      inkMuted, not a grey literal: this text sits on the
                      company's own brand wash and a fixed grey fails contrast
                      on the mid-tones contractors actually pick. */}
                  {item.detail ? (
                    <RichTextPdf
