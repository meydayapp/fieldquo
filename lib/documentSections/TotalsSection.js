...
function taxRowValue(data, company, language, money) {
  ...
  const statement = taxStatement({
    ...
    // The job address answers first — services on real property are taxed
    // where the property is.
    siteAddress: data.siteAddress || null,
    // subtotal − discount: a document with nothing on it yet is $0 of tax at
    // a known rate, not an unresolved blank.
    taxableBase: Number(data.subtotal ?? 0) - Number(data.discount ?? 0),
    lang: language,
  });
