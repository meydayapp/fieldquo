...
//
// ── The deposit's "how to pay" ──────────────────────────────────────────────
//
// When the schedule parses, the first card is money due at approval, and a
// client reading "50% deposit" wants to know where to send it. So a QUOTE
// with a schedule prints the company's payment methods under the cards —
// the same block the invoice carries (lib/documentSections/HowToPaySection.js),
// with the quote number as the reference and no pay link (that arrives with
// the deposit request). An invoice never prints it here: its own how_to_pay
// section does, and a company that adds payment_terms to an invoice
// template must not get the addresses twice. This replaced a bare
// "Accepted: Cash · E-transfer · Cheque" line that named methods and gave
...
import { depositHowToPay } from "@/lib/payments/offlineMethods";
import { HowToPayPdfRows } from "./HowToPaySection";
...
export function renderEmailHtml({ data = {}, company = {}, language }) {
  ...
  const howTo = depositHowToPay({ data, company, language, schedule });
  ...
  return `
    <div style="margin-top:20px;font-family:Helvetica,Arial,sans-serif;">
      ...
      ${howToPayEmailHtml(howTo, { theme: t })}
    </div>
  `;
}
    ...
    <View style={{ marginTop: 14 }} wrap={false}>
      ...
      {howTo ? (
        <View style={{ marginTop: 8 }}>
          <Text style={{ fontSize: 7.5, fontFamily: PDF_FONT_BOLD, color: t.accentText, letterSpacing: 1, marginBottom: 4 }}>
            {howTo.title.toUpperCase()}
          </Text>
          <HowToPayPdfRows block={howTo} theme={t} />
        </View>
      ) : null}
    </View>
  );
