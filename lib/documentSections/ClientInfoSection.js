...
// [label, value] pairs for the "Details" panel from the custom fields the
// loader attached. Shared with the email/web renderers through
// lib/documentSections/customFacts.js so all three print the same words.
...
export function renderEmailHtml({ data = {}, company = {}, language }) {
  return preparedForBlock({
    ...
    client: data.client || {},
    jobAddress: jobAddressLine(data, documentLabels(language)),
  });
}
...
export function PdfSection({ data = {}, company = {}, language }) {
  ...
  const facts = [
    ...
    // The company's own extra boxes flagged for the document — a PO number
    // on an invoice is the case. Already filtered to showOnDocuments + a
    // non-empty answer by lib/customFields/values.js loadDocumentCustomFields;
    // the loader is the only place that decides what the client sees.
    ...documentCustomFacts(data.customFields, { date, labels }),
  ].filter(Boolean);
  ...
  return (
    <View style={{ flexDirection: "row", gap: 8, marginBottom: 16 }}>
      <Panel theme={t} title="Prepared for" flex={1.3}>
        ...
        {/* Where the work is, when it is somewhere else — a GC's office is
            never the site. See jobAddressLine. */}
        {(() => {
          const job = jobAddressLine(data, labels);
          if (!job) return null;
          return (
            <View style={{ marginTop: 4 }}>
              <Text style={{ fontSize: 6.5, fontFamily: PDF_FONT_BOLD, color: t.inkFaint, letterSpacing: 0.8 }}>
                {String(job.label).toUpperCase()}
              </Text>
              <Text style={{ fontSize: 8, color: t.inkMuted, lineHeight: 1.4 }}>{job.value}</Text>
            </View>
          );
        })()}
      </Panel>
