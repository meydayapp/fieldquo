...
export function PdfSection({ data = {}, company = {}, language }) {
  ...
  const acceptText =
    total > 0
      ? labels.signatureAcceptWithTotal.replace("{total}", money(total))
      : labels.signatureAcceptNoTotal;
  ...
  return (
    ...
    >
      ...
      >
        {acceptText}
        {company.paymentTerms ? labels.signaturePaymentTermsNote : ""}
      </Text>

      {data.signature ? renderSigned(data.signature, t, labels) : renderBlank(t, labels)}
    </View>
  );
}
...
function renderSigned(sig, t, labels) {
  const when = sig.signedAt ? new Date(sig.signedAt) : null;
  const whenStr = when && !Number.isNaN(when.getTime()) ? when.toUTCString() : "—";

  // The audit line — who, when, and the hash that ties this signature to the
  // exact quote content agreed to (see lib/documents/signatureAudit.js). Built
  // from separately-translated fragments rather than one template string
  // because ip/hash are each independently optional.
  const auditParts = [labels.signatureElectronicallySigned];
  if (sig.ip) auditParts.push(labels.signatureFromIp.replace("{ip}", sig.ip));
  if (sig.documentHash) {
    auditParts.push(
      labels.signatureDocumentRef.replace("{hash}", sig.documentHash.slice(0, 16)),
    );
  }
    ...
    <View>
      <View style={{ flexDirection: "row", gap: 16, alignItems: "flex-end" }}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={{ fontSize: 7, fontFamily: PDF_FONT_BOLD, color: t.inkMuted }}>
            {sig.name || labels.signatureFieldLabel}
          </Text>
        </View>
        <View style={{ flex: 1 }}>
          ...
          <View style={{ borderBottom: `1 solid ${t.inkFaint}`, marginBottom: 3 }} />
          <Text style={{ fontSize: 7, fontFamily: PDF_FONT_BOLD, color: t.inkMuted }}>
            {labels.signatureDateSignedLabel}
          </Text>
        </View>
      </View>
...
function renderBlank(t, labels) {
  const fields = [
    labels.signatureFieldLabel,
    labels.signatureNameFieldLabel,
    labels.signatureDateFieldLabel,
  ];
  return (
