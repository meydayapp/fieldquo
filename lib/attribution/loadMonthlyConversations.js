...
/** Meta's platform strings, as CONVERSATION_SOURCES spells them. */
const SOURCE_FOR_PLATFORM = { facebook: "meta_messenger", instagram: "meta_instagram" };
...
export async function loadMonthlyConversations({
  ...
  const rows = threads.map((t) => {
    ...
    const outcome = conversationOutcome({
      conversation: {
        id: t.id,
        source: SOURCE_FOR_PLATFORM[t.channel?.platform] || null,
