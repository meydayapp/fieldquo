...
export const HELP_TREE = Object.freeze({
  "getting-started": [
    ...
    // `alsoScreens`: the More page is described here, not in an article of
    // its own — it is the rail's last row, drawn as tiles.
    A("the-sidebar-and-where-everything-is", "The sidebar, and where everything is", { screen: "home", alsoScreens: ["more"] }),
  ...
  "leads-and-quotes": [
    ...
    // The painting takeoff's arithmetic and its rate sets (lib/pricing/
    // paintTakeoff.js, Settings › Services › Painting rates).
    A("painting-by-production-rate", "Painting priced by production rate", { feature: "paint_takeoff", related: ["group-a-quote-by-room-or-scope", "upsell-add-ons"] }),
  ...
  "jobs-and-scheduling": [
    ...
    // Daily sheets (2026-09-21): objectives, before/after, evaluation, and
    // the performance-pay rule that is OFF until the owner writes one.
    A("daily-sheets", "Daily sheets and performance pay", { screen: "daily-sheets", related: ["timesheets-and-approving-hours", "settings-field-work"] }),
    ...
    // The paid "Build the material list" in the same panel (968a8e06).
    A("the-ai-material-list", "The AI material list", { feature: "ai_material_list", related: ["materials-on-a-job"] }),
  ...
  "invoices-and-payments": [
    ...
    // The offline methods by country and the "How to pay" block they print
    // — lib/payments/offlineMethods.js.
    A("offline-payment-methods", "Cash, e-transfer, cheque — and Zelle, Venmo or Cash App in the US", { screen: "settings-payments", feature: "offline_payment_methods", related: ["how-clients-pay-online", "record-a-manual-payment"] }),
    ...
    A("service-plans", "Service plans (recurring billing)", { screen: "plans", alsoScreens: ["settings-maintenance-plans"], feature: "service_plans", only: true }),
    ...
    // The slug outlived the bookkeeping export it was named for (owner,
    // 2026-09-24: import, not export — lib/export/companyDataExport.js). A
    // slug is a URL, so it stays; the article is now what an accountant CAN
    // be given. It no longer claims the Expenses row: articleForScreen takes
    // the first match, and this entry, earlier in the tree, was answering
    // "help for this screen" on Expenses ahead of the Expenses article.
    A("the-accounting-export", "What to hand your accountant", { feature: "expenses" }),
  ...
  clients: [
    ...
    // No `screen`: the list is reached from Clients and from Settings →
    // Follow-ups → Past clients, not from a sidebar row of its own.
    A("past-client-callbacks", "Past-client callback rotation", { related: ["settings-follow-ups", "the-client-record"] }),
    A("client-notes", "Client notes", { feature: "clients" }),
    // What clients raise from their portal (0bdea545) — the More › Work &
    // sales row's article. Tickets have their own screen and no article
    // of their own until this one; the row pointed at the client-portal
    // article, which describes the portal from the client's side and says
    // nothing about the queue, and resolved to nothing on /app/help.
    A("client-tickets", "Client tickets: what clients raise from their portal", { screen: "client-tickets-queue", feature: "client_portal", related: ["the-client-portal-as-a-client", "the-leads-board"] }),
  ...
  "team-and-access": [
    ...
    A("the-t5018-year-end-list", "The T5018 year-end figures", { screen: "subcontractors", feature: "contractor_payouts" }),
    ...
    // The HR file — documents, onboarding, policies, the log book, and the
    // compliance view that composes them. One feature key (hr_compliance)
    // across the five; the two with a sidebar row carry its screen slug.
    A("employee-documents", "Employee documents and expiry reminders", { feature: "hr_compliance", related: ["hr-and-compliance"] }),
    A("new-hire-onboarding", "New-hire onboarding checklists", { feature: "hr_compliance", related: ["employee-documents", "company-policies"] }),
    A("company-policies", "Company policies and acknowledgements", { screen: "settings-policies", feature: "hr_compliance", related: ["hr-and-compliance"] }),
    A("the-manager-log-book", "The manager's log book", { screen: "manager-log", feature: "hr_compliance" }),
    A("hr-and-compliance", "HR & compliance overview", { feature: "hr_compliance", related: ["employee-documents", "new-hire-onboarding", "company-policies"] }),
    // FieldQuo's own sales tier, not a contractor screen: the call-centre
    // agencies FieldQuo hires and the reps under them (lib/sales/agency.js).
    // Here because it is the one public, translated place the agency's
    // portal can point at; it names no contractor feature and no sidebar row.
  ],
  ...
  "marketing-and-website": [
    ...
    A("influencer-programme", "Influencer programme", { screen: "influencer", feature: "referrals", related: ["refer-another-business"] }),
  ...
  "mobile-and-crew": [
    ...
    A("your-schedule-on-your-phone", "Your schedule on your phone", { screen: "my-schedule", feature: "crew_shifts" }),
    ...
    // The employee home (2026-09-13): the Home tab, the trade / cover /
    // claim flow behind Find cover, and the availability request.
    A("your-home-screen", "Your home screen", { screen: "my-home", feature: "crew_shifts", related: ["trading-and-covering-shifts", "changing-your-availability"] }),
    A("trading-and-covering-shifts", "Trading and covering shifts", { feature: "crew_shifts", related: ["your-home-screen"] }),
    A("changing-your-availability", "Changing your availability", { screen: "settings-availability", feature: "crew_shifts", related: ["your-home-screen"] }),
    ...
    A("working-without-signal", "Working without signal: invoices and punches that wait to sync", { related: ["settings-field-work", "clock-in-and-out-on-your-phone"] }),
  ...
  settings: [
    ...
    A("settings-my-calendar", "My calendar", { screen: "settings-my-calendar", feature: "scheduling", related: ["settings-your-hours", "google-calendar"] }),
    ...
    A("settings-field-work", "Field work: offline mode, labour rate, performance pay", { screen: "settings-field-work", related: ["working-without-signal", "daily-sheets"] }),
    ...
    A("settings-work-email", "Work email", { screen: "settings-work-email", related: ["settings-email-domain", "settings-my-calendar"] }),
    ...
    // The same page has a rail row (AI › AI team) since the 2026-09-21 shell.
    A("settings-ai-employee", "AI employee", { screen: "settings-ai-employee", alsoScreens: ["ai-team"], feature: "ai_employee" }),
  ...
  integrations: [
    ...
    A("google-calendar", "Google Calendar", { feature: "scheduling", related: ["settings-my-calendar", "the-appointments-calendar"] }),
...
export function articleForScreen(screen) {
  ...
  return (
    HELP_ARTICLES.find((a) => a.screen === screen) ||
    HELP_ARTICLES.find((a) => Array.isArray(a.alsoScreens) && a.alsoScreens.includes(screen)) ||
    null
  );
}
