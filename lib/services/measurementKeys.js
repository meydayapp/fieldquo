...
//   drywall   wallSqft + ceilingSqft; a sheet line carries `coverage: 32`
//             (sq ft per 4×8 sheet) so qty = area ÷ 32 × (1 + waste)
...
//             not the source; a post line carries `coverage: 8` (ft per post)
//   concrete  areaSqft from tracedArea.js#measureTracedArea; the cubic-yard
//             line carries coverage = 324 ÷ thickness in inches (81 sq ft per
//             cu yd at 4 in)
//
// WIRING, NOT DONE HERE: these trades do not yet mount the room takeoff or
// the tracer on their quotes (lib/pricing/takeoffTrades.js lists flooring
// only, and adding a trade there without its builder form fails
// check-takeoff-render and double-prices a group). That is the builder pass.
...
export const MEASUREMENT_KEYS = {
  ...
  // ── Generic — typed on the estimate from the room dimensions; no module computes them ──
  ...
  perimeterLf: { label: "Perimeter (linear ft)", unit: "linear_ft", kind: "length", source: "manual — typed on the estimate" },
...
export const TRADE_MEASUREMENTS = {
  ...
  flooring_install: ["floorSqft", "linearFt"],
  tiling: ["floorSqft", "wallSqft"],
  drywall: ["wallSqft", "ceilingSqft"],
  drywall_install: ["wallSqft", "ceilingSqft"],
  siding: ["wallSqft"],
  fence_services: ["edgingFt"],
  concrete: ["areaSqft"],
