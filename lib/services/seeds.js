 ...
 */
export const TEMPLATE_LANGUAGES = ["en", "fr", "es", "it", "de", "uk", "pa", "tl"];
...
//       coverage?: { per, unit: sqft|linft|cuft|each } // material lines only: how much of the
//                                                      //   figure one unit covers; qty =
//                                                      //   ceil(figure × waste ÷ per)
//     }],
...
/** The currency a country's companies bill in, for the per-country material costs. */
function countryCurrency(country) {
  return { US: "USD", CA: "CAD" }[country] || null;
}

/**
 * A trade's seed PLUS every row another trade's file tags for it.
 *
 * A service many trades sell (caulking, gutter cleaning, a dryer vent) lives
 * once, in the most specific trade's file, and lists every quote type that
 * sells it in `categories`. Seeding trade X installs X's own rows and every
 * other file's row whose `categories` include X — deduplicated by seedKey,
 * the trade's own rows first. Null when neither exists.
 */
export function serviceSeedsForCompanyTrade(trade) {
  const own = serviceSeedsFor(trade);
  const seen = new Set((own?.services || []).map((s) => s.seedKey));
  const tagged = [];
  for (const other of seededTrades()) {
    if (other === trade) continue;
    for (const s of SERVICE_SEEDS[other].services || []) {
      if (seen.has(s.seedKey)) continue;
      if (Array.isArray(s.categories) && s.categories.includes(trade)) {
        seen.add(s.seedKey);
        tagged.push(s);
      }
    }
  }
  if (!own && !tagged.length) return null;
  return { ...(own || { trade, categories: [] }), services: [...(own?.services || []), ...tagged] };
}
 ...
 */
export function seedTemplateFor(service, { language, currency, country } = {}) {
  ...
  const lines = raw.map((line, i) => {
    ...
    // A material costed from the per-country reference (app/data/
    // materialReference.js) uses THAT country's shelf price, pack and
    // coverage when the company bills in its currency; with no such price
    // the US figure is converted like every other seed number and the line
    // says so. Everything else converts as before.
    const local = cc && currency === countryCurrency(cc) && line?.unitCostByCountry && Number.isFinite(Number(line.unitCostByCountry[cc]));
    const costSource = line?.materialRef ? (local ? "reference" : currency && currency !== "USD" ? "fx-estimate" : "reference") : undefined;
    return {
      kind: line?.kind,
      name,
      description,
      qty: line?.qty,
      unit: (local && line?.unitByCountry?.[cc]) || line?.unit,
      unitPrice: local && Number.isFinite(Number(line?.unitPriceByCountry?.[cc])) ? Number(line.unitPriceByCountry[cc]) : suggestedIn(line?.unitPrice, currency),
      unitCost: local ? Number(line.unitCostByCountry[cc]) : suggestedIn(line?.unitCost, currency),
      taxable: line?.taxable,
      measurementKey: line?.measurementKey,
      wastePct: line?.wastePct,
      coverage: (local && line?.coverageByCountry?.[cc]) || line?.coverage,
      ...(line?.materialRef ? { materialRef: line.materialRef, costSource } : {}),
