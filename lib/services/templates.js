...
// calls it through lib/quotes/serviceTemplateLines.js templateOffered and
// wires the "Add with its template lines" action — it decides nothing itself.
//
...
// builder passes the figures the quote's scope groups hold as `measurements`
// to expandTemplate (lib/quotes/serviceTemplateLines.js measurementsFromGroups
// says which form stores which figure). The calculator produces the quantity, the line supplies
...
export function sanitiseTemplateLine(input) {
  ...
  // Where a material's cost came from (seed data only): the reference row it
  // was costed against, and whether the cost is that country's shelf price
  // ("reference") or the US price converted at the fixed benchmark rate
  // ("fx-estimate") — so the screen can say "estimated from US price".
  if (kind === "material" && typeof input.materialRef === "string" && /^[a-z0-9_]{1,64}$/.test(input.materialRef)) line.materialRef = input.materialRef;
  if (kind === "material" && ["reference", "fx-estimate"].includes(input.costSource)) line.costSource = input.costSource;
...
export function lineText(line, language, defaultLanguage = "en") {
  ...
  if (!entry?.name) {
    // No rendering in the document's language: English next (every seed
    // line carries it, under `translations.en` when the company writes in
    // another language), then the company's own words — never a machine
    // translation (non-negotiable #6). Still flagged `missing` either way.
    const en = language !== "en" && defaultLanguage !== "en" ? line?.translations?.en : null;
    if (en?.name) return { name: en.name, description: en.description || "", missing: true };
...
// ── Words the company wrote itself (2026-09-25) ─────────────────────────────
//
// A seeded line carries its catalogue translations in `translations`. A line
// the company TYPED has none, and a seeded line it RENAMED still carries the
// seed's translations of the old words — the editor round-trips
// `translations` through an edit, and nothing records which words they were
// of. So a French quote printed the English the company typed, or the seed's
// French for a name the company had since changed.
//
// Those words are drafted on save as phrases instead (lib/i18n/phrases.js,
// namespaces templateLineName / templateLineDescription — PATCH
// /api/products/[id]), keyed by the hash of the text, and GET /api/products
// sends each product's drafts beside its lines as `templateLinePhrases`:
// { name: { [text]: { [lang]: text } }, description: { … } }. They are NOT
// written into `translations`: the editor would save them back into the row,
// a second copy that goes stale on the next rename. The line's words are
// still copied onto the quote line when it is added, so a sent quote keeps
// what it said (non-negotiable #6) — drafted once on save, never at send.

/**
 * The name/description texts of `lines` that are the company's own words,
 * for scheduling: a line with no catalogue translations at all, or a
 * name/description the previous version of the row did not have (new, or
 * renamed — a renamed seeded line keeps stale translations, see above).
 * Unchanged seeded lines are left out: they already carry every language and
 * re-drafting them would pay for words FieldQuo already wrote.
 *
 * @returns { names: string[], descriptions: string[], fresh: boolean } —
 *          `fresh` when any returned text is new to the row (the save banner
 *          speaks only then).
 */
export function authoredTemplateText(lines, previousLines = null) {
  const list = Array.isArray(lines) ? lines : [];
  const prev = Array.isArray(previousLines) ? previousLines : [];
  const text = (v) => (typeof v === "string" ? v.trim() : "");
  const prevNames = new Set(prev.map((l) => text(l?.name)).filter(Boolean));
  const prevDescriptions = new Set(prev.map((l) => text(l?.description)).filter(Boolean));
  // "Renamed" needs a previous version to have been renamed FROM. A row with
  // no template before this save has nothing to compare against, and a line
  // arriving with translations there came from the catalogue, not a keyboard.
  const hadLines = prevNames.size > 0;
  const names = new Set();
  const descriptions = new Set();
  let fresh = false;
  for (const l of list) {
    if (!l || typeof l !== "object") continue;
    const seeded = Boolean(l.translations && typeof l.translations === "object" && Object.keys(l.translations).length);
    const name = text(l.name);
    const description = text(l.description);
    if (name && (!seeded || (hadLines && !prevNames.has(name)))) {
      names.add(name);
      if (!prevNames.has(name)) fresh = true;
    }
    if (description && (!seeded || (hadLines && !prevDescriptions.has(description)))) {
      descriptions.add(description);
      if (!prevDescriptions.has(description)) fresh = true;
    }
  }
  return { names: [...names], descriptions: [...descriptions], fresh };
}

/** Every distinct line name and description across `products`, for the read. */
export function templateLineTexts(products) {
  const names = new Set();
  const descriptions = new Set();
  for (const p of Array.isArray(products) ? products : []) {
    for (const l of Array.isArray(p?.templateLines) ? p.templateLines : []) {
      if (typeof l?.name === "string" && l.name.trim()) names.add(l.name);
      if (typeof l?.description === "string" && l.description.trim()) descriptions.add(l.description);
    }
  }
...
/**
 * One product's slice of the loaded drafts — `{ name, description }` maps
 * holding only its own lines' texts — or null when it has none, so a product
 * with no drafted words carries nothing extra to the browser.
 */
export function templateLinePhrasesFor(product, maps) {
  const pick = (source, text) => (typeof text === "string" && source && typeof source === "object" ? source[text] : null);
  const name = {};
  const description = {};
  // Keyed by the TRIMMED text: expandTemplate looks lines up after the
  // sanitiser has trimmed them, whatever spacing the stored row kept.
  for (const l of Array.isArray(product?.templateLines) ? product.templateLines : []) {
    const n = pick(maps?.name, l?.name);
    if (n) name[l.name.trim()] = n;
    const d = pick(maps?.description, l?.description);
    if (d) description[l.description.trim()] = d;
  }
  return Object.keys(name).length || Object.keys(description).length ? { name, description } : null;
}

/**
 * The line's text from its drafted phrases, or null when neither its name
 * nor its description has a draft in `language` (then lineText decides,
 * exactly as before). A draft is preferred over `translations` because a
 * draft exists only for words the company wrote — where `translations`, if
 * any, are the seed's words for what the line used to say.
 */
export function phrasedLineText(line, phrases, language, defaultLanguage = "en") {
  if (!language || !phrases || typeof phrases !== "object") return null;
  const draft = (map, text) => {
    const hit = typeof text === "string" && text.trim() && map && typeof map === "object" ? map[text.trim()]?.[language] : null;
    return typeof hit === "string" && hit.trim() ? hit : null;
  };
  const name = draft(phrases.name, line?.name);
  const description = draft(phrases.description, line?.description);
  if (!name && !description) return null;
  const base = lineText(line, language, defaultLanguage);
  const hasDescription = typeof line?.description === "string" && Boolean(line.description.trim());
  return {
    name: name || base.name,
    description: description || base.description,
    missing: name && (description || !hasDescription) ? false : base.missing,
  };
}
 ...
 *          amount, unitCost, cost, taxable, measurementKey, measured, needsMeasurement,
...
export function expandTemplate(product, { measurements = null, currency = null, fromCurrency = null, fx = null, language = null, defaultLanguage = "en" } = {}) {
  ...
  return lines.map((line) => {
    ...
    // The company's own words in the document's language when they have a
    // draft (GET /api/products sends them as templateLinePhrases); otherwise
    // the catalogue translation or the words as written, as always.
    const text = phrasedLineText(line, product?.templateLinePhrases, language, defaultLanguage) || lineText(line, language, defaultLanguage);
    ...
    return {
      ...
      rate: unitRate,
      amount,
      // The unit cost too, so a caller writing a quote line can carry it as
      // the line's own `unitCost` without dividing a rounded total back.
      unitCost,
