...
// carries `coverage` — how much of the figure one unit covers: 32 sq ft per
// drywall sheet, 8 ft per fence post, 324 ÷ inches sq ft per cubic yard of
// concrete — so qty = figure ÷ coverage (× 1 + waste on a material line).
//
...
export function sanitiseTemplateLine(input) {
  ...
  if (isMeasurementKey(input.measurementKey)) {
    ...
    // "coverage"): 32 sq ft per drywall sheet, 8 ft per post. 1 = the
    // figure itself, and is not stored.
    const cov = Number(input.coverage);
    if (Number.isFinite(cov) && cov > 0 && cov !== 1) line.coverage = Math.round(cov * 1000) / 1000;
...
export function validateTemplateLines(input) {
  ...
  input.forEach((l, i) => {
    ...
    if (l.coverage !== undefined && !(Number(l.coverage) > 0)) problems.push(`[${i}] coverage ${l.coverage} must be a positive number`);
    if (l.coverage !== undefined && l.measurementKey === undefined) problems.push(`[${i}] coverage without a measurementKey`);
  });
  return problems;
}
  ...
  return lines.map((line) => {
    ...
    if (line.measurementKey) {
      ...
      if (m == null) {
        ...
        const coverage = line.coverage > 0 ? line.coverage : 1;
        quantity = Math.round((m / coverage) * (1 + waste / 100) * 100) / 100;
      }
    }
