...
// the company already holds). The route (app/api/settings/services/confirm)
 ...
 * seeder uses; null = "set your rate"), and `held` for a row the company
 * already has by seedKey — shown, ticked and locked, never offered twice.
 */
export function candidateGroups(candidates, { language = "en", currency = "CAD", heldSeedKeys = [] } = {}) {
  const held = new Set((Array.isArray(heldSeedKeys) ? heldSeedKeys : []).filter((k) => typeof k === "string"));
