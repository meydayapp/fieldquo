 ...
 */
/**
 * The job with everything deriveSourcingLines needs attached: the quote's
 * scope groups with their categories, the frozen costing, the company's rate
 * overrides per category and its recipe overrides.
 *
 * Shared by the regenerate below and by the AI build
 * (lib/materials/build.js), which hands the same derived lines to the model
 * as facts — one loader, so the two cannot disagree about which rates a job
 * is sourced against.
 *
 * @returns {{ job, ratesById: Map } | null}
 */
export async function loadJobForSourcing(jobId, companyId) {
...
export async function regenerateSourcingList(jobId, companyId) {
  ...
  // Excluded rows are kept too: "don't offer this again" is a statement the
  // AI build reads, and a regenerate that deleted the row would un-say it.
  const keep = job.materials.filter((m) => m.purchasedAt || m.addedByHand || m.excludedAt);
  const toRemove = job.materials.filter(
    (m) => !m.purchasedAt && !m.addedByHand && !m.excludedAt,
  );
    ...
    ...(fresh.length
      ? [
          db.jobMaterial.createMany({
            // Marked as the takeoff's own so the panel draws them under
            // "Primary" and refuses to edit their quantity — see the schema
            // note on JobMaterial.source.
            data: fresh.map((l) => ({ ...l, jobId, source: "takeoff", group: "primary" })),
          }),
        ]
      : []),
  ]);
...
export async function recordMaterialPrice({
  ...
  qty,
  actualCost,
  // Optional, for a price read off a confirmed receipt (lib/receipts/record.js):
  // the Expense row the purchase became and the store it came from. Both
  // columns already existed on MaterialPriceEntry and nothing wrote them.
  expenseId = null,
  supplier = null,
}) {
  ...
  try {
    ...
    await db.materialPriceEntry.create({
      data: {
        ...
        price: unitPrice,
        ...(expenseId ? { expenseId } : {}),
        ...(supplier ? { supplier: String(supplier).slice(0, 200) } : {}),
      },
    });
