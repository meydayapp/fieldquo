...
export async function renderQuoteDocumentPdf({ db = realDb, quote: given, variant = "as_sent" }) {
  ...
  const customFields = await loadDocumentCustomFields(db, quote.companyId, "quote", quote.id);
...
export async function renderInvoiceDocumentPdf({ db = realDb, invoice: given }) {
  ...
  const customFields = await loadDocumentCustomFields(db, invoice.companyId, "invoice", invoice.id);
