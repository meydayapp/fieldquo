...
export const DOCUMENT_KINDS = [
  ...
  "photo",
  "invoice",
  // The client preparation guide as it was emailed — filed by
  // lib/prepGuide/send.js with source "prep_guide", one row per send, a
  // resend superseding the last. Not a money kind: it is the checklist the
  // client got, and the crew is exactly who should be able to read it.
  "prep_guide",
  // A signed release of liability — filed by lib/waivers/service.js when the
  // client ticks every acknowledgement and signs (kind "waiver", source
  // "waiver_signed"). Not a money kind: it carries no price, and the crew
  // is exactly who needs to know the homeowner agreed to keep the dog out.
  "waiver",
  "other",
];

/**
 * The kinds the system files on its own — never through the upload form.
 *
 * "quote" was added to the set for this. The owner's ask: "the job should
 * have the original quote, the approved contract and invoice as part of the
 * documents". The quote AS SENT (unsigned, kind "quote") and the quote AS
 * SIGNED (kind "contract") are two different documents with two different
 * questions behind them — "what did we offer" and "what did they agree to" —
 * and a homeowner who ticked an add-on between the two has different totals
 * on each. See lib/jobs/documentAutofile.js.
 */
export const AUTOFILED_KINDS = new Set(["quote", "contract", "invoice"]);
 ...
 *
 * "quote" joined the set with autofiling: the quote as sent IS the rate card
 * for that job, line by line, which is the document non-negotiable #4 refuses
 * to publish and the one a crew member is withheld from on the quote page
 * itself (redactQuote). A kind the crew can open that shows what the office
 * hides one tab over is not a smaller leak.
 */
export const MONEY_KINDS = new Set(["quote", "contract", "invoice"]);
