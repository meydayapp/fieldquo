 ...
 *
 * `waiting_client`: sent to the homeowner for signature and not yet signed
 * (lib/jobs/changeOrderAddendum.js). Money-wise it is `pending` — it affects
 * nothing until the client signs — and it is a separate value so the job page
 * can say "Waiting on client approval" rather than "Not yet agreed", and so
 * the staff-side Mark agreed cannot short-circuit a signature that was asked
 * for: only the client's signature or a staff Reject moves it on.
 */
export const CHANGE_ORDER_STATUSES = ["pending", "waiting_client", "approved", "rejected"];

/** True when the change order is out with the client and unsigned. */
export function isWaitingOnClient(co) {
  return changeOrderStatus(co) === "waiting_client";
}
...
/**
 * Words on the invoice line, English by default; the bill route passes the
 * invoice's own language (lib/i18n/documentLabels.js: changeOrder,
 * changeOrderApprovedBy). A document keeps the language it was created in
 * (AGENTS.md #6), so the line follows the invoice, not the viewer.
 */
const BILL_WORDING_EN = {
  changeOrder: "Change order",
  // {name} and {date} — the client's signature name or the staff decider.
  approvedBy: (name, date) => `Approved by ${name}${date ? ` on ${date}` : ""}`,
};

/**
 * "Approved by L. Angelos on Sep 21, 2026" — from the client's signature when
 * there is one, else the staff decision. Nothing when neither names anyone:
 * a row written before decisions were attributed has no stamp to print, and
 * an invented one would put a name on a document.
 */
function approvedByFor(co) {
  const sig = co?.signature;
  if (sig && typeof sig === "object" && String(sig.name || "").trim()) {
    return { name: String(sig.name).trim(), at: sig.signedAt || co.decidedAt || co.createdAt || null };
  }
  const staff = String(co?.decidedBy?.name || "").trim();
  // A row logged as agreed before decisions were stamped has createdAt as the
  // only date it can honestly claim — the day it was written down.
  if (staff) return { name: staff, at: co.decidedAt || co.createdAt || null };
  return null;
}

function approvalStamp(co, w) {
  const by = approvedByFor(co);
  if (!by) return null;
  const date = by.at ? (typeof w.date === "function" ? w.date(by.at) : new Date(by.at).toISOString().slice(0, 10)) : "";
  return w.approvedBy(by.name, date);
}
...
export function changeOrderSummary(changeOrders = []) {
  ...
  const counts = { approved: 0, pending: 0, waiting_client: 0, rejected: 0, unrecognised: 0 };
  let approvedTotal = 0;
  let pendingTotal = 0;
  let rejectedTotal = 0;
  let billedTotal = 0;
  ...
  return {
    counts,
    total: counts.approved + counts.pending + counts.waiting_client + counts.rejected + counts.unrecognised,
 ...
 */
export function billChangeOrders({ invoice, changeOrders = [], wording = null } = {}) {
  const refuse = (reason) => ({ ok: false, reason, lineItems: [], added: 0 });
  const w = { ...BILL_WORDING_EN, ...(wording && typeof wording === "object" ? wording : {}) };
  ...
  // The line SAYS it is a change order, which one, and who approved it — the
  // mockup's "Change order CO-2 · Second coat … · approved by L. Angelos
  // Sep 21". `description` carries the label because every renderer prints
  // description and only the PDF prints `detail`; the approval stamp goes in
  // `detail` so the PDF a client keeps can show it under the line.
  const all = Array.isArray(changeOrders) ? changeOrders : [];
  const lineItems = toBill.map((co) => ({
    description: [`${w.changeOrder} ${changeOrderLabel(co, all)}`, String(co.description ?? "").trim()].filter(Boolean).join(" · "),
    quantity: 1,
    amount: round2(num(co.priceDelta)),
    ...(approvalStamp(co, w) ? { detail: approvalStamp(co, w) } : {}),
