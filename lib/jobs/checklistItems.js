...
/**
 * The catalogue key for a phase, for anything a PERSON reads.
 *
 * The two tables above stay as they are: `groupChecklistByPhase` puts `label`
 * on a plain object that goes into a PDF and into an email, neither of which
 * has a t(), and both of which already resolve language their own way. A
 * SCREEN, though, has t() and had been printing the English straight out of
 * the table — which is how /app/settings/checklists showed "On the job" in a
 * Spanish interface.
 *
 * A key derived from the phase rather than a fourth hand-written map: the
 * three phases are a closed set defined ten lines up, and a map would be the
 * copy that rots when a fourth phase is added.
 */
export function phaseLabelKey(phase) {
  const known = CHECKLIST_PHASES.includes(phase) ? phase : "during";
  return `app.checklists.phase.${known}`;
}
...
export const RESPONSE_TYPES = [
  ...
  "single_select",
  "multi_select",
  // The three the per-trade templates brought (lib/checklists/typedItems.js):
  // a green/amber/red condition call, a photo the item itself asks for, and a
  // signature drawn on the phone. Added at the END so nothing that indexes or
  // iterates the older seven changes what it sees first.
  "stoplight",
  "photo",
  "signature",
];
...
export const RESPONSE_TYPE_LABELS = {
  ...
  stoplight: "Good / Watch / Problem",
  photo: "Photo",
  signature: "Signature",
};

/// The item types a person picks in Settings → Checklists, and the stored
/// `responseType` each one is. One stored vocabulary, not two: `type` is
/// accepted on the way in (the seeds and the editor speak it) and folded onto
/// the `responseType` the construction library already uses, so a "numeric"
/// inspection reading and a "number" template field are the same thing on a
/// visit. `check` has no responseType on purpose — an absent type has always
/// meant "a tick", and stamping one would change what legacy rows claim.
export const ITEM_TYPE_TO_RESPONSE = {
  check: null,
  text: "text",
  number: "numeric",
...
/// Languages an item's own wording can be carried in (`item.i18n`). The app's
/// nine; the seeds fill the eight FieldQuo supports (lib/i18n/documentLabels.js).
export const ITEM_LANGUAGES = ["en", "fr", "es", "it", "de", "uk", "tl", "pa", "zh"];

/// A drawn signature is stored the way every other signature in this product
/// is — a PNG data URL (WaiverSign, QuoteApproval). Capped so a pathological
/// canvas cannot bloat a job row past what a phone can load.
...
/**
 * The stored responseType for an item that names a `type` instead. Returns
 * undefined for no type at all and null for `check` — the caller writes no
 * responseType in both cases.
 */
function responseTypeFromItemType(value) {
  if (value === undefined || value === null || value === "") return undefined;
  const key = String(value).trim().toLowerCase();
  if (key in ITEM_TYPE_TO_RESPONSE) return ITEM_TYPE_TO_RESPONSE[key];
  // An unknown `type` falls back to a tick rather than pass/fail: `type` is
  // the template vocabulary, where the default IS a tick.
  return null;
}
...
function cleanI18n(value) {
  ...
  for (const lang of ITEM_LANGUAGES) {
    ...
    const checklist = cleanString(entry.checklist);
    const options = Array.isArray(entry.options)
      ? entry.options.map(cleanString).filter(Boolean)
      : [];
    if (!label && !section && !checklist && !options.length) continue;
    out[lang] = {
      ...(label && { label }),
      ...(section && { section }),
      ...(checklist && { checklist }),
...
/**
 * What the crew recorded, cleaned for its type. A string or number for most;
 * a list of picked options for a multi-select; `{ name, dataUrl, signedAt }`
 * for a signature. Anything malformed is dropped rather than stored half-way —
 * a signature with no image is not a signature.
 */
function cleanResponse(value) {
  if (typeof value === "number") return Number.isFinite(value) ? value : null;
  if (Array.isArray(value)) {
    const picked = value.map(cleanString).filter(Boolean);
  ...
  if (value && typeof value === "object") {
    ...
    if (
      ...
      dataUrl.length > SIGNATURE_MAX_CHARS
    )
      return null;
    ...
    return {
      ...
      dataUrl,
      signedAt: Number.isFinite(signedAt.getTime()) ? signedAt.toISOString() : null,
    };
  }
 ...
 * @param {boolean} [opts.keepI18n]  Preserve each item's other-language
 *                                   wording (default true). Kept on the copy
 *                                   too, so a crew member reads the list in
 *                                   their own language on the job
 *                                   (lib/checklists/typedItems.js localizeItem).
 */
export function normalizeChecklistItems(items, opts = {}) {
  ...
  const keepI18n = opts.keepI18n !== false;
  ...
  return items
    .map((item) => {
      ...
      if (rawType) next.responseType = normalizeResponseType(rawType);
        ...
        // The template vocabulary (`type: "stoplight"`) folds onto the same
        // field — see ITEM_TYPE_TO_RESPONSE.
        const fromType = responseTypeFromItemType(item.type);
        if (fromType) next.responseType = fromType;
      }
      ...
      // ── Typed-template structure (lib/checklists/typedItems.js) ───────
      //
      // `required`: the job cannot be closed while this is unanswered.
      // `checklist` / `templateId`: which list put the item here, so a job
      // holding two forms can draw them as two forms and a list is not
      // applied twice. All three are structure, not state, so they survive a
      // template copy exactly as `section` does.
      if (item.required === true) next.required = true;
      const checklist = cleanString(item.checklist);
      if (checklist) next.checklist = checklist;
      const templateId = cleanString(item.templateId);
      if (templateId) next.templateId = templateId;
      if (keepI18n) {
        const i18n = cleanI18n(item.i18n);
        if (i18n) next.i18n = i18n;
      }
      ...
      if (keepDone) {
        const response = cleanResponse(item.response);
