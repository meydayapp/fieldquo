 ...
 *
 * ── `labelKey` beside `label`, for the same reason the badge map has one ───
 *
 * These labels were English strings, so the four buttons on the job page said
 * "On my way" in a French office. Each move now carries a catalogue key and
 * keeps the English as the fallback — the same `[key, English]` discipline
 * VISIT_STATUS_LABELS follows, written out in full so check:translations can
 * see every key.
 *
 * `office: true` marks a move the OFFICE makes from a desk (complete, cancel,
 * reopen); the one without it — "on my way" — is a crew member's, and the
 * calendar does not offer it because the person at the calendar is not in
 * the van.
 */
export function visitActions(status) {
  switch (status) {
    case "on_the_way":
      return [
        { to: "completed", labelKey: "app.visitAction.markComplete", label: "Mark complete", tone: "primary", office: true },
    ...
    default:
      ...
      return [
        { to: "on_the_way", labelKey: "app.visitAction.onMyWay", label: "On my way", tone: "primary", texts: true },
...
/** The button wording for one move, through the caller's `t`. */
export function visitActionLabel(action, t) {
  if (!action) return "";
  return t ? t(action.labelKey, action.label) : action.label;
}
