...
export function buildPhotoReportData({ job, client, photos, now = new Date() }) {
  const groups = stageTimeline(photos).map((g) => ({
    ...
    photos: g.photos.map((p) => ({
      // displayPhotoUrl() picks the flattened (annotated) asset when one
      // exists, the untouched original otherwise — this report is the
      // "hand it to a client or an insurer" document, so markup drawn on an
      // issue photo (evidence of pre-existing damage, pointed out with an
      // arrow) needs to actually show up here, not just in the app. Then
      // resized, never the original resolution — see REPORT_PHOTO_WIDTH
      // above. Any URL that isn't a Cloudinary /upload/ URL comes back
      // unchanged rather than mangled (resizedUrl's own guarantee), so a
      // foreign URL still renders, just uncapped.
      url: resizedUrl(displayPhotoUrl(p), { width: REPORT_PHOTO_WIDTH }),
      caption: p.caption || "",
