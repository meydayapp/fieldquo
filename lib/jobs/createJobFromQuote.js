...
export async function ensureJobForAcceptedQuote(quoteId, deps = {}) {
  ...
  if (created && job) {
    ...
    // The trade's "add automatically" checklists (lib/checklists/autoAdd.js).
    // Same contract: a checklist hiccup never undoes the job.
    try {
      await attachAutoChecklists(prisma, { jobId: job.id, companyId: job.companyId, quoteId });
    } catch (err) {
      console.error("[createJobFromQuote] auto checklists:", err?.message);
    }
