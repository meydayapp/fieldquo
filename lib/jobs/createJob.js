...
import { attachAutoChecklists } from "@/lib/checklists/autoAdd";
...
export async function createJob(
  db,
  {
    ...
    originalJobId,
    callbackReason,
    // Which of the client's installed equipment a WARRANTY callback is about.
    // Optional; refused on any other reason — see lib/equipment/installed.js.
    warrantyEquipmentId,
    // Where the work happens. Optional, and null means NOT ASKED rather than
    // "same as the client" — see Job.siteAddress's own comment for why the
    // billing address is the wrong default.
    siteAddress,
  },
) {
  if (!clientId || !title) {
  ...
  const owned = await assertOwnedIds(db, companyId, {
    clientId,
    quoteId,
    originalJobId,
    clientEquipmentId: warrantyEquipmentId || null,
  });
  if (!owned.ok) return { job: null, error: owned.error, status: owned.status };

  // The tenant check above proved the row is ours; this proves it is the same
  // client's and that the job is a warranty callback at all. Read fresh rather
  // than trusted from the form that offered the picker.
  if (warrantyEquipmentId) {
    const equipment = await db.clientEquipment.findFirst({
      where: { id: warrantyEquipmentId, companyId },
      select: { id: true, clientId: true },
    });
    const verdict = warrantyLinkVerdict({ job: { clientId, callbackReason }, equipment });
    if (!verdict.ok) return { job: null, error: verdict.error, status: verdict.status };
  }
  ...
  const job = await db.job.create({
    data: {
      ...
      warrantyEquipmentId: warrantyEquipmentId || null,
      siteAddress: normaliseSiteAddress(siteAddress),
    },
    include: { client: true },
  });

  // One Google call per address, at the moment it is set. Awaited rather
  // than fired and forgotten because a serverless function may be frozen the
  // instant it responds; bounded by lib/geo/geocodeJob.js's own timeout so a
  // slow third party cannot hold the job hostage. A failure leaves nulls —
  // the job is created either way, and the pin is the only thing missing.
  if (job.siteAddress) {
    const coords = await geocodeJob(db, job);
    Object.assign(job, {
  ...
  // Waivers marked "attach to every job" (Settings › Presentation) —
  // best-effort, and with THIS db (the job may be created inside a
  // transaction): a skipped waiver must never fail a job.
  await attachDefaultWaivers({ companyId, jobId: job.id, db }).catch((err) =>
    console.error("[jobs] default waivers failed:", err?.message),
  );

  // Checklists set to "add automatically" for this job's trade
  // (lib/checklists/autoAdd.js). Same best-effort rule as the waivers.
  if (quoteId) {
    await attachAutoChecklists(db, { jobId: job.id, companyId, quoteId }).catch((err) =>
      console.error("[jobs] auto checklists failed:", err?.message),
    );
  }
