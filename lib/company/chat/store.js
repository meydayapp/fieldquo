...
/**
 * Loaded with every room in the LIST: the job, the members, and the last
 * thing somebody SAID — one row, for the preview line. Not the last 200:
 * the unread and mention counts come from a count query (withUnread below),
...
async function withUnread(client, member, rooms) {
  ...
  const seen = rooms.map((r) => ({
    id: r.id,
    lastSeenAt: (r.members || []).find((m) => m.open !== false && m.memberId === member.id)?.lastSeenAt || null,
  }));
  const notMine = notMineWhere(member);
  const scope = { companyId: member.companyId };
...
export async function roomsFor(member, { client = db } = {}) {
  ...
  return withUnread(client, member, rooms);
}
...
export async function roomFor(member, roomId, { client = db } = {}) {
  ...
  const room = await client.companyChatRoom.findFirst({
    ...
    include: {
      ...
      members: LIST_INCLUDE.members,
      messages: { orderBy: { createdAt: "desc" }, take: THREAD_TAKE, select: MESSAGE_SELECT },
    },
  });
  if (!room) return null;
 ...
 */
export async function markRoomSeen(member, roomId, { upTo = null, client = db } = {}) {
