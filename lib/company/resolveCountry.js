...
import { STRIPE_COUNTRIES } from "@/lib/pricing/ladder";
...
/**
 * Which country a company is BILLED as — the question the plan list and the
 * checkout ask before choosing a currency (lib/pricing/ladder.js
 * currencyForCountry).
 *
 * ══ Why neither function above answers it ══════════════════════════════════
 *
 * resolveCountry hears only CA and US, so an Australian company's "AU" came
 * back null and Account & Billing told it "tell us where your business is"
 * with the answer in the column. statedCountry hears the eleven signup
 * countries, but a company address picked through Google in Company Settings
 * can write any code — an Italian one lands "IT" — and since 2026-09-24 every
 * country Stripe serves is billed (the USD ladder). So: the column when it
 * names a country Stripe serves, else resolveCountry's reading of the
 * address and province, unchanged. Junk in the column ("ZZ") is still not a
 * country, and nothing is inferred.
 *
 * @returns {{ country: string|null, source: "column"|"address"|"province"|null }}
 */
export function billingCountry(record) {
  const raw = String(record?.country || "").trim().toUpperCase();
  const stated = raw === "UK" ? "GB" : raw;
  if (STRIPE_COUNTRIES.has(stated)) return { country: stated, source: "column" };
  return resolveCountry(record);
}
