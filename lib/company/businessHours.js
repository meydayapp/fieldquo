 ...
 */
export function groupHours(hours, { weekStartsOn = 0, locale = "en-CA" } = {}) {
  ...
  return runs.map((run) => {
    const first = SHORT_NAMES[run.days[0]];
    const last = SHORT_NAMES[run.days[run.days.length - 1]];
    return {
      ...run,
      label:
        run.days.length === 1
...
export function openState(hours, timezone = "America/Toronto") {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    return {
      open: false,
      opensAt: entry.open,
      opensDay: i === 0 ? null : DAY_NAMES[entry.day],
    };
  }
