...
// ── Day names live in lib/format/dayNames.js now ───────────────────────────
//
// They were defined here because the tenant website needed them first, which
// made them invisible to every other screen with a weekday in it — and the
// opening-hours EDITOR, three files away, went on printing the English table
// under a Spanish heading for exactly that reason. Moved out, re-exported
// here, so there is one implementation and every caller that already imports
// `dayNames` from this module keeps working (scripts/check-tenant-surfaces.mjs
// is one of them).
//
// DAY_LABELS / DAY_SHORT stay ENGLISH on purpose: they are the schema.org
// `dayOfWeek` IRI segments — https://schema.org/Monday is that URL in every
// language — and translating them would break the structured data that puts
// "Open ⋅ Closes 5 PM" in a Google result. Anything a PERSON reads calls
// dayNames(locale) instead.
import {
  dayNames,
  ENGLISH_DAY_NAMES,
  ENGLISH_DAY_SHORT,
} from "@/lib/format/dayNames";

export { dayNames };

const DAY_NAMES = ENGLISH_DAY_NAMES;
const SHORT_NAMES = ENGLISH_DAY_SHORT;
