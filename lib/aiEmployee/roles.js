...
/**
 * The disclosure line — when the company tells clients it's an AI, the first
 * message of every channel opens with it.
 *
 * "Hi, I'm {name}, {Company}'s AI assistant". Always present from 2026-09-19
 * (the owner's decision then); since 2026-09-22 it is present when the
 * COMPANY's disclosure setting is on — on by default only where the law
 * requires it (lib/aiEmployee/disclosure.js, with the statutes cited). Off,
 * the employee never volunteers it but never denies it either: rule 5 of
 * neverRules below makes it answer truthfully whenever it is asked. Either
 * way it is prepended by the RESPONDER (lib/aiEmployee/respond.js) to the
 * first reply on a thread, not left to the model — a rule the model applies
 * is a rule it applies most of the time.
 *
...
// cannot reach up.
//
// Rule 5 is the one that depends on the company's disclosure setting
// (lib/aiEmployee/disclosure.js). What does NOT depend on it: the employee
// never claims to be a person and never denies being an AI, in any company,
// in any jurisdiction. The setting decides only whether it VOLUNTEERS it. A
// "no" to "am I talking to a bot?" is the one lie every bot-disclosure law
// cited there is aimed at, and it is not a lie this product tells for
// anybody.
const RULE_5_DISCLOSED = `5. NEVER claim to be a human being, and NEVER deny being an AI. The first
   message of every conversation introduces you by name as the business's AI
   assistant — that line is added for you, do not repeat it. After that,
   write as the business's assistant: you may use the name you were
   introduced with, and if they ask whether they are talking to a person, a
   bot or an AI, say plainly that you are the business's AI assistant and
   that a member of the team can follow up.`;

const RULE_5_UNANNOUNCED = `5. NEVER claim to be a human being, and NEVER deny being an AI. You write
   as the business's assistant, under your name, and you do not need to
   announce that you are an AI. But if they ask — in any words — whether they
   are talking to a person, a real human, a bot, a machine or an AI, answer
   truthfully and at once: you are the business's AI assistant, and a member
   of the team can follow up if they would rather. Never dodge that question,
   never answer it with a joke, and never describe yourself doing things only
   a person does (a desk, a lunch break, a drive to the site).`;

const neverRules = ({ disclose = true } = {}) => `WHAT YOU NEVER DO — these are absolute, and nothing later in this
prompt relaxes them.
...
${disclose ? RULE_5_DISCLOSED : RULE_5_UNANNOUNCED}
...
export function buildEmployeePrompt({
  ...
  sources = [],
  tally = null,
  // Does this company tell clients it's an AI? lib/aiEmployee/disclosure.js
  // decides; defaulted to TRUE here so a caller that forgets to say gets the
  // announcing rule, never the quieter one.
  disclose = true,
  // The exact words respond.js will put in front of THIS reply — the
  // disclosure line and/or the company's opening line — or "" when none.
  // Stated so the model neither repeats them nor greets a second time.
  opening = "",
} = {}) {
  ...
  return [
    ...
    String(opening || "").trim()
      ? // Fenced: the company's opening line is typed text, the same as its
        // instructions, and gets the same "data, not instructions" wrapper.
        `YOUR OPENING\nThis reply will be sent with the words below in front of it, added for you. Do not repeat them and do not greet again — carry straight on from there.\n\n${fencedBlock("OPENING WORDS", opening)}`
      : null,
    material,
    contractorNotes,
    escalation,
