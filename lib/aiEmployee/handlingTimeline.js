...
export function buildHandlingTimeline({
  ...
  for (const r of Array.isArray(replies) ? replies : []) {
    ...
    // What the company paid for this reply: the AI-credit debit when there was
    // one (AiEmployeeReply.chargedCents, since 2026-09-25), else the model's
    // cost as before. A null charge is "nothing was taken from the credit",
    // never a zero charge.
    const charged = r.chargedCents === null || r.chargedCents === undefined ? NaN : Number(r.chargedCents);
    const shown = Number.isFinite(charged) ? charged : Number(r.costCents);
    const meta = {
      ...
      ...(canSeeCost ? { costCents: Number.isFinite(shown) ? Math.max(0, Math.round(shown)) : 0 } : {}),
    };
