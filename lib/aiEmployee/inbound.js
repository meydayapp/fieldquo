...
export async function aiEmployeeOnInbound({ companyId, threadId, messageId, platform = null, language = null }) {
  try {
    ...
    // The employee that answers this channel, if one is on. The cheap door:
    // most companies have no employee, and the ones that do mostly have it
    // off — one small read is the whole cost for them, and no model call,
    // no source read and no reply row happen at all.
    const employee = await employeeForChannel(companyId, channel);
    if (!employee) return { acted: false, reason: "disabled" };

    // A reply is a reversible act; the mode says whether it goes alone.
    const sendsAlone = mayActAlone({ mode: sendMode(employee), risk: RISK_REVERSIBLE, tainted: true });
    ...
    const result = await respondToMessage({
      ...
      channel,
      language,
      // `ask` passes no sender, so respondToMessage cannot send even if the
      ...
      send: sendsAlone ? (text) => deliver({ companyId, threadId, text }) : null,
    });
