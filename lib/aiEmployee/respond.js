...
export async function respondToMessage({
  ...
  let [employee, company, thread, message] = await Promise.all([
    ...
    prisma.company.findUnique({
      ...
      select: { id: true, name: true, businessHours: true, timezone: true, defaultLanguage: true, ...DISCLOSURE_COMPANY_SELECT },
    }),
    threadId
  ...
  const opening = firstReply
    ? [
        disclose
          ...
          : null,
        String(employee.greeting || "").trim() || null,
      ]
        .filter(Boolean)
        .join(" ")
    : "";
  ...
  try {
    const result = await runLoop({
      system: buildEmployeePrompt({ employee, company, sources: picked, tally, disclose, opening }),
  ...
  if (!dryRun && !afterHandOff && threadId && message?.sentAt && (await superseded(prisma, { threadId, messageId, sentAt: message.sentAt }))) {
    ...
    return { replied: false, reason: SKIP.BURST_MERGED, mode: verdict.mode, replyId, tools };
  }
  ...
  if (!text) {
    return { replied: false, reason: "no_text", mode: verdict.mode, replyId, tools, proposalIds };
  }
