...
export async function respondToMessage({
  ...
  const [employee, company, thread, message] = await Promise.all([
    // The employee that answers THIS channel — or, for the test box, the
    // named one, still read under companyId. An employee switched off is
    // null here, and null is the NO_EMPLOYEE verdict: nothing routes to it,
    // nothing is proposed for it, nothing is metered against it.
    employeeId
