...
/**
 * The front desk's one call. Metered like every other call — checked BEFORE,
 * recorded AFTER — through meterFor("ai_employee_front_desk"), on the
 * standard tier. Like the replies it is the company's to pay, from its AI
 * credit (lib/ai/walletMeter.js), charged once per thread: the debit's ref is
 * the thread id, because a thread is classified once.
 *
 * Never throws: a vendor failure or an empty answer is `other`, with the
 * reason saying so, and the thread still gets an assignee.
 */
export async function classifyIntent({ companyId, text, threadId = null, prisma = db, deps = {} }) {
  const {
  ...
  try {
    const meter = await meterForFn(FRONT_DESK_FEATURE, {
      companyId,
      prisma,
      now,
    ...
    if (usage) {
      await meter
        .record(usage, {
          ref: threadId ? `${FRONT_DESK_FEATURE}:${threadId}` : null,
          note: "AI employee front desk — routing a new conversation",
        })
        .catch(() => {});
    }
...
export async function assignThread({ companyId, thread, channel, text, prisma = db, deps = {}, now = new Date() }) {
  ...
  if (!intent) {
    if (needsClassification(team)) {
      const verdict = await classifyIntent({ companyId, text, threadId: thread.id, prisma, deps });
