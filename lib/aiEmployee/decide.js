...
import { modeOf, MODE_ASK } from "./permission";
...
export const SKIP = Object.freeze({
  ...
  NO_CREDIT: "no_credit",
  // WhatsApp, Facebook and Instagram (the last two since 2026-09-22 — before
  // that this guard never fired on a Messenger thread, because respond.js only
  // computed the window for WhatsApp). The employee may not send free text
  // more than 24 hours after the customer last wrote — see
  // lib/messaging/serviceWindow.js. It is
  ...
  // ── Routing (lib/aiEmployee/routing.js) ────────────────────────────────
  // A person replied from Conversations and the thread is theirs until "Let
  // {name} continue" is pressed. Sticky, unlike HUMAN_REPLIED.
  HUMAN_TOOK_OVER: "human_took_over",
  // The thread is held by another employee. Refused at respondToMessage's
  // door, which is what makes "one employee per conversation" a fact.
  NOT_ASSIGNEE: "not_assignee",
  // A newer message from the same person arrived while this one waited or
  // was being composed; the newer one's turn answers both.
  BURST_MERGED: "burst_merged",
});
...
/**
 * The mode, read through lib/aiEmployee/permission.js.
 *
 * Used to be a boolean read as `autoReplyEnabled === true`; it is now the
 * three-word `mode` column, and modeOf() applies the same rule the boolean
 * did — an absent, null or unrecognised value is the timid one (`ask`), never
 * a truthier guess. MODE_SUGGEST is kept as a name for `ask` so a reader of
 * an older AiEmployeeReply row's screen wording still finds it.
 */
export { MODE_ASK, MODE_ACCEPT_EDITS, MODE_AUTO } from "./permission";
export const MODE_SUGGEST = MODE_ASK;

export function sendMode(employee) {
  return modeOf(employee);
}
 ...
 * @param humanTookOver MessageThread.humanTookOverAt is set — a member wrote
 *                      on this thread and nobody has pressed "Let {name}
 *                      continue" since. Read straight off the thread row;
 *                      it is a fact, not an inference.
 ...
 *                      it runs. TRUE is the default because FieldQuo's own
 *                      channels (web chat, SMS) have no window — a website
 *                      chat must not be silenced by Meta's rule. All three
 *                      of Meta's platforms pass a computed value.
 *
...
export function shouldReply({
  ...
  // A person took the thread. Checked before humanReplied because it is the
  // broader claim: humanReplied says somebody answered THIS message, this
  // says the conversation is theirs until they say otherwise.
  if (humanTookOver) return no(SKIP.HUMAN_TOOK_OVER);
  ...
  // ── LAST, and absolute ──────────────────────────────────────────────────
  //
  // The 24-hour customer service window — WhatsApp's, and Messenger's and
  // Instagram's standard messaging window. Placed at the end because every
  // guard above it is a decision somebody made — a setting, a cap, a status —
  // and this one is a rule of Meta's that no setting can override. A reply
  // composed past it would be a model call billed for a message Meta was
  // always going to refuse (WhatsApp error 131047; the Send API's
  // outside-window error on a Page), so it is stopped BEFORE the call rather
  // than failing at the send.
  //
  // Explicit `=== false`, matching the business-hours guard above it: `null`
  // or `undefined` here means "nobody computed a window", which is the web
  // chat and SMS, and reading that as closed would silence the employee on
  // channels that never had this rule.
  //
  // The employee has no template path, and on Facebook and Instagram no
  // HUMAN_AGENT tag either (not a permission FieldQuo holds). That is
  // deliberate and is the honest limit: a message outside the window is a
  // business-initiated one with a policy attached, and choosing to send one is
