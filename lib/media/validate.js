...
// ══ The WIDER document set, for an authenticated messaging attachment ═══════
//
// Deliberately a second set rather than an addition to the one above, and the
// paragraph above is the whole reason: DOCUMENT_TYPES also guards the PUBLIC,
// unauthenticated self-quote upload, where every entry is attack surface a
// stranger can reach and "no customer has their kitchen plan in Word" is still
// true.
//
// A contractor answering a homeowner on WhatsApp is a different situation.
// They are signed in, they are inside their own company, and the thing they
// are holding is a quote in .docx or a material list in .xlsx — the formats
// WhatsApp itself accepts and, until this change, the ones the file picker
// silently refused while WHATSAPP_MEDIA_LIMITS.document happily listed them.
// That mismatch was the dead control in reverse: the send path could carry a
// spreadsheet the picker would not let anybody choose.
//
// So this is exactly Meta's own list (see lib/messaging/whatsappMediaLimits.js
// for the citation) and nothing else. Not ZIP, not CAD, not the QuickBooks
// exports the migration service takes — those have their own classifier for
// their own route, and widening by analogy is how an allowlist stops meaning
// anything.
export const MESSAGING_DOCUMENT_TYPES = new Set([
  "application/pdf",
  "text/plain",
  "application/msword",
...
// The extension each of those wants on its Cloudinary `raw` public_id, for the
// reason uploadPublicId gives below: a raw asset is served with the
// Content-Type its extension implies, and one with none downloads as a mystery
// file. Keyed by MIME because that is what the browser hands us; the uploaded
// FILENAME is never used to build a public_id — see uploadPublicId.
...
// ── The ceiling that actually binds, and why it is smaller than all of them ─
//
// Every limit below is enforced INSIDE the upload route. On Vercel the route
// never runs for a body over ~4.5 MB: the platform answers 413 at the edge,
// with no JSON in it, so the browser got `data === null` and the uploader
// printed its last-resort sentence — "That file couldn't be uploaded." with
// no reason. A phone photo is routinely 3–12 MB, so the product advertised a
// 15 MB photo limit and refused most photos without saying why. That is a
// control that appears to work and does not.
//
// Nothing here CHANGES what is accepted — the server's limits are unchanged.
// This is the number the browser needs in order to say the true thing before
// and after a refusal. It is Vercel's documented Serverless Function request
// body limit, stated once here rather than guessed at three call sites.
export const UPLOAD_REQUEST_MAX_BYTES = Math.floor(4.5 * 1024 * 1024); // 4.5 MB — Vercel's request body cap

/** "4.5 MB" / "0.8 MB" — one decimal, because "0 MB" is not a size. */
...
/**
 * Is this file bigger than one request may carry? Pure, and the same answer
 * the browser gives before uploading and after a 413 comes back.
 */
export function overRequestLimit(size) {
  return Number(size || 0) > UPLOAD_REQUEST_MAX_BYTES;
}
...
// A messaging document gets WhatsApp's own ceiling instead of the 25 MB above,
// and the number is Meta's rather than one of ours: WHATSAPP_MEDIA_LIMITS
// .document.maxBytes is 100 MB, the send path enforces exactly that, and an
// upload boundary that refused at 25 MB would mean the composer accepted a
// file the picker had already offered and the send would have carried. It is
// the same ceiling VIDEO_MAX_BYTES already sits at, so nothing new is being
// asked of Cloudinary or of the function's memory.
export const MESSAGING_DOCUMENT_MAX_BYTES = 100 * 1024 * 1024; // 100 MB — Meta's
 ...
 * @param {{allowLogo?: boolean, allowMessagingDocuments?: boolean}} [opts]
 *   allowLogo permits SVG — authenticated branding only. Defaults false:
 *   client media never accepts a vector.
 *   allowMessagingDocuments widens the document set to Meta's own eight
 *   formats at WhatsApp's own 100 MB ceiling — authenticated messaging
 *   attachments only, and for the reason MESSAGING_DOCUMENT_TYPES gives. Both
 *   default false, so every existing caller (including the two public ones) is
 *   unchanged.
 * @returns {{ ok: boolean, kind?: "photo"|"video"|"document", resourceType?: "image"|"video"|"raw", mimeType?: string, error?: string }}
 */
export function classifyMedia(file, { allowLogo = false, allowMessagingDocuments = false } = {}) {
  ...
  const isDocument =
    DOCUMENT_TYPES.has(type) || (allowMessagingDocuments && MESSAGING_DOCUMENT_TYPES.has(type));
  ...
  if (!isPhoto && !isVideo && !isDocument) {
    ...
    return {
      ok: false,
      error: allowMessagingDocuments
        ? "Upload a photo (JPEG, PNG, HEIC…), a video (MP4, MOV, WebM), or a PDF, Word, Excel, PowerPoint or text document."
  ...
  if (size > cap) {
    ...
    // "PDF" only when a PDF is the only document this call accepts. Telling
    // somebody holding a 120 MB .pptx that "that PDF is too large" is a
    // refusal about a file they are not holding.
    const noun =
      kind === "document" ? (allowMessagingDocuments ? "document" : "PDF") : kind;
  ...
  // The MIME carried out as well, so uploadPublicId can put the right
  // extension on a raw asset without the caller re-reading `file.type` — which
  // is how the two would drift and a .xlsx would land as `something.pdf`.
  return { ok: true, kind, resourceType, mimeType: type };
}
 ...
 *
 * The EXTENSION follows the MIME type when one is given, so a .docx does not
 * land as `something.pdf` and download as a corrupt PDF. It defaults to `pdf`
 * when nothing is passed, which is what every caller before the messaging
 * attachment widened this got and still gets.
 *
 * @param {string} kind  a `kind` from classifyMedia.
 * @param {{randomId?: function, mimeType?: string}} [opts]
 * @returns {string|undefined}  a public_id, or undefined to let Cloudinary pick.
 */
export function uploadPublicId(
  kind,
  { randomId = () => globalThis.crypto.randomUUID(), mimeType = null } = {},
) {
  if (kind !== "document") return undefined;
  const ext =
    DOCUMENT_EXTENSIONS[String(mimeType || "").toLowerCase().split(";")[0].trim()] || "pdf";
  return `${randomId()}.${ext}`;
}
...
// ══ The migration-service upload boundary — a SEPARATE, wider allowlist ═════
//
// classifyMedia() above is deliberately narrow (photo/video/one-PDF) and its
// own comment says why: it also guards PUBLIC, unauthenticated endpoints, so
// every entry it accepts is attack surface a stranger can reach. Widening it
// to take QuickBooks/Jobber exports would put that surface on the self-quote
// upload too, for a need that endpoint doesn't have.
//
// The paid data-migration service's document upload is a different shape of
// problem: authenticated only (a billing admin at a company with a
// MigrationRequest already accepted), never served back to the public, and
// its entire reason to exist is accepting exactly the export formats
// classifyMedia refuses — QuickBooks (.qbo/.qbb/.iif), spreadsheets
// (.csv/.xls/.xlsx), or a zipped bundle of them.
//
// MIME sniffing is unreliable for these: browsers commonly report
// `application/octet-stream` (or nothing at all) for .qbo/.qbb/.iif, which
// classifyMedia's "no type -> reject" rule would refuse outright even though
// the file is exactly what this route wants. So this classifier trusts the
// FILE EXTENSION first and the MIME type second, rather than requiring a
// MIME match the way the public classifier does — a deliberate loosening,
// scoped to a route no stranger can reach.
export const MIGRATION_DOCUMENT_EXTENSIONS = new Set([
  "csv", "xls", "xlsx", "txt", "tsv",
  "qbo", "qbb", "qbx", "qbw", "iif", // QuickBooks
  "zip",
  "pdf",
]);

export const MIGRATION_DOCUMENT_MIME_TYPES = new Set([
  "text/csv",
  "application/csv",
  ...
  "application/pdf",
  // What browsers actually send for the QuickBooks formats — effectively
  // "unknown binary". Accepted here ONLY because the extension is also
  // checked; a bare octet-stream with a disallowed or missing extension is
  // still refused below.
  "application/octet-stream",
]);

export const MIGRATION_DOCUMENT_MAX_BYTES = 25 * 1024 * 1024; // 25 MB, same ceiling as classifyMedia's documents
...
/**
 * Classify one migration-service document upload.
 *
 * @param {{type?: string, size?: number, name?: string}} file
 * @returns {{ ok: boolean, resourceType?: "raw", error?: string }}
 */
export function classifyMigrationDocument(file) {
  const ext = extensionOf(file?.name);
  const type = typeof file?.type === "string" ? file.type.toLowerCase() : "";
  const size = Number(file?.size);

  const extOk = MIGRATION_DOCUMENT_EXTENSIONS.has(ext);
  const typeOk = type !== "" && MIGRATION_DOCUMENT_MIME_TYPES.has(type);

  // The extension is load-bearing here (unlike classifyMedia, which never
  // trusts a filename) precisely because the MIME type is the unreliable
  // half for these formats. Either signal being wrong on its own is fine;
  // both being wrong (or absent) is refused.
  if (!extOk && !typeOk) {
    return {
      ok: false,
      error:
        "Upload a QuickBooks export, a spreadsheet (CSV/XLS/XLSX), a ZIP, or a PDF.",
    };
  }
  ...
  if (size > MIGRATION_DOCUMENT_MAX_BYTES) {
    return {
      ok: false,
      error: `That file is larger than ${MB(MIGRATION_DOCUMENT_MAX_BYTES)}. Split the export or zip it first.`,
    };
  }

  // Everything here goes up as "raw" — Cloudinary stores the bytes untouched,
  // which is right for a spreadsheet or a proprietary export it has no reason
  // to try to rasterise (same reasoning classifyMedia gives for PDFs).
