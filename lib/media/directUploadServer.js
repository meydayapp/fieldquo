...
import { planUpload, cloudinarySignature, decideVerify } from "@/lib/media/directUpload";
...
export async function verifyResponse(scope, body) {
  ...
  // The decision lives in lib/media/directUpload.js (decideVerify), with the
  // Admin API lookup and the error log injected, so the check script runs the
  // SAME path — including the rate-limited fallback — without a network.
  const out = await decideVerify({
    ...
    lookup: lookupAsset,
    record: recordUploadEvent,
  });
  return NextResponse.json(out.body, { status: out.status });
}

// The SDK's own `timeout` option only arms a socket timer — it never rejects —
// so a stalled lookup would hold the verify (and the person's upload) for as
// long as the platform lets the function run. Raced here instead, and a
// timeout is "transient" to lookupFailureKind.
const LOOKUP_TIMEOUT_MS = 8000;
let lastNearCapWarning = 0;

async function lookupAsset(publicId, resourceType) {
  let timer;
  const timeout = new Promise((_, reject) => {
    timer = setTimeout(() => {
      const err = new Error("Admin API lookup timed out");
  ...
  try {
    const asset = await Promise.race([
      cloudinary.api.resource(publicId, { resource_type: resourceType, type: "upload" }),
      timeout,
    ]);
    warnWhenNearCap(asset);
    return asset;
  } finally {
    clearTimeout(timer);
  }
...
/**
 * Cloudinary's rate-limit headers, as the SDK surfaces them on a successful
 * answer. Under 10% of the hour's allowance left is logged once an hour per
 * server instance — the notice BEFORE the fallback starts carrying uploads.
 */
function warnWhenNearCap(asset) {
  const allowed = Number(asset?.rate_limit_allowed);
  const remaining = Number(asset?.rate_limit_remaining);
  if (!Number.isFinite(allowed) || !Number.isFinite(remaining) || allowed <= 0) return;
  if (remaining > allowed * 0.1) return;
  const now = Date.now();
  if (now - lastNearCapWarning < 60 * 60 * 1000) return;
  lastNearCapWarning = now;
  recordError({
    area: "upload",
    code: "admin_api_near_cap",
    message: `Cloudinary Admin API: ${remaining} of ${allowed} calls left this hour. Upload verification falls back to signature-only when they run out.`,
    detail: { allowed, remaining, resetAt: asset?.rate_limit_reset_at || null },
  });
}

/**
 * The error-log side of decideVerify. A signature-only acceptance is written
 * to PlatformErrorLog (area "upload", code "upload_signature_only") — one row
 * per upload, with the company, so /platform/errors counts them and names who
 * was affected. Never throws (recordError's contract).
 */
async function recordUploadEvent(event) {
  if (event.type === "accepted_on_signature") {
    console.warn("[upload/verify] accepted on signature only:", event.reason, event.publicId);
    await recordError({
      area: "upload",
      code: "upload_signature_only",
      message:
        event.reason === "rate_limited"
          ? "Cloudinary's Admin API was rate-limited; an upload was accepted on its signature alone (size not re-read)."
          : "Cloudinary's Admin API lookup failed transiently; an upload was accepted on its signature alone (size not re-read).",
      companyId: event.companyId || null,
      detail: { reason: event.reason, publicId: event.publicId, resourceType: event.resourceType, lookup: event.lookup },
    });
  } else if (event.type === "lookup_failed") {
    console.error("[upload/verify] Admin API lookup failed:", event.lookup);
  } else if (event.type === "refused_after_lookup") {
    ...
    console.warn("[upload/verify] refused", event.code, event.publicId);
  }
}
