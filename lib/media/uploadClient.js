...
export async function uploadFile(file, { endpoint = "/api/upload", purpose, filename, onProgress, fetchImpl } = {}) {
  ...
  const verified = await postJson(doFetch, route.verify, {
    ...
    // Unsigned, so never trusted for a decision: it only picks the delivery
    // extension if the server has to accept on the signature alone (Admin API
    // rate-limited) — see judgeOnSignature.
    format: stored.data.format,
    filename: name,
  });
  ...
  return {
    ...
    // null stays null — Number(null) is 0, and "0 bytes" beside a file reads
    // as a failed upload. A signature-only verify has no byte count.
    bytes: typeof d.bytes === "number" && Number.isFinite(d.bytes) && d.bytes > 0 ? d.bytes : null,
    mimeType: type || null,
  };
}
