...
/**
 * A facet at or under this pitch is flat for drainage purposes: its azimuth
 * is a placeholder, and nothing about which edge is "down" can be read off
 * it. 3° is under 1/12. Read by the gutter measurement.
 */
export const FLAT_PITCH_DEGREES = 3;

/**
 * Share of the roof's ground area on facets at or under FLAT_PITCH_DEGREES.
 * Pure; 0 when there are no usable facets.
 */
export function lowSlopeShare(segments) {
  let flat = 0;
  let total = 0;
  for (const seg of Array.isArray(segments) ? segments : []) {
    ...
    if (a <= 0) continue;
    total += a;
    if ((Number(seg?.pitchDegrees) || 0) <= FLAT_PITCH_DEGREES) flat += a;
  }
 ...
 *    freezing climate and neither is optional, so both are counted — and the
 *    eaves get TWO courses. The field is linear feet of 36"-wide membrane
 *    laid, which is what the sell rate, the crew-hours and the roll count
 *    all multiply, so a 6 ft band at the eave is two feet of membrane per
 *    foot of eave. OBC 9.26.5.1 (and NBC / Quebec 9.26.5.1, word for word)
 *    wants eave protection "a minimum of 900 mm up the roof slope to a line
 *    not less than 300 mm inside the inner face of the exterior wall": with
 *    an 18" overhang and a 2x6 wall that is 40" along a 6/12 slope, 43" at
 *    8/12, 46" with a 24" overhang — one 36" course never reaches, so
 *    Ottawa and Gatineau roofers lay the first 6 ft (Firon Roofing, Peace of
 *    Mind Exteriors), and real quotes read "6 ft of ice and water all around
 *    the roof including valleys". The valley gets one course centred on it.
 *    GTA quotes at 3 ft on ≤6/12 exist; a company that runs one course
 *    halves the eave half of the field. Arithmetic and sources in
 *    docs/research/ROOFING-RATES-2026.md §4.
 ...
 */
/** Courses of 36" membrane at the eave. Two: 6 ft, see takeoffPatch(). */
export const ICE_WATER_EAVE_COURSES = 2;

export function takeoffPatch(linears) {
  if (!linears) return null;
  return {
    iceWaterFt: ICE_WATER_EAVE_COURSES * linears.eaveFt + linears.valleyFt,
 ...
 */
/**
 * The English sentences, as `t(key, fallback, vars)` fallbacks: the builder
 * hands in the app catalogue's `t` so a French or Spanish estimator reads
 * the summary in their own language (app.roof.sum.* in appMessages.js, nine
 * languages); a caller with no `t` — the check script — gets English.
 */
const plainT = (key, fallback, vars = {}) => fallback.replace(/\{(\w+)\}/g, (m, k) => (k in vars ? String(vars[k]) : m));

export function summarise(linears, t = plainT) {
  if (!linears) return null;
  const shapeKey = {
    simple_gable: "gable",
    gable: "gable",
  ...
  // homeowner without re-reading it — so the article travels with the shape
  // word, per language, rather than being computed from an English initial.
  const shape = {
    gable: t("app.roof.shape.gable", "a gable roof"),
    hip: t("app.roof.shape.hip", "a hip roof"),
    intersecting: t("app.roof.shape.intersecting", "an intersecting roof"),
  }[shapeKey];
  const n = linears.directions.length;
  const ways = t("app.roof.sum.ways", `${n} way${n === 1 ? "" : "s"}`, { value: n });
  return {
    headline: t("app.roof.sum.headline", "{facets} facets facing {ways} — read as {shape}.", { facets: linears.facets, ways, shape }),
    measured: [
      t("app.roof.sum.perimeter", "{ft} ft around the building", { ft: linears.perimeterFt }),
      t("app.roof.sum.eaveRake", "{eave} ft of eave and {rake} ft of rake", { eave: linears.eaveFt, rake: linears.rakeFt }),
      t("app.roof.sum.internal", "{ft} ft where facets meet", { ft: linears.internalFt }),
    ],
    derived: [
      t(
        "app.roof.sum.derived",
        "Ridge {ridge} ft, hip {hip} ft and valley {valley} ft are that {internal} ft split the way {shape} usually splits it. Nothing in the roof model says which side of a shared edge slopes away from it.",
        { ridge: linears.ridgeFt, hip: linears.hipFt, valley: linears.valleyFt, internal: linears.internalFt, shape },
      ),
    ],
    cannotKnow: [
      t("app.roof.sum.cannot.layers", "Layers to strip, and how much sheathing is bad — nobody can see either from above."),
      t("app.roof.sum.cannot.penetrations", "Vent boots, box vents, skylights and chimneys — count them off the satellite image."),
      t("app.roof.sum.cannot.flashing", "Step flashing, which is roof meeting wall. The roof model has no walls in it."),
      t("app.roof.sum.cannot.storeys", "Storeys, which change the labour but not the materials."),
    ],
  };
}
