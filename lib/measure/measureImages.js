...
//     this file build the outline still from them);
 ...
 */
export function measureImageSource(takeoff) {
...
export async function withCapturedMeasureImages(scopeGroups, { companyId } = {}) {
  ...
  for (const g of scopeGroups) {
    ...
    const source = measureImageSource(next);
 ...
 * still and its caption, or null. This — and only this — is what the public
 * quote route and the document sections read off a takeoff.
 */
