...
import { roofLinears, lowSlopeShare, unionBox } from "@/lib/measure/roofGeometry";
import { DEFAULT_ROOF_ZOOM, MAX_TILE_PX, DEFAULT_SCALE, zoomToFitBox, stillFrame } from "@/lib/measure/imageScale";
...
export async function geocodeAddress(address, key = serverMapsKey()) {
  ...
  try {
    ...
    return {
      ...
      // The raw label too, for callers with a different bar than "rooftop":
      // lib/geo/geocodeJob.js accepts a street-interpolated pin (fine for a
      // 250 m fence) but refuses APPROXIMATE, which is a town centroid.
...
export function summariseRoof(insights) {
  ...
  return {
    ...
    // How much of the roof is flat (≤ 3°). A gutter hangs on an eave, and a
    // flat facet has no eave — its azimuth is a placeholder — so the gutter
    // measurement reads the whole outline instead when this is most of the
    // roof. Computed here, from the segments this function already walks,
    // rather than by a second pass over the raw response somewhere else.
    lowSlopeShare: lowSlopeShare(segments),
  };
}
 ...
 *
 * Zoom 19 and 640×640 since 2026-09-18 (was 20 and 640×400): the old frame
 * clipped the eaves of anything over ~50 ft deep, and the owner could not see
 * the edge of the roof. See DEFAULT_ROOF_ZOOM in imageScale.js for the
 * numbers. `marker` defaults ON here — the pin is how a client sees which
 * house was measured — and the paving still (satellite.js) defaults it off.
 */
export function satelliteImageUrl(
  lat,
  lng,
  { zoom = DEFAULT_ROOF_ZOOM, size = `${MAX_TILE_PX}x${MAX_TILE_PX}`, scale = DEFAULT_SCALE, marker = true } = {},
) {
  ...
  return (
    ...
    `&maptype=satellite` +
    (marker ? `&markers=color:0xffffff00%7C${lat},${lng}` : "") +
    `&key=${key}`
  );
}

/**
 * The still a roof measurement is shown on: centred on the pin, at the
 * requested zoom unless Solar's bounding box says the roof would clip, in
 * which case one step out at a time until it fits (zoomToFitBox). PURE.
 *
 * Returns the frame the takeoff stores (lat, lng, zoom, scale, width,
 * height, marker) plus `requestedZoom` and `zoomedOutToFit`, so the panel
 * can say "Zoomed out to fit the roof" rather than silently showing a wider
 * picture than the one asked for. Never null for a valid pin: a roof with no
 * usable box gets the requested zoom, which is what it got before.
 */
export function roofStillFrame(insights, location, requestedZoom = DEFAULT_ROOF_ZOOM) {
  const box = insights?.boundingBox || unionBox(insights?.solarPotential?.roofSegmentStats) || null;
  const fit = zoomToFitBox({
    box,
    center: location,
    zoom: requestedZoom,
    width: MAX_TILE_PX,
  ...
  return { ...frame, requestedZoom: fit.requestedZoom, zoomedOutToFit: fit.steppedOut, fits: fit.fits };
}
 ...
 */
export async function measureRoof(address, { zoom = DEFAULT_ROOF_ZOOM } = {}) {
  ...
  if (!insights) {
    ...
    // the size manually.
    const still = roofStillFrame(null, { lat: geo.lat, lng: geo.lng }, zoom);
    return {
      ok: false,
      ...
      still,
      satelliteImageUrl: satelliteImageUrl(geo.lat, geo.lng, still ? { zoom: still.zoom } : {}),
    };
  }
  ...
  // The frame the still is served in — stepped out from `zoom` if the roof
  // Solar measured would not fit inside it. Same object the takeoff stores.
  const still = roofStillFrame(insights, { lat: geo.lat, lng: geo.lng }, zoom);
