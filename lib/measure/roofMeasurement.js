...
export function summariseRoof(insights) {
  ...
  for (const seg of segments) {
    ...
    if (segM2 <= 0) continue;
    const { rise } = degreesToPitch(seg?.pitchDegrees);
    byPitch.set(rise, (byPitch.get(rise) || 0) + segM2);
    weightedDegSum += (Number(seg?.pitchDegrees) || 0) * segM2;
    weightTotal += segM2;
  }

  let predominantRise = null;
  let heaviest = -1;
  for (const [rise, m2] of byPitch) {
    if (m2 > heaviest) {
      heaviest = m2;
  ...
  const avgDegrees = weightTotal > 0 ? weightedDegSum / weightTotal : 0;
  const predominant =
    predominantRise != null
      ? { rise: predominantRise, run: 12, degrees: Math.round(avgDegrees * 10) / 10 }
      : degreesToPitch(avgDegrees);
