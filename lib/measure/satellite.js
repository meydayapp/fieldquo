...
export function staticSatelliteUrl(input = {}, key = serverMapsKey()) {
  ...
  // No marker unless asked. A pin sits over the middle of the property and
  // hides whatever is underneath it, which on a driveway job is the thing
  // being measured. The roof still asks for one: nothing is traced on it, and
  // the pin is how the client sees which house was measured.
  if (input?.marker) params.set("markers", `color:0xffffff00|${req.lat},${req.lng}`);
  return `https://maps.googleapis.com/maps/api/staticmap?${params.toString()}`;
}

// satelliteProxyPath() — the same-origin, KEYLESS path the browser loads the
// image from — moved to lib/measure/imageScale.js on 2026-09-18 so the measure
// panels can build it for a zoom the estimator tapped without a round trip.
// Re-exported above, unchanged, for every caller that imports it from here.
...
export async function measureFromAddress(address, options = {}) {
  ...
  const request = {
    ...
    marker: Boolean(options.marker),
  };

  const scale = imageScale(request);
  const url = satelliteProxyPath(request);
  // The request as the takeoff will store it, so a reopened quote rebuilds
  // this exact still from arithmetic — no second geocode, no guessed zoom.
  const frame = stillFrame(request);
