...
const ENTRIES = [
  ...
  },
  {
    ...
    label: "AI marketing image generation",
    blurb:
      "Generates one image per creative from a prompt, with or without a " +
      "reference photo — IMAGE_GENERATION_CENTS off the company's AI " +
      "credit wallet.",
    // Hidden, unlike everything else in this file: the canvas editor that
    // would actually call this endpoint is separate, unshipped work (see the
    // AI image-generation build notes). The generation route itself is real
    // and fully wired, but shipping it "on" with no product surface pointing
    // at it would mean a billable endpoint reachable only by someone who goes
    // looking for it — the exact half-built exposure AGENTS.md warns against.
    // Flip it on deliberately, in the platform console, once the editor lands.
    defaultState: "hidden",
    adoptionField: null,
    navKeys: [],
    routePrefixes: [],
    apiPrefixes: ["/api/marketing/designer/images"],
    apiExempt: [],
    cronPaths: [],
    spends: true,
  },
