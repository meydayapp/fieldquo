...
const ENTRIES = [
  ...
  },
  {
    ...
    label: "Crew chat",
    blurb:
      "The company talking to itself: #general for everyone on the roster, a " +
      "room per active job for the crew booked on it and the office, and " +
      "direct messages. Nothing leaves the company; FieldQuo's own staff chat " +
      "is a different set of tables.",
    // "on", not "hidden": the header's rule for a feature built in future is
    // about UNFINISHED things, and this one ships whole — list, rooms,
    // mentions, push, the mobile tab. A company that does not want its crew
    // chatting is a decision the console records here by flipping it.
    defaultState: "on",
    // No per-company adoption boolean: there is nothing to switch on beyond
    // having a roster. The rooms follow from Member and Job rows that exist
    // already.
    adoptionField: null,
    navKeys: ["app.nav.chat"],
    routePrefixes: ["/app/chat"],
    apiPrefixes: ["/api/chat"],
    apiExempt: [],
    cronPaths: [],
    // A push notification about a mention costs FieldQuo nothing at a
    // vendor: Web Push is the browser's own service. No model call anywhere
    // in this slice.
    spends: false,
  },
  {
  ...
  {
    key: "ai_material_list",
    label: "AI material list",
    blurb:
      "The grouped \"nothing forgotten\" material list built from a job's " +
      "approved quote and takeoff — MATERIAL_LIST_CENTS off the company's AI " +
      "credit wallet per build, not the plan's included allowance. The free " +
      "\"Rebuild from the quote\" stays free.",
    defaultState: "on",
    adoptionField: null,
    navKeys: [],
    // A button inside the job page's Materials card
    // (app/components/jobs/JobMaterials.js), which lives under a route no
    // feature claims. Only the paid build route is gated.
    routePrefixes: [],
    apiPrefixes: ["/api/jobs/[id]/materials/build"],
    apiExempt: [],
    cronPaths: [],
    spends: true,
  },
  {
    ...
    //
    // /api/designer/copy (job-context caption/hashtag generation, added once
    // the AI text bridge landed) is listed explicitly for the same reason —
    // it's a billable complete() call, and this array is matched by EXACT
    // prefix, not a "/api/designer" wildcard, so it would otherwise run
    // ungated even while the feature is hidden or locked.
    //
    // /api/marketing/designer/job-post composes a post out of a job's photos
    // and asks the model for a headline and caption on the way — a billable
    // complete() call and a design write, so it belongs on the same list as
    // the two it sits between rather than being the one door left open when
    // this feature is withheld.
    apiPrefixes: [
      ...
      "/api/marketing/designer/job-post",
    ],
    apiExempt: [],
    cronPaths: [],
    spends: true,
  },
  {
    ...
    // Money flow (income, expenses, what's left, the daily chart) is a
    // SECTION of this same page, not a second dashboard — see the header of
    // app/app/analytics/kpis/page.js. It reads from its own route,
    // /api/analytics/money-flow, so it belongs in the list here too: without
    // it, hiding or locking this feature for a company would still leave a
    // reachable, ungated endpoint quietly serving the exact figures the
    // hidden page was supposed to withhold.
    apiPrefixes: ["/api/analytics/kpis", "/api/analytics/money-flow"],
    apiExempt: [],
    cronPaths: [],
    spends: false,
  },
  {
    key: "page_messaging",
    label: "Facebook & Instagram messages",
    blurb:
      "The contractor's Facebook Page and Instagram business conversations, " +
      "answered from FieldQuo, stored, and rolled up once a month into which " +
      "conversations became jobs and how fast each was answered. Page and " +
      "business accounts only — never anybody's personal messages.",
    // ── Why "preview" and not "hidden" ──────────────────────────────────
    //
    // This file's own header says a feature built in FUTURE should default to
    // hidden, "because for an unfinished thing 'nobody has said anything'
    // genuinely does mean 'not ready'". The rule is right and this is not that
    // case, which is why the deviation is written down rather than made
    // quietly.
    //
    // Nothing here is unfinished. The inbox, the conversation, the outcome
    // control, the send path, the signed webhook and the monthly review are
    // all built and all real. What is missing is a PERMISSION FROM META:
    // pages_messaging is not approved for this app yet (lib/meta/client.js,
    // META_MESSAGING_SCOPE), and Meta will not review it until they can see
    // the feature working. Hiding the screen would hide the one surface that
    // explains that — and would leave nothing for a reviewer to look at.
    //
    // `preview` is exactly the state for this: usable, and LABELLED as such.
    // The screen says in one sentence that FieldQuo is waiting on Meta, the
    // composer is disabled with the same reason on it, and no conversation is
    // invented for a real company. Flip to "on" when the approval lands.
    defaultState: "preview",
    // No per-company adoption boolean. Unlike the crew inbox (a number
    // somebody claims) there is nothing for a contractor to switch on beyond
    // connecting a Page — and a connected Page is a MessagingChannel row, not
    // a Company column, because a company can have several.
    adoptionField: null,
    navKeys: ["app.nav.messages"],
    routePrefixes: ["/app/messages"],
    // Both trees. "/api/messaging" is the back-office half (list, thread,
    // reply, monthly review); "/api/meta/messaging" is Meta's own webhook,
    // which is exempt below. Listing only the first would leave the webhook
    // outside anything this key claims, which is how a route ends up gated by
    // nobody.
    apiPrefixes: ["/api/messaging", "/api/meta/messaging"],
    apiExempt: [
      {
        path: "/api/meta/messaging/webhook",
        guard:
          "X-Hub-Signature-256 HMAC-SHA256 over the raw body, keyed with " +
          "META_APP_SECRET and compared timing-safely in " +
          "lib/messaging/webhookSignature.js; refuses when the secret is " +
          "unset. The tenant is resolved by looking Meta's Page id up in our " +
          "own MessagingChannel rows, never from the payload.",
        reason:
          "Meta delivering a message a homeowner has ALREADY sent. There is " +
          "no session — Meta has no FieldQuo login — and refusing it at the " +
          "door would lose the enquiry permanently rather than protect " +
          "anything, the same argument /api/crew/inbound makes for an " +
          "inbound SMS. A withheld feature hides the INBOX, which is what " +
          "the gate on the pages and the other routes does.",
      },
    ],
    // Bringing a snoozed conversation back on the date somebody parked it
    // until. Listed here because a Snooze control with no scheduler behind it
    // is a lead deleted with a friendly label on it — see the route's header.
    // Two. The snooze one brings a parked conversation back; the media one
    // fetches the picture a homeowner attached — WhatsApp hands the webhook a
    // media id and no bytes, and Messenger/Instagram hand it a CDN link that
    // expires, so neither is a picture until something re-hosts it. Both are
    // listed here because a control with no scheduler behind it (a Snooze
    // button, an attachment that says "still arriving") is the dead control
    // AGENTS.md's first rule forbids.
    cronPaths: [
      "/api/cron/messaging-snooze",
      "/api/cron/messaging-media",
      // The backstop for the connect-time pull of a Page's existing
      // conversations — see app/api/cron/messaging-import/route.js.
      "/api/cron/messaging-import",
    ],
    // Messaging costs FieldQuo nothing at a vendor: Meta charges nothing to
    // receive or send a Page message, and there is no model call in this
    // slice. If an AI summary is ever added to the monthly review it must go
    // through lib/ai/provider.js, and this becomes true on the same day.
    spends: false,
  },
  {
    ...
    blurb:
      "The contractor's own WhatsApp Business number, connected through Meta's " +
      "Embedded Signup and answered in the same inbox as their Facebook and " +
      "Instagram conversations. Bound by WhatsApp's 24-hour customer service " +
      "window: outside it only a template Meta approved in advance may be " +
      "sent, and FieldQuo refuses free text by name rather than letting Meta " +
      "reject it.",
    // ── Why "preview", the same deviation page_messaging made ───────────
    //
    // Nothing here is unfinished. The signed webhook, the envelope parser, the
    // send path, the template list, the 24-hour window and the AI employee's
    // refusal outside it are all built and all real. What is missing is a
    // PERMISSION FROM META: whatsapp_business_messaging is not approved for
    // this app (lib/meta/client.js, META_WHATSAPP_SCOPE), and Meta will not
    // review it until they can watch a screencast of the feature working — so
    // the flow has to exist, labelled, before there is anything to submit.
    //
    // `preview` is exactly that state: usable, and LABELLED as such. Settings
    // says in one sentence what is blocked, the connect route refuses
    // server-side, and no conversation is invented for a real company. Flip to
    // "on" when the approval lands.
    defaultState: "preview",
    // No per-company adoption boolean, for the reason page_messaging gives:
    // there is nothing to switch on beyond connecting a number, and a
    // connected number is a MessagingChannel row, not a Company column.
    adoptionField: null,
    // No nav row and no page prefix of its own. The inbox is /app/messages,
    // which page_messaging already claims and gates — a second gate on the
    // same screen would mean withholding either feature emptied a screen that
    // still has two working platforms on it. What this key gates is the
    // CONNECT flow and the WEBHOOK, which is the whole of what is WhatsApp's.
    navKeys: [],
    routePrefixes: [],
    apiPrefixes: ["/api/meta/whatsapp", "/api/settings/whatsapp"],
    apiExempt: [
      {
        path: "/api/meta/whatsapp/webhook",
        guard:
          "X-Hub-Signature-256 HMAC-SHA256 over the raw body, keyed with " +
          "META_APP_SECRET and compared timing-safely in " +
          "lib/messaging/webhookSignature.js; refuses when the secret is " +
          "unset. The tenant is resolved by looking WhatsApp's PHONE NUMBER " +
          "ID (value.metadata.phone_number_id) up in our own " +
          "MessagingChannel rows — never the WABA in entry.id, and never " +
          "anything else in the payload.",
        reason:
          "Meta delivering a message a homeowner has ALREADY sent to the " +
          "contractor's WhatsApp number. There is no session — Meta has no " +
          "FieldQuo login — and refusing it at the door would lose the " +
          "enquiry permanently rather than protect anything, the same " +
          "argument /api/meta/messaging/webhook and /api/crew/inbound make. " +
          "A withheld feature hides the CONNECT flow, which is what the gate " +
          "on /api/settings/whatsapp does.",
      },
    ],
    cronPaths: [],
    // Nothing at a vendor. Meta charges the CONTRACTOR for WhatsApp
    // conversations, billed to their own WhatsApp Business Account — not to
    // FieldQuo — so no FieldQuo money moves when a message is sent. The AI
    // employee answering a WhatsApp thread does spend, and that is metered
    // under `ai_employee`, where it belongs.
    spends: false,
  },
  {
    key: "ai_employee",
    label: "AI employee",
    blurb:
      "An agent that answers a homeowner's message on the company's behalf — " +
      "a closer, a receptionist or a troubleshooter, working from the " +
      "company's own price book and the policy, guide or manual they upload. " +
      "Drafts for a person by default; sends on its own only when the company " +
      "switches that on separately.",
    // ── Why "preview", the same deviation page_messaging made ───────────
    //
    // Nothing here is unfinished: the roles, the guards, the tool loop, the
    // metering, the settings screen and its test box are all built and all
    // real. What is missing is the CHANNEL — this answers messages that arrive
    // through page_messaging, and Meta has not approved that for this app. A
    // company can configure an employee, upload material and test it against
    // the real code path today; what it cannot do is have a reply leave the
    // building, and the screen says so in one sentence rather than offering an
    // auto-send switch with nothing behind it. Flip to "on" alongside
    // page_messaging when the App Review comes back.
    defaultState: "preview",
    // No Company column. The adoption switch is AiEmployee.enabled — one row
    // per company, carrying five other settings that belong beside it — and
    // hoisting a boolean onto Company to satisfy this field would be a second
    // answer to "is it on?" that could disagree with the first.
    adoptionField: null,
    navKeys: ["app.settings.aiEmployee"],
    routePrefixes: ["/app/settings/ai-employee"],
    apiPrefixes: ["/api/ai-employee"],
    apiExempt: [],
    cronPaths: [],
    // TRUE. Every reply is a model call through lib/ai/provider.js, metered by
    // lib/ai/usage.js against the company's own allowance — including the test
    // box, which really calls the model because a preview that did not would
    // be showing a contractor an answer produced by code that never runs.
    spends: true,
  },
];
