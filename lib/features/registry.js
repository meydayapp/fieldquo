...
const ENTRIES = [
  ...
  },
  {
    ...
    label: "Marketing Designer — AI image tools",
    blurb:
      "AI background removal and AI image generation inside the canvas " +
      "editor (app/components/designer/). The editor itself — templates, " +
      "uploads, stock photos, shapes, text, exports — is free and gates " +
      "nothing here; only these two AI actions cost FieldQuo money at a " +
      "vendor, per the owner's 2026-08-30 correction that AI image " +
      "generation is the sole premium piece of this feature.",
    // HIDDEN, not "on". The editor is ported and the endpoints are real, but
    // lib/designer/aiImageAdapter.js still stands at its vendor seam and no
    // /app page mounts DesignerLoader yet. Switching this on would put a
    // billable feature in front of people whose only possible experience of
    // it is a disabled button. Flip it in the platform console once the seam
    // is closed and a page mounts the editor.
    defaultState: "hidden",
    ...
    adoptionField: null,
    // Embedded inside the designer component tree, not a routed /app page —
    // see app/components/designer/DesignerLoader.js's own note that nothing
    // imports it from a page yet. Nothing to hide a nav row for either.
    navKeys: [],
    routePrefixes: [],
    // Only the two AI-costing routes, not "/api/designer" wholesale — the
    // template gallery, uploads and stock photos must stay reachable exactly
    // as they are when this feature is hidden or locked, because none of
    // them is the thing being withheld. Scoping narrowly here is the same
    // call marketing_campaigns makes above for its own public routes.
    // All three AI-costing routes. Two worktrees built this feature from
    // opposite ends — the editor's own sidebars call /api/designer/*, and
    // the standalone generation endpoint is /api/marketing/designer/images.
    // Both exist now, so both are gated; listing only one set would leave a
    // billable route ungated, which is worse than either half alone.
    apiPrefixes: [
