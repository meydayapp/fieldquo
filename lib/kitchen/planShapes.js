...
import { normaliseFinish, colorFor, EXISTING_COLOR } from "./finishes";
// Imported rather than restated. pricing.js has no imports of its own — that is
// deliberate, see the note above LOWER_TIERS — so there is no cycle here, and a
// second copy of "is this piece in the job?" is exactly the duplication that
// lets a drawing and a price disagree.
import { isOutOfScope } from "./pricing";

/** Local alias: this file reads better as `excluded(el)` at the call sites. */
const excluded = (el) => isOutOfScope(el);
...
/**
 * Diagonal hatching over a rect — the drafting convention for "existing, to
 * remain".
 *
 * Colour alone cannot carry this. `colorFor` greys an excluded cabinet, but a
 * client is free to choose a grey kitchen, and then the two are the same
 * picture with two different prices behind it. The hatch is a mark no finish
 * can produce, so the distinction survives every palette.
 *
 * Plain lines rather than an SVG <pattern>: the PDF adapter draws the same
 * shape list, and patterns do not survive it.
 */
function addExistingHatch(shapes, rc, spacing = 5) {
  const { x, y, w, h } = rc;
  if (!(w > 0) || !(h > 0)) return;
  for (let d = -h + spacing; d < w; d += spacing) {
    // Clipped to the box by parameter rather than by a clip path, for the same
    // reason as above.
...
export function planShapes(design) {
  ...
  for (const el of bases) {
    ...
    for (const rc of rects) {
      ...
      if (excluded(el)) addExistingHatch(shapes, rc);
  ...
  for (const el of uppers) {
    ...
    for (const rc of rects) {
      ...
      if (excluded(el)) addExistingHatch(shapes, rc, 6);
    }
  }
  ...
  // ── Legend ──────────────────────────────────────────────────────────────
  //
  // Returned rather than drawn, because the space for it is in the sheet's foot
  // band — the same band the scale bar goes in — and that band belongs to the
  // adapters, which already own the head band for the title. Empty when nothing
  // is excluded: a legend explaining a symbol that isn't on the drawing is
  // clutter, and worse, it invites the reader to hunt for one.
  const legend = elements.some(excluded)
    ? [
        {
          color: EXISTING_COLOR,
          hatched: true,
          label: "Existing — stays as is, not included in this quote",
        },
      ]
    : [];

  return { shapes, width: W, height: D, pad, legend };
}

/**
 * The legend row, as shapes, drawn at a given baseline.
 *
 * Same arrangement as scaleBarShapes: the adapters position it and this decides
 * what it looks like, so the screen and the PDF cannot describe the same symbol
 * two different ways.
 */
export function legendShapes(legend = [], { x = 0, y = 0 } = {}) {
  const shapes = [];
  const sw = 9;
  const sh = 5;
  let cx = x;
  for (const entry of legend) {
    const rc = { x: cx, y: y - sh + 1, w: sw, h: sh };
    shapes.push({
      type: "rect",
      ...rc,
      fill: entry.color || EXISTING_COLOR,
      stroke: shade(entry.color || EXISTING_COLOR, -0.25),
      width: 0.4,
    });
    if (entry.hatched) addExistingHatch(shapes, rc, 3);
    shapes.push({
      type: "text",
      x: cx + sw + 3,
      y,
    ...
    cx += sw + 3 + entry.label.length * 3.1 + 8;
  }
  return shapes;
}
...
export function elevationShapes(design, wallId) {
  ...
  for (const el of onWall) {
    ...
    // After the faces, not before: hatching under a door panel is hidden by it,
    // and the panel is most of the box.
    if (excluded(el)) addExistingHatch(shapes, { x, y, w, h: boxH }, 6);
