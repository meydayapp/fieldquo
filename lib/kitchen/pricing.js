...
export const DEFAULT_CABINET_RATES = {
  ...
  // ── closet & laundry casework ($/lf) ──────────────────────────────────────
  //
  // ZERO ON PURPOSE, and they must stay zero until somebody supplies a real
  // number. Every rate above is one cabinet maker's real, sold kitchen price.
  // Nobody has given us a closet or a laundry price, and a plausible-looking
  // guess is the worst of the three options here: a contractor sends the quote
  // without noticing, and the invented number is what the client signs.
  //
  // A kind whose rate is 0 prices at 0 and SAYS SO — see `rateMissing` on
  // priceCabinet and `unpriced` on getKitchenBreakdown, which the designer
  // surfaces before the quote can go out.
  //
  // Laundry sink bases, uppers over the appliances and broom/utility talls are
  // deliberately absent from this list: those are base, wall and tall cabinets
  // in every way that costs money, so they price off lfBase / lfUpper / lfTall
  // rather than inviting a shop to maintain a second number for the same box.
  lfClosetHang: 0, // single- and double-hang rail sections
  lfClosetShelf: 0, // open shelf stacks, and the corner unit
  lfClosetDrawer: 0, // drawer banks
  lfClosetShoe: 0, // shoe racks
  lfFoldingCounter: 0, // laundry folding counter (a surface, not a box)
...
const KIND_RATE_KEY = {
  closetSingleHang: "lfClosetHang",
  closetDoubleHang: "lfClosetHang",
  closetShelfStack: "lfClosetShelf",
  closetCorner: "lfClosetShelf",
  closetDrawerBank: "lfClosetDrawer",
  closetShoeRack: "lfClosetShoe",
  foldingCounter: "lfFoldingCounter",
};
...
export function normaliseRates(input) {
  ...
  for (const k of [
    ...
    // Closet & laundry. Coerced here like every other rate so a card saved
    // before these existed — or one carrying "" from an emptied settings
    // field — can't price a closet at NaN.
    "lfClosetHang", "lfClosetShelf", "lfClosetDrawer", "lfClosetShoe",
    "lfFoldingCounter",
  ]) {
...
function lfRateFor(kind, rates) {
  // Closet/laundry kinds first: they bill against their own key and must not
  // fall through to a kitchen tier, which would put a real $850/lf tall rate on
  // a hanging section nobody has priced.
  const own = Object.prototype.hasOwnProperty.call(KIND_RATE_KEY, kind)
    ? KIND_RATE_KEY[kind]
    : null;
  if (own) return rates[own];
...
export function estimateBoxPrice(el, rates = DEFAULT_CABINET_RATES) {
  ...
  // Cost-plus is a model of RTI kitchen box fits — an intercept and a $/inch
  // per tier, measured against real invoices. There is no such measurement for
  // a hanging section or a shoe rack, and running one through the base-cabinet
  // curve would produce a number with a decimal point and no evidence behind
  // it. Zero, flagged by priceCabinet, is the honest output.
  if (Object.prototype.hasOwnProperty.call(KIND_RATE_KEY, el.kind)) return 0;
  const w = widthOf(el);
  let raw;
...
//
// `rateMissing` means the card has no rate for this kind — the closet and
// laundry rates ship at 0 (see DEFAULT_CABINET_RATES). Such a piece prices at
// NOTHING, deliberately: not at the drawer surcharge alone, which would put a
// considered-looking $300 on a quote for a drawer bank whose carcass nobody has
// priced. Zero is visibly wrong; $300 is invisibly wrong.
export function priceCabinet(el, rates = DEFAULT_CABINET_RATES) {
  ...
  const perLfMode = (rates.cabinetPricingMode || "perLinearFt") === "perLinearFt";
  const noRate = perLfMode
    ? !(num(lfRateFor(el.kind, rates), 0, 0, 1e7) > 0)
    : Object.prototype.hasOwnProperty.call(KIND_RATE_KEY, el.kind);
