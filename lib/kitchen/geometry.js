...
// The closet and laundry pieces, adopted here rather than left in pricing.js.
//
// This is what makes them real. `planShapes.isKnownKind` filters on this table,
// so a closet or a laundry cabinet priced and drawn correctly in the designer
// was silently dropped from the PDF plan and from the public self-quote — the
// estimator saw it, charged for it, and the client's document did not show it.
// A piece that bills and does not print is worse than one that does neither.
//
// The dependency runs geometry → pricing and must not run back: pricing.js
// keeps its own BASE_H literal for exactly that reason.
import { ROOM_ELEMENT_KINDS } from "./pricing";
