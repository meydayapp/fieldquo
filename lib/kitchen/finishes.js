...
/**
 * The neutral a cabinet is drawn in when it is not part of the job.
 *
 * A warm grey rather than the client's colour, because the colour is the thing
 * being SOLD. Painting an excluded cabinet in the new finish draws a kitchen
 * where everything matches and then hands over a price that only does half of
 * it — the most expensive misunderstanding this drawing could cause.
 *
 * Mid-tone on purpose: it has to sit legibly against a white plan sheet and
 * against the wall colour on an elevation, and it must not be mistaken for one
 * of the CABINET_COLORS a client could actually have picked.
 */
export const EXISTING_COLOR = "#b8b3ab";

/**
 * The colour a given element's doors are painted.
 *
 * The one hook every surface goes through — the plan, the elevations, the PDF
 * and the designer's own face preview all call this — which is why the
 * out-of-scope neutral lives here rather than in each renderer. A fifth surface
 * added later gets the treatment without being told about it.
 */
export function colorFor(el, finish) {
  const f = normaliseFinish(finish);
  if (el?.config?.outOfScope) return EXISTING_COLOR;
