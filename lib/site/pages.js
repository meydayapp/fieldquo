...
/**
 * The "Client login" page's segment (/client, /fr/client). Not a page in the
 * catalogue and never stored in `pages`: it exists only while the company has
 * CompanySite.clientPortalEnabled on, and is rendered by the site route with
 * the site's own header and footer around a single email box. A company page
 * someone happened to name "client" is shadowed while the login is on and
 * comes back when it is switched off.
 */
export const CLIENT_LOGIN_SLUG = "client";
