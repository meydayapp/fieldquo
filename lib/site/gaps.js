...
// ── Two languages on one row ─────────────────────────────────────────────
//
// `question`, `detail` and `action.label` are English, for the log line, the
// check scripts and any reader with no catalogue to hand. `questionKey`,
// `detailKey` and `action.labelKey` name the app.siteGaps.* entries the builder
// renders through t() — with `params` for the one sentence that carries a
// number — so a French owner reads "Vous avez 6 photos" rather than the English
// this module is written in. The keys are derived from `key` so a new gap
// cannot ship half-translated: the check that walks this file asserts every
// row's three keys exist in the catalogue.

function keysFor(key) {
  return {
    questionKey: `app.siteGaps.${key}.question`,
    detailKey: `app.siteGaps.${key}.detail`,
    actionLabelKey: `app.siteGaps.${key}.action`,
  };
}
 ...
 * @returns [{ key, question, questionKey, detail, detailKey, params, action, blocks, severity }]
 */
export function siteGaps({
  ...
  return gaps.map((gap) => {
    const { questionKey, detailKey, actionLabelKey } = keysFor(gap.key);
    return {
      ...gap,
      questionKey,
      detailKey,
      params: gap.params || {},
      action: gap.action ? { ...gap.action, labelKey: actionLabelKey } : gap.action,
    };
  });
}
