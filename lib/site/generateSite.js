...
export async function generateSite({
  ...
  interview = {},
  existingBlocks = [],
  // An EXPLICIT template / style pick from the builder's chooser. A direct
  // instruction, not a hint from prose — so it's honoured deterministically,
  // with no model round-trip. That is also the only path that varies on
  // localhost, where there is no AI key and every prompt-driven Regenerate
  // otherwise collapses to the single data-driven preset.
  forceComposition = null,
  forceStyle = null,
  onUsage,
}) {
  ...
  // ── Explicit template / style pick ───────────────────────────────────────
  //
  // The company clicked a specific LAYOUT or STYLE in the builder's chooser.
  // That's a direct choice, so it wins outright and instantly — rebuild in the
  // chosen shape, carry their photos AND their authored copy across (a
  // re-layout of the same business must not throw away the headline they
  // wrote), apply the chosen style. No model call, so it works offline and
  // never spends quota. `generated:false` because no words were written — but
  // `forced` is set so the builder says "Applied the Booking-first layout"
  // rather than the silent-fallback apology.
  const pickedPreset =
    forceComposition && COMPOSITION_KEYS.includes(forceComposition) ? forceComposition : null;
  const pickedStyle =
    forceStyle && SITE_STYLE_KEYS.includes(forceStyle) ? forceStyle : null;

  if (pickedPreset || pickedStyle) {
    const shape = pickedPreset || fallbackPreset;
    return {
      ...nothing,
      blocks: applyIntentVariants(
        carryCopy(build(sectionsForPreset(shape, available).sections), existingBlocks),
      ),
      styleKey: pickedStyle || intent.styleKey || DEFAULT_SITE_STYLE,
      composition: shape,
      forced: { composition: pickedPreset, style: pickedStyle },
    };
  }
...
company's own settings and must be reproduced exactly.`,
  ...
  // Not yet on complete()'s schema mode. The reply carries a variable SECTION
  // LIST whose per-section shapes differ, so a strict schema would have to
  // enumerate every section type as an anyOf and mark every field required —
  // which validateComposition() below then re-checks anyway, because the real
  // rule ("a section the company has no data for is refused") is a database
  // question no schema can answer. The win here is smaller than elsewhere and
  // the blast radius is the largest of any AI call in the product; it is worth
  // doing, separately, with check-site-blocks.mjs extended first.
  let copy;
  try {
    copy = JSON.parse(
 ...
 */
// Prose fields carried across a TEMPLATE switch, per type. Changing the layout
// is a re-arrangement of the same business, not a fresh draft — losing the
// headline and FAQ the company wrote every time they tried a different shape is
// exactly why "change the template" felt like it did nothing worth keeping.
//
// Excludes the DB-derived lists (services / testimonials / areas): build()
// rebuilds those from the company's own rows so they can't go stale, and an old
// copy would fight the fresh one. faq / process / credentials items ARE carried
// — build() leaves those empty (they're authored, not derived), so carrying is
// purely additive.
const CARRY_TEXT = {
  ...
  beforeafter: ["heading", "intro"],
  testimonials: ["heading"],
  faq: ["heading", "items"],
  process: ["heading", "intro", "steps"],
  areas: ["heading", "intro"],
  credentials: ["heading", "intro", "items"],
  cta: ["heading", "sub", "buttonLabel"],
  quoteform: ["heading", "intro"],
  booking: ["heading", "intro"],
  hours: ["heading", "note"],
  contact: ["heading", "intro"],
};

function carryCopy(blocks, existing) {
  if (!Array.isArray(existing) || existing.length === 0) return blocks;
  const prev = new Map();
  for (const b of existing) if (b?.type && !prev.has(b.type)) prev.set(b.type, b);
  return blocks.map((block) => {
    const old = prev.get(block.type)?.content;
    if (!old) return block;
    const carried = {};
    for (const field of CARRY_TEXT[block.type] || []) {
      const v = old[field];
      // Only carry a value that's actually there — an empty string or [] would
      // blank out the factual default build() just put in.
      if (Array.isArray(v) ? v.length : v) carried[field] = v;
    }
    return Object.keys(carried).length ? withContent(block, carried) : block;
  });
}
