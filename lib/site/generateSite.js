...
Pick from exactly these keys and order them the way this business should be read:

  hero          the headline. Always first.
  services      what they do.
  beforeafter   draggable before/after photos. Only if told pairs exist.
  gallery       photos of their work. Only if told photos exist.
  about         who they are.
  ...
  quoteform     the instant quote form.
  booking       the booking calendar.
  hours         opening hours. Only if told hours exist.
  faq           the questions you wrote.
  contact       phone, email, map. Always last.
...
- Business is about getting on a calendar (cleaning, inspections, maintenance)?
  Put "booking" high.
- Very little content at all? Return a SHORT page. Six sections that are full
  beat twelve that are empty. Never pad.
...
export async function generateSite({
  ...
  // The no-AI page. Its SHAPE comes from the company's own data, so two
  // companies without AI still get different pages — if only the AI path varied,
  // every company during an outage would be back to one template.
  const fallbackPreset = compositionFromData(available, { serviceCount: services.length });
  ...
  return {
    ...
    styleKey: pickVariant(copy.styleKey, SITE_STYLE_KEYS, DEFAULT_SITE_STYLE),
