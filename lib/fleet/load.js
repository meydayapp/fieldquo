...
import { vehicleCostSummary } from "./cost";
...
export async function loadFleet({ db, member, full, asOf = new Date() }) {
  ...
  // The running costs, fetched only for a member who may see them. Two reads
  // a dispatcher's payload would strip anyway are two reads not made — and,
  // more to the point, money a dispatcher must not see never leaves the
  // database on their request, rather than being loaded and then removed.
  //
  // Expenses are keyed by ASSET (that is the column Settings → Expense
  // tracking writes), maintenance by fleet record. Everything since the
  // window opened, so cost.js can apply both windows itself; it is the one
  // place the window is defined, and it is what the check script pins.
  const { expenses, maintenance } = seeCost
    ? await loadCostRows({
        db,
        companyId: member.companyId,
        assetIds: assets.map((a) => a.id),
        vehicleIds: details.map((d) => d.id),
      })
    : { expenses: new Map(), maintenance: new Map() };
  ...
  const rows = joinVehicles({ assets, details }).map((row) => {
    const priced =
      row.asset && seeCost
        ...
        : row;
    const costed = seeCost ? withCost(priced, { expenses, maintenance, asOf }) : priced;
    const decorated = seeCost ? costed : stripVehicleCost(costed);
...
/**
 * The Expense and VehicleMaintenance rows the cost block is computed from,
 * grouped by asset id and by vehicle id respectively.
 *
 * EVERY row, not the last twelve months. The cost per km spans the oldest and
 * newest odometer readings on file (lib/fleet/cost.js), and a van whose two
 * readings are eighteen months apart needs the fuel from eighteen months ago
 * in the numerator. Cutting the fetch at the window would quietly halve the
 * figure for exactly the van with the longest record. A vehicle's own expense
 * rows are a small slice of the company's, and the select is four columns.
 */
async function loadCostRows({ db, companyId, assetIds, vehicleIds }) {
  const expenses = new Map();
  const maintenance = new Map();

  if (assetIds.length) {
    const rows = await db.expense.findMany({
      where: { companyId, assetId: { in: assetIds } },
      select: {
        id: true,
        assetId: true,
        category: true,
    ...
    for (const row of rows) {
      if (!expenses.has(row.assetId)) expenses.set(row.assetId, []);
      expenses.get(row.assetId).push(row);
    }
  }

  if (vehicleIds.length) {
    const rows = await db.vehicleMaintenance.findMany({
      where: { vehicleId: { in: vehicleIds } },
      select: { vehicleId: true, costCents: true, performedAt: true, odometerKm: true },
    });
    for (const row of rows) {
      if (!maintenance.has(row.vehicleId)) maintenance.set(row.vehicleId, []);
      maintenance.get(row.vehicleId).push(row);
    }
  }
...
/**
 * The cost block and the itemised expense list, on one row.
 *
 * The list is the last twelve months only and amounts are numbers — a
 * Prisma Decimal serialises as a string, and the card would otherwise be
 * formatting "412.8" through a money formatter that expects a number.
 */
function withCost(row, { expenses, maintenance, asOf }) {
  const own = row.assetId ? expenses.get(row.assetId) || [] : [];
  const log = row.id ? maintenance.get(row.id) || [] : [];
  const cost = vehicleCostSummary({
    asset: row.asset,
    vehicle: row,
    expenses: own,
    maintenance: log,
  ...
  const since = cost.window.yearStart;
  return {
    ...row,
    cost,
    expenses: own
