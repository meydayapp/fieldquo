 ...
 *
 * Two layers. The Asset's own money columns, and the row-level `cost` block
 * and `expenses` list lib/fleet/load.js attaches (lib/fleet/cost.js) — fuel
 * totals and a cost per km are the same class of number as the purchase
 * price, and an orphan row (no asset) can still carry them, which is why the
 * row-level strip runs before the asset check rather than inside it.
 */
export function stripVehicleCost(row) {
  if (!row) return row;
  const { cost: costBlock, expenses, ...bare } = row;
  if (!bare.asset) return bare;
  const {
    cost,
    salvageValue,
    bookValue,
