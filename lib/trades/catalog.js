...
export const TRADE_CATALOG = {
  ...
  paving: {
    ...
    // Traced on the satellite map and priced per sqft off the paving book's
    // three surfaces — lib/estimate/instantEstimate.js `paving`.
    instantTrade: "paving",
    primary: true,
  },
  fence_services: {
  ...
  gutter_services: {
    ...
    // Eavestrough measured from the roof model at the address, priced by the
    // foot and the downspout — lib/estimate/instantEstimate.js estimateGutters.
    // The one category the estimator serves, so it is where the draft files.
    instantTrade: "gutters",
    primary: true,
  },
  ...
  lawn_care: {
    ...
    // Treatment PROGRAMS priced by lawn-size band from the address —
    // lib/estimate/lawnCare.js. Distinct from lawn_mowing's per-visit price
    // below: a mowing company sells visits, a treatment company sells
    // programs, and the two estimators are different products.
    instantTrade: "lawn_care",
    primary: true,
  },
  lawn_mowing: {
    label: "Lawn Mowing",
  ...
  // ── Kitchen Design & New Installs — its OWN opt-in, not inferred ──────────
  //
  // The owner's report, 2026-08-30: "New kitchen installs can be done by a
  // few types of contractors not just kitchen refinishers, and not all
  // kitchen cabinet refinishers do them." Before this key existed, the
  // Kitchen Designer button (app/app/quotes/[id]/page.js) was shown whenever
  // a quote had ANY scope group whose category key matched
  // /cabinet|kitchen|countertop|remodel/ — which got it wrong in both
  // directions: a cabinet refinisher who never installs a new kitchen got
  // the tool on every refinishing AND every countertop quote, while a
  // general contractor who genuinely does build new kitchens but has never
  // touched cabinet_refinishing/cabinet_refacing had no way to reach it at
  // all. See lib/kitchen/access.js — the tool is now gated on THIS key being
  // explicitly enabled, not on what happens to be drawn on one quote.
  //
  // No `instantTrade`: laying out a kitchen isn't a $/sqft estimate, it's the
  // interactive designer itself. `industries: []` is deliberate too — this
  // isn't a painting-industry or a construction-industry trade, it's sold by
  // both and by neither exclusively, so it's reachable via "show other
  // trades" for any company rather than presumed onto one industry preset
  // (the same reasoning fence_repair/fence_restoration above already use).
  kitchen_design: {
    label: "Kitchen Design & New Installs",
    icon: "Ruler",
    sortOrder: 69,
    industries: [],
  },

  // ── Trades added 2026-09-21 from the service-seed work ────────────────────
  //
  // The benchmark that seeded app/data/serviceSeeds/* carries books for
  // sixteen field-service industries this catalogue had no key for. The
  // owner's word: "the owner wants them available as trades." Each one is a
  // trade a homeowner books someone to come out for; none is a shop, a
  // profession or a client type (those stay skipped — docs/SERVICE-SEEDS.md
  // lists them with the reason).
  //
  // `labelTranslations` is declared here — the first entries to carry one —
  // because these keys arrive with names in the nine app languages and the
  // seeder (prisma/seed.js, scripts/seed-categories.mjs) would otherwise
  // create the row with an English label and a null translation column that
  // a tenant website in French then renders in English. Written on CREATE
  // only: an existing row's translations are the product's own data and a
  // re-seed does not overwrite them.
  //
  // Industries are the marketing slugs that surface the trade at signup. The
  // three electrical-adjacent trades sit under "electrical"; the rest are
  // reachable through "show other trades" (industries: []) until the owner
  // decides which preset should offer them — see categoriesWithoutIndustry().
  air_duct_cleaning: {
    label: "Air Duct Cleaning",
    icon: "Wind",
    sortOrder: 70,
    industries: ["hvac", "cleaning"],
    labelTranslations: { fr: "Nettoyage de conduits d'air", es: "Limpieza de ductos de aire", uk: "Чищення повітропроводів", pa: "ਏਅਰ ਡਕਟ ਸਫ਼ਾਈ", tl: "Paglilinis ng Air Duct", de: "Luftkanalreinigung", zh: "风管清洁", it: "Pulizia dei condotti dell'aria" },
  },
  deck_patio: {
    label: "Decks & Patios",
    icon: "Layers",
    sortOrder: 71,
    industries: ["construction-contracting", "landscaping"],
    labelTranslations: { fr: "Terrasses et patios", es: "Terrazas y patios", uk: "Тераси та патіо", pa: "ਡੈੱਕ ਅਤੇ ਪੈਟੀਓ", tl: "Deck at Patio", de: "Terrassen und Patios", zh: "露台与庭院", it: "Terrazze e patio" },
  },
  doors_windows: {
    label: "Doors & Windows",
    icon: "DoorClosed",
    sortOrder: 72,
    industries: ["construction-contracting", "handyman"],
    labelTranslations: { fr: "Portes et fenêtres", es: "Puertas y ventanas", uk: "Двері та вікна", pa: "ਦਰਵਾਜ਼ੇ ਅਤੇ ਖਿੜਕੀਆਂ", tl: "Pinto at Bintana", de: "Türen und Fenster", zh: "门窗", it: "Porte e finestre" },
  },
  lighting: {
    ...
    sortOrder: 73,
    industries: ["electrical"],
    labelTranslations: { fr: "Installation d'éclairage", es: "Instalación de iluminación", uk: "Встановлення освітлення", pa: "ਲਾਈਟਿੰਗ ਇੰਸਟਾਲੇਸ਼ਨ", tl: "Pag-install ng Ilaw", de: "Beleuchtungsinstallation", zh: "照明安装", it: "Installazione di illuminazione" },
  },
  security_systems: {
    ...
    sortOrder: 74,
    industries: ["electrical"],
    labelTranslations: { fr: "Systèmes de sécurité", es: "Sistemas de seguridad", uk: "Системи безпеки", pa: "ਸੁਰੱਖਿਆ ਸਿਸਟਮ", tl: "Mga Security System", de: "Sicherheitssysteme", zh: "安防系统", it: "Sistemi di sicurezza" },
  },
  sewer_septic: {
    label: "Sewer & Septic",
    icon: "Droplets",
    sortOrder: 75,
    industries: ["plumbing"],
    labelTranslations: { fr: "Égouts et fosses septiques", es: "Alcantarillado y fosas sépticas", uk: "Каналізація та септики", pa: "ਸੀਵਰ ਅਤੇ ਸੈਪਟਿਕ", tl: "Sewer at Septic", de: "Abwasser und Klärgruben", zh: "下水道与化粪池", it: "Fognature e fosse settiche" },
  },
  smart_home: {
    label: "Smart Home & Audio-Video",
    icon: "Home",
    sortOrder: 76,
    industries: ["electrical", "handyman"],
    labelTranslations: { fr: "Domotique et audio-vidéo", es: "Domótica y audio-video", uk: "Розумний дім та аудіо-відео", pa: "ਸਮਾਰਟ ਹੋਮ ਅਤੇ ਆਡੀਓ-ਵੀਡੀਓ", tl: "Smart Home at Audio-Video", de: "Smart Home und Audio-Video", zh: "智能家居与影音", it: "Domotica e audio-video" },
  },
  solar_energy: {
    label: "Solar & EV Charging",
    icon: "Sparkles",
    sortOrder: 77,
    industries: ["electrical"],
    labelTranslations: { fr: "Solaire et recharge de VÉ", es: "Solar y carga de vehículos eléctricos", uk: "Сонячна енергія та зарядка електромобілів", pa: "ਸੋਲਰ ਅਤੇ EV ਚਾਰਜਿੰਗ", tl: "Solar at EV Charging", de: "Solar und E-Auto-Laden", zh: "太阳能与电动车充电", it: "Solare e ricarica EV" },
  },
  moving: {
    ...
    sortOrder: 78,
    industries: [],
    labelTranslations: { fr: "Déménagement", es: "Mudanzas", uk: "Переїзди", pa: "ਮੂਵਿੰਗ ਸੇਵਾਵਾਂ", tl: "Serbisyong Paglilipat", de: "Umzugsservice", zh: "搬家服务", it: "Traslochi" },
  },
  wildlife_control: {
    ...
    sortOrder: 79,
    industries: [],
    labelTranslations: { fr: "Contrôle de la faune", es: "Control de fauna", uk: "Контроль дикої фауни", pa: "ਜੰਗਲੀ ਜੀਵ ਕੰਟਰੋਲ", tl: "Wildlife Control", de: "Wildtierkontrolle", zh: "野生动物防治", it: "Controllo della fauna selvatica" },
  },
  caulking_sealants: {
    label: "Caulking & Sealants",
    icon: "Paintbrush",
    sortOrder: 80,
    industries: ["handyman"],
    labelTranslations: { fr: "Calfeutrage et scellants", es: "Calafateo y selladores", uk: "Герметизація та ущільнювачі", pa: "ਕੌਲਕਿੰਗ ਅਤੇ ਸੀਲੈਂਟ", tl: "Caulking at Sealant", de: "Abdichtung und Dichtstoffe", zh: "填缝与密封", it: "Sigillature e sigillanti" },
  },
  furniture_upholstery: {
    ...
    sortOrder: 81,
    industries: [],
    labelTranslations: { fr: "Meubles et rembourrage", es: "Muebles y tapicería", uk: "Меблі та оббивка", pa: "ਫਰਨੀਚਰ ਅਤੇ ਅਪਹੋਲਸਟਰੀ", tl: "Muwebles at Upholstery", de: "Möbel und Polsterei", zh: "家具与软装", it: "Mobili e tappezzeria" },
  },
  glass: {
    label: "Glass & Glazing",
    icon: "Square",
    sortOrder: 82,
    industries: ["construction-contracting"],
    labelTranslations: { fr: "Vitrerie", es: "Vidriería", uk: "Скло та скління", pa: "ਕੱਚ ਅਤੇ ਗਲੇਜ਼ਿੰਗ", tl: "Salamin at Glazing", de: "Glas und Verglasung", zh: "玻璃安装", it: "Vetreria" },
  },
  marine_services: {
    ...
    sortOrder: 83,
    industries: [],
    labelTranslations: { fr: "Services maritimes", es: "Servicios marinos", uk: "Морські послуги", pa: "ਮਰੀਨ ਸੇਵਾਵਾਂ", tl: "Serbisyong Marino", de: "Bootsservice", zh: "船舶服务", it: "Servizi nautici" },
  },
  home_organization: {
    label: "Home Organization & Interior Design",
    icon: "Home",
    sortOrder: 84,
    industries: [],
    labelTranslations: { fr: "Organisation et design d'intérieur", es: "Organización y diseño de interiores", uk: "Організація простору та дизайн інтер'єру", pa: "ਘਰ ਦੀ ਵਿਵਸਥਾ ਅਤੇ ਇੰਟੀਰੀਅਰ ਡਿਜ਼ਾਈਨ", tl: "Pag-aayos ng Bahay at Interior Design", de: "Wohnorganisation und Innenarchitektur", zh: "家居整理与室内设计", it: "Organizzazione e interior design" },
  },
  baby_proofing: {
    label: "Baby Proofing",
    icon: "Home",
    sortOrder: 85,
    industries: ["handyman"],
    labelTranslations: { fr: "Sécurisation pour bébé", es: "Seguridad para bebés", uk: "Захист для немовлят", pa: "ਬੇਬੀ ਪਰੂਫ਼ਿੰਗ", tl: "Baby Proofing", de: "Kindersicherung", zh: "婴儿安全防护", it: "Sicurezza per bambini" },
  },
};
...
export function seedRows() {
  return tradeKeys().map((key) => ({
    ...
    // Only the trades that declare one (the 2026-09-21 additions). Both
    // seeders pass this to `create` and never to `update`, so a row's
    // translations — edited or machine-drafted since — are never replaced.
    ...(TRADE_CATALOG[key].labelTranslations && {
      labelTranslations: TRADE_CATALOG[key].labelTranslations,
    }),
  }));
}
...
/**
 * Does this company SELL the service an instant estimator prices?
 *
 * The one gate behind the owner's question — "if TrueFinish only has 3
 * selected quote types, why do I have the option to show the other 11?" A
 * trade he has not switched on under Services is not his to configure and not
 * his to offer, so the settings screen does not list it and the public widget
 * never offers it. Both screens read THIS, rather than one of them grouping by
 * `offeredAsService` while the other checked something else — which is how a
 * cabinet painter came to have a roofing rate card.
 *
 * An estimator the catalogue claims no category for returns false rather than
 * being waved through: scripts/check-trade-catalog.mjs fails the build on that
 * case, so it is OUR data that is wrong, and "we can't tell" must not resolve
 * to "offer it to strangers".
 */
export function instantTradeOffered(trade, enabledCategoryKeys) {
  const keys = categoryKeysForInstantTrade(trade);
  if (!keys.length) return false;
  const enabled = new Set(Array.isArray(enabledCategoryKeys) ? enabledCategoryKeys : []);
