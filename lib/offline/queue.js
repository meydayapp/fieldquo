...
export function isNetworkFailure(err) {
  // `code: "network"` is lib/media/uploadClient.js's UploadError for a
  // request that never got an answer — the same fact, named by the helper.
  return err instanceof TypeError || err?.name === "TypeError" || err?.name === "AbortError" || err?.code === "network";
}
...
async function replayPhoto(item, { fetch }) {
  ...
  // The shared helper, so a queued phone photo over Vercel's 4.5 MB request
  // cap still goes up (bytes straight to Cloudinary). `fetch` is the injected
  // one, so the check script drives the sign and verify legs.
  let data;
  try {
    data = await uploadFile(blob, { endpoint: "/api/upload", purpose: "invoices", filename: name || "photo.jpg", fetchImpl: fetch });
  } catch (err) {
    // Still offline: thrown on, so replayQueue stops and keeps the rest queued.
    if (isNetworkFailure(err)) throw err;
    return { status: classifyResponse(err?.status || 400), error: err?.message || "Upload failed." };
  }
...
async function replayTimesheet(item, { fetch }) {
  ...
  const res = await fetch("/api/time-clock", {
    ...
    body: JSON.stringify({
      action: p.action,
      ...(p.action === "out" ? {} : { jobId: p.jobId || null, taskId: p.taskId || null }),
      ...(p.kind ? { kind: p.kind } : {}),
