...
async function notifyDecided(row, outcome, member, ctx) {
  ...
  let byName = "";
  if (member?.userId) {
    const user = await db.user.findUnique({ where: { id: member.userId }, select: { name: true } }).catch(() => null);
    byName = user?.name || "";
  }
  return notifyEvent({
    ...
    params: { kind: row.kind, when: whenLabel(row.shift, ctx), outcome, byName },
  });
}
