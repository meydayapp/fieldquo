...
export async function timelineFor(member, from, to, opts = {}) {
    ...
    member.userId && quoter
      ? db.appointment.findMany({
          ...
          select: {
            ...
            client: { select: { name: true } },
            quoteId: true,
            // The kind the client booked, and the number or email it needs —
            // so a phone call on the day reads "Phone call — we'll ring
            // 555-0199" rather than a street the client never gave.
            booking: { select: { mode: true, clientPhone: true, clientEmail: true } },
          },
        })
      : [],
    member.userId
  ...
  for (const a of appointments) {
    const mode = a.booking?.mode || null;
    items.push({
      kind: "appointment",
      id: a.id,
      start: a.scheduledAt,
      ...
      // The kind the client booked, with the number or email it needs. The
      // card (app/components/me/WorkerHome.js) words it in the crew member's
      // own language through lib/booking/bookingModes.js — the same table the
      // client's letter uses — which is why the facts travel, not a sentence.
      // Null for a visit: its address IS the line, and the card prints it.
      booking:
        mode && mode !== "visit"
          ? { mode, phone: a.booking.clientPhone || null, email: a.booking.clientEmail || null }
          : mode
            ? { mode }
            : null,
