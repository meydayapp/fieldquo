...
export async function clockJobOptions(db, { companyId, full, userId, now, timezone } = {}) {
  ...
  const built = buildJobOptions(todayVisits, openJobs);

  // The plan steps a person can book time against — open steps only, per
  // job in the picker. One query for the whole picker; a job with no plan
  // gets `steps: []` and the screen shows no second picker for it.
  const ids = built.options.map((o) => o.id);
  const steps = ids.length
    ? await db.task.findMany({
        where: { jobId: { in: ids }, planStep: true, status: { in: ["open", "in_progress"] } },
        select: { id: true, jobId: true, title: true, sortOrder: true },
        orderBy: [{ sortOrder: "asc" }, { createdAt: "asc" }],
      })
    : [];
  return attachSteps(built, steps);
}

/**
 * Pure: hang each job's open plan steps on its option. Exported for
 * scripts/check-job-plan.mjs.
 */
export function attachSteps(built, steps = []) {
  const byJob = new Map();
  for (const st of Array.isArray(steps) ? steps : []) {
    if (!st?.jobId) continue;
    if (!byJob.has(st.jobId)) byJob.set(st.jobId, []);
    byJob.get(st.jobId).push({ id: st.id, title: st.title });
  }
  return {
    ...built,
    options: built.options.map((o) => ({ ...o, steps: byJob.get(o.id) || [] })),
  };
}
