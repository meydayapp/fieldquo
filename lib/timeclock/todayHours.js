...
export function todayHoursFrom(entries, now = Date.now()) {
  ...
  for (const e of entries) {
    ...
    // from the day.
    //
    // Less the unpaid breaks taken so far, a running one counted up to now —
    // the same rule the clock-out will apply when it books this entry
    // (lib/timeclock/entryHours.js), so the live figure and the booked one
    // agree at the moment the person presses Clock out.
    const elapsed = Math.max(0, at - started);
    total += Math.max(0, elapsed - unpaidBreakMs(e.breaks, started, at)) / 3_600_000;
  }
