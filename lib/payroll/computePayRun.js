...
export function computeWorkerLine({
  ...
  for (const a of Array.isArray(adjustments) ? adjustments : []) {
    ...
    if (a.kind === "deduction") {
      ...
      // (lib/payroll/buildPayRun.js), "leave" is not passed today and stays
      // "manual". Anything unexpected is "manual" — never a blank.
      const source = a.source === "bonus" ? "bonus" : "manual";
