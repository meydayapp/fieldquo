...
// Exported, not copied: the scheduler's labour-cost line
// (lib/shifts/labourCost.js) prices scheduled overtime at this multiplier so
// the number a manager sees before publishing is the number payroll will
// compute afterwards. A second 1.5 in another file is the copy that rots.
...
export const DEFAULT_OT_THRESHOLD_WEEKLY = 40;
...
/**
 * Progressive tax from configurable slabs — the structure real income tax
 * actually has (Canadian federal/provincial brackets, US federal brackets, UK
 * PAYE bands). Modelled on Frappe HR's "Income Tax Slab": the BRACKETS ARE
 * DATA the company's accountant supplies, not constants compiled into FieldQuo.
 * That keeps annual rate changes a settings edit instead of a deploy, and keeps
 * responsibility for the numbers where it belongs.
 *
 * Slabs are annual thresholds (that's how every published table is written), so
 * pay is annualised, taxed, then divided back down to the period. Taxing a
 * single paycheque against annual brackets would under-withhold enormously.
 *
 * @param periodGross  gross for THIS period
 * @param perYear      pay periods per year (26 for biweekly, etc.)
 * @param slabs        [{ upTo: number|null, percent: number }] ascending; a
 *                     null/absent `upTo` is the top open-ended band.
 * @returns tax for THIS period
 */
export function taxFromSlabs(periodGross, perYear, slabs) {
  const gross = Math.max(0, num(periodGross));
  const periods = Math.max(1, num(perYear, 1));
  const bands = (Array.isArray(slabs) ? slabs : [])
    .map((s) => ({
      upTo: s?.upTo == null ? Infinity : Math.max(0, num(s.upTo)),
      percent: Math.min(100, Math.max(0, num(s?.percent))),
    }))
    // Ascending by threshold so a misordered config can't tax the wrong band.
    .sort((a, b) => a.upTo - b.upTo);

  if (!bands.length || gross === 0) return 0;
  ...
  for (const band of bands) {
    ...
    const taxableInBand = Math.min(annual, band.upTo) - lower;
    if (taxableInBand > 0) annualTax += (taxableInBand * band.percent) / 100;
    lower = band.upTo;
    if (!Number.isFinite(lower)) break;
  }

  return money(annualTax / periods);
}
...
export function computeWorkerLine({
  ...
  adjustments = [],
  deductions = [],
  // Pay periods per year — needed to annualise for progressive tax slabs.
  // Defaults to biweekly, the most common field-service payroll cycle.
  perYear = 26,
} = {}) {
  ...
  // Salary for the period. A worker can be salaried AND log hours for job
  // costing; only pay the salary — paying both would double-pay them.
  //
  // Resolved FIRST, because the hourly block below has to defer to it. Until
  // this was reordered the comment said "salary only" while the code paid
  // regular + overtime AND the salary: the hourly block ran unconditionally
  // and the salary block simply added on top. Reachable in practice because
  // the job-costing fallback (Member.laborCostPerHour) hands every salaried
  // worker a nonzero hourly rate, so `rate > 0` was never the guard it looked
  // like. Executed: salary 1000 + 10h × 20 paid 1200; it pays 1000 now.
  const salary = Math.max(0, num(salaryPerPeriod));
  const salaried = salary > 0;

  // Hourly earnings — for a worker who is NOT on a salary. The logged hours
  // still exist and still cost the job; they are just not paid a second time.
  if (!salaried && rate > 0 && regularHours > 0) {
    const amount = money(regularHours * rate);
    items.push({ label: `Regular hours (${regularHours} × ${rate})`, amount, kind: "earning", source: "hours" });
    gross += amount;
  }
  if (!salaried && rate > 0 && overtimeHours > 0) {
  ...
  for (const a of Array.isArray(adjustments) ? adjustments : []) {
    ...
    if (a.kind === "deduction") {
      ...
      // `source` names where the line came from on the payslip: "manual" for
      // a typed adjustment, "bonus" for a daily-sheet performance bonus
      // (lib/payroll/buildPayRun.js), "commission" for job commissions the
      // owner ticked on the run screen (lib/commissions/), "leave" is not
      // passed today and stays "manual". Anything unexpected is "manual" —
      // never a blank.
      const source = a.source === "bonus" || a.source === "commission" ? a.source : "manual";
      items.push({ label: String(a.label), amount, kind: "earning", source });
      gross += amount;
    }
  }
  ...
  for (const d of allDeductions) {
    if (!d?.label) continue;
    let amount;
    if (Array.isArray(d.slabs) && d.slabs.length) {
      // Progressive brackets, annualised then divided back to the period.
      amount = taxFromSlabs(gross, d.perYear ?? perYear, d.slabs);
      if (!amount) continue;
      items.push({ label: `${d.label} (progressive)`, amount, kind: "deduction", source: "supplied" });
...
export function computePayRun({
  ...
  return {
    ...
    frequency: PAY_FREQUENCIES[frequency] ? frequency : "biweekly",
    lines,
    grossTotal,
    deductionTotal,
    netTotal,
