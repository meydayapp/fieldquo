...
export async function runContractorPayout({
  ...
  // The company comes along for its currency: the transfer is sent in the
  // currency the company bills in (Company.currency), not a hardcoded one.
  // Read here, once, from the worker's own relation — never off a request.
  const worker = await db.worker.findUnique({
    where: { id: workerId },
    include: { company: { select: { id: true, currency: true } } },
  });
  ...
  try {
    const transfer = await payoutToContractor({
      worker,
