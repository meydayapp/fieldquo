...
import { unclaimedBonusSheetsWhere } from "./runClaims";
...
export async function buildPayRun({
  ...
  // Workers whose commission due the owner TICKED on the run screen. Nothing
  // else puts a commission on anyone's pay: the preview lists what is due
  // (meta.commissions) and adds it only for the ids named here. See the
  // commissions block below.
  ...
  // ── Performance bonuses from the daily sheets ─────────────────────────
  //
  // A DailyObjectiveSheet carries bonusCents only when the company HAS a
  // performance-pay rule and a coordinator evaluated the day
  // (lib/dailySheets/bonus.js); null means "no rule" and produces no line.
  // Sheets already on a pay run (payRunId set) are excluded, so a correction
  // run cannot pay a bonus twice; the commit stamps the ids this run picked
  // up and cancelling that run frees them (lib/payroll/runClaims.js).
  // Wrapped for the same reason as leave:
  // a lookup failure must not stop payroll, and the preview says nothing was
  // added rather than guessing.
  const bonusByWorker = new Map();
  const bonusSheetIds = [];
  try {
    const bonusSheets = await db.dailyObjectiveSheet.findMany({
      where: unclaimedBonusSheetsWhere({ companyId, workerIds, start, end }),
      select: { id: true, workerId: true, bonusCents: true, date: true },
    });
    for (const s of bonusSheets) {
      const cur = bonusByWorker.get(s.workerId) || { cents: 0, days: 0, ids: [] };
      cur.cents += Number(s.bonusCents) || 0;
      cur.days += 1;
      cur.ids.push(s.id);
      bonusByWorker.set(s.workerId, cur);
      bonusSheetIds.push(s.id);
    }
  } catch (err) {
    console.error("[payroll] daily-sheet bonus lookup failed:", err?.message);
  }

  // ── Job commissions, offered — never added unasked ───────────────────
  //
  // Every ledger row (lib/commissions/sync.js) earned by the END of the
  // period's last day and on no pay run yet — including rows earned in an
  // earlier period nobody paid out, and refund reversals, which net against
  // them. It becomes an earning line ONLY for a worker in
  // includeCommissionWorkerIds: the owner's instruction was that pay is not
  // changed without them including it. The grouping and that rule are pure,
  // in lib/commissions/payRun.js, where scripts/check-commissions.mjs runs them.
  let commissionsDue = [];
  let commissionEntryIds = [];
  try {
    const endOfDay = new Date(end.getTime() + 86400000 - 1);
    const rows = await db.jobCommissionEntry.findMany({
      where: { companyId, payRunId: null, createdAt: { lte: endOfDay } },
      select: { id: true, memberId: true, memberName: true, amount: true, jobId: true },
    });
    if (rows.length) {
      const memberRows = await db.member.findMany({
        where: { companyId, id: { in: [...new Set(rows.map((r) => r.memberId))] } },
        select: { id: true, userId: true, user: { select: { name: true, email: true } } },
      });
      const offered = commissionLinesForRun({
        rows,
        members: memberRows,
        workers,
        include: includeCommissionWorkerIds,
      });
      commissionsDue = offered.due;
      commissionEntryIds = offered.entryIds;
      for (const [workerId, lines] of Object.entries(offered.adjustments)) {
        adjustmentsByWorker = {
          ...adjustmentsByWorker,
          [workerId]: [...(adjustmentsByWorker[workerId] || []), ...lines],
        };
      }
    }
  } catch (err) {
    // Payroll must still run when commissions cannot be read — and the
    // preview then shows no commission section, rather than a wrong one.
    console.error("[payroll] commission lookup failed:", err?.message);
  }
  ...
  const payload = workers.map((w) => {
    ...
    // The performance bonus, as ONE earning line per person per run, labelled
    // with the number of days it covers. The per-day breakdown stays on the
    // sheets, which the payslip's reader can open.
    const bonus = bonusByWorker.get(w.id);
    if (bonus && bonus.cents > 0) {
      adjustments.push({
        label: `Performance bonus — ${bonus.days} day${bonus.days === 1 ? "" : "s"}`,
        amount: Math.round(bonus.cents) / 100,
        kind: "earning",
        source: "bonus",
      });
    }
  ...
  return {
    ...computed,
    meta: {
      // The sheets whose bonus this run carries; the commit stamps them.
      bonusSheetIds,
      // Commission due per person, and the ledger rows the TICKED ones carry
      // — the commit stamps exactly these with the run's id.
      commissions: commissionsDue,
      commissionEntryIds,
      periodStart: start,
      periodEnd: end,
