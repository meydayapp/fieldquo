...
export async function diagnoseNumber(companyId) {
  ...
  const [agent, credit] = await Promise.all([
    ...
    // Priced at THIS number's rate. A toll-free minute costs more than a local
    // one, and asking at the local default reports a company as funded that the
    // spend gate would refuse.
    canTakeCall(companyId, number.numberType).catch(() => ({ allowed: false })),
  ]);
  ...
  return shape(verdict, {
    ...
    publicNumber: number.publicNumber || null,
    boundAgent,
    wantAgent: agent?.providerAgentId || null,
    // Carried out rather than left encoded in the verdict, so the readiness
    // chain in lib/voice/readiness.js can reuse this observation instead of
    // making a second get-phone-number call and reaching its own conclusion.
    // Two modules that both decide whether a number exists will disagree, and
    // the one nobody looks at is the one that ends up on screen.
    existsAtProvider,
    agentEnabled: Boolean(agent?.enabled),
    hasCredit: Boolean(credit?.allowed),
    balanceCents: credit?.cents ?? null,
...
function shape(verdict, extra) {
  ...
  return {
    ...
    source: null,
    publicNumber: null,
    // Three-state, and the default is "we did not look" rather than false —
    // `no_number` and `not_configured` never asked the provider anything, and
    // saying `false` there would assert an absence nobody observed.
