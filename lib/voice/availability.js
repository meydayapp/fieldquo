...
import { bookingDurationMinutes } from "@/lib/booking/bookingModes";
import {
  CALLBACK_DELAY_MINUTES,
  CALLBACK_MINUTES,
  withinBusinessHours,
} from "./callbackWindow";
import { normaliseEmail } from "./tools";
// How a time is READ OUT depends on which language(s) the receptionist speaks,
// and that is the agent's setting, not the company's — the same rule
// provision.js provisions with and prompt.js instructs with.
import { resolveSpokenLanguage, spokenLocales } from "./agentLanguage";
...
const POLICY_SELECT = {
  ...
  promoFeeCents: true,
  promoActive: true,
  // Whose calendar it lands on, so the agent can answer "who will call me?"
  // rather than "I can't say exactly who" about an appointment it is booking
 ...
 *
 * ── In the language the agent speaks, and in BOTH when it speaks two ──────
 *
 * This was "en-CA" for everybody, so a French receptionist was handed
 * "Tuesday, August 12 at 2:00 PM" and read it — an English date in the middle
 * of a French sentence, on the one line of the call the caller writes down.
 * Each locale gets its own full phrase ("mardi 12 août à 14 h"), because the
 * joining word and the clock convention are part of the language, not
 * decoration on the date.
 *
 * A bilingual agent gets both, separated by " / ", and the LANGUAGE section of
 * its prompt tells it to read only the one matching the language it is
 * speaking at that moment. The label is model input, not TTS input, so the
 * separator is never heard.
 *
 * Exported for the check script; pure.
 */
export function speak(date, timeZone, locales = ["en-US"]) {
  ...
  return list
    .map((locale) => {
      ...
      return `${day} ${AT[locale] || AT["en-US"]} ${time}`;
    })
    .join(" / ");
}

/** The word between the day and the time, per locale. */
const AT = { "en-US": "at", "fr-CA": "à", "es-419": "a las" };

/** The locale(s) this company's receptionist reads a time out in. */
function localesFor(company) {
  return spokenLocales(
    resolveSpokenLanguage(company?.voiceAgent?.spokenLanguage, company?.defaultLanguage),
...
export async function bookableSlots(companyId, preferredDate = null) {
  const company = await db.company.findUnique({
    ...
    select: {
      timezone: true,
      businessHours: true,
      // For the spoken label — see speak().
      defaultLanguage: true,
      voiceAgent: { select: { spokenLanguage: true } },
    },
  });
  const timeZone = company?.timezone || "America/Toronto";
  const locales = localesFor(company);
  ...
  for (const eventType of eventTypes) {
    ...
    for (const [, slots] of Object.entries(byDate || {})) {
      for (const iso of slots || []) {
        ...
        //
        // This was `date <= new Date()` — anything not already past. A callback
        // offered for four minutes' time is one nobody can make, so the floor
        // is the same lead time the agent talks about.
        //
        // It is a FILTER on real availability, which is the correction of a
        // mistake worth recording: this branch briefly generated callback times
        // from OPENING HOURS instead, to make them sooner. It made them sooner
        // and wrong — it booked a caller in at 8:30 with the estimator whose
        // Monday starts at three, because opening hours know nothing about
        // whose calendar a booking lands on, who is on leave, or what is
        // already taken. computeAvailableSlots knows all three.
        if (date.getTime() < Date.now() + CALLBACK_DELAY_MINUTES * 60 * 1000) continue;
        // And never when the business is shut, which personal availability can
        // disagree with: somebody available at seven should not have a customer
        // rung on the company's behalf before it opens.
        if (!withinBusinessHours(date, timeZone, company?.businessHours)) continue;
        found.push({
          id: slotId(eventType.id, iso),
          eventTypeId: eventType.id,
          iso,
          at: date.getTime(),
          label: speak(date, timeZone, locales),
        });
      }
    }
  }
...
export async function bookSlot({
  companyId,
  // The VoiceCall this booking came from, when there is one. Used to link the
  // client back onto the call so the quote draft attaches to the same person
  ...
  reason = null,
  mode = null,
  // Who took the booking. "phone_assistant" for the receptionist on a call,
  // "ai_employee" for the chat employee (lib/aiEmployee/tools.js). Read by
  // app/app/appointments/page.js to badge a visit nobody at the company
  // spoke to — the same badge, whichever robot arranged it.
  ...
  const [eventTypes, company] = await Promise.all([
    ...
    db.company.findUnique({
      where: { id: companyId },
      // The whole row, as before, plus the one VoiceAgent column the closing
      // sentence's date is spoken in.
      include: { voiceAgent: { select: { spokenLanguage: true } } },
    }),
  ]);
  ...
  //
  // Checked for the MODE being booked, below, once that is decided — the fee
  // is per mode now (lib/booking/fee.js), and a free phone call at a company
  // that charges for the in-person visit is still a free phone call.
  ...
  // ── The SAME list the prompt and the tools were built from ─────────────
  //
  // This read offeredModes(company) — every mode the company offers — while the
  // agent was given phoneBookableModes(), which withholds "visit" from a
  // company that charges for a consultation. On a real call that mismatch was
  // fatal, and it deadlocked:
  //
  //   phone modes are ["call"], so toolDefinitions omits BOTH the `mode`
  //   parameter (only sent when there is a choice) and the `address` parameter
  //   (only sent when a visit is possible). The agent therefore cannot say
  //   "this is a call". `mode` arrives undefined, this line fell to
  //   offered[0] — "visit" — and the address guard below refused for a missing
  //   address the agent had no field to send. It asked the caller for the
  //   address, got it, re-sent the identical arguments, and was refused again.
  //
  // So the mode comes from visitPolicy, which is the one function the prompt,
  // the tool definitions and the slot list all already read. A second answer to
  // "what may this agent arrange?" is exactly the copy that rots.
  const offered = visitPolicy({ company, eventTypes }).bookableModes;
  const bookedMode = offered.includes(mode) ? mode : offered[0] || offeredModes(company)[0];
  const { feeCents } = effectiveBookingFeeCents(company, eventType, bookedMode);
  ...
  // ── A visit with no address is a van with nowhere to go ─────────────────
  //
  // This used to book it anyway: `location` fell back to eventType.location,
  // which is a LABEL ("On-site visit"), so the appointment showed a time, a
  // name and a phrase that reads like a destination and isn't. Somebody was
  // expected in a driveway nobody had written down.
  //
  // Enforced here rather than by making `address` a required tool parameter,
  // and that is the whole reason it is a refusal instead: a model fills a
  // required field, and the value it fills an unknown address with is invented.
  // An invented street is worse than a failed booking, because the failed one
  // comes back to the agent, which asks for the address and tries again.
  if (wantsAddress && !visitAddress) {
    return { ok: false, reason: "address_required" };
  }

  // ── A phone call is not a consultation ──────────────────────────────────
  //
  // The event type's duration describes the appointment it was CREATED for —
  // "Consultation with Daniel", sixty minutes, somebody sitting in a kitchen.
  // The phone books callbacks against that same type because it is the free
  // one, so a five-minute "can you ring me back" was reserving a full hour of
  // that person's day. Two of them took Daniel's Monday down to nothing.
  //
  // The company now has a length of its own for a call and for a video call
  // (Company.callMinutes / videoMinutes — lib/booking/bookingModes.js), and
  // when they have set one it is used as they set it. When they have NOT,
  // the callback floor still applies and it only ever SHORTENS: a callback
  // that runs long costs nobody a booking, where twenty minutes blocked for a
  // five-minute call costs them the slot beside it. A visit keeps the event
  // type's own length exactly, because that one really is somebody driving
  // over.
  const modeMinutes = bookingDurationMinutes({ company, eventType, mode: bookedMode });
  const durationMin =
    bookedMode === "call" && !company.callMinutes
      ? Math.min(CALLBACK_MINUTES, modeMinutes)
      : modeMinutes;
  ...
  // ── Matching on a RAW phone string is how six Emilios happened ──────────
  //
  // This was a findFirst keyed on companyId plus phone — an exact string
  // compare. The caller rings from +18192387263 and their client record says
  // "819-238-7263", so nothing matched and a new client was created. Every
  // booking from that number minted another one, and by the time anybody
  // looked there were four records for one man, which then made him
  // AMBIGUOUS to the quote drafter — so the next call attached to nobody at
  // all, and the quote opened with no client, no address, and Ontario tax on
  // a job in Gatineau. One bad comparison, three screens of consequences.
  //
  // matchCallerToClient is the matcher that already gets this right: it
  // normalises through toE164 and normaliseEmail, prefers email over phone
  // over caller ID, and refuses rather than guessing when several match. It is
  // shared rather than reimplemented — the whole failure above is what a second
  // copy of "is this the same person?" costs.
  const { clientCandidates, matchCallerToClient } = await import("@/lib/ai/callQuoteDraft");
  // The email is a MATCHING KEY as well as a destination — matchCallerToClient
  // prefers it over a phone number, because two people share a landline far
  // more often than they share an inbox.
  const cleanEmail = normaliseEmail(email);
  const contact = { name: clientName, email: cleanEmail, phone, fromE164: phone };
  const candidates = await clientCandidates(companyId, contact);
  const matched = matchCallerToClient(contact, candidates);

  let clientId = matched.clientId;

  // ── When several match, attach to the OLDEST rather than mint another ────
  //
  // The drafter's answer to ambiguity is to attach nothing, which is right for
  // it: a quote on the wrong customer is worse than a quote on nobody. A
  // booking cannot do that — Appointment.clientId is required, somebody is
  // expected at three o'clock — so the choice is between a new duplicate and a
  // considered guess.
  //
  // The oldest is the considered guess: the duplicates are what the bug above
  // created, so the original record is the one with the customer's history on
  // it. It is never silent — the reason goes in the notes, where the person
  // ringing them back will read it.
  ...
  if (!clientId && candidates.length) {
    ...
    clientId = oldest.id;
    ambiguityNote = `More than one client on file matches this caller (${matched.ambiguous || "several"}). Attached to the oldest record — check it is the right one.`;
  }
  ...
  if (!clientId) {
    const created = await db.client.create({
      data: {
        companyId,
        name: clientName,
        // Null when we genuinely have none: an empty string reads as "we asked
        // and they have none" while null reads as "nobody asked", and only one
        // of those is true. The agent asks for it now, so it is usually set.
        email: cleanEmail,
        phone: phone || null,
  ...
  //
  // The owner asked that the call actually collect who, where, on what number
  // and what for — and the appointment had nowhere to put the last of those.
  // Whoever took the job saw a name, a time and, when they were lucky, a
  // street; the reason the caller rang lived only in the transcript, which
  // nothing on the schedule links to.
  //
  // The number is written into the note as well as onto the client, because
  // the two are allowed to differ: a repeat caller matched on an old number
  // keeps the client record they already had, and the number they asked to be
  // rung on today is only recorded here.
  const noteReason =
    typeof reason === "string" && reason.trim() ? reason.trim().slice(0, 1000) : null;
  const notes =
    [
      noteReason,
      ambiguityNote,
      ...
      visitAddress ? `Address: ${visitAddress}` : null,
      source === "ai_employee" ? "Booked by the AI employee." : "Booked by the phone assistant.",
    ]
      .filter(Boolean)
      .join("\n") || null;
  ...
  const appointment = await db.appointment.create({
    data: {
      ...
      // Only ever a real street. The fallback to eventType.location used to
      // sit here and it is exactly what the comment above warns against: it
      // wrote "Phone or on-site visit" into the field a crew member reads as an
      // address, and that string is now sitting on a real appointment in
      // production. It became unreachable when bookSlot started REFUSING a
      // visit with no address, so leaving it in only preserved the chance of
      // writing a label where a destination belongs.
      location: wantsAddress ? visitAddress : null,
      ...(point && { latitude: point.lat, longitude: point.lng }),
      status: "scheduled",
      // Written, and read: /app/schedule and the appointment drawer both show
      // this. A field nobody displays would be the failure AGENTS.md opens with.
      notes,
  ...
  const booking = await db.booking.create({
    data: {
      ...
      // Empty string rather than null when we have none — the column is
      // required — but the agent asks for it now, and finalizeBooking sends the
      // confirmation the moment it is a real address instead of skipping it.
      ...
      mode: bookedMode,
      status: "confirmed",
      // Who took it. The calendar badges this, so somebody driving to a job can
      // tell an appointment a person made from one a robot did.
      source,
      address: visitAddress,
      ...(point && { latitude: point.lat, longitude: point.lng }),
  ...
  // The client, back onto the call. Without this the quote drafted from the
  // same call runs its own resolution and can land on a different answer than
  // the appointment did — two records for one caller, one screen apart.
  if (callId) {
    await db.voiceCall
      .updateMany({ where: { id: callId, companyId }, data: { clientId: client.id } })
      .catch(() => {});
  }
  ...
  try {
    ...
    followUp = await finalizeBooking({ company, eventType, booking, clientId: client.id });
  } catch (err) {
    console.error("[voice] booking follow-up failed:", err?.message);
  }

  // The estimator's Google Calendar, if they connected one — and for a video
  // call, the Meet link the confirmation prints. Never blocks the call.
  ...
  return {
    ...
    // saying it to people who were never sent one. Read from what the
    // follow-up REPORTS, not from the row: when finalizeBooking never ran
    // (it failed to load, or threw), an email address on the booking was
    // still being turned into "you'll get a confirmation shortly" for a
    // letter nobody posted.
    confirmationSent: Boolean(followUp?.emailed),
    ...
    mode: bookedMode,
    label: speak(when, company?.timezone || "America/Toronto", localesFor(company)),
  };
}
