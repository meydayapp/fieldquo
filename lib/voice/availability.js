...
import { nextCallbackTimes } from "./callbackWindow";
...
export async function bookableSlots(companyId, preferredDate = null) {
  ...
  // ── A callback is not an appointment slot ───────────────────────────────
  //
  // These used to come out of the same availability an on-site VISIT is booked
  // from, so a caller asking "can someone ring me back?" at seven in the
  // evening was offered Thursday at three and Monday the seventh — the next
  // free ESTIMATE slots, days away. Right for a visit, where somebody drives
  // over and blocks out two hours. Absurd for a ten-minute phone call.
  //
  // phoneBookableModes already puts "call" first for any company that offers
  // one, so the primary mode is the honest test of what this agent is
  // arranging. When it is a callback, the times come from the clock and the
  // company's OPENING HOURS — see lib/voice/callbackWindow.js, including why
  // a company with no hours on file is offered nothing at all rather than a
  // guessed nine-to-five.
  if (policy.bookableModes[0] === "call") {
    return await callbackSlots(companyId, [...allowed], timeZone);
  }
...
/**
 * Callback times, paired with a real appointment type so they can be booked.
 *
 * The event type is what makes a callback land on somebody's calendar at all —
 * Appointment needs an owner and Booking needs an eventTypeId — so a computed
 * time is attached to the first free type exactly as a calendar slot would be.
 * Everything downstream, including the clash check in bookSlot, is unchanged.
 */
async function callbackSlots(companyId, allowedIds, timeZone) {
  if (!allowedIds.length) return [];

  const [company, eventType] = await Promise.all([
    db.company.findUnique({
      where: { id: companyId },
      select: { businessHours: true, timezone: true },
    }),
    db.eventType.findFirst({
      where: { companyId, active: true, id: { in: allowedIds } },
      select: { id: true },
    }),
  ]);
  if (!eventType) return [];

  const times = nextCallbackTimes({
    now: new Date(),
  ...
  // Anything already booked on this type is not free, the same rule the
  // calendar path applies — two callbacks at the same minute is one person
  // double-booked.
  const from = times[0].at;
  const to = new Date(times[times.length - 1].at.getTime() + 60 * 60 * 1000);
  ...
  const busy = new Set(taken.map((b) => b.startTime.getTime()));

  return times
    .filter((t) => !busy.has(t.at.getTime()))
    .map((t) => ({
      id: slotId(eventType.id, t.at.toISOString()),
      iso: t.at.toISOString(),
      // "Monday, September 7 at 10:15 a.m." is how you say a date next week and
      // a strange way to say twenty past the hour. Today gets the short form.
      label: t.sameDay ? speakTime(t.at, timeZone) : speak(t.at, timeZone),
    }));
}

/** Just the clock time, for something happening within the hour. */
function speakTime(date, timeZone) {
