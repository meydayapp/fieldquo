...
export async function bookSlot({
  ...
  //
  // Overriding the company's own number is deliberate and it only ever
  // SHORTENS: a callback that runs long costs nobody a booking, where an hour
  // blocked for a five-minute call costs them every other slot in it. A visit
  // keeps the configured duration exactly, because that one really is somebody
  // driving over.
  const durationMin =
    bookedMode === "call"
      ? Math.min(CALLBACK_MINUTES, eventType.durationMinutes || CALLBACK_MINUTES)
