...
import { visitPolicy } from "./visitPath";
  ...
  const [company, eventTypes] = await Promise.all([
    db.company.findUnique({
      ...
      select: {
        ...
        currency: true,
        // Whether they do in-person visits. bookSlot() writes mode "visit" and
        // creates an appointment somebody drives to, so a phone/video-only
        // company must not have one booked on its behalf.
        bookingModes: true,
        bookingSlug: true,
        slug: true,
      },
