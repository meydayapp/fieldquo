...
export const PROVIDER_CHOICES = {
  ...
  vocab_specialization: "general",
  ambient_sound: null,
  // Deliberately OFF, and it stays off. Turning it on was tried while chasing
  // dead air and it is the wrong lever: it lets the provider vary, per turn,
  // the pace the owner just picked on the settings screen, which turns an
  // exposed control into a suggestion. The dead air was never responsiveness —
  // `responsiveness` is already at its maximum — it was tools that went silent
  // for the length of an HTTP request. See speak_during_execution in tools.js.
  ...
  volume: 1,
  end_call_after_silence_ms: 60000,
  // ── Ten seconds was OUR number, not the provider's ───────────────────
  //
  // This is how long the agent waits after ITS OWN last sentence before
  // prompting again, and a real caller sat through exactly ten seconds of it
  // and said "Are you there?". On a phone call that is an eternity: a person
  // who has stopped hearing anything after four seconds assumes the line is
  // dead, and the one who says so is the polite one — the rest hang up.
  //
  // Twice, rather than once, for the same reason. A single reminder that the
  // caller talks over leaves the agent silent for the rest of the call, and the
  // provider skips a reminder that gets interrupted.
