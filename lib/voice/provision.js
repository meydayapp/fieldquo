...
import { pickableVoices, pickDefaultVoice } from "./voices";
// Which language(s) the phone speaks: the agent's own setting, falling back
// to the company's language. One rule shared with prompt.js and
// availability.js so the provider, the greeting and a spoken slot agree.
import {
...
// How it SOUNDS — the four choices the owner makes on the settings screen, plus
// the eighteen knobs we set for them. Every one of these used to be whatever
// Retell shipped that week; see lib/voice/agentTuning.js for the reasoning on
// each, and for why the ASR vendor is the one field we deliberately do not pin.
import { agentTuningPayload, llmTuningPayload } from "./agentTuning";
...
async function factsFor(companyId) {
  const [company, enabled, areas] = await Promise.all([
    db.company.findUnique({
      ...
      select: {
        ...
        // ── Facts a caller asks for and the agent could not answer ────────
        //
        // All of these were in the row already and none of them reached the
        // prompt. "Where are you?" got a city. "Do you take e-transfer?" got
        // nothing. A receptionist that cannot answer the three easiest
        // questions on the phone is the one nobody trusts with the fourth.
        address: true, website: true, paymentMethods: true,
      },
    }),
  ...
  return {
    ...
    // Passed through so companyFacts can decide; it omits an absent one rather
    // than saying "not set", the same rule every other fact here follows.
...
async function voiceFor(agent, language) {
  // Their own choice always wins, and costs nothing to honour.
  if (agent?.voice) return agent.voice;

  // ── Otherwise the best CURATED voice, resolved live ─────────────────────
  //
  // Every agent shipped on 11labs-Adrian: the most expensive voice Retell
  // sells — $0.040/min against $0.015 for everything else — and the one its own
  // comparison warns about, "exact spelling is less reliable and you may notice
  // occasional pacing or tone quirks". On a receptionist that reads phone
  // numbers back digit by digit and spells email addresses aloud that is not
  // small print; both have been got wrong on real calls.
  //
  // Resolved from /list-voices rather than written down. A default typed from
  // memory is the trap the whole picker exists to avoid: an unknown voice_id
  // fails /create-agent outright, so the company does not get a worse voice —
  // their receptionist is left unprovisioned.
  //
  // The old ElevenLabs id is the fallback, deliberately: if the provider cannot
  // be reached, a working agent on a pricier voice beats no agent at all.
  const fallback = language === "fr" ? DEFAULT_VOICE_ID_FR : DEFAULT_VOICE_ID;
  try {
    const raw = await listVoices();
    return pickDefaultVoice(pickableVoices(raw, { language }), { language, fallback });
  } catch {
    return fallback;
  }
}
...
export async function buildAgentConfig(companyId, origin) {
  ...
  //
  // The agent's own choice from Settings › Voice, or the company's language
  // when nobody has chosen. A pair goes to Retell as an ARRAY of two locales,
  // primary first (Retell falls back to the first when it cannot tell which
  // language a reply is in), and never as the deprecated `multi` scalar — see
  // lib/voice/agentLanguage.js for the documentation this was read from.
  const speaks = resolveSpokenLanguage(agent?.spokenLanguage, facts.language);
  const locales = spokenLocales(speaks, facts.language);
  const voiceLanguage = primaryLanguage(speaks, facts.language);
  const voiceId = await voiceFor(agent, voiceLanguage);
  ...
  const llmPayload = {
    ...
    begin_message: buildGreeting({
      company: facts.company,
      greeting: agent?.greeting,
      language: facts.language,
      spoken: speaks,
    }),
    // bookableModes travels with the tools: without it the descriptions say
    // "come out" to a phone-only company. See MODE_WORDS.
    general_tools: toolDefinitions(origin, { canBook, transferTo, bookableModes: visit.bookableModes }),
    // The model, its temperature, the priority tier and strict tool calls.
    // Spread LAST so it is obvious nothing above it can be overwritten by
    // tuning — the prompt is not something this file is allowed to touch.
    ...llmTuningPayload(),
  };

  const agentPayload = {
    agent_name: `${facts.company.name} — receptionist`,
    language: retellLanguage(locales),
    voice_id: voiceId,
    ...
    // Interruption, denoising, responsiveness, backchannel and the handbook —
    // built from the company's four saved choices, or from the defaults when
    // they have never opened the screen. Read straight off the VoiceAgent row,
    // so the value the settings card shows and the value the phone runs come
    // from the same place; there is no second copy to drift.
    ...agentTuningPayload(agent),
    ...(webhookUrl ? { webhook_url: webhookUrl } : {}),
  };

  return { agent, facts, canBook, visit, transferTo, webhookUrl, maxCallMs, llmPayload, agentPayload, locales, voiceId };
}

/**
 * A fingerprint of the INSTRUCTIONS we would push for this company.
 *
 * Covers the prompt, the greeting and the tool definitions — the three things
 * that change when WE change them, as opposed to when the company does. It
 * deliberately excludes the agent payload: `max_call_duration_ms` moves with the
 * credit balance and `webhook_url` moves with the deployment origin, so hashing
 * either would report drift on every run for ever.
 *
 * Tools are stringified as sent rather than key-sorted: the array order is
 * deterministic (toolDefinitions builds it the same way every time) and a
 * reordering IS a change worth re-pushing.
 */
export function instructionsHash(llmPayload) {
  const material = JSON.stringify({
    prompt: llmPayload?.general_prompt ?? "",
    greeting: llmPayload?.begin_message ?? "",
    tools: llmPayload?.general_tools ?? [],
  });
...
export async function provisionAgent(companyId, origin) {
  ...
  const { agent, facts, canBook, transferTo, webhookUrl, maxCallMs, llmPayload, agentPayload, locales, voiceId } =
  ...
  try {
    ...
    try {
      ...
      const outboundPayload = {
        ...
        // The same locale(s) and the same voice as the inbound agent, resolved
        // once in buildAgentConfig rather than twice: a client rung back by a
        // bilingual business is rung back by the same bilingual receptionist.
        language: retellLanguage(locales),
        voice_id: voiceId,
        ...
        max_call_duration_ms: maxCallMs,
        // ...and the same four choices, for the same reason. A client who rang
        // yesterday and is rung back today is talking to one business. Tuning
        // only the inbound half would give the same company two receptionists
        // that sound different, and the caller has no way to know why.
        ...agentTuningPayload(agent),
        ...(webhookUrl ? { webhook_url: webhookUrl } : {}),
      };
    ...
    await db.voiceAgent.upsert({
      ...
      create: {
        ...
        // Written only here, where the push actually succeeded. Stamping it
        // before the provider accepted it would tell the resync everything is
        // current while the provider still holds last week's prompt.
        provisionedHash: instructionsHash(llmPayload),
      },
      update: {
        providerAgentId,
        providerLlmId,
