...
import { quoteTopicsForCompany, photoDestination } from "./quoteQuestions";
...
export async function buildAgentConfig(companyId, origin) {
  const [agent, facts, eventTypes, maxCallMs] = await Promise.all([
    ...
    db.eventType.count({ where: { companyId, active: true } }),
  ...
  return { agent, facts, canBook, transferTo, webhookUrl, maxCallMs, llmPayload, agentPayload };
}
