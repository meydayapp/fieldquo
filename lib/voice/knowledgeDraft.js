...
prose — return them exactly as given.`,
  ...
  // Not on complete()'s schema mode, and it cannot be: `wording` is a MAP
  // keyed by question id, and an open map is precisely what strict structured
  // outputs forbid — every object must set `additionalProperties: false` and
  // list every key in `required`. A schema built per request from the ids we
  // sent would satisfy the vendor and break the feature, because it would
  // force a rewording for every question including the ones `use` deliberately
  // left out. Changing the reply to `[{ id, wording }]` would fix that, but it
  // is a prompt change to a surface that is read aloud to strangers in the
  // company's own voice, and it belongs in its own session with its own check
  // run. See lib/ai/jsonSchema.js.
  let parsed;
  try {
