...
export function offersVisits(company) {
  return offeredModes(company).includes("visit");
}

// The set and the reading of `bookingModes` live in lib/booking/bookingModes.js
// now, with the durations and the client-facing words — this file used to
// carry its own copy of both, which is the copy that rots. Re-exported so the
// phone's callers keep importing them from here.
import { BOOKING_MODES, offeredModes } from "@/lib/booking/bookingModes";
export { BOOKING_MODES, offeredModes };

/**
 * How to SAY a mode, in the three places that have to agree about it.
 *
 * The agent's tool descriptions, the prompt and the sentence it reads out after
 * booking all name the same thing, and they were about to name three different
 * things: `classifyEventTypes` labels every event type "a visit" whatever it
 * is, the tools said "come out", and bookSlot wrote mode "visit" regardless. A
 * caller told somebody is driving over when a phone call was booked is the
 * failure a customer actually notices, so the wording comes from one table.
 */
export const MODE_WORDS = {
  visit: {
    noun: "a visit",
    // What the agent is finding times FOR.
    offer: "come out and look",
    // What it tells them has been arranged.
    booked: "someone will come out to you",
  },
  call: {
    noun: "a phone call",
    offer: "call you back",
    booked: "someone will call you",
  },
  video: {
    noun: "a video call",
    offer: "meet you on a video call",
    booked: "someone will video call you",
  },
};

/** "a visit", "a phone call", or "a visit or a phone call". */
export function modePhrase(modes, key = "noun") {
  const words = (Array.isArray(modes) ? modes : [])
    .map((m) => MODE_WORDS[m]?.[key])
    .filter(Boolean);
  if (!words.length) return MODE_WORDS.visit[key];
  if (words.length === 1) return words[0];
  return `${words.slice(0, -1).join(", ")} or ${words[words.length - 1]}`;
}

/**
 * Which modes the PHONE may arrange, as opposed to which the company offers.
 *
 * Two rules, and both come out of one real call: the agent told a caller it had
 * scheduled him for Monday at three, booked nothing, and — had he asked for
 * somebody to come out instead — could have given away a consultation the
 * company charges for.
 *
 * ── A callback is the safe default, so it is named first ──────────────────
 *
 * modePhrase() reads this array in order, and the mode named first is the one a
 * model reaches for. A callback costs the business a phone call it was going to
 * make anyway; a visit commits a van, two hours and somebody's afternoon. So a
 * company that does both has the phone offer the call first, and arrange a
 * visit only when the caller actually wants the job looked at. The prompt says
 * that in words too — ordering alone is a hint, and this is meant to be a rule.
 *
 * ── An in-person visit is only offered when it is genuinely free ──────────
 *
 * The fee lives on the EventType and the mode lives on the COMPANY, and nothing
 * tied the two together. A company charging for an in-person consultation while
 * offering a free phone one still had "visit" in bookableModes, so the agent
 * could book the free type as a visit somebody drives to. The fee was not
 * waived; it was never charged. That is the same leak classifyEventTypes exists
 * to close, arriving through the other axis.
 *
 * So when this company charges for anything, "visit" is withheld — but only
 * where a callback can take its place. A visit-only company with one free type
 * and one paid type still books the free one, because dropping it there leaves
 * them unable to book at all, which is a regression wearing a fix's clothes.
 */
export function phoneBookableModes({ company = {}, paid = [] } = {}) {
  const offered = offeredModes(company);
  const charges = Array.isArray(paid) && paid.length > 0;

  // ── A mode with a fee of its own is never booked by the phone ───────────
  //
  // Since the fee became per mode (lib/booking/fee.js) a company can charge
  // for the video call and nothing for the phone call. The agent cannot take
  // money in any mode, so a mode that charges is dropped here outright — and
  // then the older rule below still applies to the visit, whose fee is on the
  // event type rather than the company.
  const free = offered.filter(
    (m) => m === "visit" || effectiveBookingFeeCents(company, null, m).feeCents === 0,
  );

  // Withheld only when there is something to withhold it in favour of.
  const usable =
    charges && free.includes("call")
      ? free.filter((m) => m !== "visit")
      : free;

  // "call" to the front; everything else keeps the company's own order rather
  // than one nobody chose.
  return usable.includes("call")
    ? ["call", ...usable.filter((m) => m !== "call")]
    : usable;
}
  ...
  for (const et of Array.isArray(eventTypes) ? eventTypes : []) {
    ...
    if (feeCents > 0) {
      ...
      free.push({
        id: et.id,
        name: String(et.name || "a visit"),
        // ── Who the caller will actually be speaking to ──────────────────
        //
        // A caller asked "who's gonna call me?" and the agent answered "I can't
        // say exactly who" — while the appointment it had just booked was
        // called "Consultation with Daniel" and carried Daniel's user id. The
        // product knew and never told it.
        //
        // Null when the type has no owner, which is a real state: an
        // unassigned appointment lands on nobody's calendar in particular and
        // the honest answer is the vague one.
        ownerName: et.user?.name ? String(et.user.name).slice(0, 60) : null,
      });
    }
  }
...
export function visitPolicy({
  ...
  // ── What the phone may book, and the mode it was wrongly tied to ─────────
  //
  // This used to be `offersVisits(company) && free.length > 0`, which locked
  // booking to companies that do IN-PERSON visits. Everything else fell through
  // to the link or, with no link, to "someone will ring you back" — including
  // the single easiest appointment in the whole product to take on a phone
  // call. A callback has no travel, no address, no deposit and no payment, and
  // on the real call this came from the agent asked "is there a time that works
  // best for you to take the call?", got "tomorrow afternoon", and wrote it
  // down as a note. Nobody checked whether anyone was free tomorrow afternoon,
  // and nothing was booked.
  //
  // So the gate is the FEE, which is the thing the agent genuinely cannot
  // handle, and not the mode. A free bookable type is bookable in whatever
  // modes the company offers; a paid one still goes to the booking page,
  // because taking money is that page's job and this has not changed.
  //
  // The mode still matters — it decides what the agent SAYS and whether an
  // address is asked for — so it is carried rather than assumed.
  const canBook = free.length > 0;
  // Not offeredModes() directly: the phone prefers a callback and withholds a
  // visit at a company that charges for one. See phoneBookableModes.
  const modes = phoneBookableModes({ company, paid });
  ...
  return {
    ...
    // How the agent may arrange to meet them, and therefore what it may say it
    // has arranged. Empty on every non-"book" path for the same reason
    // bookableEventTypeIds is: nothing may be booked, so nothing may be named.
    bookableModes: canBook ? modes : [],
    freeVisits: free,
    paidVisits: paid,
