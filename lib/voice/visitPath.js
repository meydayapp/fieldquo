...
export const BOOKING_MODES = ["call", "visit", "video"];

/**
 * How this company is willing to meet somebody — call, visit, video.
 *
 * Same reading as canBookVisit() and BookingFlow: an empty or absent
 * `bookingModes` is the schema default ["visit"], meaning "nothing chosen yet",
 * never "nothing offered".
 */
...
export function phoneBookableModes({ company = {}, paid = [] } = {}) {
  ...
  const usable =
    charges && offered.includes("call")
      ? offered.filter((m) => m !== "visit")
      : offered;
