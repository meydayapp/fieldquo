...
/**
 * The newest consent row that is still in date, or undefined.
 *
 * Split out of consentVerdict so the settings card can ask the SAME question
 * without also asking "is it a decent hour right now?" — the card is answering
 * "would this ever be callable", not "may we dial this second", and a report
 * that said "outside calling hours" at nine in the evening would tell an owner
 * to wait for a call that consent was never going to permit.
 *
 * One copy: consentVerdict below uses it too, so the standing the card shows
 * and the standing the dialler enforces cannot drift.
 */
export function liveConsent(consents = [], now = new Date()) {
...
export function consentVerdict({ optedOut, consents = [], now = new Date(), timeZone }) {
  ...
  const live = liveConsent(consents, now);
  if (!live) {
    return {
      allowed: false,
