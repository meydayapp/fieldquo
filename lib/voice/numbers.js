...
export function isSharedTestNumber(e164) {
  return sharedTestNumbers().includes(String(e164));
}

/**
 * The same list, for readers that need the numbers rather than a yes/no.
 *
 * /platform/voice-numbers is one: a number Retell bills FieldQuo for and no
 * company holds is the billing leak that page exists to find, and the test line
 * is neither held nor a leak. Naming FieldQuo's own numbers is the only way the
 * page can tell the two apart.
 *
 * Read literally rather than through a computed key: scripts/check-env-docs
 * scans for `process.env.NAME`, and a dynamic lookup drops the variable out of
 * the deployment checklist — the same reason salesNumbers() spells its own out.
 */
export function sharedTestNumbers() {
  return (process.env.RETELL_TEST_NUMBER || "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
}
