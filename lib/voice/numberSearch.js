...
// Re-exported so every existing caller keeps its import, and so there is one
// answer to "which country is this" rather than two that can disagree.
import { countryForAreaCode, countryForRegion } from "./nanp";

export { countryForAreaCode, countryForRegion, CANADIAN_AREA_CODES } from "./nanp";
...
export async function searchLocalNumbers({
  // NOT defaulted. A default here beats the derivation below — which is
  // precisely how this stayed broken after the route's own "CA" default was
  // removed: `country || countryForAreaCode(code)` never reaches the second
  // half when the parameter has already supplied the first.
  country = null,
  areaCode = null,
  locality = null,
  region = null,
  ...
  // ── The country is DERIVED from the area code, not defaulted ───────────
  //
  // It used to be `country || "CA"`, and the panel that calls this has no
  // country selector — so every search was Canadian. Searching 716 for a list
  // of Buffalo contractors returned "none available", which is a true sentence
  // about the wrong country and indistinguishable from an empty inventory.
  //
  // An explicit `country` still wins, for a caller that knows better and for
  // the locality and region searches below, where there is no area code to
  // derive from. `countryForAreaCode` returns null for a string that is not an
  // area code at all, and the old default stands behind that — a search with
  // no area code and no country is the Canadian one it always was.
  const iso = String(country || countryForAreaCode(code) || "CA").toUpperCase();
 ...
 * already been sold.
 */
export async function isStillAvailable(e164, { country = null } = {}) {
  if (!twilioConfigured()) return null;
  const wanted = String(e164 || "");
  if (!/^\+\d{8,15}$/.test(wanted)) return false;

  // ── The country comes from the NUMBER, not from a default ──────────────
  //
  // This defaulted to "CA" and searchLocalNumbers no longer does, which made
  // the two disagree in the worst possible place: the owner searched 646,
  // found (646) 760-1763, pressed Buy, and was told "+16467601763 was taken
  // while you were choosing" — while the same number went on showing as
  // available in the list beside the message.
  //
  // Nothing had been taken. The list asked the United States and the re-check
  // asked Canada, and an empty Canadian answer for a New York number is
  // indistinguishable from somebody else having bought it. A confident,
  // specific, wrong sentence — the same shape as "no numbers free in 716",
  // and from the same missed default, because the first fix stopped at the
  // search and this is the second call site.
  //
  // A +1 number carries its own area code, so there is nothing to guess: the
  // digits are right there. `country` still wins when a caller states one, and
  // the old default stands behind a number that is not NANP at all.
  const iso = String(
    country || (/^\+1\d{10}$/.test(wanted) ? countryForAreaCode(wanted.slice(2, 5)) : null) || "CA",
  ).toUpperCase();
