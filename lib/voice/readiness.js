...
function link(id, state, reason, { fixer = null, fix = null, detail = null, blockedBy = null } = {}) {
  return { id, state, reasonKey: reasonKeyFor(id, reason), reason, fixer, fix, detail, blockedBy };
}

/**
 * "we could not ask" — the only honest answer downstream of a dead provider.
 *
 * ── `blockedBy`, and why six shrugs were worse than one ────────────────────
 *
 * A chain of ten links where link 2 is missing used to render EIGHT identical
 * rows reading "We couldn't check this one. Nothing is claimed either way." —
 * true of each one in isolation, and useless: the one row that mattered was
 * buried in a column of them, and nothing said the other seven were consequences
 * rather than seven separate problems. The owner's words about the same shape on
 * the platform console: "i have no idea what to do with that information."
 *
 * So an unknown link now names the link it is WAITING ON. The value is resolved
 * transitively below, to the first link that is actually broken — a diagnostic
 * for a chain has one job, which is to point at the first break.
 *
 * Deliberately an extra FIELD rather than a new `reason`. The reasons are
 * translated into six languages under keys the check script asserts over
 * (readinessCopy.js); a new one would need six new sentences and would change
 * what every existing screen says. This adds a fact and leaves the sentence
 * alone, so a caller that ignores it renders exactly what it rendered before.
 */
function unreachable(id, blockedBy = null) {
  return link(id, UNKNOWN, "unchecked", { fixer: "unknown", blockedBy });
}
...
export function resolveReadiness(o = {}) {
  ...
  //
  // ── "unchecked" here usually means we had nothing to ask ABOUT ───────────
  //
  // Both callers only phone the provider when there is something to look up —
  // see the `if (configured && e164)` gate in checkSalesReadiness. So a
  // configured deployment with no number produces a provider link that reads
  // "we couldn't check", which is technically true and thoroughly misleading:
  // nothing failed, nothing was asked. It is waiting on the number, and saying
  // so is what stops the row that matters being buried.
  const nothingToAsk =
    o.number?.haveRow !== true && !o.agent?.wantAgentId && !o.engine?.wantLlmId;
  const providerLink = !o.configured
    ...
    : o.providerReachable === true
      ...
      : o.providerReachable === false
        ...
        : link("provider", UNKNOWN, "unchecked", {
            fixer: "unknown",
            blockedBy: nothingToAsk ? "number" : null,
          });
  ...
  if (n.haveRow === false) {
    ...
    numberLink = unreachable("number", "provider");
  ...
  // ── 3. The agent object ──────────────────────────────────────────────────
  let agentLink;
  if (!canAsk) agentLink = unreachable("agent", "provider");
  ...
  else agentLink = unreachable("agent", "provider");
  links.push(agentLink);

  // ── 4. The response engine (the prompt object) ───────────────────────────
  let engineLink;
  if (!canAsk) engineLink = unreachable("engine", "provider");
  ...
  else if (e.existsAtProvider !== true) engineLink = unreachable("engine", "provider");
  ...
  let bindingLink;
  if (numberLink.state !== OK || agentLink.state !== OK)
    // Whichever of the two is not yet good, in chain order. Naming both would
    // be two things to fix where the first may well cure the second.
    bindingLink = unreachable("binding", numberLink.state !== OK ? "number" : "agent");
  ...
  else if (bindingLink.state === UNKNOWN) switchLink = unreachable("switch", "binding");
  else
  ...
  let webhookLink;
  if (agentLink.state !== OK) webhookLink = unreachable("webhook", "agent");
  // No expected URL is not a chain problem — nothing upstream would fix it — so
  // this one stays a plain shrug rather than pointing at an innocent link.
  ...
  let promptLink;
  if (engineLink.state !== OK) promptLink = unreachable("prompt", "engine");
  // Same distinction as the webhook link: a config we could not build is not
  // the engine's fault, and pointing at the engine would send somebody to fix
  // a link that is already fine.
  else if (!o.expected) promptLink = unreachable("prompt");
...
/**
 * Point every waiting link at the FIRST break, not at its neighbour.
 *
 * Each link names the one immediately upstream of it, which is the only thing
 * it can know locally. Left there, a page shows "waiting on the phone service"
 * against six links while the phone service is itself waiting on a number that
 * was never set — six correct sentences that between them hide the one fact
 * worth acting on.
 *
 * So each pointer is followed until it reaches a link that is genuinely broken
 * (`fail`), or one that is waiting on nothing. A cycle would hang this, and the
 * chain is hand-written and could grow one, so the walk is bounded by the
 * number of links rather than trusted to terminate.
 *
 * Mutates in place: the links are freshly built objects owned by this call.
 */
function resolveBlockedBy(links) {
  const byId = new Map(links.map((l) => [l.id, l]));
  for (const l of links) {
    if (!l.blockedBy) continue;
    let target = byId.get(l.blockedBy);
    for (let hops = 0; hops < links.length && target; hops += 1) {
      // A broken link is the answer. So is one that nothing upstream explains.
      if (target.state === FAIL || !target.blockedBy) break;
      const next = byId.get(target.blockedBy);
      if (!next || next === l) break;
      target = next;
    }
    // Never point a link at itself, and never point at a link that is fine —
    // both would send somebody to look at something that is not the problem.
    l.blockedBy = target && target.id !== l.id && target.state !== OK ? target.id : null;
  }
}
