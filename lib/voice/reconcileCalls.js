...
import { durationSecondsOf } from "./callDuration";
...
// FieldQuo's OWN line. Imported one way only — salesCall.js knows nothing about
// this file — so there is no cycle to reason about.
import { isSalesNumber, reconcileSalesCall } from "@/lib/platform/salesCall";
 ...
 *
 * Moved to lib/voice/callDuration.js when lib/platform/salesCall.js became a
 * second reader of the same question, and re-exported here so this file's own
 * rule 2 ("never invent a charge") still reads as one thing in one place.
 */
export { durationSecondsOf };
...
export async function reconcileVoiceCalls(opts = {}) {
  ...
  // FieldQuo's own sales line, recognised SECOND — see the branch below.
  // Injectable like everything else here: `isSalesNumber` reads an environment
  // variable, and a check script that had to set one before it could execute
  // this function is a check script that tests the environment.
  const ourOwnNumber = opts.isSalesNumber || isSalesNumber;
  const recordOurOwn = opts.reconcileSalesCall || reconcileSalesCall;
  ...
  const tally = {
    ...
    unknownDuration: 0,
    unknownNumber: 0,
    // Calls on FieldQuo's own sales line: seen, and (unless this run is scoped
    // to one tenant) written to PlatformVoiceCall. Deliberately NOT counted as
    // unattributed — they are attributed, to FieldQuo, and reporting them as a
    // leak is the same false alarm /platform/voice-numbers used to raise about
    // the number itself.
    platformOwn: 0,
    platformReconciled: 0,
  ...
  for (const call of items) {
    ...
    if (!number) {
      // ── FieldQuo's OWN line ────────────────────────────────────────────
      //
      // A SECOND recognition, in the same order and for the same reason as
      // app/api/voice/webhook/route.js: the tenant lookup runs first, and only
      // a call it could not place is offered here. FieldQuo's sales number is
      // configuration rather than a VoicePhoneNumber row (there is no Company
      // for it to belong to), so before this branch a dropped webhook on a
      // sales call had NO second path — no retry, no sweep, a record wrong for
      // ever, on the one line FieldQuo reads every word of.
      //
      // No billing, no attachment sync, no credit: FieldQuo's own minutes are
      // FieldQuo's own cost. See lib/platform/salesCall.js.
      if (ourOwnNumber(e164)) {
        tally.platformOwn++;
        // A scoped run is one tenant pressing "recover my missed calls". It
        // must not sweep FieldQuo's own line on their behalf — counted, so the
        // number is still explained, and not written.
        if (!onlyCompanyId) {
          try {
            const res = await recordOurOwn({
              call,
              prisma,
              fetchDetail,
              now: new Date(now),
            });
            if (res?.recorded) tally.platformReconciled++;
            else tally.errors++;
          } catch (err) {
            tally.errors++;
            console.error("[voice/meter] sales call reconcile failed:", providerCallId, err?.message);
          }
        }
        continue;
      }
