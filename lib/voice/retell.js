 ...
 *
 * ── /v3/, and this one really is load-bearing ──────────────────────────────
 *
 * Retell versions these paths one at a time (see listNumbers, still on /v2/).
 * The legacy list endpoints were REMOVED on 15/06/2026 — `POST /v2/list-calls`
 * is gone, not merely deprecated — so a reconciler written against v2 would
 * 404 on every run and report "provider unreachable" for ever, which fails
 * safe but never bills anybody. v3 also changed the filter shape: flat
 * `after_start_timestamp` scalars became TYPED filters, `{ type, op, value }`,
 * which is why the window below looks like it does rather than like two dates.
 *
 * @param sinceMs / untilMs  epoch ms, inclusive window on start_timestamp
 * @returns { items, pagination_key, has_more }
 */
