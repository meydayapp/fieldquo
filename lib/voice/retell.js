...
/**
 * The kinds of failure this boundary can produce, as a closed set.
 *
 * ── Why a kind, when there is already a status ─────────────────────────────
 *
 * Because `status` cannot say all of it. Status 0 used to mean BOTH "no key is
 * configured" and "the network never answered", which are the two failures with
 * the most different remedies in the whole file — one is a Vercel variable and
 * one is a Refresh. Every caller that wanted to tell them apart had to
 * re-derive the distinction from the message string, and every caller that
 * couldn't be bothered wrote `.catch(() => null)` and threw the answer away.
 * That is where "Couldn't read the voice numbers just now" came from.
 *
 * `status` is unchanged and still authoritative for HTTP branching (the detach
 * retry reads 400/422; readiness reads 404). `kind` is the axis a HUMAN needs.
 */
export const RETELL_ERROR_KINDS = [
  "not_configured", // no RETELL_API_KEY on this deployment. Nothing was sent.
  "rejected", // 401 — a key is set and Retell refused it
  "forbidden", // 403 — the key is accepted, this operation is not
  "rate_limited", // 429 — too many requests, nothing wrong with the setup
  "not_found", // 404 — Retell answered, and does not have this thing
  "invalid_request", // other 4xx — our body was wrong
  "provider_error", // 5xx — their end
  "timeout", // aborted at timeoutMs — no reply at all
  "unreachable", // transport failed — DNS, TLS, socket
];
...
class RetellError extends Error {
  constructor(message, { status, body, endpoint, kind, timeoutMs } = {}) {
    super(message);
    this.name = "RetellError";
    this.status = status;
    this.body = body;
    this.endpoint = endpoint;
    // Only set on a timeout. The screen quotes the actual budget rather than a
    // number typed into copy, which has already drifted once in this codebase.
    this.timeoutMs = timeoutMs ?? null;
    // Always set. A caller that branches on `kind` must never have to handle
    // undefined, or the default branch becomes the shrug this exists to end.
    this.kind = kind || (Number.isFinite(status) && status > 0 ? kindForStatus(status) : "unreachable");
  }
}
export { RetellError };
...
async function call(endpoint, { method = "POST", body, timeoutMs = 15000 } = {}) {
  ...
  if (!key) {
    throw new RetellError(
      ...
      { status: 0, endpoint, kind: "not_configured" },
    );
  }
  ...
  try {
    ...
    if (err.name === "AbortError") {
      throw new RetellError("The phone provider didn't respond in time.", {
        status: 504,
        endpoint,
        // NOT provider_error, which 504 would otherwise imply: nobody sent us a
        // 504. We invented it because we gave up waiting, and the remedy
        // ("try again") is different from a gateway failing at their end.
        kind: "timeout",
        timeoutMs,
      });
    }
...
/* ─────────────────────── a demo's line, simulated here ──────────────────────
 *
 * A demo account (Company.isDemo) may set up the receptionist end to end and
 * must never be able to make FieldQuo buy, or a stranger dial, a real phone
 * number — see lib/voice/demoLine.js for the full reasoning and
 * app/api/settings/voice/number/route.js for where a demo lands here instead
 * of the real purchase.
 *
 * ── Why the fake lives INSIDE this file, not beside it ─────────────────────
 *
 * Every caller above this boundary — provision.js, syncNumberAttachment,
 * numberRelease.js, diagnose.js, readiness.js — already calls attachAgent(),
 * getNumber() and releaseNumber() with nothing but an e164. None of them know
 * or need to know a company is a demo; they only ever see a phone number. So
 * the seam is the E.164 itself: NANP reserves NPA-555-0100 through
 * NPA-555-0199 in EVERY area code for fiction, and lib/voice/demoLine.js is
 * the only caller that ever asks buyNumber() for one. Every function below
 * that takes an e164 recognises that shape on its own and never touches the
 * network for it — which means "runs exactly like a real number" is true of
 * every layer ABOVE this file by construction, not by a flag threaded through
 * five modules that all have to remember to check it.
 *
 * createAgent/updateAgent/createRetellLlm/updateRetellLlm are deliberately
 * NOT simulated — a demo's VoiceAgent is a REAL Retell agent and LLM. Retell
 * prices calls and numbers, never a bare agent object sitting unattached, so a
 * real agent that is never bound to a real number costs nothing and lets the
 * greeting, the voice, the tuning and the prompt all demo honestly, pushed and
 * re-pushed exactly as a paying company's are.
 *
 * ── The in-memory binding cache, and its one honest limitation ─────────────
 *
 * syncNumberAttachment calls attachAgent() and then immediately getNumber() to
 * read back what it just set — the belt-and-braces check that has caught two
 * real Retell ambiguities in this codebase already. A simulated number has no
 * external state for that read-back to disagree with, so this Map is the
 * "provider" a simulated getNumber() consults: written by attachAgent, read by
 * getNumber, moments apart, in the same request. It does not survive a cold
 * start, which only matters to a caller that asks getNumber() for a simulated
 * e164 WITHOUT having attached it first in the same process — the one such
 * caller is lib/voice/diagnose.js's rarely-hit "Fix" path, and the honest
 * answer there (no attachment known) is the same one a company sees for any
 * number that has genuinely never been attached, so nothing is misreported —
 * only, occasionally, less freshly than it could be.
 */
const SIMULATED_AREA_CODES = ["613", "819", "514", "437", "416"];

/** NPA-555-01XX — reserved by NANP for fiction. Never issued to a subscriber. */
export const SIMULATED_E164_RE = /^\+1\d{3}55501\d{2}$/;

export function isSimulatedNumber(e164) {
  return SIMULATED_E164_RE.test(String(e164 || ""));
}

const simulatedBindings = new Map(); // e164 -> providerAgentId | null
const simulatedReleased = new Set(); // e164 that has been "released"

function randomSimulatedE164(areaCode) {
...
/** buyNumber's demo branch. Never touches the network. */
async function simulateBuyNumber({ areaCode } = {}) {
  const e164 = randomSimulatedE164(areaCode);
  return { phone_number: e164, phone_number_pretty: e164, nickname: "FieldQuo demo line" };
}

async function simulatedAttachAgent(e164, agentId) {
  simulatedReleased.delete(e164);
  simulatedBindings.set(e164, agentId || null);
  return { phone_number: e164, inbound_agents: agentRouting(agentId) };
}

async function simulatedGetNumber(e164) {
  if (simulatedReleased.has(e164)) {
    // Mirrors the real provider's 404 on a released number — see
    // numberRelease.js's confirmGone(), which reads this status and only this
    // status as proof the number is really gone.
    throw new RetellError("No such number.", {
      status: 404,
      endpoint: "simulated:get-phone-number",
    });
  }
  const bound = simulatedBindings.has(e164) ? simulatedBindings.get(e164) : null;
  return {
    phone_number: e164,
    inbound_agents: agentRouting(bound),
    nickname: "FieldQuo demo line",
  };
}
...
export function buyNumber({
  ...
  tollFree = false,
  country = "CA",
  // Set ONLY by lib/voice/demoLine.js, for a company whose isDemo is true.
  // Every other caller in this codebase omits it, so nothing about the real
  // purchase path below changes for a single real company. See the "a demo's
  // line, simulated here" block above for the whole design.
  demo = false,
}) {
  if (demo) return simulateBuyNumber({ areaCode });
...
export function releaseNumber(e164) {
  if (isSimulatedNumber(e164)) {
    simulatedReleased.add(e164);
    simulatedBindings.delete(e164);
    return Promise.resolve({ phone_number: e164, deleted: true });
  }
 ...
 * syncNumberAttachment.
 */
export async function getNumber(e164) {
  if (isSimulatedNumber(e164)) return simulatedGetNumber(e164);
...
export async function attachAgent(e164, agentId) {
  if (isSimulatedNumber(e164)) return simulatedAttachAgent(e164, agentId);
 ...
 * @param statuses  which `call_status` values to ask for. Defaults to the two
 *                  the METER wants; pass null to drop the filter entirely and
 *                  see calls that are still running. Only one caller does that
 *                  — findPlacedCallForTask in lib/voice/outboundCall.js, whose
 *                  question is "did we already dial this?", for which a call
 *                  placed ninety seconds ago and still connected is the single
 *                  most important answer and the one an ended-only filter hides.
...
export function listCalls({
  ...
  if (Array.isArray(statuses) && statuses.length) {
    filter.call_status = { type: "enum", op: "in", value: statuses };
  }
 ...
 */
/**
 * Every voice this account may use.
 *
 * ── Fetched, never hardcoded ──────────────────────────────────────────────
 *
 * `voice_id` is required by /create-agent and an invalid one fails the whole
 * push — so a curated list typed from memory is a list that silently breaks
 * somebody's receptionist the first time a name is wrong or a voice is retired.
 * The picker is populated from this, and a submitted id is only accepted if it
 * appears here.
 *
 * https://docs.retellai.com/api-references/list-voices
 *
 * Returns [{ voice_id, voice_name, provider, gender, accent?, age?,
 * preview_audio_url? }] where provider is one of elevenlabs, openai, cartesia,
 * minimax, fish_audio, platform.
 */
export function listVoices() {
  return call("/list-voices", { method: "GET" });
}
