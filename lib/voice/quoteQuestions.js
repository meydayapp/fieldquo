...
const TRADE_FIELD_CATEGORY = {
  ...
  // The gutter estimator (5378cfbe) files under Gutters & Eavestroughs. The
  // category has no intake fields today — see ASKS_ONLY_THE_ADDRESS — but
  // the day it gets some, the phone must find them under this key rather than
  // under "gutters", which names no category at all.
  gutters: "gutter_services",
};

/**
 * Trades whose phone questions are the job address and nothing else, BY
 * DECISION, with the reason. Not a way to silence the check below: an entry
 * is honoured only while the trade maps to its own catalogue category and
 * that category really has no intake fields and the measure shape reads
 * nothing from the conversation. Give the category a field and the entry
 * stops mattering; map the trade wrong and it stops protecting it.
 */
const ASKS_ONLY_THE_ADDRESS = {
  gutters:
    "the run and the downspouts come off the roof model at the address, and " +
    "storeys are confirmed on site (lib/estimate/callEstimate.js, gutter_address); " +
    "gutter_services carries no intake fields anywhere in the product",
};
...
export function unmappedFieldTrades() {
  ...
  for (const trade of Object.keys(INSTANT_ESTIMATE_TRADES)) {
    ...
    const decidedAddressOnly =
      Object.prototype.hasOwnProperty.call(ASKS_ONLY_THE_ADDRESS, trade) &&
      fieldCategoryFor(trade) === categoryKeyForTrade(trade) &&
      shape.needsAddress === true &&
      !(shape.required || []).length &&
      !(shape.reads || []).length;
    if (!decidedAddressOnly) out.push(trade);
  }
  return out;
}
