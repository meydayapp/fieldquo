...
//
// ══ Rule 6 used to be two rules. The owner asked for one ═══════════════════
//
// Same merge as lib/voice/prompt.js rule 5, and the same shared text: an
// outbound call this product places can reach someone in exactly the state
// CRISIS_RULE describes just as easily as an inbound one can, whether that's a
// property emergency or a personal one, and there is no reason for an outbound
// call to say it differently than an inbound one. See lib/ai/crisisRule.js for
// why the text is not duplicated here, and for why it is now one rule instead
// of two.

import { CRISIS_RULE } from "@/lib/ai/crisisRule";

/**
 * A quote total as the agent would say it.
 *
 * Lives here, with no import of its own, because two callers need the exact
 * same string and neither may have its own copy: triggers.js writes it onto the
 * queued task, and outboundCall.js re-derives it at dial time to check the
 * figure has not moved since. A second formatter would make that comparison
 * fail on formatting and silently drop a figure that never changed — or, worse,
 * pass on two spellings of two different numbers. (The file itself now imports
 * CRISIS_RULE, above — that import is for OUTBOUND_RULES, not this function.)
 *
 * Rounded to whole units: nobody reads out cents on the phone, and "fourteen
 * thousand four hundred and ten" is what the client's email says too.
 */
export function spokenTotal(amount, currency = "CAD") {
...
export const PURPOSES = {
  ...
  quote_approved: {
    ...
    brief: (v) =>
      [
        // The email went first — the callback is only ever queued once the quote
        // has actually been emailed (lib/voice/triggers.js). So the agent is
        // discussing a document they are holding, not announcing a number, and
        // saying so is what keeps this inside rule 3.
        "They have already been emailed this quote. You are talking about a document they have in writing — you are not delivering a price.",
        v.quoteTotal
          ? `The quote they were sent came to ${v.quoteTotal}. You may read that figure back to them. You may NOT change it, round it, discount it or add to it.`
          : `You do not have the quote figure. Do not guess it and do not work one out — point them at the email and say someone will go through the numbers with them.`,
