 ...
 */
export function claimVerdict({ entries, numberRowsCreatedAt = [], now = new Date(), windowMs = CLAIM_WINDOW_MS }) {
  const at = (v) => {
  ...
  let newest = null;
  for (const e of rows) {
    if (e.kind !== "number_setup") continue;
    if (typeof e.ref !== "string" || !e.ref.startsWith("number_setup:")) continue;
