...
import { accessForCompany, CANCELLED_DAYS } from "@/lib/billing/access";
import { IMAGE_GENERATION_CENTS, VISION_PASS_CENTS, MATERIAL_LIST_CENTS } from "@/lib/ai/imageEconomics";
...
import { releaseHeldNumber, standDownIfLastNumber } from "./numberRelease";
import {
  balanceFor,
  poolForKind,
  POOLS,
...
export const SPEND_KINDS = {
  ...
  /**
   * One generated marketing image, and one paid vision pass over a quote.
   *
   * ── Why these are on the SAME balance as the phone ────────────────────────
   *
   * This ledger is already not "the voice balance" — it carries crew texts and
   * crew line rental too, and lib/voice/credits.js frames it as one prepaid
   * balance for everything FieldQuo spends on a tenant's behalf. A contractor
   * who has topped up once, and is then told this particular AI needs a
   * different wallet, feels cheated and is right to.
   *
   * Both are one-off: nothing recurs, nothing is reserved monthly. A company
   * that generates nothing pays nothing, which is the property that makes a
   * prepaid balance fair rather than a subscription in disguise.
   */
  image_generation: { key: "image_generation", recurring: false },
  image_vision: { key: "image_vision", recurring: false },
  /**
   * One AI-built material list for a job. Same wallet as the two above and
   * for the same reason; text only, so the cost basis is in
   * lib/ai/imageEconomics.js beside them rather than in a file of its own.
   */
  material_list: { key: "material_list", recurring: false },
  /**
   * The AI employee's replies and its front-desk triage — the company's own
   * chat bot, the company's to pay (owner, 2026-09-25). Same AI wallet as the
   * three above, but with NO flat price: a reply's cost moves with the
   * conversation, so it is charged from its own token counts by
   * lib/ai/walletMeter.js, which also holds its affordability gate.
   *
   * `perCall` makes spendVerdict REFUSE these kinds. priceSpend has no price
   * for them, and a gate that read "costs 0¢ → allowed" would be a free
   * pass for any future caller that reached for checkSpend by habit.
   */
  ai_employee_reply: { key: "ai_employee_reply", recurring: false, perCall: true },
  ai_employee_front_desk: { key: "ai_employee_front_desk", recurring: false, perCall: true },
};

/**
 * Which feature has to be switched on for this kind to spend anything.
 *
 * spendAvailable() asked `voice_receptionist` for every kind, which was right
 * while every kind was a phone. It is not right for a marketing image: a
 * company that FieldQuo has withdrawn the receptionist from must still be able
 * to make an advert.
 *
 * Only the image kinds are mapped away from the default here. The crew kinds
 * arguably belong to `crew_inbox`, but nothing routes them through checkSpend
 * today (lib/crew/messaging.js debits the ledger directly), so re-pointing them
 * would be a behaviour change with no caller to verify it against — a separate
 * job, done deliberately, not a side effect of adding images.
 *
 * An unmapped key resolves to hidden and therefore refuses, which is why the
 * two new keys can be named here before the features exist: the plumbing is
 * inert until the registry entry and the feature land together.
 */
export const FEATURE_FOR_KIND = {
  image_generation: "marketing_designer",
  image_vision: "ai_vision",
  material_list: "ai_material_list",
};
...
export function priceSpend(kind, numberType = "local") {
  switch (kind) {
    ...
    // Flat, and deliberately not per-photo or per-pixel. The cost basis and the
    // margin arithmetic live in lib/ai/imageEconomics.js next to the vendor
    // rates they were derived from, because a price with no recorded reasoning
    // is a price nobody can safely change later.
    case "image_generation":
      return IMAGE_GENERATION_CENTS;
    case "image_vision":
      return VISION_PASS_CENTS;
    case "material_list":
      return MATERIAL_LIST_CENTS;
    default:
      return 0;
  }
}
...
export function spendVerdict({ kind, numberType = "local", balanceCents }) {
  ...
  if (SPEND_KINDS[kind].perCall) {
    // Priced per call from token counts, gated in lib/ai/walletMeter.js.
    return { allowed: false, kind, needCents: 0, balanceCents: balance, shortfallCents: 0, reason: "priced_per_call" };
  }
 ...
 */
export async function spendAvailable(companyId, kind = null) {
  return featureAllowsSpend(companyId, FEATURE_FOR_KIND[kind] || "voice_receptionist");
}
...
export async function checkSpend({ companyId, kind, numberType = "local", prisma = db, available }) {
  ...
  const offered = available === undefined ? await spendAvailable(companyId, kind) : available;
  if (!offered) {
    return {
      allowed: false,
      kind,
      ...
      balanceCents: await balanceFor(companyId, prisma, poolForKind(kind)),
      shortfallCents: 0,
  ...
  // The kind's OWN wallet. Reading the voice balance to decide whether an
  // image may be generated would let a phone top-up pay for pictures.
  return spendVerdict({ kind, numberType, balanceCents: await balanceFor(companyId, prisma, poolForKind(kind)) });
}
 ...
 */
export async function refundReservation({ companyId, ref, cents, note, forKind = null }) {
  if (!companyId || !ref) return null;
  return addCredit({
    companyId,
    cents,
    // A refund must land in the wallet the money came out of. `forKind` is the
    // kind that was reserved; without it an AI refund would be credited to the
    // phone balance, which is a transfer between wallets dressed as a refund.
    // Defaulting to the voice adjustment keeps every existing caller identical.
    kind: poolForKind(forKind) === POOLS.AI ? "ai_adjustment" : "adjustment",
 ...
 * @param access     accessFor() output — whether they still have a subscription.
 *                   Defaults null, which means "don't ask", so every existing
 ...
 *   warn_cancelled they cancelled; rent has STOPPED and the number goes back on
 *                 a stated date. Warned on the same cadence as an unpaid one.
 *   skip          not an active rented number (porting, released, no rental),
 *                 or FieldQuo has withdrawn the feature
 */
export function rentDecision({ number, balanceCents, now = new Date(), available = true, access = null }) {
  ...
  // ── A demo's line is never billed ────────────────────────────────────────
  //
  // Checked here, explicitly, for the same reason `feature_unavailable` is
  // checked first: the alternative is an accident. `rentFor()` below would
  // already return 0 for a row created with `monthlyCents: 0` and fall into
  // `no_rental` a few lines down — a TRUE answer for the wrong REASON, and one
  // that would silently stop being true the day anything else legitimately
  // zeroes that column. Naming the real cause here means a reviewer asking
  // "why does this number never bill" gets an answer instead of a coincidence.
  //
  // `simulated` rather than a company lookup: this function is pure (a row, a
  // balance, a clock) and stays that way — the row already carries the one
  // fact that matters, set once by lib/voice/demoLine.js at creation, and nothing
  // here needs to know or ask what a demo account is.
  if (number.simulated) return { action: "skip", reason: "simulated" };
  ...
  // ── They cancelled FieldQuo ──────────────────────────────────────────────
  //
  // Nothing used to notice. The subscription ended, the account went read-only
  // and then locked, and this cron carried on taking US$4 a month out of a
  // prepaid balance for a receptionist nobody could reach any more — silently,
  // for as long as the credit lasted, on an account with nobody logging in to
  // see it. Then it released the number as a DELINQUENCY, with an email about
  // an unpaid rental, which is not what happened.
  //
  // Two distinct states, and the difference is the whole point:
  //
  //   readonly  they cancelled and the 30-day window is open. Charging stops
  //             IMMEDIATELY — they are not using it and would not know they
  //             were paying. The number is still theirs, and it is still
  //             answering: they may resubscribe, and taking a contractor's
  //             business line away during a window we ourselves describe as
  //             "not a punishment" would make it exactly that.
  //   locked    the window has closed. Nobody is coming back, and FieldQuo is
  //             paying the carrier every month for a number that rings for a
  //             company that no longer exists. It goes back.
  //
  // ── past_due is deliberately NOT a release ───────────────────────────────
  //
  // A declined card produces `locked` too, seven days after a bank's fraud
  // hold. Releasing on that would destroy the number printed on somebody's van
  // over a payment problem they may not know about, and it cannot be undone.
  // Only `canceled_expired` — a DECISION, plus thirty days — releases anything.
  // Unpaid rent has its own grace path below and always did.
  // ── FieldQuo ended it for a terms breach ─────────────────────────────────
  //
  // lib/billing/access.js answers `locked` with reason "terms" the moment the
  // platform sets Subscription.accessLockedAt — no thirty-day window, because
  // that window is for people who chose to leave. The number goes back at
  // once for the same reason `canceled_expired` releases it: nobody is coming
  // back, and rent drawn from a locked company's balance is the silent
  // charge this branch exists to stop. The balance itself is not touched:
  // prepaid credit stays on the ledger, unspent, and nothing here refunds or
  // confiscates it.
  if (access?.reason === "terms") {
    return { action: "release", reason: "subscription_ended_terms", cents, balanceCents: Math.round(Number(balanceCents) || 0) };
  }
  const cancelled = access?.reason === "canceled" || access?.reason === "canceled_expired";
  if (cancelled) {
    if (access.reason === "canceled_expired") {
      return { action: "release", reason: "subscription_ended", cents, balanceCents: Math.round(Number(balanceCents) || 0) };
    }
    const warnedRecently =
      number.rentWarnedAt &&
      now.getTime() - new Date(number.rentWarnedAt).getTime() < RENT_REMIND_EVERY_DAYS * DAY;
    // The date is derived from THEIR cancellation, not from now, so it does not
    // walk forward every time the cron runs and the email says the same day
    // twice running.
    const endsAt = access.since ? addDays(new Date(access.since), CANCELLED_DAYS) : null;
    return warnedRecently
      ? { action: "grace_wait", reason: "cancelled_warned_recently", cents, endsAt }
      : { action: "warn_cancelled", reason: "subscription_cancelled", cents, endsAt };
  }
...
export async function billNumberRent(number, { now = new Date(), origin } = {}) {
  ...
  // Resolved here for the same reason `available` is: a second caller ("bill
  // now" from the console, a backfill) must not be able to charge a company
  // that left just by forgetting to ask.
  const access = await accessForCompany(number.companyId, now).catch(() => null);
  const decision = rentDecision({ number, balanceCents, now, available, access });
  ...
  switch (decision.action) {
    ...
    case "release": {
      ...
      // The same stand-down as the contractor's own button. Unattended releases
      // need it MORE, not less: nobody is looking at the screen to notice that
      // the receptionist is still switched on over a line that has gone.
      const standDown = await standDownIfLastNumber(number.companyId, { now });
      const sent = await notifyRent(number, decision, { origin });
      return { ...decision, released: true, notified: sent, ...standDown };
    }
...
async function notifyRent(number, decision, { origin } = {}) {
  try {
    ...
    const copy = {
      ...
      // ── The two cancellation notices ────────────────────────────────────
      //
      // Keyed separately from the delinquency copy above, because sending a
      // contractor "the rental went unpaid" when they simply cancelled is the
      // product telling them something untrue about their own account — and it
      // reads as a demand for money on the way out of the door.
      warn_cancelled: {
        heading: "Your number stops being charged, and then goes back",
        sub: num,
        paragraphs: [
          `You've cancelled FieldQuo, so we've stopped taking the ${money(decision.cents)} monthly rental for ${num}. Nothing more comes out of your phone credit for it.`,
          decision.endsAt
            ? `The number keeps answering until ${date(decision.endsAt)}. After that it goes back to the carrier and cannot be recovered — by us or by you — so anything printed on a van, a lawn sign or a Google listing with ${num} on it would stop ringing. If you're moving it to another provider, start the port before then.`
            : `The number keeps answering for now. When it goes back to the carrier it cannot be recovered, so if you're moving it to another provider, start the port soon.`,
          "Resubscribe before then and it simply stays yours.",
        ],
      },
      release_cancelled: {
        heading: "Your number has gone back",
        sub: num,
        paragraphs: [
          `Your FieldQuo subscription ended and ${num} has now been released back to the carrier. Anything printed with that number on it has stopped ringing.`,
          "We stopped charging rent for it the day you cancelled, so there is nothing outstanding. If you come back, you can set up a new number from the receptionist settings — it will be a different number.",
          "If you were forwarding your own number to it, that forwarding now goes nowhere — turn it off with ##002# from your phone.",
        ],
      },
    }[
      // A release has two entirely different causes and they must not share a
      // sentence. `reason` is what distinguishes them; `action` is only "the
      // number went back".
      decision.reason === "subscription_ended" ? "release_cancelled" : decision.action
    ];

    if (!copy) return false;

    // A cancelled company is not being asked for money, so it is not told what
    // its credit is or sent to a top-up screen. Both would read as a bill.
    const aboutCancellation =
      decision.action === "warn_cancelled" || decision.reason === "subscription_ended";
    ...
    const { subject, html } = buildPlatformNotice({
      ...
      facts: aboutCancellation
        ? [
            ["Number", num],
            ["Monthly rental", `${money(decision.cents)} — stopped`],
            ...(decision.endsAt ? [["Works until", date(decision.endsAt)]] : []),
          ]
        : [
            ["Number", num],
      ...
      cta: aboutCancellation
        ? { url: `${base}/app/settings/account-billing`, label: "Resubscribe" }
        : { url: `${base}/app/settings/voice`, label: "Add credit" },
    });
    ...
    // was edited to.
    const result = await sendEmail({ companyId: number?.companyId, from: await getPlatformFrom(), to, subject, html });
