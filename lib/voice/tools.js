  ...
  const tools = [
    {
      ...
      url: url("save-caller"),
      // ── The dead air, and it was ours ────────────────────────────────
      //
      // Both of these were false, and the transcript of a real 44-second call
      ...
      //   user   "Are you there?"
      //
      // The route returns a sentence WRITTEN TO BE SPOKEN and nothing spoke
      // it. The agent went quiet for the whole HTTP round trip and then stayed
      // quiet, so the caller filled the silence — on the one call in ten where
      // they don't simply hang up.
      //
      // `speak_during_execution` stays false on purpose: this fires mid-
      // sentence, as soon as a name and a number exist, and "one moment" in the
      // middle of someone reading their phone number out is worse than the
      // pause it covers. `speak_after_execution` is what closes the gap — the
      // model reads the result, including `say`, and answers.
      speak_during_execution: true,
      speak_after_execution: true,
      // Static, not a prompt: the model would otherwise compose a line, which
      // costs a round trip to say "one moment". The words are deliberately the
      // ones a person says while writing — they cover the pause instead of
      // announcing it.
      execution_message_type: "static_text",
      execution_message_description: "Let me get that down.",
      // ── The two-minute default is the real danger ────────────────────
      //
      // Retell's `timeout_ms` defaults to 120000 and `speak_during_execution`
      // false means silence for the whole of it. A slow write could therefore
      // hold a caller in dead air for two minutes with no way out. This
      // endpoint creates a lead and scores it; if that has not finished in
      // eight seconds something is wrong, and the agent recovering from a
      // timeout error is far better than a caller listening to nothing.
      timeout_ms: 8000,
      parameters: {
        type: "object",
        properties: {
          ...
          email: {
            type: "string",
            description:
              "Their OWN email — ASK for it on any call about a quote, because " +
              "it is where the quote gets sent and it is the one people forget. " +
              "But never guess it: if you read an address out to them — for " +
              "photos, for anything — that address belongs to the business, not " +
              "to them, and it must not come back here.",
          },
          address: {
            type: "string",
    ...
    tools.push(
      {
        ...
        speak_during_execution: true,
        // Explicit rather than left to the provider's default. Both of these
        // return a `say` sentence the caller is meant to hear — the slot that
        // was offered, or the time that was booked — and relying on an
        // undocumented default for whether it gets spoken is how save_caller
        // above went silent for a month.
        speak_after_execution: true,
        execution_message_type: "static_text",
        execution_message_description: "Let me check what's free.",
        // Reading a calendar, not writing one. Same reasoning as save_caller:
        // the provider's 120000 default is two minutes of silence.
        timeout_ms: 10000,
        parameters: {
          type: "object",
          properties: {
      ...
      },
      {
        ...
        description:
          ...
          "it was given. " +
          // ── The sentence that stops a booking nobody made ──────────────
          //
          // An agent read a caller a real slot from check_availability, never
          // called this, and told him it was scheduled. The prompt carries the
          // rule twice now; it is repeated here because the tool description
          // is what the model is reading at the moment it decides whether
          // calling this is necessary, and "I already told him the time" is
          // exactly when it feels least necessary.
          "NOTHING IS BOOKED UNTIL THIS TOOL RETURNS. Do not tell the caller " +
          "an appointment exists before you have called this and it came back " +
          "successful." +
          (modes.length > 1
            ? " Say which kind you are booking in `mode`: someone who wants a " +
              "quote or a question answered wants a phone call; someone who " +
              "wants the job looked at wants a visit. Prefer the phone call."
            : ""),
        url: url("book"),
        speak_during_execution: true,
        // Explicit rather than left to the provider's default. Both of these
        ...
        speak_after_execution: true,
        execution_message_type: "static_text",
        execution_message_description: "Booking that in for you now.",
        // The longest of the three: it geocodes an address and runs the shared
        // finalise step. Still nowhere near the provider's two-minute default.
        timeout_ms: 15000,
        parameters: {
          type: "object",
          properties: {
            ...
            phone: {
              type: "string",
              description:
                "The number to reach them on, exactly as they gave it. An " +
                "appointment nobody can ring is not one.",
            },
            // ── Why somebody is being sent, in the caller's words ─────────
            //
            // The owner asked for this and the appointment had nowhere to put
            // it: whoever takes the job saw a name, a time and, if they were
            // lucky, a street. bookSlot writes it to Appointment.notes, which
            // is what the schedule actually shows.
            reason: {
              type: "string",
              description:
                "Why they want the appointment, in their own words and in one " +
                "or two sentences — what the work is, what they asked about. " +
                "Whoever takes it reads this and nothing else beforehand.",
            },
            // ── The web booking page 400s without this ───────────────────
            //
            // clientEmail is the one hard requirement on /api/booking/[slug]/
            // confirm, and the phone had no way to supply it — so every
            // phone-booked appointment was written with an empty email,
            // finalizeBooking skipped the confirmation letter, and the agent
            // fell back to "if you'd like it in writing, give me an email
            // address" AFTER the booking was already made. An email arriving
            // later through save_caller does not retro-send anything.
            email: {
              type: "string",
              description:
                "Their email, so the confirmation can be sent. Ask for it " +
                "before you book — an appointment nobody can confirm in " +
                "writing is one they may not remember. Their OWN address: if " +
                "you read one out to them for photos, that one belongs to the " +
                "business and must not come back here.",
            },
          ...
          // `reason` is required and `address` deliberately is not. A model
          // will invent a value to satisfy a required field, and an invented
          // street is a van sent to a stranger's house — so the address is
          // asked for in the prompt and ENFORCED in bookSlot, which refuses a
          // visit without one. A reason cannot be invented in the same harmful
          // way: the worst case is a vague sentence.
          required: ["slot", "name", "phone", "reason"],
        },
      },
    );
  }
...
/**
 * What the agent says when a booking is refused, per reason.
 *
 * This was a ternary with one branch and an else, so every refusal that was not
 * a fee came out as "that one's just gone" — a clash. For a slot the caller was
 * only just offered that is a lie, and it is the specific lie that teaches the
 * agent to go and offer a different time instead of fixing what was actually
 * wrong. A missing address gets asked for; a slot id the model invented sends
 * it back to check_availability rather than inventing a second one.
 */
export const SAY_ON_REFUSAL = {
  fee_due:
  ...
  address_required:
    "Before I can book someone in, I need the address the work's at. What is it?",
  // The slot did not come from check_availability, or it did and the id got
  // mangled on the way back. Either way the fix is to look again, not to guess.
  bad_slot:
    "Sorry — I've lost that time. Let me check what's free and we'll pick one.",
  unknown_event_type:
    "Sorry — I've lost that time. Let me check what's free and we'll pick one.",
  unknown_company:
    "I can't get into the calendar just now. Let me take your details and someone will ring you straight back.",
  taken: "That one's just gone. Would another time work?",
};
