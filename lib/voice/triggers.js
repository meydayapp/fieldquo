...
// One rule for "which locale is this company", shared with provision.js so the
// language the agent SPEAKS and the language it reads a date back in cannot
// drift apart.
...
export function describeAppointmentTime(startTime, { timezone, language = "en" } = {}) {
  ...
  const locale = agentLanguage(language);
  try {
...
export function approvedQuoteCallGate(quote) {
  ...
  // A past job entered after the fact has no sentAt, so the next line already
  // refuses it; this one says so by name, so a future path that stamps
  // sentAt on such a row still cannot ring a client about a job from 2024.
  if (quote.historicalImportedAt) return { allowed: false, reason: CALLBACK_REFUSED.NOT_EMAILED };
...
/**
 * What the agent is told about the quote it is ringing about.
 *
 * Exported because the MANUAL button on a quote queues the same call
 * (app/api/quotes/[id]/call), and a second hand-built context is the copy that
 * rots — the day somebody adds a field here, the button keeps briefing the
 * agent with last month's shape.
 *
 * The total is a figure a HUMAN approved and the client has ALREADY been
 * emailed. The agent may read it back; it may not change it, and
 * placeQueuedCall drops it if the quote's total no longer matches what was
 * queued — a figure the client has not received in writing must never be
 * spoken.
 */
export function quoteCallContext(quote) {
  const total = spokenTotal(quote?.total, quote?.company?.currency || "CAD");
  const summary = serviceSummary(quote?.lineItems);
  return {
