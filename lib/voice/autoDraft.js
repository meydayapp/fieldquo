 ...
 *
 * ── CRISIS_DETECTED also flags the call for a person ───────────────────────
 *
 * This function runs on EVERY finished call (see the file header), so it is
 * the one place a crisis call is guaranteed to pass through even when nobody
 * ever opens the receptionist screen. Every other skip reason only writes
 * `quoteDraftSkipped` — a caption under a call a contractor might glance at.
 * This one also sets `needsReview`, the SAME flag save-caller sets for a
 * property emergency (app/api/voice/tools/[tool]/route.js) and the SAME queue
 * the receptionist screen already surfaces flagged calls in. The point is not
 * to draft a quote from somebody's worst moment; it is also not to let that
 * moment pass through the system with nothing marking that it happened. The
 * transcript stays exactly where it always was — this only decides who gets
 * pointed at it.
 */
async function skip(companyId, callId, reason) {
  try {
    await db.voiceCall.updateMany({
      where: { id: callId, companyId },
      data: {
        quoteDraftSkipped: reason,
        ...(reason === DRAFT_REASONS.CRISIS_DETECTED ? { needsReview: true } : {}),
      },
    });
  } catch {
