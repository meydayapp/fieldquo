...
export function quoteIntakeSection(topics = [], photosTo = null) {
  ...
  const out = [
    ...
    `    form. Once you have their name and number, anything else is a bonus. Do`,
    `    not work down the list, do not ask the same thing twice, and if they`,
    `    sound like they want to go, let them go. Half the answers and a real`,
    `    phone number beats a full set from a call that ended badly.`,
...
export function visitSection(policy = {}) {
  ...
  if (mode === "book") {
    ...
    if (bookableModes.length > 1) {
      out.push(
        "",
        `You can arrange ${modePhrase(bookableModes)}. Ask which they want if it is`,
        `not obvious: someone who wants a price or an answer wants a phone call;`,
        `someone who wants the job looked at wants a visit. Only ask for an address`,
        `when it is a visit.`,
      );
    }
