...
//
// ══ Rule 5 used to be two rules. The owner asked for one ═══════════════════
//
// This file used to split "gas leak, flood" (property, rule 5, written here)
// from "a caller who is a danger to themselves" (rule 5b, imported from
// lib/ai/crisisRule.js). The owner's instruction collapsed that split: one
// emergency, one line, say 911, then keep going with whatever the call was
// about. So rule 5 below is now just CRISIS_RULE — the property wording and
// the personal-danger wording both live in that one shared string now, not
// split across two rules in this file. Its text still lives in
// lib/ai/crisisRule.js, not here, because the receptionist is not the only
// prompt whose words can reach a person — the outbound caller, FieldQuo's own
// sales line and the in-app copilot all import the identical CRISIS_RULE
// rather than each carrying their own paraphrase of it, for the reason every
// other shared rule in this file gets its own module: four almost-identical
// copies is how one of them quietly stops matching the other three.
...
// Which language(s) this agent speaks — one rule, shared with provision.js
// (what the provider is told) and availability.js (how a slot is read out), so
// the prompt cannot say French while the provider transcribes English.
import { languagesOf, languageOfLocale, resolveSpokenLanguage, spokenLocales } from "./agentLanguage";
 ...
 */
function companyFacts({ company, services = [], areas = [], hours, timeZone = null, locales = ["en-US"] }) {
  ...
  // standing in.
  //
  // And written in the language(s) the agent SPEAKS, one date per locale.
  // This was "en-CA" for everybody, so a French receptionist was handed
  // "Tuesday, August 12" and, when a caller said "demain", had to translate
  // the day name itself before it could count. A bilingual agent gets both
  // spellings — "mardi 12 août 2026 / Tuesday, August 12, 2026" — so
  // whichever language the caller is using, the day is already in it.
  if (timeZone) {
    try {
      const now = new Date();
      const today = locales
 ...
 * @param language  the COMPANY's language — decides what answers when `spoken`
 *                  is unset, and which of a bilingual pair leads
 * @param spoken    VoiceAgent.spokenLanguage — "en" | "fr" | "es" | "en-fr" |
 *                  "en-es", or null for "follow the company"
 */
export function buildAgentPrompt({
  ...
  const speaks = resolveSpokenLanguage(spoken, language);
  const locales = spokenLocales(speaks, language);
  const parts = [
    SYSTEM_RULES,
    "",
    // ── Which language the call happens in ──────────────────────────────
    //
    // The provider's `language` field sets transcription and the voice, and
    // for a single language Retell now says the agent "is automatically
    // instructed to respond in the configured language". This section stays
    // anyway: it is the one place the bilingual behaviour is spelled out
    // (Retell's multilingual agent "chooses based on what the caller speaks
    // (and any instructions you give in the prompt)"), it carries the date
    // and disclosure rules a locale cannot, and nothing here said it before —
    // every prompt in this file being English meant every receptionist
    // answered in English, including a French company's, unless its owner
    // happened to type a French greeting.
    //
    ...
    // the one the model follows most reliably. This section is what changes
    // what is SAID.
    languageRule(speaks, language),
    "",
    "ABOUT THIS BUSINESS",
    companyFacts({ company, services, areas, hours, timeZone, locales }),
  ];
...
const SPOKEN = {
  en: {
    ...
    rule: "Conduct the entire call in English. If the caller speaks to you in another language, switch to theirs and stay there.",
    name: "English",
    // Two halves, so a bilingual greeting can be assembled in either order
    // without a third copy of the words. `hello` is the one-word opener.
    hello: "Hello",
    thanks: (name) => `thanks for calling ${name}`,
    help: "How can I help?",
    // How the recorded-call notice (rule 4b) and the "are you a person?"
    // answer (rule 4) sound in this language. Given to the model verbatim,
    // because both are lines it must get right first time in a language it
    // may only be speaking because the caller switched to it.
    recorded: "just so you know, this call's recorded",
    assistant: "I'm an assistant answering for the business",
    // A worked date, so "say dates in this language" is a demonstration and
    // not a policy.
    dateExample: "Tuesday, August 12 at 2:00 PM",
  },
  fr: {
    ...
    name: "French (Canadian French)",
    hello: "Bonjour",
    thanks: (name) => `merci d'avoir appelé ${name}`,
    help: "Comment puis-je vous aider?",
    recorded: "juste pour que vous le sachiez, cet appel est enregistré",
    assistant: "je suis un assistant qui répond pour l'entreprise",
    dateExample: "mardi 12 août à 14 h",
  },
  es: {
    ...
    name: "Spanish (Latin American Spanish)",
    hello: "Hola",
    thanks: (name) => `gracias por llamar a ${name}`,
    help: "¿En qué puedo ayudarle?",
    recorded: "para que lo sepa, esta llamada se graba",
    assistant: "soy un asistente que atiende por la empresa",
    dateExample: "martes 12 de agosto a las 2:00 p. m.",
  },
};

/**
 * The single greeting for one language, capitalised as a sentence.
 * "Thanks for calling Peintures Boves, how can I help?"
 */
function singleGreeting(lang, name) {
  const s = SPOKEN[lang] || SPOKEN.en;
  return `${caseFirstLetter(s.thanks(name), "upper")}, ${caseFirstLetter(s.help, "lower")}`;
}

/**
 * Re-case the first LETTER, not the first character: Spanish opens a question
 * with "¿", and "¿En qué puedo ayudarle?" mid-sentence has to become "¿en qué",
 * which charAt(0) cannot reach.
 */
function caseFirstLetter(text, to) {
  return String(text).replace(/\p{L}/u, (ch) => (to === "upper" ? ch.toUpperCase() : ch.toLowerCase()));
}

/**
 * The bilingual greeting: both languages in one short line, primary first.
 *
 * "Bonjour, hello — merci d'avoir appelé Peintures Boves. Comment puis-je
 * vous aider? How can I help?" — the Ottawa–Gatineau pattern. A caller hears
 * their own language inside the first two words and answers in it, which is
 * what tells the agent (and the speech recogniser) which one this call is in.
 * The thank-you is said once, in the primary language, because "merci d'avoir
 * appelé X, thanks for calling X" is the greeting a homeowner hangs up on.
 */
function bilingualGreeting([first, second], name) {
  const a = SPOKEN[first] || SPOKEN.en;
  const b = SPOKEN[second] || SPOKEN.en;
  return `${a.hello}, ${b.hello.toLowerCase()} — ${a.thanks(name)}. ${a.help} ${b.help}`;
}

/**
 * The LANGUAGE section: which language(s) to speak, and the three things a
 * locale setting cannot carry — how to follow a caller across languages, how
 * a date is said in each, and how the two disclosures sound.
 *
 * ── The bilingual rule is the reason this section exists ──────────────────
 *
 * Retell's multilingual agent picks a response language "based on what the
 * caller speaks (and any instructions you give in the prompt)". Left to
 * itself a model handed a bilingual greeting tends to keep being bilingual —
 * every sentence twice, which doubles the call and reads as a phone menu. So
 * the rule is explicit: the greeting is the ONLY bilingual line; after it,
 * one language at a time, the caller's, and switch when they switch.
 *
 * ── Dates ─────────────────────────────────────────────────────────────────
 *
 * The one place a bilingual agent visibly breaks: check_availability hands it
 * a slot label and it reads the label. availability.js now labels a slot in
 * every language the agent speaks, and this tells the model to read only the
 * one that matches — so "Tuesday, August 12" never lands in the middle of a
 * French sentence.
 */
function languageRule(speaks, companyLanguage) {
  const langs = languagesOf(speaks);
  const order = spokenLocales(speaks, companyLanguage).map(languageOfLocale);
  const lines = ["LANGUAGE"];

  if (langs.length === 1) {
    const s = SPOKEN[langs[0]] || SPOKEN.en;
    lines.push(s.rule);
    if (langs[0] !== "en") {
      lines.push(
        `Say dates and times the way they are said in ${s.name}: "${s.dateExample}", never an English day or month name.`,
        `The recorded-call notice in rule 4b is said in ${s.name}: "${s.recorded}". If asked whether you are a person, the answer is "${s.assistant}".`,
      );
    }
    return lines.join("\n");
  }
  ...
  const a = SPOKEN[first];
  const b = SPOKEN[second];
  lines.push(
    `This business serves callers in both ${a.name} and ${b.name}, and the greeting is bilingual on purpose. It is the ONLY bilingual line of the call.`,
    `After the greeting, speak only the language the caller answers in — ${a.name} or ${b.name} — and stay in it. If they switch languages, switch with them. Never say the same thing in both languages, and never mix the two in one sentence. If they speak a third language, switch to theirs and stay there.`,
    `Say dates and times in the language you are speaking at that moment: in ${a.name} "${a.dateExample}", in ${b.name} "${b.dateExample}". When a tool gives you a time in both languages separated by " / ", read only the one that matches the language you are speaking.`,
    `The recorded-call notice in rule 4b is said in the caller's language: in ${a.name} "${a.recorded}"; in ${b.name} "${b.recorded}".`,
    `If asked whether you are a person, answer in the caller's language: in ${a.name} "${a.assistant}"; in ${b.name} "${b.assistant}".`,
  );
  return lines.join("\n");
}
 ...
 * The first thing the caller hears.
 *
 * @param language the COMPANY's language
 * @param spoken   VoiceAgent.spokenLanguage, or null to follow the company
 */
export function buildGreeting({ company, greeting, language = "en", spoken = null } = {}) {
  ...
  const speaks = resolveSpokenLanguage(spoken, language);
  const order = spokenLocales(speaks, language).map(languageOfLocale);
  ...
  // the agent gets a chance to switch — and a French-only greeting out of a
  // bilingual shop's phone loses the English half of Ottawa the same way.
  if (order.length > 1) return bilingualGreeting(order, name).slice(0, 300);
  return singleGreeting(order[0], name).slice(0, 300);
}
