...
import { createPhoneCall, listCalls } from "./retell";
 ...
 *
 * @param once  De-dupe across EVERY status rather than only live ones, so this
 *              subject is called at most once for this purpose for ever. Set by
 *              the quote callback: a quote re-sent a week later found no live
 *              task and queued a second closing call to the same customer. Not
 *              the default — a rescheduled visit genuinely warrants a second
 *              reminder, and a blanket "once" would silently lose it.
 */
...
export async function placeQueuedCall(task, { now = new Date() } = {}) {
  ...
  // ── Past this line the phone is RINGING, so nothing may throw ────────────
  //
  // Everything below is bookkeeping about a call that has already been placed,
  // and the caller reads a thrown error as "that didn't happen" — which is how
  // a failed upsert turned into a task requeued and a second real call to the
  // same person. The row is worth having and is not worth a re-dial: a
  // VoiceCall we fail to write is exactly what lib/voice/reconcileCalls.js
  // exists to put back, and it will, from the provider's own list.
  //
  ...
  try {
    ...
    console.error("[voice/outbound] placed call but couldn't record it:", providerCallId, err?.message);
  }

  // Optional-chained for the same reason: an unknown purpose reaching this far
  // would throw on `.label` AFTER the call was made, and the caller would read
  // the throw as a call that never happened.
  return { placed: true, reason: purposeSpec(task.purpose)?.label || task.purpose, providerCallId };
}

/**
 * Did a previous, dead run already dial this task?
 *
 * ── Why this exists, and why it is not an idempotency key ──────────────────
 *
 * lib/voice/autoTopup.js survives a claim that died mid-flight by REUSING its
 * attempt token: Stripe's idempotency key makes the retry return the original
 * PaymentIntent instead of taking a second payment, so a reclaim is free. Retell
 * has no equivalent — `POST /v2/create-phone-call` takes no idempotency key and
 * no client-supplied call id, and every request that reaches it dials the phone.
 * There is nothing to replay.
 *
 * So the same guarantee has to be bought the only other way: ASK before dialling
 * again. The `metadata.taskId` we stamp on every outbound call (see the
 * createPhoneCall above) is the token — it survives at the provider whether or
 * not our own transaction did, which is precisely the case a stale claim covers.
 *
 * ── An unreadable provider is not evidence that nothing was dialled ────────
 *
 * This THROWS when the list cannot be read, and the drainer holds the task
 * rather than dialling. Same discipline as reconcileVoiceCalls' rule 3: our
 * failure to see must never be resolved in the direction that costs a stranger
 * a second phone call.
 *
 * @returns the provider's call object, or null when it genuinely has none.
 */
export async function findPlacedCallForTask(
  task,
  { now = new Date(), list = listCalls, marginMs = 5 * 60 * 1000 } = {},
) {
  if (!task?.id) return null;

  // From when the dead claim was stamped, less a margin for clock skew between
  // us and the provider. No claim stamp (a row that predates claiming) falls
  // back to an hour, which is four stale windows — wide enough to cover any run
  // that could plausibly have dialled it.
  const claimedMs = task.lastTriedAt ? new Date(task.lastTriedAt).getTime() : NaN;
  const sinceMs = Number.isFinite(claimedMs)
    ? claimedMs - marginMs
    : now.getTime() - 60 * 60 * 1000;

  const res = await list({
    sinceMs,
    untilMs: now.getTime() + marginMs,
    limit: 200,
    // Everything, including calls still connected — see the note on `statuses`
    // in lib/voice/retell.js.
    statuses: null,
  });

  const items = Array.isArray(res?.items) ? res.items : [];
  return items.find((c) => c?.metadata?.taskId === task.id) || null;
}
...
async function quoteStateNow(task) {
  ...
  // Declined between the send and the dial — a client can turn a quote down
  // from the emailed link inside the fifteen minutes the cron waits. The closer
  // asks whether they want to go ahead; they have already said no, and asking
  // again by robot is the version of this feature nobody wants.
  if (quote.status === "declined") {
    return { refuse: "They turned this quote down.", context: {} };
  }
