...
/**
 * Which of the receptionist's locales each voice PROVIDER can pronounce.
 *
 * ── Why per provider and not per voice ────────────────────────────────────
 *
 * Retell's dashboard greys a language out per voice ("Each voice (and pinned
 * voice model) only supports a subset of languages" —
 * https://docs.retellai.com/agent/multilingual). The API does not expose that
 * subset: /list-voices returns voice_id, voice_name, provider, accent, gender,
 * age and preview_audio_url, and nothing about language (OpenAPI spec
 * revision 2026-09-14). So the finest grain we can filter on is the provider,
 * from https://docs.retellai.com/build/language-support — "A ✓ means the
 * language is supported by at least one of that provider's voice models",
 * with model-level exceptions only for ElevenLabs (`eleven_flash_v2`, English
 * only) and Fish Audio's `s1`. Cartesia, the whole shortlist, "support their
 * full language set on every model", which is why the three curated voices
 * survive every selector value and the picker is never empty.
 *
 * ── Read on 2026-09-17; the dashboard is the source of truth ──────────────
 *
 * English, French (fr-FR, fr-CA) and Spanish (es-ES, es-419) are ✓ for every
 * provider listed. Inworld is missing Filipino and Ukrainian; Fish Audio is
 * missing Filipino. Those two locales are not selectable here — uk and tl
 * companies get English until lib/voice/prompt.js can hold the conversation
 * — but they are recorded so the filter has a real documented case to be
 * tested against, rather than a table in which everything is true.
 *
 * A provider absent from this table supports nothing, on purpose: an
 * unknown vendor's voice offered for French is a guess about somebody's
 * business line.
 */
export const PROVIDER_LOCALES = {
  platform: ["en-US", "fr-CA", "es-419", "fil-PH", "uk-UA"],
  elevenlabs: ["en-US", "fr-CA", "es-419", "fil-PH", "uk-UA"],
  cartesia: ["en-US", "fr-CA", "es-419", "fil-PH", "uk-UA"],
  openai: ["en-US", "fr-CA", "es-419", "fil-PH", "uk-UA"],
  minimax: ["en-US", "fr-CA", "es-419", "fil-PH", "uk-UA"],
  fish_audio: ["en-US", "fr-CA", "es-419", "uk-UA"],
  inworld: ["en-US", "fr-CA", "es-419"],
};

/**
 * Can this voice pronounce every locale the agent will speak?
 *
 * A bilingual agent needs one voice for both languages — Retell's language
 * support page: "For a multilingual agent, every selected language must be
 * covered by a single voice" — so the test is ALL of them, not any.
 *
 * @returns { ok: true } or { ok: false, missing: ["fil-PH"] }
 */
export function voiceSupportsLocales(voice, locales = ["en-US"]) {
  const covered = PROVIDER_LOCALES[String(voice?.provider || "").toLowerCase()] || [];
  const list = Array.isArray(locales) && locales.length ? locales : ["en-US"];
  const missing = list.filter((l) => !covered.includes(l));
...
/**
 * The pickable list, split into what can speak the agent's language(s) and
 * what cannot — with the reason, so the screen can say why a voice went.
 *
 * Applied AFTER pickableVoices rather than inside it, so `keep` — the voice
 * answering today — is judged like any other: a company switching to
 * bilingual while on a voice that has no French must be told, not silently
 * left on it (Retell refuses the combination and the agent stays on the old
 * language) and not silently moved off it (the first save would change how
 * the business sounds with nobody having chosen that).
 *
 * @returns { voices, excluded: [{ ...voice, missing: [locale] }] }
 */
export function splitByLocales(voices = [], locales = ["en-US"]) {
  const out = { voices: [], excluded: [] };
  for (const v of Array.isArray(voices) ? voices : []) {
    const verdict = voiceSupportsLocales(v, locales);
    if (verdict.ok) out.voices.push(v);
    else out.excluded.push({ ...v, missing: verdict.missing });
  }
  return out;
}
