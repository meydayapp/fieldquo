...
/**
 * Turn off the switches that now have nothing behind them.
 *
 * ══ The dead control this closes ═══════════════════════════════════════════
 *
 * Releasing the last number left "Answer my calls" and "Call clients back
 * automatically" both switched ON, over a line that no longer exists. The
 * screen then said, underneath the still-on switch, "Set up a number above
 * first — there's nothing for it to answer on." A switch that is on, and
 * explains beside itself that it cannot work, is the failure class this repo
 * keeps finding: a control that appears to work and doesn't.
 *
 * It is worse than cosmetic on the outbound half. `outboundCallsEnabled` is the
 * contractor's own consent for FieldQuo to ring their clients. Leaving it set
 * over a released number means that the day they buy a new number, the product
 * starts calling their customers again on a permission they last thought about
 * months ago — without asking.
 *
 * ══ Only when it was the LAST one ══════════════════════════════════════════
 *
 * A company can hold more than one number; one production company holds three.
 * Switching the receptionist off because they gave up a spare would take their
 * phone answering away as a side effect of tidying up, which is the same
 * failure pointing the other way.
 *
 * Never throws. The number is already gone at the provider by the time this
 * runs, and a failure to tidy a boolean must not turn a completed release into
 * an error the contractor is told to retry.
 *
 * @returns { standDown, inbound, outbound } — what was actually changed, so the
 *          caller can say so rather than implying it.
 */
export async function standDownIfLastNumber(companyId, { now = new Date() } = {}) {
  const unchanged = { standDown: false, inbound: false, outbound: false };
  if (!companyId) return unchanged;
  try {
    const remaining = await db.voicePhoneNumber.count({
    ...
    const [agent, company] = await Promise.all([
      ...
      db.company.findUnique({ where: { id: companyId }, select: { outboundCallsEnabled: true } }),
    ]);

    // Written only when it is actually ON. An unconditional update would stamp
    // updatedAt on every release and make the activity log claim a change that
    // did not happen.
    const inbound = agent?.enabled === true;
    const outbound = company?.outboundCallsEnabled === true;
    if (inbound) {
      await db.voiceAgent.update({ where: { companyId }, data: { enabled: false } });
    }
    if (outbound) {
      await db.company.update({
        where: { id: companyId },
        data: { outboundCallsEnabled: false },
      });
    }
    return { standDown: inbound || outbound, inbound, outbound, at: now };
  } catch {
    // Deliberately swallowed — see above. The release itself succeeded.
    return unchanged;
  }
}
