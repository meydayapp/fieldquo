...
import { summariseMargin, MARGIN_FLOOR_RATIO } from "./providerCost";
...
/**
 * MEASURED margin — what we billed against what Retell actually charged.
 *
 * ══ Why this is a different function from derivedSpend ═════════════════════
 *
 * `derivedSpend` above multiplies minutes by a CONSTANT SOMEBODY TYPED. This
 * one reads `VoiceCall.providerCostCents`, which is the provider's own figure
 * for that call (lib/voice/providerCost.js). They answer different questions
 * and must not be conflated:
 *
 *   derivedSpend    "roughly how fast is the shared pool draining" — needs to
 *                   cover every call, so an estimate is the only option.
 *   measuredMargin  "are we actually making money on a phone call" — needs to
 *                   be TRUE, so it covers only the calls we have a real cost
 *                   for and says how many that is.
 *
 * The first real datum made the case for both: a 3:51 call billed at $1.40
 * cost $0.670, which is 17.4¢ per real minute against the 16¢ the constant
 * assumes. Healthy, and wrong, on the first call anybody checked.
 *
 * ── Billed comes from the LEDGER, not from a recomputation ─────────────────
 *
 * The revenue side reads VoiceCreditEntry rather than calling costForSeconds
 * again. What a contractor was charged is a historical fact sitting in a row;
 * re-deriving it would report today's rate card against yesterday's calls and
 * quietly rewrite the margin every time pricing changes. Debits are stored
 * negative, so revenue is the negation.
 *
 * ── Rounding is separated, not counted as margin ───────────────────────────
 *
 * FieldQuo bills whole minutes rounded up — 3:51 bills as 4:00. That is
 * deliberate and it stays, but it means some of the spread is rounding. The
 * summary reports `costCentsPerRealMinute` off REAL seconds so the two can be
 * told apart; a margin that only exists because of rounding is one billing
 * change away from disappearing, and this page exists to show that.
 */
export async function measuredMargin({ days = 30, prisma = db, now = Date.now() } = {}) {
  const since = new Date(now - days * 24 * 60 * 60 * 1000);

  const calls = await prisma.voiceCall.findMany({
    where: { createdAt: { gte: since }, providerCostCents: { not: null } },
    select: { providerCallId: true, companyId: true, durationSec: true, providerCostCents: true },
  });

  if (!calls.length) {
    return { ...summariseMargin([]), days, worst: [] };
  }

  const entries = await prisma.voiceCreditEntry.findMany({
    where: { ref: { in: calls.map((c) => `call:${c.providerCallId}`) } },
    select: { ref: true, cents: true },
  });
  const billedByRef = new Map(entries.map((e) => [e.ref, -Number(e.cents)]));

  const rows = calls.map((c) => ({
    providerCallId: c.providerCallId,
    companyId: c.companyId,
    durationSec: Number(c.durationSec),
    // Decimal comes back as a Prisma Decimal object; Number() is the documented
    // way across this codebase and the values here are cents, far inside the
    // safe-integer range even at four decimal places.
    providerCostCents: Number(c.providerCostCents),
    // A call with a provider cost and NO ledger entry is a real thing — a
    // zero-length call is never charged — and it is 0 revenue, not unknown
    // revenue. It belongs in the average, dragging it down, because Retell
    // still charged us for it.
    billedCents: billedByRef.get(`call:${c.providerCallId}`) ?? 0,
  }));

  const summary = summariseMargin(rows);

  // The individual calls worth looking at, worst first. An aggregate that says
  // "48% overall" hides the six calls that lost money inside it, and those are
  // the ones with something to learn from — an expensive voice, a long transfer
  // leg, an international dial that got past the country allow-list.
  const worst = rows
    .map((r) => ({ ...r, spreadCents: r.billedCents - r.providerCostCents }))
    .filter((r) => r.billedCents <= 0 || r.spreadCents / r.billedCents < MARGIN_FLOOR_RATIO)
    .sort((a, b) => a.spreadCents - b.spreadCents)
    .slice(0, 10);

  return { ...summary, days, worst };
}
...
export async function poolStatus({ prisma = db, now = Date.now(), days = 30 } = {}) {
  ...
  const [concurrency, spend, margin] = await Promise.all([
    configured
    ...
    measuredMargin({ days, prisma, now }).catch(() => null),
  ]);
  ...
  return {
    ...
    margin,
    alerts: alertsFor({ conc, remainingCents, runwayDays, margin, error: concurrency?.error }),
  };
}
 ...
 */
export function alertsFor({ conc, remainingCents, runwayDays, margin, error } = {}) {
  const out = [];
  ...
  // ── Margin, and why it is the only alert here based on MEASURED numbers ───
  //
  // Everything above this point is derived from a hand-typed cost constant, so
  // none of it can tell you the constant is wrong. This can. It fires only on
  // calls carrying Retell's own `call_cost`, so it stays silent rather than
  // guessing when nothing has been priced yet — and `covered` is stated in the
  // message so nobody reads a two-call sample as a trend.
  if (margin && margin.covered > 0) {
    if (margin.negative > 0) {
      out.push({
        level: "critical",
        code: "margin_negative",
        message:
          `${margin.negative} call(s) cost FieldQuo more at Retell than the contractor was ` +
          `charged. Measured from Retell's own per-call cost on ${margin.covered} of ` +
          `${margin.total} calls — this is a real loss per call, not an estimate. Check the ` +
          `voice and model on those agents, and the outbound country list.`,
      });
    } else if (margin.marginRatio !== null && margin.marginRatio < MARGIN_FLOOR_RATIO) {
      out.push({
        level: "warn",
        code: "margin_thin",
        message:
          `Voice margin is ${Math.round(margin.marginRatio * 100)}% measured across ` +
          `${margin.covered} call(s) — below the ${Math.round(MARGIN_FLOOR_RATIO * 100)}% floor. ` +
          `Retell is charging about ${margin.costCentsPerRealMinute?.toFixed(1)}¢ per real minute.`,
      });
    }

    // The constant drifting away from reality is worth saying OUT LOUD, because
    // it is the input to every other money figure on this page and nothing else
    // would ever catch it. 20% either way rather than any difference: this is a
    // blended average over a mix of voices and models and is never going to
    // land exactly on one number.
    const real = margin.costCentsPerRealMinute;
    if (real !== null && real > 0) {
      const drift = Math.abs(real - PROVIDER_COST_CENTS_PER_MINUTE) / PROVIDER_COST_CENTS_PER_MINUTE;
      if (drift > 0.2) {
        out.push({
          level: "warn",
          code: "cost_constant_stale",
          message:
            `RETELL_COST_CENTS_PER_MINUTE is ${PROVIDER_COST_CENTS_PER_MINUTE}¢, but Retell's own ` +
            `figures across ${margin.covered} call(s) work out at ${real.toFixed(1)}¢ per real ` +
            `minute. Every derived number on this page — burn rate, runway, remaining credit — ` +
            `uses that constant, so they are all off by the same share until it is updated.`,
        });
      }
    }
  }
