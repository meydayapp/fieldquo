...
// ═══════════════════════════════════════════════════════════════════════════
// The SALES numbers — Twilio, not Retell — and where their webhooks point
// ═══════════════════════════════════════════════════════════════════════════
//
// A different provider and a different failure. FieldQuo's reps call and are
// called back on PlatformSmsNumber rows bought at Twilio, and each one
// carries the voice URL it was bought with (`voiceUrl`). That URL is a fact
// about the moment of purchase: if the app's origin moves, or a number was
// bought from a preview deployment, the number keeps ringing the old host,
// and the only symptom is a rep saying a client rang and nothing happened.
// Twilio also signs each webhook against the URL it called, so a host that
// differs from the one the app answers on can fail the signature check on
// every real call (app/api/rep-dial/inbound logs that as
// `signature_rejected` now; before 2026-09-17 it logged nothing).
//
// So this compares each held number's stored voice URL against the origin
// the app is running on right now, and says so per number. Pure, like the
// Retell audit above and for the same reason.
//
// What it CANNOT say, and says so: the number's INBOUND PROCESSING REGION.
// Twilio holds that per number (routes.twilio.com/v2/PhoneNumbers/{e164},
// `voice_region`), we store nothing about it, and a comparison here would be
// invention. `region` is null with `regionSource: "not_recorded"`; the
// console path and the API call to read it are in the floor board's text.

/** The host of a URL, or null for anything that is not one. */
export function hostOf(url) {
  try {
    return new URL(String(url || "")).host.toLowerCase() || null;
  } catch {
    return null;
  }
}
...
/**
 * @param numbers   PlatformSmsNumber rows for the sales purposes, active only:
 *                  `{ e164, purpose, voiceUrl, assignedRepName? }`
 * @param origin    the app origin this deployment answers on (getAppOrigin)
 * @param inboundPath  the path the voice URL must carry
 */
export function salesNumberWebhookAudit({ numbers = [], origin = null, inboundPath = "/api/rep-dial/inbound" } = {}) {
  const expectedHost = hostOf(origin);
  const lines = (Array.isArray(numbers) ? numbers : [])
    .filter((n) => n && typeof n.e164 === "string")
    .map((n) => {
      const voiceUrl = typeof n.voiceUrl === "string" && n.voiceUrl.trim() ? n.voiceUrl.trim() : null;
      const host = hostOf(voiceUrl);
      let path = null;
      try {
        path = voiceUrl ? new URL(voiceUrl).pathname : null;
      } catch {
        path = null;
      }
      const state = !voiceUrl
        ? "no_voice_url"
        : !host
          ? "unparseable"
          : !expectedHost
      ...
      return {
        ...
        assignedRepName: n.assignedRepName || null,
        voiceUrl,
        host,
        expectedHost,
        state,
        pointsHere: state === "points_here",
        // Honest absence. See the header.
        region: null,
        regionSource: "not_recorded",
      };
    });
  return {
    lines,
    ...
    counts: {
      held: lines.length,
      pointsHere: lines.filter((l) => l.pointsHere).length,
      wrong: lines.filter((l) => l.state === "wrong_host" || l.state === "wrong_path" || l.state === "unparseable").length,
      noVoiceUrl: lines.filter((l) => l.state === "no_voice_url").length,
    },
  };
}
