...
export function marketingMetadata({ path = "/", title, description, ...extra }) {
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      images: [OG_IMAGE],
      // No `locale` here. Language on this site is chosen in the browser (the
      // account, a per-tab switch, the device) with no locale-prefixed routes, so every URL serves every language — claiming
