...
/**
 * The grouping locale used when nobody says otherwise.
 *
 * A named constant rather than a literal inside formatAmount so the two places
 * that need to agree on "what English looks like" — this module and the check
 * script that reads it — cannot drift. The reader's real locale arrives from
 * app/i18n/numberLocale.js, which is not imported here: this module is pure
 * arithmetic and copy, and pulling a UI concern into it would put a second
 * language table one import away from the price maths.
 */
export const DEFAULT_NUMBER_LOCALE = "en-CA";

/* ═══════════════════════════════════════════════════════════════════════════
   How a number reaches the sentence it sits in
   ═══════════════════════════════════════════════════════════════════════════

   Every workings line is a sentence with figures in it, and the figures are
   punctuated differently in every language the marketing catalogue carries.
   Building the sentence here in English and translating the whole thing would
   put "1,250" inside a French paragraph; building it here with the reader's
   locale would mean this module knowing about languages.

   So a builder no longer formats: it hands back the raw values TAGGED with
   what kind of quantity each one is, and the page renders them. The English
   `workings` string is still produced exactly as before — the check script
   reads it, and an English reader sees the same sentence they saw yesterday. */

// Prefixed `as*` rather than named for the quantity: two of the builders
// already have a local `share` and one a local `days`, and a tagger shadowed
// by the value it is meant to tag is the kind of bug that only shows up in one
// language.
const asMoney = (n) => Object.freeze({ n, kind: "money" });
const asCount = (n) => Object.freeze({ n, kind: "count" });
const asMinutes = (n) => Object.freeze({ n, kind: "minutes" });
const asDays = (n) => Object.freeze({ n, kind: "days" });
const asShare = (n) => Object.freeze({ n, kind: "share" });
/** A phrase that is itself a translated key, not a number. */
const asPhrase = (key, values) => Object.freeze({ key, kind: "phrase", values });
  ...
  Object.freeze({
    ...
    omitWhen(v) {
      if (v.quotesPerMonth === 0) {
        return {
          key: "marketing.savings.omit.quote_writing.noQuotes",
          text: "You told us you send no quotes in a month, so there is no quote-writing time here to shorten.",
        };
      }
      ...
      if (
        ...
        return {
          key: "marketing.savings.omit.quote_writing.alreadyFast",
          // unitLabel, not `${formatAmount(n)} minutes`. A contractor who
          // types 1 was told "a quote takes you 1 minutes" — the exact defect
          // unitLabel's own docstring exists to prevent, on the one branch
          // that is reached ONLY when the answer is small enough to hit it.
          // Found by scripts/check-savings-i18n.mjs rebuilding this sentence
          // from the catalogue and getting a different one back.
          text: `You told us a quote takes you ${unitLabel(v.quoteDeskMinutes, "minutes")}, which is already at or under what it takes here, so there is nothing on this line for us to claim.`,
          values: { minutes: asMinutes(v.quoteDeskMinutes) },
        };
      }
      return null;
    },
    build(v, fmt) {
      ...
      const sourceKey = answered
        ? "marketing.savings.line.quote_writing.sourceAnswered"
        : "marketing.savings.line.quote_writing.sourceDefault";
      return {
        amount,
        ...
        workingsKey: "marketing.savings.line.quote_writing.workings",
        workingsValues: {
          quotes: asCount(v.quotesPerMonth),
          months: asCount(A("months_per_year")),
          saved: asMinutes(saved),
          source: asPhrase(sourceKey, { minutes: asMinutes(today) }),
          fieldquo: asMinutes(A("quote_desk_minutes_fieldquo")),
          hourly: asMoney(v.hourlyCost),
        },
      };
    },
  }),
  Object.freeze({
    ...
    build(v, fmt) {
      ...
      return {
        ...
        workingsKey: "marketing.savings.line.under_billing.workings",
        workingsValues: { revenue: asMoney(revenue), share: asShare(shareRow.value) },
      };
    },
  }),
  Object.freeze({
    ...
    build(v, fmt) {
      ...
      return {
        ...
        workingsKey: "marketing.savings.line.change_orders.workings",
        workingsValues: { revenue: asMoney(revenue), share: asShare(shareRow.value) },
      };
    },
  }),
  Object.freeze({
    ...
    omitWhen(v) {
      if (v.quotesPerMonth <= v.projectsPerMonth) {
        return {
          key: "marketing.savings.omit.quotes_chased.winsAll",
          text: "You win everything you quote, on the numbers you gave us. There is nothing here to chase.",
        };
      }
      return null;
    },
    build(v, fmt) {
      ...
      return {
        ...
        workingsKey: "marketing.savings.line.quotes_chased.workings",
        workingsValues: {
          unwon: asMoney(unwonValue),
          recovery: asShare(A("quote_recovery_share")),
          margin: asShare(A("gross_margin")),
        },
      };
    },
  }),
  Object.freeze({
    ...
    build(v, fmt) {
      ...
      return {
        ...
        workingsKey: "marketing.savings.line.admin_time.workings",
        workingsValues: {
          hours: asCount(v.adminHoursPerWeek),
          share: asShare(shareRow.value),
          weeks: asCount(A("weeks_per_year")),
          hourly: asMoney(v.hourlyCost),
        },
      };
    },
  ...
  Object.freeze({
    ...
    build(v, fmt) {
      ...
      return {
        ...
        workingsKey: "marketing.savings.line.invoice_sooner.workings",
        workingsValues: {
          revenue: asMoney(revenue),
          days: asDays(daysRow.value),
          cost: asShare(A("cost_of_money")),
        },
      };
    },
  }),
 ...
 *
 * Returns `{ key, text, values }` — the translation key, the English sentence,
 * and any figures inside it. The English sentence is not a fallback anybody
 * intends to render: it is what a caller with no catalogue (the check script)
 * reads, and what a language missing the key falls back to.
 */
 ...
 *
 * Still exported as a string rather than being written as a t() key here, and
 * the reason changed: it used to be that the page was English-only and one
 * translated sentence inside an English calculator is the half-translation
 * failure. The page is translated now. What keeps this a string is that
 * scripts/check-savings.mjs reads it — under plain node, with no React and no
 * language context — to assert that it names both currencies and says what
 * decides between them. The catalogue carries the translation under
 * marketing.savings.currency.short/long, and check:savings-i18n asserts the
 * English of those two keys is character-identical to this.
 */
 ...
 *
 * ── The locale is an argument now, and en-CA is only the default ───────────
 *
 * It used to be hardcoded, with a comment arguing that a fixed locale avoids a
 * hydration mismatch. The argument was for the wrong constant: what avoids the
 * mismatch is server and client agreeing, not English winning. The reader's
 * language is already decided before either render — /pricing and /compare
 * both group their figures through numberLocaleFor(language) for exactly this
 * reason — so passing it through is stable AND right, where "en-CA" was stable
 * and wrong for six of the nine languages this catalogue carries. A French or
 * Ukrainian reader writes 12 500, a German or Italian one 12.500, and a
 * Punjabi one groups at a different place entirely.
 *
 * Default kept so every existing caller — scripts/check-savings.mjs among them
 * — reads exactly what it read before.
 */
export function formatAmount(n, locale = DEFAULT_NUMBER_LOCALE) {
  const v = Number(n);
...
export function subscriptionCost({ seats, crew } = {}) {
  ...
  return {
    ...
    // A custom size carries its counts in the name; see costCompare.js.
    label: tier.custom ? tier.name : tier.label,
    monthly: tier.price,
...
/**
 * The largest business FieldQuo has a price for on its own — the custom cap
 * (lib/pricing/ladder.js), not the top published rung, since the fifth rung
 * prices everything up to a hundred people. The same numbers as
 * costCompare.js's LADDER_CEILING, read from the same constants.
 */
export const LADDER_CEILING = Object.freeze({
  seats: CUSTOM_MAX_SEATS,
  crew: CUSTOM_MAX_SEATS + CUSTOM_CREW_GAP,
});
  ...
  for (const builder of LINE_BUILDERS) {
    ...
    if (reason) {
      omitted.push({
        key: builder.key,
        label: builder.label,
        // A string, as it has always been. The key and its figures ride
        // alongside rather than replacing it.
        reason: reason.text,
        reasonKey: reason.key,
        reasonValues: reason.values || null,
      });
      continue;
    }
    ...
    lines.push({
      ...
      workingsKey: built.workingsKey,
      workingsValues: built.workingsValues,
    });
  }
