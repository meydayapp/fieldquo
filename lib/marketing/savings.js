...
// probably the largest thing a contractor does that software can shorten:
// contractors report 45–90 minutes of desk work on a residential estimate, and
// the ones who already work from templates report 10–15. The whole distance
// between those two figures is a price book, saved material recipes, a
// measurement you did not have to take by hand, and one button that sends it.
// See quote_desk_minutes_today for where the numbers came from and how they
// are cited.
//
...
const ASSUMPTION_ROWS = [
  {
    ...
    label: "Desk time an estimate takes you today",
    value: 45,
    unit: "minutes",
    display: "45 minutes",
    represents:
      "The part of producing one estimate that happens at a desk — working out the quantities, pricing them, writing the thing up and formatting it. Not the drive and not the walkthrough.",
    reasoning:
      "Contractors' own reported figures, supplied to us as ranges: one painter breaks a residential estimate into 20–60 minutes working it out plus 10–30 minutes typing and emailing it; another puts a properly done one at about an hour and a half; a separate thread reports 30–45 minutes even on small jobs and 1–2 hours on large ones. A step-by-step breakdown of a single quote gives 45 minutes for the calculating and writing, with the drive, the walkthrough and the sending listed separately. Forty-five is that breakdown's own figure, it sits below the middle of everything reported, and it excludes the sending — which we also remove. We did not run these surveys and we are not presenting them as ours.",
    basis: "reported",
  },
  {
    ...
    reasoning:
      "Contractors who already work from templates report getting a quote out in 10–15 minutes. We take the TOP of that range, which is the end that makes our saving smaller — 15 rather than 10 costs this line a third of its value. It is not zero and it should not be: somebody still has to look at the job and decide what is in it. What goes away is the arithmetic and the typing.",
    basis: "reported",
  },
  {
...
export const INPUT_FIELDS = Object.freeze([
  ...
  Object.freeze({
    ...
    help: "Wage plus what you carry on top of it. If it is your own time — and writing quotes usually is — what you would bill that hour at.",
  }),
  Object.freeze({
    key: "tools",
    kind: "choice",
...
export const LINE_BUILDERS = Object.freeze([
  Object.freeze({
    ...
    mechanism:
      "Your rates and your material quantities are already in the system, the roof or the driveway can be measured from the address instead of by hand, three price options come off one build, and one button sends it in your colours. What is left is deciding what is in the job — which is your work, not typing.",
    proof: [
    ...
    build(v, fmt) {
      const saved = A("quote_desk_minutes_today") - A("quote_desk_minutes_fieldquo");
      const quotesPerYear = v.quotesPerMonth * A("months_per_year");
      const amount = (quotesPerYear * saved * v.hourlyCost) / A("minutes_per_hour");
      return {
        amount,
        workings: `${fmt(v.quotesPerMonth)} quotes a month × ${assumptionRow("months_per_year").display} months × ${fmt(saved)} minutes saved on each (${assumptionRow("quote_desk_minutes_today").display} of desk work today, ${assumptionRow("quote_desk_minutes_fieldquo").display} from a price book) × ${fmt(v.hourlyCost)} an hour`,
      };
    },
  }),
  Object.freeze({
