...
const ENTRIES = [
  ...
  },
  {
    ...
    group: "winning_work",
    availability: "every_plan",
    // The pricing and the API are real; the SCREEN is not.
    // scripts/check-route-callers.mjs lists /api/quotes/tier-group under
    // NO_FRONT_DOOR, and nothing in app/ creates or renders a trio. This read
    // "shipped" on /features, /pricing, /compare and the glossary — four public
    // surfaces selling a control nobody can press.
    readiness: "partial",
    limits:
      "The pricing behind it is built and the three quotes can be produced behind the scenes, but there is no screen for it yet — so today you would be building all three yourself. Ask us before you buy on this one.",
    featureKey: null,
    proof: [
  ...
  {
    key: "paint_takeoff",
    name: "Painting priced by production rate",
    summary:
    ...
    limits: null,
    featureKey: null,
    // Pure arithmetic in lib/pricing/paintTakeoff.js (hours from production
    // rates, gallons from coverage, situation-named rate sets per estimate
    // type), the builder panel that drives it, and the settings card where a
    // company edits its rates. scripts/check-paint-takeoff.mjs executes the
    // arithmetic against the owner's own completed jobs to the cent.
    proof: [
      { path: "lib/pricing/paintTakeoff.js", holds: ["export function paintTakeoff", "export const PAINT_RATE_SET_DEFAULTS", "export function rateSetFor"] },
      { path: "app/components/quotes/builder/PaintAreas.js", holds: ["paintTakeoff("] },
      { path: "app/components/quotes/builder/TradeTakeoff.js", holds: ["InteriorPaintTakeoff", "ExteriorPaintTakeoff"] },
      { path: "app/app/settings/services/PaintRateSets.js", holds: ["PAINT_ESTIMATE_TYPES"] },
    ],
  },
  {
    key: "aerial_measure",
  ...
  // The key is unchanged on purpose — compareCopy, the Projul tier map, the
  // trade glossary and nine feature-page catalogues all point at it — but the
  // NAME now says what the feature is. It used to be one narrow thing (a
  // sub's quote as a cost line) with a limits sentence listing four things it
  // could not do; those four are built (lib/subcontractors/, /app/subcontractors,
  // the "Subs on this job" panel, the year-to-date paid figure), so the
  // sentence is gone and the entry is shipped. Docs/OPEN-WORK-APP.md asked for
  // exactly this.
  //
  // The summary used to promise "year-end tax forms", on the strength of the
  // T5018/1099 year-end CSV. That file is gone from the product — companies
  // can import but not export (owner, 2026-09-24) — and the route that still
  // exists answers 403, so it is not proof of anything a customer can use.
  // What is left and true is the figure itself, on screen: what each sub was
  // paid in a year, per sub, from their payments.
  {
    ...
    proof: [
      ...
      // The roster, the money and the paperwork.
      { path: "lib/subcontractors/money.js", holds: ["jobSubcontractCost", "yearToDatePaid"] },
      { path: "lib/subcontractors/expiry.js", holds: ["subcontractorAttention"] },
      { path: "app/api/subcontractors/route.js", holds: ["export async function POST"] },
      { path: "app/api/subcontractors/[id]/payments/route.js", holds: ["export async function POST"] },
      { path: "app/api/subcontractors/[id]/documents/route.js", holds: ["export async function POST"] },
      { path: "app/api/subcontractors/route.js", holds: ["yearToDatePaidBySubcontractor"] },
      { path: "app/api/jobs/[id]/subcontractors/route.js", holds: ["export async function POST"] },
      { path: "app/app/subcontractors/page.js", holds: ["paidThisYear"] },
      { path: "app/components/jobs/JobSubcontractors.js", holds: [] },
      { path: "lib/costing/actualJobCost.js", holds: ["jobSubcontractCost"] },
    ],
  },
  ...
  },
  {
    ...
    proof: [
      ...
      { path: "app/app/scheduler/page.js", holds: ["DayBoard"] },
    ],
  },
  {
    key: "crew_shifts",
    ...
    featureKey: null,
    proof: [
      { path: "app/api/shifts/route.js", holds: ["export async function POST", "validateBreaks"] },
      { path: "app/api/shifts/publish/route.js", holds: ["export async function POST"] },
      // The day board and its coverage strip — lunches and breaks per shift,
      // who is left on site per half hour. lib/shifts/coverage.js is the pure
      // arithmetic, executed by check:shift-board.
      { path: "app/app/scheduler/DayBoard.js", holds: ["coverageSlots"] },
      { path: "lib/shifts/coverage.js", holds: ["export function coverageSlots", "export function validateBreaks"] },
      { path: "app/app/schedule/page.js", holds: [] },
      // "Everyone sees their own shifts" is the employee home: the next
      // shift on /app/me, the trade / cover / claim flow behind it, and the
      // availability request a manager approves. lib/shiftRequests/state.js
      // is the pure state machine, executed by check:employee-home.
      { path: "app/app/me/page.js", holds: ["WorkerHome"] },
      { path: "lib/shiftRequests/state.js", holds: ["export function transition"] },
      { path: "lib/availability/days.js", holds: ["export function normaliseDays"] },
    ],
  },
  {
    key: "recurring_jobs",
  ...
  },
  {
    ...
    group: "doing_the_job",
    availability: "every_plan",
    // Was "partial", on the grounds that only a visit created from the visit
    // screen picked up a template. The visit screen itself can apply a template
    // to ANY visit after the fact (VisitChecklist's applyTemplate), and a
    // recurring visit carries its list forward — so the caveat described a
    // starting state, not a missing capability. Applying a template is a
    // choice rather than a default on purpose; that is a design, not a gap.
    readiness: "shipped",
    limits: null,
    featureKey: null,
    proof: [
      ...
      { path: "app/components/jobs/VisitChecklist.js", holds: ["applyTemplate"] },
      { path: "lib/jobs/recurrence.js", holds: ["checklistItems: last.checklistItems"] },
    ],
  },
  {
    key: "suggested_tasks",
  ...
  },
  {
    ...
    proof: [
      ...
      // The forgotten clock-out: every fifteen minutes, "Still clocked in?"
      // to the worker and a note to the manager; never an auto clock-out.
      { path: "app/api/cron/time-clock-watch/route.js", holds: ["forgottenClockOut"] },
    ],
  },
  {
    key: "timesheets",
    ...
    proof: [
      ...
      // The attendance verdict beside each punch (lib/shifts/attendance.js,
      // executed by check:shift-notify). The timesheet CSV that used to be
      // cited here is not: bulk export is off (owner, 2026-09-24) and its
      // route answers 403, so it proves nothing a customer can do.
      { path: "lib/shifts/attendance.js", holds: ["export function attendanceFor"] },
    ],
  },
  {
    key: "crew_inbox",
  ...
  },
  {
    ...
    proof: [
      ...
      // "and holidays": the statutory calendar computed by rule, and the
      // blackout / most-off-at-once limits the request route judges.
      { path: "lib/leave/statutoryHolidays.js", holds: ["export function holidaysFor"] },
      { path: "lib/leave/rules.js", holds: ["export function concurrentRefusal", "export function blackoutRefusal"] },
    ],
  },
  ...
  },
  {
    ...
    name: "Every other way to get paid",
    summary:
    ...
    featureKey: null,
    proof: [
      { path: "lib/payments/offlineMethods.js", holds: ["export function buildHowToPay", "export function methodsForCountry"] },
      { path: "lib/documentSections/HowToPaySection.js", holds: ["export function PdfSection"] },
      { path: "app/app/settings/payments/PaymentMethodsCard.js", holds: ["paymentMethodDetails"] },
    ],
  },
  {
    key: "stripe_connect",
  ...
  },
  {
    ...
    group: "running_the_business",
    availability: "every_plan",
    // "partial", not "shipped" — a "shipped" entry may not carry a limits
    // note (this file's own rule, asserted below), and the honest version of
    // this feature has one: real, and worth naming, not a reason to hide it.
    readiness: "partial",
    // This used to say nothing in FieldQuo linked a dollar of spend to a
    // lead. lib/analytics/campaignRollup.js now does, for exactly one path:
    // a MarketingSpend row synced from Meta carries campaignId, a LeadRequest
    // imported from a Meta lead form carries metaCampaignId, and the two are
    // the same id. So the limit narrows rather than disappears — the
    // homeowner who saw the ad and PHONED is still unattributed, and so is
    // every channel that is not Meta — and the sentence names the boundary
    // rather than the old blanket. "Blended" is still the honest claim for
    // the rest (lib/analytics/kpis.js's buildBlendedCostPerLead).
    limits:
      "Per-campaign cost per lead covers leads that arrived through a Meta " +
      "lead form. Every other channel — and a homeowner who saw the ad and " +
      "phoned — is still blended across everything, because nothing links " +
      "that spend to that lead.",
    // The screen lives under app/app/marketing/*, which app/app/marketing/
    // layout.js wraps in <FeatureGate feature="marketing_campaigns">
    // wholesale — this entry's proof includes that page, so it has to name
    // the same gate or this file's own check (below) calls it out as an
    // undeclared, withholdable feature hiding in "included in every plan".
    featureKey: "marketing_campaigns",
    proof: [
      { path: "app/api/marketing-spend/route.js", holds: ["export async function POST"] },
      { path: "app/app/marketing/spend/page.js", holds: ["fetchArray(\"/api/marketing-spend\")"] },
      { path: "lib/analytics/kpis.js", holds: ["export function buildBlendedCostPerLead"] },
      { path: "lib/meta/insightsImport.js", holds: ["export function buildImportPlan"] },
      { path: "lib/analytics/campaignRollup.js", holds: ["export function buildCampaignRollup"] },
    ],
  },
  {
    key: "payroll",
    ...
    readiness: "partial",
    limits:
      "FieldQuo works out gross pay and produces a payslip per person. It does " +
      "not pay employees, file your payroll taxes or export the run — " +
      "deductions are the ones you or your accountant supply.",
    featureKey: null,
    proof: [
      ...
      // The payslip, one person at a time — the document a worker is owed.
      // The run's CSV export used to be cited here; bulk export is off
      // (owner, 2026-09-24) and that route answers 403.
      { path: "app/api/payroll/runs/[id]/payslip/[lineId]/route.js", holds: ["export async function GET"] },
    ],
  },
  {
    ...
    limits:
      ...
      "rate you set. It cannot pay a fixed bid to another company.",
    featureKey: null,
    proof: [
      {
  ...
  {
    key: "languages",
    name: "Your client's language",
    summary:
      "Send a quote in the language your client speaks; a signed document keeps the words it was signed with.",
    group: "running_the_business",
    availability: "every_plan",
    // Was "partial" with a caveat that only English and French were switched
    // on. That stopped being true when the PDF font stack was fixed
    // (lib/documents/pdfFont.js) — the caveat outlived the bug it described,
    // and the sales guide printed it. The proof below is the actual coverage:
    // every language in app/i18n/languages.js has a hand-written document
    // catalogue and a covering-email catalogue, and the PDF can set all of them.
    readiness: "shipped",
    limits: null,
    featureKey: null,
    proof: [
      ...
      { path: "lib/i18n/clientLanguage.js", holds: [] },
      { path: "lib/i18n/documentLabels.js", holds: ["export function documentLabels"] },
      { path: "lib/i18n/emailCopy.js", holds: ["export function emailCopy"] },
      { path: "lib/documents/pdfFont.js", holds: ["Gurmukhi"] },
    ],
  },
  {
    key: "ai_copilot",
    ...
    proof: [
      { path: "app/api/ai/copilot/route.js", holds: ["export async function POST", "meterFor(\"copilot\""] },
  ...
  {
    key: "hr_compliance",
    name: "Employee files and compliance",
    summary:
    ...
    featureKey: null,
    proof: [
      { path: "app/api/hr/workers/[workerId]/documents/route.js", holds: ["export async function POST", "parseWorkerDocumentBody"] },
      { path: "app/api/hr/me/policies/[id]/acknowledge/route.js", holds: ["export async function POST", "parseAcknowledgement"] },
      { path: "lib/onboarding/run.js", holds: ["export function reconcile", "export function runPatch"] },
      { path: "lib/hr/compliance.js", holds: ["export function complianceRows"] },
      { path: "app/api/cron/hr-reminders/route.js", holds: ["reminderDue"] },
      { path: "app/app/settings/team/compliance/page.js", holds: [] },
    ],
  },
];
...
export const MATRIX_EXCLUSIONS = Object.freeze([
  Object.freeze({
    subject:
      "An AI employee that answers a homeowner's message — a closer, a receptionist or a troubleshooter",
    registryKey: "ai_employee",
    reason:
      "Built end to end: the role presets and their tool lists, the eight " +
      "guards that decide whether to answer at all, the tool loop through " +
      "lib/ai/provider.js, the metering, the uploaded resource material, the " +
      "settings screen and a test box that runs the real reply path. What it " +
      "has no way to do is SEND, because the channel it answers on is " +
      "page_messaging and Meta has not approved that for this app — so on a " +
      "real company today it drafts, and the draft waits for a person. " +
      "Selling 'an AI employee that replies to your Facebook messages' on a " +
      "public page would be selling the half that is blocked. Move this into " +
      "the table on the same day page_messaging moves — the App Review, round " +
      "two, docs/META-APP-REVIEW-SUBMISSION.md. The FORMATS are the second " +
      "honest limit and belong in whatever copy this becomes: FieldQuo reads " +
      "text material (.txt, .md, .csv, pasted text) and refuses a PDF or a " +
      "Word file by name, because there is no reader for either in " +
      "package.json — see lib/aiEmployee/sources.js.",
  }),
  Object.freeze({
    subject:
      "Facebook Page and Instagram messages in FieldQuo, and the month-end read of which conversations became jobs",
    registryKey: "page_messaging",
    reason:
      "Built end to end — the signed webhook, the stored conversations, the " +
      "reply path, the outcome per thread and the monthly review — but Meta " +
      "has not approved pages_messaging or instagram_manage_messages for this " +
      "app, so no real contractor can connect a Page yet. The screen says so " +
      "and the composer is disabled with the reason printed on it, which is " +
      "honest INSIDE the product; saying it on a public page would sell a " +
      "thing nobody can switch on. Move this into the table on the day the " +
      "App Review comes back and META_MESSAGING_APPROVED is set — see " +
      "docs/META-APP-REVIEW-SUBMISSION.md, round two.",
  }),
  Object.freeze({
    subject:
      "WhatsApp Business messages answered in FieldQuo, beside Facebook and Instagram",
    registryKey: "whatsapp_messaging",
    reason:
      "Built end to end — Meta's Embedded Signup, the signed webhook, the " +
      "stored conversations, the send path, the approved-template list and " +
      "the 24-hour customer service window that decides which of the two may " +
      "be sent — but Meta has not approved whatsapp_business_messaging for " +
      "this app, so no real contractor can connect a number yet. Settings " +
      "says so and the connect route refuses server-side, which is honest " +
      "INSIDE the product; saying it on a public page would sell a thing " +
      "nobody can switch on. There is a SECOND honest limit that belongs in " +
      "whatever copy this becomes even after approval, and it must not be " +
      "left out: WhatsApp itself refuses a typed message more than 24 hours " +
      "after the customer last wrote, and only a template Meta approved in " +
      "advance may be sent then. A page promising 'message your customers on " +
      "WhatsApp' without that sentence is promising something WhatsApp does " +
      "not allow. Move this into the table on the day META_WHATSAPP_ENABLED " +
      "is set — see docs/META-APP-REVIEW-SUBMISSION.md.",
  }),
  Object.freeze({
    subject: "A marketing design canvas with AI image tools",
    registryKey: "marketing_designer",
    reason:
      "The editor (app/components/designer/) is built and its spend gate is " +
      "real, but no /app page mounts DesignerLoader yet — see that file's " +
      "own comment — and the AI actions have no vendor wired behind them " +
      "(lib/designer/aiImageAdapter.js's TODO seam, pending a sibling " +
      "worktree's lib/ai/images.js). Claiming this on a public page before a " +
      "contractor can actually open the screen is the exact promise-before-" +
      "the-product failure this file exists to catch. Move this into the " +
      "table once DesignerLoader is reachable from a real page.",
  }),
  Object.freeze({
    subject: "Publishing an ad straight from the designer to Instagram or a Facebook Page",
    registryKey: "marketing_designer",
    reason:
      "A Publish control is built (app/components/designer/PublishModal.js) " +
      "and the container-then-publish flow it calls (lib/social/) is real " +
      "code, but there is no Meta app connection behind it yet — " +
      "lib/social/metaConnection.js always reports not connected, honestly, " +
      "until a sibling worktree's per-tenant Meta OAuth lands. Even once it " +
      "does, Meta's own instagram_content_publish and pages_manage_posts " +
      "permissions require App Review and Business Verification, both " +
      "unstarted and each measured in weeks, not days. Claiming a working " +
      "publish-to-social feature before a contractor can actually connect an " +
      "account and Meta has approved the app is the exact promise-before-" +
      "the-product failure this file exists to catch — see " +
      "docs/SOCIAL-PUBLISHING.md for the full state and what's blocked.",
  }),
  Object.freeze({
  ...
  Object.freeze({
    subject: "A paid, closer AI read of a quote's own photos",
    registryKey: "ai_vision",
    reason:
      "Real and shipped — app/components/quotes/SuggestAddOns.js — but it is " +
      "an in-context upsell inside the quote builder, priced per use in AI " +
      "credit, not a plan-tier differentiator a painter compares Solo against " +
      "Scale on. It belongs next to the quote it reviews, not on this table.",
  }),
  Object.freeze({
    subject: "Generating marketing images with AI",
    registryKey: "marketing_designer",
    reason:
      "The generation endpoint is built and gated (lib/ai/images.js, " +
      "app/api/marketing/designer/images/route.js) but the canvas editor a " +
      "contractor would actually use is separate, unshipped work, and the " +
      "registry entry defaults to hidden until it lands. Naming it here would " +
      "claim a product surface nobody can reach yet.",
  }),
  Object.freeze({
    subject: "The KPI dashboard",
    registryKey: "kpi_dashboard",
    reason:
      "Shipped and reachable in the app (app/api/analytics/kpis, " +
      "app/app/analytics/kpis) — win rate, margin, on-time completion, what's " +
      "owed. Excluded here because no public /features page has been written " +
      "for it yet; claim it once one exists rather than pointing this row at " +
      "a page that doesn't.",
  }),
]);
  ...
  planColumns: Object.freeze([
    ...
    Object.freeze({
      ...
      mentionedIn: Object.freeze([
        ...
        // Copies Scale's value onto a custom-size Plan row when one is minted
        // (43b70c4d), and echoes it on the tier it prices — carrying the
        // column, not enforcing it.
