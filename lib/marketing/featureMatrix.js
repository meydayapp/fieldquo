...
const ENTRIES = [
  ...
  },
  {
    ...
    readiness: "partial",
    // Deliberately NOT "spend against the jobs it actually brought in" — that
    // was the false claim (docs/META-ADS-INTEGRATION.md Part 1): nothing
    // links a channel's spend to a specific LeadRequest, so a per-channel
    // cost-per-lead is refused the same way lib/analytics/kpis.js's
    // NOT_TRACKED.costPerLead refuses it. "Blended" is the honest claim —
    // total spend over total real leads, company-wide, no attribution
    // implied — see lib/analytics/kpis.js's buildBlendedCostPerLead.
    limits:
      "Cost per lead is blended across every channel, not broken out per " +
      "channel or per campaign — nothing in FieldQuo links a specific dollar " +
      "of spend to a specific lead yet.",
