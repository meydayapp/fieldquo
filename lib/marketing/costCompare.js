...
/**
 * The largest business FieldQuo has a price for on its own.
 *
 * This was the top of the four published rungs (10 seats, 15 crew). Since
 * the fifth rung shipped (18 September) — a custom size, lib/pricing/ladder.js
 * customTier — prices everything up to a hundred people, so the ceiling the
 * "you are past our plans" sentence names is the custom cap: forty-seven
 * seats and fifty-two crew. Read off the ladder's own constants, never
 * typed here.
 */
export const LADDER_CEILING = Object.freeze({
  seats: CUSTOM_MAX_SEATS,
  crew: CUSTOM_MAX_SEATS + CUSTOM_CREW_GAP,
  people: CUSTOM_MAX_SEATS + CUSTOM_MAX_SEATS + CUSTOM_CREW_GAP,
});
...
export function fieldquoCost({ officeSeats, fieldCrew }) {
  ...
  return Object.freeze({
    ...
    // A custom size is named with its counts — "Custom · 11 seats · 16
    // crew" — because "Custom" alone tells a reader nothing about the price
    // beside it.
