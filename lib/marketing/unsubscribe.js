 ...
 */
//
// `language` is the email's (renderTemplateSections' options.language). The
// line used to be English on every send, so a French client's French
// follow-up ended in an English legal line — the one sentence CASL requires
// them to be able to read. The sentence is lib/i18n/emailCopy.js's
// templateBlocks.marketingFooter; `a` places the link inside it, because
// where the link falls in the sentence is word order, not markup.
export function unsubscribeFooterRow({ token, request, theme, language = "en" }) {
  const url = unsubscribeUrl(token, request);
  const words = emailCopy(language).templateBlocks;
  const link = (text) =>
    `<a href="${escapeAttr(url)}" style="color:${theme.muted};text-decoration:underline;">${escapeHtml(text)}</a>`;
  return `<tr><td style="background:${theme.bg};border-top:1px solid ${theme.border};padding:14px 30px 20px;font-size:11px;line-height:1.6;color:${theme.muted};">${words.marketingFooter(escapeHtml(theme.companyName || words.thisCompany), link)}</td></tr>`;
}
