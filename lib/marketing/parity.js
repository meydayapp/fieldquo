...
import * as roofrTiers from "./tierFeatures/roofr";
import * as paintscoutTiers from "./tierFeatures/paintscout";
...
function publishedLadder(tiers, map) {
  ...
  for (const t of tiers) {
    ...
    if (price < (existing.monthly ?? Infinity)) {
      ...
      existing.perExtraUser = t.perExtraUser ?? existing.perExtraUser ?? null;
      existing.unlimitedUsers = t.unlimitedUsers ?? existing.unlimitedUsers ?? false;
      existing.limitNote = t.limitNote ?? existing.limitNote ?? null;
    }
  ...
  return ordered.map((t) => {
    ...
    return {
      ...
      // ── Three facts about headcount and limits a seat count cannot hold ──
      //
      // `unlimitedUsers` — Roofr prints "Unlimited users" on every card and
      // records seatsIncluded: null; without this flag a null seat count reads
      // as "nobody checked", which is a different fact from "as many as you
      // like". `perExtraUser` — PaintScout's "$20/user/month", the number
      // shopMath prices a shop with. `limitNote` — Roofr Starter's "10 trial
      // proposals, invoices & work orders", printed beside the $0 so a free
      // tier that stops at ten proposals cannot read as where a business runs.
      unlimitedUsers: t.unlimitedUsers === true,
      perExtraUser: typeof t.perExtraUser === "number" ? t.perExtraUser : null,
      limitNote: typeof t.limitNote === "string" ? t.limitNote : null,
      phrases,
      unmapped: phrases.filter((p) => !Object.hasOwn(map, p)),
      adds: added,
      covers: new Set(carried),
...
/**
 * The paid add-on of theirs that carries this capability, or null.
 *
 * ══ Why an add-on's keys count as "they sell it" ══════════════════════════
 *
 * PaintScout puts scheduling, the pipeline, tasks and reminders on a $99/mo
 * Operations add-on and on no tier at all. A page reading only the tier
 * ladder would list all four as "not listed on their pricing page", which is
 * false — they are listed, priced, on the same page, beside the plan. The
 * Jobber ladder never hit this because Plus bundles all three of Jobber's
 * add-ons, so every add-on key was already covered by a tier.
 *
 * So a key found only on a paid add-on is reported as sold separately: it is
 * not "missing" (neverListed excludes it), it is "covered" for the parity
 * count (the mapping is generous to them on purpose), and the head-to-head
 * row prints the add-on's label and price instead of "Not in their plans".
 * The only thing it is not is FREE, and the add-on row on the same page says
 * what it costs.
 */
export function addOnWith(competitorId, key, asOf = null) {
  for (const addOn of addOnsFor(competitorId, asOf)) {
    if (addOn.includedFree) continue;
    if ((addOn.keys || []).includes(key)) return addOn;
  }
  return null;
}

/** Every capability their paid add-ons carry, as a Set. */
function addOnKeys(competitorId, asOf = null) {
  return new Set(
    addOnsFor(competitorId, asOf)
      .filter((a) => !a.includedFree)
      .flatMap((a) => a.keys || []),
  );
}

/**
 * Does headcount move this competitor's price at all?
 *
 * Read off the unit they declare in competitors.js, never guessed from a seat
 * count. Roofr (unlimited users, flat fee plus per-report charges) and Projul
 * (flat annual fee) say no; the "what it costs for a shop like yours" section
 * is built on "they bill every login" and must not render for a company whose
 * page says the opposite.
 */
export function billsPerHead(competitorId) {
  const competitor = COMPETITORS.find((c) => c.id === competitorId);
  const unit = competitor ? PRICING_UNITS[competitor.pricingUnit] : null;
  return Boolean(unit) && unit.mapsTo !== "none";
}
...
export function parityFor(competitorId, { keys = null, asOf = null } = {}) {
  ...
  const sold = addOnKeys(competitorId, asOf);
  // A key on a paid add-on counts as covered — see addOnWith. It is not on
  // any tier, so `needed` below still lands on the cheapest tier carrying
  // everything the TIERS carry, and the add-on row prices the rest.
  const covered = wanted.filter((k) => top.covers.has(k) || sold.has(k));
  const missing = wanted.filter((k) => !top.covers.has(k) && !sold.has(k));
  ...
  const needed =
    ladder.find((t) => covered.every((k) => t.covers.has(k) || sold.has(k))) || top;
...
export function neverListed(competitorId, asOf = null) {
  ...
  const sold = addOnKeys(competitorId, asOf);
  return FEATURE_MATRIX.filter(
    (e) => e.readiness === "shipped" && !top.has(e.key) && !sold.has(e.key),
  );
}
...
export const DERIVED_SEAT_TOTAL = "seat_total";

/**
 * A per-seat vendor's price for a shop bigger than the plan includes.
 *
 * PaintScout: $119 includes one user and each further user is $20, so a shop
 * of eleven is $119 + 10 × $20 = $319. Both inputs are printed on their page;
 * the count is the reader's own headcount, printed in the same box; and the
 * arithmetic is declared so scripts/check-compare-pages.mjs can re-run it
 * from the markup. `exact` is true by construction — integers in, integer
 * out — and is carried anyway so derivationProps has one shape.
 */
export function derivedSeatTotal({ base, perUser, extraUsers }) {
  if (typeof base !== "number" || typeof perUser !== "number") return null;
  if (!Number.isInteger(extraUsers) || extraUsers < 0) return null;
  const value = Math.round((base + perUser * extraUsers) * 100) / 100;
  return {
    op: DERIVED_SEAT_TOTAL,
    inputs: [base, perUser],
    count: extraUsers,
    months: MONTHS_PER_YEAR,
    value,
    exact: Math.abs(base + perUser * extraUsers - value) < 1e-9,
  };
}
 ...
 */
export function recomputeDerived({ op, inputs = [], months = MONTHS_PER_YEAR, count = null } = {}) {
  if (op === DERIVED_SEAT_TOTAL) {
    if (inputs.length !== 2 || !Number.isInteger(count) || count < 0) return null;
    return Math.round((inputs[0] + inputs[1] * count) * 100) / 100;
  }
...
export function derivationProps(d) {
  ...
  return {
    ...
    // Only a seat total has a count. Absent rather than "null" on the others,
    // so the check's parser can tell "no count" from a count it failed to read.
    ...(Number.isInteger(d.count) ? { "data-derived-count": String(d.count) } : {}),
  };
}
...
export function shopMath({ estimators = 1, crew = 0 } = {}, competitorId, asOf = null) {
  ...
  // A vendor whose price does not move with headcount has no answer to "what
  // does it cost for a shop like yours" that this section can print: the
  // question assumes every login is billed, and Roofr's and Projul's pages
  // say the opposite. Null competitor, and the caller renders nothing rather
  // than a row that prices a flat fee three times and calls it a comparison.
  if (!billsPerHead(competitorId)) {
    return {
      heads,
      headcountBilled: false,
      fieldquo: plan
  ...
  // of them — not just the ones who write quotes. A tier that includes fewer
  // than that but prices the next user (PaintScout: one included, $20 each
  // after) seats them at that price; the arithmetic is declared below.
  const priced = ladder.filter((t) => typeof t.price === "number");
  const tier =
    priced.find(
      (t) => t.seats === null || t.seats >= heads || typeof t.perExtraUser === "number",
    ) ||
    priced.at(-1) ||
    null;
  const extraUsers =
    tier && typeof tier.perExtraUser === "number" && typeof tier.seats === "number" && heads > tier.seats
      ? heads - tier.seats
      : 0;
  const seatTotal =
    tier && extraUsers > 0
      ? derivedSeatTotal({ base: tier.price, perUser: tier.perExtraUser, extraUsers })
      : null;
  const theirPrice = seatTotal ? seatTotal.value : tier ? tier.price : null;

  return {
    heads,
    headcountBilled: true,
    fieldquo: plan
      ? { label: plan.label, price: plan.price, seats: plan.seats, crew: plan.crewSeats }
      : null,
    competitor: tier
      ? {
          ...
          // The plan price and the per-user price, separately, so the page
          // can show the working beside the total.
          basePrice: tier.price,
          perExtraUser: typeof tier.perExtraUser === "number" ? tier.perExtraUser : null,
          extraUsers,
        }
      : null,
    ...
    savesPerMonth: plan && tier ? Math.round((theirPrice - plan.price) * 100) / 100 : null,
    savesPerYear:
      plan && tier ? Math.round((theirPrice - plan.price) * MONTHS_PER_YEAR * 100) / 100 : null,
    // The same number again, with its working attached. See DERIVED_SAVING.
    saving: plan && tier ? derivedSaving({ theirs: theirPrice, ours: plan.price }) : null,
    // And the seat arithmetic that produced `theirPrice`, when there was any.
    seatTotal,
  };
}
