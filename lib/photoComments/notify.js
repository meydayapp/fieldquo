...
async function crewSmsChannel({ companyId, phone, body }, deps = {}) {
  ...
  // companyId for the same reason emailChannel below passes it: the seam is
  // the COMPANY, not the address or the number, because a demo can be
  // re-dressed and its members edited. See lib/sms/demoSms.js.
  const result = await send({ to: e164, from: line.e164, body, companyId, purpose: "photo_mention" });
