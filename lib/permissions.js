...
//
// notes was the last one written and read by nothing, and it no longer is.
// It gates the INTERNAL notes on people — the call-back log on a lead
// (LeadNote) and the private notes on a client (Client.notes): read at
// view_all, written at view_edit_all, and a lead note removed at
// view_edit_delete_all, which is the rung that used to withhold a power nobody
// had (there was no delete endpoint; there is one now, DELETE
// /api/leads/[id]/notes/[noteId]). The floor, "jobs and visits only", is a
// visit's own notes, which every level keeps. A quote's or an expense's
// `notes` column is NOT this dial's — that text belongs to the document it
// sits on and the document's own dial answers for it. The reasoning, and the
// list of routes, is the Notes section of lib/permissions/enforce.js;
// scripts/check-notes-dial.mjs executes it.
//
// EVERY CATEGORY IN THE EDITOR NOW GATES SOMETHING. Keep it that way: a dial
// added here without a route that reads it is a dead control in the sense
// AGENTS.md means — a company sets it, sees it saved, and makes a staffing
...
export const PERMISSIONS = {
  ...
  supervisor: [
    "quote:create",
    "quote:convert",
    // Reassign a quote to somebody other than the person creating it. Same
    // rule as appointment:assign, and for the same reason: creating your own
    // (unassigned or self-assigned) quote is not a staffing decision, handing
    // one to a colleague is.
    "quote:assign",
...
export const PERMISSION_DENIALS = {
  ...
  "quote:assign":
    "Only an owner, admin or supervisor can assign a quote to someone else.",
...
export const PERMISSION_CATEGORIES = {
  ...
  // Internal notes on people: a lead's call-back log and a client's private
  // notes. The floor keeps a visit's own notes (which the visit routes gate on
  // jobs and assignee). See lib/permissions/enforce.js, Notes, for what each
  // rung opens.
  notes: {
    label: "Notes",
    levels: [
  ...
  // ── An incident naming an injured employee is sensitive ─────────────────
  //
  // The owner's own framing: "a crew member should be able to report one; not
  // everyone should be able to read every one." So the floor is not "none" —
  // withholding the ability to REPORT a safety incident from the person
  // standing on site is the one thing this grid must never do — it's
  // `report_own`: file a report, and see the ones you filed. Seeing
  // everyone's is a separate, higher grant, same shape as payroll's
  // view_own → view_all step.
  //
  // "own" here means "reported by you" — see the scoping note in
  // app/api/safety-incidents/route.js for why it does not also include
  // "incidents where you were the injured party but somebody else filed it".
  //
  // scopeFilter's own convention (enforce.js) reads "_all" in the stored
  // value as "sees everything" — view_all and view_edit_all both qualify.
  safety: {
    label: "Safety Incidents",
    levels: [
      { value: "none", label: "No access" },
      { value: "report_own", label: "Report incidents, and view their own" },
      { value: "view_all", label: "View everyone's incidents" },
      {
        value: "view_edit_all",
        label: "View everyone's incidents and follow up on them",
      },
    ],
  },
};
...
export const PERMISSION_PRESETS = {
  ...
  worker: {
    ...
    values: {
      ...
      jobs: "view_only",
      invoices: "none",
      // The person standing on the ladder is the one who has to be able to
      // report what happened on it. See the category's own comment.
      safety: "report_own",
      showPricing: false,
      jobCosting: false,
      payments: false,
  ...
  // booking page, invite an estimator. NOT set a labour rate: that is payroll
  // wherever it is written from, and PATCH /api/settings/members clamps it
  // behind payroll:view_all, which no preset holds — this comment used to
  // list it and the sales guide repeated it. An estimator writes quotes; none
  // of that is their job, and granting it to reach the billing tier would be
  // paying for authority nobody asked for.
  //
  ...
  estimator: {
    ...
    values: {
      ...
      jobs: "view_only",
      invoices: "view_only",
      // An estimator is on site for the assessment as often as a crew member
      // is on site for the work — same reasoning as Crew's own grant above.
      safety: "report_own",
      showPricing: true,
  ...
  dispatcher: {
    ...
    values: {
      ...
      // A team lead needs to see every incident to escalate one, and to close
      // one out with a follow-up note — same "view_edit_all" grant notes and
      // clientsProperties already give this preset.
      safety: "view_edit_all",
      showPricing: true,
      jobCosting: false,
      payments: false,
    },
