...
import { accountLanguageContext } from "./i18n/accountLanguage";
import { RESET_PAGE, withLandingPage, verifyPageLink } from "./authLinks";
...
// A team invitation lives a WEEK, not Better Auth's default 48 hours.
//
// The people being invited are crew. A contractor sends the invite on Friday
// afternoon from the van; the new hire opens it on Monday morning, or the
// week after when they actually start — and under the default they found a
// link that had died on Sunday, with nothing on Manage Team to send another
// (there is now: POST /api/settings/members/pending/[id]/resend). Seven days
// covers "sent it before the weekend, opened it on the first day" without
// leaving a credential-shaped link alive for a month.
//
// Exported because the pending route and the Team page show the expiry off
// the Invitation row, and a resend renews it by this same figure — Better Auth
// reads the option for both the create and the resend path
...
//
// The RESET link points at the auth API first, on purpose: that endpoint
// checks the token and then redirects to the page with `?token=…`, or with
// `?error=INVALID_TOKEN` if it has run out. Linking the page directly would
// skip that check and the reader would only learn the link was stale AFTER
// typing a new password.
//
// The CONFIRMATION link no longer does. It lands on /verify-email with the
// token, because the page has to say WHICH address was confirmed and compare
// it with whoever is signed in on that browser — see lib/authLinks.js, which
// also explains why both links now carry the recipient's language.
// RESET_PAGE, withLandingPage and verifyPageLink live there so the check
 ...
 * hooks; and the company is a FieldQuo concept entirely. The company is
 * context in a sentence, never authority: authEmails.js uses it as plain text.
 */
async function recipientContext(user) {
  // lib/i18n/accountLanguage.js — the same lookup /verify-email and
  // /reset-password render from, so the email and its page agree. Never
  // throws: a lookup failure costs the reader a personalised sentence, never
  // the email, and the email then goes out in English.
  const { company, language } = await accountLanguageContext({ id: user?.id });
  return { company, language: language || "en" };
}
 ...
 */
async function sendAccountEmail({ kind, render, user, link, expiresMinutes }) {
  try {
    const { company, language } = await recipientContext(user);

    // The link is built AFTER the language is known, because it carries it:
    // the page it opens renders in the language this email was written in.
...
export const auth = betterAuth({
  ...
  emailAndPassword: {
    ...
    async sendResetPassword({ user, url }) {
      await sendAccountEmail({
        ...
        user,
        link: (language) => withLandingPage(url, RESET_PAGE, language),
  ...
  emailVerification: {
    ...
    //
    // The consequence the page has to own: a link opened in a browser signed
    // in as SOMEBODY ELSE confirms the address and leaves that other session
    // exactly where it was (Better Auth only touches sessions for a change of
    // address or with auto sign-in — api/routes/email-verification.mjs). So
    // /verify-email names the address it confirmed and, when the session is
    // another account, says so and asks which one to carry on as.
    ...
    async sendVerificationEmail({ user, url }) {
      await sendAccountEmail({
        ...
        render: verifyEmail,
        user,
        link: (language) => verifyPageLink(url, language),
  ...
  plugins: [
    organization({
      creatorRole: "owner",
      // Seconds — the plugin passes it to getDate(…, "sec"). See the constant.
      invitationExpiresIn: INVITATION_EXPIRES_DAYS * 24 * 60 * 60,
