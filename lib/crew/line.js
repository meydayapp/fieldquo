...
import { isDemoCompany, createSimulatedCrewLine } from "@/lib/demo/simulatedSpend";
...
export async function claimCrewLine({ companyId, e164, origin, source = "shared_test" }) {
  ...
  // A demo is not lent the shared number either. Nothing is bought here — the
  // number is already FieldQuo's — but the loan would still put a REAL,
  // textable line on a demo account's screen, and a rep who reads it aloud has
  // handed a prospect a number that reaches FieldQuo's own inbound webhook. It
  // also holds the loan against the one company that could have been proving
  // the feature for real. Same simulated line as a purchase; see below.
  if (await isDemoCompany(companyId)) {
    return purchaseCrewLine({ companyId, e164: normalised, origin });
  }
  ...
  // ── Is it already doing a job? ─────────────────────────────────────────
  //
  // The two checks above answer "does FieldQuo's Twilio account own this
  // number, and can it text". Both are yes for every number FieldQuo owns —
  // including every sales rep's line and every contractor's receptionist
  // number — and neither asks whether the number is already committed.
  //
  // On 2026-09-10 that gap handed a contractor +17166383616, a line assigned to
  // a sales rep the same day, and repointed its SMS webhook at /api/crew/inbound
  // so the rep's inbound texts arrived in a customer's crew inbox. The claim
  // charged nothing, because a claim never does.
  //
  // Asked BEFORE the upsert and before the webhook is repointed. A check that
  // runs afterwards has already done the damage.
  const committed = await numberCommitment(normalised, {
    forCompanyId: companyId,
    sharedE164: sharedTestLineE164(),
  });
  // `self` is not a refusal — reconnecting your own line is the ordinary path
  // through here, and the upsert below is what makes it idempotent.
  if (committed && committed.kind !== "self") {
    return { ok: false, reason: committed.reason, status: committed.kind === "crew" ? 409 : 400 };
  }
...
export async function purchaseCrewLine({ companyId, e164, origin }) {
  ...
  // ── A sales demo never buys anything ──────────────────────────────────────
  //
  // First, before the vendor checks below, so the simulated path works on a
  // machine with no Twilio credentials at all — the same "never touches the
  // network" property lib/voice/demoLine.js gives the Retell side.
  //
  // Why this was missing and why it mattered: everything below is correct for
  // a real company and was catastrophic for a demo. A bought number outlives
  // the demo, keeps billing FieldQuo, and is a real line a stranger can text
  // while the account is re-dressed as a different trade next week — that is
  // demoLine.js's reasoning verbatim, and it was already true here. Worse,
  // wipeContent() never touched CrewInboxNumber, so the number survived every
  // reset and nothing in the product would have shown it again.
  //
  // The row is real; only the provider and the money are not:
  //
  //  - `provider: "simulated"` is refused by SMS_CAPABLE_PROVIDERS in
  //    lib/crew/capability.js, which already returns an explicit
  //    "provider_no_sms" verdict. So the number exists and displays, and no
  //    outbound text is ever attempted. That column exists precisely to say
  //    which provider a row belongs to; this is what it is for.
  //  - `source: "demo"` keeps it out of app/api/cron/crew-line-rent, which
  //    queries `source: "dedicated"` — the same way a shared_test loan is
  //    already excluded from billing rather than skipped in the loop.
  //  - No reserveSpend call at all. Credits are FieldQuo's money too; a demo
  //    that burns them is a smaller version of the same bug.
  if (await isDemoCompany(companyId)) {
    const existingDemo = await db.crewInboxNumber.findUnique({ where: { companyId } });
    if (existingDemo) {
      return {
        ok: false,
    ...
    const line = await createSimulatedCrewLine({
      companyId,
      webhookUrl: inboundWebhookUrl(origin),
    });
    return { ok: true, line, chargedCents: 0, simulated: true };
  }
