...
export async function fileHeldMessage({ companyId, messageId, jobId }) {
  ...
  await fileToVisit({ companyId, jobId, jobVisitId: visit.id, mediaUrls: msg.mediaUrls, note: msg.body });
...
/**
 * Append media + a note to a job's visit, AND record each photo as a JobPhoto
 * with the stage the message implies.
 *
...
async function fileToVisit({ companyId, jobId, jobVisitId, mediaUrls, note }) {
  if (!jobVisitId || !Array.isArray(mediaUrls) || !mediaUrls.length) return;
  ...
  if (companyId) {
    ...
    await db.jobPhoto.createMany({
      data: mediaUrls.map((url) => ({
        ...
        url,
        stage,
        caption: note ? String(note).slice(0, 200) : null,
      })),
    });
  }
}
...
export async function handleCrewMessage({ companyId, fromPhone, body, mediaUrls = [], point = null, now = new Date() }) {
  ...
  if (decision.action === "file") {
    ...
    if (decision.resolvedAsk && pending?.row) {
      ...
      await fileToVisit({ companyId, jobId, jobVisitId: visitIdFor(jobId, candidates), mediaUrls: hosted, note: body });
      await db.crewInboundMessage.update({
        where: { id: row.id },
        data: { status: "filed", method: decision.method, jobId, jobVisitId: visitIdFor(jobId, candidates), resolvedAt: now },
      });
