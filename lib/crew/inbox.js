...
async function fileToVisit({ companyId, jobId, jobVisitId, mediaUrls = [], note }) {
  ...
  //
  // ── Company-defined tags do NOT get the same treatment ──────────────────
  //
  // A photo texted in lands with no tags, same as one uploaded through
  // JobPhotoCurator — the owner (or crew, from the job page) applies them
  // afterward. inferStage()'s word list is four fixed, product-critical
  // outcomes tuned and tested against real crew phrasing (see
  // scripts/check-gallery.mjs); a company's tag vocabulary is unbounded and
  // company-specific, so guessing "Sanding" vs. "Priming" from a text like
  // "second coat on" would be a noisier, unreviewed guess with no equivalent
  // safety net — wrong stage mis-files a before/after; a wrong TAG guess
  // would just be wrong and nobody would notice it was ever guessed. Tags stay
  // an explicit, human action everywhere a photo can be tagged.
