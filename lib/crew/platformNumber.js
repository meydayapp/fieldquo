 ...
 */
export const PLATFORM_NUMBER_PURPOSES = ["system", "shared_test", "sales"];
...
function webhookUrlFor(purpose, origin) {
  ...
  return purpose === "sales" ? `${base}/api/sms/inbound` : inboundWebhookUrl(origin);
}
...
export async function buyPlatformNumber({ e164, purpose = "system", origin }) {
  ...
  try {
    bought = await twilioRest.incomingPhoneNumbers.create({
      ...
      smsUrl: webhookUrlFor(purpose, origin),
      smsMethod: "POST",
