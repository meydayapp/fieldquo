...
function webhookUrlFor(purpose, origin) {
  ...
  // The system outbound number takes every homeowner's STOP/START — the same
  // route, for the same reason as a sales number. It was falling through to
  // the crew inbox: the first System number the owner bought (2026-09-19,
  // +1 716 274 7905) arrived pointed at /api/crew/inbound and had to be
  // repointed by hand at Twilio.
  if (purpose === "system") return `${base}/api/sms/inbound`;
  return inboundWebhookUrl(origin);
}

/**
 * The VOICE webhook — the route that answers a contractor ringing back the
 * number a rep called them from. Null for every other purpose, matching
 * PlatformSmsNumber.voiceUrl's own rule: an SMS number carrying a voice URL
 * would answer calls nobody meant it to answer.
 *
 * ══ "sales" carries BOTH webhooks, since 2026-09-06 ═══════════════════════
 *
 * The owner's decision: the sales team has ONE number that calls and texts,
 * and crew texting lives on its own numbers (system / shared_test). Before
 * this, "sales" was text-only and "sales_voice" voice-only, so a contractor
 * who was called from one number and texted from another saw two strangers.
 * Now a Sales number is the caller id, the ring-back number and the texting
 * number at once. "sales_voice" stays as a voice-only purpose for a second
 * pool number in another area code, where texting from it would be the
 * wrong thing (reps text from the team's Sales number, and a STOP has to
 * land where the texts came from).
 */
export const VOICE_PURPOSES = Object.freeze(["sales", "sales_voice"]);

function voiceWebhookUrlFor(purpose, origin) {
  const base = String(origin || "").replace(/\/+$/, "");
  return VOICE_PURPOSES.includes(purpose) ? `${base}/api/rep-dial/inbound` : null;
}

/**
 * The number's STATUS callback — Twilio posting the inbound call's final
 * status whatever TwiML was running when it ended.
 *
 * Without it a caller who hangs up while the rep's browser is ringing leaves
 * an attempt row with every provider column null and no way to tell it from
 * a call still in progress: none of the `action` URLs fire on a caller
 * hang-up. The `?stage=status` branch of the inbound route turns this into
 * the missed-call record and the push (lib/sales/calls/missed.js). Same
 * signed route, so one URL rule covers both.
 *
 * Exported so the number audit can say what a held number SHOULD carry.
 */
export function voiceStatusCallbackUrlFor(purpose, origin) {
  const url = voiceWebhookUrlFor(purpose, origin);
  return url ? `${url}?stage=status` : null;
}
...
export async function buyPlatformNumber({ e164, purpose = "system", origin }) {
  ...
  const statusCallback = voiceStatusCallbackUrlFor(purpose, origin);
  let bought;
  try {
    bought = await twilioRest.incomingPhoneNumbers.create({
      ...
      // The call-status callback, in the same purchase for the same reason
      // as the voice URL: a number that answers calls and cannot say when
      // one ended is a number that loses every missed call.
