...
export function auditCrewLines({
  numbers = [],
  rows = [],
  // ── What FieldQuo holds a number FOR ──────────────────────────────────
  //
  // PlatformSmsNumber rows, keyed by e164. This audit joined Twilio's numbers
  // to TENANT claims and to nothing else, so a number FieldQuo bought for its
  // own sales team had no claim, fell to the last branch, and was labelled
  // "free to lend" — which is the shared TEST line's description and the
  // opposite of the truth about a sales number. The owner bought one, read
  // that under it, and reasonably asked whether it had gone in wrong.
  //
  // Defaulted to an empty array so every existing caller keeps working and
  // simply gets `purpose: null`, which is the honest answer when nobody asked.
  platformNumbers = [],
  ...
  for (const p of Array.isArray(platformNumbers) ? platformNumbers : []) {
    if (p && p.e164) platform.set(p.e164, p);
  }
  ...
  const lines = held.map((n) => ({
    ...describe(n, claims.get(n.e164) || null),
    purpose: platform.get(n.e164)?.purpose || null,
    assignedRepId: platform.get(n.e164)?.assignedRepId || null,
    assignedRepName: platform.get(n.e164)?.assignedRep?.name || null,
    assignedAdminId: platform.get(n.e164)?.assignedAdminId || null,
    // Email: PlatformAdmin has no name column.
    assignedAdminName: platform.get(n.e164)?.assignedAdmin?.email || null,
  }));
  ...
  return {
    ...
    counts: {
      held: held.length,
      claimed: claims.size,
      // "Free to lend" is a CREW-line idea: a number no tenant's crew inbox
      // has claimed. A sales rep's number, the system From and a shared test
      // line are held by FieldQuo for their own purpose and are not lendable
      // — counting them as free said "6 free to lend" over six numbers the
      // reps were dialling from (the owner, 2026-09-19). Those are counted
      // under their purpose instead.
      free: lines.filter((l) => !l.claim && !l.purpose).length,
      sales: lines.filter((l) => l.purpose === "sales" || l.purpose === "sales_voice").length,
      system: lines.filter((l) => l.purpose === "system").length,
      testLine: lines.filter((l) => l.purpose === "shared_test").length,
