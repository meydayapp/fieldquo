...
import { outstandingRecoveryOrZero, recoveryOnCharge } from "@/lib/stripe/connectFeeLedger";
import {
  BANK_DEBIT_MAX_CENTS,
  bankDebitAmountEligible,
  debitCapabilityForCountry,
  bankDebitSessionOptions,
  isBankDebitMethod,
} from "@/lib/stripe/bankDebit";
import { AFFIRM_CAPABILITY, affirmCapabilityWanted, affirmOffered } from "@/lib/stripe/affirm";
 ...
 *
 * `deps.company` is the company row (offerFinancing) — the Affirm request
 * below is opt-in, and the poll passes it; the check script passes a stub.
 *
 * @returns {Promise<object|false>} the UPDATED account Stripe returned when
 *   an update was sent, so the caller reads the new capability statuses
 *   without a second retrieve; false when nothing was missing.
 */
export async function ensureChargeCapabilities(account, deps = {}) {
  ...
  // Bank debit is requested here rather than at creation because the
  // capability is country-specific (acss_debit_payments exists only for a
  // Canadian account, us_bank_account_ach_payments only for a US one) and an
  // account's country is set during Express onboarding, after creation.
  // Requesting the wrong one is a Stripe error; requesting none leaves the
  // contractor card-only. See lib/stripe/bankDebit.js.
  const wanted = { ...CHARGE_CAPABILITIES };
  const debit = debitCapabilityForCountry(account?.country);
  if (debit) wanted[debit] = { requested: true };
  // Affirm is the same mechanism, with one more gate: only once the
  // contractor has switched "Offer pay-over-time" on. Requested for US and
  // CA accounts only — Stripe rejects it elsewhere, and the settings card
  // says Affirm is not available for that country instead. Never
  // un-requested when the toggle goes off: lib/stripe.js pins the session
  // to card unless offerFinancing is on, so a granted capability the
  // contractor has turned off shows nothing. See lib/stripe/affirm.js.
  if (affirmCapabilityWanted({ account, company: deps.company })) {
    wanted[AFFIRM_CAPABILITY] = { requested: true };
  }
  ...
  const updated = await client.accounts.update(account.id, {
    capabilities: Object.fromEntries(missing.map((k) => [k, wanted[k]])),
  });
  return updated || true;
}

/**
 * Stripe refused a Checkout Session that named Affirm on an account our
 * column said was active. The session is retried card-only by the caller —
 * a homeowner in a driveway still gets a pay link — but the refusal is
 * RECORDED (the platform error log, area "stripe_affirm") and the column is
 * flipped to "inactive" so the settings card stops claiming Affirm is on the
 * link. Before this the fallback was a console.warn: the toggle looked on,
 * the client saw card, and nobody was told. The next status poll re-reads
 * Stripe's own answer, so if Stripe still says active the card says active
 * again and the error log holds the refusal for support to read.
 *
 * Never throws: it runs inside the fallback path of a money route.
 * `deps.db` / `deps.recordError` are the seams the check script executes
 * this through.
 */
export async function reportAffirmRefused({ company, invoice, err }, deps = {}) {
  ...
  await log({
    ...
    message: `Stripe refused Affirm on a pay link; the link was created card-only: ${err?.message || err}`,
  ...
  try {
    if (company?.id) {
      await store.company.update({
        where: { id: company.id },
        data: { stripeAffirmStatus: "inactive" },
      });
    }
  } catch (dbErr) {
    console.error("[stripe] could not mark Affirm inactive:", dbErr?.message);
  }
}
...
// which trues EVERY charge up to Stripe's actual fee the same way.
//
// `recoveryCents` is Stripe's Connect account-fee balance the company still
// owes, riding on this charge as an addition to the application fee
// (lib/stripe/connectFeeLedger.js). The two parts are written into the
// intent's metadata so settlement can split them exactly; Stripe metadata
// values are strings.
export function destinationChargeParams({
  ...
  const estimate = processingFeeCents({ amountCents, currency, method });
  const recovery = Math.max(0, Math.trunc(Number(recoveryCents)) || 0);
  return {
    application_fee_amount: estimate + recovery,
    transfer_data: { destination: company.stripeAccountId },
    on_behalf_of: company.stripeAccountId,
    metadata: {
      ...metadata,
      companyId: company.id,
      fq_fee_estimate_cents: String(estimate),
      fq_recovery_cents: String(recovery),
    },
  };
}

/**
 * destinationChargeParams with the company's outstanding Connect account
 * fees (in this charge's currency) added, capped so the whole application
 * fee never exceeds the charge; the remainder carries forward. `deps.ledger`
 * is the seam scripts/check-processing-fee.mjs uses; production reads the
 * real ledger, and a ledger read failing costs nothing but a later recovery.
 */
export async function destinationChargeParamsWithRecovery(args, deps = {}) {
  const read = deps.ledger || outstandingRecoveryOrZero;
  const outstanding = await read({ companyId: args.company?.id, currency: args.currency });
  const estimate = processingFeeCents({
    amountCents: args.amountCents,
    currency: args.currency,
    method: args.method,
  });
  const { recoverCents } = recoveryOnCharge({
    outstandingCents: outstanding,
    processingFeeCents: estimate,
    amountCents: args.amountCents,
  });
  return destinationChargeParams({ ...args, recoveryCents: recoverCents });
}
...
export async function createInvoiceCheckoutSession({
  ...
  amountCents,
  // "card" (the default — with Affirm on top when the company offers
  // financing and the amount qualifies) or a bank-debit method:
  // "acss_debit" (Canada) / "us_bank_account" (US). A bank session is
  // single-method, so its fee — 1% + 40¢ capped at $5 for PAD — is exact at
  // creation and needs no true-down. The caller (the portal pay route) has
  // already checked the company can take that method; this refuses anything
  // it does not know rather than letting Stripe reject it opaquely.
  method = "card",
}, deps = {}) {
  if (method !== "card" && !isBankDebitMethod(method)) {
  ...
  const baseParams = {
    ...
    payment_intent_data: await destinationChargeParamsWithRecovery(
      {
        company,
  ...
  // ── Bank debit: one method, its own fee, Stripe's mandate ─────────────
  //
  // Returned before the Affirm branch on purpose: financing is a card-page
  // add-on, and a bank-debit session that also offered a card would be the
  // two-method session whose fee cannot be known at creation.
  if (isBankDebitMethod(method)) {
    // Stripe caps one PAD session at $3,000.00 CAD (measured — see
    // BANK_DEBIT_MAX_CENTS). Asked for more it answers `amount_too_large`,
    // which reached a homeowner as a bare 500 on 2026-09-19. The portal
    // stops rendering the button above the cap and its pay route refuses
    // first, in the client's language; this is the belt to those braces, so
    // a caller that never looked (a cron, a future route) still gets the
    // refusal in words and a 400, never Stripe's opaque error. `code` is
    // for callers that translate; the sentence is the English fallback.
    if (!bankDebitAmountEligible({ method, amountCents: unit_amount })) {
      const maxCents = BANK_DEBIT_MAX_CENTS[method];
      const err = new Error(
        `Bank debit is available up to ${(maxCents / 100).toLocaleString("en-CA", { style: "currency", currency })} per payment — this one is ${(unit_amount / 100).toLocaleString("en-CA", { style: "currency", currency })}, so it is card only.`,
      );
      err.status = 400;
      err.code = "bank_debit_over_cap";
      err.maxCents = maxCents;
      err.amountCents = unit_amount;
      throw err;
    }
    return stripe.checkout.sessions.create(
      {
        ...
        payment_method_options: bankDebitSessionOptions(method, { client: invoice.client }),
      },
      opts,
    );
  }
  ...
  //
  // Offered on top of card ONLY when the company opted in, Stripe has
  // ACTIVATED the affirm_payments capability on their connected account
  // (Company.stripeAffirmStatus, written from Stripe's answer by the status
  // poll and account.updated — lib/stripe/affirm.js), AND the invoice
  // qualifies: USD/CAD, $50–$30,000. Outside any of that Stripe rejects the
  // whole session, so it is gated here and the link is card-only.
  //
  // The try/fallback stays as a LAST resort — a homeowner must get a pay
  // link even when Stripe's answer changed between the poll and now — but
  // it is no longer silent: the refusal is recorded and the column flipped
  // so the settings card stops saying "active". See reportAffirmRefused.
  if (affirmOffered({ company, amountCents: unit_amount, currency })) {
    try {
      ...
      await reportAffirmRefused({ company, invoice, err }, deps);
    }
  }
