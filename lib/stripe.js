...
import { invoicePaymentConfigurationFor } from "@/lib/stripe/invoicePaymentConfiguration";
...
export async function createInvoiceCheckoutSession({
  ...
  const baseParams = {
    ...
    // The existing provisional collection remains the card policy. The
    // actual method selected by Checkout is recorded at settlement.
  ...
  //
  // Bank debit remains a dedicated single-method session. It returns before
  // the normal Dynamic Payment Methods path so its mandate and fee handling
  // stay unchanged.
  ...
  // Normal invoice Checkout uses Dynamic Payment Methods. The configuration
  // is selected server-side from the contractor preference; there is no
  // provider list and no fallback to an unrestricted/default configuration.
  return stripe.checkout.sessions.create(
    {
      ...baseParams,
      payment_method_configuration: invoicePaymentConfigurationFor(company),
    },
    opts,
  );
}
