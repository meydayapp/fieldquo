...
export function hrefFor({ entityType, entityId }) {
  ...
  switch (entityType) {
    ...
    case "leave":
      return `/app/time-off`;
    // The employee home's own lists. The request id is not in the URL: the
    // screen shows the caller's requests, and a row they may not act on is a
    // row they should not be sent to.
    case "shiftRequest":
      return `/app/me/requests`;
    case "availabilityRequest":
      return `/app/me/availability`;
    case "shoutout":
      return `/app/me`;
    // The rota events open the person's OWN schedule, the forgotten
    // clock-out opens the clock, the manager's copies open the timesheet or
    // the board. Lists, like leave: worth opening even after the shift moved
    // again. The entityId is the shift or entry the row is about and is kept
    // on the event for the record, not for the link.
    case "schedule":
      return `/app/me/schedule`;
    case "clock":
      return `/app/clock`;
    case "timesheets":
      return `/app/settings/team/timesheets`;
    case "scheduler":
      return `/app/scheduler`;
    // The HR file. The worker's rows open their own screens (lists — the
    // document or policy is on the page); the manager's rows open the
    // person's file, where entityId is the worker.
    case "me_documents":
      return `/app/me/documents`;
    case "me_onboarding":
      return `/app/me/onboarding`;
    case "me_policies":
      return `/app/me/policies`;
    case "me_notes":
      return `/app/me/notes`;
    case "hr_worker":
      return `/app/settings/team/people/${entityId}`;
    case "aiEmployeeProposal":
      return `/app/settings/ai-employee#proposals`;
    case "messageThread":
      return `/app/messages?thread=${entityId}`;
    // Supplies. The office's row opens the Requests tab of Purchasing — a
    // list, worth opening after the row moved on; the requester's row opens
    // their own requests on the phone, where the request id would only point
    // at a row they cannot act on.
    case "supplyRequest":
      return `/app/purchasing?tab=requests`;
    case "mySupplyRequest":
      return `/app/me/supplies`;
    case "clientTicket":
      return `/app/tickets/${entityId}`;
    // New seeded services: the entityId is a release tag, not a record, so
    // it is not in the URL. Settings › Services is where each trade's
    // "Add missing services for my trade" lives.
    case "serviceSeeds":
      return `/app/settings/services`;
    default:
      return null;
  }
}
...
export function noteKeysFor({ params }) {
  ...
  // The swap vocabulary (shift.request.*): which kind of request, and how it
  // ended. Closed lists from lib/shiftRequests/state.js — trade, cover, or a
  // claim of an open shift; approved, declined, expired.
  if (["trade", "cover", "claim"].includes(p.kind)) keys.push(`app.notif.swap.${p.kind}`);
  if (["approved", "declined", "expired"].includes(p.outcome)) keys.push(`app.notif.outcome.${p.outcome}`);
  // Why the quote did not reach them, and therefore which screen to open — a
  // wrong address is fixed on the client record, a refused send is fixed in
  // Settings, and an unconfigured deployment is FieldQuo's to fix. Closed
  // vocabulary: lib/email/sendFailure.js's SEND_FAILURE_CAUSES.
  if (["address", "rejected", "unconfigured"].includes(p.cause)) {
    keys.push(`app.notif.cause.${p.cause}`);
  }
  // Which kind of note is waiting for the person's signature. Closed
  // vocabulary: lib/hr/notes.js's NOTE_KINDS, of which only the two that
  // can require an acknowledgement ever reach a row.
  if (p.noteKind === "warning" || p.noteKind === "write_up") keys.push(`app.notif.noteKind.${p.noteKind}`);

  return keys;
}

/** Every param key noteKeysFor can consume, for the check script's audit. */
export const NOTE_PARAMS = ["kind", "settled", "autoApproved", "fromCall", "temperature", "cause", "noteKind", "outcome"];
