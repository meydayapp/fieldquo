...
/** The company's default language — what a User with no stated language
 *  reads in. Best-effort: "en" when it cannot be read. */
async function companyLanguage(prisma, companyId) {
  try {
    const row = await prisma.company.findUnique({ where: { id: companyId }, select: { defaultLanguage: true } });
 ...
 *                                   NAME the recipients: only members whose
 *                                   userId is in this list — AND who satisfy
 *                                   the type's audience — are written a
 *                                   delivery. For the events about one person
 *                                   (a shift, a forgotten clock-out). An
 *                                   empty list delivers to nobody; absent
 *                                   means the whole audience, as before.
...
export async function notifyEvent(
  { companyId, type, entityId = null, params = null, amount = null, currency = null, actorUserId = null, actorName = null, recipientUserIds = undefined },
  deps = {},
) {
  ...
  try {
    ...
    const recipients = await resolveRecipients({ companyId, type, actorUserId, recipientUserIds, deps: { db: prisma } });
    ...
    // ── The push copy ─────────────────────────────────────────────────────
    //
    // The same recipients, the same sentence the feed row renders (in each
    // reader's language), as a Web Push to whichever browsers they turned
    // notifications on in — lib/notify/push.js. Fire-and-forget: the row
    // above is the record; a push service being down must not fail or slow
    // the webhook that produced this event. Money is NOT in the payload,
    // by construction: the sentence keys carry no amount, and the row's
    // `amount` is not passed. `tag` matches the bell's own in-tab notify()
    // for the same delivery, so a person with both sees each event once.
    //
    // Never when the check script's stub db is in play (`deps.db`): the
    // sender is executed by its own check against its own stub.
    if (!deps.db) {
      const href = hrefFor({ entityType: meta.entityType || null, entityId: entityId || null });
      const sentenceParams = safeParams(meta, params) || {};
      void pushToUsers({
        userIds: recipients.map((m) => m.userId).filter(Boolean),
        fallbackLanguage: await companyLanguage(prisma, companyId),
        payload: async (language) => ({
          title: await appSentence(language, `app.notif.type.${type}`, sentenceParams),
          tag: `notif:${event.id}`,
          url: href || "/app",
        }),
      });
