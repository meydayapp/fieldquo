 ...
 * @param {string[]} [p.recipientUserIds] name the recipients (see selectRecipients)
 ...
 */
export async function resolveRecipients({ companyId, type, actorUserId = null, recipientUserIds = undefined, deps = {} }) {
  ...
  return selectRecipients({ members, type, actorUserId, recipientUserIds });
}
 ...
 */
export function selectRecipients({ members, type, actorUserId = null, recipientUserIds = undefined }) {
  const problems = typeProblems(type);
  if (problems.length) return [];
  ...
  // ── Named recipients narrow; they never widen ────────────────────────
  //
  // A shift is about one person and a forgotten clock-out is about one
  // person and their manager. The caller names them, and the audience test
  // below still runs on each: naming somebody the catalog's audience would
  // refuse (a member set to "no access" on the schedule) delivers nothing to
  // them. `undefined` is the old behaviour — the whole audience; an empty
  // array is "nobody", which a call site that computed no recipients means.
  const named =
    recipientUserIds === undefined || recipientUserIds === null
      ? null
      : new Set((Array.isArray(recipientUserIds) ? recipientUserIds : []).filter(Boolean).map(String));

  return list.filter((member) => {
    if (!member || member.active === false) return false;
    if (named && (!member.userId || !named.has(String(member.userId)))) return false;
