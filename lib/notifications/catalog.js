...
export const NOTIFICATION_TYPES = {
  ...
  // ── 7. A quote that did not arrive ──────────────────────────────────────
  //
  // Manny Conto gave 26 doors, 7 drawers, photos, an address and a colour, and
  // typed his email as `Macksab  1@hotmail.com`. The quote bounced. Nothing in
  // the product said so — the contractor worked it out himself, asked twice,
  // and by then the momentum was gone. Basir Mohmand lost two weeks the same
  // way: quote sent 30 May, "I don't have your email" on 24 June, sitting in
  // spam the whole time. Two losses in a corpus of twenty, both to plumbing.
  //
  // AGENTS.md's first rule, applied to an email: a quote that did not arrive
  // must never look the same as one that did. `sentAt` already refuses to be
  // written on a failure, so the QUOTE looks right; what was missing was
  // anybody being TOLD. This is that, through the one notification path rather
  // than a second alerting mechanism.
  //
  // `severity: critical` alongside a chargeback, deliberately. Both are money
  // already in motion that stops unless somebody acts today.
  //
  // No money flag: the row says a document did not land, never what it was
  // worth. The figure is on the quote behind it, under its own showPricing
  // gate.
  //
  // NOT emitted for a bounce — nothing in this deployment learns about one.
  // There is no Resend webhook and no delivery-event store, so every emission
  // is a SYNCHRONOUS refusal at send time. See lib/email/sendFailure.js, which
  // says so in one place rather than letting each call site imply otherwise.
  "quote.undelivered": {
    label: "Quote not delivered",
    severity: "critical",
    money: false,
    audience: { category: "quotes", level: "view_only" },
    // Dispatchers and Managers chase work. A quote nobody received is the most
    // actionable thing on their board.
    supervisors: true,
    entityType: "quote",
    // `cause` is a CLOSED vocabulary of three — see SEND_FAILURE_CAUSES in
    // lib/email/sendFailure.js — which is what lets it be a param at all. The
    // raw provider message is deliberately absent: it is open-ended English
    // written by Resend, and a param carrying it would print a vendor string
    // at a contractor in a language they may not read.
    params: ["quoteNumber", "clientName", "cause"],
  },

  // ── 8–10. The rota reaching the person on it ────────────────────────────
  //
  // Publishing a week flipped `published` and told nobody; the worker found
  // out by opening the app, or on the morning. These three are the Homebase
  // shape: your shifts are out, your shift moved, your shift is gone.
  //
  // ══ One recipient, named by the caller ═════════════════════════════════
  //
  // Every earlier type is an AUDIENCE — "whoever holds payments" — and the
  // resolver fans out to all of them. A shift is about ONE person, and
  // telling the whole crew that Marc is on at eight is noise for everyone
  // and a privacy leak for Marc. So the call site passes `recipientUserIds`
  // (lib/notifications/notify.js) and the resolver INTERSECTS: the audience
  // below still has to be satisfied — a member whose schedule dial says
  // "no access" is not told about a schedule — and only the named people
  // among those who satisfy it are written a delivery. The audience is the
  // floor of the schedule ladder, which every preset holds, so in practice
  // the worker is told and nobody else is.
  //
  // `when` is the one param, composed by lib/shifts/shiftNotify.js in the
  // RECIPIENT's language and the company's zone — "Mon 14 Sep, 8:00–16:00 at
  // Sophie Dubois, 12 rue Principale". A departure from "compose at read
  // time", made on purpose: the row has exactly one reader and the sender
  // knows who, so the sentence can be theirs; and a date formatted at read
  // time in the browser's zone would put a 22:00 shift on the wrong day for
  // a worker whose phone is set to another one. No money, no job total —
  // the site and the client's name are what a crew member on that job
  // already sees on the block.
  "shift.published": {
    label: "Shifts published",
    severity: "normal",
    money: false,
    audience: { category: "schedule", level: "view_own" },
    supervisors: true,
    entityType: "schedule",
    params: ["when"],
  },
  ...
  // ── 11–13. The time clock noticing ───────────────────────────────────────
  //
  // app/api/cron/time-clock-watch, every fifteen minutes. "Still clocked
  // in?" goes to the worker alone (named recipient, same mechanism as the
  // shift types); the manager's copy and the no-show go to the worker's
  // reporting line (lib/org/reportingLine.js) when one is drawn, else to
  // everyone with `user:manage` — the same fallback "leave.requested" uses
  // and for the same reason: most companies never draw an org chart, and a
  // no-show nobody is told about is the failure this exists to remove.
  // Nothing here closes an entry or docks an hour; it is a question and a
  // fact, and the pay decision stays on the timesheet with a person.
  "timeclock.stillClockedIn": {
    label: "Still clocked in?",
    severity: "high",
    money: false,
    audience: { category: "timeTracking", level: "view_record_own" },
    supervisors: true,
    entityType: "clock",
    params: ["since"],
  },
  "timeclock.stillClockedInManager": {
    label: "Somebody is still clocked in",
    severity: "normal",
    money: false,
    audience: { capability: "user:manage" },
    supervisors: true,
    entityType: "timesheets",
    params: ["workerName", "since"],
  },
  "attendance.noShow": {
    label: "No-show",
    severity: "high",
    ...
    params: ["workerName", "when"],
  },

  // ── The HR file: documents, onboarding, policies, notes ─────────────────
  //
  // Named recipients: the call site passes `recipientUserIds` to
  // notifyEvent and the resolver intersects them with the audience. The worker-facing types
  // sit on the floor of the schedule ladder, which every preset holds, so
  // the person named is told and nobody else is; "your licence expires in
  // 7 days" is not an audience, it is a person. The manager-facing types
  // stay on `user:manage`, the authority a person's file sits behind
  // (lib/hr/access.js), so a supervisor who can open the file is told and a
  // Crew member is not.
  //
  // No money, and the params are titles, names and counts — never a licence
  // number and never a note's text. The row says a write-up is waiting for
  // a signature; what it says is on the screen behind the row.
  "hr.document.expiring": {
    ...
    audience: { category: "schedule", level: "view_own" },
    supervisors: true,
    entityType: "me_documents",
    params: ["title", "days"],
  },
  "hr.document.expiringManager": {
    label: "A team member's document is expiring",
    severity: "normal",
    money: false,
    audience: { capability: "user:manage" },
    supervisors: true,
    entityType: "hr_worker",
    params: ["workerName", "title", "days"],
  },
  "hr.onboarding.started": {
    label: "Your onboarding checklist",
    severity: "normal",
    money: false,
    audience: { category: "schedule", level: "view_own" },
    supervisors: true,
    entityType: "me_onboarding",
    params: ["count"],
  },
  "hr.onboarding.overdue": {
    label: "Onboarding items overdue",
    severity: "high",
    money: false,
    audience: { category: "schedule", level: "view_own" },
    supervisors: true,
  ...
  "hr.policy.toAcknowledge": {
    label: "A policy to read and sign",
    severity: "normal",
    money: false,
    audience: { category: "schedule", level: "view_own" },
    supervisors: true,
    entityType: "me_policies",
    params: ["title"],
  },
  "hr.note.toAcknowledge": {
    label: "A note to acknowledge",
    severity: "high",
    money: false,
    audience: { category: "schedule", level: "view_own" },
    supervisors: true,
  ...
  // ── The employee home: swaps, availability, shout-outs ─────────────────
  //
  // Named recipients: the call site passes `recipientUserIds` to
  // notifyEvent and lib/notifications/recipients.js INTERSECTS them with
  // the audience — naming somebody the audience refuses delivers nothing,
  // and naming nobody delivers to nobody. The
  // worker-facing four sit on the floor of the schedule ladder, which every
  // preset holds, so the person named is told and nobody else is. The two
  // manager-facing ones keep the capability audience, for the reason
  // leave.requested gives: the approver is computed on every read
  // (lib/org/reportingLine.js), so the feed tells everyone who could act
  // and the screen says who it is waiting on.
  //
  // `when` is the shift's day and times, composed by the route in the
  // company's language and zone — the same departure from "compose at read
  // time" the shift types make, for the same reason. `kind` (trade | cover)
  // and `outcome` (approved | declined | expired) are closed vocabularies
  // rendered per reader from app.notif.* keys.
  "shift.request.peer": {
    label: "Shift trade or cover asked of you",
    severity: "normal",
    money: false,
    audience: { category: "schedule", level: "view_own" },
    supervisors: true,
    entityType: "shiftRequest",
    params: ["fromName", "kind", "when"],
  },
  "shift.request.manager": {
    label: "Shift swap awaiting approval",
    severity: "normal",
    money: false,
    audience: { capability: "user:manage" },
    supervisors: true,
    entityType: "shiftRequest",
    params: ["fromName", "toName", "kind", "when"],
  },
  ...
  "availability.requested": {
    ...
    params: ["workerName", "effectiveFrom"],
  },
  "availability.decided": {
    label: "Availability change decided",
    severity: "normal",
    money: false,
    audience: { category: "schedule", level: "view_own" },
    supervisors: true,
    entityType: "availabilityRequest",
    params: ["outcome", "effectiveFrom"],
  },
  "shoutout.received": {
    label: "Shout-out",
    severity: "normal",
  ...
  // ── The AI employee: a yes it needs, and a stop it made ─────────────────
  //
  // Both land on the people who can act — the same `user:manage` rung the
  // employee's settings screen requires — because a proposal a crew member
  // reads and cannot approve is a badge that nags the wrong person. `tool`
  // and `channel` are interpolated: "Wants to book an appointment, from the
  // website chat" is the whole sentence somebody needs before opening it.
  "ai_employee.proposal": {
    label: "AI employee needs your approval",
    severity: "high",
    money: false,
    audience: { capability: "user:manage" },
    supervisors: true,
    entityType: "aiEmployeeProposal",
    params: ["tool", "channel"],
  },
  "ai_employee.handoff": {
    label: "AI employee handed a conversation to you",
    severity: "high",
    money: false,
    audience: { capability: "user:manage" },
    supervisors: true,
    entityType: "messageThread",
    params: ["reason", "channel"],
  },

  // ── Supplies: a request from the field, and its two answers ─────────────
  //
  // The request lands on whoever holds purchasing — the same `expenses:
  // view_record_edit_all` rung the Purchasing page and every purchasing route
  // require (lib/purchasing/access.js), so the person told is the person
  // whose Mark ordered button will work. The two answers go to the ONE person
  // who asked: the route passes recipientUserIds = [requester], which narrows
  // the audience to them, and the audience is the floor every preset holds so
  // a crew member is inside it.
  "supply.requested": {
    label: "Supply requested from the field",
    severity: "normal",
    money: false,
    audience: { category: "expenses", level: "view_record_edit_all" },
    supervisors: true,
    entityType: "supplyRequest",
    params: ["fromName", "item", "quantity"],
  },
  "supply.ordered": {
    label: "Your supply request was ordered",
    severity: "normal",
    money: false,
    audience: { category: "schedule", level: "view_own" },
    supervisors: true,
    entityType: "mySupplyRequest",
    params: ["item", "quantity"],
  },
  "supply.restocked": {
    label: "Your supply request is restocked",
    severity: "normal",
    money: false,
    audience: { category: "schedule", level: "view_own" },
    supervisors: true,
    entityType: "mySupplyRequest",
    params: ["item", "quantity", "where"],
  },

  // ── Client tickets (2026-09-24) ────────────────────────────────────────
  //
  // A client raised an issue from the portal, or answered one. Same rung as a
  // new enquiry (requests: view_only) — the people who pick up what clients
  // ask for — and narrowed to the assignee by lib/clientTickets/service.js
  // once somebody has the ticket. The subject is the client's own words; the
  // row opens the ticket.
  "client_ticket.opened": {
    label: "A client reported an issue",
    severity: "normal",
    money: false,
    audience: { category: "requests", level: "view_only" },
    supervisors: true,
    entityType: "clientTicket",
    params: ["clientName", "subject"],
  },
  "client_ticket.replied": {
    label: "A client replied on their ticket",
    severity: "normal",
    money: false,
    audience: { category: "requests", level: "view_only" },
    supervisors: true,
  ...
  // ── New seeded services for the company's trades (2026-09-24) ──────────
  //
  // Raised once per release by scripts/notify-new-service-seeds.mjs, never by
  // a request. Owners and admins only: `user:manage` excludes Crew/Estimator
  // (employee holds no such capability) and `supervisors: false` excludes
  // Manager/Dispatcher — the same two roles "Add missing services for my
  // trade" accepts, so nobody is told to press a button that refuses them.
  // `entityId` is the release tag (lib/services/newSeedNotice.js), the
  // idempotency marker; the row opens Settings › Services whatever it holds.
  "services.new_seeds": {
    label: "New services for your trade",
    severity: "normal",
    money: false,
    audience: { capability: "user:manage" },
    supervisors: false,
    entityType: "serviceSeeds",
    params: ["count", "trades"],
  },
