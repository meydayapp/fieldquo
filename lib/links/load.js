...
async function shape(company) {
  ...
  const candidates = linkCandidates({
    ...
    enabledEstimators: await priceableEstimators(company),
 ...
 *
 * The scope query runs only when painting is the deciding trade. Any other
 * priceable trade already makes the page real, and this runs for a stranger
 * on a phone.
 */
async function priceableEstimators(company) {
  ...
  const scopes = paintingScopesOffered(await companyEnabledCategoryKeys(company.id));
