...
const SELECT = {
  ...
  province: true,
  country: true,
  // The card page (app/c/[slug]) prints the address and hands out a vCard;
  // the bio link ignores both.
  address: true,
  postalCode: true,
  reviewUrl: true,
  defaultLanguage: true,
  // The company's own on/off for the Kitchen Designer, read with the enabled
  // trades by the one rule in lib/kitchen/key.js — the kitchen row below.
 ...
 */
export async function loadLinkPageData(slug, { language = null } = {}) {
  const company = await findBookingCompany(slug, SELECT);
  if (!company) return null;
  return shape(company, { language });
}
...
/**
 * @param language  the label language for the rows. The bio link renders in
 *                  the company's default (the page is theirs); the digital
 *                  business card (app/c/[slug]) passes the VISITOR's, since
 *                  a stranger who scanned a sticker reads the buttons, and
 *                  falls back to the company's own. Labels only — every
 *                  URL, slug and switch is the same either way.
 */
async function shape(company, { language = null } = {}) {
  ...
  const candidates = linkCandidates({
    ...
    // The SAME rule /quote/[slug]/kitchen applies before it renders
    // (companyOffersKitchenDesign in lib/kitchen/access.js calls exactly this
    // function): a kitchen-building trade enabled, unless the company's own
    // override says otherwise. Called pure over keys already read, because
    // this runs for a stranger on a phone. Not a feature flag.
    offersKitchenDesign: kitchenDesignerOnPure(categoryKeys, company.kitchenDesignerOverride),
  });
