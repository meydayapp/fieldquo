 ...
 * value that fails the check falls to the scheme's ink or page colour, and
 * THAT is checked too: the fallback used to be written raw, so a malformed
 * `pageBg` escaped the block and wrote arbitrary rules onto every public bio
 * link (`--lp-page:red;}body{display:none}` closed the selector). The chain
 * now ends at a literal that cannot come from a company, so the sink is safe
 * by construction rather than by the derivation happening to behave.
 *
...
export function linkPageTokenCss(schemes, selector = ".lp", { scheme = "auto" } = {}) {
  const block = (c, scheme) => {
    // The last resort is a literal, never a company value: black on a light
    // page and white on a dark one are both readable, and neither can carry a
    // brace.
    const LAST_RESORT = scheme === "dark" ? "#ffffff" : "#000000";
    const safe = (v, fallback) => {
      if (HEX.test(String(v))) return v;
      return HEX.test(String(fallback)) ? fallback : LAST_RESORT;
    };
