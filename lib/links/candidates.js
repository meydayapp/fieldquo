 ...
 * @param input.offersKitchenDesign  true when the "Kitchen Design & New
 *                             Installs" service is switched on — the exact
 *                             gate /quote/[slug]/kitchen applies, so the row
 *                             exists only when the page it opens does
 */
...
export function linkCandidates(input = {}) {
  ...
  if (slug) {
    ...
    // The public kitchen designer. A row only when the page it opens would
    // render: the page 404s for a company that never switched the service on
    // (lib/kitchen/access.js), and a dead link on a bio page is the one thing
    // this file exists to prevent. Sits under "Get a price" beside the quote
    // form because that is what it is — a way to ask for a number, with a
    // drawing attached.
    if (input.offersKitchenDesign === true) {
      out.push({
        key: "kitchen",
        kind: "internal",
        url: `/quote/${encodeURIComponent(slug)}/kitchen`,
        label: extra.kitchenDesign,
        group: "price",
        defaultOn: true,
      });
    }
