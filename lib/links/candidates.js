...
export function linkCandidates(input = {}) {
  ...
  if (mail) {
    out.push({ key: "email", kind: "contact", url: mail, label: extra.email, group: "contact", defaultOn: false });
  }

  const review = safeUrl(company.reviewUrl);
  if (review) {
    out.push({ key: "review", kind: "external", url: review, label: extra.review, group: "more", defaultOn: false });
  }
