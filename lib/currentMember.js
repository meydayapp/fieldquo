...
async function getImpersonatedMember(request) {
  ...
  if (claims.mode === "demo_sandbox") {
    ...
    // ── The demo companies have no members at all ─────────────────────────
    //
    // Not an edge case — it is every one of them. scripts/seed-demos.mjs
    // deliberately creates no logins, because this codebase has no
    // server-side sign-up path (non-negotiable #1: people arrive by
    // invitation) and adding a second user-creation route would be a hole in
    // the rule. So `demo1@fieldquo.com` is a label on the company, not an
    // account, and there is no password for it anywhere.
    //
    // Falling through to the read-only stub here would have made
    // demo_sandbox mode do nothing at all for the only companies it exists
    // to serve — which is what shipped, and is fixed here.
    //
    // So: owner rights, no user. Quote.createdById and Invoice.createdById
    // are both nullable, which covers the demo path — build a quote, send it,
    // convert it, invoice it. Task.createdById is NOT nullable, and
    // lib/tasks/autoCreate.js already returns null and skips when it can't
    // attribute one, so an auto-task quietly doesn't appear. That is the
    // known limit of a demo run, and it is a better trade than either
    // inventing a user or leaving the demos unusable.
    return {
      id: null,
      userId: null,
      companyId: company.id,
      authOrgId: company.authOrgId,
      // Real owner rights: PERMISSIONS.owner is ["*"], so the granular grid
      // and the coarse check both pass. Safe because the mode is only ever
      // minted for a company whose isDemo is true in the database.
      role: "owner",
      impersonation: true,
      impersonationMode: "demo_sandbox",
      platformAdminId: claims.platformAdminId,
...
/**
 * Not where the paid data-migration service's write path is enforced —
 * deliberately. That is a SEPARATE mechanism (lib/migrations/state.js's
 * canWrite(), checked by each app/api/platform/migrations/[id]/writes/*
 * route) with its own, narrower gate: a superadmin, a paid MigrationRequest,
 * create-only. Impersonation stays absolutely read-only here, with no
 * carve-out — a support session looking at a live account unprompted must
 * never be able to write, however that write is later justified elsewhere.
 * See AGENTS.md non-negotiable #3 for why the two are kept apart.
 */
 ...
 *   the one page that fixes the problem. Two routes use it too, both of which
 *   only record that the caller was here and change nothing the company owns:
 *   POST /api/track (page views) and POST /api/presence (the activity stamp
