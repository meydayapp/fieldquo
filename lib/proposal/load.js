...
import { resolveProposalSections, sanitisePresentation, dayPlan, paintSpec } from "@/lib/proposal/sections";
...
export async function loadProposalContent({ companyId, company, language = "en", presentation = null, db = realDb }) {
  ...
  const content = {
    about,
    gallery: gallery.map((p) => ({ before: p.beforeUrl, after: p.afterUrl, caption: p.caption || "" })),
    documents: visibleDocs.map(presentDocument),
