...
/**
 * The body as the thread DRAWS it: a run of blank lines folded to one.
 *
 * An office's automated text arrived with five empty lines before its own
 * "Reply STOP to unsubscribe." (Advance Appliance, 2026-09-18), and
 * `whitespace-pre-wrap` drew every one of them — a hundred pixels of
 * nothing in a pane that had a hundred pixels. What was said is not
 * changed: the row keeps its bytes, the export and the console read them
 * whole, and a single blank line still separates paragraphs. Only runs of
 * three or more line breaks are folded, so a rep's own "\n\n" before the
 * footer looks exactly as they typed it. Pure.
 */
export function displayBody(text) {
