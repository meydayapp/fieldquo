...
const CATEGORY_SELECT = (companyId) => ({
  ...
  // Tells a service the company created itself from the catalogue's — its
  // name is a drafted phrase, not a labelTranslations entry.
  companyId: true,
  companySettings: {
    where: { companyId },
    select: {
...
export async function loadPrepGuideJob(db, { jobId, companyId }) {
  ...
  // The company's own service names in every drafted language — the guide's
  // language is resolved later, in the pure builder below.
  const categoryPhrases = await loadPhraseTranslations(
    db,
    companyId,
    "categoryLabel",
    customCategoryPhrases(categories).map((e) => e.text),
  );

  return { job, categories, documents, categoryPhrases };
}
...
export function buildPrepGuide(loaded, { cloudName } = {}) {
  const { job, categories, documents, categoryPhrases } = loaded;
  ...
  const trService = phraseMapLookup(categoryPhrases, language);
  ...
  const sections = (categories.length ? categories : [null]).map((category) => {
    ...
    return {
      ...
      label: category ? serviceName(category, language, trService, "en") : "",
      guide,
      steps: content.steps,
    };
  });
