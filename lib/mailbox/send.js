...
// ANY failure — a refused password, a provider rate limit, a timeout, our own
// hourly throttle — the message goes out through the existing sender instead
// and the reason is written on the mailbox row, where the settings card
...
import { hourlySendLimit, loginFor } from "./presets";
import { throttleVerdict } from "./sendThrottle";
...
export async function trySendThroughMailbox(dbArg, { companyId, mail }, deps = {}) {
  ...
  const verdict = throttleVerdict(conn, { now, limit: hourlySendLimit(conn) });
  if (!verdict.ok) return fail("hourly_limit", `This mailbox has sent ${verdict.count} emails this hour; its provider allows about ${verdict.limit}.`);
