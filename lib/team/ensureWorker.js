 ...
 */
export async function ensureWorkerForMember({ companyId, userId, title = null }) {
  if (!companyId || !userId) return { worker: null, created: false, linked: false };
  // `title` is the job title typed on the New User form and carried on the
  // PendingTeamProfile until now (see the schema note on Worker.title). It is
  // only ever FILLED IN — written on a fresh row, or onto a hand-entered row
  // that has none — never used to overwrite a title an admin already set on
  // the Workers screen, which is the more deliberate of the two edits.
  const pendingTitle = typeof title === "string" && title.trim() ? title.trim() : null;
    ...
    if (byEmail) {
      const worker = await db.worker.update({
        ...
        data: {
          userId,
          active: true,
          ...(pendingTitle && !byEmail.title ? { title: pendingTitle } : {}),
        },
      });
...
/**
 * Resolve the Worker row for a quick-add invite: reattach to an existing row
 * matched by email, or create a fresh one. Used by POST /api/team/quick-add,
 * which creates the Worker synchronously (unlike the full New User page,
 * which defers to ensureWorkerForMember above on invite ACCEPTANCE).
 *
 * `existingWorker` is supplied by the caller rather than re-queried here — it
 * is the same company-scoped lookup the route already runs to refuse a
 * genuine duplicate (an ACTIVE match), so by the time this is called it can
 * only ever be null or an INACTIVE row. Reattaching UPDATES that row rather
 * than creating a second one: a second row is a second, empty employment
 * record for someone who already has one, and their real TimeEntry,
 * PayRunLine, Payout and LeaveBalance history stays attached to the row
 * nothing points at any more.
 *
 * @returns { worker, created } — `created` is true only for a brand-new row,
 *   which is what the caller uses to decide whether an invite failure right
 *   afterward should roll the row back (a reactivated row is somebody's real
 *   employment record and must never be deleted just because the email
 *   didn't send).
 */
export async function resolveQuickAddWorker({
  member,
  existingWorker,
  cleanEmail,
  fullName,
  ...
  profile,
  hourlyRate,
  // Already normalised by the route (lib/team/personLabel.js normaliseTitle):
  // a string, or null for "no title given".
  title = null,
}) {
  const type = workerType === "contractor" ? "contractor" : "employee";
  if (existingWorker) {
    const worker = await db.worker.update({
      ...
      data: {
        ...
        // paid. This one is about where their hours cost the business: on a
        // job, or on the business. See lib/team/workProfile.js.
        ...
        hourlyRate,
        // Re-adding somebody is a fresh statement of who they are, so the
        // title on the form wins here — unlike the invite-accept path above,
        // this row is being re-entered by an admin on purpose.
        title,
      },
    });
