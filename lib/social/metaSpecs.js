...
export function nextContainerAction(statusCode, attempt = 0) {
  switch (statusCode) {
    ...
    case "PUBLISHED":
      // Already published — most likely a retried request against a
      // container a previous call already finished. Treat as done, not an
      // error, so a flaky network retry can't produce a duplicate post.
      return { action: "publish" };
    case "IN_PROGRESS":
