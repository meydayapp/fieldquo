...
//
// Refunds: a refund FieldQuo issued is its own Payment row (kind "refund", a
// negative amount — lib/invoices/refund.js) and is listed as its own line with
// method "refund", totalled under Refunds. There is still no credit-note
// object. A negative amount on a PAYMENT row cannot arise from the app (POST
// /api/payments refuses a non-positive amount, and refuses an overpayment
// rather than banking a credit) — if one is ever passed in, it is emitted
// verbatim and warned about, never quietly summed away.
//
...
const LIMITATIONS = [
  ...
  "Processing fees are shown only on payments taken through FieldQuo after fees began to be recorded on the payment. Older card payments and every manual payment leave the fee columns blank.",
...
export function buildAccountingExport({
  ...
  const bucket = (code) => {
    if (!totals.has(code)) {
      totals.set(code, { invoiced: 0, tax: 0, paid: 0, refunded: 0, fees: 0, accountFees: 0, expensed: 0 });
    }
  ...
  for (const family of families) {
    ...
    // One rule for every reader — lib/invoices/issueDate.js — which also dates
    // a past job entered after the fact by its real dates.
    const issue = invoiceIssueDate(dateSource);
    const issued = dayKey(issue.date);
    let issuedFrom =
      {
        sentAt: "sentAt (emailed)",
        createdAt: "createdAt (raised)",
        endDate: "endDate (past job, work ended)",
        paidDate: "paidDate (past job, paid)",
      }[issue.from] || "unknown";
    if (!issued) {
      issuedFrom = "unknown";
      warn(
  ...
  //
  // "Amount" is GROSS — what the client paid and what the invoice balance was
  // reduced by. "Processing fee" and "Net deposited" are what Stripe deducted
  // and what actually reached the bank, read from the Payment row
  // (lib/stripe/paymentIntentFee.js fills them at settlement). They are
  // columns on the payment's own line rather than a separate fee row so the
  // payments file still has one line per payment; the bookkeeper posts the
  // gross to income and the fee to a merchant-fees expense from the same
  // line, and the bank feed matches "Net deposited". Blank — not 0.00 — for
  // a manual payment or a Stripe payment recorded before fees were passed
  // through: an empty cell says "no fee is known", a zero says "no fee".
  const paymentHeader = [
    "Date",
    "Invoice number",
    "Client",
  ...
  for (const p of payments) {
    ...
    // A refund FieldQuo issued is its own negative line (Payment.kind
    // "refund" — lib/invoices/refund.js), listed with method "refund" and
    // totalled in the summary's Refunds column. Only a negative amount on a
    // PAYMENT row is the anomaly the warning below describes.
    const isRefund = p?.kind === "refund";
    if (amount < 0 && !isRefund) {
      warn(
        "negative_payment",
        `A payment of ${amount} appears on ${key}. FieldQuo refuses non-positive payments and records a refund as its own "refund" line, so this did not come from the app. It is listed as-is and NOT netted off the total.`,
        { invoiceId: p?.invoiceId ?? null },
      );
    }

    const code = currencyOf(p);
    if (isRefund) bucket(code).refunded += -amount;
    else bucket(code).paid += amount;

    const feeKnown = p?.processingFeeCents != null && Number.isFinite(Number(p.processingFeeCents));
    const fee = feeKnown ? Number(p.processingFeeCents) / 100 : null;
    const net = feeKnown && p?.netCents != null ? Number(p.netCents) / 100 : null;
    if (feeKnown) bucket(code).fees += fee;
    // Stripe's Connect account fees recovered on this payment — its own
    // column and its own summary total, never inside the processing fee.
    const accountFee =
      p?.accountFeeRecoveredCents != null && Number(p.accountFeeRecoveredCents) > 0
        ? Number(p.accountFeeRecoveredCents) / 100
        : null;
    if (accountFee != null) bucket(code).accountFees += accountFee;
    ...
    paymentRows.push([
      ...
      fee == null ? "" : money(fee),
      net == null ? "" : money(net),
      feeKnown ? p?.feeRateLabel ?? "" : "",
      accountFee == null ? "" : money(accountFee),
      isRefund ? p?.stripeRefundId ?? "" : p?.stripePaymentIntentId ?? "",
      isRefund ? p?.refundReason ?? p?.notes ?? "" : p?.notes ?? "",
    ]);
  }
  ...
  for (const code of currencyCodes) {
    ...
    summaryRows.push([
      ...
      money(t.paid),
      money(t.refunded),
      // The fees Stripe deducted from the payments above, summed only over
      // rows that carry one. Gross "Payments received" is left as is: the
      // fee is a separate expense, not a smaller income.
      money(t.fees),
      money(t.accountFees),
      money(t.expensed),
    ]);
