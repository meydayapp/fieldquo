...
/**
 * The `payment` argument below, built from a paid Checkout Session — shared by
 * all three callers so the fee columns are filled on every path and not only
 * the webhook's. `readFee` is the seam scripts/check-booking-fee.mjs uses to
 * keep a real Stripe lookup out of a check.
 */
export async function bookingPaymentFromSession(session, { readFee = settledFeeOrNull } = {}) {
  ...
  return {
    ...
    fee: paymentIntentId ? await readFee(paymentIntentId) : null,
  };
}

/**
 * @param {object} payment  What Stripe says was actually taken:
 *   { amountCents, currency, paymentIntentId, checkoutSessionId, fee }
 *   where `fee` is `{ processingFeeCents, netCents, feeRateLabel }` from
 *   lib/stripe/paymentIntentFee.js, or null when it could not be read —
 *   in which case the fee columns stay null (absence, not zero).
...
export async function settleBookingFee(bookingId, payment = {}, deps = {}) {
  ...
  const appointment = await prisma.appointment.create({
    data: {
      ...
      // The CLIENT's address, and nothing else. Never eventType.location —
      // that was the free-text "Phone or on-site visit" label, not a
      // destination, and the confirm route stopped writing it for the same
      // reason. Null for a call or a video call; the mode is on the Booking
      // row and every screen names it from lib/booking/bookingModes.js.
      location: held.address || null,
      ...(held.latitude != null &&
        held.longitude != null && {
      ...
      status: "scheduled",
      // What the homeowner answered and typed — "When needed: Today", the
      // trade's question, their own words — sits on the held Booking row
      // (see the confirm route). Appointment.notes is the row the crew reads;
      // the free path writes both, and this path silently wrote neither, so
      // every PAID booking reached the calendar with the answers stripped.
      notes: held.notes || null,
      createdById: eventType.userId,
      assignedToId: eventType.userId,
      // The estimate the hold was booked against — verified by the confirm
      // route before it was written to the Booking row, and copied onto the
      // Appointment here for the same reason the free path does it: the
      // quote page's site-visit panel reads Appointment.quoteId, not Booking.
      ...(held.quoteId && { quoteId: held.quoteId }),
    },
  });
  ...
  const claimed = await prisma.booking.updateMany({
    ...
    data: {
      ...
      ...(payment.fee && Number.isFinite(Number(payment.fee.processingFeeCents))
        ? {
            feeProcessingCents: Math.trunc(Number(payment.fee.processingFeeCents)),
            feeNetCents: Number.isFinite(Number(payment.fee.netCents))
              ? Math.trunc(Number(payment.fee.netCents))
              : null,
            feeRateLabel: payment.fee.feeRateLabel || null,
            feeEstimatedCents: Number.isFinite(Number(payment.fee.estimatedFeeCents))
              ? Math.trunc(Number(payment.fee.estimatedFeeCents))
              : null,
            feeStripeCents:
              payment.fee.stripeFeeCents != null && Number.isFinite(Number(payment.fee.stripeFeeCents))
                ? Math.trunc(Number(payment.fee.stripeFeeCents))
                : null,
            feeAccountRecoveredCents: payment.fee.accountFeeRecoveredCents
              ? Math.trunc(Number(payment.fee.accountFeeRecoveredCents))
              : null,
          }
        : {}),
    },
