...
// ══ Eight languages, because the client-facing surfaces are ═══════════════
//
...
// must not change wording as a side effect of adding languages. de and it
// joined when the booking letters themselves went through the catalogue
// (lib/i18n/emailCopy.js `visit`), which carries both.
//
// `en` stays en-US rather than documentFormatters' en-CA on purpose: this
// locale also formats the exact time on the same letters (see letterLocale),
// and "2:00 PM" is what every confirmation has said. Changing it to
// "2:00 p.m." would be a wording change nobody asked for.
const LOCALES = {
  en: "en-US",
  fr: "fr-CA",
  es: "es-419",
...
const BETWEEN = {
  ...
  de: (day, from, to) => `${day}, zwischen ${from} und ${to}`,
  it: (day, from, to) => `${day}, tra le ${from} e le ${to}`,
};

/**
 * The locale a booking letter formats its dates in, for a language.
 *
 * Exported so the exact-time line and the window line on the same letter
 * are spelled by one map: a confirmation saying "2:00 PM" next to a
 * cancellation saying "2:00 p.m." is the drift this avoids.
 */
export function letterLocale(language = "en") {
