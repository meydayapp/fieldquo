...
export async function finalizeBooking({ company, eventType, booking, clientId }) {
  ...
  //   1. Company.bookingSmsConfirmation — the switch beside the template on
  //      Settings → Messages. Off by default: a text is billable.
  ...
  const texted = await textConfirmation({ company, eventType, booking, language, manageUrl, clientId });
...
export async function bookingTextVerdict({ company, booking }) {
  if (!company?.bookingSmsConfirmation) return { send: false, reason: "switched_off", to: null };
 ...
 */
async function textConfirmation({ company, eventType, booking, language, clientId = null }) {
  try {
