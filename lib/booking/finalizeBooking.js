...
export async function finalizeBooking({ company, eventType, booking, clientId }) {
  ...
  // The appointment type's name in THAT language — the draft stored when the
  // company saved the name (lib/i18n/phrases.js), else the name as typed.
  // Once, here, so the letter and the text cannot name it differently; never
  // translated now, at send time (non-negotiable 6).
  const tr = await loadPhrases(db, company.id, language, [{ ns: "eventTypeName", text: eventType?.name }]);
  const typeName = tr("eventTypeName", eventType?.name || "");
  ...
  const texted = await textConfirmation({ company, typeName, booking, language, manageUrl, clientId });
 ...
 */
async function textConfirmation({ company, typeName, booking, language, clientId = null }) {
  try {
    ...
    const result = await sendSms({
      ...
      body: renderMessage({
        ...
        values: {
          ...
          // Already in the client's language — see finalizeBooking.
          service: typeName || "",
      ...
      companyId: company.id,
      // The delivery receipt's labels (model SmsDelivery). The APPOINTMENT when
      // the booking became one — that is the row the calendar draws, so it is
      // where "Confirmation text: not delivered" has to appear.
      purpose: "booking_confirmation",
      ref: booking.appointmentId
        ? { type: "appointment", id: booking.appointmentId }
