...
export async function loadVisitByToken(token) {
  ...
  // The appointment type's name in every language it has a draft in
  // (lib/i18n/phrases.js), loaded with the visit because the page's language
  // is decided later, in visitView — and every route (the page, cancel, move,
  // the calendar file) goes through here, so none can forget it. A failed
  // read is {}: the name as typed, never a broken page.
  const eventTypeNames =
    (await loadPhraseTranslations(db, booking.eventType.company.id, "eventTypeName", [booking.eventType.name]))[
 ...
 */
export function visitView({ booking, eventType, company, eventTypeNames = null }, now = new Date()) {
  ...
  return {
    ...
    // In the page's language: the draft stored when the company saved the
    // name (loadVisitByToken brings them), else the name as they typed it.
    // The cancel and move routes take the letter's and the text's name from
    // here too, so all three say the same words.
    eventTypeName: (typeof eventTypeNames?.[language] === "string" && eventTypeNames[language]) || eventType.name,
