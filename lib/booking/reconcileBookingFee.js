...
export async function reconcileBookingFee(booking, opts = {}) {
  ...
  // The fee lookup is a Stripe call of its own; a check that fakes the session
  // must be able to fake this too rather than reach a real client.
  const readFee = opts.readFee || undefined;
  ...
  if (session && session.payment_status === "paid") {
    const result = await settle(
      booking.id,
      await bookingPaymentFromSession(session, readFee ? { readFee } : {}),
    );
    return {
