...
import { recoverDispute, returnWonDispute } from "@/lib/stripe/disputeRecovery";
...
export async function settleChargeEvent(event, { prisma = db, deps = {} } = {}) {
  ...
  if (connect?.recorded) {
    // A refund or a lost dispute takes back the commission earned on the
    // money (lib/commissions/sync.js). After the recorder's own write, with
    // the real client — never inside anyone's transaction — and it never
    // throws, so it cannot make Stripe redeliver.
    if (connect.invoiceUpdated) await syncCommissionsForInvoice(prisma, connect.invoiceId);

    // ── Who pays for the chargeback ──────────────────────────────────────
    //
    // Stripe has just debited the PLATFORM for the disputed amount and the
    // dispute fee. Pull both back from the contractor's transfer now, and
    // send the held amount back if the dispute is later won — see
    // lib/stripe/disputeRecovery.js. Awaited and allowed to throw, unlike
    // the notice below: a failed reversal must make Stripe redeliver the
    // event so it is retried (both legs are idempotent on the dispute id),
    // because a chargeback FieldQuo quietly absorbed is the exact money
    // leak this codebase exists to prevent.
    let recovery = null;
    if (isDispute && connect.payment) {
      const dispute = event.data.object;
      if (event.type === "charge.dispute.created") {
        recovery = await recoverDispute(
          { payment: connect.payment, dispute },
          { db: prisma, ...(deps.stripe ? { stripe: deps.stripe } : {}) },
        );
      } else if (event.type === "charge.dispute.closed" && dispute?.status === "won") {
        recovery = await returnWonDispute(
          { payment: connect.payment, dispute },
          { db: prisma, ...(deps.stripe ? { stripe: deps.stripe } : {}) },
        );
      }
    ...
    return {
      handled: true,
      kind: isRefund ? "refund" : "dispute",
      result: recovery ? { ...connect, recovery } : connect,
    };
  }
