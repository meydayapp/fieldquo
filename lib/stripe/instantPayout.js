...
export async function createInstantPayout({ company, userId, requestId }, deps = {}) {
  ...
  // The platform switch, checked here as well as in the route: a payout is
  // money leaving a balance, and a route is not the only way to call this.
  const enabled = deps.enabled ?? INSTANT_PAYOUTS_ENABLED;
  if (!enabled) return { ok: false, reason: "disabled", status: 404 };
