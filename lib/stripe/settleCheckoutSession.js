...
import { actualPaymentMethod, readActualPaymentMethod } from "@/lib/stripe/actualPaymentMethod";
...
export async function settleCheckoutSession(session) {
  ...
  if (invoiceId) {
    ...
    const selectedPaymentMethod = fee?.feeRateLabel ||
      await readActualPaymentMethod(session.payment_intent, stripe);
...
export async function markPendingPayment(prisma, { invoiceId, session, paymentIntentId }) {
  ...
  await prisma.invoice.update({
    where: { id: inv.id },
    data: {
      pendingPaymentMethod: await readActualPaymentMethod(session?.payment_intent, stripe),
...
export async function failCheckoutSession(session, deps = {}) {
  ...
  let reason = null;
  let selectedPaymentMethod = actualPaymentMethod(session?.payment_intent);
  try {
    const client = deps.stripe || stripe;
    const intent = await client.paymentIntents.retrieve(paymentIntentId, {
      expand: ["latest_charge", "payment_method"],
    });
    selectedPaymentMethod = actualPaymentMethod(intent) || selectedPaymentMethod;
