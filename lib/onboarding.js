...
export async function getOnboardingStatus(companyId) {
  ...
  // `minutes: [low, high]` — how long the step takes, shown small beside it
  // on the dashboard card ("1–3 min"). Counted from what the form the row
  // opens actually asks for to satisfy `done` (fields, decisions, uploads,
  // hand-offs to another site), not guessed; the count is on each line. The
  // high end is the contractor who has to go and FIND something — a logo
  // file, a bank transit number — because that, not typing, is the time.
  const steps = [
    {
      key: "logo",
      ...
      // BrandingForm: one upload, an optional colour pick, Save. Most of the
      // time is finding the image file.
      minutes: [1, 3],
    },
    {
      key: "business_info",
      ...
      href: "/app/settings",
      // CompanyDetailsFields: phone + one address pick (autocomplete fills
      // city, province, postal code and country), Save.
      minutes: [1, 3],
    },
    {
      key: "services",
      ...
      // ServicesEditor: tick one trade, Save.
      minutes: [1, 2],
    },
    {
      key: "pricing",
      ...
      href: "/app/settings/services",
      // One rate and its unit, Save — a number the contractor already knows.
      // Often already done: a trade with a price book counts as priced.
      minutes: [1, 3],
    },
    {
      key: "payments",
      ...
      // One click here, then Stripe's own onboarding: roughly 15–20 fields —
      // legal identity, date of birth, business number, bank transit and
      // account numbers, sometimes an ID photo. The widest range on the list
      // because it is the one that needs papers looked up.
      minutes: [8, 20],
    },
  ];
  ...
  if (taxRegDone || !taxRegDismissed) {
    steps.push({
      ...
      // TaxRegistrationFields: the number (plus an optional name for it), or
      // the "not registered" box, Save. The number may need looking up.
      minutes: [1, 3],
  ...
  // No "skipped" count here, on purpose: nothing on this list can be skipped.
  // The tax row's "I don't have one" takes it OFF the list (above) — it is
  // neither done nor skipped, it does not apply — so the card's "n of total
  // done" is simply doneCount over steps.length.
  ...
  // step that uses them.
  return { steps, percent, complete, doneCount, total: steps.length };
}
