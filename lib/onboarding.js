...
export async function getOnboardingStatus(companyId) {
  ...
  if (!worksAlone) {
    steps.push({
      key: "team",
      // Two keys rather than one with an optional suffix: the licence count is
      // a parenthetical in English and not necessarily one elsewhere, so the
      // translation owns the whole sentence. `labelValues` fills {used}/{max}.
      labelKey: seatLimit
        ? "app.onboarding.step.teamSeats"
        : "app.onboarding.step.team",
      labelValues: seatLimit ? { used: seatsUsed, max: seatLimit } : undefined,
      label: seatLimit
