...
export const OWNED_ID_FIELDS = {
  ...
  // The job a callback job points back at (Job.originalJobId) — same model,
  // same rule as jobId above; a separate entry because a route proves it as
  // its own named field, not because the check differs.
  originalJobId: { model: "job", where: (id, companyId) => ({ id, companyId }) },
  ...
  // Job.warrantyEquipmentId: a foreign row would put another company's
  // customer's furnace — name, serial, warranty — on this company's job page.
  // The same-client rule sits on top of this in lib/equipment/installed.js.
  clientEquipmentId: { model: "clientEquipment", where: (id, companyId) => ({ id, companyId }) },
  ...
  // JobSubcontractor.subcontractorId and SubcontractorPayment.subcontractorId:
  // a foreign sub would put ANOTHER company's electrician on this company's
  // job, and the payment recorded against it would land on that company's
  // T5018 total. jobSubcontractorId is the assignment a payment is booked
  // to — its own key, not merged onto subcontractorId, because a payment
  // route writes both and one must not prove the other.
  subcontractorId: { model: "subcontractor", where: (id, companyId) => ({ id, companyId }) },
  jobSubcontractorId: { model: "jobSubcontractor", where: (id, companyId) => ({ id, companyId }) },
