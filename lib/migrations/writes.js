  ...
  return db.$transaction(async (tx) => {
    ...
    const quote = await tx.quote.create({
      data: {
        ...
        clientId,
        quoteNumber,
        // FieldQuo created this row on the company's behalf, under the one
        // sanctioned write exception (AGENTS.md non-negotiable 3). Recorded so
        // that a company looking at "where do our quotes come from" is not
        // told a migrated record was one of theirs.
        createdVia: requireCreatedVia("migration"),
        status: "draft",
        subtotal: total,
        total,
