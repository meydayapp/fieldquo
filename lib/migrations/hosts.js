...
export async function loadMigrationHosts(now = new Date()) {
  ...
  const [windows, demoBookings, migrationBookings] = await Promise.all([
    ...
    db.demoBooking.findMany({
      ...
      // `source` says how long the booking is: a sales walkthrough is an
      // hour and blocks two slots (bookingSpan); everything else is one.
      select: { hostAdminId: true, scheduledAt: true, source: true },
    }).then((rows) => rows.map(bookingSpan)),
