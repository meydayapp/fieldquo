...
export function redactClient(member, client) {
  ...
  // The notes dial is a SECOND filter on the same row, applied whichever way
  // the clientsProperties check went: a member allowed the full record is not
  // thereby allowed the private notes on it. See redactNotesField.
  if (hasLevel(member, "clientsProperties", "full_view"))
    return redactNotesField(member, client);
...
export function redactLead(member, lead) {
  ...
  // Same second filter as redactClient: the call-back log rides along on the
  // lead detail as `notes`, and it is the notes dial, not the client dial,
  // that says who may read it.
  if (hasLevel(member, "clientsProperties", "full_view"))
    return redactNotesField(member, lead);
  ...
  out.restricted = true;
  return redactNotesField(member, out);
}
...
// ── Notes ─────────────────────────────────────────────────────────────────
//
// The dial that was written and read by nothing. lib/permissions.js carried
// it in the editor and every preset for as long as the grid has existed, and
// its header said plainly that it gated no note anywhere — an owner could set
// a hire to "View notes on jobs and visits only", see it saved, and that hire
// read and wrote the call-back log on every lead and the private notes on
// every client exactly like a Manager.
//
// ── What a "note" is, for this dial ────────────────────────────────────────
//
// The product has no single Note model. `notes` is a column on a dozen rows,
// and most of those are not notes in the sense the four labels mean — a quote's
// `notes` is the paragraph the HOMEOWNER reads under the line items, an
// expense's `notes` is what the receipt was for. Those belong to the document
// or record they sit on and its own dial already answers who may read and
// write them. Putting them under this dial would break the presets that
// already exist: an Estimator sits at notes:view_all and must still be able to
// write the terms paragraph on a quote they are allowed to create.
//
// What the labels describe is the INTERNAL, people-facing record — what the
// office knows about a household that the household never sees:
//
//   * LeadNote — the call-back log on a request ("left a voicemail, trying
//     Tue"). Read on GET /api/leads/[id] (as `notes`) and
//     GET /api/leads/[id]/notes; written on POST /api/leads/[id]/notes;
//     removed on DELETE /api/leads/[id]/notes/[noteId].
//   * Client.notes — the private notes on a client ("gate code 4417, dog is
//     friendly, difficult about scheduling"). Read wherever a client row is
//     returned; written on POST /api/clients and PATCH /api/clients/[id].
//
// The floor, "View notes on jobs and visits only", is a JobVisit's own
// `notes` — the note the crew leave when they mark a visit done. Every level
// keeps that, and keeps writing it on their own visit, because the person on
// the ladder is the one who has to be able to say what happened on it. The
// visit routes gate it on jobs and on the visit's assignee and nothing here
// narrows that further.
//
// The rungs above the floor:
//
//   view_all              may READ lead notes and client notes
//   view_edit_all         may also WRITE them (add a lead note, set a
//                         client's notes)
//   view_edit_delete_all  may also REMOVE a lead note
//
// A client's notes are one text field, so "delete" there is clearing it,
// which is an edit. The delete rung is real on the lead log, where a note is a
// row of its own and removing one is a distinct act.
//
// Fall-open and fall-closed follow hasLevel exactly, for the reason it gives:
// owners, admins, a member with no grid and a grid that never mentions notes
// are all unrestricted.

/** May this member read notes on leads and clients (not only on visits)? */
export function canSeeAllNotes(member) {
  return hasLevel(member, "notes", "view_all");
}

/** May this member add a lead note or set a client's private notes? */
export function canEditNotes(member) {
  return hasLevel(member, "notes", "view_edit_all");
}

/** May this member remove a lead note? */
export function canDeleteNotes(member) {
  return hasLevel(member, "notes", "view_edit_delete_all");
}

/**
 * Strip the `notes` field off a client or lead row unless the member may read
 * notes beyond their own visits.
 *
 * Marks the row `notesRestricted` when it did so, for the same reason
 * redactClient marks `restricted`: a page has to be able to say "hidden by
 * your access level" rather than render a client with no notes, which reads
 * as a client nobody has written anything about. Only set when there WAS a
 * notes field to remove — a row that never carried one has nothing to
 * announce.
 *
 * Returns a NEW object when it changes anything; never mutates the input.
 */
export function redactNotesField(member, row) {
  if (!row || typeof row !== "object") return row;
  if (canSeeAllNotes(member)) return row;
  if (!("notes" in row)) return row;
  const out = { ...row };
  delete out.notes;
  out.notesRestricted = true;
  return out;
}
...
const INVOICE_MONEY_FIELDS = [
  ...
  "amountPaid",
  "amountDue",
  // What Send would ask for (lib/invoices/sendAsk.js): a stage amount and
  // the collected figure are the total in another shape.
  "sendAsk",
];
