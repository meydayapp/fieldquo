...
export const NAV_REQUIREMENTS = {
  ...
  // The inbox. An inbound Page or WhatsApp message IS a request — a stranger
  // asking whether the company does this kind of work — which is why
  // GET /api/messaging/threads refuses at requests:view_only, matching
  // GET /api/leads. The row had no rule, so a Crew member (requests: none)
  // saw Messages, opened it, and met the refusal behind it.
  "app.nav.messages": { category: "requests", level: "view_only" },

  // Client tickets — what clients raised from their portal. The same dial
  // and the same rung as the inbox: GET /api/client-tickets refuses below
  // requests:view_only (lib/clientTickets/access.js), and the notification a
  // new ticket raises goes to that audience (lib/notifications/catalog.js).
  // The row had no rule, so a Crew member saw it and opened a refusal.
  "app.nav.clientTickets": { category: "requests", level: "view_only" },
  ...
  // The subs — the COMPANIES this contractor hires per job, not the people
  // on the roster. Same axis and same three roles as the vans, matching what
  // GET /api/subcontractors requires (`user:manage`, via
  // lib/subcontractors/access.js): a sub's name and whether their insurance
  // is in date are operations. What the sub is OWED is a second, stricter
  // gate inside the payload (jobCosting), so a supervisor sees the roster
  // without seeing the money. scripts/check-subcontractors.mjs executes this
  // row and that route's gate against every preset and fails if they part.
  "app.nav.subcontractors": { role: ["owner", "admin", "supervisor"] },
  // The manager's log book: the roles that hold `user:manage`, which is
  // what /api/hr/log checks (lib/hr/access.js).
  "app.nav.log": { role: ["owner", "admin", "supervisor"] },
  ...
  // The AI team (/app/settings/ai-employee) has a rail row since 2026-09-21,
  // the way Your team always did. Same answer its settings capability gives:
  // SETTINGS_ROW_CAPABILITY says user:manage, which the owner, admin and
  // supervisor seats hold (lib/permissions.js) — so the rail row and the
  // settings row never disagree about who gets in.
  "app.nav.aiTeam": { role: ["owner", "admin", "supervisor"] },
  ...
  // The influencer's commission ledger and payout details: money about the
  // company, so the same two roles as Refer & Earn and Plan. The row is also
  // absent entirely for a company that is not enrolled (AdminSidebar).
  "app.nav.influencer": { role: ["owner", "admin"] },
  ...
  // POST /api/receipts requires expenses "view_record_edit_own" — the lowest
  // rung, the one Crew holds. A crew member snapping their own receipt is the
  // case this entry exists for.
  "app.quickAdd.receipt": { category: "expenses", level: "view_record_edit_own" },
};
