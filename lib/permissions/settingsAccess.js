...
export const SETTINGS_SIDEBAR_CHROME_KEYS = [
  "app.settings.title",
  "app.settings.search",
  // The slide panel's "← Back to menu" and the phone strip's "All settings"
  // (2026-09-21): chrome of the one rail's settings list, not rows.
  "app.settings.backToMenu",
  "app.settings.allSettings",
];
...
export const SETTINGS_ROW_CAPABILITY = {
  ...
  // FieldQuo's paid data-migration service: sets a Stripe Checkout session
  // in motion for the company's card. GET /api/migrations and every mutating
  // route beside it refuse with isBillingAdmin(member.role) — see
  // app/api/migrations/route.js.
  "app.settings.migration": "billing",
  ...
  // Switching commissions on and choosing their basis: PATCH
  // /api/commissions/settings refuses anyone but an owner or admin
  // (lib/commissions/access.js#canConfigureCommissions), the same set
  // isPayrollAdmin names — and the team's rates on the same page are pay.
  "app.settings.commissions": "payroll",
  ...
  // GET/PUT/POST/DELETE /api/settings/website, and the languages/ and photos/
  // routes beside it → isWebsiteAdmin(member.role) in lib/site/access.js,
  // owner or admin. Was "user:manage" while the parent route asked for that
  // and the two sub-routes asked for owner-admin: a supervisor saw the row,
  // could save and publish, and got a 403 adding a language or a photo.
  // owner-admin because that is what all three routes now say.
  "app.settings.website": "owner-admin",
  ...
  // GET /api/ai-employee, and the sources/test/suggestions routes beside it,
  // all requirePermission(member.role, "user:manage"). Same rung as the phone
  // receptionist directly above and for the same two reasons: it decides what
  // gets said to customers in the company's name, and every reply it writes
  // spends the company's AI allowance.
  "app.settings.aiEmployee": "user:manage",
  ...
  // Company policies and who has signed them: the HR file's gate
  // (lib/hr/access.js), the same `user:manage` Manage Team sits behind.
  "app.settings.policies": "user:manage",
  ...
  // Offline caching, the labour sell rate and the performance-pay rule:
  // app/api/settings/field-work refuses anyone but an owner or admin.
  "app.settings.fieldWork": "owner-admin",
  ...
  // Their own calendar feed link. GET/POST /api/calendar/feed read and
  // rotate the CALLER's Member.calendarFeedToken and nothing else — there is
  // no id to name somebody else's — and the feed behind the link is scoped
  // by the caller's own grid through loadScheduleFeed. A crew member's phone
  // showing a crew member's day is the whole point.
  "app.settings.myCalendar": "everyone",
  // Their own work mailbox. GET /api/mailbox answers every member with the
  // caller's own connections (all of the company's for an owner/admin), and
  // every connect route lets a member connect THEIR OWN mailbox; the company
  // mailbox and its sending switch refuse anyone but an owner or admin.
  "app.settings.workEmail": "everyone",
  ...
  "app.settings.planTemplates": "everyone",
  ...
  // POST /api/settings/job-photo-tags and PATCH /api/settings/job-photo-tags/[id]
  // → the routes' own requireManage, ["owner", "admin", "supervisor"] — same
  // set as user:manage, same shape as checklists just above. The GET stays
  // open on purpose and is NOT gated by this: JobPhotoCurator.js and
  // JobPhotoTimeline.js read it to tag/filter a job's own photos, and that is
  // exactly the screen a crew member (or anyone with jobs access) works from.
  "app.settings.jobPhotoTags": "user:manage",
...
export const SETTINGS_ROW_REQUIREMENTS = {
  ...
  // GET /api/settings/plan-templates → requireToggle(full, "showPricing").
  // A maintenance plan is a per-visit price; same audience as the price book.
  "app.settings.planTemplates": { toggle: "showPricing" },
...
export const SETTINGS_ROWS_VISIBLE_WITH_PARTIAL_ACCESS = {
  ...
  // GET /api/settings/plan-templates → requireToggle(full, "showPricing"),
  // the price book's own rule and this row's grid rule.
  "app.settings.planTemplates":
