...
import { parseMentions, parseParticipantKey, participantKey } from "./mentions";
import { directPushRecipient, directPushSnippet } from "./directPush";
import { unreadWhere, mentionsWhere, countByRoomArgs, countsByRoom } from "@/lib/chat/unreadQuery";
import { appSentence, pushToPlatformAdmins, pushToReps } from "@/lib/notify/push";

/** The membership rows every room read carries: titles and read state. */
...
/**
 * Loaded with every room in the LIST: the members, and the last thing
 * somebody SAID — one row, for the preview line. Not the last 200: the
 * unread and mention counts come from a count query (unreadByRoom below),
 * because a slice of the oldest rows stops containing the newest ones the
 * day a room outgrows it — lib/chat/unreadQuery.js has the whole story.
 */
const LIST_INCLUDE = {
...
/** The viewer's own membership row in a room, from the loaded members. */
function membershipOf(room, viewer) {
  return (room?.members || []).find((m) => m.open !== false && sameParticipant(participantOf(m), viewer)) || null;
}

/** The where fragment that leaves the viewer's own messages out of a count. */
function notMineWhere(viewer) {
  const column = viewer?.kind === "user" ? "authorPlatformAdminId" : "authorSalesRepId";
  return { OR: [{ [column]: null }, { [column]: { not: viewer.id } }] };
}

/**
 * Unread and mention counts for these rooms, as the viewer, by two grouped
 * counts. Attached to each room as `unread` and `mentions`, which is what
 * lib/staff/rooms.js roomListRow reads.
 */
async function withUnread(client, viewer, rooms) {
  if (!rooms.length) return rooms;
  const seen = rooms.map((r) => ({ id: r.id, lastSeenAt: membershipOf(r, viewer)?.lastSeenAt || null }));
  const notMine = notMineWhere(viewer);
  const unreadArgs = unreadWhere({ rooms: seen, at: "sentAt", notMine });
  const mentionArgs = mentionsWhere({ rooms: seen, at: "sentAt", notMine, key: participantKey(viewer) });
  const [unread, mentions] = await Promise.all([
    unreadArgs ? client.staffMessage.groupBy(countByRoomArgs(unreadArgs)) : [],
    mentionArgs ? client.staffMessage.groupBy(countByRoomArgs(mentionArgs)) : [],
  ]);
...
/** Every room this viewer is an OPEN member of, each with its unread counts. */
export async function roomsFor(viewer, { client = db } = {}) {
  const rooms = await client.staffRoom.findMany({
    where: { members: { some: { ...memberWhere(viewer), open: true } } },
    include: LIST_INCLUDE,
    orderBy: { lastMessageAt: "desc" },
  });
  return withUnread(client, viewer, rooms);
}
 ...
 */
export async function roomFor(viewer, roomId, { client = db } = {}) {
  if (!roomId) return null;
  const room = await client.staffRoom.findFirst({
    where: { id: roomId, members: { some: { ...memberWhere(viewer), open: true } } },
    include: {
      members: MEMBERS_INCLUDE,
      messages: { orderBy: { sentAt: "desc" }, take: THREAD_TAKE, select: MESSAGE_SELECT },
    },
  });
  if (!room) return null;
...
export async function postStaffMessage({ viewer, roomId, body }) {
  ...
  const [message] = await db.$transaction([
    ...
    // their own room showing unread until they reload. Their own words never
    // count, so the stamp only matters for what OTHERS said before it — and
    // anything that lands after it is genuinely unseen. Typing is looking,
    // so the room counts as open for the push rule too.
    db.staffRoomMember.update({ where: { id: member.id }, data: { lastSeenAt: new Date(), lastOpenedAt: new Date() } }),
  ]);
  // The push copy. A direct room pushes EVERY message to the other side
  // (lib/staff/directPush.js says why, and when not); a channel pushes only
  // an @mention, to the people named, never the whole room, and never the
  // author. Fire-and-forget after the transaction has committed
  // (lib/notify/push.js); the row is the record. Reps get it in their
  // language; platform staff in English.
  pushAfterPost({ viewer, roomId, text, mentions });
  return message;
}

/**
 * Decide which push a posted message earns, and send it.
 *
 * One room read serves both branches. A direct room takes the direct push
 * and NOT the mention push as well — the other side is the only person a
 * mention could name, and two notifications for one message is the thing
 * that gets notifications turned off.
 */
function pushAfterPost({ viewer, roomId, text, mentions }) {
  try {
    void db.staffRoom
      .findUnique({
        where: { id: roomId },
        select: {
          name: true,
          kind: true,
          members: { where: { open: true }, select: { platformAdminId: true, salesRepId: true, lastOpenedAt: true, open: true } },
        },
      })
      .then((room) => {
        if (!room) return;
        if (room.kind === "direct") return pushDirect({ viewer, roomId, room, text });
        if (mentions.length) return pushMentions({ viewer, roomId, room, text, mentions });
        return undefined;
      })
      .catch(() => null);
  } catch {
...
/** Send one payload to a set of participants, each portal's deep link its own. */
function pushToPeople({ repIds, adminIds, payloadFor, roomId }) {
  if (repIds.length) void pushToReps({ salesRepIds: repIds, payload: payloadFor(`/sales/team?room=${encodeURIComponent(roomId)}`) });
  if (adminIds.length) {
    void pushToPlatformAdmins({
      platformAdminIds: adminIds,
      payload: async () => payloadFor(`/platform/chat?room=${encodeURIComponent(roomId)}`)("en"),
    });
  }
}

function pushMentions({ viewer, roomId, room, text, mentions }) {
  ...
  const payloadFor = (url) => async (language) => ({
    title: await appSentence(language, "app.notify.mention.title", { name: author, room: roomName }),
    body: snippet,
    tag: `staff-mention:${roomId}`,
    url,
  });
  pushToPeople({ repIds, adminIds, payloadFor, roomId });
}

/**
 * "New message from Daniel: …" to the other side of a direct room — unless
 * they are looking at it (lastOpenedAt within the last half minute; see
 * lib/staff/directPush.js). Same tag as a mention in this room, so a person
 * with both the page and the push open sees the message announced once.
 */
function pushDirect({ viewer, roomId, room, text }) {
  const recipient = directPushRecipient({ room, members: room.members, viewer });
  if (!recipient) return;
  const author = viewer.name || viewer.email || "";
  const snippet = directPushSnippet(text);
  const payloadFor = (url) => async (language) => ({
    title: await appSentence(language, "app.notify.directMessage.title", { name: author }),
    body: snippet,
    tag: `staff-mention:${roomId}`,
    url,
  });
  pushToPeople({
    repIds: recipient.kind === "rep" ? [recipient.id] : [],
    adminIds: recipient.kind === "user" ? [recipient.id] : [],
    payloadFor,
    roomId,
  });
}
 ...
 * does nothing for a non-member.
 *
 * `upTo` is the sentAt of the last message in the payload the route is
 ...
 *
 * `lastOpenedAt` is the other fact this write records — that the room was
 * drawn for them just now — and it is stamped on every read whether or not
 * anything new was shown. lib/staff/directPush.js reads it to skip the push
 * for words already on the reader's screen. The two used to be one column,
 * and could not be once lastSeenAt stopped meaning "now".
 */
export async function markRoomSeen({ viewer, roomId, upTo = null, now = new Date() }, { client = db } = {}) {
  if (!viewer || !roomId) return false;
  const member = await client.staffRoomMember.findFirst({
    where: { roomId, ...memberWhere(viewer), open: true },
    select: { id: true },
  });
  if (!member) return false;
  const seen = upTo ? new Date(upTo) : null;
  const writes = [client.staffRoomMember.update({ where: { id: member.id }, data: { lastOpenedAt: now } })];
  if (seen && !Number.isNaN(seen.getTime())) {
    writes.push(
      client.staffRoomMember.updateMany({
        where: { id: member.id, OR: [{ lastSeenAt: null }, { lastSeenAt: { lt: seen } }] },
