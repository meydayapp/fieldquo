...
//
// StaffRoomMember.lastSeenAt is written every time the room is read
// (lib/staff/store.js markRoomSeen, on every GET of the thread — the open,
// and the 15-second poll while it stays open). A reader whose lastSeenAt is
// younger than DIRECT_PUSH_LOOKING_MS has the conversation on screen, and a
// push for words already in front of them is noise. Thirty seconds covers
 ...
 * @param members  the room's OPEN members: `{ platformAdminId, salesRepId, lastSeenAt }`
...
export function directPushRecipient({ room, members = [], viewer, now = new Date(), lookingMs = DIRECT_PUSH_LOOKING_MS } = {}) {
  ...
  const others = (Array.isArray(members) ? members : [])
    ...
    .map((m) => ({ participant: participantOf(m), lastSeenAt: m.lastSeenAt || null }))
  ...
  const [other] = others;
  if (other.lastSeenAt) {
    const seen = new Date(other.lastSeenAt).getTime();
    if (!Number.isNaN(seen) && now.getTime() - seen < lookingMs) return null;
  }
