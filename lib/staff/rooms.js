...
//
// The cost is a count per room instead of a column read. The store does it
// as two grouped counts across the viewer's rooms (lib/chat/unreadQuery.js)
// and attaches the numbers to each room; roomListRow reads them. It counted
// over the rows loaded for the preview once, and that was the bug the query
// exists to close: the preview slice was the OLDEST 200 rows, so a room that
// outgrew it would have stopped counting its newest messages — the only
// ones that can be unread. unreadFor below is still the rule, over rows, and
// the check requires the query to agree with it.
//
...
export function roomListRow(room, viewer) {
  ...
  return {
    ...
    lastWho: last
      ...
      : null,
    // The store's grouped counts when it attached them (lib/staff/store.js
    // roomsFor — the messages on a listed room are ONE preview row, and a
    // count over that would be wrong); otherwise the rule over the rows
    // given, for a caller that handed in the whole thread.
    unread: Number.isFinite(room?.unread) ? room.unread : unreadFor({ messages, lastSeenAt, viewer }),
    ...
    mentions: Number.isFinite(room?.mentions) ? room.mentions : mentionsFor({ messages, lastSeenAt, viewer }),
  };
}
