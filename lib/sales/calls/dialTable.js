...
//      calls only — never ringing time, never the rep's browser leg. The
//      MEAN talk time is over real conversations only: a mean over
//      "answered" calls averages in every voicemail greeting, and that is
//      the number the floor board printed until 2026-09-21.
//
// ══ 2026-09-21, later the same day: the floor board reads this too ════════
//
// The performance page was rebuilt on these rules and the floor board was
// not, so the owner saw, for one rep on one day, "Calls from +1 716 638 3616
// — their line: 13" (a line, when his ninety-odd calls went out from two
// other reps' lines), "Answered (carrier) 64.3%" (a greeting counted),
// "Conversation (transcript) 8 of 9" (transcript coverage, read as a rate)
// and a mean talk time over the nine "answered". Every one of those is a
// second definition of something this file already defined. So
// reporting.js's repCallStats now carries dialTableRow() as `table`, and
// the floor draws THAT — the same object the performance page draws — and
// this file grew the three things a day view needs that a period table did
// not: the lines a rep's calls went out from (a prospect rings back the
// number they saw), the count of real conversations the TRANSCRIPT decided
// beside how many were transcribed at all, and the mean over conversations.
//
...
import { callbackState, isOutbound, when } from "./attemptRows";
...
export function dialTableRow(attempts = [], { now = new Date() } = {}) {
  ...
  for (const row of joined) {
    ...
    const talk = Number.isFinite(row.talkSeconds) && row.talkSeconds > 0 ? row.talkSeconds : 0;
    if (talk > 0) {
      answered += 1;
      talkSeconds += talk;
    }
    ...
    // The carrier's answering-machine verdict decided it (lib/sales/calls/amd.js):
    // voicemail whatever the transcript or the clock says.
    if (c?.basis === "amd") fromAmd += 1;
    if (c?.talked) {
      buckets.realConversation += 1;
      conversationSeconds += talk;
      // The transcript's own verdict — the prospect's track carried twenty
      // or more words (conversation.js). This is the only count a screen
      // may label "(transcript)": `fromTranscript` above is how many calls
      // the transcript DECIDED, talked or not, and printing that as a rate
      // is the "8 of 9" the floor board used to show.
      if (c.basis === "transcript") realFromTranscript += 1;
    } else buckets.voicemailOrBrief += 1;
  }
  const talked = buckets.realConversation;

  // The lines the calls went out from, by attempt — so a floor board can say
  // "from 2 lines" and name them, since a prospect rings back the number
  // they saw. Attribution is never made the other way round: these are the
  // lines under THIS rep's attempts, not the attempts under a line.
  const byLine = new Map();
  let noLine = 0;
  for (const row of placed) {
    const e164 = typeof row.fromE164 === "string" && row.fromE164 ? row.fromE164 : null;
    if (!e164) {
      noLine += 1;
      continue;
    }
    const cur = byLine.get(e164) || { e164, calls: 0, rules: new Set() };
    cur.calls += 1;
    if (row.callerIdRule) cur.rules.add(row.callerIdRule);
    byLine.set(e164, cur);
  }
  const lines = [...byLine.values()]
    .map((l) => ({ e164: l.e164, calls: l.calls, rules: [...l.rules].sort() }))
    .sort((x, y) => y.calls - x.calls || x.e164.localeCompare(y.e164));
  ...
  return {
    ...
    /** Picked-up dials the transcript decided (talked or not) — how many were transcribed. Never a rate's numerator. */
    conversationFromTranscript: fromTranscript,
    conversationFromClock: joined.length - fromTranscript - fromAmd,
    /** Picked-up dials the carrier's machine verdict decided — voicemail, whatever else the row says. */
    conversationFromAmd: fromAmd,
    /** Real conversations by the transcript's own verdict — the only figure a screen may call "Real conversation (transcript)". */
    realConversationFromTranscript: realFromTranscript,
    /** The lines the placed calls presented, most used first, and how many presented none (handset dials). */
    lines: { rows: lines, count: lines.length, noLine },
    reached: reachedRate,
    ...
    /** Prospect-leg seconds on the real conversations only, and their mean. Null — not zero — with no conversation to average. */
    conversationTalkSeconds: conversationSeconds,
    meanConversationSeconds: talked > 0 ? Math.round(conversationSeconds / talked) : null,
