...
export function normaliseEvent(event) {
  ...
  return {
    ...
    // Carried through since 2026-09-21: an open row is measured to its last
    // keepalive plus the Off window (agentState.js rowPeriodEnd), and a
    // normaliser that dropped the beat would measure it to "now" — the
    // 76-hour write-up the owner saw.
