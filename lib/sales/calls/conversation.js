...
// dialTable.js's `conversationFromTranscript` / `conversationFromClock` say
// how many of each went into the figure, so a reader knows which rule most
// of it obeyed.
//
...
// pickupFigures() — the four bands with "picked up" and "conversation" rates
// over them — lived here from 2026-09-20 to 2026-09-21. lib/sales/calls/
// dialTable.js's dialTableRow() is the same partition with the owner's
// words on it, joined to the carrier by the prospect leg's sid, and it is
// what every screen prints now; a second function producing the same four
// counts under other names is exactly the "second definition" the floor
// board was caught carrying. The bands and the two thresholds stay above,
// because dialTableRow() is built on them.
