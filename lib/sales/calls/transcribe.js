 ...
 */
export async function transcribeAttempt(attemptId, { force = false, fetchImpl = fetch, client = db, score = scoreAttempt } = {}) {
  ...
  if (typeof score === "function") {
    qa = await score(attempt.id, { client }).catch((err) => ({ ok: false, reason: "threw", message: err?.message }));
  }
