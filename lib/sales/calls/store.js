...
export async function saveDisposition({
  ...
  const existing = await client.salesCallAttempt.findFirst({
    ...
    select: { id: true, prospectId: true, leadId: true, toE164: true, disposition: true, dispositionAutoLogged: true },
  });
  if (!existing) {
    ...
    if (existing.prospectId) {
      ...
      if (pool) {
        retry = retryWriteFor({
          ...
          prospect: overriding ? { ...pool, attemptCount: Math.max(0, Number(pool.attemptCount || 0) - 1) } : pool,
