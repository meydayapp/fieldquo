...
import { conferenceRecordingAttrs } from "./recording";
  ...
  dial.conference(
    {
      ...
      // The caller is the first into the room, and Twilio takes a
      // conference's recording setting from the first participant. One
      // mixed track — a conference cannot be recorded per leg — filed by
      // /api/rep-dial/recording against the transfer's attempt.
      ...conferenceRecordingAttrs({ origin, transferId: transfer.id }),
    },
  ...
  dial.conference(
    {
      ...
      statusCallbackMethod: "POST",
      statusCallbackEvent: "join leave",
      // Same setting on every leg so the room records whichever joins first.
      ...conferenceRecordingAttrs({ origin, transferId }),
    },
    conferenceName,
  );
  return twiml;
 ...
 *                  A `number` target's `value` was read off the allow-list
 *                  in lib/sales/transferNumbers.js by the route that built
 *                  the list; the status callback below is set on the leg
 *                  regardless of its kind, so a phone that rings out reaches
 *                  onTargetEnded exactly as a browser that does. One thing a
 *                  phone does that a browser cannot: its own voicemail
 *                  answers. That counts as a pick-up here — there is no
 *                  machine detection on this leg, on purpose (it is billed
 *                  per call and was not approved) — and the picker says so.
  ...
  for (const action of Array.isArray(actions) ? actions : []) {
    try {
      if (action.type === "hold" || action.type === "unhold") {
        ...
        // `parked=1` only on a queue TRANSFER: the caller chose nothing and
        // the rep chose the queue, so the first line is "please hold", not
        // "everyone is on another call" (lib/sales/calls/queue.js). A rescue
        // — the rep vanished mid-transfer — keeps the plain wording.
        const query = new URLSearchParams({ stage: "queue", round: "0", attemptId: transfer.attemptId || "" });
        if (action.parked) query.set("parked", "1");
        await twilioRest.calls(sid).update({
          url: `${origin}/api/rep-dial/inbound?${query.toString()}`,
          method: "POST",
        });
      } else {
