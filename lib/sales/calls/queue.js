 ...
 * a surprise.
 */
function holdSay(round, maxRounds, repName, parked = false) {
  const who = typeof repName === "string" && repName.trim() ? repName.trim() : null;
  const last = round >= maxRounds - 1;

  if (round === 0) {
    if (parked) {
      // A rep put them here on purpose. "Everyone is on another call" would
      // be a lie about the person they were just speaking to; what is true
      // is that the next free person is being found for them.
      return [
        "Please hold while I find the next free person for you.",
 ...
 * @param parked       did a rep put this caller here on purpose — a transfer
 *                     to the queue (lib/sales/calls/transfer.js, kind
 *                     "queue")? Changes the FIRST round's words only: the
 *                     bound, the hold and the voicemail are identical, because
 *                     a parked caller is owed exactly the same promises.
...
export function queueStep({
  ...
  return {
    ...
    say: justRang && at === 0 ? requeueSay({ repName }) : holdSay(at, cap, repName, parked),
    playUrl: music,
...
export function requeueSay({ repName = null } = {}) {
  // `repName` is the rep who was RUNG and did not answer — the route passes
  // it only when that rep was on the plan. Naming a rep the plan skipped
  // ("Daniel did not pick up" about a rep nobody rang) is the lie the owner
  // heard on 2026-09-17; the line without a name is what a skipped rep gets.
  const who = typeof repName === "string" && repName.trim() ? repName.trim() : null;
  return [
    who ? `${who} isn't picking up right now.` : "Nobody picked up.",
    "One moment — I'll try someone else on the team.",
  ];
}
