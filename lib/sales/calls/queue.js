...
export function queueStep({
  ...
  // A round that cannot be read is the FIRST round, not an excuse to loop.
  // Twilio echoes our own query string back, but a caller who edits one — or a
  // redelivery that loses it — must land somewhere bounded.
  const n = Number(round);
  const at = Number.isFinite(n) && n > 0 ? Math.floor(n) : 0;
