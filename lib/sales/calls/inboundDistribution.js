...
// target — the number's owner and the available sweep — is kept only if
// their id is in `frenchRepIds`. The transfer number is a real phone, not a
// rep, and is kept. If that leaves nobody, the plan is empty and the route
// falls through to the hold queue and voicemail exactly as it does when
// nobody is signed in; never a silent drop. The say-lines are
// language-neutral already and are left alone.
//
// ══ …except the rep who just spoke to them (2026-09-17) ═══════════════════
//
// The language rule exists so an anglophone rep is not handed a Quebec
// STRANGER. The rep who rang this contractor minutes ago is not a stranger
// to them — whatever language that conversation was in, it happened, and
// the contractor is ringing back to continue it. The owner tested it with
// his own 819 number: he dialled himself as an English-only rep, rang the
// number back twenty seconds later, and the plan skipped him for having no
// French, rang the number's offline owner instead, and then told him
// "Daniel did not pick up" — a rep nobody had rung. So the last caller is
// pushed past the language rule; the owner and the sweep still obey it.
//
...
/**
 * How long after a rep's OWN dial to this caller their browser is still rung
 * for the ring-back, whatever their presence row says.
 *
 * ── Why presence is not enough for the last caller ────────────────────────
 *
 * On 2026-09-17 a rep dialled a contractor from the console, spoke for a
 * minute, hung up, and the contractor rang straight back — thirteen seconds
 * later. Her presence row said nothing at all: she had dialled from the
 * browser without ever pressing "Available", so livePresence() printed her as
 * offline and never seen, and `reachable()` skipped her. The ONE rep who was
 * demonstrably at a registered Device — the outbound leg had a CallSid from
 * it two minutes earlier — was the one the plan would not ring.
 *
 * A dial from a browser is stronger evidence of a browser than a button is.
 * So the last caller is rung when their dial was recent, and presence is
 * consulted only when it was not. Thirty minutes is the length of a
 * conversation-and-callback, not a shift: beyond it the button rules again.
 */
export const RECENT_CALLER_MINUTES = 30;
...
/** A presence row that has been heard from inside the stale window. */
function heardFrom(presence, now) {
  if (!presence || typeof presence !== "object") return false;
  if (!presence.everSeen || presence.stale === true) return false;
  const seen = presence.lastSeenAt ? new Date(presence.lastSeenAt) : null;
  if (!seen || Number.isNaN(seen.getTime())) return false;
  return now.getTime() - seen.getTime() <= PRESENCE_STALE_MINUTES * 60 * 1000;
}

/** Was this dial inside RECENT_CALLER_MINUTES of now? Unparseable is "no". */
export function recentDial(at, now = new Date()) {
  const t = at ? new Date(at) : null;
  if (!t || Number.isNaN(t.getTime())) return false;
  const age = now.getTime() - t.getTime();
  return age >= 0 && age <= RECENT_CALLER_MINUTES * 60 * 1000;
}

export const LAST_CALLER_RING = "ring";
export const LAST_CALLER_HOLD = "hold";
export const LAST_CALLER_SKIP = "skip";

/**
 * What to do about the rep who dialled this caller last.
 *
 * ── The owner's rule, 2026-09-17 ──────────────────────────────────────────
 *
 * A contractor ringing back the rep who just rang them reaches THAT rep
 * first when she is `available` OR `after_call` — after_call is "just hung
 * up, writing the outcome", which is exactly when a callback lands (the
 * real one came thirteen seconds after the rep hung up). When she is
 * `on_call` with somebody else the caller is HELD for her, not sent to
 * voicemail: the queue re-reads presence every round and rings her the
 * moment the other call ends, and she is told they are waiting. Paused,
 * offline and stale are excluded — a rep at lunch is not rung because she
 * dialled somebody before it.
 *
 * ── Plus the evidence of the dial itself ──────────────────────────────────
 *
 * The rep in the real case had NO presence row at all: she dialled from the
 * console without pressing "Available", so livePresence printed her as
 * offline-never-seen, and every rule above would skip her. Her outbound leg
 * two minutes earlier carried a CallSid from a registered Device, which is
 * better evidence of a browser than any button. So a dial inside
 * RECENT_CALLER_MINUTES rings her regardless of presence — UNLESS she
 * declared paused or offline AFTER that dial, which is a newer statement
 * than the dial and wins. `since` on the presence row is when the current
 * state began, which is what makes that comparison possible.
 */
export function lastCallerVerdict(presence, { lastCalledAt = null, now = new Date() } = {}) {
  const p = presence && typeof presence === "object" ? presence : null;
  const fresh = heardFrom(p, now);
  if (fresh && (p.state === STATE_AVAILABLE || p.state === STATE_AFTER_CALL)) return LAST_CALLER_RING;
  if (fresh && p.state === STATE_ON_CALL) return LAST_CALLER_HOLD;
  if (recentDial(lastCalledAt, now)) {
    const declaredAway = p && p.everSeen && (p.state === STATE_OFFLINE || p.state === STATE_PAUSED);
    const since = declaredAway && p.since ? new Date(p.since) : null;
    const afterDial = since && !Number.isNaN(since.getTime()) && since.getTime() > new Date(lastCalledAt).getTime();
    if (!afterDial) return LAST_CALLER_RING;
  }
  return LAST_CALLER_SKIP;
}
 ...
 * @param lastCalledAt   when that dial was. Within RECENT_CALLER_MINUTES the
 *                       rep is rung regardless of presence — see the constant.
...
export function ringPlan({
  ...
  const pushRep = (salesRepId, why, { pastLanguageRule = false } = {}) => {
    if (!salesRepId || seen.has(salesRepId)) return;
    // The rule sits in the one function every step goes through, so no
    // step — owner, sweep — can forget it. Only the last caller passes it,
    // and says so at the call site (see the header).
    if (!pastLanguageRule && !mayRing(salesRepId)) return;
  ...
  //    thing to an owner there is, and it costs no column. Rung when
  //    available or writing up, HELD for when on another call, and rung on
  //    the strength of a recent dial when presence knows nothing — see
  //    lastCallerVerdict().
  let holdFor = null;
  if (lastCalledBy) {
    const verdict = lastCallerVerdict(byId.get(lastCalledBy), { lastCalledAt, now });
    if (verdict === LAST_CALLER_RING) {
      pushRep(lastCalledBy, recentDial(lastCalledAt, now) ? "they rang this contractor minutes ago" : "they rang this contractor last", { pastLanguageRule: true });
    } else if (verdict === LAST_CALLER_HOLD && !seen.has(lastCalledBy)) {
      holdFor = { salesRepId: lastCalledBy, why: "they rang this contractor last and are on another call" };
    }
  }
  ...
  return {
    targets: targets.slice(0, MAX_RING_TARGETS),
    ringSeconds: RING_SECONDS,
    reason,
    needsFrench: Boolean(needsFrench),
    // The rep the caller should be HELD for when nobody else can be rung:
    // the route pushes "X is ringing back" to them and the queue re-plans
    // every round, so the hold ends the moment their call does. Null when
    // there is nobody in that position.
    holdFor,
  };
}
