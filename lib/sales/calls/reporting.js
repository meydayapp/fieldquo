...
import { connectFigures, wasConnected } from "./conversation";
import {
  PAUSE_REASONS,
  PAUSE_REASON_ORDER,
  activityTotals,
...
export function measuredDurations(attempts, { from = null, to = null } = {}) {
  ...
  let bridged = 0;
  let answered = 0;
  // Calls the carrier reported as picked up (`answeredAt`), whether or not
  // any billed seconds followed. Its own count beside `measuredOf`: an
  // answered call with zero talk seconds is a pick-up-and-hang-up, and
  // folding it into either bucket would misstate both.
  let connected = 0;
  let talkSeconds = 0;
  let holdSeconds = 0;
  let holdOf = 0;
  ...
  for (const row of attempts) {
    ...
    if (when(row?.answeredAt)) connected += 1;
  ...
  return {
    total,
    bridged,
    /** Calls the carrier reported answered. The performance page's "connected". */
    connected,
    /**
     * The carrier's answer rate over the calls it could see (bridged ones):
     * a measurement, labelled as the carrier's — NOT_TRACKED_CALLS.connectRate
     * says why it is never called the rep's connect rate.
     */
    carrierAnswerRate: rate(connected, bridged),
