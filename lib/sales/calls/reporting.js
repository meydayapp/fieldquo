...
/**
 * Supervised calls in a set of attempts: how many, by kind, and the seconds
 * a supervisor spent on them. Zero is a real zero — "nobody listened" — so
 * it is not nulled; `seconds` is null when nothing was measured.
 */
export function supervisionFigures(attempts) {
  if (!Array.isArray(attempts)) return null;
  const byKind = { listen: 0, whisper: 0, barge: 0, take: 0 };
  let calls = 0;
  let seconds = 0;
  let measured = 0;
  for (const row of attempts) {
    const kind = typeof row?.supervisionKind === "string" ? row.supervisionKind : null;
    if (!kind || !(kind in byKind)) continue;
    calls += 1;
    byKind[kind] += 1;
    if (Number.isFinite(row.supervisionSeconds)) {
      measured += 1;
      seconds += row.supervisionSeconds;
    }
  }
  return { calls, byKind, seconds: measured > 0 ? seconds : null, measuredOf: measured };
}
...
export function repCallStats({
  ...
  const inWindow = Array.isArray(attempts)
    ? onlyProspectDials(attempts).filter((row) => !(a || b) || inRange(row?.dialledAt, a, b))
    : null;
  ...
  return {
    ...
    /**
     * How many of this rep's calls a supervisor was on, by what they did,
     * and the seconds — from SalesCallAttempt.supervisionKind /
     * supervisionSeconds (lib/sales/calls/supervision.js). A "take" is
     * counted as a take even though the supervisor barged first: the row
     * keeps the last kind, which is the one that mattered.
     */
    supervised: inWindow ? supervisionFigures(inWindow) : null,
    reportedReachRate,
...
export function teamCallRows({
  ...
  for (const row of onlyProspectDials(attempts)) {
