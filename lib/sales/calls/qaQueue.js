...
import { isSampledOut } from "./sampling";
...
const ATTEMPT_SELECT = {
  ...
  salesRepId: true,
  salesRep: { select: { id: true, name: true, workName: true, language: true } },
...
export function queueRow(attempt) {
  ...
  return {
    ...
    // "not_sampled": the platform's sample left this call out
    // (lib/sales/calls/sampling.js). Its own state rather than a failure,
    // so the queue can say so and the platform can transcribe or score it
    // on demand — and never retries it by itself.
    state: !attempt.transcribedAt
      ? isSampledOut(attempt.transcriptError)
        ? "not_sampled"
...
export async function callQaDetail({ attemptId, repIds = null, client = db } = {}) {
  // The platform (repIds null) also reads the rep's sub-reason, the
  // carrier's machine verdict, the supervisor's audit and the recording
  // marks (docs/SALES-OUTCOMES.md). An agency's read stops at the
  // scorecard: the audit is the owner's verdict on the owner's rep.
  const platform = repIds === null || repIds === undefined;
    ...
    select: {
      ...
      ...(platform
        ? {
            ...
            amdAt: true,
            amdMs: true,
            dispositionAudit: { select: { verdict: true, notes: true, auditorName: true, disposition: true, subDisposition: true, updatedAt: true } },
            recordingMarks: { select: { id: true, authorKind: true, authorName: true, atSeconds: true, note: true, createdAt: true }, orderBy: { atSeconds: "asc" } },
          }
        : {}),
    },
  });
  ...
  return {
    ...
    ...(platform
      ? {
          ...
          subDispositionDetail: attempt.subDispositionDetail || null,
          dispositionAutoLogged: attempt.dispositionAutoLogged === true,
          dispositionNote: attempt.dispositionNote || null,
          amdResult: attempt.amdResult || null,
          amdAt: iso(attempt.amdAt),
          amdMs: Number.isFinite(attempt.amdMs) ? attempt.amdMs : null,
          audit: attempt.dispositionAudit ? { ...attempt.dispositionAudit, updatedAt: iso(attempt.dispositionAudit.updatedAt) } : null,
          marks: (attempt.recordingMarks || []).map((m) => ({ ...m, createdAt: iso(m.createdAt) })),
        }
      : {}),
    ...
    qa: qa
      ? {
          ...
          skippedReasonText: qa.skippedReason
            ? SKIP_REASONS[qa.skippedReason] || (isSampledOut(qa.skippedReason) ? SKIP_REASONS.sampled_out : isFailedScore(qa) ? `The model call failed: ${qa.skippedReason.slice("ai_failed:".length)}` : qa.skippedReason)
            : null,
        }
      : null,
  };
...
export async function reviewCallQa({ attemptId, reviewer, body, repIds = null, now = new Date(), client = db } = {}) {
  ...
  const attempt = await client.salesCallAttempt.findFirst({
    ...
    select: { id: true, transcript: true, salesRep: { select: { name: true, workName: true } }, prospect: { select: { businessName: true } }, qa: { select: { id: true } } },
  });
  ...
  const qa = attempt.qa
    ...
    : await client.salesCallQa.create({
        data: {
          ...
          scoredAt: now,
          model: null,
          deterministic: analyseTranscript(attempt.transcript, { repName: repPublicName(attempt.salesRep), businessName: attempt.prospect?.businessName }),
          overall: null,
