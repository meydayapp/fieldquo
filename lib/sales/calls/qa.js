...
/** The talk ratio a discovery call should sit in — the rep's share of the words. */
export const TALK_RATIO_BAND = Object.freeze({ min: 0.35, max: 0.6 });
...
export function analyseTranscript(transcript, { repName = null, businessName = null } = {}) {
  ...
  return {
    ...
    talk: { repSeconds, contractorSeconds, unknownSeconds, ratio: talkRatio },
    disclosure: {
...
export function buildQaPrompt({ playbook, playbookMatched, objections, transcript, language, repName, businessName, callLanguage }) {
...
export async function scoreAttempt(
  ...
  if (attempt.qa && !force) return { ok: true, reason: "already", message: "Already scored." };
...
export async function scoreMissing({ limit = 20, retryFailed = false, client = db } = {}) {
  ...
  for (const id of ids) {
    ...
    results.push({ id, ...(await scoreAttempt(id, { client, force: retryFailed })) });
  }
  return { attempted: ids.length, done: results.filter((r) => r.ok && r.reason === "done").length, results };
}
