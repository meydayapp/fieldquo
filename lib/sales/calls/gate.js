...
export async function requireCallingRep(request) {
  ...
  const row = await db.salesRep.findUnique({
    ...
    select: {
      ...
      testAccount: true,
      // What this rep may dial that is not a prospect — a colleague's
      // browser, a typed number with no record (SalesRep.canCallColleagues /
      // canCallOffCampaign, set on /platform/sales/reps). Read fresh per
      // request for the same reason as testAccount: a superadmin who
      // switched a privilege off a minute ago switched it off.
