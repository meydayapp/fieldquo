 ...
 *
 * ── The derived state is the whole answer (2026-09-21) ───────────────────
 *
 * This used to gate on `everSeen` too — "signing in is not a rep saying they
 * are ready for the next prospect" — and refused a rep with no activity row.
 * The owner's rule reversed that: a rep is Available the moment a portal tab
 * is present, no button needed, and Off two minutes after it goes
 * (agentState.js's header). livePresence() now derives `state` from the
 * keepalive, so a rep with no row and a fresh keepalive arrives here as
 * `available` and one whose keepalive stopped arrives as `offline`; the
 * state carries the verdict, and a second gate on the row's existence would
 * put the board and the phone back in disagreement. `stale` is kept as a
 * refusal for any caller still building rows by hand; the derivation never
 * sets it.
 */
...
export function fallbackSayFor({ repName = null } = {}) {
  // `repName` is the rep's PUBLIC name — lib/sales/repIdentity.js
  // repPublicName: their work name, else their first name — and is spoken as
  // given. This used to take the real full name and cut it to the first
  // word, which would say "Jesus" to a caller who was rung by "Daniel".
...
export function inboundPlan({
  ...
  const base = {
    ...
    transferTo: null,
    timeoutSeconds: 0,
    // `publicName` is what a caller may hear; `name` stays for FieldQuo's own
    // screens and logs. lib/sales/repIdentity.js.
    rep: rep && rep.id ? { id: rep.id, name: rep.name || null, publicName: repPublicName(rep) } : null,
  ...
  const fallbackSay = fallbackSayFor({ repName: base.rep?.publicName || null });
...
export function salesVoiceInboundState({
  ...
  if (held.length === 0) {
    return {
      ...
      count: 0,
      webhookUrl,
      text: "FieldQuo holds no number a contractor can ring back, so nothing answers. Buy one under Twilio numbers with the purpose set to Sales — one number the team calls AND texts from — or Sales voice for a call-only number in another area code.",
      warning: null,
      tone: "gap",
    };
  }
