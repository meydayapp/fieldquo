...
export function salesVoiceInboundState({
  ...
  if (!transferConfigured) {
    return {
      ...
      count: held.length,
      webhookUrl,
      text: `${held.length} ring-back ${held.length === 1 ? "number is" : "numbers are"} held. A contractor ringing one back is answered, told there is nobody free, and the call is logged against the rep who rang them. Nobody can be put through — FIELDQUO_SALES_TRANSFER_TO is unset.${pointAt}`,
      tone: "gap",
    };
  }
  ...
  return {
    ...
    count: held.length,
    webhookUrl,
    text: `${held.length} ring-back ${held.length === 1 ? "number is" : "numbers are"} held. A contractor ringing one back is put through to the transfer destination, and the call is logged against the rep who rang them either way.${pointAt}`,
