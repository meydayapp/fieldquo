...
export function inboundPlan({
  ...
  const base = {
    ...
    // an announcement, with nobody else on the line whose consent could be at
    // issue — and it IS built now. When this was written it was not, for a
    // plainer reason than consent: the recording had somewhere to be written
    // and nowhere to be read, and a `voicemailUrl` column nothing reads is
    // AGENTS.md failure class 1. Both halves landed together — the queue's
    // <Record> stage writes it, app/api/rep-dial/inbound's after-voicemail
    // stage attaches it, and the superadmin floor board plays it. `record`
    // below still means the CONVERSATION, and it is still false everywhere.
    record: false,
...
export function salesVoiceInboundState({
  ...
  if (anyLive === false) {
    return {
      ...
      count: held.length,
      webhookUrl,
      text: `${held.length} ring-back ${held.length === 1 ? "number is" : "numbers are"} held and a transfer destination is set, but nobody is on the floor right now. A contractor ringing back is answered and logged, and is not put through to a phone the floor has said nobody is at.${pointAt}`,
      tone: "gap",
    };
  }

  // Browsers ring whether or not a desk phone is configured — see
  // inboundPlan(). The transfer number is the fallback after them, and its
  // absence is said in words rather than reported as "nobody can be put
  // through", which is what this used to claim and was never true of the
  // browsers.
  const desk = transferConfigured
    ? " If no browser picks up, the standing transfer number rings next."
    : " No standing transfer number is set (FIELDQUO_SALES_TRANSFER_TO), so when no browser picks up the caller holds, is re-offered to the floor, and is then asked for a message.";
  return {
    state: "connects",
    count: held.length,
    webhookUrl,
    text: `${held.length} ring-back ${held.length === 1 ? "number is" : "numbers are"} held. A contractor ringing one back rings the number's owner and whoever rang them last in their browser, then anybody available, and the call is logged against the rep who rang them either way.${desk}${pointAt}`,
