...
import { validateSubDispositionPick } from "./subDispositions";
...
export const DISPOSITIONS = Object.freeze({
  ...
  text_instead: {
    code: "text_instead",
    // ── "Text me instead" (the owner, 2026-09-18) ─────────────────────────
    //
    // A company that would rather text than talk. Reached — they said so —
    // and the claim is HELD, because the conversation is the rep's, it has
    // just moved channel. The retry rule (lib/sales/retryRules.js) is
    // FINAL for the dialler: no re-dial is scheduled, because ringing
    // somebody who asked to be texted is the call they declined. The next
    // touch is a text, and the call panel opens the composer on this
    // number the moment the outcome is saved (CallPanel saveOutcome).
    label: "They asked to be texted instead",
    hint: "They'd rather text than talk. No re-dial is scheduled; the composer opens on their number.",
    reached: true,
    claim: CLAIM_HOLD,
    prospectStatus: null,
    // A conversation happened, the same reason callback says "contacted".
  ...
  not_a_fit: {
    ...
    prospectStatus: "needs_review",
    // The lead is lost, with the rep's note as the reason — the owner's
    // rule of 2026-09-21 (lib/sales/leadStatus.js). The prospect row is
    // needs_review, as above; the two are different facts about one call.
    leadStatus: "lost",
    doNotContact: false,
    requiresNote: true,
    requiresCallback: false,
 ...
 * @param subDisposition        the rep's sub-reason (lib/sales/calls/subDispositions.js).
 *                              Required when the outcome's list is non-empty;
 *                              refused when it has none.
 * @param subDispositionDetail  the one detail a sub-reason may ask for.
 * @param subLists              effectiveSubDispositions().lists for this
 *                              request; the defaults when the caller has none.
 * @param callbackLimits        `{ maxDaysAhead, maxOpenPerRep, openPersonal }`
 *                              — the platform's own bounds on a callback
 *                              (lib/sales/calls/outcomeSettings.js) and how
 *                              many personal callbacks the rep already holds
 *                              open. Null skips them (the line's own writes,
 *                              which are never callbacks).
 */
export function planDisposition({
  ...
  if (d.requiresCallback) {
    ...
    // ── The platform's own, tighter bounds ────────────────────────────
    //
    // OMniLeads' Grupo.tiempo_maximo_para_agendar and
    // cantidad_agendas_personales (lib/sales/calls/outcomeSettings.js has
    // the citation): how far ahead, and how many open at once. Refused in
    // one plain sentence each, before any write — the sheet prints it
    // under the buttons.
    if (callbackLimits && typeof callbackLimits === "object") {
      const days = Number(callbackLimits.maxDaysAhead);
      if (Number.isFinite(days) && days > 0) {
        const platformLimit = at.getTime() + days * 24 * 60 * 60 * 1000;
        if (callback.getTime() > platformLimit) {
          // No plural ternary: the bound is printed as a date, which reads
          // the same at one day and at fourteen.
          return refuse(`Callbacks can be booked no later than ${new Date(platformLimit).toISOString().slice(0, 10)} (${days} days ahead). Pick a sooner time, or log "Not now" and it comes back on its own.`);
        }
      }
      const max = Number(callbackLimits.maxOpenPerRep);
      const open = Number(callbackLimits.openPersonal);
      if (Number.isFinite(max) && max > 0 && Number.isFinite(open) && open >= max) {
        return refuse(`You already have ${open} callbacks waiting — the limit is ${max}. Make some of those calls first; the list is on the queue.`);
      }
    }
  } else if (callbackAt) {
  ...
  // ── The sub-reason ──────────────────────────────────────────────────────
  //
  // Validated here, not in the route, so the check script executes it with
  // the rest of the plan: an outcome whose list is non-empty is refused
  // without a pick, and a pick on an outcome with no list is a bug in the
  // sheet and refused too. The lists are the platform's effective ones
  // (subDispositions.js); a caller with none gets the defaults.
  const sub = validateSubDispositionPick({ code: d.code, subDisposition, detail: subDispositionDetail, lists: subLists });
  if (!sub.ok) return refuse(sub.reason);
  ...
  return {
    ...
    attempt: {
      ...
      // A callback starts PERSONAL — the rep who promised it delivers it
      // (OMniLeads' AgendaContacto.TYPE_PERSONAL). callbackRepId is filled
      // by the store with the writing rep; the agenda sweep may later move
      // both to "global" and another rep. Every other outcome clears the
      // four, so a row the rep overwrites (an auto-log corrected to "not
      // now") does not keep a stale promise.
      callbackScope: callback ? "personal" : null,
      callbackRepId: callback ? "self" : null,
...
export function autoLogOutcome(row) {
  ...
  // ── A machine picked up, and the carrier said so ─────────────────────
  //
  // Twilio's AMD verdict on the prospect leg (lib/sales/calls/amd.js;
  // written by app/api/rep-dial/amd when `sales.amd.enabled` is on).
  // "machine_*" outranks the clock and the rep's hang-up: whether the rep
  // spoke into it for forty seconds or hung up at the beep, the outcome
  // the RETRY RULE needs is voicemail's — two days, next block, three
  // messages — not hung_up's three hours. The rep may overwrite it from
  // the undo strip like any auto-log, and the history row carries a
  // "machine" chip so nobody reads "I left a message" as the rep's claim.
  // Never on `human`, `fax` or `unknown`: those fall through to the rules
  // below exactly as before.
  if (answered && typeof row.amdResult === "string" && row.amdResult.startsWith("machine_")) {
    return { code: "voicemail", reason: "amd_machine", talkSeconds: talkOf(row, answered, ended) };
  }
