 ...
 */
export const AUTO_LOGGED_CODES = Object.freeze(["no_answer", "busy", "hung_up"]);
...
export function autoLogOutcome(row) {
  ...
  if (row.hungUpBy === HUNG_UP_BY_REP) {
    // The rep pressed the button. They were there; they say what happened.
  ...
  if (!terminal) return { code: null, reason: "not_reported", talkSeconds: null };
  if (status === "completed") {
    // Completed with no answer stamp: the status webhook's "answered" event
    // was missed. Nobody can say whether anyone spoke; the rep is asked.
