...
export function ownsVoicemail(attempt, { salesRepId, ourNumbers = [] } = {}) {
  ...
  // `fromE164`, the real column, with the alias still accepted so the audio
  // route's existing mapping keeps working. Reading the alias FIRST was the
  // shape of the bug: voicemailWhere spoke schema and this spoke alias, and
  // nothing compared them — so the query threw for exactly the reps who had a
  // number while this function looked fine in isolation. One name is the
  // column; the other is a courtesy.
  const ours = attempt.fromE164 || attempt.ourE164;
  return Boolean(ours && mine.has(ours));
}
