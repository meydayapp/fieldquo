...
export const CONVERSATION_CODES = Object.freeze(
  ["callback", "text_instead", "reached_interested", "agreed_link_sent", "reached_not_interested"].filter((c) => Object.hasOwn(DISPOSITIONS, c)),
);
...
export function buildRepFunnel({ rep, counts, monthKey, references = null }) {
  ...
  return {
    rep: rep ? { id: rep.id, name: rep.name, staffLabel: repStaffLabel(rep), code: rep.code || null, active: rep.active !== false, agency: rep.agency || null } : null,
    monthKey,
    stages,
