...
// itself is `SalesRep.managerId`, which landed in prisma/schema.prisma on
// 2026-09-03, shaped after Worker.manager @relation("WorkerReports") — the
// shape leave approval already uses, rather than a second way to say
// "reports to".
//
// ══ The column exists. The tier is still OFF. ═════════════════════════════
//
// Read that as written, because the gap is the kind that ships a dead control.
// Everything in this file is pure and tested and has NO production caller:
// nothing queries `managerId` to build a `reportIds` array, `repViewer()` is
// called only by the check script, and there is no screen that sets a manager.
// `TEAM_LEAD_NOTE_VISIBILITY_FROM` below is null, which is what makes note
// visibility refuse everything.
//
// Turning it on is three things and only the first is mechanical: read the
// reporting line, build a screen to edit it, and set that date. The date is a
// product decision the owner owns — see the constant's own note.
