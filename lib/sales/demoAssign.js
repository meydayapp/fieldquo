...
//
// The four writes to `SalesRep.demoCompanyId`, and the only ones.
//
// A rep CLAIMS a free pool demo. A superadmin ASSIGNS one to a named rep, or
// RELEASES the one a rep holds. A rep is POINTED at one of their own per-rep
// demos (pointRepAtOwnDemo, called by lib/sales/repDemo.js). Everything else
// about a demo — its trade, its data, its login — is somebody else's file.
//
...
async function readPool(client) {
  const [demos, assignments] = await Promise.all([
    client.company.findMany({
      // The POOL: the seeded fixtures nobody owns. A company with a
      // demoOwnerRepId was created for one rep by lib/sales/repDemo.js and is
      // not claimable, assignable or countable here — offering it would hand
      // one rep's half-built walkthrough to another.
...
/**
 * Point a rep at one of their OWN demos — the fourth write, and the reason it
 * is here rather than in lib/sales/repDemo.js.
 *
 * repDemo.js is imported by a rep-facing route, and scripts/check-sales-auth.mjs
 * scans every such module for writes to SalesRep; this file is the one
 * declared exemption for demoCompanyId, so the write lives here beside the
 * other three and the declaration stays true. The escalation argument in the
 * header holds unchanged: the target is re-read and must be a demo, owned by
 * this very rep, and not retired — so the worst a rep can aim this at is a
 * fixture that was seeded for them. It never satisfies attribution, and it
 * cannot take a demo off a colleague because a colleague's demo has a
 * different demoOwnerRepId.
 *
 * Not a compare-and-set on null, deliberately: this is how a rep MOVES between
 * their own trades, and how a rep coming off the pool is moved onto their own
 * demo. The pool's claim keeps its null-WHERE; this is a different door with a
 * different guard.
 */
export async function pointRepAtOwnDemo({ repId, companyId }, client = db) {
  const company = await client.company.findUnique({
    where: { id: companyId },
    select: { id: true, isDemo: true, demoOwnerRepId: true, demoRetiredAt: true },
  });
  if (!company || company.isDemo !== true || company.demoOwnerRepId !== repId || company.demoRetiredAt) {
...
/**
 * Clear a rep's pointer when the demo it points at is THEIR OWN and has just
 * been retired with nothing to move to (repDemo.js retireRepDemo). Same
 * guard shape as pointRepAtOwnDemo — the row must be this rep's own demo —
 * so a rep cannot clear a pool assignment through this door; the pool's
 * release is releaseDemoFromRep, the superadmin's.
 */
export async function unpointRepFromOwnDemo({ repId, companyId }, client = db) {
  const company = await client.company.findUnique({
    where: { id: companyId },
    select: { id: true, isDemo: true, demoOwnerRepId: true },
  });
  if (!company || company.isDemo !== true || company.demoOwnerRepId !== repId) {
    return { ok: false, status: 404, error: "That is not one of your demo companies." };
  }
  await client.salesRep.updateMany({
    where: { id: repId, demoCompanyId: company.id },
