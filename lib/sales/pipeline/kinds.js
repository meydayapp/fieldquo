...
export const TASK_KINDS = [
  ...
  // Claimed lane only — the detour chain.js takes between the score and the
  // brief when a rep is waiting: what the business's own pages say about it,
  // one model call, every inference pinned to its sentence. The backlog goes
  // straight to the brief.
  "INFER_FROM_SITE",
  "GENERATE_RESEARCH_BRIEF",
  // Claimed lane only — the tail chain.js adds after the brief when a rep is
  // waiting. The backlog never reaches it: a script costs a model call per
  // prospect and is worth it for a call that will be made, not for a row.
...
export const PROVIDER_BY_KIND = {
  ...
  // One model call per claimed prospect, on the same budget as the brief
  // and the script beside it. Three claimed-lane stages now share the
  // openai row's 40 a run; the claim rate, not discovery, bounds them.
  INFER_FROM_SITE: "openai",
  GENERATE_RESEARCH_BRIEF: "openai",
  // The second stage on the openai budget, sharing the brief's 40 a run:
  // both are one call per prospect, and the claimed lane is bounded by how
  // many prospects reps claim in a day, not by discovery.
  GENERATE_CALL_SCRIPT: "openai",
};
