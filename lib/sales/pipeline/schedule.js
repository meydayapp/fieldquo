 ...
 */
/**
 * @param minDelayMs  a wait the handler asked for — today only the crawler's
 *        per-host DNS backoff (lib/sales/crawl/policy.js). The retry waits the
 *        LONGER of this and the ladder, capped at BACKOFF_MAX_MS: a hold
 *        shorter than the ladder changes nothing, and a hold longer than six
 *        hours is cut to six so the task cannot be parked past the day.
 */
export function failureOutcome({ attempts, maxAttempts = MAX_ATTEMPTS, permanent = false, minDelayMs = 0 } = {}) {
  ...
  const asked = Number.isFinite(Number(minDelayMs)) ? Math.max(0, Number(minDelayMs)) : 0;
  return { status: "queued", delayMs: Math.min(Math.max(backoffMs(made), asked), BACKOFF_MAX_MS) };
}
