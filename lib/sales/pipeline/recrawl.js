...
// quarter. The order is what "matters now" already means to Places, the
// register lookup and the BBB batch; the crawler walks the same list.
//
// ══ The cap, and why it matches the Places sweep ══════════════════════════
//
// placesSweep.js takes at most SWEEP_PER_TICK (20) lookups a tick across
// every rep. The same number here, for the same reason: a tick is one
// invocation of a cron that fires every minute, and the hour's total is
// what bounds the work, not the tick. Twenty crawls a minute is 1,200 an
...
/** Re-crawls queued per cron tick, across both tiers. placesSweep.js's
 *  SWEEP_PER_TICK, and the header says why the same number. */
