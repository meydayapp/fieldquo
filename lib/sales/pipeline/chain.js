...
//
// ══ …and a stage that finished with NOTHING NEW stops the chain on purpose ══
//
// A re-crawl of an unchanged site (lib/sales/crawl/crawlSite.js, outcome
// `unchanged`) is a success — the §20 cache working — and it is also the one
// success whose successors have nothing to do: the same pages produce the
// same technologies, the same capabilities, the same score, and a brief
// that is re-phrased at a model for no new fact. So a handler may return
// `done: true, advance: false`, and `shouldAdvance` honours it. It is the
// handler's statement that the downstream rows already describe this
// result, and it is only ever true when a previous run wrote them: the
// crawler reports `unchanged` only against a stored hash, and a stored hash
// means a crawl that chained. Everything else advances exactly as before.
//
...
import { inheritedPayload, notBeforeFor, taskPriority } from "./priority";
...
export const NEXT_STAGE = Object.freeze({
  ...
  // Only the claimed lane queues this stage (CLAIMED_TAIL). Its successor is
  // still declared here so a task that reaches it without the lane — the
  // handler refuses it, terminally — hands the prospect on to the brief
  // rather than ending the chain by accident.
  INFER_FROM_SITE: "GENERATE_RESEARCH_BRIEF",
  // Null for the backlog. The claimed lane continues — see CLAIMED_TAIL.
  GENERATE_RESEARCH_BRIEF: null,
  GENERATE_CALL_SCRIPT: null,
});

/**
 * The stages only a claimed prospect gets, keyed by the stage they follow.
 *
 * A separate table rather than a branch in NEXT_STAGE so the backlog's chain
 * reads as the plain list it is, and the cost decision — a model call per
 * claimed prospect, none per backlog row — is one table with a name.
 *
 * Two shapes in it. The DETOUR: after the lead score, a claimed prospect
 * goes through INFER_FROM_SITE before the brief, so the brief's known facts
 * and the script's WHAT WE INFERRED both carry what its own pages say; the
 * backlog goes straight from the score to the brief. The TAIL: after the
 * brief, the script. Where this table names a successor for a kind, the
 * claimed lane takes it over NEXT_STAGE's.
 */
export const CLAIMED_TAIL = Object.freeze({
  CALCULATE_LEAD_SCORE: "INFER_FROM_SITE",
  INFER_FROM_SITE: "GENERATE_RESEARCH_BRIEF",
  GENERATE_RESEARCH_BRIEF: "GENERATE_CALL_SCRIPT",
});
 ...
 *
 *  @param priority  the task's lane (priority.js). "claimed" follows
 *                   CLAIMED_TAIL wherever it names a successor; anything
 *                   else follows NEXT_STAGE alone. */
export function nextStageFor(kind, { priority = null } = {}) {
  if (!Object.hasOwn(NEXT_STAGE, kind)) {
    throw new Error(`sales pipeline: ${kind} has no place in NEXT_STAGE — add it to chain.js`);
  }
  if (priority === "claimed" && Object.hasOwn(CLAIMED_TAIL, kind)) {
    return CLAIMED_TAIL[kind];
  }
  return NEXT_STAGE[kind];
}

/**
 * The stages in the order one lane runs them, walked from the tables above
 * so there is one order and not two. research.js derives its chains from
 * this; a check asserts the claimed walk passes through every stage the
 * backlog walk does, in the same order, plus the claimed-only ones.
 */
export function laneOrder(priority = null, { from = "CRAWL_WEBSITE" } = {}) {
  const walked = [from];
  const seen = new Set(walked);
  let kind = nextStageFor(from, { priority });
  while (kind && !seen.has(kind)) {
    walked.push(kind);
    seen.add(kind);
    kind = nextStageFor(kind, { priority });
  }
  return walked;
}
...
export function shouldAdvance({ result, task, maxAttempts = MAX_ATTEMPTS } = {}) {
  // Only a finished stage may decline its successor — a failure that says
  // `advance: false` is still a failure, and the rule below decides it.
  if (result?.done === true) return result?.advance !== false;
...
export async function advanceChain({ kind, task, db, next = undefined, notBefore = null } = {}) {
  const successor = next === undefined ? nextStageFor(kind, { priority: taskPriority(task) }) : next;
  ...
  // ── The lane travels with the prospect, not with one task ────────────────
  //
  // A claimed prospect's crawl was put ahead of the backlog by a notBefore in
  // the past (priority.js). Its successor is queued HERE, and if it were
  // queued with the column's default it would land behind every backlog row
  // again — a priority exactly one stage deep. So the successor inherits the
  // lane and the same notBefore. An explicit `notBefore` from the caller
  // still wins: that is a deliberate hold, and a hold outranks a lane.
  const lane = inheritedPayload(task);
  await enqueuePipelineTask(
    {
      kind: successor,
      prospectId,
      campaignId: task?.campaignId ?? null,
      payload: { prospectId, ...lane },
      idempotencyKey: successorKey({ kind: successor, prospectId, taskId: task?.id }),
      notBefore: notBefore ?? notBeforeFor(taskPriority(task)),
    },
    { deps: { db } },
  );
