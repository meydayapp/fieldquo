...
//                           OR — with `recrawl` on — a lastCrawledAt older
//                           than MIN_RECRAWL_MS and no crawl task created
//                           inside that interval either (recrawlDue). See
//                           "Re-crawl" below.
...
//   INFER_FROM_SITE         claimed lane only, before the brief: no run row,
//                           a run older than the crawl, or one written by an
//                           older prompt. Queued alone — it chains into the
//                           brief and the script itself.
...
//
// ══ Re-crawl: a site read once was never read again ═══════════════════════
//
// The owner asked: "is the web crawler able to reassess if the website is
// changed?" It could — policy.js recrawlDecision() lets a crawl through
// once lastCrawledAt is MIN_RECRAWL_MS (30 days) old, and fingerprint.js
// stops an UNCHANGED site being re-analysed — but nothing ever asked for a
// second crawl: this planner skipped CRAWL_WEBSITE the moment lastCrawledAt
// was set, and the chain only queues successors, never a predecessor. So a
// contractor who installed a booking widget in March was pitched in
// September on the site they had in February.
//
// Now `recrawl` (on by default in the claimed lane — a rep is dialling this
// row — and passed explicitly by lib/sales/pipeline/recrawl.js for the
// rows next in the shared enrichment order) queues a CRAWL_WEBSITE when
// recrawlDue() says so. NEVER for the pool: a re-crawl is a crawl, and
// 300,000 of them every month for rows nobody will dial is the cost the
// backlog's own ceiling exists to refuse. The crawler does the rest —
// unchanged: the stamp moves and nothing downstream runs (crawlWebsite.js
// `advance: false`); changed: the chain runs exactly as on a first crawl,
// and the brief's WHAT WE KNOW says "website changed since <date>"
// (lib/sales/intel/brief.js, from the evidence rows' own dates).
import { laneOrder } from "./chain";
import { enqueuePipelineTask } from "./tasks";
import { RESEARCH_PRIORITIES, notBeforeFor } from "./priority";
import { CALL_SCRIPT_VERSION, defaultScriptLanguage } from "@/lib/sales/intel/callScript";
import { SITE_INFERENCE_VERSION } from "@/lib/sales/intel/siteInference";
import { MIN_RECRAWL_MS, recrawlDecision } from "@/lib/sales/crawl/policy";
...
/** The stages a prospect WITH a website goes through on the backlog, in
 *  order. Walked from chain.js so there is one order, not two. */
export const RESEARCH_CHAIN = Object.freeze(["ENRICH_BUSINESS", ...laneOrder("backlog")]);
...
/** The stages only a claimed prospect gets — whatever the claimed walk passes
 *  through that the backlog walk does not, in the claimed lane's order.
 *  Walked from chain.js's own tables so this file cannot name a stage the
 *  chain does not queue. Today: INFER_FROM_SITE (the detour before the
 *  brief) and GENERATE_CALL_SCRIPT (the tail after it). */
export const CLAIMED_TAIL = Object.freeze(laneOrder("claimed").filter((k) => !RESEARCH_CHAIN.includes(k)));
...
/**
 * Is this crawled prospect due a second look? Pure.
 *
 * Two clocks, and BOTH must have run out:
 *
 *   lastCrawledAt   older than the interval — policy.js's own rule, so this
 *                   planner and the crawler agree on "due"; a task queued
 *                   here is never one the crawler then skips as recent.
 *   the last task   no CRAWL_WEBSITE task CREATED inside the interval, any
 *                   status. A crawl that was refused (robots.txt, a host
 *                   that stopped resolving) never moves lastCrawledAt, and
 *                   without this clock the planner would queue that refusal
 *                   again on every call. One try per interval is the bound —
 *                   and it is a real second chance, because a robots.txt is
 *                   a thing people edit.
 *
 * @param prospect  { lastCrawledAt }
 * @param tasks     this prospect's CRAWL_WEBSITE rows, any status
 * @returns { due: boolean, reason, previousCrawledAt: Date|null }
 */
export function recrawlDue({ prospect, tasks = [], now = new Date(), minIntervalMs = MIN_RECRAWL_MS } = {}) {
  const at = now instanceof Date ? now : new Date(now);
  const previous = prospect?.lastCrawledAt ? new Date(prospect.lastCrawledAt) : null;
  if (!previous || Number.isNaN(previous.getTime())) return { due: false, reason: "never_crawled", previousCrawledAt: null };
  const decision = recrawlDecision({ prospect, now: at, minIntervalMs });
  if (decision.act !== "crawl") return { due: false, reason: decision.reason, previousCrawledAt: previous };
  const floor = at.getTime() - minIntervalMs;
  const recentTask = (Array.isArray(tasks) ? tasks : []).find((t) => {
    const created = t?.createdAt ? new Date(t.createdAt).getTime() : NaN;
    return Number.isFinite(created) && created > floor;
  });
  if (recentTask) return { due: false, reason: "tried_recently", previousCrawledAt: previous };
  return { due: true, reason: "due", previousCrawledAt: previous };
}
 ...
 * @param inferenceRun  the ProspectInferenceRun row, or null
 * @param recrawl   may a crawled site be crawled again once it is due?
 *                  Defaults to the lane: a claimed row is one a rep is
 *                  dialling; the backlog lane is "nobody waiting" and gets
 *                  it only when recrawl.js says the row is next in
 *                  dispatch. See "Re-crawl" in the header.
 * @param now       the clock recrawlDue reads
...
export function researchPlan({
  ...
  for (const kind of chain) {
    ...
    if (kind === "CRAWL_WEBSITE") {
      if (prospect.lastCrawledAt) {
        if (!recrawl) continue;
        const again = recrawlDue({ prospect, tasks: of(kind), now });
        if (!again.due) continue;
        // The task says what it is. `recrawl` is what the console counts
        // (recrawl.js); `previousCrawledAt` is the date the site was last
        // read, kept on the row because the crawler is about to overwrite
        // it. Not inherited by the successors (priority.js) — the brief
        // reads the change off the evidence rows, which outlive any task.
        return {
          ...none,
          enqueue: [spec(kind, { recrawl: true, previousCrawledAt: again.previousCrawledAt.toISOString() })],
          skipped: null,
        };
      }
  ...
  // Stale three ways, for the inference run and the script alike: no row;
  // a row older than the crawl; a row written by an earlier prompt. The
  // third is what makes a prompt rewrite reach the rows — the owner's voice
  // correction bumped CALL_SCRIPT_VERSION and 101 stored scripts would
  // otherwise have kept the old voice until their site was crawled again.
  // The handler's hash check is the second gate: a row already at the
  // current version spends nothing.
  const staleRow = (row, version) =>
    !row ||
    (prospect.lastCrawledAt && row.crawledAt && new Date(row.crawledAt) < new Date(prospect.lastCrawledAt)) ||
    (row.promptVersion != null && row.promptVersion !== version);
  const STALE_BY_KIND = {
    INFER_FROM_SITE: () => staleRow(inferenceRun, SITE_INFERENCE_VERSION),
    GENERATE_CALL_SCRIPT: () => staleRow(script, CALL_SCRIPT_VERSION),
  };

  // The first stale stage is queued and the loop stops: the inference stage
  // chains into the brief and the script on its own (chain.js), so queueing
  // the script beside it would race the chain into a duplicate — and a
  // script written before the inferences landed is the generic one again.
  for (const kind of CLAIMED_TAIL) {
    const isStale = STALE_BY_KIND[kind];
    if (!isStale) throw new Error(`researchPlan: no staleness rule for claimed stage ${kind}`);
    if (!isStale()) continue;
    if (tries(kind) > MAX_REQUEUES) return { ...none, skipped: "requeues_exhausted" };
    return { ...none, enqueue: [spec(kind)], skipped: null };
  }
 ...
 * @param priority     "claimed" | "backlog"
 * @param now          the clock — recrawlDue reads it, and the audit stamps it.
 * @param recrawl      may a due site be crawled again? researchPlan says
 *                     what the default means.
 * @param excludeTaskIds  task rows the plan must not see. One caller: a
 *                     stage calling from INSIDE its own run (the crawl
 *                     handler, after an unchanged re-crawl declined its
 *                     successors), whose row is still `claimed` and would
 *                     otherwise read as "the chain is moving" — the plan
 *                     would wait for a successor that task has just said
 *                     it will not queue.
 *
 ...
 */
export async function ensureResearchQueued({
  db,
  prospectIds = [],
  priority = "claimed",
  now = new Date(),
  ...
  const [prospects, tasks, scripts, runs] = await Promise.all([
    db.prospect.findMany({
      where: { id: { in: ids } },
      // province and assignedRepId decide which LANGUAGE of script is the
      // prospect's default — the one whose row the staleness rule reads.
      select: { id: true, websiteUrl: true, lastCrawledAt: true, doNotContactAt: true, campaignId: true, province: true, assignedRepId: true },
    }),
    ...
    typeof db.prospectCallScript?.findMany === "function"
      ? db.prospectCallScript.findMany({
          ...
          select: { prospectId: true, language: true, crawledAt: true, generatedAt: true, promptVersion: true },
        })
      : Promise.resolve([]),
    typeof db.prospectInferenceRun?.findMany === "function"
      ? db.prospectInferenceRun.findMany({
          where: { prospectId: { in: ids } },
  ...
  const excluded = new Set((Array.isArray(excludeTaskIds) ? excludeTaskIds : []).filter(Boolean));
  ...
  // ── The script row that counts is the DEFAULT language's ────────────────
  //
  // A prospect can hold up to three ProspectCallScript rows now, one per
  // language. The pipeline writes and re-checks only the default one
  // (callScript.js defaultScriptLanguage — Quebec → fr, else the assigned
  // rep's portal language, else en); the others are on-demand rows the
  // route refreshes when a rep asks for them, and a stale French row must
  // not queue a pipeline task that would write English. The reps' languages
  // are read in one query; a client without SalesRep (a check's stub)
  // reads as "no rep", which decides en for everywhere but Quebec.
  const repIds = [...new Set(prospects.map((p) => p.assignedRepId).filter(Boolean))];
  const reps =
    repIds.length && typeof db.salesRep?.findMany === "function"
      ? await db.salesRep.findMany({ where: { id: { in: repIds } }, select: { id: true, language: true } })
      : [];
  const repById = new Map(reps.map((r) => [r.id, r]));
  const scriptById = new Map();
  for (const p of prospects) {
    const wanted = defaultScriptLanguage({ prospect: p, rep: p.assignedRepId ? repById.get(p.assignedRepId) || null : null });
    const row = scripts.find((s) => s.prospectId === p.id && (s.language || "en") === wanted) || null;
    if (row) scriptById.set(p.id, row);
  }
  const runById = new Map(runs.map((r) => [r.prospectId, r]));
  ...
  for (const id of ids) {
    const plan = researchPlan({
      ...
      inferenceRun: runById.get(id) || null,
      recrawl,
      now,
    });
