...
import { pgSafeString } from "@/lib/text/pgSafeText";
...
async function completeTask({ task, token, now, note, deps = {} }) {
  ...
  return settle({
    ...
    data: {
      ...
      // pgSafeString before the slice: a note quoting scraped page text or a
      // model's reply can carry a NUL, and a task that cannot record WHY it
      // finished is the silent drop this design refuses. See
      // lib/text/pgSafeText.js.
      lastError: note ? pgSafeString(String(note)).slice(0, 500) : null,
      claimToken: null,
      claimExpires: null,
    },
  });
 ...
 */
async function failTask({ task, token, now, reason, permanent = false, minDelayMs = 0, maxAttempts = MAX_ATTEMPTS, deps = {} }) {
  const prisma = deps.db || db;
  const outcome = failureOutcome({ attempts: task.attempts, maxAttempts, permanent, minDelayMs });
  const message =
  ...
  const ok = await settle({
    ...
    data: {
      ...
      lastError: pgSafeString(message).slice(0, 500),
...
export async function runClaimedTask({ task, now = new Date(), token, deps = {} }) {
  ...
  const outcome = await failTask({
    ...
    // A handler may ask for a longer wait than the ladder — see
    // failureOutcome. It may not ask for a shorter one.
    minDelayMs: Number(result?.retryAfterMs) || 0,
  });
