...
//
// ══ The one export here that is not pure ═══════════════════════════════════
//
// `ensureResearchQueued` lives in ./research.js and is re-exported from this
// file because this is the module the campaign screen, the claim route and
// the checks already import for "what is the pipeline doing about this
// prospect" — and "queue what it is not doing" is the other half of that
// question. Two import paths for one function would be the copy that rots;
// this is one function with one home and a second door that says where the
// home is.
import { TASK_KINDS } from "./kinds";

export { ensureResearchQueued } from "./research";
...
export const STAGES = Object.freeze([
  ...
  },
  {
    ...
    label: "Reading what their site says about them",
    what: "Only for a business a rep has claimed: a model reads the about, services and home pages and records the owner's name, the years, the crew, the service area — each with the exact sentence it came from, or not at all. One model call, once per crawl.",
  },
  {
    kind: "GENERATE_RESEARCH_BRIEF",
    label: "Writing the rep's brief",
    what: "The one page a rep reads before dialling. Spends money at a model when a rep is waiting; composed from rows alone for the backlog.",
  },
  {
    kind: "GENERATE_CALL_SCRIPT",
    label: "Writing the call script",
    what: "Only for a business a rep has claimed: turns the brief, the capabilities and the tier playbook into the script for this one call. One model call, once per crawl.",
  },
]);
