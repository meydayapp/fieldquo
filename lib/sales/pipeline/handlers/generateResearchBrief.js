...
/** Everything the card is built from. Seven queries, no N+1. */
export async function loadBriefInputs(prisma, prospectId) {
  const [prospect, capabilities, technologies, inferences, opportunities, score, crawls] = await Promise.all([
    prisma.prospect.findUnique({
      ...
      select: {
        ...
        // What Google Places confirmed or contradicted — one fact on the
        // card, and the "permanently closed" line the script must carry.
        placesVerdict: true,
        placesResult: true,
        // Read for the call script, which stamps the crawl it describes on
        // its row. The brief itself does not use it: composeBrief decides
        // "crawled" from decided capabilities, not from this column.
        lastCrawledAt: true,
        // Who to ask for — every named person from every source; the brief
        // leads with the best (lib/sales/intel/people.js) and cites it.
    ...
    // The dates the crawler wrote evidence — two of them is "the site
    // changed", and the card says so (brief.js websiteChange).
    loadCrawlHistory(prospectId, { deps: { db: prisma } }),
  ]);
  return { prospect, capabilities, technologies, inferences, opportunities, score, crawls };
}
