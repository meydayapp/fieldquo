...
// ── Raised from 100, and this is the biggest lever on discovery ──────────
//
// Measured on the NY campaign: discovery completions were EXACTLY twenty
// minutes apart, dead consistent, at 2-3 per hour. Not because a page is slow —
// because the pipeline is ONE FIFO queue and discovery enqueues its next page
// BEHIND the several hundred research tasks its own results just created. It
// gets one slot per lap of the queue, and a lap is twenty minutes.
//
// So the way to bank faster is not to run more often — it is to do more per
// lap. Five times the rows for the same one slot, and it is nearly free: the
// snapshot is already downloaded and parsed in memory (snapshotCache), so a
// bigger page is more of a list already in RAM, not five more fetches.
//
// It does not raise total cost. Acceptance is still bounded by targetCount and
// research is still bounded by researchBudget() — this changes how quickly the
// same bounded work happens, not how much of it there is.
...
export async function runDiscoverBusinesses({ task, payload = {}, now = new Date(), db: prisma }) {
  ...
  if (stopBefore) {
    ...
    return { done: true, note: `stopped: ${stopBefore}${stoppedShort(campaign, stopBefore) ? " — every source is blocked, nothing ran out; recorded as paused, not completed" : ""}` };
  }
  ...
  const short = stopAfter ? stoppedShort(merged, stopAfter) : false;
  await prisma.prospectCampaign.update({
    where: { id: campaign.id },
    data: {
      sourceState: merged.sourceState,
      ...(stopAfter ? { status: statusFor(stopAfter, { short }), ...(short ? {} : { completedAt: now }) } : {}),
    },
  });
  ...
  return {
    ...
    note:
      ...
      (stopAfter ? ` — ${stopAfter}${short ? " (every source is blocked, nothing ran out; recorded as paused, not completed)" : ""}` : ""),
  };
}
...
export async function promoteToResearch({ prisma, campaignId, limit = PROMOTE_LIMIT }) {
  ...
  const where = {
    ...
    // discovering that.
    doNotContactAt: null,
    // A row merged into another is researched through its survivor.
    mergedIntoId: null,
  };
...
/**
 * Did the campaign stop because every source DIED, rather than ran out?
 *
 * `source_ended` is the stop reason for "no source is open", and a source is
 * closed by ending (no more rows — clean) or by being blocked (five failures
 * or broken settings — not clean). A campaign whose every source is blocked
 * and none ended found nothing it was going to find, and that is not
 * "completed": the word the status vocabulary has for "not running,
 * positions kept, can be resumed" is `paused`, and that is what is recorded
 * — see statusFor. A campaign with one source ended and one blocked still
 * completes, with the blocked one named on the screen, as before.
 *
 * Exported for the check. Pure over the campaign row.
 */
export function stoppedShort(campaign, stopReason) {
  if (stopReason !== "source_ended") return false;
  const keys = campaignSourceKeys(campaign);
  if (!keys.length) return false;
  const states = keys.map((key) => sourceStateFor(campaign, key));
  return states.every((s) => s.blocked) && !states.some((s) => s.ended);
}

/**
 * The status a stop reason records.
 *
 * `short` is stoppedShort() above: every source blocked, none ended. The
 * vocabulary — draft, running, paused, cancelled, completed (schema, and the
 * routes' own checks) — has no "stopped", and one is deliberately not
 * invented: every reader of the column would have to learn it, and the
 * start route refuses `completed` outright ("create a new one") while it
 * lets `paused` resume. A campaign that stopped short can be fixed and
 * resumed, so `paused` is the status that is true of it. The source cards
 * say WHY it is paused, and the campaign screen says so in its header.
 */
function statusFor(stopReason, { short = false } = {}) {
  if (stopReason === "paused") return "paused";
  if (stopReason === "cancelled") return "cancelled";
  if (short) return "paused";
  return "completed";
}
 ...
 *
 * A campaign that stopped short (every source blocked) is recorded `paused`
 * with no completedAt — it did not complete.
 */
async function finish(prisma, campaign, stopReason, now) {
  const short = stoppedShort(campaign, stopReason);
  const status = statusFor(stopReason, { short });
  if (campaign.status === status) return;
  await prisma.prospectCampaign.update({
    where: { id: campaign.id },
    data: { status, ...(short ? {} : { completedAt: now }) },
  });
}
