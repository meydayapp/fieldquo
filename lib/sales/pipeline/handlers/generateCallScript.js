...
export async function generateCallScript({
  ...
  if (!checked.ok) {
    return { done: false, retry: false, language: lang, reason: `call_script: rejected — ${checked.problems.join(", ")}` };
  }
  ...
  if (!voice.ok) {
    ...
    const again = await askModel({
      system: CALL_SCRIPT_SYSTEM,
      prompt: `${callScriptPrompt(inputs)}\n\n${voiceRetryNote(voice, { long: longSentences(checked.script) })}`,
      maxTokens: CALL_SCRIPT_MAX_TOKENS,
      schema: callScriptSchema(),
      schemaName: CALL_SCRIPT_AI_AREA,
      onUsage: meter(":retry"),
    ...
    if (!again?.ok) {
      ...
      return { done: false, retry, language: lang, reason: `call_script: retry — ${again?.reason || "unknown"}` };
    }
    checked = validateCallScript(again.data);
    if (!checked.ok) {
      return { done: false, retry: false, language: lang, reason: `call_script: rejected on retry — ${checked.problems.join(", ")}` };
    }
    voice = voiceLint(checked.script, lintOptions);
    if (!voice.ok) {
      const where = voice.findings.slice(0, 3).map((f) => `${f.field}: ${f.problems.join("/")}`).join("; ");
      return { done: false, retry: false, language: lang, reason: `call_script: rejected — voice (${where})` };
    }
  }
