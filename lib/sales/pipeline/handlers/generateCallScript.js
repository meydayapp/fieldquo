...
export async function loadCallScriptInputs(prisma, prospectId, { assemble = assembleProspectPlaybook } = {}) {
  ...
  const [playbook, rep, pageRows, directory, siteKind] = await Promise.all([
    assemble({
      prospectId,
      rep: null,
      useAi: false,
    ...
    rows.prospect.assignedRepId
      ? prisma.salesRep.findUnique({ where: { id: rows.prospect.assignedRepId }, select: { name: true, workName: true, language: true } })
      : null,
    ...
    // Whose pages these are. A directory's copy is the directory's words,
    // and a script that quoted "we have served the community since 1985"
    // off Ethical Services' about page would say it about the wrong company.
  ...
  return {
    ...
    // The name the rep introduces themselves by — work name, else first
    // name (lib/sales/repIdentity.js). The prompt still says "first name";
    // callScriptInputs keeps the first word, so a two-word work name opens
    // with its first word, the way a rep says it on the phone.
...
export async function generateCallScript({
  ...
  // The line itself goes in the reason — a task note that says
  // "no_finite_verb" and not which sentence cannot be acted on.
  const where = (findings) => findings.slice(0, 3).map((f) => `${f.field}: ${f.problem || f.problems.join("/")} "${String(f.snippet || f.sentence || "").slice(0, 100)}"`).join("; ");
  const retryOnce = async (note) => {
    ...
    const second = validateCallScript(again.data, { language: lang });
    if (!second.ok) {
  ...
  let checked = validateCallScript(result.data, { language: lang });
  let retried = false;
  if (!checked.ok) {
    retried = true;
  ...
  // The model's own words, made storable — lib/text/pgSafeText.js. `script`
  // is a jsonb column and a single U+0000 anywhere inside it fails the whole
  // upsert with "invalid byte sequence for encoding UTF8: 0x00", which is
  // what abandoned five of these tasks between 2026-09-14 and -09-21. A
  // model echoing a page's stray control byte back inside a citation is the
  // realistic path; the crawl's own write is sanitised too, so neither end
  // can carry one now.
  const data = {
    script: pgSafe(checked.script),
