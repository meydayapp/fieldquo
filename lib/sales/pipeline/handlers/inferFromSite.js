...
import { loadStoredSiteKind } from "@/lib/sales/intel/db";
import { SERVICES_INFERENCE_KIND, parseServicesValue } from "@/lib/sales/intel/servicesOffered";
import {
...
export async function loadSiteInferenceInputs(prisma, prospectId) {
  const [prospect, pageRows, siteKind, servicesRow] = await Promise.all([
    ...
    // Whose pages these are. The prompt's heading is "WHAT THEIR OWN WEBSITE
    // SAYS", and a directory's about page is not that: an owner name or a
    // founding year read off it would be the directory's, filed against the
    // business. So a listing hands the model no pages at all — the run is
    // recorded as empty and nothing is spent (handleInferFromSite).
    loadStoredSiteKind(prospectId, { deps: { db: prisma } }),
    // The evidence-cited service list ANALYZE_CAPABILITIES wrote
    // (servicesOffered.js). Handed to the model as SERVICES THEY LIST, and
    // the fence its services_offered answer is checked against.
    prisma.prospectInference.findUnique({
  ...
  return {
    prospect,
    pages: siteKind ? [] : selectPageExcerpts(pageRows),
    services: siteKind ? [] : parseServicesValue(servicesRow?.value).map((s) => s.name),
    siteKind,
    crawledAt: prospect.lastCrawledAt ?? null,
  };
}
...
export async function handleInferFromSite({
  ...
  if (!inputs.pages.length) {
    ...
    return {
      ...
      note: rows.siteKind
        ? `the listed website is ${rows.siteKind.kind === "platform_profile" ? "a platform profile" : "a directory"}${rows.siteKind.brand ? ` (${rows.siteKind.brand})` : ""}, not their own — its pages are not read as the business's words; nothing spent, the run is recorded`
        : "no page text was crawled for this prospect — nothing to infer from, nothing spent; the run is recorded so the next claim skips it until a crawl lands",
    };
  }
  ...
  const droppedNote = dropped.length
    ? `dropped ${dropped.length}: ${dropped.map((d) => `${d.kind} (${d.reason}${d.names?.length ? `: ${d.names.join(", ")}` : ""})`).join(", ")}`
    : null;
  const namesDropped = kept.filter((k) => k.droppedNames?.length).flatMap((k) => k.droppedNames);
  return {
    done: true,
    note: [
      ...
      droppedNote,
      namesDropped.length ? `service name(s) not in the evidence, dropped: ${namesDropped.join(", ")}` : null,
      inputs.services.length ? `${inputs.services.length} evidence-cited service(s) handed to the model` : null,
