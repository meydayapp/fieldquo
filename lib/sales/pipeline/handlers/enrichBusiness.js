 ...
 */
export function routeAfterEnrich(prospect, { derivedSite = null } = {}) {
  const url = typeof prospect?.websiteUrl === "string" ? prospect.websiteUrl.trim() : "";
  if (url) return { next: "CRAWL_WEBSITE", reason: "website_listed", crawlUrl: null };

  // ── A guessed domain is worth fetching, and is not a website ────────────
  //
  // Quebec's RBQ register publishes no website column at all, so before this
  // branch existed every one of its 54,264 licences routed straight past the
  // crawler — and tradeDetect.js, which is the ONLY thing that can give a
  // register row a trade, never got a page to read. The result was a source
  // that banked rows nothing could ever make callable.
  //
  // lib/sales/discovery/rbq/derivedSite.js derives a domain from the licence
  // email and writes it as a `derived_site` ProspectInference — a hypothesis,
  // stored where a hypothesis goes. This reads that inference and sends the
  // crawler at it.
  //
  // What it does NOT do is fill `websiteUrl` in on the way past. The URL is
  // handed to the crawl as an argument and the column stays null, so:
  //   • `hasWebsite` is still only ever answered by a crawl;
  //   • a re-run of this stage re-derives nothing and reads the same
  //     inference, so there is no path by which the guess ages into a fact;
  //   • the reason below says `derived`, and every note, screen and task row
  //     downstream repeats that word rather than "listed".
  const derived = typeof derivedSite === "string" ? derivedSite.trim() : "";
  if (derived) {
    return { next: "CRAWL_WEBSITE", reason: "website_derived", crawlUrl: `https://${derived}` };
  }

  // NOT "they have no website" — "we have no address to fetch". The
  // distinction is the whole of this file's closing comment.
  return { next: "DETECT_OPPORTUNITIES", reason: "no_website_to_crawl", crawlUrl: null };
}
...
/**
 * @param payload { prospectId?, promote? } — prospectId falls back to the
 *        task's own column, the same as every other stage, so both enqueue
 *        shapes work.
 *
 *        `promote: false` keeps step 3 out of it. Measured on 2026-09-11:
 *        every one of the 13,734 prospects this stage had promoted to
 *        `researching` was thereby OUT of a rep's reach, because
 *        claimCandidateWhere() in lib/sales/prospectView.js admits
 *        `discovered` and nothing else — and every one of the 184,799 still
 *        claimable had a lastCrawledAt of null. Research made a business
 *        unclaimable. lib/sales/pipeline/research.js queues this stage for a
 *        prospect that is claimed or claimable, and neither may leave the
 *        pool for having been looked at, so it sends `promote: false`. The
 *        discovery fan-out keeps promoting: its bound on re-offering a row
 *        (promoteToResearch's status filter) is what the transition is for.
 */
export async function handleEnrichBusiness({ task, payload = {}, db: prisma } = {}) {
  ...
  // Loaded BEFORE the suppression check, not after the route is chosen. A
  // derived domain is a domain FieldQuo is about to fetch, so it has to be on
  // the same list as any other — and this file's header names the cheapest
  // place to stop as the one that should. The crawler checks it again, which
  // is the same deliberate double gate the header describes.
  const derivedSite = prospect.websiteUrl ? null : await loadDerivedSite(prospect.id, { deps: { db: prisma } });
  ...
  const suppression = await checkSuppression(prisma, {
    ...
    domain: prospect.domain || prospect.websiteUrl || derivedSite?.domain || null,
  });
  ...
  const route = routeAfterEnrich(prospect, { derivedSite: derivedSite?.domain || null });
  const mayPromote = payload.promote !== false && prospect.status === RESEARCHABLE_STATUS;
  ...
  let promoted = 0;
  if (mayPromote || Object.keys(repairs).length) {
    const written = await prisma.prospect.updateMany({
      where: { id: prospect.id, status: prospect.status },
      data: {
        ...repairs,
        ...(mayPromote ? { status: RESEARCHING_STATUS } : {}),
      },
    });
    promoted = mayPromote ? written.count : 0;
  }
  ...
  return {
    ...
    note: [
      // The reason says `website_derived` when the address was guessed, and it
      // is repeated verbatim onto the task row. A superadmin reading a task
      // list must be able to tell a crawl of a published site from a crawl of
      // a hypothesis without opening anything.
      ...
      payload.promote === false ? "left claimable (promote: false)" : null,
    ]
      .filter(Boolean)
      .join("; "),
  };
