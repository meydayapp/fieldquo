...
// than about the page:
//
//   1. A null NEVER overwrites a known value WRITTEN BY THE SAME DETECTOR
//      VERSION. "We could not look today" must not erase "we looked last
//      week and there was a booking page." A crawl that fails after a
//      successful one would otherwise silently downgrade every finding to
//      unknown, and the rep would stop being told anything.
//
//      The version clause is from 2026-09-13. Detector "1" wrote `false` off
//      crawls that had only guessed at the site (capabilityDetect.js, "A
//      guessed crawl is not a look"), and detector "2" answers `null` for
//      exactly those. Preserving a "1" false against a "2" null would keep
//      the unearned claim on the rep's screen for ever — the one thing the
//      re-crawl exists to remove. So a value from an OLDER version is
//      superseded by the current detector's answer, null included: the
//      older row may have been right, and it may not, and silence is the
//      safe direction until the current rule has earned it again.
...
//
// ══ The services they list, decided here too ══════════════════════════════
//
// Same stage, same reason as the trade: what a contractor sells is written
// in their menu, their sitemap, their JSON-LD and their WordPress page
// index, all of which the crawler stored as evidence rows.
// lib/sales/intel/servicesOffered.js reads those rows — and ONLY those
// rows — into a cited list; this file writes it as the `services`
// ProspectInference, evidenceIds pointing at the rows each name came from.
// Rule 5 applies: a run that finds no service writes nothing, so last
// month's menu is not erased by this month's blocked crawl. And a listing's
// menu is the directory's, never written (the same gate as the trade).
//
// ══ Whose site it is, decided before anything is read off it ══════════════
//
// lib/sales/intel/siteKind.js classifies the URL on the record — their own
// site, a directory, a profile on a platform, or unknown — from the pages,
// the host, and how many other prospects list the same host (counted here,
// because the classifier has no database). The answer travels INTO
// detectCapabilities and inferTrade rather than being applied afterwards, so
// there is one place where "a directory's booking widget is not theirs" is
// decided and no second reader that forgot. Two writes are gated on it here
// as well: `Prospect.email` is never filled from a listing (the address on
// Ethical Services' pages was the directory's own service@), and the trade
// is never established from a listing's copy — the same gate a derived site
// that failed corroboration goes through.
...
import { emailFromEvidence } from "@/lib/sales/intel/prospectEmail";
import {
  TRADE_DETECTOR,
...
import { classifySiteKind, notTheirOwnSite } from "@/lib/sales/intel/siteKind";
import {
  SERVICES_DETECTOR,
  SERVICES_DETECTOR_VERSION,
  SERVICES_INFERENCE_KIND,
  SERVICE_EVIDENCE_TYPES,
  servicesFrom,
  servicesValue,
} from "@/lib/sales/intel/servicesOffered";
import { countProspectsOnHost, loadDerivedSite } from "@/lib/sales/intel/db";
...
export async function handleAnalyzeCapabilities({ task, payload = {}, db, now = new Date() } = {}) {
  ...
  // ── Whose site is on the record? ────────────────────────────────────────
  const siteUrl = prospect.websiteUrl || (derivedSite?.domain ? `https://${derivedSite.domain}` : null);
  const sharedHostCount = siteUrl ? await countProspectsOnHost(siteUrl, { excludeProspectId: prospectId, deps: { db } }) : null;
  const siteKind = classifySiteKind({
    pages: crawl,
    businessName: prospect.businessName,
    alsoKnownAs: prospect.tradingNames || [],
    websiteUrl: siteUrl,
    sharedHostCount,
  });
  const listing = notTheirOwnSite(siteKind.kind);

  const result = detectCapabilities({ crawl, technologies, prospect, siteKind });
  const written = await writeCapabilities({ db, prospectId, result, now, listing });
  ...
  // A listing's copy is the directory's words, not the business's: the
  // trade is not established from it, for the reason a derived site that
  // failed corroboration is refused — a confident answer about the wrong
  // site costs more than none.
  const trade = inferTrade({ crawl, prospect, siteBelongsToProspect: identity.corroborated && !listing });
  ...
  const servicesWritten = await writeServices({
    db,
    prospectId,
    now,
    businessName: prospect.businessName,
    allowed: identity.corroborated && !listing,
  });
  ...
  return {
    ...
    note: [
      ...
      `${result.pagesConsidered} page(s) rendered${result.pagesRecovered ? ` (${result.pagesRecovered} recovered from a script payload)` : ""}`,
      result.rendered === "js_shell" ? "site is rendered by JavaScript; absence withheld everywhere" : null,
      result.eligibility.deep ? "absence provable" : `absence withheld: ${result.eligibility.reason}`,
      servicesWritten.note,
      written.preserved ? `${written.preserved} earlier finding(s) kept` : null,
      written.emailWritten ? `email ${written.emailWritten.email} written from ${written.emailWritten.emailSource}` : null,
      listing ? siteKindNote(siteKind) : null,
...
/**
 * What the run says about a URL that is not the business's own site, naming
 * the site by its own title so a superadmin can see at once which directory
 * it was and why the rows below it went null.
 */
function siteKindNote(siteKind) {
  const what = siteKind.kind === "platform_profile" ? "a platform profile" : "a directory";
  const name = siteKind.brand || siteKind.host || "unnamed";
  return `the listed website is ${what} (${name}), not their own — WEBSITE false, other capabilities withheld, email and trade not written from it (${siteKind.reasons[0] || "no reason"})`;
}
...
/**
 * The evidence-cited service list, written as one inference row.
 *
 * Reads the rows by type with their ids (the citation is the point), so
 * this is its own query rather than loadCrawl's: that loader drops ids and
 * deliberately excludes `sitemap_url` and `wp_page`, which are not pages.
 *
 * @returns { count, note }
 */
async function writeServices({ db, prospectId, now, businessName = null, allowed }) {
  if (!allowed) return { count: 0, note: null };
  const rows = await db.prospectEvidence.findMany({
    where: { prospectId, source: "website", type: { in: [...SERVICE_EVIDENCE_TYPES] } },
    orderBy: { observedAt: "desc" },
    take: 800,
    select: { id: true, type: true, rawValue: true, normalizedValue: true, sourceUrl: true, observedAt: true },
  });
  const found = servicesFrom({ evidence: rows, businessName });
  if (!found.services.length) return { count: 0, note: "no service names in the menu, sitemap, structured data or page index" };

  const data = {
    value: servicesValue(found.services),
    confidence: Math.max(...found.services.map((s) => s.confidence)),
    evidenceIds: found.evidenceIds,
    source: "derived",
    observedAt: now,
    modelVersion: `${SERVICES_DETECTOR}/${SERVICES_DETECTOR_VERSION}`,
  };
  await db.prospectInference.upsert({
    where: { prospectId_kind: { prospectId, kind: SERVICES_INFERENCE_KIND } },
    update: data,
    create: { prospectId, kind: SERVICES_INFERENCE_KIND, ...data },
  });
  const by = Object.entries(found.counts).map(([k, v]) => `${k} ${v}`).join(", ");
  return { count: found.services.length, note: `${found.services.length} service(s) listed (${by})` };
}

async function writeCapabilities({ db, prospectId, result, now, listing = false }) {
  let preserved = 0;
  let emailWritten = null;

  const existing = await db.prospectCapability.findMany({
    where: { prospectId },
    select: { code: true, value: true, detectorVersion: true },
  });
  const known = new Map(existing.map((row) => [row.code, row]));
  ...
  await db.$transaction(async (tx) => {
    ...
    for (const capability of result.capabilities) {
      ...
      // one this version of the detector made. A row from an older version
      // (or one with no version at all) is superseded.
      const before = known.get(capability.code);
      if (
        capability.value === null &&
        before?.value != null &&
        String(before.detectorVersion || "") === CAPABILITY_DETECTOR_VERSION
      ) {
        preserved++;
        continue;
      }
      ...
      // ── The address itself, not only the fact that there is one ─────────
      //
      // "Publishes an email address" was on the rep's card and the lead made
      // from the prospect had no email: the mailto: had been in the evidence
      // row since the crawl. Prospect.email is the copy the forms read,
      // written from the strongest EMAIL_CONTACT evidence in the same
      // transaction as the evidence itself, so the two can never disagree.
      // Overwritten on each crawl that finds one — the site is the source of
      // truth and a new crawl is newer truth — and never cleared: a crawl
      // that found nothing is rule 1 again, an unknown that must not erase a
      // finding. What a rep typed on the LEAD is never touched from here.
      //
      // Never from a LISTING. The mailto: on a directory's pages is as likely
      // the directory's own address as the business's — service@ethicalservices
      // .com reached a lead this way — and a rep who emails it writes to the
      // wrong company. The EMAIL_CONTACT row still says an address was seen
      // and where; the copy the forms read is not filled from it.
      if (capability.code === "EMAIL_CONTACT" && capability.value === true && !listing) {
        const found = emailFromEvidence(capability.evidence);
        if (found) {
          emailWritten = found;
          await tx.prospect.updateMany({
            where: { id: prospectId },
            data: { email: found.email, emailSource: found.emailSource },
          });
        }
      }
    }
