 ...
 */
//
// `demoBooking` joined on 2026-09-11 with the one-hour walkthrough
// (lib/sales/nextSteps.js): a booking on FieldQuo's OWN demo calendar, for a
// company that has signed up and finished onboarding, made by the rep who
// sold it. It is the same row the homepage writes for a stranger, on the
// same calendar, and it is a smaller grant than that public route makes —
// this one needs a signed-in rep, a lead linked to a real company, and the
// company's own checklist complete. It moves no milestone, mints nothing,
// and decides no money; the specialist's calendar is the only thing it
// touches. The route's WHERE and the walkthrough gate carry the per-row
// boundary, as salesRepId does for SalesEvent.
export const REP_CALENDAR_WRITES = ["salesEvent", "demoBooking"];
...
export async function requireCalendarRep(request) {
  ...
  const row = await db.salesRep.findUnique({
    ...
    select: {
      ...
      name: true,
      workEmail: true,
      // The name the prospect knows the rep by (lib/sales/repIdentity.js) —
      // the walkthrough heads-up tells the specialist both names.
      workName: true,
      active: true,
      endedAt: true,
      acceptedAt: true,
