...
export async function leadContactSnapshot(leadId, salesRepId) {
  ...
  const lead = await db.salesLead.findFirst({
    ...
    select: {
      // The id comes BACK from this rep-scoped read, so the caller links the
      // event to a lead the database proved is theirs — never to the id the
      // browser sent, even when the two are equal. That is the difference
      // between a foreign key a check can prove and one it cannot.
      id: true,
      businessName: true,
      contactName: true,
      phone: true,
