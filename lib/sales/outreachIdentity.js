...
import { codeFromName } from "./repCode.js";
import { repPublicName } from "./repIdentity.js";
 ...
 *
 * @param {{ name?: string|null, workName?: string|null, email?: string|null, workEmail?: string|null }} rep
...
export function resolveSendingIdentity(rep, sendingDomain) {
  ...
  // usable, falling back to a slug of the name the prospect knows them by —
  // never to a shared address, which would make two reps indistinguishable in
  // a prospect's inbox. The fallback used to be SalesRep.code, which is the
  // rep's REAL name slugged; a rep working as "Daniel" would have sent from
  // jesus-perez@. The reply mailbox itself is bought and named by the owner,
  // so it should be named after the work name too — that is his to choose.
  const publicName = repPublicName(rep);
  const local = localPartOf(reply) || (publicName ? localPartOf(codeFromName(publicName)) : null);
  if (!local) {
    return {
      ok: false,
      from: null,
