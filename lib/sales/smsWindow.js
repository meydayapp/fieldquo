...
import { WINDOW_MODES, WINDOW_MODE_ENFORCE, WINDOW_MODE_WARN } from "./callingRules";

/**
 * The platform console's override for the prospect's jurisdiction, as
 * resolved by lib/sales/windowPolicy.js — the SAME row that relaxes the
 * calling window relaxes the texting window, because the owner's control is
 * "this state, this hard", not one switch per channel. Read here rather than
 * re-resolved: this file has no jurisdiction table and must not grow one.
 * Anything unrecognised is enforce.
 */
function modeOf(windowPolicy) {
  const mode = windowPolicy && typeof windowPolicy === "object" ? windowPolicy.mode : null;
  return WINDOW_MODES.includes(mode) ? mode : WINDOW_MODE_ENFORCE;
}
...
export function describeSalesSmsWindow() {
  ...
  return (
    ...
    `every day, in the lead's own time zone`
  );
}
 ...
 * ══ The override ══════════════════════════════════════════════════════════
 *
 * `windowPolicy` is the console's resolved answer for the prospect's state.
 * Under `off` the window is not applied and the answer is allowed with
 * `override: "off"`, so the screen can say so; under `warn` the answer is
 * allowed, `override: "warn"`, and `warning` carries the sentence the refusal
 * would have been. Under `enforce` (or with no policy) nothing here changed.
 * An unknown time zone is treated the same way as an out-of-hours minute: a
 * window that is not applied has no clock to read.
 *
 * @returns { allowed, reason, retryLater, override, warning }
 */
export function withinSalesSmsHours(now = new Date(), timeZone = null, { windowPolicy = null } = {}) {
  const mode = modeOf(windowPolicy);
  const relaxed = mode !== WINDOW_MODE_ENFORCE;
  const pass = (warning) => ({
    allowed: true,
    reason: null,
    retryLater: false,
    override: relaxed ? mode : null,
    warning: mode === WINDOW_MODE_WARN ? warning || null : null,
  });
  ...
  if (!local) {
    ...
    if (relaxed) return pass(reason);
    return { allowed: false, reason, retryLater: false, override: null, warning: null };
  }
  ...
  if (
    ...
    const reason = `Outside the texting window where they are (${describeSalesSmsWindow()}).`;
    if (relaxed) return pass(reason);
    return { allowed: false, reason, retryLater: true, override: null, warning: null };
  }
...
/**
 * The window as a screen tag: open until when, or closed until when.
 *
 * For the header of a conversation — "Open until 9:00 PM CDT" / "Closed —
 * opens 8:00 AM" — derived from SALES_SMS_WINDOW rather than retyped, so the
 * tag and the refusal can never disagree about the hours. With no zone there
 * is nothing to evaluate and `known` is false; the screen says so rather
 * than guessing from an area code.
 *
 * `until` is the boundary instant: the close when open, the next opening
 * when closed. Computed from the minutes left in the local day, which is
 * exact except across a daylight-saving change on the same day — an hour
 * out, once a year, on a tag, is accepted; the SEND is still judged by
 * withinSalesSmsHours at the moment it happens.
 *
 * `override` is the console's mode when it is not enforce, so the tag can say
 * "closed — warn only" or "closed — window off" instead of a plain "closed"
 * beside a composer that will send. The clock facts are unchanged by it:
 * `open` is still whether the hours are inside the window, because that is
 * the fact the rep is being warned about.
 *
 * @returns { known: false } | { known: true, open: boolean, until: Date, timeZone, override }
 */
export function salesSmsWindowState(now = new Date(), timeZone = null, { windowPolicy = null } = {}) {
  const mode = modeOf(windowPolicy);
  const override = mode === WINDOW_MODE_ENFORCE ? null : mode;
  const local = localTimeIn(timeZone, now);
  if (!local) return { known: false, open: false, until: null, timeZone: timeZone || null, override };
  const minute = local.minute;
  const { startMinute, endMinute } = SALES_SMS_WINDOW;
  const MIN = 60 * 1000;
  if (minute >= startMinute && minute < endMinute) {
    return { known: true, open: true, until: new Date(now.getTime() + (endMinute - minute) * MIN), timeZone, override };
  }
  // Before opening today, or after closing: the next opening is today's or
  // tomorrow's start.
  const untilOpen = minute < startMinute ? startMinute - minute : 24 * 60 - minute + startMinute;
  return { known: true, open: false, until: new Date(now.getTime() + untilOpen * MIN), timeZone, override };
}
