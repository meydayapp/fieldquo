 ...
 *
 * @returns { known: false } | { known: true, open: boolean, until: Date, timeZone }
 */
export function salesSmsWindowState(now = new Date(), timeZone = null) {
  const local = localTimeIn(timeZone, now);
  if (!local) return { known: false, open: false, until: null, timeZone: timeZone || null };
  const minute = local.minute;
  const { startMinute, endMinute } = SALES_SMS_WINDOW;
  const MIN = 60 * 1000;
  if (minute >= startMinute && minute < endMinute) {
    return { known: true, open: true, until: new Date(now.getTime() + (endMinute - minute) * MIN), timeZone };
  }
  ...
  return { known: true, open: false, until: new Date(now.getTime() + untilOpen * MIN), timeZone };
}
