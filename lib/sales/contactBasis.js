...
export const SOURCE_CONTACT_BASIS = {
  ...
  // ══ A Google Maps listing the owner's Mac read ════════════════════════════
  //
  // lib/sales/intel/promoteListings.js writes a Prospect from an
  // ExternalListing (source google_maps) the matcher could not attach to any
  // register row. The phone is the one the business itself put on its public
  // listing for customers to ring; the same reasoning as Overture for a live
  // call, and the same `undetermined` for the two electronic channels, on
  // which nobody has yet taken a position. Named here so the card says
  // "Google Maps listing" and not "google_maps" at a rep about to dial.
  google_maps: {
    label: "Google Maps listing",
    email: { state: "undetermined", reason: "Nobody has established what consent, if any, a Google Maps listing carries." },
    sms: { state: "undetermined", reason: "Nobody has established what consent, if any, a Google Maps listing carries." },
    phone: {
      state: "permitted",
      reason:
        "The number is the one the business published on its own Google listing for customers to ring. " +
        "A business-to-business call is exempt from the National DNCL; FieldQuo's internal list and the " +
        "calling hours still apply.",
    },
  },
  // ══ The three US licence boards ═══════════════════════════════════════════
  //
  // Named here so the prospect screen can say where a row came from. Without an
  // entry, `contactBasisFor` returns a null label and the screen prints the raw
  // provider key — "us_ca_cslb" — at a rep who is about to phone a stranger.
  //
  // ── The channel states are deliberately `undetermined`, not `permitted` ───
  //
  // The RBQ and Overture entries above rest on CASL, which is Canadian law and
  // reaches none of these. The US framework is a different statute per channel
  // — TCPA for calls and texts, CAN-SPAM for email, plus each state's own
  // telemarketing act — and CAN-SPAM in particular runs the OPPOSITE way to
  // CASL: it is opt-out, so the reasoning that prohibits emailing an RBQ
  // address does not transfer. Writing "permitted" here on that reasoning
  // would be an inference from the wrong country's law.
  //
  // `undetermined` blocks nothing. It is the honest state for a question
  // nobody has answered, and lib/sales/contactBasis.js's own header says so.
  // What it must NOT be read as is a gap: the calling gate is a separate
  // mechanism and it is fully populated — lib/sales/callingRules.js now
  // carries 49 states plus DC, so a call is already governed whatever this
  // says.
  //
  // ── Email is close to moot for these three, and that is measured ─────────
  //
  // The extractor's own counts: CSLB publishes an email on 0% of 219,255
  // licences, Washington L&I 0% of 75,917, Oregon CCB 0% of 45,483. Whatever
  // position is eventually taken on emailing a US public record, these three
  // sources supply almost nothing to email.
  us_ca_cslb: {
    label: "California CSLB licence register",
    email: {
      state: "undetermined",
      reason:
        "Nobody has established what CAN-SPAM and California law permit for an address taken from the " +
        "CSLB register. The register publishes an email on 0% of its 219,255 active licences, so this " +
        "is close to moot in practice.",
    },
    sms: {
      state: "undetermined",
      reason:
        "Nobody has established what the TCPA and California's own telemarketing act permit for a text " +
        "to a number from the CSLB register. Call instead — the register is a phone source, 99.9% complete.",
    },
    phone: {
      state: "undetermined",
      reason:
        "Whether a call is allowed is decided by lib/sales/callingRules.js, which reads the state's own " +
        "telemarketing law and refuses when it has not read one. This row does not widen or narrow that.",
    },
  },
  us_wa_lni: {
    label: "Washington L&I contractor register",
    email: {
      state: "undetermined",
      reason:
        "Nobody has established what CAN-SPAM and Washington law permit for an address from the L&I " +
        "register. It publishes an email on 0% of its 75,917 active licences.",
    },
    sms: {
      state: "undetermined",
      reason:
        "Nobody has established what the TCPA and RCW 19.158 permit for a text to a number from the L&I " +
        "register. Note that RCW 19.158.050 registration is recorded as outstanding — see callingRules.js.",
    },
    phone: {
      state: "undetermined",
      reason:
        "Decided by lib/sales/callingRules.js, which also carries Washington's outstanding registration " +
        "warning. This row neither widens nor narrows it.",
    },
  },
  us_or_ccb: {
    label: "Oregon CCB contractor register",
    email: {
      state: "undetermined",
      reason:
        "Nobody has established what CAN-SPAM and Oregon law permit for an address from the CCB register. " +
        "It publishes an email on 0% of its 45,483 active licences.",
    },
    sms: {
      state: "undetermined",
      reason:
        "Nobody has established what the TCPA and ORS 646.551 permit for a text to a number from the CCB " +
        "register.",
    },
    phone: {
      state: "undetermined",
      reason:
        "Decided by lib/sales/callingRules.js. This row neither widens nor narrows it.",
    },
  },
};
