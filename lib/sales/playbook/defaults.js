...
//   - Gong's cold-call data (100k calls in 2019, 300M in 2024) and Cognism's
//     guides, read on 2026-09-17 — docs/sales/RESEARCH-cold-calling-2026.md
//     has every number with its source. What they settled for THESE four
//     scripts: the permission-based opener is the measured best (11.18%)
//     and "did I catch you at a bad time" the measured worst (2.15%), so the
//     shared OPENER stands; stating the reason is 2.1x, and the PIVOT states
//     it inside the first minute on the contractor's own answer; the close
//     gets the invite sent while he is still on the line (CLOSE_ASK); and
//     the 70/30 talk ratio Cognism recommends is for the fifteen minutes,
//     not the cold call — Gong's successful cold calls have the REP talking
//     fifty-five per cent, in bursts of about thirty-five seconds, which is
//     what the fit stage is written to be.
//
...
import { TURNAROUND_DISCOVERY, TURNAROUND_FIT, TURNAROUND_PAIN } from "./turnaround";
import { RECORDING_ASIDE } from "./recordingDisclosure";

// ══ The trade's own three, in every fit stage ═════════════════════════════
//
// The owner, 2026-09-19: "each trade should have the selling points for
// that". The four playbooks are chosen by what the crawler SAW (a
// competitor, no website, no booking, an email-only quote request), not by
// trade, so the fit stage argued the observed gap and said nothing a roofer
// would recognise as roofing. This prompt, the same string in all four, names
// the three things lib/sales/tradeSellingPoints.js sells to the prospect's
// trade — resolved at render time through `{tradePitch}`, so the words live
// in one file and a refresh of the built-ins carries the LINE, not thirty-
// nine copies of the list. A prospect with no trade refuses this one prompt
// (renderLine's rule) and the fit line above it still reads.
export const TRADE_PITCH_LINE = "And for your trade, the three that matter most: {tradePitch}.";
...
export const PLAYBOOK_VARS = Object.freeze([
  ...
  "competitor",
  "repName",
  // The three things FieldQuo sells to THIS prospect's trade, as one spoken
  // clause — lib/sales/tradeSellingPoints.js tradePitchClause(). Resolved at
  // render from Prospect.tradeKey, so the four playbooks carry one line and
  // thirty-nine trades read their own. Null for a prospect with no trade,
  // which refuses the prompt that names it and nothing else.
  "tradePitch",
]);
...
//
// ══ The turnaround question, in all four ══════════════════════════════════
//
// The owner, 2026-09-14: does the pitch ask how long it takes from the
// homeowner reaching out to the quote being in their hands? It did not. Now
// every discovery stage asks it, every pain stage asks how often somebody
// else's quote got there first (their number, not ours), and every fit stage
// answers the number they gave with the one product claim the owner wants
// stated against it. The three sentences are constants in turnaround.js,
// with their French and Spanish, and imported here — see that file's header
// for why they are one string each and not four.
//
...
/**
 * The close's first three sentences, the same in all four.
 *
 * The alternative choice first (Futrell), then Gong's closing question —
 * "do you have your calendar handy?" is the one their 100k-call analysis
 * names as the question that secures next steps, because it is not one a
 * buyer can easily dodge and it is not aggressive — and then the invite,
 * sent WHILE THEY ARE STILL ON THE PHONE. Gong: it is harder to decline an
 * invitation you have already accepted than one you have not answered, so
 * get the meeting before you hang up. The product keeps the promise: the
 * call panel's next-step form books the demo and sends the .ics from the
 * rep's own mailbox (app/components/sales/NextSteps.js, through
 * /api/sales/events/[id]/invite), so "while we're on the phone" is a thing
 * the rep can actually do. The reminder the night before is the rep's own
 * text from the event on their calendar — nothing in the sales portal sends
 * on a timer by decision (SalesCheckIn's schema header).
 */
export const CLOSE_ASK =
  "What works better for you, mornings or afternoons? Have you got your calendar handy? " +
  "I'll send the invite while we're on the phone, and a reminder the night before.";

// The recording disclosure sits right after the name, before the candour
// line — the call is recorded (lib/sales/calls/recording.js) and the rep is
// the one who says so. lib/sales/playbook/recordingDisclosure.js has the
// reasoning and the French and Spanish; this is the English, once, for all
// four playbooks.
export const OPENER =
  `Hi — is that {businessName}? {repName} here, from FieldQuo — ${RECORDING_ASIDE.en}. ` +
  `${CANDOUR}, and you've never heard of me. ${PERMISSION_ASK}`;
    ...
    stages: {
      ...
      discovery: {
        ...
        prompts: [
          ...
          // Replaced "How far out are you booked — weeks, or months?": of the
          // three, the one whose answer nothing later in the script used.
      ...
      pain: {
        ...
        prompts: [
          ...
          // Replaced "And when one goes out late, what usually happens to
          // it?" — the same implication, now asked as the number of times
          // somebody else's quote got there first, so the answer is a figure
          // and it is theirs.
          TURNAROUND_PAIN,
        ],
      },
      ...
      //
      // ── And the turnaround, first, because it is the number they just gave ─
      //
      // Three claims now, and the order is the order of what they said: the
      // time they named comes first (TURNAROUND_FIT — the owner's sentence,
      // see turnaround.js), then the name, then the review. Three clauses
      // came out to keep the stage inside MAX_SAY, each of which restated the
      // sentence before it: "somebody holding three quotes cannot tell that
      // two were written in the same software", "on every quote", and "not
      // what other contractors charge … yours do not go out" — the last now
      // "your own jobs, nobody else's", which is the same fact
      // (non-negotiable #8) in five words.
      fit: {
        say:
          `Then three things, and they answer what you just said. First, the time you just gave me. ${TURNAROUND_FIT} ` +
          "Second, whose name is on it: the quote, the invoice, the booking page and every email " +
          "between them carry yours, and none of them carry ours. Third, what happens before it goes: " +
          "every quote gets read back to you, free — what you have left off, and whether the price " +
          "sits above or below what you have actually been winning at. Your own jobs, nobody " +
          "else's. Does the one you use now tell you that before you send it? And before you have " +
          "to raise it: nothing comes off {competitor} to look at this — nothing switched off, " +
          "nothing imported, nobody learning anything this week.",
        prompts: [TRADE_PITCH_LINE],
      },
    ...
    stages: {
      ...
      discovery: {
        ...
        prompts: [
          ...
          // Replaced "How far will you travel for a job worth having?": the
          // radius never fed the pitch; the turnaround does.
          TURNAROUND_DISCOVERY,
        ],
      },
      current_process: {
      ...
      pain: {
        ...
        prompts: [
          ...
          TURNAROUND_PAIN,
          // Kept, and last: the fit stage's first sentence points back at
          // the photos this question surfaces.
      ...
      fit: {
        say:
          ...
          "out of what you've already done, and I can show you one in fifteen minutes. And the " +
          `time you gave me a minute ago: ${TURNAROUND_FIT}`,
        prompts: [TRADE_PITCH_LINE],
      },
      objections: { say: "", prompts: [] },
    ...
    stages: {
      ...
      discovery: {
        ...
        prompts: [
          ...
          // Replaced "Who picks up when you're out on a job?": the gap it
          // pointed at — the wait between reaching out and hearing back —
          // is what the turnaround question measures, end to end.
          TURNAROUND_DISCOVERY,
      ...
      fit: {
        say:
          ...
          "raise: you keep the site you've got. It gains a button. It doesn't get rebuilt. And " +
          `once you're there, the time you gave me: ${TURNAROUND_FIT}`,
        prompts: [TRADE_PITCH_LINE],
      },
      objections: { say: "", prompts: [] },
      next_step: {
    ...
    stages: {
      ...
      discovery: {
        ...
        prompts: [
          ...
          // Replaced "Who else can see it — anybody, or just you?": the
          // weakest of the three, and the turnaround is the whole of this
          // playbook's case.
          TURNAROUND_DISCOVERY,
        ],
      },
      ...
      fit: {
        say:
          ...
          `email you still can. And the time you gave me: ${TURNAROUND_FIT}`,
        prompts: [TRADE_PITCH_LINE],
      },
      objections: { say: "", prompts: [] },
      next_step: {
