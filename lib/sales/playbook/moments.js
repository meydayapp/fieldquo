...
const MOMENTS = [
  {
    ...
    // ══ Who answers the phone at a plumbing company ═══════════════════════
    //
    // Not a receptionist. Cognism's own definition of a gatekeeper includes
    // "spouses and relatives", and at one to twenty people that is who it is:
    // the apprentice with the phone on speaker in the van, the office — which
    // is often the spouse or a bookkeeper — or the owner's partner at home.
    // Two of those three are not gatekeepers at all. The person who "just
    // does the paperwork" is frequently the person who types the quotes up in
    // the evening, which makes them the user this product is for and the one
    // conversation on the call worth having. So the lines below sort the
    // three before asking to be handed on, and the fourth line asks the
    // office the same question the opener asks the owner.
    //
    // Gong's 300M-call data puts a gatekeeper at minus thirty-nine per cent
    // on the odds of a meeting, and its advice is to dial the mobile and to
    // call outside office hours. The listing often carries a mobile; the
    // calling window's first and last slots are when an owner-operator
    // answers his own phone. That is a note to the rep, not a script line.
    //
    // ══ The one Cognism move that is deliberately NOT taken ══════════════
    //
    // Their script answers "what is it regarding?" with "I'm just following up
    // on an email" or "chasing up on some emails", and one of their gatekeeper
    // talk tracks fabricates a prior visit ("how was your son's football
    // match?"). Both work, and both are lies: there is no email and there was
    // no visit. A rep who says it and is put through to somebody who asks
    // which email has lost the call and the relationship in one sentence, and
    // FieldQuo would be a company whose reps open by inventing a history.
    // Everything else in their gatekeeper advice — brevity, the first name,
    // ask for help, no pitch to the person who cannot buy — is taken as
    // written. The first name is asked for rather than pretended.
    lines: [
      {
        label: "Opening",
        text:
          "Morning — it's {repName} from FieldQuo. Who's the best person to talk to about how " +
          "the quotes go out — is that the owner, or is that you?",
      },
      {
        label: "If they ask what it is regarding",
        text:
          "It's about how a quote gets from the driveway to the customer. I've not spoken to him " +
          "before — I read your website this morning and rang off the back of it. Is he the one " +
          "who prices the work, or is that somebody else?",
      },
      {
        label: "The apprentice, or somebody in the van",
        text:
          "No problem. When's he easiest to catch — first thing before you're on site, or the end " +
          "of the day? And what's his name, so I'm not asking for the boss next time?",
      },
      {
        label: "The office, or the spouse who does the paperwork",
        text:
          "Then you might be the one I should be talking to. When a quote goes out, is it you " +
          "typing it up, or does it land on him in the evening?",
      },
      {
