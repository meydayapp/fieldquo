...
import { tradePitchClause } from "@/lib/sales/tradeSellingPoints";
...
export function playbookVars({ prospect = {}, index = {}, rep = null } = {}) {
  ...
  return {
    ...
    // The name the rep says on the call — work name, else first name
    // (lib/sales/repIdentity.js). Was the full real name, which the opener
    // "{repName} here, from FieldQuo" then read out in full.
    repName: repPublicName(rep) || rep?.firstName || null,
    // English, like every other rules line: the playbook is read verbatim.
    // The rep's-language version of the same three, with their proofs, is
    // the block CallPlaybook draws under the script (TradePoints).
    tradePitch: tradePitchClause(prospect?.tradeKey || null, "en"),
  };
}
...
export function buildCallScript({
  ...
  const stages = ordered.map((row) => {
    ...
    return {
      ...
      // The catalogue key for the name, so the rep reads "Why them" in their
      // own language. The lines below it stay verbatim — see stages.js.
      nameKey: meta?.nameKey || null,
      // The author's note. Collapsed on the rep's screen, open on the
      // platform console. Never the thing a rep reads first.
