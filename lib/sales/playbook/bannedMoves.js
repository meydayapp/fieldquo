...
// argument.
//
// ══ What the call data confirmed, added, and refused to add (2026-09-17) ═══
//
// docs/sales/RESEARCH-cold-calling-2026.md reads Gong's 100k- and 300M-call
// analyses and Cognism's guides against this table:
//
//   CONFIRMED  "bad-time question" — Gong's own number is on the entry now:
//              2.15% success against 11.18% for a permission-based opener,
//              and "40% less likely to book" in the 2019 set. Saylor and Gong
//              agree, from opposite ends.
//   ADDED      "buzzword" — a pitch built on "all-in-one platform" language
//              books at 5.5% in Gong's data; problem language at 16%. The
//              first is a marketing sentence, the second is a sentence about
//              their week. Business names are the trap: "Liberty Seamless
//              Gutters" is a real prospect, so "seamless" on its own is not
//              in the pattern — only "seamlessly".
//   NOT ADDED  "how are you" / "how've you been". Saylor names it insincere
//              from a stranger; Gong's 2019 data has "how've you been" at
//              6.6x the baseline and their 2024 data has "how's it going"
//              at 7.6% — worse than permission, far better than "bad time".
//              The data will not support a ban, so there is none. It is
//              simply not scripted, because the permission opener measured
//              higher and because it claims an acquaintance a cold caller
//              does not have. The model prompt says so in its own words.
...
export const BANNED_MOVES = Object.freeze([
  ...
  },
  {
    ...
    why:
      ...
      "Gong, 300M calls: 2.15% success for this opener against 11.18% for a permission-based " +
      "one; their 2019 set puts it at 40% fewer meetings booked.",
    pattern:
  ...
  },
  {
    move: "buzzword",
    why:
      "Gong, 300M calls: a pitch built on buzzwords ('all-in-one platform', 'single source of " +
      "truth', 'revolutionize') books at 5.5%; a pitch in problem language books at 16%. A " +
      "contractor hears the first as a brochure. Say the problem in his own words instead.",
    // "seamless" alone is not here: it is in prospects' business names
    // ("Liberty Seamless Gutters"), and an opener that says the business's
    // name back to them is required, not banned. "platform" and "solution"
    // are kept out of the regex too — the model prompt bans them in words,
    // but an objection LABEL says "another platform" and a sweep that failed
    // on the label a rep scans for would be fighting the wrong copy.
    pattern:
      /\b(?:all[- ]in[- ]one|single source of truth|revolutioni[sz](?:e|es|ed|ing)|game[- ]chang(?:er|ing)|seamlessly|end[- ]to[- ]end|cutting[- ]edge|best[- ]in[- ]class|next[- ]generation|streamlin(?:e|es|ed|ing)|synerg(?:y|ies|istic))\b/i,
  },
  {
