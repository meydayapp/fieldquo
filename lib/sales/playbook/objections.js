...
const OBJECTIONS = [
  ...
  },
  {
    ...
    //
    // It used to offer "if you want it all back, or gone, that's a written
    // request we act on". "Back" is gone: companies can import but not export
    // (owner, 2026-09-24; lib/export/companyDataExport.js), so the answer
    // says so before the prospect finds out after signing. What does still
    // download is the single document — a quote, an invoice, a payslip PDF.
    // A COPY on written request stays: privacy law (Québec Law 25, PIPEDA,
    // GDPR access and portability requests) entitles a company to one, and
    // FieldQuo provides it by hand. That is not a self-serve export, and the
    // answer must not make it sound like one.
    response:
      ...
      "gets read is the review that runs on your own quotes, and it reads only yours. You " +
      "should hear this from me now: your list comes in easily, but there's no button that " +
      "exports it all back out. Quotes, invoices and payslips download as PDFs, one at a time. " +
      "If you want a copy of your data, or want it gone, that's a written request we act on, " +
      "not a button that quietly does " +
      "half of it. Tell me what you'd need to see in writing and I'll send exactly that.",
  },
  {
    ...
    // integration, and since 2026-09-24 no bookkeeping export either — it used
    // to offer "an export of the invoices and the job costs", and companies can
    // now import but not export (lib/export/companyDataExport.js). A rep who
    // lets a contractor discover either after signing has sold the one thing
    // that gets a refund demanded. Futrell: raise it first and the prospect has
    // nothing left to catch you on.
    response:
      ...
      "before you find it yourself: there's no link that pushes invoices into it, and there's " +
      "no export file to hand them either. What they can have is each invoice as a PDF, one at " +
      "a time. If a monthly file for your bookkeeper is a must, you should know that now, not " +
      "after you've signed. Tell me how they get the numbers today and I'll tell you straight " +
      "whether this changes that.",
  },
