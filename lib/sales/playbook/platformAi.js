...
export async function recordPlatformAiUsage({
  ...
  salesRepId = null,
  ref = null,
  // A caller whose spend is not in tokens — transcription is billed by the
  // audio minute — hands the figure it computed. Tokens stay zero, the
  // budget still sees the money.
  costMicros: costMicrosGiven = undefined,
}) {
  ...
  const data = {
    ...
    completionTokens,
    totalTokens,
    costMicros: Number.isFinite(costMicrosGiven)
      ? Math.round(costMicrosGiven)
