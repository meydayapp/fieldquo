 ...
 *
 *   salesQueueClaim  the claim LOG — one row per claim this rep took, closed
 *             with when and why it ended. lib/sales/queueBatch.js writes it
 *             beside every lease write, and it is what the per-rep daily
 *             ceiling and the seven-day "not the same hundred tomorrow" rule
 *             are counted from. Never deleted; a rep can only add rows about
 *             their own claims and close their own.
 *
 * scripts/check-prospect-ui.mjs asserts the queue route writes to these
 * models and no other, and scripts/check-sales-batch-claim.mjs holds
 * queueBatch.js to the same list.
 */
export const REP_QUEUE_WRITES = ["prospect", "salesQueueClaim"];
...
export async function requireQueueRep(request) {
  ...
  const row = await db.salesRep.findUnique({
    ...
    select: {
      ...
      email: true,
      name: true,
      // The name the rep introduces themselves by — the script's {repName}
      // and the signup opener (lib/sales/repIdentity.js repPublicName).
      workName: true,
      active: true,
      endedAt: true,
      acceptedAt: true,
      kind: true,
      // The languages they sell in. Every claim this gate fronts passes the
      // rep into claimCandidateWhere(), which reads this column to keep a
      // Quebec row away from a rep without French — lib/sales/leadLanguage.js.
      sellsIn: true,
