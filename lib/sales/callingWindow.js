 ...
 * clock, and lib/sales/callingRules.js's nextOpening() walks up to eight days
 * in five-minute steps across every candidate zone to answer "when does this
 * open" — thousands of reads for one screen. Caching the formatter is
 * behaviour-preserving: a formatter is stateless once built, and a bad zone
 * still throws out of the constructor on every call because only successes are
 * cached.
 */
