...
//
// ══ Rule 8: the banned moves, on the generated script too ═════════════════
//
// lib/sales/playbook/bannedMoves.js is the one table of what a FieldQuo line
// may never say — the seed scripts, the objection library, the battlecards
// and the moments are all swept with it. The AI script was not. A generated
// opener that asked "did I catch you at a bad time?" (Gong, 300M calls:
// 2.15% against 11.18% for the permission opener) passed every lint here and
// was read out. So every spoken sentence is run through bannedMovesIn() and
// a hit is a finding, quoted back on the retry with the move named. Two
// fields are exempt: `objections[].they`, which is the PROSPECT's sentence,
// and `doNotSay`, which is a list of things not to say and may quote one.
// The table is English; a French or Spanish script passes through it
// untouched, which is a gap named here rather than a rule pretended.
//
 ...
 */
export const SPOKEN_FIELDS = Object.freeze(["opener", "ifSomeoneElseAnswers", "whatWeSaw", "whyThemNow", "threeQuestions", "objections", "closeAsk"]);
...
export const VOICE_PROBLEMS = Object.freeze({
  ...
  banned_move: "a sentence made a move the playbook bans — see lib/sales/playbook/bannedMoves.js",
});
...
const FR_AUX = [
  ...
  "vaut", "valent", "vaudrait", "vaudra", "suffit", "suffisent", "ressemble", "ressemblent", "permet", "permettent", "permettrait",
  "évite", "évitez", "évitent", "revient", "reviennent", "revenez", "reviens", "fonctionne", "fonctionnent",
  "engage", "engagez", "embauche", "embauchez", "rentre", "rentrez", "rentrent", "garde", "gardez", "gardent",
  "voyons", "regardons", "parlons", "faites", "prenez", "réfléchissez", "décidez", "comptez", "restez", "attendez",
  "signez", "testez", "essayez", "installez", "changez", "perdez", "gagnez", "manquez", "ratez", "oubliez",
  "installe", "installent", "gagne", "gagnent", "rate", "ratent", "oublie", "oublient", "tourne", "livre", "livrent",
  "merci",
];
...
export function hasFiniteVerb(sentence, { language = "en" } = {}) {
  ...
  // Interjections: two words in English ("No problem."); three in French
  // and Spanish, where the same interjection takes an article or a
  // negation ("Pas de problème.", "Claro que sí.", "Con mucho gusto.").
  if (tokens.length <= (lang === "en" ? 2 : 3)) return true;
 ...
 */
export function lintSentence(sentence, { spoken = true, language = "en", ask = false, banned = spoken } = {}) {
  const problems = [];
  const s = String(sentence || "");
  ...
  if (banned && bannedMovesIn(s).length) problems.push("banned_move");
  return problems;
}

/** Which banned moves a sentence makes, by name — for the retry note. */
export function bannedMovesInSentence(sentence) {
  return bannedMovesIn(String(sentence || ""));
}
...
export function voiceLint(script, { sources = [], ignoreWords = [], language = "en" } = {}) {
  ...
  const walk = (value, path) => {
    ...
    if (typeof value === "string") {
      ...
      // The prospect's own sentence is not the rep's; a rep is allowed to be
      // TOLD "are you busy?" — see rule 8 in the header.
      const banned = spoken && !/\.they$/.test(path);
      for (const sentence of splitSentences(value)) {
        const problems = lintSentence(sentence, { spoken, language, ask, banned });
        if (problems.length) {
          const finding = { field: path, sentence, problems };
          if (problems.includes("banned_move")) finding.moves = bannedMovesInSentence(sentence);
...
export function voiceRetryNote(lint, { limit = 6, long = [] } = {}) {
  const lines = (lint?.findings || []).slice(0, limit).map((f) => {
    ...
    const moves = Array.isArray(f.moves) && f.moves.length ? ` (${f.moves.join(", ")})` : "";
    return `- ${f.field}: "${f.sentence}" — ${why}${moves}`;
  });
  ...
  return [
    ...
    "No banned move: no apology, no promise about your own time, no bad-time question, no question one word ends, no invented scarcity, no other customer, no flattery, no line that ends the sequence, no buzzword.",
