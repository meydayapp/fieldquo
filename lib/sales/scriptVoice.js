...
export const VOICE_PROBLEMS = Object.freeze({
  too_long: `a sentence ran past ${MAX_WORDS} words`,
...
export function lintCitations(script, sources = [], { ignoreWords = [] } = {}) {
  const findings = [];
  const material = (Array.isArray(sources) ? sources : []).map((s) => ({ url: s?.url || "", text: squeeze(s?.text) })).filter((s) => s.text);
  ...
  for (const c of citations) {
    ...
    const inSource = material.some((m) => m.text.includes(quote));
    if (!inSource) {
...
export function lintSentence(sentence) {
  ...
  if (wordCount(s) > MAX_WORDS) problems.push("too_long");
  ...
  if (!hasFiniteVerb(s)) problems.push("no_finite_verb");
 ...
 */
export function voiceRetryNote(lint, { limit = 6 } = {}) {
