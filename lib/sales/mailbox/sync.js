...
import { chooseThread, matchLead } from "./threading";
...
const LEAD_SELECT = { id: true, email: true, phone: true, prospect: { select: { email: true } } };
...
/** Threads with no lead whose counterpart is now one of the rep's leads. */
export async function claimUnmatchedThreads(db, salesRepId, leads) {
  const byEmail = new Map();
  for (const l of leads) {
    for (const a of [l.email, l.prospect?.email]) {
      const k = String(a || "").trim().toLowerCase();
      if (!k) continue;
      // Two leads at one address: nobody claims (see matchLead).
      byEmail.set(k, byEmail.has(k) ? null : l.id);
    }
  }
  ...
  for (const t of orphans) {
    const leadId = byEmail.get(t.counterpart);
    if (!leadId) continue;
    await db.salesThread.updateMany({ where: { id: t.id, leadId: null }, data: { leadId } });
    n += 1;
  }
  return n;
}
