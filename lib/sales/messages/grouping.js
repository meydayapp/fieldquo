...
export function speakerOf(item) {
  if (!item) return null;
  // An explicit author wins, and this is what lets one component draw a
  // two-party text thread AND a staff channel with six people in it.
  //
  // Rocket.Chat's isMessageSequential breaks a group on `current.u._id !==
  // previous.u._id` — the AUTHOR, not the side. Direction alone is the same
  // answer only while there are exactly two parties: in a channel every
  // message from everybody else is "in", so four people talking in turn would
  // group into one block under whoever spoke first, with three of them
  // silently attributed to a name they did not say.
  //
  // SMS rows carry no `who` and keep the direction rule, which is exact for
  // them.
  if (item.who) return `who:${item.who}`;
...
export function groupThread(items, { windowSeconds = GROUPING_WINDOW_SECONDS, timeZone = null } = {}) {
  ...
  let previous = null;
  // The last DATED row of any kind, for the day-divider decision. Separate
  // from `previous` on purpose: a draft or a system row is not a grouping
  // anchor, but it does sit on a day, and judging the NEXT message's day
  // against null — the way the anchor is reset — emitted a second "Yesterday"
  // under every "Called · no answer" row. Seen in the rendered harness, not
  // in the check, which is why the check now drives this branch too.
  ...
  for (const item of ordered) {
    ...
    if (dated && startsNewDay(item, previousDated, timeZone)) {
