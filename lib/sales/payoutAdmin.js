...
import { repStaffLabel } from "./repIdentity";
 ...
 *
 * `payeeKind` and `names` (id → name) let the view say WHO earned each line
 * when the payee is a call-centre agency (lib/sales/agency.js): the batch
 * sits under the agency's id and its entries carry the employees' ids, so
 * the screen shows "Agency · 3 employees" and a line per earner. For every
 * other batch the earner is the payee and `byEarner` is a single line the
 * screen does not draw.
 */
export function batchView(batch, entries, repName = null, { payeeKind = null, names = null } = {}) {
  ...
  for (const e of lines) {
    const id = e.salesRepId || batch.salesRepId;
    if (!perEarner.has(id)) perEarner.set(id, []);
    perEarner.get(id).push(e);
  }
  const byEarner = [...perEarner.entries()]
    .map(([salesRepId, rows]) => ({
      salesRepId,
      name: (names instanceof Map ? names.get(salesRepId) : names?.[salesRepId]) || null,
      cents: balanceCents(rows),
  ...
  return {
    ...
    repName,
    payeeKind: payeeKind || "rep",
    isAgency: payeeKind === "agency",
    byEarner,
...
export async function payoutsOverview({ period = "week", now = new Date(), prisma = db } = {}) {
  const [reps, entries, batches] = await Promise.all([
    prisma.salesRep.findMany({
      select: { id: true, name: true, workName: true, active: true, kind: true },
  ...
  // The payroll screen names the PERSON paid, with the work name beside it
  // ("Jesus Pérez — works as Daniel") — a payout is made out to the real
  // name, and the owner knows the rep by both (lib/sales/repIdentity.js).
  const names = new Map(reps.map((r) => [r.id, repStaffLabel(r) || r.name]));
  const kinds = new Map(reps.map((r) => [r.id, r.kind || "rep"]));
  return {
    snapshot: owedSnapshot({ entries, batches, period, now }),
    table: periodTable({ reps, entries, batches, period, now }),
    batches: batches.map((b) =>
      batchView(b, entries, names.get(b.salesRepId) || null, { payeeKind: kinds.get(b.salesRepId) || "rep", names }),
    ),
  };
}
...
export async function repPayouts({ salesRepId, prisma = db } = {}) {
  ...
  const [entries, batches, rep] = await Promise.all([
    // The rep's own entries, AND every entry in a batch under this rep's id.
    // The two sets are the same for everyone except a call-centre agency,
    // whose batches gather its employees' entries (lib/sales/agency.js) —
    // reading by salesRepId alone showed an agency's batch summing to $0
    // beside a totalCentsAtClose that said otherwise.
    prisma.salesCommissionEntry.findMany({
      where: { OR: [{ salesRepId }, { payoutBatch: { salesRepId } }] },
      select: ENTRY_SELECT,
    }),
    ...
    prisma.salesRep.findUnique({ where: { id: salesRepId }, select: { kind: true } }),
  ]);
  const earnerIds = [...new Set(entries.map((e) => e.salesRepId).filter(Boolean))];
  const earners = earnerIds.length
    ? await prisma.salesRep.findMany({ where: { id: { in: earnerIds } }, select: { id: true, name: true } })
    : [];
  const names = new Map(earners.map((r) => [r.id, r.name]));
  return {
    batches: batches.map((b) => batchView(b, entries, null, { payeeKind: rep?.kind || "rep", names })),
    // Accruing = this rep's OWN unbatched entries. An agency's employees'
    // open entries accrue on their own cards; the agency's card sums them
    // at the close, when they become its batch.
    accruingCents: balanceCents(entries.filter((e) => !e.payoutBatchId && e.salesRepId === salesRepId)),
  };
}
