...
export function buildRepRows({
  ...
  const rows = reps.map((rep) => {
    ...
    return {
      id: rep.id,
      name: rep.name,
      // "Jesus Pérez — works as Daniel" for FieldQuo's own tables; `name`
      // stays the real name every other reader of this row already uses.
      staffLabel: repStaffLabel(rep),
      code: rep.code,
      agency: agencyOf(rep),
...
export const NOT_TRACKED = [
  ...
  // "Calls made, talk time, connect rate" was on this list until 2026-09-17,
  // with a reason that was true when written: no human calling path. There
  // has been one since 2026-09-14 — SalesCallAttempt, browser dials bridged
  // through Twilio — and the figures are now on the page, read through
  // lib/sales/calls/reporting.js's repCallStats/teamCallRows (the floor
  // board's own functions) by lib/sales/performanceReport.js. What that
  // module still refuses to print about calls is its own list,
  // NOT_TRACKED_CALLS, and travels under `calls.notTracked`.
  {
    key: "timeToClose",
...
export function buildSalesPerformance({
  ...
  // ── The one scope ───────────────────────────────────────────────────────
  //
  // null is everyone: the platform's page. An array of rep ids is a team:
  // the agency's page (app/api/sales/agency/performance), handed the ids its
  // route read fresh. Applied HERE to every input rather than by the caller
  // to some of them, so a scoped report's headline, rep table, funnel and
  // lead pipeline are all the same subset — a second builder for the agency
  // would be the copy that drifts (AGENTS.md failure class 4).
  repIds = null,
} = {}) {
  const scope = Array.isArray(repIds) ? new Set(repIds) : null;
  const keep = (id) => scope === null || scope.has(id);
  const reps = allReps.filter((r) => keep(r?.id));
  const attributions = allAttributions.filter((a) => keep(a?.salesRepId));
  const entries = allEntries.filter((e) => keep(e?.salesRepId));
  const batches = allBatches.filter((b) => keep(b?.salesRepId));
  const leads = allLeads.filter((l) => keep(l?.salesRepId));
  ...
  return {
    period: { from, to },
    scope: { repIds: scope === null ? null : [...scope] },
    headline: {
