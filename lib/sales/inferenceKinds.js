...
export const SITE_INFERENCE_KINDS = Object.freeze([
  ...
  "languages_spoken",
  "busy_season",
  // The eleventh, from 2026-09-14: the services they offer, in their own
  // words. Constrained harder than the others — the model is handed the
  // evidence-cited list (lib/sales/intel/servicesOffered.js) and every name
  // it answers must be a substring of that list or of the page text
  // (siteInference.js validateSiteInferences), so it cannot add "siding"
  // to a roofer who never mentioned it.
  "services_offered",
]);
...
/**
 * The evidence-cited service list lib/sales/intel/servicesOffered.js writes
 * from the crawler's rows — menu, sitemap, JSON-LD, WordPress index — as
 * ONE `ProspectInference` row whose value is a JSON list of
 * `{ name, source, sourceUrl, tradeKey, confidence }`.
 *
 * Four readers, on both sides of the client boundary: prospectView.js
 * renders it as the "Services they list" fact row, brief.js as a known
 * line, callScript.js as a prompt line, and inferFromSite.js hands the
 * names to the model as the fence its services_offered answer is checked
 * against. The kind and the parser live here so they cannot drift.
 */
export const SERVICES_INFERENCE_KIND = "services";

/** The names in a stored `services` value, in order, or [] for anything
 *  that is not the JSON list servicesOffered.js writes. */
export function servicesNamesFromValue(value) {
  try {
    const parsed = JSON.parse(String(value ?? ""));
    if (!Array.isArray(parsed)) return [];
    return parsed
      .map((s) => (s && typeof s === "object" && typeof s.name === "string" ? s.name.trim() : ""))
      .filter(Boolean)
      .slice(0, 25);
  } catch {
    return [];
