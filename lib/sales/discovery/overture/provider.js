...
import { boundedSnapshotCache } from "../snapshotCache";
 ...
 */
// Bounded, and one shape with the RBQ and board providers — see
// lib/sales/discovery/snapshotCache.js for why an unbounded Map here was a
// slow out-of-memory kill once eighty campaigns were running.
const cache = boundedSnapshotCache();
...
async function loadSnapshot(url, { fetchImpl = fetch } = {}) {
  ...
  // `notes` travel with the load so the campaign screen can print "the
  // manifest counts a bigger extract than this file" rather than the reader
  // silently trusting a count it has just disproved.
  const loaded = { manifest: parsed.manifest, rows: parsed.rows, unreadable: parsed.unreadable, notes: parsed.notes || [] };
  cache.set(url, loaded, parsed.rows.length);
  return loaded;
}
...
export function inTerritory(business, territory, { haversineKm } = {}) {
  ...
  /**
   * Two spellings of the same place, compared as places rather than as strings.
   *
   * ══ What the plain string compare was actually dropping ═════════════════
   *
   * Measured, not estimated, against the whole 2026-08-19.0 Overture places
   * release for the nine trade categories — 945,532 US and Canadian rows:
   *
   *   Ontario 67 · British Columbia 29 · Alberta 25 · Québec 15 · Quebec 5 ·
   *   Manitoba 4 · Saskatchewan 1 · Nova Scotia 1 · California 1 · Calif 2
   *
   * 150 rows. Small, and silently wrong in the one direction that looks like
   * nothing: a province-wide Ontario campaign skipped 67 businesses and
   * reported no error, because "Ontario" and "ON" are different strings.
   *
   * The lowercase forms — `ca`, `tx`, `On`, `Fl` and the rest, 60-odd rows —
   * were NEVER the bug. `text()` already folded case, so they matched. Saying
   * so here because "the region column is messy" invites a fix aimed at the
   * wrong half of the mess.
   *
   * ══ Why the fall-through is a literal compare and not a refusal ══════════
   *
   * `normaliseSubdivision` returns null for a string it does not recognise —
   * "Oakville" and "Vaughan" appear in the region column, and "De Wy" appears
   * once and means nothing. When either side fails to normalise there is no
   * place to compare, so the ORIGINAL literal compare stands. That keeps this
   * a strict widening: every row that matched before still matches, and the
   * unrecognisable rows behave exactly as they did rather than being newly
   * dropped or newly admitted.
   *
   * The normaliser is imported from lib/sales/callingRules.js rather than
   * copied. That file already owns the alias table because the calling gate
   * needs it, and a second copy here is AGENTS.md failure class #4 — the copy
   * is the one that rots, because it is the one nobody looks at. It also means
   * the territory filter and the jurisdiction lookup can never disagree about
   * which state a row is in.
   */
  const samePlace = (rowValue, filterValue, normalise) => {
    const a = normalise(rowValue);
    const b = normalise(filterValue);
    if (a && b) return a === b;
    return text(rowValue) === text(filterValue);
  };

  if (territory.country && !samePlace(address.country, territory.country, normaliseCountry)) return false;
  if (territory.province && !samePlace(address.province, territory.province, normaliseSubdivision)) return false;
