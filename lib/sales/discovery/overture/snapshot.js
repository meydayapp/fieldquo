...
export function readSnapshot(body) {
  const text0 = String(body ?? "");
  const lines = text0.split("\n");
  // A file cut in transit ends mid-line: no trailing newline, and the last
  // non-empty line does not parse. A complete file ends with "\n" and every
  // line is JSON. This is the fact that separates "the download broke" from
  // "the manifest counts a bigger extract than this file" — see below.
  const lastLine = lines.slice().reverse().find((l) => l.trim().length > 0) ?? "";
  let lastLineParses = false;
  try { JSON.parse(lastLine.trim()); lastLineParses = true; } catch { lastLineParses = false; }
  const endedCleanly = text0.endsWith("\n") && lastLineParses;
  let manifest = null;
  const rows = [];
  let unreadable = 0;
  let started = false;
  ...
  const notes = [];
  // The count is a claim the file makes about itself. When it disagrees with
  // what arrived there are two different stories, and the reader used to tell
  // only one of them:
  //
  //   · The download broke. The file ends mid-line. Refuse — accepting it
  //     would make a city look like it has 300 painters.
  //   · The file is complete and the manifest counts a BIGGER extract. That is
  //     what "overture-US-CA.part1/.part2" turned out to be: one 78,247-row
  //     extract split into 50,000 + 28,247, each part still carrying the
  //     whole extract's count. Both files were refused as "truncated in
  //     transit" for a day, 0 of 78,247 accepted, while every byte was there
  //     (downloaded and counted 2026-09-11). Accept the rows; carry the
  //     discrepancy as a note the campaign screen prints, so nobody is told
  //     the file is bigger than it is.
  if (!problems.length && manifest.count !== rows.length + unreadable) {
    if (!endedCleanly) {
      problems.push(
        `The snapshot says it holds ${manifest.count} rows and ${rows.length + unreadable} arrived, and the file ends mid-line — it was truncated in transit.`,
      );
    } else {
      notes.push(
        `The manifest says ${manifest.count} rows; this file holds ${rows.length + unreadable} and ends cleanly. Read as a part of a larger extract.`,
      );
    }
  }

  return { manifest, rows, unreadable, problems, notes, endedCleanly };
}
