...
export function funnelRows(campaign = {}) {
  ...
  return [
    ...
    },
    {
      ...
      note: "The same source record already held, refreshed in place rather than written twice — or a row a superadmin merged into another, which keeps carrying it.",
      kind: "drop",
    },
    {
      key: "rejected",
...
export function campaignProgress(campaign = {}) {
  const asked = Math.max(0, Math.floor(Number(campaign?.targetCount) || 0));
  const accepted = Math.max(0, Math.floor(Number(campaign?.acceptedCount) || 0));
  const found = Math.max(0, Math.floor(Number(campaign?.foundCount) || 0));
  // ── A part of a larger extract cannot reach the whole extract's target ──
  //
  // The Overture/CSLB library is split at 50,000 rows a file, and one target
  // is typed for the whole set — 78,247 for California, which is what the
  // manifest of every part says the EXTRACT holds. Part 2 holds 28,247 rows.
  // It read all of them, accepted 23,494, and the screen said "23494 of 78247
  // accepted (30%) · completed": a finished campaign reporting itself less
  // than a third done. The owner asked what happened; nothing had.
  //
  // Once the source has ENDED (every source ended, none blocked — the file was
  // read to its last line) the rows the file actually held are the only
  // target it could ever have met, so that is the denominator: 23,494 of
  // 28,247 (83%). While it is still reading, the typed target stands.
  const states = campaign?.sourceState && typeof campaign.sourceState === "object" ? Object.values(campaign.sourceState) : [];
  const ended = states.length > 0 && states.every((st) => st && st.ended === true && !st.blocked);
  const target = ended && found > 0 ? Math.min(asked, found) : asked;
  if (!target) return { target: 0, accepted, percent: null };
  return {
    target,
    accepted,
    percent: Math.min(100, Math.round((accepted / target) * 100)),
    // Said on the screen when the two differ, so "of 28,247" beside a
    // campaign created with 78,247 is explained rather than puzzling.
    askedFor: asked !== target ? asked : null,
  };
}
