...
export function toDiscoveredBusiness(licence = {}, board = null, { release = null, sourceUrl = null } = {}) {
  ...
  return {
    ...
    // ── The person on the licence ─────────────────────────────────────────
    //
    // Washington's PrimaryPrincipalName, Oregon's rmi_name (the responsible
    // managing individual); null on California, whose master file names
    // nobody and whose PERSONNEL file lib/sales/intel/registerPeople.js
    // reads instead. Read by normalise.js into a field BESIDE the prospect,
    // the way derivedWebsite travels, and written by the ingest as a
    // ProspectPerson row with the board as its source — never a Prospect
    // column, so nothing can spread it into one.
    principal: licence.principal || null,
