...
import { recordBoardPrincipal } from "@/lib/sales/intel/registerPeople";
import { db } from "@/lib/db";
import { classifyBusiness } from "./classify";
import { licenceRegisterName, licenceRegisterReason } from "./licenceRegisters";
import { AUTOFILL_VIAS, buildDedupeIndex, matchExisting } from "./dedupe";
import { MERGE_SELECT, applyFills, planFills } from "./mergeProspects";
 ...
 *        `licenceRegister` is the register's name when `provider` is a
 *        contractor-licence register (lib/sales/discovery/licenceRegisters.js)
 *        and null for a directory. Passed in rather than looked up so this
 *        function stays pure.
...
export function planIngest(businesses = [], context = {}, index = null) {
  ...
  for (const business of Array.isArray(businesses) ? businesses : []) {
    ...
    let verdict = classifyBusiness(business);
    // ── A licence register answers the question the classifier cannot ────
    //
    // classify.js's tier E — "Nothing in the source says whether this is a
    // contractor or a shop" — is the honest verdict for a points-of-interest
    // row and a false one for a row read out of a register of ACTIVE
    // CONTRACTOR LICENCES: the source says exactly that, and it is the
    // regulator saying it. Measured 2026-09-11: 39,654 Quebec licence-holders
    // sat in needs_review with that sentence on them and no rep could ever
    // reach one. So needs_review becomes contractor here, with the register
    // named in the reason. ONLY needs_review: a retailer verdict is the NAME
    // saying supply / wholesale / lumber, and a building-supply yard that
    // also holds a licence is still not a call a rep should make. The trade
    // is NOT touched — see licenceRegisters.js; the row banks with
    // `tradeKey: null` exactly as before and waits for a human or a crawl.
    if (context.licenceRegister && verdict.classification === "needs_review") {
      verdict = {
        classification: "contractor",
        reasons: [...verdict.reasons, "licence_register"],
        reason: licenceRegisterReason(context.provider) || `Holds a contractor licence (${context.licenceRegister}).`,
      };
    }
    ...
    if (match.action === "update") {
      ...
      if (match.fillOnly) {
        // The record's own row was RETIRED by a merge; the survivor is the
        // row that is live. A refresh of the survivor's columns from THIS
        // source would overwrite what its own source said, so the update is
        // fill-only: the survivor's empty fields, from this record, recorded
        // on its `mergedFrom`. The retired row stays retired — a re-ingest
        // never resurrects what a human merged away.
        const { fills, tradingNames } = planFills(match.matched, [{ ...prospect, id: match.throughRetiredId }]);
        plans.push({
          action: "fill",
          id: match.matchedId,
          survivor: match.matched,
          throughRetiredId: match.throughRetiredId,
          fills,
          tradingNames,
          prospect,
          facts: shaped.facts,
      ...
      plans.push({ action: "update", id: match.matchedId, prospect, facts: shaped.facts, verdict, business, principal: shaped.principal || null });
      continue;
    }
    ...
    // ── Steps 2 and 3 fill the row they flagged against ──────────────────
    //
    // dedupe.js's header, "Steps 2 and 3 also FILL": the existing row's empty
    // fields take this record's values, and this row is STILL written and
    // still flagged. Only the two near-proof matches; the name match plans
    // nothing here. `matched` may be a stub (survivor not loaded) — then
    // there is nothing to compare against and nothing is filled.
    const autofill =
      match.action === "flag" && AUTOFILL_VIAS.includes(match.via) && match.matched && !match.matched.stub
        ? (() => {
            const { fills, tradingNames } = planFills(match.matched, [row]);
            return fills.length || tradingNames ? { targetId: match.matchedId, survivor: match.matched, fills, tradingNames, via: match.via } : null;
          })()
        : null;
    ...
    plans.push({
      ...
      // The board's named person — on insert AND update, unlike the website
      // guess above, because a ProspectPerson row is create-only (people.js):
      // a re-ingest that finds the name already there adds nothing, and one
      // that finds a NEW principal adds a second row beside the old one.
      principal: shaped.principal || null,
    });
...
export async function loadDedupeCandidates({ prospects, provider }, { deps = {} } = {}) {
  ...
  // MERGE_SELECT rather than the six key columns: a step-2/3 hit plans the
  // autofill from the existing row's empty fields, and a fill needs the
  // fields. Same cap, same order.
  ...
  // A retired candidate's hit lands on its survivor (dedupe.js, "A retired
  // row resolves to its survivor"), and the survivor has to be in the index
  // for the fill to see its fields. One more read, only when a retired row
  // was among the candidates.
  const survivorIds = [...new Set(rows.map((r) => r.mergedIntoId).filter(Boolean))].filter((id) => !rows.some((r) => r.id === id));
  if (!survivorIds.length) return rows;
  const survivors = await prisma.prospect.findMany({ where: { id: { in: survivorIds } }, select: MERGE_SELECT });
...
export async function ingestPage(
  ...
  const { plans, counters } = planIngest(
    businesses,
    {
      ...
      // Null for Overture; the register's name for the RBQ and the US boards.
      // Decided from a pure map rather than the provider registry so the
      // verdict is the same whether or not providers.js has been imported.
      licenceRegister: licenceRegisterName(provider),
    },
    index,
  );
  ...
  const fillPlans = plans.filter((p) => p.action === "fill");

  if (!inserts.length && !updates.length && !fillPlans.length) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    // ── The fills: a strong flag's, and a retired record's re-ingest ──────
    //
    // Same transaction as the insert that carries the flag, so a row can
    // never say "filled from X" about an X that was not written. Each fill
    // is guarded on the field still being empty (applyFills), and recorded
    // on the existing row's `mergedFrom` as an autofill — the human merge's
    // record, minus a retirement.
    for (const plan of inserts) {
      if (!plan.autofill) continue;
      await applyFills(tx, {
        survivorId: plan.autofill.targetId,
        survivor: plan.autofill.survivor,
        fills: plan.autofill.fills,
        tradingNames: plan.autofill.tradingNames,
        entry: autofillEntry({ from: plan.id, provider, release, via: plan.autofill.via, now }),
      });
    }
    for (const plan of fillPlans) {
      await applyFills(tx, {
        ...
        fills: plan.fills,
        tradingNames: plan.tradingNames,
        entry: autofillEntry({ from: plan.throughRetiredId, provider, release, via: "source_record", now }),
      });
    }
    if (campaign?.id) {
  ...
  // ── The board's named person, after the rows exist ────────────────────
  //
  // Outside the transaction above, on purpose: recordBoardPrincipal is
  // create-only and idempotent (the unique key on ProspectPerson), so a
  // failure here leaves a prospect without its name until the next ingest
  // and never a prospect without its row. Washington and Oregon only —
  // registerPeople.js's REGISTERS_WITH_PEOPLE decides, and California's
  // personnel comes from its own file.
  for (const plan of [...inserts, ...updates]) {
    if (!plan.principal) continue;
    try {
      await recordBoardPrincipal({ db: prisma, prospectId: plan.id, providerKey: provider, principal: plan.principal, release, now });
    } catch (err) {
      console.error(`[ingest] principal not recorded for ${plan.id}:`, err?.message || err);
    }
  }

  return {
    inserted: inserts.length,
    updated: updates.length + fillPlans.length,
    filled: inserts.filter((p) => p.autofill).length + fillPlans.length,
    skipped: plans.length - inserts.length - updates.length - fillPlans.length,
    counters,
  };
}

/** The `mergedFrom` entry an ingest-time fill writes. `kind: "autofill"` — retires nothing, and unmerge leaves it alone. */
function autofillEntry({ from, provider, release, via, now }) {
  return { version: "1", kind: "autofill", at: now.toISOString(), by: null, from, source: provider || null, release: release || null, via };
}
