...
export function planReclassification(rows = []) {
  ...
  for (const row of rows) {
    if (row.status !== "needs_review" || row.doNotContactAt || row.possibleDuplicateOfId || row.mergedIntoId) continue;
    ids.push(row.id);
      ...
      await db.$transaction(
        async (tx) => {
          const result = await tx.prospect.updateMany({
            where: { id: { in: ids }, status: "needs_review", doNotContactAt: null, possibleDuplicateOfId: null, mergedIntoId: null },
            data: {
              status: "discovered",
              classification: "contractor",
