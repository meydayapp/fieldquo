...
//
// ══ The `us_*` entries, and how THEY are proved to exist ═══════════════════
//
// Added 2026-09-03. They are US state licence-board classifications —
// `us_ca_cslb_c33` is California's "Painting and Decorating Contractor" — and
// they are namespaced by BOARD rather than by state, so two boards in one
// state can never collide in the flat map below. The board's own spelling —
// `C-33`, `CC|01` — is folded to this shape by `slugClass`, which is proved
// injective over each board's whole vocabulary so no two classes collapse
// into one category.
//
// The existence problem above is REAL for these and it is closable, which is
// the difference from Overture. lib/sales/discovery/usBoard/classes.js ships
// each board's full class vocabulary, read out of the published file rather
// than off a documentation page, and scripts/check-us-boards.mjs asserts that
// every `us_*` string here appears in it. A typo fails the build instead of
// matching nothing.
//
// It also asserts the other direction, which is the one that matters more:
// **a board's UNRESTRICTED class must never appear here.** Two thirds of every
// US register holds one — California's `B General Building` is 96,772 of
// 219,255 licences — and it means "no restriction on what I may do", not "I am
// a general contractor". Mapping it would file a hundred thousand businesses
// into one queue on the strength of a box that identifies nobody, which is the
// RBQ finding this file's Overture half already refuses in a different shape.
...
export const DISCOVERY_TRADES = {
  painting: {
    ...
    sourceCategories: ["painting", "us_ca_cslb_c33", "us_wa_lni_cc_cb"],
  },
  cabinets: {
    label: "Cabinets",
  ...
  flooring: {
    ...
    sourceCategories: ["flooring_contractors", "carpet_installation", "us_ca_cslb_c15"],
  },
  countertops: {
    label: "Countertops",
  ...
  roofing: {
    ...
    sourceCategories: ["roofing", "us_ca_cslb_c39", "us_wa_lni_cc_cd"],
  },
  plumbing: {
    label: "Plumbing",
    categoryKeys: ["plumbing"],
    sourceCategories: ["plumbing", "us_ca_cslb_c36", "us_wa_lni_cc_rk", "us_wa_lni_pc_01", "us_wa_lni_pc_02", "us_wa_lni_pc_04"],
  },
  electrical: {
    label: "Electrical",
    categoryKeys: ["electrical"],
    sourceCategories: ["electrician", "us_ca_cslb_c10", "us_wa_lni_ec_01", "us_wa_lni_ec_02"],
  },
  hvac: {
    label: "Heating and cooling",
    categoryKeys: ["hvac_install", "hvac_repair"],
    sourceCategories: ["hvac_services", "us_ca_cslb_c20", "us_wa_lni_cc_sm", "us_wa_lni_ec_6a", "us_wa_lni_ec_6b"],
  },
  landscaping: {
    label: "Landscaping",
    ...
    sourceCategories: ["landscaping", "lawn_service", "us_ca_cslb_c27", "us_wa_lni_cc_bw"],
  },
  carpentry: {
    label: "Carpentry",
    categoryKeys: ["carpentry"],
    sourceCategories: ["carpenter", "us_ca_cslb_c_5", "us_wa_lni_cc_sl"],
  },
  drywall: {
    label: "Drywall",
    categoryKeys: ["drywall", "drywall_install"],
    sourceCategories: ["drywall_services", "us_ca_cslb_c_9", "us_wa_lni_cc_bk"],
  },
  tiling: {
    label: "Tiling",
    categoryKeys: ["tiling"],
    sourceCategories: ["tiling", "us_ca_cslb_c54", "us_wa_lni_cc_re"],
  },
  siding: {
    label: "Siding",
    categoryKeys: ["siding"],
    sourceCategories: ["siding", "us_wa_lni_cc_sw"],
  },
  gutters: {
    label: "Gutters",
    categoryKeys: ["gutter_services"],
    sourceCategories: ["gutter_service", "us_wa_lni_cc_cv"],
  },
  fencing: {
    label: "Fencing",
    categoryKeys: ["fence_services", "fence_repair", "fence_restoration"],
    sourceCategories: ["fence_and_gate_sales_service", "us_ca_cslb_c13", "us_wa_lni_cc_bn"],
  },
  masonry_concrete: {
    label: "Masonry and concrete",
    categoryKeys: ["masonry", "concrete", "parging"],
    sourceCategories: ["masonry_concrete", "us_ca_cslb_c_8", "us_ca_cslb_c29", "us_wa_lni_cc_bi", "us_wa_lni_cc_bz"],
  },
  paving: {
    label: "Paving",
    categoryKeys: ["paving", "driveway_sealing"],
    sourceCategories: ["paving_contractor", "us_wa_lni_cc_ss"],
  },
  insulation: {
    label: "Insulation",
    categoryKeys: ["insulation"],
    sourceCategories: ["insulation_installation", "us_ca_cslb_c_2", "us_wa_lni_cc_so"],
  },
  restoration: {
    label: "Damage restoration",
    categoryKeys: ["restoration"],
    sourceCategories: ["damage_restoration", "fire_and_water_damage_restoration", "us_or_ccb_rrc"],
  },
  chimney: {
    label: "Chimney",
  ...
  pressure_washing: {
    ...
    sourceCategories: ["pressure_washing", "us_wa_lni_cc_pw"],
  },
  junk_removal: {
    label: "Junk removal",
  ...
  handyman: {
    ...
    sourceCategories: ["handyman", "us_wa_lni_cc_hm"],
  },
  excavation: {
    label: "Excavation",
    categoryKeys: ["excavation"],
    sourceCategories: ["excavation_service", "us_wa_lni_cc_si"],
  },
  demolition: {
    label: "Demolition",
    categoryKeys: ["demolition", "demolition_contractor"],
    sourceCategories: ["demolition_service", "us_wa_lni_cc_sf"],
  },
  garage_door: {
    label: "Garage doors",
    categoryKeys: ["garage_door"],
    sourceCategories: ["garage_door_service", "us_wa_lni_cc_og"],
  },
  locksmith: {
    label: "Locksmith",
    categoryKeys: ["locksmith"],
    sourceCategories: ["key_and_locksmith", "us_or_ccb_ocls", "us_or_ccb_rlsc"],
  },
  appliance_repair: {
    label: "Appliance repair",
    categoryKeys: ["appliance_repair"],
    sourceCategories: ["appliance_repair_service", "us_wa_lni_ec_7d"],
  },
  pest_control: {
    label: "Pest control",
    categoryKeys: ["pest_control"],
    sourceCategories: ["pest_control_service", "us_wa_lni_cc_sz"],
  },
  tree_care: {
    label: "Tree care",
    categoryKeys: ["tree_care_service"],
    sourceCategories: ["tree_services", "us_ca_cslb_c49", "us_ca_cslb_d49", "us_wa_lni_cc_rf"],
  },
  pool_spa: {
    label: "Pools and spas",
    categoryKeys: ["pool_spa"],
    sourceCategories: ["pool_cleaning", "pool_and_hot_tub_services", "us_ca_cslb_c53", "us_ca_cslb_d35", "us_wa_lni_cc_rb"],
  },
  irrigation: {
    label: "Irrigation",
    categoryKeys: ["irrigation"],
    sourceCategories: ["irrigation", "us_wa_lni_cc_bv"],
  },
  snow_removal: {
    label: "Snow removal",
  ...
  home_inspection: {
    ...
    sourceCategories: ["home_inspector", "us_or_ccb_ochi", "us_or_ccb_rhisc"],
  },
  remodeling: {
    label: "Remodelling",
...
/**
 * What a campaign banks: one trade, or every trade.
 *
 * ══ Why this is one function and not an `||` at three call sites ═══════════
 *
 * Three things read a campaign's trade — the ingest's filter, the handler's
 * refusal, and the label on the screen — and they have to agree. `campaign
 * .tradeKey || null` at each of them agreed right up until the day a second
 * column could override it, and then the ingest would have banked everything
 * while the screen still said "Painting".
 *
 * `allTrades` WINS over a trade key that is also set. A row carrying both is
 * two statements that disagree; the create route refuses to write one, and if
 * one exists anyway the broader reading is the one that cannot silently drop
 * rows the campaign was told to keep.
 *
 * NEITHER is its own state — `{ allTrades: false, tradeKey: null }` — and it is
 * deliberately NOT collapsed into "all trades". A campaign nobody finished
 * configuring must not become a campaign that banks a province.
 *
 * @returns {{ allTrades: boolean, tradeKey: string|null, stated: boolean }}
 */
export function campaignTradeScope(campaign = {}) {
  if (campaign?.allTrades === true) return { allTrades: true, tradeKey: null, stated: true };
  const raw = typeof campaign?.tradeKey === "string" ? campaign.tradeKey.trim() : "";
  return { allTrades: false, tradeKey: raw || null, stated: Boolean(raw) };
}

/** What a screen calls a campaign's trade, including when it is all of them. */
export function campaignTradeLabel(campaign = {}) {
  const scope = campaignTradeScope(campaign);
  if (scope.allTrades) return "All trades";
  // Not "All trades" and not a blank: a campaign with no trade chosen is a
  // campaign that cannot run, and the screen has to say which of the two it is.
  if (!scope.tradeKey) return "No trade chosen";
  return discoveryTradeLabel(scope.tradeKey);
}
