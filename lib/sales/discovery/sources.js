 ...
 *   unblocks  how many times a superadmin has cleared a block on this source
 *             (the configure action). Never reset. It is what makes the
 *             discovery task queued after an unblock a NEW task: the
 *             fingerprint below carries it, so the key cannot collide with
 *             the task that was abandoned before the block — which is how
 *             two California campaigns sat "running" at zero all night.
 */
export function sourceStateFor(campaign, key) {
  ...
  if (own) {
    return {
      ...
      unblocks: Number.isInteger(own.unblocks) && own.unblocks > 0 ? own.unblocks : 0,
    };
  }
...
export function cursorFingerprint(campaign) {
  return campaignSourceKeys(campaign)
    .map((key) => {
      ...
      // The unblock count rides on the position. Without it, a source
      // unblocked at cursor 4900 fingerprints exactly as it did when its task
      // at cursor 4900 was abandoned, the next enqueue finds that row on the
      // unique key, and nothing runs. With it, every unblock is a new place.
      const again = state.unblocks > 0 ? `~u${state.unblocks}` : "";
      if (state.blocked) return `${key}@blocked${again}`;
      if (state.ended) return `${key}@end${again}`;
      return `${key}@${state.cursor || "0"}${again}`;
    })
    .join("+");
}
