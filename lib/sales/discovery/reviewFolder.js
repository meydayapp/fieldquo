...
//
// A row a rep currently holds, a do-not-contact row, and a row merged into
// another (mergedIntoId set — lib/sales/discovery/mergeProspects.js). None
// needs a human here: the first is with a human already, the second was
// decided, and the third is carried by its survivor. They are excluded from
// the base query AND re-excluded in every write's where clause, because "the
// list didn't show it" is not a write guard.
//
...
/** Rows a screen shows at once by default. The same 50 as /platform/sales/prospects. */
export const REVIEW_PAGE_SIZE = 50;

/**
 * The page sizes a reviewer may pick, and the ceiling the route clamps to.
 *
 * 200 rather than "any number": the folder route computes suggestions per
 * row on every load (suggestTrades over the name, the licence and the
 * crawl), and 200 rows is where that stays under a second on the measured
 * data. A screen asking for 5,000 is not a reviewer reading names; it is a
 * scrape, and the answer is the export. Anything not on this list falls to
 * the default rather than to the nearest — a bookmark carrying `pageSize=0`
 * or `pageSize=1e9` shows the folder, not an error and not 200.
 */
export const REVIEW_PAGE_SIZES = Object.freeze([50, 100, 200]);
export const REVIEW_PAGE_SIZE_MAX = REVIEW_PAGE_SIZES[REVIEW_PAGE_SIZES.length - 1];

/** A page size from a query string or a number: one of REVIEW_PAGE_SIZES, else the default. */
export function parsePageSize(raw) {
  const n = Number(raw);
  return REVIEW_PAGE_SIZES.includes(n) ? n : REVIEW_PAGE_SIZE;
}

/** ids a POST body may exclude from a bulk decision. A page or two of unticked rows, not a second filter. */
export const REVIEW_EXCLUDE_MAX = 500;
...
export function parseReviewFilter(source = {}) {
  ...
  const ids = Array.isArray(rawIds)
    // Bounded by the LARGEST page a screen can show, not the default: with
    // 200 rows on a card, a slice at 50 trimmed the accept to a quarter of
    // what was ticked and the drift guard then refused it ("holds 50, not
    // 200") — the owner saw exactly that on 2026-09-14.
    ? [...new Set(rawIds.filter((v) => typeof v === "string").map((v) => v.trim()).filter((v) => v && v.length <= 40))].slice(0, REVIEW_PAGE_SIZE_MAX)
    : [];
  // The rows a reviewer UNTICKED before "assign to all N": the filter is the
  // decision, minus these. POST body only, like `ids`, and bounded — a
  // reviewer unticks a handful of duplicates, not a thousand rows.
  const rawExclude = get("excludeIds");
  const excludeIds = Array.isArray(rawExclude)
    ? [...new Set(rawExclude.filter((v) => typeof v === "string").map((v) => v.trim()).filter((v) => v && v.length <= 40))].slice(0, REVIEW_EXCLUDE_MAX)
    : [];
  const dups = trimmed(get("dups"), 10);
  return {
    ...
    // "hide" leaves flagged duplicates out of the list and the count. The
    // row-by-row screen sends it by default (its "Hide duplicates" toggle);
    // the badge, the facets and the By-suggestion cards never do, so the
    // number on the rail stays the number of rows needing a human. Absent
    // is "show", so a stale bookmark still shows the whole folder.
    dups: dups === "hide" ? "hide" : null,
  };
}
...
export function untouchableGuardSql(now = new Date()) {
  return Prisma.sql`"doNotContactAt" IS NULL AND "mergedIntoId" IS NULL AND ("assignedRepId" IS NULL OR "claimExpiresAt" < ${now})`;
}

/** The same guard as a Prisma where, for the writes. */
export function untouchableGuardWhere(now = new Date()) {
  return { doNotContactAt: null, mergedIntoId: null, OR: [{ assignedRepId: null }, { claimExpiresAt: { lt: now } }] };
}
...
export function reviewWhereSql(filter = {}, { now = new Date() } = {}) {
  ...
  if (f.excludeIds) parts.push(Prisma.sql`"id" NOT IN (${Prisma.join(f.excludeIds)})`);
  if (f.dups === "hide") parts.push(Prisma.sql`"possibleDuplicateOfId" IS NULL`);
  if (f.q) {
...
/**
 * What retiring a row by MERGE does to its campaign's funnel: it leaves its
 * bucket and lands in "duplicates" — the same line the funnel already keeps
 * for a source record seen twice, which is what a human has just said this
 * row is. Decided from the row as READ, like every other decision here, so
 * mergeProspects.js's unmerge can reverse exactly these deltas. A row in no
 * live bucket (rejected, converted) moves nothing.
 */
export function mergeRetireEffects(prospect = {}) {
  const bucket = reviewBucket(prospect);
  if (bucket === "other") return { bucket, counters: {} };
  return { bucket, counters: merge(leaving(bucket, prospect), { duplicateCount: 1 }) };
}

/** The delta to enter a bucket — the positive of what leaving it subtracts. */
function entering(bucket, prospect) {
  if (bucket === "needs_review") return { needsReviewCount: 1 };
  if (bucket === "banked") return { unmappedCount: 1, bankedCount: 1 };
  if (bucket === "accepted") return enteringAccepted(prospect);
  return {};
}

/**
 * What FILLING a survivor's empty fields does to its campaign's funnel: the
 * row as it was leaves its bucket, the row as it is now enters one. Usually
 * nothing — but a banked row handed a `tradeKey` by a merge is accepted from
 * that moment, and an accepted row handed a phone or a website moves in the
 * `readyCount` / `noWebsiteCount` subsets. Computed from before and after
 * rather than from the fill list, so the same call gives the unmerge its
 * reverse: pass (now, after clearing).
 */
export function fillEffects(before = {}, after = {}) {
  const b = reviewBucket(before);
  const a = reviewBucket(after);
  return merge(b === "other" ? {} : leaving(b, before), a === "other" ? {} : entering(a, after));
}
