...
export function normaliseBusiness(business = {}, context = {}) {
  ...
  return {
    ...
    // The register's named person, the same way: beside, never inside. A
    // board that names nobody (California's master file) gives null.
    principal: typeof business?.principal === "string" && business.principal.trim() ? business.principal.trim().slice(0, 160) : null,
    prospect: {
      businessName,
