...
export const ENGAGEMENTS = Object.freeze([
  ...
  },
  {
    ...
    labelKey: "app.salesPay.engagement.agency.label",
    noteKey: "app.salesPay.engagement.agency.note",
    note:
      "Works for a call-centre agency FieldQuo hired. Their commission is paid to the " +
      "agency, not to them; no leave, no deductions on FieldQuo's side.",
    accruesPaidLeave: false,
  },
  {
    key: "employee",
...
/**
 * Which methods each KIND of SalesRep may choose from.
 *
 * Upwork is how FieldQuo's own reps are engaged, and it stays on their list.
 * An influencer (lib/influencers — a customer company whose referral link
 * earns commission) has no Upwork contract with FieldQuo, so offering it
 * would record "released against a milestone" about money that has no
 * milestone to be released against. The owner's list for them, verbatim:
 * "remove Upwork, add PayPal, Interac e-transfer, Wise, bank transfer".
 *
 * Keyed by kind rather than filtered at the screen so the WRITE path can
 * refuse the same set the form offers — a form that hides Upwork and a
 * route that accepts it is a hidden option, not a removed one.
 */
export const METHODS_BY_KIND = Object.freeze({
  rep: Object.freeze(["upwork", "paypal", "interac", "wise", "bank_transfer"]),
  influencer: Object.freeze(["paypal", "interac", "wise", "bank_transfer"]),
  // A call-centre agency (lib/sales/agency.js) is a business FieldQuo pays
  // by invoice, not an Upwork contractor: the same list as an influencer.
  agency: Object.freeze(["paypal", "interac", "wise", "bank_transfer"]),
});

/** The method shapes a given kind may pick from. Unknown kind → a rep's list. */
export function payoutMethodsFor(kind) {
  const keys = METHODS_BY_KIND[kind] || METHODS_BY_KIND.rep;
  return PAYOUT_METHODS.filter((m) => keys.includes(m.key));
}

export const isEngagement = (v) => ENGAGEMENT_KEYS.has(String(v ?? ""));
export const isPayoutMethod = (v) => METHOD_KEYS.has(String(v ?? ""));

/** Is this method one THIS kind may choose? Stricter than isPayoutMethod. */
export function isPayoutMethodFor(kind, v) {
  return isPayoutMethod(v) && (METHODS_BY_KIND[kind] || METHODS_BY_KIND.rep).includes(String(v));
}
