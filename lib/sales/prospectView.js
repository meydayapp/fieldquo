 ...
 * the number.
 */
export function capabilityStatement(row, { rules = [], evidenceById = null, proofs = null } = {}) {
...
export function prospectView({
  ...
  const facts = capabilities
    .map((row) => capabilityStatement(row, { rules, evidenceById, proofs }))
    ...
    .map((f) =>
      f.code === "EMAIL_CONTACT" && f.state === "has" && prospect.email
        ? { ...f, email: prospect.email, emailSource: prospect.emailSource ?? null, detail: prospect.email }
        : f,
    )
