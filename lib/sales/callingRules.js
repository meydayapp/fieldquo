...
/**
 * How hard a jurisdiction's window and cap are applied — the platform
 * console's override, resolved by lib/sales/windowPolicy.js and handed to
 * salesCallReadiness() as `windowPolicy`. The words live HERE rather than in
 * that file because this file is the evaluator and cannot import the resolver
 * (the resolver imports this table); windowPolicy.js re-exports them so no
 * caller has to know which file they came from.
 *
 *   enforce   today's behaviour — outside the window or over the cap refuses
 *   warn      the call is allowed and a warning names the rule it is outside
 *   off       the window and the cap are not applied; a caveat says so
 *
 * None of the three touches a registration warning, a prohibition, or an
 * unread jurisdiction. The override is about the CLOCK and the CAP, and the
 * resolver holds it at `enforce` wherever a registration is outstanding.
 */
export const WINDOW_MODE_ENFORCE = "enforce";
export const WINDOW_MODE_WARN = "warn";
export const WINDOW_MODE_OFF = "off";
export const WINDOW_MODES = Object.freeze([WINDOW_MODE_ENFORCE, WINDOW_MODE_WARN, WINDOW_MODE_OFF]);
...
export const CALLING_JURISDICTIONS = Object.freeze({
  ...
  CA: {
    ...
    registration: {
      required: true,
      // Filed by the owner on 2026-09-06 as "Makes telemarketing calls that ARE
      // exempt from the National DNCL Rules" (B2B, own reps — see
      // docs/sales-intel/AUDIT-compliance.md). The operator's confirmation was
      // still in the post when this flipped; the owner's instruction was that
      // the confirmation is not a gate ("run as if it's already"), and the
      // registration itself — the thing the Rules require — was submitted.
      // `filing` is the record the header asks for, beside the flag rather than
      // only in a commit message, so check:sales-calling-window can hold the
      // flag to it. Add the confirmation number when it arrives.
      done: true,
      filing: {
        submittedAt: "2026-09-06",
        organization: "FieldQuo Inc.",
        cbn: "791503840",
        category: "exempt_b2b_own_reps",
        confirmation: null,
      },
      what:
  ...
  "US-IN": {
    ...
    citation:
      ...
      "TELEPHONE SUBSCRIBER' who is a leadive purchaser of consumer goods or " +
...
/**
 * Every subdivision this file knows, as `{ code, country, name }`, Canada
 * first then the United States, each alphabetical by name — for a picker.
 *
 * Derived from SUBDIVISION_TIME_ZONES (which codes exist) and
 * SUBDIVISION_ALIASES (what they are called: the FIRST alias listed for a
 * code is its English name), never typed out again, so a province added to
 * the zone table appears in every picker without a second list to forget.
 * The alias table is upper-case; the name is re-cased for reading, with the
 * small words ("of", "and") kept small. A code with no alias prints as its
 * code rather than as a guess.
 */
export function subdivisionOptions() {
  const CANADA = new Set(["AB", "BC", "MB", "NB", "NL", "NS", "NT", "NU", "ON", "PE", "QC", "SK", "YT"]);
  const SMALL = new Set(["OF", "AND"]);
  const nameOf = new Map();
  for (const [alias, code] of Object.entries(SUBDIVISION_ALIASES)) {
    if (nameOf.has(code) || /^[A-Z]{2}$/.test(alias)) continue;
    nameOf.set(
      code,
      alias
        .split(" ")
        .map((w, i) => (i > 0 && SMALL.has(w) ? w.toLowerCase() : w.charAt(0) + w.slice(1).toLowerCase()))
        .join(" "),
    );
  }
  const rows = Object.keys(SUBDIVISION_TIME_ZONES).map((code) => ({
    code,
    country: CANADA.has(code) ? "CA" : "US",
    name: nameOf.get(code) || code,
  }));
  rows.sort((a, b) => (a.country !== b.country ? (a.country === "CA" ? -1 : 1) : a.name.localeCompare(b.name)));
  return rows;
}
...
export function jurisdictionFor(prospect = {}) {
  const key = jurisdictionKey(prospect);
  return key ? CALLING_JURISDICTIONS[key] || null : null;
}

/**
 * The KEY into the table, without reading it.
 *
 * Split out of `jurisdictionFor` so a caller that needs to ask this table a
 * question against a DIFFERENT table — the campaign gate's check mutates
 * `registration.done` and asserts the answer flips — does not have to spell the
 * "CA" / "US-XX" rule a second time. A second copy of the key rule is how one
 * surface starts disagreeing with the queue about which state a campaign is in.
 */
export function jurisdictionKey(prospect = {}) {
  const { country, subdivision } = locationCodes(prospect);
  if (country === "CA") return "CA";
  if (country === "US" && subdivision) return `US-${subdivision}`;
  return null;
}

/** Every zone a district-sized part of this lead's subdivision observes. */
...
export function describeWindow(window) {
  ...
  if (same) {
    return (
      ...
      `${closedText}, in the lead's own time zone`
    );
  }
  return (
...
/**
 * The same window, as a catalogue key and the values it needs.
 *
 * ── Why this exists beside describeWindow rather than replacing it ────────
 *
 * describeWindow() is read by scripts/check-sales-calling-window.mjs and lands
 * in API `error` bodies where there is no reader whose language could be
 * consulted. It stays exactly as it was. This is the version a SCREEN uses,
 * because a rep on Spanish was reading "08:00–21:00 every day, in the
 * prospect's own time zone" inside an otherwise Spanish console.
 *
 * ── Four keys, not one with an optional clause ────────────────────────────
 *
 * The closed-days half is a whole extra statement ("and no calls at all on
 * Sundays"), and in half the portal's languages it does not attach where
 * English attaches it. Gluing a translated clause onto a translated stem is
 * the same defect as gluing an English one on: the join is what breaks. So
 * there are four complete sentences, and the caller picks one.
 *
 * The DAY NAMES are deliberately not keys. They come from CLDR through
 * lib/format/localeDate.js, keyed on the reader's language — the decision
 * /app/scheduler's translation already took, for the reason it recorded:
 * seven keys per language is 63 strings that Intl already ships, correctly
 * ordered, for languages this catalogue has never been translated into.
 *
 * @returns {{key:string, params:{start:string,end:string,weekendStart:string,
 *            weekendEnd:string,closedWeekdays:number[]}}|null}
 */
export function describeWindowParts(window) {
  if (!window) return null;
  const { weekday, weekend } = window;
  const closedWeekdays = Array.isArray(window.closedWeekdays) ? window.closedWeekdays : [];
  const same =
    weekday.startMinute === weekend.startMinute && weekday.endMinute === weekend.endMinute;
  const shape = same ? "everyDay" : "split";
  const closed = closedWeekdays.length ? "Closed" : "";
  return {
    key: `app.salesDial.window.${shape}${closed}`,
    params: {
      start: hhmm(weekday.startMinute),
      end: hhmm(weekday.endMinute),
      weekendStart: hhmm(weekend.startMinute),
      weekendEnd: hhmm(weekend.endMinute),
      closedWeekdays,
    },
  };
}
 ...
 *
 * The LOCALE is a parameter, defaulting to what it was hard-coded to. This
 * string reaches a rep's screen inside the "come back at…" refusal, and a
 * Spanish console printing "Tue 8 Sep" is the same half-and-half sentence the
 * rest of this file's i18n work removes. CLDR already knows the abbreviations
 * and, more importantly, the ORDER — "8 sept." and "9月8日" are not the same
 * arrangement of the same tokens, which is why this is one Intl call and not
 * seven translation keys.
 *
 * The zone name in the parentheses stays as it is: IANA identifiers are not
 * language, and "America/Toronto" is what a rep would have to type or search.
 */
export function describeLocal(at, zone, locale = "en-GB") {
  if (!at || !zone) return null;
  try {
    const text = new Intl.DateTimeFormat(locale || "en-GB", {
      timeZone: zone,
      weekday: "short",
      day: "numeric",
      month: "short",
...
export function salesCallReadiness({
  ...
  // The reader's language, for the one string in here that is a DATE rather
  // than a sentence. Everything else this function composes now travels as a
  // key; `opensAtText` cannot, because it is a formatted instant, so it is
  // formatted in the reader's locale at the point it is built. Defaulting to
  // English keeps every existing server caller byte-identical.
  language = "en-GB",
  // The platform console's override for this jurisdiction, ALREADY RESOLVED
  // by lib/sales/windowPolicy.js's effectiveWindowPolicy() — the caller
  // supplies it the way it supplies `attemptsLast24h`: a fact this function
  // reads, never one it looks up. Null or anything it does not recognise is
  // `enforce`, which is every answer this function gave before the override
  // existed. The registration hold is the resolver's job and has already
  // happened by the time a policy reaches here; this function only ever sees
  // the mode that binds.
  windowPolicy = null,
  // Is the number that will ring one of FieldQuo's OWN phones, on the test
  // list (lib/sales/testLines.js)? A fact the caller looked up, never one
  // this function decides: it sees no number and holds no list. Only the
  // literal `true` counts. When it is, the whole evaluation is skipped —
  // there is no jurisdiction, no window and no cap to apply to our own
  // desk, and the answer says so out loud rather than pretending a law was
  // read. Do-not-contact and the suppression list are NOT this function's
  // business and are checked before it by every caller, test line or not.
  testLine = false,
  // Is the rep placing the dial a TEST ACCOUNT (SalesRep.testAccount, set by
  // a superadmin on the rep's card)? The owner's own account tests the
  // dialler at every hour and is not held to the window. Judged on the SAME
  // path as a test line — one early return, one answer shape, the same
  // jurisdiction code "test" on the row so every count's exclusion applies
  // without a second edit — and the caveat names which of the two it is,
  // because "this is our own number" would be a lie about a stranger's.
  // Read from the rep row by the gate in the request that dials, never from
  // what a screen remembered. Only the literal `true` counts.
  testAccount = false,
} = {}) {
  const blockers = [];
  const warnings = [];
  ...
  const testMode = testLine === true ? TEST_MODE_LINE : testAccount === true ? TEST_MODE_ACCOUNT : null;
  if (testMode) return testReadiness({ mode: testMode, blockers, warnings, unenforced });

  const policy = windowPolicy && typeof windowPolicy === "object" ? windowPolicy : null;
  const mode = policy && WINDOW_MODES.includes(policy.mode) ? policy.mode : WINDOW_MODE_ENFORCE;
  // Whether the clock and the cap may be stepped past. Only the two relaxed
  // modes; and only for the branches below that are about the clock and the
  // cap — a prohibition, an unread state and a registration are not windows.
  const relaxed = mode !== WINDOW_MODE_ENFORCE;
  ...
  const base = {
    ...
    zoneSource,
    windowText: null,
    // The same window as a key the screen can say in the rep's language. Null
    // here and filled in below at the same moment windowText is, so the two
    // can never describe different hours.
    windowKey: null,
    windowParams: null,
    statutoryWindow: null,
    citation: jurisdiction?.citation || null,
    // The per-24h cap and how many of it have been used, echoed so a screen
    // can print "1 of 3 calls in 24 h" without re-deriving either. Null cap:
    // this jurisdiction imposes none. Null count: the caller did not count.
    attemptCap: jurisdiction?.maxCallsPer24h ?? null,
    attemptsLast24h:
      typeof attemptsLast24h === "number" && Number.isFinite(attemptsLast24h) ? attemptsLast24h : null,
    opensAt: null,
    opensAtText: null,
    // The override in force, echoed so a screen can say "warn only" or
    // "switched off" beside the hours without re-resolving it. Null when no
    // policy was supplied, which reads as enforce everywhere.
    windowOverride: policy
      ? {
          mode,
          requestedMode: WINDOW_MODES.includes(policy.requestedMode) ? policy.requestedMode : mode,
          heldByRegistration: policy.heldByRegistration === true,
          registrationGated: policy.registrationGated === true,
          note: typeof policy.note === "string" && policy.note ? policy.note : null,
        }
      : null,
    // Whether this instant is inside the window, INDEPENDENT of the decision.
    // Under enforce the decision says it; under warn or off the decision is
    // `allowed` either way, and the screen must not print "Open now" over a
    // shut window. Null until the clock has been read.
    inWindow: null,
    blockers,
    warnings,
    unenforced,
  ...
  if (!country || !subdivision) {
    blockers.push({
      // ── Why every blocker below carries a key as well as its words ──────
      //
      // These are the sentences a rep reads instead of a Call button, and
      // they were English on all nine languages of the portal. The KEY is
      // what travels; app/components/sales/DialRegion.js is the one renderer
      // for the whole region and resolves it. The English is unchanged and
      // stays the fallback — this file is executed by three check scripts
      // under bare node and must not learn to import a translator.
      code: "location_unknown",
      titleKey: "app.salesDial.blocker.locationUnknown.title",
      fixKey: "app.salesDial.blocker.locationUnknown.fix",
  ...
  if (!jurisdiction) {
    blockers.push({
      ...
      titleKey: "app.salesDial.blocker.jurisdictionUnread.title",
      fixKey: "app.salesDial.blocker.jurisdictionUnread.fix",
      params: { subdivision },
  ...
  if (!jurisdiction.verified) {
    blockers.push({
      ...
      titleKey: "app.salesDial.blocker.jurisdictionUnverified.title",
      params: { jurisdiction: jurisdiction.name },
      title: `${jurisdiction.name}'s calling rules have not been verified.`,
      // No fixKey, and `legalText` says why. The body here is the JURISDICTION
      // TABLE's citation — a statute number and the words the statute actually
      // uses. Machine-translating a quoted primary source would produce a
      // sentence that reads as the law and is not it, so it stays in the
      // language the legislature wrote it in and the renderer labels it as
      // quoted. See DialRegion.js and the note on `citation` below.
      legalText: true,
  ...
  if (jurisdiction.prohibition) {
    // Straight off the jurisdiction table, and flagged as the statute's own
    // words rather than ours. Arizona's is the only row that carries one, and
    // it quotes A.R.S. §44-1278(B)(3); restating a prohibition in a language
    // the legislature did not write it in is not a translation a rep should
    // rely on when the question is whether a call is lawful.
    blockers.push({ legalText: true, ...jurisdiction.prohibition });
  ...
  if (Array.isArray(jurisdiction.handEnforced)) {
    for (const item of jurisdiction.handEnforced) unenforced.push({ legalText: true, ...item });
  }
  ...
  // Derived from the SAME window object in the same breath as windowText, so
  // the two cannot describe different hours. A second call site working the
  // window out again is how a screen ends up printing 09:00 beside an
  // evaluator that let 08:00 through.
  const parts = describeWindowParts(window);
  const windowKeys = {
    windowKey: parts?.key || null,
    windowParams: parts?.params || null,
    // WHOSE window it is. `windowText` alone cannot say — the same hours are
    // a statute in Washington and a courtesy of FieldQuo's own in a state that
    // imposes none, and a rep asked "what makes this legal?" needs to know
    // which. Carried as a boolean rather than as the English phrase `whose`,
    // so the screen picks a whole sentence instead of splicing a possessive.
    statutoryWindow: statutory,
  };
  const whose = statutory
    ? `${jurisdiction.name}'s rule`
    : `FieldQuo's own rule — ${jurisdiction.name} imposes none`;

  // ── The override, said out loud before anything it steps past ───────────
  //
  // `off` is announced whenever the clock or the cap WOULD have been read,
  // inside the window or not: a rep should know the software is not checking
  // the hours for this state, and a caveat that only appears once a call
  // would have been refused is a caveat nobody sees until it matters.
  // `warn` says nothing until there is something to warn about.
  if (mode === WINDOW_MODE_OFF) {
    unenforced.push({
      code: "window_override_off",
      titleKey: "app.salesDial.unenforced.windowOverrideOff.title",
      fixKey: "app.salesDial.unenforced.windowOverrideOff.fix",
      params: { jurisdiction: jurisdiction.name },
      title: `${jurisdiction.name}'s calling window and cap are switched off for this dial by the platform console.`,
      fix:
        "The hours are not being checked for this call. The rule itself is unchanged in law — " +
        "this is FieldQuo's own decision, recorded with who took it and why.",
      note: policy?.note || null,
    });
  }
  const overrideWarning = (which) => ({
    code: which === "cap" ? "cap_override_warn" : "window_override_warn",
    titleKey:
      which === "cap"
        ? "app.salesDial.warning.capOverrideWarn.title"
        : "app.salesDial.warning.windowOverrideWarn.title",
    fixKey: "app.salesDial.warning.overrideWarn.fix",
    params: {
      jurisdiction: jurisdiction.name,
      cap: jurisdiction.maxCallsPer24h,
      made: attemptsLast24h,
    },
    title:
      which === "cap"
        ? `${jurisdiction.name}'s cap of ${jurisdiction.maxCallsPer24h} calls in 24 hours has been reached ` +
          `(${attemptsLast24h} made), and the cap is set to warn only.`
        : `This call is outside ${jurisdiction.name}'s calling window, and the window is set to warn only.`,
    fix:
      "A superadmin set this in the platform console under Calling windows. The statute is " +
      "unchanged — only what this screen refuses has changed. The warning is the whole of the " +
      "control: read it before you press.",
    note: policy?.note || null,
  });

  // The answer's shape when the clock could not be read at all. Under enforce
  // that is a return; under a relaxed mode the window is not applied, so the
  // evaluation continues to the cap with no opening time to offer.
  let withClock = { ...base, windowText, ...windowKeys };

  if (zones.length === 0) {
    const blocker = {
      code: "time_zone_unknown",
      titleKey: "app.salesDial.blocker.timeZoneUnknown.title",
      // Two keys, one per branch of `whose`, rather than one sentence with the
      // English words "…'s rule" slotted into it. Whose rule it is changes the
      // grammar of the sentence around it in most of these languages, and a
      // possessive built in English and dropped into Spanish is exactly the
      // half-and-half sentence this work exists to remove.
      fixKey: statutory
        ? "app.salesDial.blocker.timeZoneUnknown.fixStatutory"
        : "app.salesDial.blocker.timeZoneUnknown.fixCourtesy",
      params: { subdivision, jurisdiction: jurisdiction.name },
    ...
    // The window is not applied, so an unreadable clock is not a refusal —
    // but under `warn` the rep is told the hours could not be checked, in
    // the same words, beside the override warning that says why it went on.
    if (mode === WINDOW_MODE_WARN) {
      warnings.push(blocker);
      warnings.push(overrideWarning("window"));
    }
  } else {
    const agreement = zoneAgreement(window, zones, now);
    const opensAt = nextOpening(window, zones, now);
    const opensAtText = opensAt ? describeLocal(opensAt, zones[0], language) : null;
    withClock = { ...withClock, opensAt, opensAtText, inWindow: agreement === true };

    if (agreement === null) {
      // Split subdivision, and the halves disagree about this minute.
      const blocker = {
        code: "time_zone_ambiguous",
        titleKey: "app.salesDial.blocker.timeZoneAmbiguous.title",
        // Two keys again, and for a harder reason than grammar: with an opening
        // time this sentence tells a rep WHEN to come back, and without one it
        // tells them there is no such moment to wait for. Those are different
        // instructions, so they are different sentences rather than one with an
        // optional tail.
        fixKey: opensAtText
          ? "app.salesDial.blocker.timeZoneAmbiguous.fixOpens"
          : "app.salesDial.blocker.timeZoneAmbiguous.fixUnknownOpening",
        params: { subdivision, opensAt: opensAtText },
    ...
    if (agreement === false) {
      const blocker = {
        code: "outside_window",
        titleKey: "app.salesDial.blocker.outsideWindow.title",
        // Four combinations, four sentences: whose rule it is, and whether there
        // is an opening time to name. Assembling any of them from halves is the
        // failure named at the top of this file's i18n note.
        fixKey: `app.salesDial.blocker.outsideWindow.fix${statutory ? "Statutory" : "Courtesy"}${
          opensAtText ? "Opens" : "Wait"
        }`,
        params: { jurisdiction: jurisdiction.name, opensAt: opensAtText },
      ...
      // Allowed, and the rep is told in so many words that it is outside
      // the window and why the button is there anyway. Under `off` the
      // caveat pushed above already says the hours are not being checked.
      if (mode === WINDOW_MODE_WARN) warnings.push(overrideWarning("window"));
    }
  }
    ...
    if (typeof attemptsLast24h === "number" && Number.isFinite(attemptsLast24h)) {
      if (attemptsLast24h >= jurisdiction.maxCallsPer24h) {
        const blocker = {
          ...
          titleKey: "app.salesDial.blocker.callCapReached.title",
          fixKey: "app.salesDial.blocker.callCapReached.fix",
          params: {
            jurisdiction: jurisdiction.name,
            cap: jurisdiction.maxCallsPer24h,
            made: attemptsLast24h,
        ...
        if (mode === WINDOW_MODE_WARN) warnings.push(overrideWarning("cap"));
      }
    } else {
      unenforced.push({
        code: "call_cap_uncounted",
        titleKey: "app.salesDial.unenforced.callCapUncounted.title",
        fixKey: "app.salesDial.unenforced.callCapUncounted.fix",
        params: { jurisdiction: jurisdiction.name, cap: jurisdiction.maxCallsPer24h },
        title:
          `${jurisdiction.name} allows at most ${jurisdiction.maxCallsPer24h} calls to this ` +
          "business on the same subject in 24 hours.",
        // Said only when the caller passed NO count. Every dial and the
        // queue console count through lib/sales/calls/store.js; this
        // sentence is for a caller that could not — and it says the count
        // is unavailable, not that the product never records attempts,
        // which stopped being true when SalesCallAttempt landed.
        fix:
          "The count of recent calls to this number is not available here, so this cap " +
          "is not being checked for this dial — keep track yourself. It is a private " +
          "right of action in both states that impose it.",
      });
    }
  }
...
/** `jurisdictionCode` on a test dial's row. Re-exported by lib/sales/testLines.js, which the counts import. */
export const TEST_LINE_JURISDICTION_CODE = "test";

/**
 * Why a readiness is a test readiness. `line`: the number that will ring is
 * one of FieldQuo's own (lib/sales/testLines.js). `account`: the rep dialling
 * is a test account (SalesRep.testAccount). One path serves both; the mode
 * only picks the caveat's words.
 */
export const TEST_MODE_LINE = "line";
export const TEST_MODE_ACCOUNT = "account";

/**
 * The answer for a test dial — our own phone, or our own tester.
 *
 * Allowed, with NO jurisdiction name, no window text and no citation — the
 * screens key their "Washington's rule applies" and "quoted in English"
 * lines on those being present, and a test dial has none of them to print.
 * `jurisdiction.code` is the one field set, because recordDial() freezes it
 * into the row and every count keys its exclusion on that value.
 *
 * The caveat is in `unenforced` beside the Call button, in the rep's
 * language, and it says the two things a rep needs: WHY this is a test (our
 * number, or their account), and that the dial will not be counted. The
 * account wording adds that do-not-contact still binds, because a test
 * account rings strangers' numbers and a test line never does.
 *
 * `testLine: true` is the umbrella flag every reader keys on — the queue's
 * window grouping skips the best-time score on it, the retry pool skips its
 * hold on it — and it is true for BOTH modes; `testMode` says which. Kept
 * under its original name so the one fact "this readiness is a test" has one
 * reader-facing spelling, rather than two flags a reader could check one of.
 */
function testReadiness({ mode, blockers, warnings, unenforced }) {
  if (mode === TEST_MODE_ACCOUNT) {
    unenforced.push({
      code: "test_account",
      titleKey: "app.salesDial.unenforced.testAccount.title",
      fixKey: "app.salesDial.unenforced.testAccount.fix",
      title: "Your account is a test account.",
      fix:
        "The calling window, the 24-hour cap and the retry pool are not applied to your dials. " +
        "This dial is recorded as a test and left out of every count, stat and commission. " +
        "Do-not-contact still applies — nobody who asked us to stop can be rung from a test account.",
    });
  } else {
    unenforced.push({
      code: "test_line",
      titleKey: "app.salesDial.unenforced.testLine.title",
      fixKey: "app.salesDial.unenforced.testLine.fix",
      title: "This is one of FieldQuo's own test numbers.",
      fix:
        "The calling window, the 24-hour cap and the retry pool are not applied — nobody's evening is " +
        "being interrupted but ours. This dial is recorded as a test and left out of every count, " +
        "stat and commission.",
    });
  }
  return {
    jurisdiction: { code: TEST_LINE_JURISDICTION_CODE, name: null, verified: false },
    zones: [],
    zoneSource: "derived",
    windowText: null,
    windowKey: null,
...
function pushRegistration(warnings, jurisdiction) {
  ...
  warnings.push({
    ...
    titleKey: "app.salesDial.warning.registrationOutstanding.title",
    params: { jurisdiction: jurisdiction.name },
    title: `FieldQuo must be registered to make sales calls into ${jurisdiction.name}.`,
    // `reg.what` names a bond, a filing and a statute by number, out of the
    // jurisdiction table. Same reasoning as jurisdiction_unverified: the
    // heading is ours to translate and the legal text is not ours to restate.
    legalText: true,
    fix: reg.what,
  });
}
...
function pushDataAcquisition(warnings, jurisdiction) {
  ...
  warnings.push({
    ...
    titleKey: "app.salesDial.warning.dataAcquisitionRule.title",
    params: { jurisdiction: jurisdiction.name },
    title: `${jurisdiction.name} regulates how this lead's details may be obtained.`,
    legalText: true,
