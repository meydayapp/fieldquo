...
const REP_SELECT = { id: true, name: true, workName: true, code: true, referralToken: true, email: true, workEmail: true, timeZone: true, demoHours: true, active: true, kind: true, language: true, endedAt: true };

/**
 * The rep behind a code, or null. An influencer's code is a signup link, not a calendar.
 *
 * `repCode` is the path segment of /demo/<code>: the rep's opaque
 * referralToken on every link minted since 2026-09-25, or their legacy name
 * slug on one sent before — lib/sales/repLink.js resolves both, token first.
 * Anything else is null, and the page says "not found".
 */
export async function loadRepForDemo(repCode, client = db) {
  const code = String(repCode || "").trim();
  if (!code || code.length > 80) return null;
  const rep = await findRepByLinkCode(code, client, REP_SELECT);
...
export async function repDemoPageState({ repCode, token = null, lang = null, client = db, now = new Date() } = {}) {
  ...
  return {
    ok: true,
    language,
    // A stranger's page: the work name, else the first name — never the
    // rep's full real name (lib/sales/repIdentity.js).
    repName: repPublicName(rep),
    initials: initialsOf(repPublicName(rep)),
    timeZone,
...
export async function bookRepDemo({ repCode, token = null, slot, name, email, phone, business, lang = null, client = db, now = new Date() } = {}) {
  ...
  try {
    created = await client.$transaction(async (tx) => {
      ...
      const event = await tx.salesEvent.create({
        data: {
          ...
          notes: row
            ...
            : `They booked this on your demo page (/demo/${rep.referralToken || rep.code}). Confirmation sent to ${cleanEmail}.`,
        },
      ...
      // A demo booked is the lead reaching "demoed" — forward only, no-op
      // when it is already there or further (lib/sales/leadStatus.js).
      if (leadId) await advanceLeadStatus(tx, { leadId, salesRepId: rep.id, to: "demoed", now });
      return { event };
    });
  } catch (err) {
    if (err?.already) {
  ...
  return { ok: true, eventId: event.id, at: event.startAt.toISOString(), language, repName: repPublicName(rep), email: cleanEmail, when };
}
...
async function sendRepDemoInvite({ rep, row, event, language, cleanName, cleanEmail, cleanPhone, cleanBusiness, when, client }) {
  ...
  // The invite's organizer and sign-off: the public name (repIdentity.js).
