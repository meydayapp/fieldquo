...
const SENTENCE_WORDS = /\b(why|how|what|when|where|who|we|our|your|you|get|call|free|best|top|welcome|about|contact|quote|estimate|today|now|us|the|and|with|for|from|to|of|in|on|at|is|are|saves?|money|pourquoi|comment|nous|notre|nos|vos|votre|choisir|un|une|avec|por qu[ée]|c[óo]mo|nuestr[oa]s?|sus?|con|para)\b/i;

/** A two-letter US state or Canadian province code, as the last word of a
 *  name: "Window Cleaning Webster Ny", "Roofing Buffalo NY" — a location
 *  landing page, not a service. Measured live (2026-09-14): an exterior
 *  cleaner's sitemap carried nine of them beside its eight real services. */
const TRAILING_REGION = /\s(al|ak|az|ar|ca|co|ct|de|fl|ga|hi|id|il|in|ia|ks|ky|la|me|md|ma|mi|mn|ms|mo|mt|ne|nv|nh|nj|nm|ny|nc|nd|oh|ok|or|pa|ri|sc|sd|tn|tx|ut|vt|va|wa|wv|wi|wy|ab|bc|mb|nb|nl|ns|nt|nu|on|pe|qc|sk|yt)$/i;

/** Is this a location landing page's name rather than a service's? */
export function looksLikePlacePage(name) {
  const clean = String(name || "").trim();
  return /\b(in|near|around|serving)\s+\S+/i.test(clean) || TRAILING_REGION.test(clean) || /\d{3,}/.test(clean);
}

/** Does this label name the BUSINESS rather than a service? A heading that
 *  is the company's own name, or contains it, is its masthead. */
function isBusinessName(label, businessName) {
  const a = foldServiceName(label);
  const b = foldServiceName(businessName);
  if (!a || !b || b.length < 4) return false;
  return a === b || a.includes(b) || (b.includes(a) && a.length >= 6);
}
...
export function serviceFromPath(path) {
  ...
  if (looksLikePlacePage(name) || SENTENCE_WORDS.test(name)) return null;
...
export function serviceFromWpPage(record) {
  ...
  if (looksLikePlacePage(title)) return null;
 ...
 * @param businessName  the prospect's name, so its masthead is not a service
 ...
 */
export function servicesFrom({ evidence = [], businessName = null } = {}) {
  ...
  const add = (source, item, row) => {
    ...
    if (!key) return;
    // The company's own name is its masthead, not a service — an h1 of
    // "WNY Pool Service" or a menu item "Urato Landscaping" (live, 2026-09-14).
    if (businessName && isBusinessName(item.name, businessName)) return;
