 ...
 */
export async function loadCapabilityMatrix({ includeInactive = false } = {}) {
  const rows = await db.fieldQuoCapability.findMany({
...
export async function loadOpportunityInputs(prospectId) {
  const [capabilities, technologies, rules, matrix] = await Promise.all([
    db.prospectCapability.findMany({
      where: { prospectId },
    ...
    db.opportunityRule.findMany({ where: { active: true } }),
 ...
 * @param {{ dryRun?: boolean }} options  dryRun computes and writes nothing,
 *        which is what the superadmin screen's preview uses.
 */
export async function regenerateOpportunities(prospectId, { dryRun = false } = {}) {
  const inputs = await loadOpportunityInputs(prospectId);
