...
import { liveSuppressedContacts } from "@/lib/sales/suppression";
...
export async function promoteListings({ db = defaultDb, dry = true, regions = null, runId = null, ids = null, limit = 0, now = new Date(), onRow = null, queueResearch = ensureResearchQueued } = {}) {
  ...
  for (;;) {
    ...
    const [byPhone, byDomain, byPlace, dnc] = await Promise.all([
      ...
      // Through lib/sales/suppression.js, which normalises: the list stores
      // "acme.com", a scraper may hand over "www.acme.com". No fallback to an
      // empty list when the read fails — a sweep that cannot see the
      // do-not-contact list stops rather than promoting past it.
      liveSuppressedContacts(db, { phones, domains }),
    ]);
    const ctx = {
      regions,
      ...
      dncPhones: dnc.phones,
      dncDomains: dnc.domains,
      batchPhones,
      batchDomains,
    };
