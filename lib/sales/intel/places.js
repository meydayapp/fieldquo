...
/** True when the token is not a trade heading or a place word. */
 ...
 */
export function planPlacesWrite({ prospect, place, score = null, placeIdTakenBy = null, existingNumbers = [], now = new Date(), origin = null } = {}) {
  const snapshot = placeSnapshot(place, score, { now });
  // `origin` names a different reader of the same Google data — the bulk
  // Maps scraper (lib/sales/intel/listingMatch.js) — so the evidence rows
  // say which path read it while the Prospect columns, the confirmations
  // and the stamp are identical: a lead the bulk run reached is
  // indistinguishable on the card from one the API reached, which is the
  // point, and `placesResult.origin` is where the difference is kept.
  const detector = origin?.detector || PLACES_DETECTOR;
  const detectorVersion = origin?.detectorVersion || PLACES_DETECTOR_VERSION;
  const data = { placesCheckedAt: now, placesResult: origin ? { ...snapshot, origin: origin.detector } : snapshot };
  const gained = [];
  const conflicts = [];
  const confirmations = [];
  const evidence = [];
      ...
      if (!prospect.phoneE164) {
        ...
        if (!existingNumbers.includes(e164)) {
          contactNumber = {
            ...
            note: `Listed on Google Places as ${snapshot.phone}, checked ${now.toISOString().slice(0, 10)}. The register's number is the one on the record.`,
          };
  ...
  data.placesResult = { ...snapshot, confirmations, ...(origin ? { origin: origin.detector } : {}) };
