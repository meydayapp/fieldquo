 ...
 *
 *  Initialisms are one token however they were spaced or dotted: "B P
 *  Plumbing", "B.P. Plumbing", "B&P Plumbing" and "BP Plumbing" all carry
 *  "bp". Measured on production 2026-09-21 — 914 name_disagrees refusals,
 *  and the owner's reading of them was that a run of single letters
 *  against the same letters joined ("B P" against "BP") is the same
 *  business spelled by two clerks. The run is joined BEFORE the suffix
 *  list is applied so "A 1 Plumbing" and "A-1 Plumbing" both give "a1"
 *  rather than losing the article and keeping a bare "1". */
export function nameTokens(name) {
  const raw = stripDiacritics(name).toLowerCase().replace(/[^a-z0-9&' ]+/g, " ").split(/\s+/)
  ...
  for (let i = 0; i < raw.length; i += 1) {
    if (raw[i].length === 1 && i + 1 < raw.length && raw[i + 1].length === 1) {
      let run = raw[i];
      while (i + 1 < raw.length && raw[i + 1].length === 1) run += raw[++i];
      joined.push(run);
    } else joined.push(raw[i]);
  }
  const out = new Set();
  for (const bare of joined) {
...
/** True when the token is a trade heading or a place word — the words
 *  GENERIC_TRADE_WORDS lists, in either number. */
function tradeWord(token) {
  return GENERIC_TRADE_WORDS.has(token) || GENERIC_TRADE_WORDS.has(`${token}s`) || (token.endsWith("s") && GENERIC_TRADE_WORDS.has(token.slice(0, -1)));
}
 ...
 */
export function nameOverlap(registerName, candidateName, { placeWords = null } = {}) {
  const a = nameTokens(registerName);
  const b = nameTokens(candidateName);
  if (!a.size || !b.size) return { overlap: 0, shared: [], distinctive: false, placesDisagree: false };
  const shared = [...a].filter((t) => b.has(t));
  let overlap = shared.length / a.size;
  // The row's own city is a place word for this comparison: "Rochester
  // Remodeling and Home Builders" against "Quality Homes of Rochester" is
  // two businesses that both put the town on the sign, and with the trade
  // words set aside each would reduce to "rochester" alone. Excluded from
  // the core reading below, exactly as a trade word is; the whole reading
  // still counts it, as it always has.
  const place = placeWords instanceof Set ? placeWords : new Set(Array.isArray(placeWords) ? placeWords : []);
  const setAside = (t) => tradeWord(t) || place.has(t);
  // Two different towns on the two signs — "Servpro of El Cajon" against
  // "Servpro of Lakeside" — is two branches of one franchise, and no
  // reading of the brand word makes them one business. Reported to the
  // caller, which refuses the identity paths on it as well.
  const aPlace = [...a].filter((t) => place.has(t));
  const bPlace = [...b].filter((t) => place.has(t));
  const placesDisagree = aPlace.length > 0 && bPlace.length > 0 && !aPlace.some((t) => b.has(t)) && !bPlace.some((t) => a.has(t));

  // ── The trade words, set aside ─────────────────────────────────────────
  //
  // "Thompson Plumbing Heating & Air Conditioning Inc" against Google's
  // "Thompson Plumbing" is two of five tokens — 0.4, refused — and it is
  // the same shop with its full trade list on the licence and its short
  // name on the sign. So a second reading is taken with the trade words
  // removed from BOTH names, and the higher of the two is the overlap.
  // Three guards keep that from being a loosening of the rule, and the
  // third was learned the hard way:
  //
  //   1. both names must keep a word — a name that is all trade words
  //      ("Quality Plumbing Services") is judged whole, as before, or
  //      every such name would match every other;
  //   2. when both names carry trade words and share NONE of them —
  //      "Thompson Plumbing" against "Thompson Roofing" — the trade words
  //      are not set aside. A surname in common across two trades is two
  //      family businesses, not one, and the whole-name reading (0.5,
  //      refused) stands;
  //   3. the second reading is SYMMETRIC — shared over the LONGER of the
  //      two cores — where the whole reading is measured against the
  //      register alone. Measured against it on the first dry rematch
  //      (2026-09-21): "MK Best Roofing" against "Atlantic Do it Best
  //      Hardware", "Chimney MD" against "Allied Roofing and Chimney",
  //      "Custom Carpet Centers" against "Rent-A-Center" — one word of
  //      the register's left after the trade words, that one word
  //      somewhere in a longer name, 1.0 under a register-only reading
  //      and all three wrong. Over the longer core they are 0.33, 0.5 and
  //      0.5, and refused. Initialisms count as core words here whatever
  //      their length ("B & W Appliance" is not "D&M Appliance Repair"),
  //      which is why this filters on tradeWord rather than distinctive.
  const aCore = [...a].filter((t) => !setAside(t));
  const bCore = [...b].filter((t) => !setAside(t));
  if (aCore.length && bCore.length && !placesDisagree) {
    const aTrade = [...a].filter(tradeWord);
    const bTrade = [...b].filter(tradeWord);
    const tradesDisagree = aTrade.length > 0 && bTrade.length > 0 && !aTrade.some((t) => b.has(t)) && !bTrade.some((t) => a.has(t));
    if (!tradesDisagree) overlap = Math.max(overlap, aCore.filter((t) => b.has(t)).length / Math.max(aCore.length, bCore.length));
  }
  return {
    overlap,
    shared,
...
/** The words of a row's city (and any extra place names), for
 *  nameOverlap's core reading. */
export function placeWordsFor(...cities) {
  const out = new Set();
  for (const c of cities) for (const t of nameTokens(c)) out.add(t);
  return out;
}

/** One candidate scored against the row. Pure. */
export function scoreCandidate(prospect, place, { placeWords = null } = {}) {
  const name = nameOverlap(prospect?.businessName, place?.displayName?.text ?? place?.displayName, { placeWords: placeWords || placeWordsFor(prospect?.city) });
  ...
  return {
    ...
    placesDisagree: Boolean(name.placesDisagree),
...
export const CONFIRMATION_KEYS = Object.freeze({
  ...
  // The matched_verify verdict (lib/sales/intel/listings.js): the listing
  // answers the record's phone or website under ANOTHER name. Attached, so
  // the rep has Google's fields, and said out loud, so the rep confirms
  // which name answers before pitching to it.
  "identity:phone": "app.salesIntel.places.identity.phone",
  "identity:website": "app.salesIntel.places.identity.website",
  "identity:both": "app.salesIntel.places.identity.both",
});
...
export const CONFIRMATION_TEXT = Object.freeze({
  ...
  "identity:phone": "Google lists this number as {name} — a rebrand or a shared line; confirm on the call",
  "identity:website": "Google lists this website as {name} — a rebrand or a shared line; confirm on the call",
  "identity:both": "Google lists this number and website as {name} — a rebrand or a shared line; confirm on the call",
});

/** The verdict a listing gets when it answers the record's phone or website
 *  under a name the rule refused. Written to Prospect.placesVerdict and
 *  ExternalListing.matchVerdict alike. */
export const MATCHED_VERIFY = "matched_verify";

/**
 * The verify fact for a matched_verify write: which identity signal(s)
 * agreed, under which name. Pure; the check reads the text.
 *
 * @param identity  ["phone"] | ["domain"] | ["phone", "domain"] as
 *                  listings.js scoreProspectForListing reports them
 */
export function verifyConfirmation({ identity = [], name = "" } = {}) {
  const phone = identity.includes("phone");
  const domain = identity.includes("domain");
  const state = phone && domain ? "both" : domain ? "website" : "phone";
  return confirmation("identity", state, { name: String(name ?? "").trim() || "another name" });
}
