...
export function selectPageExcerpts(rows, { maxChars = MAX_EXCERPT_CHARS, perPage = MAX_CHARS_PER_PAGE } = {}) {
  // ── One row per URL: the page, not a detector's note about the page ─────
  //
  // `page_content` is not only the crawler's type. The capability detector
  // files its own rows under it — "718-356-2200", "no ONLINE_BOOKING signal
  // on 2 rendered page(s)" — with the page's URL as sourceUrl. The first
  // version of this function deduplicated by URL in database order, and for
  // Richmond Rolloff that handed the model twelve characters of phone number
  // as the home page while 2,180 characters of the actual page sat in the
  // next row. The longest text for a URL is the page; everything shorter
  // under the same URL is a note about it.
  const longest = new Map();
  for (const r of Array.isArray(rows) ? rows : []) {
    if (!r || r.type !== "page_content" || typeof r.rawValue !== "string" || !r.rawValue.trim()) continue;
    const url = r.sourceUrl || r.normalizedValue || "";
    const have = longest.get(url);
    if (!have || r.rawValue.length > have.rawValue.length) longest.set(url, r);
  }
  const pages = [...longest.entries()]
    .map(([url, r]) => ({ url, text: r.rawValue, priority: pagePriority(url) }))
