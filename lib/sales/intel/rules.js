...
const RULES = [
  ...
  //
  // WEBSITE is false in two cases and this rule fires on both: discovery
  // found no site at all, and the URL on the record turned out to be a
  // directory or a platform profile (lib/sales/intel/siteKind.js — Ring A
  // Ling's "website" was a carpet-cleaner directory). The sentence is
  // written to be true of either: it says "no website of their own", not
  // "no website", because the second is false for a business with a Yellow
  // Pages listing and a rep who reads it out loses the call. The rep's card
  // names which case it is (prospectView.js, "a directory (Ethical
  // Services)"); the pitch is the same pitch.
  {
    code: "NO_WEBSITE",
    name: "No website of their own",
    ...
    reasonTemplate:
      "This business has no website of its own — at most a listing on somebody " +
      "else's — so every enquiry has to arrive by phone, from somebody who " +
      "already knew the name. A site on their own domain, built from the work " +
      "they already do, is the first thing to fix.",
  },
