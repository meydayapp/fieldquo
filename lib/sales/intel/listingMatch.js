...
export function planBbbWrite({ prospect, profile, existingNumbers = [], now = new Date() } = {}) {
  ...
  // Evidence only, no column: the incorporation date says when the entity
  // was formed, which is not when the business started (bbbProfile.js).
  if (profile?.businessIncorporated) cite("businessIncorporated", `BBB lists the business as incorporated ${profile.businessIncorporated}`, profile.businessIncorporatedYear ? String(profile.businessIncorporatedYear) : null);
  if (Number.isInteger(profile?.yearsInBusiness)) cite("yearsInBusiness", `BBB lists ${profile.yearsInBusiness} years in business`, String(profile.yearsInBusiness));
...
export async function applyMapsListing({ db = defaultDb, prospectId, listing, score = null, now = new Date() } = {}) {
  ...
  const plan = planPlacesWrite({
    ...
    now,
    // main's `provenance` (the Maps scrape that landed beside this work
    // uses the same slot): which reader saw the listing, so the evidence
    // and the second number's label say "Google Maps (Apify)" while the
    // Prospect columns and the stamp are exactly a Places lookup's.
    provenance: { detector: MAPS_SCRAPER_DETECTOR, detectorVersion: MAPS_SCRAPER_DETECTOR_VERSION, surface: "Google Maps (Apify)", numberLabel: "Google Maps listing", via: MAPS_SCRAPER_DETECTOR },
  });
  await db.$transaction(async (tx) => {
    await tx.prospect.update({ where: { id: prospectId }, data: plan.data });
    if (plan.evidence.length) await tx.prospectEvidence.createMany({ data: plan.evidence });
