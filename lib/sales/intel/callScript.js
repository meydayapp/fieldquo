...
import { whoToAskFor } from "./people";
 ...
 *
 * @param prospect  { province, statedLanguage } — or anything requiredLanguageFor accepts
...
export function defaultScriptLanguage({ prospect = null, rep = null } = {}) {
  ...
  // Quebec's French is the owner's rule and outranks everything.
  if (required === "fr") return required;
  // The language the person chose on the signup form (lib/signup/leads.js),
  // when the row came from one: they told us, so neither "outside Quebec is
  // English" nor the rep's own preference outranks it. Null on every
  // discovered prospect.
  const stated = normalizeScriptLanguage(prospect?.statedLanguage);
...
export function callScriptInputs({
  ...
  const talkingPoints = [
    ...
    // The plan BBB's employee band most likely fits, as a talking point the
    // pitch can size itself on — cited to BBB and marked self-reported in
    // the sentence itself (lib/sales/intel/planFit.js). After the rule's own
    // points, never instead of them.
    ...(brief?.planFit?.sentence ? [`Likely plan (per BBB, self-reported): ${clip(brief.planFit.sentence)}`] : []),
  ];
  ...
  // What FieldQuo sells to THIS trade — the top three from the one list every
  // pitch surface reads (lib/sales/tradeSellingPoints.js), in the script's
  // language, each as "headline: the benefit. The proof." The model is told
  // to lead the pitch with the first of them. It is in the inputs and so in
  // the hash: a stored script written before the list existed regenerates on
  // its next open, at the usual under-a-cent — the owner's "each trade
  // should have the selling points for that" is worth the row.
  const scriptLang = normalizeScriptLanguage(language) || DEFAULT_SCRIPT_LANGUAGE;
  const tradePoints = tradeSellingPoints(prospect?.tradeKey || null, scriptLang, { limit: TRADE_PITCH_TOP }).points.map(
    (pt) => `${clip(pt.headline, 80)}: ${clip(pt.oneLiner)} ${clip(pt.proof)}`,
  );
...
export function directoryFacts(prospect) {
  ...
  return {
    ...
    // ── Who to ask for, as a first name ──────────────────────────────
    //
    // The card's full citation ("Alexander Singer — Owner (per BBB
    // profile, 2026-09-17)") reaches WHAT WE KNOW through brief.known;
    // this is the SPOKEN form for the opener — "is that Alexander?" — a
    // first name and a role, no surname, no source. From
    // lib/sales/intel/people.js's rule (typed beats BBB beats a
    // register), read off `prospect.people` when the caller loaded it.
    // Empty when nobody is named, never a guess.
    askFor: (() => {
      const who = whoToAskFor(Array.isArray(p.people) ? p.people : []);
      if (!who?.givenName) return "";
      return clip(`${who.givenName}${who.role ? ` (${String(who.role).toLowerCase()})` : ""}`, 80);
    })(),
  };
}
...
function factLines(facts) {
  ...
  // The spoken first name — see directoryFacts.askFor. One fact line, no
  // change to the prompt's sections.
  if (f.askFor) out.push(`Ask for: ${f.askFor}`);
  return out;
}
...
export function callScriptPrompt(inputs) {
  ...
  return [
    ...
    ...(i.tradePoints?.length
      ? [
          "",
          "WHAT WE DO FOR THEIR TRADE (the three we sell to every business in it — real, shipped; say the benefit in these words or plainer, never a feature we do not list here)",
          list(i.tradePoints),
        ]
      : []),
    ...
    "- ifSomeoneElseAnswers: two or three sentences for when the person who picks up is not the owner — the apprentice with the phone on speaker in the van, the office, the spouse who does the paperwork. Say the rep's first name and FieldQuo, ask for the owner by first name when WHAT WE INFERRED gives one, and otherwise ask who prices the work. If the person who answered writes the quotes up, they are the right person — say so TO THEM, as speech: \"and if you're the one who types the quotes up, then you're who I wanted anyway.\" Every sentence here is said out loud to the person who picked up; never a note to the rep such as \"if they are the one who writes the quotes, carry on\" — the first live version-three script wrote exactly that and a rep cannot read it out. Never say the owner is expecting the call, never say you're following up on an email, never pitch to somebody who cannot buy. Ask when the owner is easiest to catch: first thing, or the end of the day.",
