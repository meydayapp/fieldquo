...
export function callScriptPrompt(inputs) {
  ...
  return [
    ...
    "- whatWeSaw: one line per thing under WHAT WE KNOW worth saying out loud, phrased as the rep would say it. Only from the notes.",
    ...
    "- objections: for the two to five likeliest objections from the library, `they` is what they say and `you` is the team's answer in the rep's voice.",
    "- closeAsk: one sentence asking for the next step the tier playbook's last stage asks for.",
    "- doNotSay: things the rep must not claim about this business, because the notes do not support them.",
...
export const CALL_SCRIPT_PROBLEMS = Object.freeze({
  ...
  too_long: `a line was longer than ${CALL_SCRIPT_LIMITS.sentence} characters`,
  digits: "a line contained a digit",
  list_size: "a list was outside its bounds",
 ...
 *
 * @returns { ok, script, problems: [code] }
 *
...
export function validateCallScript(raw) {
  ...
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
    return { ok: false, script: null, problems: ["not_object"] };
  }
  const L = CALL_SCRIPT_LIMITS;
  const line = (v) => {
    const s = typeof v === "string" ? v.replace(/\s+/g, " ").trim() : "";
    if (!s) problems.add("empty_field");
    if (s.length > L.sentence) problems.add("too_long");
    if (/\d/.test(s)) problems.add("digits");
    return s;
  };
  const lines = (v, bounds) => {
    const arr = Array.isArray(v) ? v : [];
    if (arr.length < bounds.min || arr.length > bounds.max) problems.add("list_size");
    return arr.map(line);
  };

  const script = {
    opener: line(raw.opener),
    whatWeSaw: lines(raw.whatWeSaw, L.whatWeSaw),
    whyThemNow: line(raw.whyThemNow),
    threeQuestions: lines(raw.threeQuestions, L.threeQuestions),
    objections: (() => {
      const arr = Array.isArray(raw.objections) ? raw.objections : [];
      if (arr.length < L.objections.min || arr.length > L.objections.max) problems.add("list_size");
      return arr.map((o) => {
        if (!o || typeof o !== "object") { problems.add("bad_objection"); return { they: "", you: "" }; }
        return { they: line(o.they), you: line(o.you) };
      });
    })(),
    closeAsk: line(raw.closeAsk),
    doNotSay: lines(raw.doNotSay, L.doNotSay),
  };

  const list = [...problems];
  return { ok: list.length === 0, script: list.length ? null : script, problems: list };
}
