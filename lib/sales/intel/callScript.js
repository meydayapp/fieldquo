...
export const CALL_SCRIPT_SYSTEM =
  ...
  "a painter, an electrician, a plumber, often in a van. Spoken language, as a person talks, " +
...
export function callScriptPrompt(inputs) {
  ...
  return [
    ...
    "- opener: two to four sentences the rep says first, in the shape of the example. Name the business, say the rep's first name and FieldQuo, and in the SAME breath — as an aside, the way the example does it — that the call may be recorded; then say what we noticed in plain words and the benefit it could bring, then ask for a few minutes. The recording aside is one short clause inside the sentence that says who the rep is, never a sentence of its own and never an announcement.",
