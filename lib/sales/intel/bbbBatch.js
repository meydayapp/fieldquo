 ...
 * @returns { rows, tier counts, skippedRecent }
 */
export async function bbbBatch({ db = defaultDb, scope = "claimed", limit = 60, ids = [], now = new Date() } = {}) {
  ...
  const order = await loadEnrichmentOrder({ db, now, stampField: "bbbCheckedAt", since });
