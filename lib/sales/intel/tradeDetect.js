 ...
 * @param siteBelongsToProspect
 *                  whether this site is KNOWN to be this business's. True for
 *                  every site a source published against the business, which
 *                  is every site the crawler saw before Quebec's RBQ. False
 *                  when the domain was GUESSED — see
 *                  lib/sales/discovery/rbq/derivedSite.js — and the guess has
 *                  not been corroborated by lib/sales/intel/siteIdentity.js.
 *
 *                  A false caps the outcome at `weak`: the evidence and the
 *                  inference are still written, and `Prospect.tradeKey` is
 *                  not. The reason is the one this whole file is organised
 *                  around — a prospect filed under the WRONG trade is worse
 *                  than one filed under none — and an uncorroborated derived
 *                  domain is the one input that can produce a perfectly
 *                  confident reading of somebody else's website. Measured:
 *                  ge.com derived from a GE subsidiary's licence email would
 *                  otherwise establish a trade from General Electric's site.
 *
 *                  It defaults TRUE rather than false, and that is deliberate
 *                  rather than lax: every existing caller passes a site the
 *                  source asserted, and defaulting to false would silently
 *                  stop Overture prospects ever getting a trade — a much
 *                  larger, much quieter regression than the one it guards.
 *                  The RBQ path passes it explicitly.
 *
 ...
 */
export function inferTrade({ crawl = null, prospect = null, siteBelongsToProspect = true } = {}) {
  ...
  // Two conditions, and they are two conditions on purpose. The first asks
  // "does this SITE establish a trade"; the second asks "is this site THIS
  // BUSINESS'S". Collapsing them would make a well-marked-up page owned by
  // somebody else pass, which is precisely the failure siteIdentity.js exists
  // to catch.
  const evidenceConfirms = decisive.verifying === true || decisive.sampleSize >= 2;
  const confirmed = evidenceConfirms && siteBelongsToProspect === true;
  ...
  return {
    ...
    reason: confirmed
      ? null
      : // Named separately from `single_soft_signal`, because they are
        // different problems with different fixes: one wants more of the
        // site read, the other wants a human to say whether this is even
        // their site.
        evidenceConfirms
        ? "site_not_corroborated"
        : decisive.reason || "single_soft_signal",
    confidence: leader.confidence,
