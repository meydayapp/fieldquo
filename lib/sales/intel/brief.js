...
import { bbbFactLine, whoToAskFor } from "./people";
import { planFitForRange } from "./planFit";
import { SERVICES_INFERENCE_KIND, SITE_INFERENCE_KINDS, SITE_INFERENCE_LABELS, servicesNamesFromValue } from "@/lib/sales/inferenceKinds";
...
export const UNKNOWN_REASONS = Object.freeze({
  ...
  no_name_yet: "No register personnel file or BBB profile has named anyone yet — ask on the call and note it.",
  not_listed: "The directory that supplied this record did not list it.",
});

/**
 * "Website changed since <date>" — the one fact a re-crawl adds. Pure.
 *
 * @param crawls  the distinct dates the crawler WROTE evidence for this
 *                prospect, newest first — lib/sales/intel/db.js
 *                loadCrawlHistory. An unchanged re-crawl writes no rows
 *                (crawlSite.js), so two dates here are two crawls that
 *                differed: the site as read on the first was not the site
 *                as read on the second.
 * @returns { changedAt: Date, previousAt: Date } or null with fewer than two
 *
 * "Since <previous>" and not "on <latest>": the card cannot say WHEN the
 * site changed, only that the read on the earlier date no longer described
 * it by the later one. A sentence naming the later date as the change would
 * be a claim the rows do not carry.
 */
export function websiteChange(crawls = []) {
  const dates = (Array.isArray(crawls) ? crawls : [])
    .map((c) => (c instanceof Date ? c : new Date(c)))
    .filter((d) => !Number.isNaN(d.getTime()))
    .sort((a, b) => b - a);
  const distinct = dates.filter((d, i) => i === 0 || d.getTime() !== dates[i - 1].getTime());
  if (distinct.length < 2) return null;
  return { changedAt: distinct[0], previousAt: distinct[1] };
}
...
export function composeBrief({
  ...
  // ── The site was re-read and it had changed ────────────────────────────
  //
  // A fact for the rep, sourced to the website: what the card says below
  // about capabilities and technologies was read from the NEWER pages, and
  // whatever an earlier script or note said about the site may be out of
  // date. Absent on a first crawl and after an unchanged re-crawl — no rows,
  // no line — never "unchanged since", because a site read once carries no
  // statement about change either way.
  {
    const change = websiteChange(crawls);
    if (change) {
      fact("website_changed", "Website changed", `since ${isoDay(change.previousAt)} — re-read ${isoDay(change.changedAt)}`, {
        source: "website",
        websiteChange: { changedAt: change.changedAt.toISOString(), previousAt: change.previousAt.toISOString() },
      });
    }
  }
  ...
  //
  // The match's recorded findings (lib/sales/intel/places.js
  // planPlacesWrite, run by the Maps scrape's matcher in listings.js —
  // nothing here calls Google; the API path was retired 2026-09-20), one
  // fact in the words the rep card uses, sourced to Google rather than the
  // register. A permanently closed listing is its own fact, first, so the
  // script prompt and the opening cannot miss it: the rep confirms before
  // dialling, the row is never suppressed on Google's word alone.
  {
    const result = prospect?.placesResult && typeof prospect.placesResult === "object" ? prospect.placesResult : null;
    const confirmations = Array.isArray(result?.confirmations) ? result.confirmations.filter((c) => c && typeof c.text === "string") : [];
    if ((prospect?.placesVerdict === "matched" || prospect?.placesVerdict === "matched_verify") && confirmations.length) {
      const closed = confirmations.find((c) => c.code === "status:closed");
      if (closed) fact("google_closed", "Google says", closed.text, { source: "google_places" });
      // matched_verify: the listing answers the record's number or website
      // under another name (listings.js). Its own fact, ahead of the check,
      // so the script prompt tells the rep to confirm which name answers.
      const verify = confirmations.find((c) => c.field === "identity");
      if (verify) fact("google_verify", "Confirm on the call", verify.text, { source: "google_places" });
      fact(
        "google_check",
        "Google check",
        confirmations.filter((c) => c.code !== "status:closed" && c.field !== "identity").map((c) => c.text).join("; ") || "Matched the listing",
  ...
  //
  // A FACT, not an inference: a regulator's personnel file, a BBB profile
  // or the rep's own ear said the name, and the source is on the line —
  // "Alexander Singer — Owner (per BBB profile, 2026-09-17)". The script
  // prompt's WHAT WE KNOW carries it from here (callScript.js reads
  // brief.known), so the opener can ask "is that Alexander?". The
  // `decision_maker` inference below stays what it was: something the
  // contractor SAID on a recorded call, a different claim.
  {
    const who = whoToAskFor(Array.isArray(prospect?.people) ? prospect.people : []);
    if (who) fact("who_to_ask", "Who to ask for", who.text, { source: who.source, principal: { name: who.name, givenName: who.givenName, role: who.role, source: who.source } });
    else gap("who_to_ask", "Who to ask for", "no_name_yet");
    const bbb = bbbFactLine(prospect);
    if (bbb) fact("bbb", "BBB", bbb, { source: "bbb" });
    // The plan that band most likely fits (lib/sales/intel/planFit.js) — a
    // fact about what BBB holds carrying a guess about the plan, and the
    // sentence says which is which. Sourced to BBB so the script prompt's
    // WHAT WE KNOW cites it as BBB's, self-reported. No band, no line.
    const fit = planFitForRange(prospect?.employeeRange);
    if (fit) fact("plan_fit", "Likely plan", fit.sentence, { source: "bbb", planFit: { kind: fit.kind, seats: fit.seats, crew: fit.crew, tierKey: fit.tier?.tierKey || null } });
  }
  ...
  // ── What their own site said about them ─────────────────────────────────
  //
  // The INFER_FROM_SITE stage's rows, one line per kind it wrote, in the
  // inference layer with the sentence-evidence it cites. No gap line for a
  // kind it did not write: the pages not mentioning a busy season is not a
  // statement about their busy season, and ten "not known" lines would push
  // the real gaps off the script prompt's bounded list.
  for (const kind of SITE_INFERENCE_KINDS) {
    const row = inferences.find((i) => i?.kind === kind && text(i?.value));
    if (!row) continue;
    known_.push({
      id: kind,
      label: SITE_INFERENCE_LABELS[kind] || kind,
  ...
  // ── The services their site lists, each name cited to a row ─────────────
  //
  // The evidence-cited list (servicesOffered.js): menu labels, sitemap
  // paths, JSON-LD offers, the WordPress page index. One line, the names
  // in the business's own words, in the inference layer with the rows it
  // cites. No gap line when absent — a site with no readable menu has not
  // said it offers nothing.
  {
    const row = inferences.find((i) => i?.kind === SERVICES_INFERENCE_KIND && text(i?.value));
    const names = row ? servicesNamesFromValue(row.value) : [];
    if (names.length) {
      known_.push({
        id: "services",
  ...
  return {
    ...
    known: known_,
    unknown,
    // The plan fit on its own, for the pitch layer of the card and the
    // script prompt's talking points; the same object the "Likely plan"
    // fact above was written from.
