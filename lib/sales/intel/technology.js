...
//     inlineTokens[]  vendor init tokens read out of inline <script> bodies
//                     (html.js scanInlineScript). Loose — part of markupOf().
//     renderedText    copy recovered from a framework's render payload
//                     (structured.js). "" when there was none. NOT folded
//                     into `text`: a reader that wants the page's own words
//                     and a reader that wants what a script would have drawn
//                     are asking different questions, and capabilityDetect.js
//                     reads both under different rules.
//     headings[]      h1–h3 texts, for servicesOffered.js
//     via             how the crawler reached the page: "start" | "nav" |
//                     "probe", or null for a row written before it was kept
//     navMatch        the navigation kind the page was ranked as ("contact",
//                     "booking"…), or null
//   }
//
...
export function normalisePage(raw) {
  ...
  return {
    ...
    // From the link RECORDS when the page still has them, and from an
    // already-normalised page's own `linkTexts` otherwise — because `links`
    // above is flattened to strings, and normalising a normalised page used
    // to read those strings, find no anchor text, and empty the list. That
    // is the production path: detectTechnology.js's loadCrawl normalises,
    // and detectCapabilities / inferTrade / classifySiteKind normalise again,
    // so every label signal (a "Book Online" button, a "Claim this listing"
    // menu item) was invisible to the pipeline and visible only to a check
    // that handed the detector raw pages. Measured on the Ethical Services
    // fixture: 27 anchor texts once, 0 twice. Idempotent now — the check
    // asserts normaliseCrawl(normaliseCrawl(x)) deep-equals
    // normaliseCrawl(x).
    linkTexts: linkTextArray(Array.isArray(page.linkTexts) && page.linkTexts.length ? page.linkTexts : page.links),
    ...
    inlineTokens: strArray(page.inlineTokens, 50),
    renderedText: str(page.renderedText),
    // Strings, or the extractor's `{ level, text }` records — the text is
    // the observation either way.
    headings: strArray((Array.isArray(page.headings) ? page.headings : []).map((h) => (h && typeof h === "object" ? h.text : h)), 60),
    // Three-valued like `ok`: a string when the crawler said how it got here,
    // null when the row predates the field. capabilityDetect.js reads null as
    // "provenance unknown", never as either navigation or a probe.
    via: str(page.via),
    navMatch: str(page.navMatch),
    // Three-valued like everything else here: an integer when the crawler said
    // how many links the page had, null when nothing said. Never derived from
    // `links.length` — that is the fallback, and it belongs at the point of
    // USE so a reader can see which answer it got.
    linkCount: Number.isInteger(page.linkCount) ? page.linkCount : null,
  ...
  for (const raw of Array.isArray(rows) ? rows : []) {
    ...
    switch (raw.type) {
      case "page_fetch": {
        ...
        // How many links the page CARRIED, which is not how many rows we kept:
        // lib/sales/crawl/evidence.js stores one row per distinct observation,
        // so a page whose every link is the shared navigation contributes none
        // of its own. capabilityDetect's looksRendered() asks whether the page
        // had links at all — a body with none is a JavaScript shell — and that
        // question has to be answered by the page, not by our storage policy.
        // Absent on rows written before the counts existed, and null there
        // means "fall back to the rows", which is what those rows support.
        if (Number.isInteger(envelope.counts?.links)) page.linkCount = envelope.counts.links;
        // Set only when the envelope carries them. Rows are read newest first
        // and an older crawl of the same URL follows; its envelope has no
        // `via`, and letting it reset the field would turn a page the new
        // crawl reached by navigation back into "provenance unknown".
        if (typeof envelope.via === "string") page.via = envelope.via;
        if (typeof envelope.navMatch === "string") page.navMatch = envelope.navMatch;
        break;
      }
      case "page_content":
      ...
      case "dom_attr":
        ...
        break;
      case "inline_token":
        if (rawValue) page.inlineTokens.push(rawValue);
        break;
      case "rendered_text":
        page.renderedText = [page.renderedText, rawValue].filter(Boolean).join("\n") || null;
        break;
      case "heading":
        if (rawValue) page.headings.push(rawValue);
        break;
      default:
        break;
    }
