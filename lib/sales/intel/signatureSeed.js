...
const SIGNATURES = [
...

  {
    ...
    active: true,
    sourced:
      "VENDOR + LIVE + WILD. A real gutter company's page carries vcita's own inline snippet — " +
      "window.liveSiteAsyncInit = function(){ LiveSite.init({ id: 'WI-…' }) } — which creates " +
      "<script src=\"https://d2ra6nuwn69ktl.cloudfront.net/assets/livesite.js?…\"> at runtime " +
      "(the host literal sits in the body WITHOUT a scheme, which is why html.js scans inline " +
      "bodies for loader URLs). That CloudFront host was confirmed as vcita's by fetching " +
      "livesite.js (200, 184 KB): it declares portalHost:\"clients.vcita.com\", host:\"www.vcita.com\" " +
      "and renders #livesite_active_engage (28 occurrences). Townsquare Interactive white-labels " +
      "the same widget: at runtime it links to engage.townsquareinteractive.com/site/<id>/action/… " +
      "(Schedule Now, Make a Payment, Contact us, My Account) and that host answers live, so a " +
      "link to it IS vcita and is matched here rather than under the agency's own signature. " +
      "vcita is a client-portal, scheduling and payments product; the widget's actions are " +
      "configurable per account, which is why capabilityDetect.js proves them at 0.7 rather than " +
      "Jobber's 0.9.",
    patterns: [
      { kind: "script_src", pattern: "cloudfront.net/assets/livesite.js", weight: 0.9 },
      { kind: "script_src", pattern: "vcita.com", weight: 0.85 },
      { kind: "iframe_host", pattern: "vcita.com", weight: 0.85 },
      { kind: "link", pattern: "vcita.com", weight: 0.6 },
      { kind: "script_src", pattern: "engage.townsquareinteractive.com", weight: 0.85 },
      { kind: "iframe_host", pattern: "engage.townsquareinteractive.com", weight: 0.85 },
      { kind: "link", pattern: "engage.townsquareinteractive.com", weight: 0.65 },
      { kind: "html", pattern: "LiveSite.init", weight: 0.5 },
      { kind: "html", pattern: "livesite_active_engage", weight: 0.5 },
    ],
  },
  {
    code: "CALLRAIL",
    ...
    active: true,
    sourced:
      "LIVE + DB. js.calltrk.com answered (204 to a bare request, which is the host working). " +
      "Open fingerprint databases list js.calltrk.com/companies/…/….js scripts and a CallTrk " +
      "global. A tracking number swapper proves nothing about how the business quotes, books " +
      "or gets paid, so this maps to no capability — it is recorded because a contractor paying " +
      "for call tracking is a contractor who measures where calls come from, which is a real " +
      "talking point and a real budget line.",
    patterns: [
      { kind: "script_src", pattern: "calltrk.com", weight: 0.9 },
      { kind: "html", pattern: "calltrk", weight: 0.5 },
    ],
  },
     ...
     ═══════════════════════════════════════════════════════════════════ */
  {
    ...
    active: true,
    sourced:
      "VENDOR + LIVE + WILD. duda.co itself references cdn-website.com (100 times) and " +
      "multiscreensite.com. A real gutter company's Duda-built site loads " +
      "static.cdn-website.com/mnlt/production/…/_dm/s/rt/dist/scripts/…, irp.cdn-website.com " +
      "stylesheets, and /_dm/s/rt/smart/*.js on its own host; its form posts to " +
      "/_dm/s/rt/widgets/dmform.submit.jsp. The /_dm/ path is Duda's runtime prefix on every " +
      "site it hosts, including white-labelled ones — Townsquare Interactive resells Duda — so " +
      "it is matched as a path pattern rather than only by the CDN host.",
    patterns: [
      { kind: "script_src", pattern: "cdn-website.com", weight: 0.9 },
      { kind: "script_src", pattern: "multiscreensite.com", weight: 0.85 },
      { kind: "script_src", pattern: "/_dm/s/rt/", weight: 0.8 },
      { kind: "link", pattern: "cdn-website.com", weight: 0.7 },
      { kind: "html", pattern: "dmform.submit", weight: 0.5 },
    ],
  },
  {
    code: "TOWNSQUARE_INTERACTIVE",
    name: "Townsquare Interactive",
    isCompetitor: ADJACENT,
    active: true,
    sourced:
      "LIVE + WILD. A marketing agency that builds contractor sites on Duda and bundles vcita's " +
      "LiveSite under its own engage.townsquareinteractive.com host. The agency's own site " +
      "answered live. Matched here on the agency's OWN hosts only — a 'Site by Townsquare " +
      "Interactive' credit link, or an asset from townsquareinteractive.com. The engage host is " +
      "deliberately NOT here: it is the vcita widget wearing the agency's name, and it belongs " +
      "to the VCITA_LIVESITE signature, which is what actually proves the portal, booking and " +
      "payments. An agency credit alone proves none of those, so this maps to no capability.",
    patterns: [
      { kind: "script_src", pattern: "townsquareinteractive.com", weight: 0.85 },
      { kind: "iframe_host", pattern: "townsquareinteractive.com", weight: 0.8 },
      { kind: "link", pattern: "townsquareinteractive.com", weight: 0.65 },
      { kind: "html", pattern: "townsquareinteractive", weight: 0.5 },
    ],
  },
  {
    code: "WIX",
