...
export const CAPABILITY_DETECTOR_VERSION = "3";
...
export function absenceEligibility(crawl) {
  ...
  const followed = rendered.some((p) => p.via === "nav");
...
export function detectCapabilities({ crawl = null, technologies = [], prospect = null, siteKind = null } = {}) {
  ...
  for (const code of DETECTED_CAPABILITY_CODES) {
    if (code === "WEBSITE") {
      capabilities.push(listing ? listingWebsiteCapability(listing, pages) : websiteCapability({ prospect, normalised, pages }));
      continue;
    }
    ...
    if (listing && !KEPT_ON_A_LISTING.has(code)) {
      capabilities.push({ code, value: null, confidence: 0, evidence: [], reason: "not_their_own_site" });
      continue;
    }
