...
//
// lib/sales/intel/places.js starts from a Prospect and asks Google for it:
...
// names share nothing at all — and even then it is refused, not attached,
// because "same number, different name" is exactly the shape of a shared
// answering service or a sold business, and a human decides those.
//
...
// ══ Coordination note ══════════════════════════════════════════════════════
//
// The principal-contacts / BBB / Apify work builds enrichmentOrder.js and
// the people tables in parallel with this file. ExternalListing's base
// columns are that work's (read back from the live table); everything here
// reads only those names plus the delimited maps-scrape block in the
// schema. enrichmentPairs() below is the stand-in for its dispatch order —
// open claims only, trade × city — and should be replaced by the shared
// function the moment it lands on main.
...
export function scoreProspectForListing(listing, prospect) {
  ...
  const base = scoreCandidate(prospect, place);
  const identity = [];
  ...
  if (listing.domain && prospect.domain === listing.domain) identity.push("domain");
  ...
  for (const alt of prospect.tradingNames || []) {
    const o = nameOverlap(alt, listing.name);
  ...
  if (!accept && identity.includes("place_id")) {
    ...
    accept = true;
    reason = null;
  } else if (!accept && identity.length) {
...
export function matchListing(listing, candidates = []) {
  ...
  return {
    verdict: top.score.accept ? LISTING_VERDICTS.MATCHED : LISTING_VERDICTS.NO_CONFIDENT_MATCH,
    prospect: top.score.accept ? top.prospect : null,
    score: { ...top.score, alsoAccepted },
...
export async function applyListing({ db = defaultDb, raw, now = new Date(), runId = null, dry = false, deps = {} } = {}) {
  ...
  if (match.verdict !== LISTING_VERDICTS.MATCHED) {
  ...
  const row = listingRow(listing, {
    ...
    matchedProspectId: plan.verdict === PLACES_VERDICTS.MATCHED ? prospect.id : null,
