...
// form it can quote.
//
// ══ Services: the model may name only what the evidence names ═════════════
//
// Version 2 adds `services_offered`, and it is the one kind where a quote
// check is not enough: "Roofing, siding and gutters" is a plausible quote
// from a page that says "we handle roofing" and a menu that lists siding —
// gutters came from the model's idea of a roofer. So the prompt is handed
// the evidence-cited list (servicesOffered.js — menu, sitemap, JSON-LD,
// WordPress index) under SERVICES THEY LIST with the instruction not to add
// a service that is not in the evidence, and validateSiteInferences()
// splits the value into names and drops every name that is not a
// case- and accent-insensitive substring of the material (the page text or
// that list). A value with no surviving name is dropped and counted as
// `service_not_in_evidence`. The check asserts the drop.
//
...
export const SITE_INFERENCE_VERSION = "2";
...
export const KIND_GUIDANCE = Object.freeze({
  ...
  services_offered: "the services they offer, as a short comma-separated list in their own words — ONLY names from SERVICES THEY LIST below or words that appear on the pages; never add a service that is not in the evidence",
});

/** Names in a services_offered value, at most this many. */
export const MAX_SERVICE_NAMES = 12;

/** Accent-folded, lower-cased, punctuation collapsed. */
function fold(value) {
  return String(value ?? "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
/** The names in a comma- or "and"-separated list, trimmed, deduplicated. */
export function splitServiceNames(value) {
  const parts = String(value ?? "")
    .split(/\s*(?:,|;|\band\b|&|\/|·|•)\s*/i)
    .map((p) => p.replace(/\s+/g, " ").trim().replace(/^(?:and|et|y)\s+/i, ""))
    .filter((p) => p.length >= 2);
  const seen = new Set();
  const out = [];
...
/**
 * Which of these names the material actually contains — folded substring
 * of any source text — and which it does not.
 *
 * @returns { kept: string[], dropped: string[] }
 */
export function servicesInMaterial(names, sources = []) {
  const haystack = (Array.isArray(sources) ? sources : []).map((s) => fold(s?.text)).join("\n");
  const kept = [];
  const dropped = [];
  for (const name of Array.isArray(names) ? names : []) {
    const key = fold(name);
    if (key && haystack.includes(key)) kept.push(name);
 ...
 */
export function siteInferenceInputs({ pages = [], prospect = null, services = [] } = {}) {
  const p = prospect || {};
  return {
    ...
    // The evidence-cited service names (servicesOffered.js), as printed to
    // the model. In the hash, so a menu the crawler newly read re-reads.
...
/** The line the prompt prints for the services, and the text a quote or a
 *  name is checked against. One string, so both readers see the same one. */
export function servicesLine(inputs) {
  const names = inputs?.services || [];
...
export function siteInferenceSources(inputs) {
  const pages = (inputs?.pages || []).map((p) => ({ url: p.url || "", text: p.text || "" })).filter((s) => s.text);
  const line = servicesLine(inputs);
  // The services list is material too: a quote of it, or a name from it,
  // is a quote of what the site's own menu, sitemap or JSON-LD said.
  return line ? [...pages, { url: "services", text: line }] : pages;
}
...
export function siteInferencePrompt(inputs) {
  ...
  return [
    ...
    "",
    "SERVICES THEY LIST (read from their menu, sitemap and structured data — do not add a service that is not in the evidence)",
    "[services]",
    servicesLine(i) || "(none found — name a service only if the excerpts above state it)",
    "",
    ...
    `- services_offered: a comma-separated list of at most ${MAX_SERVICE_NAMES} names, each copied from SERVICES THEY LIST or from the excerpts. A name that is in neither is thrown away. Quote the list line or the sentence the names came from.`,
  ].join("\n");
}
...
export const DROP_REASONS = Object.freeze({
  ...
  service_not_in_evidence: "no service name in the value appears in the evidence the model was shown",
});
...
export function validateSiteInferences(raw, sources = []) {
  ...
  for (const e of entries) {
    ...
    let value = clip(spellDigits(typeof e?.value === "string" ? e.value : ""), SITE_INFERENCE_LIMITS.value);
    let servicesDropped = [];
    if (kind === "services_offered") {
      // Name by name, against the material: a value of five names of which
      // one is invented keeps four. A value with none left is dropped.
      const names = splitServiceNames(typeof e?.value === "string" ? e.value : "");
      const checked = servicesInMaterial(names, sources);
      servicesDropped = checked.dropped;
      value = clip(spellDigits(checked.kept.slice(0, MAX_SERVICE_NAMES).join(", ")), SITE_INFERENCE_LIMITS.value);
      if (!checked.kept.length) {
        dropped.push({ kind, reason: "service_not_in_evidence", names: checked.dropped });
        continue;
      }
    }
    if (!quote || !value) {
    ...
    kept.push({
      ...
      ...(servicesDropped.length ? { droppedNames: servicesDropped } : {}),
    });
  }
  return { kept, dropped };
}
