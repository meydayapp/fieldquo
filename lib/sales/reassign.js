...
export function heldBy(prospect = {}, salesRepId, now = new Date()) {
  ...
  // Retired by a merge: its claim moved to the survivor, and queueWhere no
  // longer matches it. The twin has to agree.
...
export function planReassign({
  ...
  if (toRep && toRep.id === toRepId) {
    const blocked = held.filter((p) => !repCanTake(toRep, p));
    const cannot = blocked.length;
    if (cannot > 0) {
      // Which language the target lacks, named from the rows themselves:
      // Quebec rows need French, every other provinced row needs English
      // (lib/sales/leadLanguage.js — the rule is symmetric since 2026-09-12).
      const needsFrench = blocked.filter((p) => requiredLanguageFor(p) === "fr").length;
      const needsEnglish = cannot - needsFrench;
      const what =
        needsFrench && needsEnglish
          ? `${needsFrench} in Quebec (French) and ${needsEnglish} outside it (English)`
          : needsFrench
            ? "in Quebec and can only go to a rep who sells in French"
            : "outside Quebec and can only go to a rep who sells in English";
      const missing = needsFrench && needsEnglish ? "French and English" : needsFrench ? "French" : "English";
      return {
        error: `${cannot} of the held lead${cannot === 1 ? " is" : "s are"} ${what}. ${
          toRep.name ? `${toRep.name} has` : "That rep has"
        } no ${missing} in their languages — set it on their card, or choose a rep who has.`,
        code: "language",
        cannot,
      };
    }
