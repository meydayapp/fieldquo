 ...
 * @param toRep     `{ id, active }` for the target, when the caller has it. A
 *                  target known to be inactive is refused here, and the
 *                  executor refuses one it cannot find at all.
...
export async function reassignHeld({
  ...
  const [prospects, leads, last] = await Promise.all([
    db.prospect.findMany({
      ...
      select: { id: true, assignedRepId: true, assignedAt: true, claimExpiresAt: true },
    }),
 ...
 */
export async function handoffTargets({ db, admin, excludeRepId = null } = {}) {
  ...
  const mine = adminEmail
    ...
    : null;
  return {
    targets: reps.map((r) => ({ id: r.id, name: r.name, code: r.code, isMe: Boolean(mine && mine.id === r.id) })),
