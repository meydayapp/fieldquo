 ...
 *
 *  Grown from a hand-check of the menus of 2,000 crawled prospects
 *  (scripts/measure-sales-services.mjs, 2026-09-14): "Request", "Blog:",
 *  "View All Blogs", "Terms Of Conditions", "Our Promise", "Thank You",
 *  "JOB POSITIONS", "Call or Text", "Watch Video", "Serving Area",
 *  "BEFORE & AFTER", "À propos", "Nous joindre", "Carrières", "Politique de
 *  confidentialité", "Réalisations", "Nos partenaires" were all being read
 *  as things the business sells. English, French and Spanish, because the
 *  prospects are. Matched on the whole label, punctuation trimmed. */
const NOT_A_SERVICE = /^(home|menu|back|next|previous|prev|more|read more|learn more|click here|here|skip to (main )?content|top|close|open|toggle|search|share|print|download|blogs?|news|resources|privacy( policy)?|terms( (of|and) (use|service|conditions?))?|terms of conditions|sitemap|cookies?( policy)?|accessibility|referrals?|referral program|financing|coupons?|specials?|promotions?|careers|jobs?|job positions|positions|employment|login|log in|sign in|sign up|register|cart|checkout|shop|store|subscribe|newsletter|en|fr|es|english|fran[cç]ais|espa[nñ]ol|request|request (a )?(call ?back|quote|estimate|service)|call( or text)?|text( us)?|call us|email us|watch( the)? video|video|photos?|gallery|galleries|portfolio|before (&|and) after|testimonials?|reviews?|faqs?|our (promise|team|story|process|work|history|mission|values|guarantee)|thank you|serving areas?|service areas?|areas? (we )?serve[d]?|locations?|about( us)?|contact( us)?|get in touch|view all( blogs| posts| services)?|see all|(pricing|price) guides?|guides?|resources?|partners?|our partners|links|events|community|press|media|awards|financing options|pay(ment)?s?( online)?|make a payment|client (portal|login)|customer (portal|login)|my account|schedule( now| online)?|book( now| online)?|free (quote|estimate)s?|get (a )?(free )?(quote|estimate)|estimates?|quotes?|à propos|a propos|qui sommes[- ]nous|nous joindre|contactez[- ]nous|contact|carri[èe]res?|emplois?|politique de confidentialit[ée]|conditions d'utilisation|r[ée]alisations|nos partenaires|partenaires|plan du site|plan de site|accessibilit[ée]|formulaires?|tarification|nouvelles|publications|foire aux questions( \(faq\))?|t[ée]moignages|galerie|blogue|accueil|demander une soumission|soumission( gratuite)?|obtenez une soumission|nos produits|produits|products?|our products|nos services|services|sobre nosotros|nosotros|contacto|cont[áa]ctenos|empleos?|pol[íi]tica de privacidad|inicio|galer[íi]a|testimonios|cotizaci[óo]n( gratis)?|solicitar (una )?cotizaci[óo]n|nuestros servicios|servicios|what we (do|build|offer)|what to expect|why (choose )?us|why us)[:!.]?$/i;

/** Is this label chrome — "Home", "Read more", "Privacy" — or a priority
 *  page's own name ("Contact", "About us")? Exported for
 *  lib/sales/intel/servicesOffered.js, which applies the same test to a
 *  WordPress page title and a heading that serviceMenu() applies to a link,
 *  so the two cannot disagree about what a service is not. */
/** Phrases that make a label an affordance or an article wherever they sit
 *  in it: "Schedule Your Inspection" is how you book, "Bathroom Pricing
 *  Guide" is a post. */
const CHROME_ANYWHERE = /\b(pricing guide|price guide|schedule (your|an?|my|online|now|a)|book (your|an?|my|online|now|a)|bookings?|log ?in|sign ?up|read more|learn more|blog|hire us|click here|closed captioning|what to expect|start to finish|career|careers|subcontractor|benefits package|team member|community service|inquiry|about our|about us|en d[ée]couvrir|d[ée]couvrir plus|en savoir plus|pourquoi|why choose|devis|garantie|garantee|warranty|financing|coupon|newsletter|privacy|terms)\b/i;

/** A token that is not a word: a username, an SKU, a hashtag-shaped id
 *  ("CAMDENOLIVERO33"), or a label that is mostly digits. Found in a live
 *  hand-check of 50 menus. */
const NOT_A_WORD = /^[A-Z0-9_]{9,}$|^\W*\d[\d\s.-]*\W*$|\d{3,}/;

export function isChromeLabel(label) {
  const clean = String(label || "").replace(/\s+/g, " ").trim();
  if (clean.length < 2 || clean.length > 60) return true;
  if (NOT_A_SERVICE.test(clean)) return true;
  if (CHROME_ANYWHERE.test(clean)) return true;
  if (NOT_A_WORD.test(clean)) return true;
  // An article title, not a menu item: a question, a number up front ("5
  // Benefits of Hiring Local Remodelers"), a colon-ended heading, or more
  // than five words. Menus name things in a word or three.
  if (/[?]/.test(clean) || /^\d/.test(clean) || /:$/.test(clean)) return true;
  if (clean.split(" ").length > 5) return true;
  return textKind(clean).rank !== Infinity;
}
