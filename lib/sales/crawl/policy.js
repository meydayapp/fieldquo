...
// ── DNS backoff, per host ──────────────────────────────────────────────────
//
// 772 crawls — the largest failure group there is — ended
// `robots_error:unsafe_host:dns_error:EBUSY`. EBUSY out of a lookup is not the
// host: it is the local resolver reporting that it has no capacity for one
// more question. fetchFailureOutcome does not classify it, robotsFetchOutcome
// says "unknown, retry", and the runner's ladder retried it at 1, 2, 4 and 8
// minutes — inside the same busy window, five times, for every one of those
// hosts. About 3,900 lookups spent confirming that the resolver was busy.
//
// The right answer to a resolver out of capacity is fewer lookups, spread
// out. So a host whose lookup came back busy is HELD, in CrawlHostPolicy
// where robots verdicts and 429 blocks already live, for a period that
// doubles on each busy answer and clears the moment a lookup for it succeeds.
// The crawl that finds the hold returns a retryable failure carrying the
// wait, and the runner's failureOutcome takes the longer of its ladder and
// that wait — so the five attempts spread over hours instead of minutes.

/** The first hold. Half an hour: long enough for a busy resolver to drain,
 *  short enough that a claimed prospect's crawl is not lost to the morning. */
export const DNS_BACKOFF_BASE_MS = 30 * 60 * 1000;

/** The longest hold, at the fifth busy answer and after. Same six hours as
 *  the runner's own BACKOFF_MAX_MS, for the same reason: a campaign started in
 *  the morning has finished waiting by the evening. */
export const DNS_BACKOFF_MAX_MS = 6 * 60 * 60 * 1000;

/**
 * Is this fetch error the resolver being busy, rather than the name?
 *
 * EBUSY and EAI_AGAIN are "try again later" from the resolver; EMFILE and
 * ENFILE are the process out of descriptors, which a lookup surfaces first
 * because it is the first thing that opens one. ENOTFOUND is deliberately NOT
 * here — that is the name, and fetchFailureOutcome already calls it terminal.
 */
export function resolverBusy(error) {
  return /dns_error:(EBUSY|EAI_AGAIN|EMFILE|ENFILE)\b/.test(String(error || ""));
}

/**
 * The hold to write after one more busy answer.
 *
 * @param failures  busy answers so far, BEFORE this one
 * @returns { failures, until, waitMs } — the new count and when it lifts
 */
export function nextDnsBackoff({ failures = 0, now = new Date() } = {}) {
  const made = Math.max(0, Math.floor(Number(failures) || 0));
  const at = (now instanceof Date ? now : new Date(now)).getTime();
  // Math.min BEFORE the shift, the same guard backoffMs keeps: a wild count
  // must not overflow into an Invalid Date.
  const waitMs = Math.min(DNS_BACKOFF_BASE_MS * 2 ** Math.min(made, 40), DNS_BACKOFF_MAX_MS);
  return { failures: made + 1, until: new Date(at + waitMs), waitMs };
}

/**
 * Is this host held for DNS right now?
 *
 * @returns { act: "hold", until, waitMs } | { act: "go" }
 *
 * A hold in the past is over, and a row with no hold was never held. Read
 * from the same policy row the slot decision reads, so it costs no query.
 */
export function dnsBackoffDecision({ policy = null, now = new Date() } = {}) {
  const until = policy?.dnsBackoffUntil ? new Date(policy.dnsBackoffUntil) : null;
  if (!until || Number.isNaN(until.getTime())) return { act: "go" };
  const at = (now instanceof Date ? now : new Date(now)).getTime();
  if (until.getTime() <= at) return { act: "go" };
  return { act: "hold", until, waitMs: until.getTime() - at };
}
 ...
 */
/**
 * May a crawl that could not CONNECT over https try the same host over http?
 *
 * Only for a transport failure — the socket refused, reset, timed out, or
 * the TLS handshake failed (undici reports every one of those as a bare
 * `TypeError`). Never for an answer: a 404, a 500 or a redirect over https is
 * the site talking, and a site that talks on https is not downgraded. Never
 * for a URL that was unsafe, a host that did not resolve (it will not resolve
 * on port 80 either), or a request the politeness gate refused.
 *
 * The case: www.ethicalservices.com serves http only and refuses 443. The
 * crawler's default for a schemeless address is https, so the robots fetch
 * failed, the task retried, and the site was never read.
 */
export function httpFallbackEligible(error) {
  const raw = String(error || "");
  if (!raw) return false;
  if (/^(unsafe_url|unsafe_host|not_permitted):/.test(raw)) return false;
  if (raw === "too_many_redirects" || raw === "bad_redirect_location") return false;
  if (/ENOTFOUND|EAI_AGAIN|NXDOMAIN|dns_/i.test(raw)) return false;
  return true;
}
