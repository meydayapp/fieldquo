...
export async function crawlProspectSite({
  ...
  const homePage = pageRecordFor(homeAttempt, extractIfHtml(homeAttempt), { via: "start", navMatch: null });
  pages.push(homePage);
  ...
  // routed to `/p/1187` both count. Blind probes only when the ranking is
  ...
  if (queue.length === 0) {
    ...
    probed = true;
    for (const probe of probeCandidates(vetted.url.toString())) {
  ...
  for (const candidate of queue) {
    ...
    const provenance = { via: candidate.via, navMatch: candidate.kind || null };
    if (result.blocked) {
 ...
 *
 *  `provenance` — { via, navMatch } — is stamped on every record, including a
 *  failed fetch: "the probe for /contact 404'd" is the fact that stops an
 *  absence claim, and it has to survive into the page_fetch envelope. */
function pageRecordFor(attempt, extracted, provenance = { via: null, navMatch: null }) {
  if (extracted) {
 ...
 */
async function writeCrawl({ db, prospect, pages, now, nextHash = null, changed = true, hadContent = false, derived = false, deps = {} }) {
