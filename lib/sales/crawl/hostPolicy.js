...
import { hostSlotDecision, MAX_WAIT_MS, nextDnsBackoff } from "./policy";
...
export async function recordRobots(db, { host, allowed, crawlDelayMs = null, deps = {} } = {}) {
  ...
  return db.crawlHostPolicy.update({
    ...
    data: {
      ...
      // robots.txt was fetched, so the name resolved: whatever DNS hold the
      // host carried is over, and the count starts again from nothing.
      dnsFailures: 0,
      dnsBackoffUntil: null,
    },
...
/**
 * Record that a lookup for this host came back busy, and hold the host.
 *
 * The hold doubles per busy answer (policy.js nextDnsBackoff) and is cleared
 * by recordRobots the next time the name resolves. Written as an update on
 * the row ensureHostPolicy already created, on the count that was read; two
 * runs recording the same busy answer at once both write, and the second
 * merely lands on the longer hold, which is the safe direction.
 *
 * @returns { failures, until, waitMs } — what was written
 */
export async function recordDnsFailure(db, { host, deps = {} } = {}) {
  const clock = deps.clock || (() => new Date());
  const key = String(host || "").toLowerCase();
  if (!key) return null;

  const current = await ensureHostPolicy(db, key);
  const next = nextDnsBackoff({ failures: current?.dnsFailures ?? 0, now: clock() });

  await db.crawlHostPolicy.update({
    where: { host: key },
    data: { dnsFailures: next.failures, dnsBackoffUntil: next.until },
    select: HOST_POLICY_SELECT,
  });
  return next;
}
