...
//       VOLATILE_META)
//     · script sources, sorted, WITHOUT their query strings — the declared
//       <script src> tags AND the loader URLs html.js reads out of inline
//       bodies, folded into the SAME list. Folded, not a new key: a new key
//       changes the canonical form of every page and therefore every hash,
//       which is a whole-corpus re-analysis at an OpenAI call each. In the
//       same list, a page with no inline loader hashes exactly as before and
//       only a page that HAS one — where the extraction genuinely learned
//       something — reads as changed.
...
//     · inline script and style BODIES — where the nonces live. The loader
//       URLs are read out of them (html.js) and the body is then discarded;
//       vcita's snippet appends a timestamp to its URL on every render, and
//       stableUrl() drops the query, so that is not a change either
//     · the vendor tokens read out of inline bodies (`LiveSite.init`) — they
//       are implied by the loader URL beside them and the hash needs one of
//       the two, not both
//     · how the crawler REACHED a page (`via`, `navMatch`), and the service
//       menu — the first is about us, the second is already in `links`
...
export function canonicalPage(page) {
  ...
  return {
    ...
    scripts: sorted([
      ...(page.scripts || []).map((s) => stableUrl(s?.url || s?.src)),
      ...(page.inlineScripts || []).map((s) => stableUrl(s?.url)),
    ]),
    ...
    // The copy recovered from a framework payload (html.js, structured.js).
    // Empty on every page without a payload, so this changes no hash for a
    // site that has none — the same argument the inline-loader scan made for
    // `scripts` — and a script-built site whose copy is now readable is
    // exactly the one that should re-analyse. Headings are deliberately NOT
    // here: they are a re-reading of text already hashed, and folding them in
    // would change every hash on the table for no new observation.
    // Present ONLY when there is some — a key that is always there would
    // change every hash on the table, which is the re-analysis the version
    // prefix exists to make deliberate.
    ...(page.renderedText ? { renderedText: String(page.renderedText).replace(/\s+/g, " ").trim() } : {}),
  };
}
