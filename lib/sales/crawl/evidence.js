...
//
// ══ Five more, for the sources a script-built site hides (version 3) ═══════
//
//   sitemap_url  — one URL the site's own sitemap lists (lib/sales/crawl/
//                  sitemap.js). normalizedValue is the PATH, rawValue the
//                  URL, sourceUrl the sitemap file it came from. Written
//                  whether or not the page was fetched: the path
//                  `/services/gutter-guards` is a statement by the business
//                  in a form no JavaScript menu can hide.
//   wp_page      — one page from a WordPress site's REST index
//                  (`/wp-json/wp/v2/pages`, structured.js). rawValue is a
//                  compact JSON of title, link and excerpt (≤ 1 KB);
//                  normalizedValue the path; sourceUrl the REST URL.
//   rendered_text — the copy recovered from a framework's render payload
//                  (`__NEXT_DATA__`, `__NUXT__`, Wix warmup, Squarespace
//                  context — structured.js). One row per page, rawValue the
//                  strings joined, normalizedValue the page URL, exactly as
//                  page_content is keyed. The payload itself is never stored.
//   heading      — one h1–h3 on a page, rawValue the text. servicesOffered.js
//                  reads the trade-shaped ones ("Gutter Guard Installation");
//                  the rest are on file for a reviewer.
//   structured_source — one row per structured file the crawl asked for
//                  (a sitemap, a REST index): normalizedValue is
//                  `<kind>:http_<status>` or `<kind>:error:<reason>`,
//                  rawValue a JSON of url, status, what was parsed. Written
//                  for FAILURES too, for the reason page_fetch is: "the
//                  sitemap 404'd" and "nobody asked" are different claims.
//                  NOT a page — lib/sales/intel/technology.js's
//                  pagesFromEvidence never sees it, so a 404 on
//                  /sitemap.xml can never read as a page that errored.
//
...
/** "3": the sitemap and the WordPress REST index are read (`sitemap_url`,
 *  `wp_page`, `structured_source`), framework payloads yield
 *  `rendered_text`, h1–h3 are kept as `heading`, and a page reached through
 *  the sitemap carries `via: "sitemap"`. A "2" row saw none of those, so a
 *  "2" crawl with no rendered_text says nothing about whether the site had
 *  a payload to read. */
export const CRAWL_DETECTOR_VERSION = "3";
...
export const MAX_ROWS_PER_CRAWL = Object.freeze({
  ...
  nav_link: 40,
  inline_token: 20,
  // Shallow paths survive the cap (sitemap.js orders them); a sitemap of
  // four hundred posts contributes its dozen pages.
  sitemap_url: 150,
  wp_page: 60,
  heading: 40,
  rendered_text: 10,
 ...
 *                lose. */
const NEVER_DEDUPED = new Set(["page_fetch", "page_content", "rendered_text", "structured_source"]);
 ...
 * crawler ranked, "sitemap" is a URL the site's own sitemap published (as
 * much the site's word as a link, and read when a script-built menu has no
 * links to rank), "probe" is a blind guess. A crawl whose only pages beyond
...
export function pageEvidence(page) {
  ...
  for (const heading of page.headings || []) {
    ...
    rows.push(row({ type: "heading", sourceUrl: url, rawValue: heading.text, normalizedValue: `h${heading.level || 0}:${String(heading.text).toLowerCase()}` }));
  }

  if (page.renderedText) {
    rows.push(
      row({
        type: "rendered_text",
        sourceUrl: url,
        rawValue: page.renderedText,
        // Keyed like page_content — by URL — so a reader joining the two
        // can find "the copy we recovered for this page" the same way.
        normalizedValue: url,
        // The strings are the page's own copy, but read out of a payload
        // rather than off the rendered document; one notch under a
        // page_content row, so a reader can tell the two apart by weight.
        confidence: 0.9,
      }),
    );
  }
...
/** Shallow sitemap paths first, like links: `/services/roofing` names a
 *  service, `/blog/2024/05/how-to` names a post. */
export function sitemapRank(row) {
  const path = String(row.normalizedValue || "");
  return path.split("/").filter(Boolean).length;
}

const RANKED = { link: linkRank, sitemap_url: sitemapRank };

/**
 * The rows for the structured sources a crawl asked for — sitemap files and
 * a WordPress REST index — which belong to the crawl rather than to a page.
 *
 * @param structured { sitemaps: [{ url, status, error, kind, urls, truncated }],
 *                     sitemapPages: [{ url, path, sourceUrl }],
 *                     wp: [{ url, status, error, pages: [...], type }] }
 */
export function structuredEvidence(structured) {
  if (!structured || typeof structured !== "object") return [];
  const rows = [];

  for (const file of structured.sitemaps || []) {
    rows.push(
      row({
        ...
        rawValue: JSON.stringify({
          ...
          sitemapKind: file.kind ?? null,
          urls: Number.isInteger(file.urls) ? file.urls : 0,
          truncated: Boolean(file.truncated),
        }),
        normalizedValue: file.error ? `sitemap:error:${file.error}` : `sitemap:http_${file.status ?? "none"}`,
      }),
    );
  }

  for (const page of structured.sitemapPages || []) {
    rows.push(row({ type: "sitemap_url", sourceUrl: page.sourceUrl || null, rawValue: page.url, normalizedValue: String(page.path || "").toLowerCase() }));
  }

  for (const source of structured.wp || []) {
    rows.push(
      row({
        type: "structured_source",
        ...
        rawValue: JSON.stringify({
          ...
          postType: source.type ?? null,
          pages: Array.isArray(source.pages) ? source.pages.length : 0,
        }),
        normalizedValue: source.error ? `wp_rest:error:${source.error}` : `wp_rest:http_${source.status ?? "none"}`,
      }),
    );
    for (const page of source.pages || []) {
      rows.push(
        row({
          type: "wp_page",
          sourceUrl: source.url,
          rawValue: JSON.stringify({ title: page.title, link: page.link, excerpt: page.excerpt || "", type: source.type || "page" }).slice(0, 1024),
          normalizedValue: String(page.path || "").toLowerCase(),
        }),
      );
    }
  }
 ...
 *
 * @param structured  what structuredEvidence() reads — the sitemap and
 *                    WordPress sources, when the crawl asked for them.
 */
export function crawlEvidence(pages = [], structured = null) {
  const rows = [
    ...(Array.isArray(pages) ? pages : [pages]).filter(Boolean).flatMap((p) => pageEvidence(p)),
