...
/**
 * The same re-hosting, from bytes already in hand — a connected mailbox's
 * message arrives whole over IMAP (lib/sales/mailbox/sync.js), so there is
 * no signed URL to download from. Same caps, same stored shape, same
 * "couldn't be stored" entry by name when Cloudinary refuses a file, so the
 * thread never shows nothing where the prospect sent something.
 *
 * @param files  [{ filename, contentType, content: Buffer|null, size }]
 * @returns { stored }  in SalesMessage.attachments' shape
 */
export async function storeAttachmentBuffers({ files, folder, uploadImpl = uploadBuffer }) {
  const stored = [];
  for (const raw of Array.isArray(files) ? files : []) {
    const filename = typeof raw?.filename === "string" && raw.filename.trim() ? raw.filename.trim() : "attachment";
    const mimeType = typeof raw?.contentType === "string" ? raw.contentType : null;
    const type = attachmentTypeFor(mimeType);
    const buffer = Buffer.isBuffer(raw?.content) ? raw.content : null;
    const declaredBytes = Number.isFinite(Number(raw?.size)) ? Number(raw.size) : buffer?.length ?? null;

    if (!buffer) {
      stored.push({ type, url: null, mimeType, filename, bytes: declaredBytes, fetchAttempts: 1, fetchError: "The mailbox delivered no bytes for this file." });
      continue;
    }
    if (buffer.length > MAX_ATTACHMENT_BYTES) {
      stored.push({ type, url: null, mimeType, filename, bytes: buffer.length, fetchAttempts: 1, fetchError: `Larger than the ${Math.round(MAX_ATTACHMENT_BYTES / 1024 / 1024)} MB cap; it is still in the mailbox.` });
      continue;
    }
    let url = null;
    let uploadError = null;
    try {
      const resourceType = resourceTypeFor(type);
      const uploaded = await uploadImpl(buffer, {
        folder,
        resourceType,
        publicId: resourceType === "raw" ? `${globalThis.crypto.randomUUID()}.${extensionFor(mimeType, filename)}` : undefined,
      });
      url = uploaded?.secure_url || null;
      if (!url) uploadError = "The file could not be stored.";
    } catch (err) {
      uploadError = `The file could not be stored (${err?.message || "unknown"}).`;
    }
    stored.push({ type, url, mimeType, filename, bytes: buffer.length, fetchAttempts: 1, ...(uploadError ? { fetchError: uploadError } : {}) });
  }
  return { stored };
}
