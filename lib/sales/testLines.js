...
// hours with my own phone number." The first build put the exemption on the
// NUMBER, not the person: a test line is a phone FieldQuo itself owns — the
// owner's mobile, a desk phone in the office — and a dial to it is exempt
// from the calling window, the 24-hour cap and the retry pool's hold FOR
// EVERY REP, because nobody's evening is being interrupted but our own. It
// is never exempt from do-not-contact or the suppression list: those are
// checked before the number is judged, in app/api/sales/calls/route.js, and
// this module is not consulted there.
//
// The per-ACCOUNT exemption was first rejected here — the gate exists to
// protect the stranger whose phone rings, and the law does not care whose
// account pressed the button — and the owner, told exactly that, reaffirmed
// it the same evening: his own rep account tests the dialler at all hours.
// So it exists too, as SalesRep.testAccount, set by a superadmin on the
// rep's card and audited, never a hard-coded email. It rides THIS module's
// path: salesCallReadiness({ testAccount }) returns the same shape with
// jurisdiction code "test", so every exclusion below applies to a test
// account's dials with no second edit, and do-not-contact still binds it.
//
...
// ── Calls that are not prospect reach at all (2026-09-21) ─────────────────
//
// SalesCallAttempt.kind: "prospect" is a dial to a business; "internal" is
// a rep ringing a colleague's browser; "off_campaign" is a number the rep
// typed with no record behind it (lib/sales/calls/supervision.js). The
// second two are real calls — recorded, logged, on the rep's history — and
// NOT reach: a floor board that counted a colleague call as a dial would
// report a rep who talked to Rachel for ten minutes as a rep who reached a
// contractor. So every count that already keeps test dials out keeps these
// out through the same door, and the 24-hour cap (store.js
// attemptsLast24h) deliberately does NOT use this: an off-campaign dial to
// a real number is a call to that number, and the law counts it.
export const PROSPECT_KIND = "prospect";
export const NON_PROSPECT_KINDS = Object.freeze(["internal", "off_campaign"]);

/** Is this row a call to a prospect? A row without the column is (it predates it). */
export function isProspectDial(row) {
  return !row || row.kind === undefined || row.kind === null || row.kind === PROSPECT_KIND;
}

/** The rows that count as reach: not a test dial, and a prospect call. */
export function onlyProspectDials(rows) {
  if (!Array.isArray(rows)) return rows;
  return rows.filter((row) => !isTestDial(row) && isProspectDial(row));
}

/** `where`, with test dials and non-prospect kinds excluded. */
export function prospectDialsOnly(where = {}) {
  const base = excludingTestDials(where);
  return { ...base, AND: [...base.AND, { kind: PROSPECT_KIND }] };
}
