 ...
 * charged, so its absence means something went wrong and no reward is due.
 *
...
export function qualifiesForRetention({
  ...
  // A condition, not the clock. Sixty days in on a one-month trial means they
  // have been charged; if no first payment was ever recorded, something is
  // wrong and nothing should pay out on it.
  if (!firstPaymentAt) {
