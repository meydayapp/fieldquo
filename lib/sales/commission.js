...
/**
 * The same three words, as catalogue keys.
 *
 * ── Why a second map and not a `labelKey` on each entry ───────────────────
 *
 * MILESTONE_LABELS is a plain string map that four callers index directly —
 * lib/sales/earnings.js, lib/sales/performance.js, the plans route and
 * commissionPlanAdmin.js — and turning its values into objects would rewrite
 * all four for no gain. The key is derivable from the milestone anyway, which
 * is the property that matters: a fourth milestone cannot be added and then
 * quietly ship English, because its key exists the moment it does.
 *
 * A rep reads these three words on /sales/pay beside every company they have
 * brought in. They were English in all nine languages.
 */
export const MILESTONE_LABEL_KEYS = Object.fromEntries(
  Object.keys(MILESTONE_LABELS).map((milestone) => [
    milestone,
    `app.salesPay.milestone.${milestone}`,
  ]),
);
 ...
 * being solo. That was not merely stricter — lib/onboarding.js's team step
 * was `seatsUsed > 1` and `complete` requires every step, so before
 * Company.worksAloneAt existed a solo company could NEVER be complete. (The
 * team step has since left onboarding altogether for the set-up steps card,
 * but the decision stands on its own.) The milestone would have paid nothing
 * on an entire class of legitimate sale.
 *
...
/**
 * Record milestone 1 for a company that can now take payments.
 *
 * ══ Why this is a function and not four lines in the webhook ═══════════════
 *
 * It was four lines in the webhook, and the webhook is not where activation
 * usually becomes true.
 *
 * `stripeChargesEnabled` is written from TWO places. `account.updated` in the
 * Stripe webhook, and GET /api/stripe/connect/status, which exists precisely
 * because that webhook so often never arrives: it only fires on connected
 * accounts when STRIPE_CONNECT_WEBHOOK_SECRET is set, an endpoint exists, and
 * that endpoint is configured for CONNECTED accounts — a plain account webhook
 * never sees account.updated for a Connect account at all. Miss any one and
 * the settings page polls the status route, Stripe says charges are enabled,
 * the column is written, and the product is correct everywhere.
 *
 * Except the ledger. The milestone lived only on the webhook path, so a
 * company whose column was written by the status route was activated by every
 * measure the product uses and had earned its rep nothing. Easy Roofers Inc.
 * read "Taking payments" and "No milestones recorded" on the same row of the
 * same screen, which is exactly the shape this codebase keeps finding: two
 * places that answer one question, and only one of them counted.
 *
 * So the rule lives here, both writers call it, and a nightly sweep calls it
 * again for anything both of them missed. earnMilestone's unique ref makes
 * that safe: one row per company per milestone, forever, whoever asks.
 *
 * Reads the company back rather than trusting a flag from the caller. The
 * caller has just written that column, and re-reading it is what stops the
 * milestone and the column from ever disagreeing.
 */
export async function recordActivation({
  ...
  if (!qualifiesForActivation(company)) return null;
  return earnMilestone({
    companyId,
