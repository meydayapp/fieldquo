...
import { buildIntroEmail, INTRO_EMAIL_LANGUAGES, isIntroEmailLanguage } from "./introEmail";
import {
  INTRO_REPEAT_DAYS,
  introLinkExpiry,
  introLinkUrl,
...
/** The hosted screenshot, per language — public/product/email/. */
export function introScreenshotUrl(origin, language) {
  const lang = isIntroEmailLanguage(language) ? language : "en";
  return `${String(origin || "").replace(/\/+$/, "")}/product/email/quote-phone.${lang}.png`;
}
  ...
  const sent = await deliverOutreach({
    ...
    build: (replyToken) =>
      buildIntroEmail({
        ...
        screenshotUrl: introScreenshotUrl(appOrigin, language),
