...
import { isQuebec } from "@/lib/sales/leadLanguage";
import { principalFor } from "@/lib/sales/intel/people";
...
const LEAD_SELECT = {
  ...
  prospect: {
    select: {
      ...
      province: true,
      doNotContactAt: true,
      // The people the intel passes found (BBB's principal, the register's
      // personnel) — the greeting's fallback when the card has no contact.
      people: { select: { name: true, givenName: true, role: true, source: true, seenAt: true } },
      // A signup-sourced row: which variant of the email this is, and the
      // resume token the abandoned variant's button carries.
      businessName: true,
...
export async function introEmailOffer({ rep, leadId = null, prospectId = null, client = db, now = new Date() }) {
  ...
  // A signup lead's own language (the form's) sits between Quebec's rule and
  // the rep's preference — lib/sales/intel/callScript.js defaultScriptLanguage.
  const signupLanguage = lead?.prospect ? signupStateOf(lead.prospect, { now })?.language || null : null;
  const defaultLanguage = defaultScriptLanguage({ prospect: { ...(lead?.prospect || prospect || lead), statedLanguage: signupLanguage }, rep: { language: repRow?.language || null } });
...
export async function sendIntroEmail({ rep, leadId = null, prospectId = null, toAddress, language, attemptId = null, origin = null, request = null, now = new Date() }) {
  ...
  // The rep's link code, name and number are not all on the gate's row; read
  // fresh here. The link code is the opaque referralToken, minted on first
  // use — never SalesRep.code, which is the rep's real name slugged and used
  // to sit in both the signup link and the demo URL of every intro email.
  const repRow = await db.salesRep.findUnique({ where: { id: rep.id }, select: { id: true, name: true, workName: true, referralToken: true, language: true } });
  const linkCode = repRow ? await ensureReferralToken(repRow) : null;
  ...
  const signupLink = signupLinkFor(appOrigin, linkCode, { linkToken: progress?.token || null });
  ...
  const signup = lead.prospect ? signupStateOf(lead.prospect, { now }) : null;
  ...
  const sent = await deliverOutreach({
    ...
    build: (replyToken) =>
      buildIntroEmail({
        language,
        // Signed with the name the prospect heard on the phone — the work
        // name, else the first name (lib/sales/repIdentity.js).
        rep: { name: repPublicName(repRow || rep), email: readiness.from, phone: repPhone },
        business: lead.businessName,
        country: lead.country || null,
        // Quebec reads French first and the English version underneath, after
        // a one-line disclosure in both languages (lib/sales/outreach/
        // introEmail.js BILINGUAL) — the owner's rule for Quebec leads.
        englishBelow: language === "fr" && isQuebec(lead.province || lead.prospect?.province),
        // The card's contact first; else the person the intel passes say to
        // ask for (BBB principal, register personnel — lib/sales/intel/
        // people.js ranks them). The owner, 2026-09-19: "will it include the
        // contact name if they have one?" — it did only when a rep had typed
        // one, and "Hello," went to owners we knew by name from BBB.
        contactName: lead.contactName || principalFor(lead.prospect?.people || [])?.name || null,
        ...
        // there.
        demoUrl: repDemoUrl(appOrigin, linkCode, { token: sealIntroLink({ introEmailId: rowId, leadId: lead.id, salesRepId: rep.id, kind: "demo", expiresAt }) }),
        ...
        replyToken,
        // ── The signup variants ─────────────────────────────────────────
        // lib/signup/salesFloor.js signupStateOf on the lead's prospect:
        // an abandoned signup gets its own form back, filled in
        // (`&resume=<token>` on the rep's link — a random token, never the
        // address); a new or stalled company gets the fifteen-minute setup
        // call instead of a signup link it has already used.
        variant: signup ? `signup_${signup.kind}` : null,
        resumeUrl: signup?.kind === "abandoned" && signup.resumeToken ? `${signupLink}&resume=${encodeURIComponent(signup.resumeToken)}` : null,
        stepLabel: signup?.stepLabel || null,
        stalledReason: signup?.kind === "stalled" ? signup.stateReason : null,
      }),
  });
