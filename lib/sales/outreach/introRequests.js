...
//   demo         stamps demoRequestedAt once and pushes the rep. There is no
//                per-rep public booking page (the public /demo is FieldQuo's
//                own, hosted by platform admins), so a demo is a request the
//                rep answers by ringing to fix a time; the page says so and
//                prints the rep's number. Nothing invents a slot.
...
/**
 * The rep's unhandled requests, for Today.
 *
 * @returns {{ callbacks: number, demos: number, items: [{ id, leadId, businessName,
 *             kind, requestedAt }] }}
 */
export async function introRequestsForRep({ salesRepId, client = db, now = new Date(), limit = 50 } = {}) {
  const rows = await client.salesIntroEmail.findMany({
    where: {
      salesRepId,
      handledAt: null,
      OR: [{ callbackRequestedAt: { not: null } }, { demoRequestedAt: { not: null } }],
    },
    orderBy: { sentAt: "desc" },
    take: 200,
    select: { id: true, leadId: true, callbackRequestedAt: true, demoRequestedAt: true, lead: { select: { businessName: true } } },
  });
  if (!rows.length) return { callbacks: 0, demos: 0, items: [] };
  ...
  const items = [];
  for (const r of rows) {
    for (const [kind, at] of [["callback", r.callbackRequestedAt], ["demo", r.demoRequestedAt]]) {
      if (!at || dialledAfter(r.leadId, at)) continue;
      items.push({ id: r.id, leadId: r.leadId, businessName: r.lead?.businessName || null, kind, requestedAt: at.toISOString() });
    }
  }
