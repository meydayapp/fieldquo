 ...
 * @param screenshotUrl  the hosted quote-on-a-phone image, absolute.
...
export function buildIntroEmail({
  ...
  const html =
    ...
    `<img src="${escapeAttr(screenshotUrl)}" width="300" alt="${escapeAttr(copy.screenshotAlt)}" ` +
    `style="display:block;width:300px;max-width:100%;height:auto;border:1px solid ${c.border};border-radius:10px;">` +
    `<p style="margin:8px 0 0 0;font-family:${FONT};font-size:13px;line-height:1.5;color:${c.inkMuted};">${escapeHtml(copy.screenshotCaption)}</p>` +
    `</td></tr>` +
  ...
  const text = [
    ...
    "",
    `${copy.screenshotCaption} ${screenshotUrl}`,
    "",
    `${copy.cta}: ${signupLink}`,
    copy.ctaNote,
    "",
