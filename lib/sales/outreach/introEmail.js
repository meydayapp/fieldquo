...
export const INTRO_COPY = Object.freeze({
  en: Object.freeze({
    ...
    intro:
      ...
      "I'm {rep}, from FieldQuo — a Canadian company that builds the back office for {trade} businesses.",
    gap:
  ...
  fr: Object.freeze({
    ...
    intro:
      ...
      "Je suis {rep}, de FieldQuo — une entreprise canadienne qui bâtit le bureau administratif des entreprises {trade}.",
    gap:
  ...
  es: Object.freeze({
    ...
    intro:
      ...
      "Soy {rep}, de FieldQuo — una empresa canadiense que construye la oficina administrativa para empresas {trade}.",
    gap:
...
export function buildIntroEmail({
  ...
  const values = { business: businessName, first, rep: repName, trade: introTradePhrase(tradeKey, lang) };
  ...
  const tradePoints = tradeSellingPoints(tradeKey, lang, { limit: TRADE_PITCH_TOP }).points;
  const tradeIntro = tradePoints.length ? fill(copy.tradePointsIntro, values) : null;
  const subject = sanitiseHeaderText(fill(copy.subject, values), 200);
  const greeting = first ? fill(copy.greetingNamed, values) : copy.greeting;
  const intro = fill(copy.intro, values);
  const callbackLabel = fill(copy.callback, values);
  ...
  const altText = sanitiseHeaderText(screenshotAlt, 200) || copy.screenshotAlt;
  const caption = isPhone ? copy.screenshotCaption : fill(copy.screenshotCaptionTakeoff, values);
  ...
  const html =
    ...
    p(escapeHtml(intro)) +
    p(escapeHtml(copy.gap)) +
    // The trade's three, numbered, between the gap and the eight — so the
    // roofer reads the satellite measure before the list every trade gets.
    (tradeIntro
      ...
      : "") +
    p(`<strong>${escapeHtml(copy.pointsIntro)}</strong>`, "margin-bottom:8px;") +
    `<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 10px 0;">${pointsHtml}</table>` +
    // The AI, in one paragraph after the eight — the owner's addition, and
    // kept to a paragraph so the grid stays the eight points it is.
    p(escapeHtml(copy.ai), "margin-bottom:6px;") +
    `</td></tr>` +
    // The screenshot: a real quote as the homeowner sees it, at phone width.
    ...
    `</td></tr>` +
    // The primary button and the sentence under it.
    `<tr><td align="center" style="padding:22px 22px 6px 22px;">` +
    button({ href: signupLink, label: copy.cta, bg: c.primary.bg, fg: c.primary.fg }) +
    `<p style="margin:10px 0 0 0;font-family:${FONT};font-size:13px;line-height:1.5;color:${c.inkMuted};">${escapeHtml(copy.ctaNote)}</p>` +
    `</td></tr>` +
    // The two secondary buttons, side by side where there is room and
    // wrapped where there is not — inline-block tables in a centred cell.
    ...
    button({ href: demoUrl, label: copy.demo, bg: c.secondary.bg, fg: c.secondary.fg, border: c.border, bold: false, inline: true }) +
    `</td></tr>` +
    // Sign-off.
    `<tr><td style="padding:18px 22px 24px 22px;">` +
    p(escapeHtml(copy.signoff), "margin-bottom:4px;") +
    p(
      `<strong>${escapeHtml(repName)}</strong><br>FieldQuo` +
        (repPhone ? `<br>${escapeHtml(copy.phoneLabel)}: ${escapeHtml(repPhone)}` : "") +
        `<br><a href="mailto:${escapeAttr(repEmail)}" style="color:${c.accentText};">${escapeHtml(repEmail)}</a>`,
      "margin-bottom:0;",
    ) +
    `</td></tr>` +
    // The footer: why, where we are, how to stop.
  ...
  const text = [
    ...
    copy.gap,
    "",
    // Numbered, not dashed: the check counts the eight "- " bullets below
    // and these three are a different list.
    ...(tradeIntro ? [tradeIntro, ...tradePoints.map((pt, i) => `${i + 1}. ${pt.headline} — ${pt.oneLiner}`), ""] : []),
    copy.pointsIntro,
    ...copy.points.map((pt) => `- ${pt.title} — ${pt.clause}`),
    "",
    copy.ai,
    "",
    `${caption} ${screenshotUrl}`,
    "",
    `${copy.cta}: ${signupLink}`,
    copy.ctaNote,
    "",
    ...
    repName,
    "FieldQuo",
    ...(repPhone ? [`${copy.phoneLabel}: ${repPhone}`] : []),
    repEmail,
    "",
    "—",
    copy.footerWhy,
