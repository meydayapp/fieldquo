...
import { heartbeat } from "./calls/store";
 ...
 */
// Exported since 2026-09-21 for exactly one other caller, app/api/sales/
// presence — the shell's keepalive, a POST this gate would refuse as a
// write. It stamps the same one column, through the same fenced function.
export async function stampLastSeen(salesRepId, now = new Date()) {
  try {
...
function shouldBeat(repId) {
  const t = lastBeat.get(repId) || 0;
  if (Date.now() - t < BEAT_EVERY_MS) return false;
  lastBeat.set(repId, Date.now());
  return true;
}
...
export async function requireSalesRep(request) {
  ...
  const row = await db.salesRep.findUnique({
    ...
    select: {
      ...
      name: true,
      code: true,
      // The name outsiders see (lib/sales/repIdentity.js repPublicName) and
      // the opaque half of their link (lib/sales/repLink.js). Selected on the
      // gate because nearly every route that composes something a prospect
      // reads starts from this row, and a route that forgot to select the
      // work name would silently fall back to the rep's real first name.
      workName: true,
      referralToken: true,
      ...
      sessionEndedBy: true,
      // The agency tier (lib/sales/agency.js): an employee's engagement and
      // manager decide whether they may see pay; an agency's kind decides
      // whether it sees its team. Read here, once, fresh per request.
      engagement: true,
      managerId: true,
  ...
  // Any authenticated request is proof the rep is there. The portal shell
  // beats once a minute, but a background tab's timers are throttled and the
  // board then said "their browser has not said anything since 12:48" about
  // a rep who was dialling (owner, 2026-09-16). Stamping the open presence
  // row here, at most every thirty seconds per instance, keeps the board
  // honest about a rep who is working without adding a write to every call.
  if (shouldBeat(rep.id)) heartbeat(rep.id).catch(() => {});
