 ...
 */
export function leadContactability(lead = {}, { optedOut = null, ourNumbers = [] } = {}) {
  ...
  // ── One of ours ────────────────────────────────────────────────────────
  //
  // callPlan() refuses these at dial time and this screen did not know, so a
  // rep pressed Call, read "that is one of our own numbers", and was left with
  // a live button beside the reason it could not work. Checked here so the
  // control is never drawn — the same answer, before the press instead of
  // after it.
  const phone = leadPhoneE164(lead);
  if (phone && (Array.isArray(ourNumbers) ? ourNumbers : []).includes(phone)) {
    return {
      callable: false,
      code: "our_own_number",
      title: "That is one of FieldQuo's own numbers.",
      text:
        "Ringing it would bridge a loop and bill both legs, so there is nothing to dial. If it is " +
        "really this business's number, it is on one of FieldQuo's number lists by mistake — a " +
        "released number should not be on any of them.",
    };
  }
 ...
 */
export function leadDialView(lead = {}, { optedOut = null, ourNumbers = [], windowPolicy = null, testLine = false, testAccount = false } = {}) {
  return {
    phoneE164: leadPhoneE164(lead),
    contact: leadContactability(lead, { optedOut, ourNumbers }),
    callingContext: {
      ...leadCallingContext(lead),
      // The lead's number is one of FieldQuo's own test lines
      // (lib/sales/testLines.js) — looked up by the route against the
      // setting, never decided here. Only the literal true travels.
      testLine: testLine === true,
      // The rep reading this lead is a test account (SalesRep.testAccount),
      // off the rep row the gate read in this request. Re-passed by the
      // screen exactly like testLine; only the literal true travels.
      testAccount: testAccount === true,
      // The platform console's calling-window override for this lead's
      // state, already resolved by lib/sales/windowPolicy.js on the server
      // and passed in — this file decides nothing about hours (see the
      // header), and it does not decide this either. Null is enforce.
