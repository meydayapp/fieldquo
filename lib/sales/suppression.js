...
/**
 * Is ANY of these numbers suppressed?
 *
 * ══ Why the question is asked about the business, not the number ══════════
 *
 * A STOP is recorded against the number it arrived from. That was sufficient
 * while a business had exactly one number on file. Free dial ended that: a
 * contractor texts STOP from the shop line, a rep is given the owner's mobile
 * and rings THAT, and a per-number check finds nothing and lets the call
 * through. The person who asked us to stop gets rung anyway, from a system
 * that recorded their request correctly.
 *
 * So every number on the record is asked about, and one hit anywhere refuses
 * the contact. Over-refusing costs FieldQuo a prospect it was told to drop;
 * under-refusing is the violation — the same asymmetry ALL_CHANNELS is
 * documented with in lib/sales/salesSms.js.
 *
 * ══ Fails CLOSED, per number ══════════════════════════════════════════════
 *
 * A lookup that throws returns a suppressed verdict rather than being skipped.
 * "We could not check" is not permission to ring, and a loop that swallowed
 * one failure and carried on would turn a database blip into exactly the call
 * this function exists to stop.
 *
 * Shared rather than written out at each call site: it is the same eight lines
 * in the dial route and the texting route, and AGENTS.md failure class #4 says
 * which of two copies rots — the one nobody looks at.
 *
 * @returns { suppressed, reason, phone } — `phone` names WHICH number carried
 *          the refusal, because "one of their numbers is on the list" is not
 *          an answer a rep can do anything with.
 */
export async function firstSuppression(db, { phones = [], channel = "phone" } = {}) {
  for (const phone of Array.isArray(phones) ? phones : []) {
    if (!phone) continue;
    const verdict = await checkSuppression(db, { channel, phone }).catch((err) => ({
      suppressed: true,
      reason:
        `The do-not-contact list could not be read (${err?.message}), so this is refused ` +
        "rather than risked.",
    }));
    if (verdict?.suppressed) return { ...verdict, phone };
  }
  return { suppressed: false, reason: null, phone: null };
}

/**
 * Which of these phones and domains carry a LIVE suppression, on any channel?
 *
 * The batch form, for a sweep that decides whether a scraped listing may even
 * become a prospect (lib/sales/intel/promoteListings.js). It reads one page of
 * listings at a time, so asking checkSuppression() per row would be a query
 * per listing; this is one query per page.
 *
 * Normalised here, like every other reader in this file. The sweep used to
 * query the table with the listing's own strings, which only matched when a
 * scraper happened to spell a domain the way the list stores it — a
 * "www.acme.com" listing slipped past an "acme.com" do-not-contact row.
 *
 * Any live row blocks, whatever its channels: a business that asked us to stop
 * on one channel is not one to promote into a calling queue. Over-refusing is
 * the failure this list is allowed to have (see firstSuppression above).
 *
 * The answer is keyed by the values AS PASSED, so the caller can compare its
 * own listing fields without knowing how the list normalises them. A read that
 * throws is left to throw: a sweep that cannot see the list must stop, not
 * promote as though it were empty.
 *
 * @returns {Promise<{ phones: Set<string>, domains: Set<string> }>}
 */
export async function liveSuppressedContacts(db, { phones = [], domains = [] } = {}) {
  const byKey = new Map(); // "kind:normalised" -> the caller's own spellings
  const add = (kind, raw) => {
    const value = normaliseSuppressionValue(kind, raw);
    if (!value) return;
    const id = `${kind}:${value}`;
    if (!byKey.has(id)) byKey.set(id, { kind, value, raws: new Set() });
    byKey.get(id).raws.add(raw);
  };
  for (const p of Array.isArray(phones) ? phones : []) add("phone", p);
  for (const d of Array.isArray(domains) ? domains : []) add("domain", d);

  const out = { phones: new Set(), domains: new Set() };
  if (!byKey.size) return out;

  const rows = await db.salesSuppression.findMany({
    where: {
      removedAt: null,
      OR: [...byKey.values()].map(({ kind, value }) => ({ kind, value })),
    },
    select: { kind: true, value: true },
  });
  for (const row of rows) {
    const hit = byKey.get(`${row.kind}:${row.value}`);
    if (!hit) continue;
    const target = row.kind === "phone" ? out.phones : out.domains;
    for (const raw of hit.raws) target.add(raw);
  }
  return out;
}
