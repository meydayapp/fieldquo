...
//
// ══ The sender fallback, and exactly how far it reaches ═══════════════════
//
// Since 2026-09-18 there is ONE exception, and it is narrower than it looks.
// A reply that carries no token at all — a prospect who typed the rep's
// reply address by hand, a mail client that dropped the +tag, a Namecheap
// forward of a message the prospect sent straight to <rep>@fieldquo.com —
// is filed when, and only when, BOTH halves hold on rows read fresh:
//
//   1. the message was addressed to a mailbox of OURS that names a rep —
//      the local part at the reply domain, or the rep's work mailbox
//      itself — so the rep is chosen by an address we own, never by the
//      sender;
//   2. the From matches exactly ONE of that rep's leads by address. Two
//      leads with the same address (`ambiguous_sender`) file nowhere, and
//      the copy is still forwarded so a human decides.
//
// What a forged From can do under that rule: put a message into the one
// thread between that rep and that lead — which is what the lead could do
// anyway — and, if it says "unsubscribe", suppress that lead's address. It
// cannot reach another rep, another lead, or an address the rule did not
// name. The row records `filedBy: "sender"` so a reader can tell the two
// apart, and scripts/check-sales-email.mjs runs the rule against a stranger,
// a duplicate lead and a rep with no mailbox.

import { bareAddress, detectOptOut, leadOptedOut, newReplyToken } from "./outreach";
import { replyDomainLocalPart } from "./resendInbound";
...
export async function contactOptedOut(db, { leadId, email, phone, channel = "email" } = {}) {
  ...
  if (suppression.suppressed) {
    // `reasonKey`/`reasonParams` ride alongside the English `reason` rather
    // than replacing it. The sentence is still composed here, where the seven
    // sources are known; what changed is that the KNOWLEDGE travels as well as
    // the words, so /sales/leads/[id] and /sales/threads/[id] can say it in
    // the rep's own language instead of rendering the English. Callers with no
    // translator — the API `error` bodies, the check scripts — read `reason`
    // and are untouched.
    return {
      optedOut: true,
  ...
  if (leadOptedOut(inbound)) {
    return {
      ...
      // The EIGHTH source, and the one that is not on the suppression list at
      // all: detectOptOut read an unsubscribe out of a reply nobody has filed
      // yet. It gets its own key rather than borrowing the list's `reply` one,
      // because the two sentences make different claims — that one says a row
      // exists, this one says a message does.
      reasonKey: "app.salesSuppression.reason.detectedReply",
      reasonParams: {},
      via: "reply",
    };
  }

  return { optedOut: false, reason: null, reasonKey: null, reasonParams: null, via: null };
}
 ...
 */
export async function fileInboundMessage(db, parsed, { replyDomain } = {}) {
  if (!parsed?.token) {
    const fallback = await threadBySender(db, parsed, { replyDomain });
    if (!fallback.thread) return { filed: false, reason: fallback.reason };
    return writeInbound(db, parsed, fallback.thread, { filedBy: "sender" });
  }
  ...
  const thread = await db.salesThread.findUnique({
    ...
    select: {
      ...
      salesRepId: true,
      subject: true,
      // BOTH addresses. Outbound now goes out from workEmail (see
      // lib/sales/outreachSender.js's repSendingAddress), so an echo of our own
      // sent item carries that in its From — matching only the login address
      // would have quietly reopened the double-filing bug the echo check below
      // exists to close. The login address stays in the comparison because
      // messages sent before that change carry it.
      salesRep: { select: { email: true, workEmail: true } },
  ...
  return writeInbound(db, parsed, thread, { filedBy: "token" });
}

/**
 * The sender fallback described in the header. Returns the thread to file
 * into — creating one when the lead has none yet, since a first email FROM
 * a prospect is a conversation too — or the reason there is none.
 */
async function threadBySender(db, parsed, { replyDomain } = {}) {
  const sender = bareAddress(parsed?.fromAddress);
  if (!sender) return { thread: null, reason: "no_token" };

  // Half one: which of OUR mailboxes was written to.
  const local = replyDomainLocalPart(parsed?.toAddress, replyDomain);
  const bareTo = bareAddress(parsed?.toAddress);
  const rep = local
    ? await db.salesRep.findFirst({
        where: { workEmail: { startsWith: `${local}@`, mode: "insensitive" }, active: true },
        select: { id: true, email: true, workEmail: true },
      })
    : bareTo
      ? await db.salesRep.findFirst({
          where: { workEmail: { equals: bareTo, mode: "insensitive" }, active: true },
          select: { id: true, email: true, workEmail: true },
        })
      : null;
  if (!rep) return { thread: null, reason: "no_token" };

  // Our own message coming back carries no token only when a mailbox rule
  // stripped it; it is still ours.
  if ([rep.workEmail, rep.email].map((a) => String(a || "").toLowerCase()).includes(sender)) {
    return { thread: null, reason: "own_outbound" };
  }

  // Half two: exactly one of that rep's leads at that address.
  const leads = await db.salesLead.findMany({
    where: { salesRepId: rep.id, email: { equals: sender, mode: "insensitive" } },
    select: { id: true, email: true, phone: true },
    take: 2,
  });
  if (leads.length !== 1) {
    return { thread: null, reason: leads.length ? "ambiguous_sender" : "no_token" };
  }
  const lead = leads[0];

  const existing = await db.salesThread.findFirst({
    where: { salesRepId: rep.id, leadId: lead.id },
    orderBy: { lastMessageAt: "desc" },
    select: { id: true, leadId: true, salesRepId: true, subject: true },
  });
  if (existing) {
    return { thread: { ...existing, salesRep: rep, lead }, reason: null };
  }
  // No thread yet: one is created INSIDE writeInbound's transaction, with
  // the message, so a write that fails leaves no empty conversation behind
  // — the same rule lib/sales/outreachSender.js applies to a failed send.
  return {
    thread: {
      id: null,
      leadId: lead.id,
...
/** Everything after a thread has been chosen — by token or by sender. */
async function writeInbound(db, parsed, thread, { filedBy }) {
  // ── The echo check ───────────────────────────────────────────────────────
  //
  ...
  const sender = bareAddress(parsed.fromAddress);
  const ourOwn = [thread.salesRep?.workEmail, thread.salesRep?.email]
    .map((a) => String(a || "").toLowerCase())
    .filter(Boolean);
  if (sender && ourOwn.includes(sender)) {
  ...
  if (parsed.providerId && thread.id) {
  ...
  const { message, suppressed, threadId } = await db.$transaction(async (tx) => {
    const target =
      thread.id ||
      (
        await tx.salesThread.create({
          data: {
            salesRepId: thread.salesRepId,
            leadId: thread.leadId,
            counterpart: sender || null,
    ...
    const created = await tx.salesMessage.create({
      data: {
        ...
        body: parsed.body || "",
        providerId: parsed.providerId,
        ccAddresses: joinAddresses(parsed.cc),
        filedBy,
        sentAt,
      },
    });
    ...
    await tx.salesThread.updateMany({
      where: { id: target, lastMessageAt: { lt: sentAt } },
      data: { lastMessageAt: sentAt },
    });
    // lastInboundAt likewise — and a prospect who writes again UN-archives
    // the conversation, since an answer to an archived thread is still an
    // answer. Only when this message is the newest inbound one: a replayed
    // week-old message must not pull a thread the rep filed away back out.
    await tx.salesThread.updateMany({
      where: { id: target, OR: [{ lastInboundAt: null }, { lastInboundAt: { lt: sentAt } }] },
    ...
    return { message: created, suppressed: wrote, threadId: target };
  });
