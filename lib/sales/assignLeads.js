 ...
 * "available" here either. The check holds both sites to the same
 * expression.
 */
 ...
 *
 * Caps: `count` at QUEUE_BATCH_MAX and at what is left of the rep's daily
 * claim cap, counted from the same claim log the rep's press counts from —
 * a console assignment is a claim on the rep's day, and a rep at 250 is at
 * 250 whoever pressed. The shift bound is the rep's real one
 ...
 *            skippedForWindow, skippedForLanguage, remainingToday, reason,
...
export async function assignBatchToRep({
  ...
  const takenToday = await db.salesQueueClaim.count({ where: { salesRepId: rep.id, localDate } });
  ...
  const want = Math.min(request.count, QUEUE_BATCH_MAX, remainingToday);
 ...
 *            remainingToday, batchId }` or `{ error }`.
 */
export async function assignProspectsToRep({ db, rep, admin, ids = [], now = new Date(), notify = null } = {}) {
  ...
  const zone = await lastKnownZoneFor({ db, salesRepId: rep.id, now });
  const localDate = localDateIn(zone, now) || now.toISOString().slice(0, 10);
  const [rows, takenToday, last] = await Promise.all([
    db.prospect.findMany({ where: { id: { in: wanted } }, select: pickedSelect() }),
  ...
  // The daily cap, in order: the rows past it are refused with the reason
  // rather than silently dropped.
  const room = accepted.slice(0, remainingToday);
  for (const row of accepted.slice(remainingToday)) {
    refused.push({
      id: row.id,
      businessName: row.businessName,
      reason: `${rep.name || "This rep"} is at the daily ceiling of ${QUEUE_DAILY_CLAIM_CAP} claims.`,
    });
  }
  if (room.length === 0) {
    return { assigned: 0, assignedIds: [], refused, remainingToday, batchId: null };
  }
 ...
 * does, likewise.
 */
export async function unassignFromRep({ db, rep, admin, ids = [], now = new Date() } = {}) {
  ...
  const rows = await db.prospect.findMany({
    where: { id: { in: wanted } },
    select: { id: true, businessName: true, assignedRepId: true, claimExpiresAt: true, assignedRep: { select: { name: true } } },
  });
  const byId = new Map(rows.map((r) => [r.id, r]));
  const refused = [];
  const candidates = [];
