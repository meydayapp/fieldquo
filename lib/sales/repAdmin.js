...
// signup link from that one number.
//
// ── The reason this comment used to give is NOT TRUE, and is corrected here
//
// It said the sharing was a compliance posture, because "a STOP arriving there
// is honoured across every rep at once" and per-rep numbers "would fragment
// that opt-out". That is not how the opt-out is stored. handleSalesInboundSms
// suppresses `value: from` — the CONTRACTOR's number — across ALL_CHANNELS,
// and the send path checks the LEAD's phone. The record is against them, not
// against whichever of our numbers they happened to reply to, so a STOP sent
// to any FieldQuo number already blocks that contractor for every rep and
// every channel. Per-rep sending numbers would not fragment anything.
//
// Left shared anyway, and the honest reasons are the operational ones: every
// sending number must be registered for A2P/10DLC before it may text a US
// contractor at all, so numbers are not free to add, and one number is one
// registration and one reputation to keep clean. That is a real argument. The
// compliance one was not, and a decision resting on a false premise is one
// nobody can revisit sensibly — which is the point of correcting it rather
// than deleting it.
//
// CALLS are the opposite and deliberately so: lib/sales/numbers.js's
// callerIdForRep() gives a rep their ASSIGNED number and shares only the
// unassigned ones, because a callback has to reach one specific person.
//
...
export const NUMBER_CAPABILITIES = [
  ...
  },
  {
    ...
    available: false,
    detail:
      "Not built, and no longer the same gap it was. FIELDQUO_SALES_NUMBER is " +
      "one environment variable naming FieldQuo's own line — the one the AI " +
      "receptionist answers — not a pool anything can allocate from, and no " +
      "model links a phone number to a rep. Assigning one would need a schema " +
      "change, so there is no control here rather than one that would appear " +
      "to work. What DOES happen now: a contractor ringing back one of the " +
      "shared sales_voice numbers is answered, and the call is filed against " +
      "the rep who last called them from that number — derived from the call " +
      "log rather than from an assignment, so it needs no column and cannot go " +
      "stale. The sales floor board says which state that is in.",
    // Still null, and still for the reason it always was: this field names the
    // screen where you would DO the thing, and there is no screen that assigns
    // a number to a rep because there is no such assignment. The floor board
    // is named in the sentence above rather than here — it is where you READ
    // what happens, which is a different question.
    where: null,
  },
];
...
export function salesNumberState({ e164 = null, lookupFailed = false } = {}) {
  ...
  return {
    ...
    detail:
      ...
      "link. Buy one under Twilio numbers with the purpose set to Sales — it must " +
