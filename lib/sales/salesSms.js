 ...
 */
export async function salesSmsStatus({ rep, lead, origin, now = new Date(), linkToken = null }) {
...
export async function deliverSignupLinkSms({ rep, lead, origin, now = new Date() }) {
  ...
  const status = await salesSmsStatus({ rep, lead, origin, now, linkToken: progress?.token || null });
