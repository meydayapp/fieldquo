...
import { BUSINESS_LEAD_SELECT, businessKeyOf, leadsOnNumber, mergeThreadsByBusiness } from "./messages/business";
...
export async function salesSmsStatus({ rep, lead, origin, now = new Date(), linkToken = null, purpose = "reply" }) {
  ...
  return salesSmsReadiness({
    ...
    signupLink: signupLinkFor(origin, rep?.code, { linkToken }),
    fromNumber,
...
export async function handleSalesInboundSms({
  ...
  //
  // ══ Three lookups, in the order of how sure each one is ═══════════════
  //
  //   1. The last text WE sent to this number — the rep who asked the
  //      question owns the answer, and the lead that text was about is the
  //      lead this reply is about. Not the lead's own salesRepId alone: two
  //      reps can hold leads for one business.
  //   2. A lead whose phone IS this number, compared normalised. This used
  //      to be "the most recently updated lead with any phone at all",
  //      discarded when it did not match — the same bug leadForThread()'s
  //      header describes on the thread route, which meant a contractor
  //      texting first from the number on their lead was filed to nobody.
  //   3. A stored contact number (SalesContactNumber) — the owner's cell a
  //      rep was given on a call — through its lead, or through the leads
  //      on its prospect. A reply from that cell belongs in the same
  //      conversation as the shop line the link went to.
  //
  // When none of the three names a rep the row is stored with NO rep: it
  // reaches the floor — salesConversations() lists an unowned inbound
  // thread to every rep who can text, and the first reply claims it
  // (deliverReplySms). Never dropped: a text that reached us is on record
  // whether or not anybody can yet say whose it is.
  ...
  const lastOut = fromE164
    ...
    : null;
  let lead = null;
  if (fromE164 && !lastOut?.leadId) {
    const onNumber = await leadsOnNumber(db, fromE164);
    lead = onNumber.find((l) => l.convertedCompanyId) || onNumber[0] || null;
    if (!lead && db.salesContactNumber) {
      const contact = await db.salesContactNumber
        .findFirst({ where: { e164: fromE164 }, select: { salesLeadId: true, prospectId: true, addedBySalesRepId: true } })
        .catch(() => null);
      if (contact?.salesLeadId || contact?.prospectId) {
        lead = await db.salesLead
          .findFirst({
            where: contact.salesLeadId ? { id: contact.salesLeadId } : { prospectId: contact.prospectId },
            orderBy: { updatedAt: "desc" },
            select: { id: true, salesRepId: true, businessName: true, contactName: true, convertedCompanyId: true },
          })
          .catch(() => null);
      }
    }
  ...
  const matchedLead = lastOut?.lead || lead || null;
  let storedId = null;
  await db.salesSmsMessage
    .create({
      data: {
...
export async function salesConversations({ salesRepId, client = db, limit = 50, readStates = null } = {}) {
  const rows = await client.salesSmsMessage.findMany({
    // The rep's own rows, and the FLOOR: an inbound text nobody owns yet —
    // handleSalesInboundSms found no rep who had texted the number and no
    // lead carrying it — is listed to every rep who can text, as a
    // stranger's number, until one of them answers and the reply claims
    // it. The alternative was the row existing and no screen showing it,
    // which is the "dropped on the floor" this file's inbound header
    // describes, one table later.
    where: { OR: [{ salesRepId }, { salesRepId: null, direction: "in" }] },
    orderBy: { sentAt: "desc" },
    take: 500,
    select: {
  ...
  for (const m of rows) {
    ...
    if (!threads.has(other)) {
      threads.set(other, {
        ...
        businessKey: businessKeyOf(m.lead),
        count: 0,
        unanswered: false,
    ...
    t.count += 1;
    if (m.salesRepId === salesRepId) t.unowned = false;
    if (m.direction === "in") {
...
export async function salesThread({ salesRepId, withE164, numbers = null, client = db } = {}) {
  ...
  const rows = await client.salesSmsMessage.findMany({
    where: {
      // The rep's rows on these numbers, and the floor's — unowned inbound
      // rows on the same numbers, which a reply from here will claim.
      OR: [
        { salesRepId, OR: [{ toE164: { in: set } }, { fromE164: { in: set } }] },
        { salesRepId: null, direction: "in", fromE164: { in: set } },
      ],
    },
    orderBy: { sentAt: "asc" },
    select: {
  ...
  if (typeof client.salesSmsMessage.updateMany === "function") {
    await client.salesSmsMessage
      .updateMany({
        ...
        data: { salesRepId: rep.id, ...(lead?.id ? { leadId: lead.id } : {}) },
      })
