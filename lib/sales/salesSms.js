...
export async function handleSalesInboundSms({ to, from, body }) {
  ...
  const lastOut = fromE164
    ? await db.salesSmsMessage
        .findFirst({
          ...
          orderBy: { sentAt: "desc" },
          select: { salesRepId: true, leadId: true },
        })
        .catch(() => null)
    : null;
  ...
  await db.salesSmsMessage
    .create({
      data: {
        direction: "in",
        salesRepId: lastOut?.salesRepId || lead?.salesRepId || null,
 ...
 */
export async function salesConversations({ salesRepId, client = db, limit = 50 } = {}) {
  ...
  for (const m of rows) {
    ...
    if (!threads.has(other)) {
      threads.set(other, { e164: other, lastAt: m.sentAt, lastBody: m.body, lastDirection: m.direction, leadId: m.leadId || null, count: 0, unanswered: false });
    }
 ...
 */
export async function deliverReplySms({ rep, lead, text, origin, now = new Date(), client = db } = {}) {
  ...
  const result = await sendSms({ to: status.to, from: status.from, body });
  if (!result?.ok) {
    await recordError({
      area: "sales_sms",
      code: "reply_not_sent",
