...
import { findDomainByName, listDomains } from "@/lib/email/resendDomains";
...
/**
 * The domain Resend receives replies at, or undefined for "the rep's own
 * mailbox, as before". docs/SALES-OUTREACH.md §5b. Trimmed and lower-cased
 * here so every reader sees one spelling; NOT validated here — readiness does
 * that, and names the blocker.
 */
...
let receivingCache = { domain: null, value: null, at: 0 };

export function invalidateSalesSenderCache() {
  domainCache = { value: null, at: 0 };
  receivingCache = { domain: null, value: null, at: 0 };
}

/**
 * Is SALES_REPLY_DOMAIN verified on the Resend account AND receiving-enabled?
 *
 * Resend's domain record answers both halves: `status === "verified"` and
 * `capabilities.receiving === "enabled"` (lib/email/resendDomains.js's
 * getDomain passes the second through). Receiving is a separate switch from
 * sending on the same domain page, with its own MX record, and a domain that
 * is verified for sending but not receiving bounces every reply — so both
 * halves are required, and the blocker text in outreachReadiness names the
 * switch.
 *
 * @returns true / false / null — null when Resend could not be asked, OR when
 *          its response carried no `capabilities` object at all (an older API
 *          shape), because "we could not tell" must not read as "no". The
 *          readiness message for null says to confirm it in the dashboard.
 */
export async function replyDomainReceiving(domain = replyDomain()) {
  if (!domain) return null;
  if (receivingCache.domain === domain && Date.now() - receivingCache.at < DOMAIN_TTL_MS) {
    return receivingCache.value;
  }
  if (!process.env.RESEND_API_KEY) return false;

  try {
    const record = await findDomainByName(domain);
    let value;
    if (!record) value = false;
    else if (record.status !== "verified") value = false;
    else if (!record.capabilities) value = null;
    else value = record.capabilities.receiving === "enabled";
    receivingCache = { domain, value, at: Date.now() };
    return value;
  } catch (err) {
    console.error("[sales] couldn't ask Resend whether the reply domain receives:", err?.message);
    return null;
  }
}
...
export async function outreachStatus(rep) {
  ...
  return outreachReadiness({
    ...
    // The Resend Receiving door (docs/SALES-OUTREACH.md §5b). The receiving
    // lookup is cached for ten minutes like the sending one, and skipped
    // entirely when no reply domain is configured.
    replyDomain: replyDomain(),
    replyDomainReceiving: await replyDomainReceiving(),
    resendWebhookSecretSet: Boolean(process.env.RESEND_INBOUND_WEBHOOK_SECRET),
  });
}
 ...
 * @param attachments  Resend attachments [{ filename, content (base64) }],
 *                     for the one case that has one: a calendar invite for a
 *                     demo the lead agreed to on the phone
 *                     (app/api/sales/events/[id]/invite). The stored
 *                     SalesMessage keeps the text; the .ics is derivable
 *                     from the event row and is not copied into the thread.
 * @returns { ok: true, threadId, messageId } | { ok: false, status, error }
 */
export async function deliverOutreach({ rep, lead, thread, subject, body, attachments = [] }) {
  if (!lead?.email) {
  ...
  // remembering to ask.
  // ── Where this address came from, asked in the same breath ──────────────
  //
  // Not every address FieldQuo holds may lawfully be emailed, and the reason
  // has nothing to do with anyone opting out. An address ingested from a
  // government licence register carries no CASL implied consent: the regulator
  // published it, the business did not, and suppressionRules.js's CONSENT
  // section quotes the CRTC on both halves of that. So the do-not-contact list
  // is silent about these people — there was never anything to opt out of —
  // and a list-only check would answer "not suppressed" to a message the law
  // does not permit at all.
  //
  // Resolved here rather than passed in: this is the last statement before
  // mail leaves the building, and a provenance the calling route remembered to
  // look up is a provenance the next route will forget.
  ...
  // readiness.replyDomain is null unless SALES_REPLY_DOMAIN passed every check
  // above — set, a real domain, and not known to be non-receiving — so the
  // Reply-To is only ever moved to a domain that can take the reply.
  const replyTo = replyToAddress(
    sendingAddress,
    replyToken,
    readiness.replyAddressing,
    readiness.replyDomain,
  );
  if (!replyTo) {
    return {
      ok: false,
