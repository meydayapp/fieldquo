...
export async function outreachStatus(rep) {
  return outreachReadiness({
    repEmail: rep?.email,
    senderDomainVerified: await senderDomainVerified(rep?.email),
...
export async function deliverOutreach({ rep, lead, thread, subject, body }) {
  ...
  const replyTo = replyToAddress(rep.email, replyToken, readiness.replyAddressing);
  if (!replyTo) {
    return {
      ok: false,
      status: 409,
      error:
        `A reply address couldn't be built from ${rep.email}. A mailbox whose ` +
  ...
  const result = await sendEmail({
    ...
    html: email.html,
    text: email.text,
    from: `${sanitiseHeaderText(rep.name, 120)} <${rep.email}>`,
    replyTo,
  });
  ...
  if (result?.error || !result?.id) {
    ...
    await recordError({
      ...
      detail: { leadId: lead.id, salesRepId: rep.id, from: rep.email },
    }).catch(() => {});
    return {
      ok: false,
      status: 502,
