...
import { STATE_AFTER_CALL, STATE_AVAILABLE } from "./calls/agentState";
...
export const AUTODIAL_REASONS = Object.freeze({
  ...
  /** The write-up window after the last call has not run out (agentState.js). */
  write_up_window: "write_up_window",
});
 ...
 * @param state          the rep's presence state — DERIVED (agentState.js
 *                       livePresence), so `after_call` here is the write-up
 *                       window the platform set, not a button.
 * @param writeUpEndsAt  when that window ends (ms, ISO or Date), from the
 *                       same presence; null when the state is not
 *                       after_call or the window is held for a missing
 *                       outcome.
...
export function nextDial({
  ...
  // The write-up window (the owner's sixty seconds after every call, a
  // platform setting): the dialler waits for it, named as such, and the
  // screen arms again when the presence flips to available at its end —
  // the one automatic resumption, because the window ending is the
  // platform's clock and not a page load. A window held for a missing
  // outcome (`writeUpEndsAt` null) is the same stop with no time on it.
  if (state === STATE_AFTER_CALL) {
    const ends = writeUpEndsAt == null ? null : opensAtMs({ opensAt: writeUpEndsAt });
    return stop(AUTODIAL_REASONS.write_up_window, { state, endsAt: ends });
  }
  // Break, Dinner, Meeting, Training, Busy, Off, on a call: none of them is
  // available, and the dialler does not argue with a status. Coming back is
  // the rep pressing Available, never the countdown starting itself.
