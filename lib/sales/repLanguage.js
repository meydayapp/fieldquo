...
//
// That permanent key no longer exists (2026-09-25, lib/i18n/statedLanguage.js):
// the header's switcher now lasts for one tab's session, and the old value is
// removed from every browser on load. So the incident above cannot recur even
// for a rep who never chose — and the column below is still what lasts.
//
 ...
 *     A decision. The provider skips every browser signal — this tab's
 *     switch, the device — so a marketing-site click cannot overrule it on a
 *     shared browser.
 *
 *   · Anything else → `{ language: null, fromAccount: false }`.
 *     No statement, so the provider falls back the way it was built to: this
 *     tab's explicit switch, then the device's own language
 *     (lib/i18n/statedLanguage.js). Null rather than "en"
