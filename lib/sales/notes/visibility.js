 ...
 * open. Since 2026-09-16 the column HAS a writer — a call-centre agency sets
 * itself as its reps' manager (lib/sales/agency.js) — and that line scopes
 * the agency's PAY and its FLOOR, not their notes: no notes route reads
 * `managerId` to build a viewer, and lib/sales/team.js's
 * TEAM_LEAD_NOTE_VISIBILITY_FROM is still null — the date from which a team
 * lead may read a report's notes, which is a promise to every rep who has
 * already typed one and therefore the owner's decision, not this file's.
 *
 * Flip this only when both are true. The screen's sentence is derived
