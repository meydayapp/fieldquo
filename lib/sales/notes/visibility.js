...
//
// **There is no reporting line on SalesRep.** The model carries email, name,
// workEmail, code, active, the invite fields, a commission plan and a demo
// company — and nothing that points at another rep or at a team. Worker DOES
// have `managerId` (`Worker.manager @relation("WorkerReports")`), used by leave
// approval, but a Worker is a CONTRACTOR'S employee inside a tenant. A SalesRep
// is FieldQuo staff and is not a Worker, not a Member and not in any company.
// The two identity spaces never meet.
//
// So a manager tier here would mean inventing an org chart: a column, a screen
// to set it, and a rule about what happens to notes when a rep changes
// managers. That is a product decision, not an implementation detail, and
// AGENTS.md says to ask rather than pick one. **This ships superadmin-only**,
// and the platform screen says why in as many words rather than leaving the
 ...
 *
 * Exported as a fact rather than left implicit, so the platform screen can
 * state it and the check can assert the screen states it. If SalesRep ever
 * gains a manager column, this flips and the screen's sentence has to change
 * with it — which is the point.
 */
export const HAS_REPORTING_LINE = false;
...
export const PLATFORM_NOTICE = {
  ...
  detail:
    ...
    "rewrite is not a record. There is no manager tier: SalesRep carries no " +
    "reporting line, so scoping a view to “my reps” would mean " +
    "inventing an org chart. Superadmins see all reps; nobody else sees any.",
};
