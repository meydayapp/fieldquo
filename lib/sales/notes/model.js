...
export const NOTES_UNAVAILABLE = {
  ...
  body: {
    ...
    // ── Why `code` was already here, and what it is now used for ───────────
    //
    // It has always ridden beside `error` so a caller could switch on the
    // state rather than match the sentence. What reads it now is the screen's
    // LANGUAGE choice: app/components/sales/RepNoteUnavailable.js maps the
    // code to a catalogue entry, so a rep who cannot save a note is told why
    // in their own language instead of being handed this file's English.
    //
    // The sentence stays. It is what a route returns to a caller with no
    // reader — a fetch from a script, a log line — and it is the fallback for
    // a build whose catalogue predates the key.
    code: "notes_model_missing",
  },
};

/**
 * The catalogue entry for one refusal code from this module.
 *
 * A map rather than a derived key, and one entry rather than a pattern: there
 * is exactly one refusal here, and a second one added later should have to
 * appear in this map deliberately rather than mint a key nobody has written.
 */
export const NOTES_REFUSAL_KEYS = Object.freeze({
  notes_model_missing: "app.salesNotes.unavailableModelMissing",
});
