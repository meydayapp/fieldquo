...
export const RETRY_RULES = Object.freeze({
  ...
  hung_up: Object.freeze({
    kind: RETRY_KIND_RETRY,
    // Not the busy rule's fifteen minutes: somebody who picked up and put
    // it straight down was at the phone and did not want THIS call. Ring
    // again in a different part of their day, and only twice more.
    delayMinutes: 3 * 60,
    sameDay: true,
    maxAttempts: 3,
    rotateBlock: true,
    why: "They picked up and put it down. They were there; try a different part of their day, and not again straight away.",
  }),
  reached: Object.freeze({
    // The day-end word for an INBOUND call a rep answered and never wrote up
    // (dispositions.js dayEndOutcome). What was said is unknown, so this is
    // gatekeeper's rule — a day on, a different part of their day — rather
    // than a final: the conversation is not lost, only its record is.
    kind: RETRY_KIND_RETRY,
    delayMinutes: 24 * 60,
    sameDay: false,
    maxAttempts: 4,
    rotateBlock: true,
    why: "They rang back and were spoken to, and nobody wrote down what was said. A day on, so the thread is picked up rather than dropped.",
  }),
  missed: Object.freeze({
    // The day-end word for an INBOUND call nobody at FieldQuo picked up. They
    // wanted to talk: ring them back soon, in the same part of the day they
    // chose to ring in. Busy's ceiling, so RETRY_MAX_ATTEMPTS does not move.
    kind: RETRY_KIND_RETRY,
    delayMinutes: 30,
    sameDay: true,
    maxAttempts: 6,
    rotateBlock: false,
    why: "They rang us and nobody picked up. Half an hour, same part of the day — they chose the time.",
  }),
  ...
  // "Text me instead": no re-dial, ever — ringing somebody who asked to be
  // texted is the call they declined. The next touch is a text from the
  // thread, which the call panel opens the moment this is saved.
  text_instead: Object.freeze({ kind: RETRY_KIND_FINAL, delayMinutes: null, sameDay: false, maxAttempts: null, rotateBlock: false, why: "They asked to be texted. No re-dial; the next touch is a text, from the thread the outcome opens." }),
  ...
  reached_not_interested: Object.freeze({
    // "Not now" (the owner, 2026-09-14): not a close, a later pass. Thirty
    // days, platform-editable like the other retries. The ceiling is busy's
    // six so RETRY_MAX_ATTEMPTS — the growth model's constant — does not
    // move; the count is cumulative across outcomes, so a row that has been
    // rung five times and then says "not now" is exhausted rather than
    // scheduled, and the recycle brings it back.
    kind: RETRY_KIND_RETRY,
    delayMinutes: 30 * 24 * 60,
    sameDay: false,
    maxAttempts: 6,
    rotateBlock: false,
    why: "Not now is not no. A later pass in a month, with what they said on top of the script.",
  }),
...
// Ninety days, not thirty: "Not now" defaults to a month and the platform
// has to be able to push it OUT as well as in.
export const RETRY_DELAY_MAX_MINUTES = 90 * 24 * 60;
