...
//                  CODE is the rep's opaque referralToken, or — for links
//                  handed out before 2026-09-25 — their legacy name slug.
...
import { findRepByLinkCode } from "./repLink";

/**
 * The door an INFLUENCER referral comes through: a company that signed up on
 * /refer/<code> where the code belongs to an influencer company
 * (Company.influencerRepId — see lib/influencers). Its own name rather than
 * "link" because the platform console and the ledger need to tell "a rep
 * handed out /signup?sales=" from "a customer's referral link earned a
 * commission" — but it goes through decideAttribution() exactly like the
 * others, and a source never changes the rules.
 */
export const INFLUENCER_SOURCE = "influencer_link";

/** The doors an attribution can come through. Anything else is refused. */
export const ATTRIBUTION_SOURCES = ["link", "manual", "admin", "lead_link", INFLUENCER_SOURCE];
...
async function loadRep(tx, { code, salesRepId }) {
  ...
  if (!code) return null;
  // The opaque referral token first, the legacy name slug second — so a link
  // minted today and a card printed last month both still attribute. An
  // unknown value is null here and `unknown_rep` in the verdict: nothing is
  // written. lib/sales/repLink.js holds the precedence rule, once.
  return findRepByLinkCode(code, tx, { id: true, email: true, active: true, endedAt: true, code: true });
}
