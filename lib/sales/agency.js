...
/** The agency behind a rep, as a small object for a response — or null. */
export function agencyOf(rep, manager = rep?.manager) {
  return isAgencyEmployee(rep, manager) ? { id: manager.id, name: manager.name } : null;
}
  ...
  const [attributions, entries, calls] = await Promise.all([
    ...
    callsReady
      ? client.salesCallAttempt.findMany({
          where: { salesRepId: { in: ids }, direction: "out", dialledAt: { gte: weekStart } },
