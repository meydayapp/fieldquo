...
export async function createAgencyRep({ agency, name, email, sellsIn = null, language = null, request = null, client = db }) {
  ...
  // — is `setupRequestedAt` on the row above, and nothing else. It used to
  // ALSO write a PlatformErrorLog row, which put a to-do on the errors page
  // beside refused webhooks and rejected credentials, had to be resolved by
  // hand, and stayed there after the number and the mailbox arrived. The
  // console now counts the flagged rows instead
  // (app/api/platform/sales/reps/count → the "Sales reps" badge), so the
  // notice clears itself through clearSetupIfComplete() below. The audit
  // trail is unchanged: agencyAudit's `sales_rep_invited` entry, just above,
  // is the record that this happened.
...
export async function agencyTeam({ agencyId, origin = null, now = new Date(), client = db } = {}) {
  ...
  const reps = await client.salesRep.findMany({
    ...
    select: {
      ...
      workEmail: true,
      code: true,
      // The opaque half of each employee's link and the name prospects see
      // (lib/sales/repLink.js, lib/sales/repIdentity.js).
      referralToken: true,
      workName: true,
      active: true,
      endedAt: true,
  ...
  // Each employee's link is built from their opaque token, never the name
  // slug in `code`. An employee who predates the token gets one minted here —
  // this list is where the agency copies links from, so it IS the first use,
  // one row at a time (lib/sales/repLink.js). No origin, no link, no mint.
  const linkCodes = new Map();
  for (const r of reps) {
    linkCodes.set(r.id, r.referralToken || (origin ? await ensureReferralToken(r, client).catch(() => null) : null));
  }
  ...
  return reps.map((r) => {
    ...
    return {
      id: r.id,
      name: r.name,
      // What prospects call this employee — the agency sees both.
      workName: r.workName || null,
      staffLabel: repStaffLabel(r),
      email: r.email,
      code: linkCodes.get(r.id) || null,
      signupLink: signupLinkFor(origin, linkCodes.get(r.id)),
      active: r.active,
      endedAt: r.endedAt,
