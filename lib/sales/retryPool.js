...
import { RETRY_RULES, effectiveRetryRules, nextAttempt, retryStateOf } from "./retryRules";
import { callingWindowFor } from "./callWindowScore";

const isDate = (v) => v instanceof Date && !Number.isNaN(v.getTime());

/**
 * The rule table this request runs on: the defaults, overridden by whatever
 * SalesRetryRule rows the platform has saved (lib/sales/retryRules.js
 * effectiveRetryRules). Read ONCE per request and handed down — the
 * disposition write, the queue's regroup, the lead screen's "Retry 2 of 3"
 * all see the same table, and a superadmin saving mid-request cannot make
 * one row's ceiling differ from the next's.
 *
 * A client without the delegate (the schema not yet pushed, a scripted stub
 * that has not scripted it) and a read that throws both answer the
 * defaults: the rule must never be the reason a disposition fails to save.
 */
export async function loadRetryRules({ db } = {}) {
  const delegate = db?.salesRetryRule;
  if (!delegate || typeof delegate.findMany !== "function") return RETRY_RULES;
  try {
    const rows = await delegate.findMany({});
    return effectiveRetryRules(rows);
  } catch (err) {
    console.error("[sales/retry] SalesRetryRule read failed; running on the defaults:", err?.message || err);
    return RETRY_RULES;
  }
}
...
export function retryWindowFor(prospect = {}) {
  // One resolver, shared with the best-window score, so a retry is rolled
  // into the same window the queue scores it in.
  return callingWindowFor(prospect);
}
 ...
 * @param rules      loadRetryRules()'s table for this request; the defaults
 *                   when the caller has none.
 *
 * @returns {{ decision, data, timeZone }} — `data` is what to write;
 *          `decision` is nextAttempt()'s answer, for the response.
 */
export function retryWriteFor({ outcome, prospect = {}, timeZone = null, callbackAt = null, now = new Date(), rules = RETRY_RULES } = {}) {
 ...
 */
export function retryViewFor(prospect = {}, { repZone = null, language = "en", now = new Date(), rules = RETRY_RULES } = {}) {
  const s = retryStateOf(prospect, now, rules);
  return {
...
export function regroupForRetry(windows, retries = {}, { shiftEnd = null, now = new Date() } = {}) {
  ...
  for (const [id, r] of Object.entries(retries || {})) {
    ...
    // One of FieldQuo's own test lines is never held by the pool: the
    // schedule the last outcome wrote is about not ringing a BUSINESS again
    // too soon, and this is our own desk. The rule's row still says
    // "Retry 2 of 3"; the group it sits in is the one the window put it in.
    if (byId[id].testLine === true) continue;
    if (r.exhausted) {
      later.push(id);
