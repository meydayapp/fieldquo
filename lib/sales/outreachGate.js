...
// salesEmailDraft joined on 2026-09-18: an email the rep has started and not
// sent, in its own table for the reason prisma/schema.prisma gives on the
// model — a message row means mail moved, and a draft is not that. It is the
// one table on this list a rep may DELETE from: discarding a draft destroys
// nothing that happened. Messages and threads still cannot be deleted, and
// scripts/check-sales-outreach.mjs still asserts it.
// salesIntroEmail and salesContactEmail joined on 2026-09-18, with the "we
// tried calling you" email (lib/sales/outreach/introSend.js). The first is
// one row per send — where it went, which language, what its links asked
// for — the rep's own record of an email they pressed Send on, and the
// 14-day guard against sending the same address twice. The second is the
// email counterpart of SalesContactNumber: an address a prospect gave the
// rep, saved on the lead so the closed recipient set can admit it. Neither
// moves a milestone or mints an entry. What the rep still cannot do through
// them: stamp a REQUEST. callbackRequestedAt and demoRequestedAt are set
// only by the public link route on a POST the prospect pressed, and
// scripts/check-sales-intro-email.mjs asserts no route under /api/sales
// names those columns.
...
export async function requireOutreachRep(request) {
  ...
  const row = await db.salesRep.findUnique({
    ...
    select: {
      ...
      workEmail: true,
      name: true,
      // The name outsiders see — every route behind this gate composes
      // something a prospect reads, and a route handed a row without it
      // would sign as the rep's real first name. lib/sales/repIdentity.js.
      workName: true,
      // The opaque half of the rep's link (lib/sales/repLink.js), so a route
      // that builds one skips a read when it is already minted.
      referralToken: true,
      active: true,
      endedAt: true,
      acceptedAt: true,
      // Read for canAuthenticate: an influencer's ledger row is not a login.
      kind: true,
      // The agency tier (lib/sales/agency.js), read here for the same reason
      // gate.js reads it: /api/sales/payout is behind THIS gate, and an
      // agency's employee must be refused there on a row read in this
      // request, not on a flag a screen remembered.
      engagement: true,
      managerId: true,
      commissionPlanId: true,
      manager: { select: { id: true, kind: true, name: true, email: true } },
