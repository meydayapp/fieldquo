...
// own — queueBatch.js), except in "Callable now", which is sorted first by
// the BEST-WINDOW SCORE (lib/sales/callWindowScore.js — 3–4 pm in the
// prospect's zone before 10–11 am before the rest of the open window, and a
// Wednesday before a Friday; the owner's 2026-09-13 decision from Belkins'
// 175k-dial benchmark) and then by closing time, so that in the afternoon an
// Atlantic row that shuts at seven is rung before a Pacific row that shuts
// at eleven. A due retry still goes in front of both — lib/sales/retryPool.js
// regroupForRetry moves it there afterwards, and its header says why: the
// rotation aimed that row at THIS part of the day, and a score is a
// statistical preference while a due retry is a plan already half-executed.
//
// ══ Whose clock ═══════════════════════════════════════════════════════════
//
...
import { callingWindowFor, isBestTimeNow, windowScore } from "./callWindowScore";
...
export function repClock(at, { repZone = null, language = "en", now = new Date() } = {}) {
  ...
  try {
    const time = new Intl.DateTimeFormat(language || "en", {
      ...
      hour: "2-digit",
      minute: "2-digit",
      // `hourCycle: "h23"`, not `hour12: false`: under ICU the latter is
      // the h24 cycle in some locales, and midnight printed as "24:05" —
      // which is what a Karachi rep's give-back strip said on 2026-09-17
      // (the sweep runs at :05 past their midnight). h23 is 00:05.
      hourCycle: "h23",
...
export function windowFor({
  ...
  language = "en",
  policyContext = null,
  // The row's number is one of FieldQuo's own test lines
  // (lib/sales/testLines.js). Looked up by the caller against the number the
  // dial would ring; the readiness then answers "callable now" at any hour,
  // with no window score — a test dial is never "the best minute to ring".
  testLine = false,
  // The rep holding the queue is a test account (SalesRep.testAccount): every
  // row is callable now, unscored, unheld — the same answer a test line gets,
  // from the same path. Read off the rep row by the route, never by a screen.
  testAccount = false,
} = {}) {
  ...
  // The best-window score, only for a row callable now, in the PROSPECT's
  // zone (or its candidate zones when the province is split and nobody has
  // resolved it) and in the window the row is judged by. A row callable now
  // because the console switched its window off scores null — the score
  // never promotes a minute the window rule would refuse.
  let score = null;
  if (callableNow && !readiness.testLine) {
    ...
    score = windowScore({ now, timeZone: zones, window: callingWindowFor(prospect) });
  }
  ...
  return {
    ...
    // A test dial — one of our own phones (lib/sales/testLines.js) or a test
    // account's row: the chip says so and the retry pool's regroup leaves it
    // in "Callable now". `testMode` says which of the two it is.
    testLine: readiness.testLine === true,
    testMode: readiness.testMode || null,
    ...
    // lib/sales/callWindowScore.js: higher is a better minute to ring this
    // row; null when not callable now. `bestTimeNow` is the chip.
    windowScore: score,
    bestTimeNow: isBestTimeNow(score),
    kind,
 ...
 *
 * @param rows     `[{ id, country, province, timeZone, name, testLine? }]` in claim order.
...
export function groupByWindow(
  rows = [],
  { repZone = null, shiftEnd = null, now = new Date(), language = "en", policyContext = null, testAccount = false } = {},
) {
  ...
  const entries = list.map((row, position) => {
    const w = windowFor({
      ...
      language,
      policyContext,
      testLine: row.testLine === true,
      testAccount: testAccount === true,
    });
    byId[row.id] = w;
    return { id: row.id, position, w, name: String(row.name || "").trim() };
  });

  const stamp = (iso) => (iso ? Date.parse(iso) : Number.POSITIVE_INFINITY);
  // The owner's ask of 2026-09-15: within a window the list reads A→Z, so a
  // rep can find a business by name. Alphabetical sits UNDER the window
  // rules, not over them — best window first, shuts soonest first — because
  // those decide who is lost if the rep waits; only rows that tie on both
  // are ordered by name. Claim order is the last tie-break, for two rows
  // with the same name.
  const byName = (a, b) => a.name.localeCompare(b.name, undefined, { sensitivity: "base", numeric: true });

  // Callable now: the best window first (the header says whose numbers),
  // then shuts soonest, A→Z for ties. A row with no score (its window is
  // off, its zone unreadable) sorts after every scored row; a row with no
  // closing time (nothing shuts it within a day) sorts last, not first.
  const scoreOf = (e) => (Number.isFinite(e.w.windowScore) ? e.w.windowScore : -1);
  const now_ = entries
    .filter((e) => e.w.kind === WINDOW_GROUP_NOW)
    .sort((a, b) => scoreOf(b) - scoreOf(a) || stamp(a.w.closesAtIso) - stamp(b.w.closesAtIso) || byName(a, b) || a.position - b.position);
  ...
  // A→Z inside each opening group and in "later" too — same rule as above.
  const opensSorted = entries.filter((x) => x.w.kind === WINDOW_GROUP_OPENS).sort((a, b) => byName(a, b) || a.position - b.position);
  for (const e of opensSorted) {
  ...
  const later = entries.filter((e) => e.w.kind === WINDOW_GROUP_LATER).sort((a, b) => byName(a, b) || a.position - b.position);
