...
export function groupByWindow(
  ...
  // then shuts soonest, claim order for ties. A row with no score (its
  // window is off, its zone unreadable) sorts after every scored row; a row
  // with no closing time (nothing shuts it within a day) sorts last, not first.
  const scoreOf = (e) => (Number.isFinite(e.w.windowScore) ? e.w.windowScore : -1);
  const now_ = entries
    .filter((e) => e.w.kind === WINDOW_GROUP_NOW)
    .sort((a, b) => scoreOf(b) - scoreOf(a) || stamp(a.w.closesAtIso) - stamp(b.w.closesAtIso) || a.position - b.position);
