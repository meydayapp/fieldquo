...
export function canAuthenticate(rep) {
  if (!rep) return false;
  // An influencer's ledger row (lib/influencers) is a SalesRep by table and
  // not by any other measure: it never gets a password or an accepted
  // invite, and it is refused here by kind as well, so that no later change
  // to how invites work can turn a customer's commission ledger into a
  // sales-portal login. They read their ledger on /app/influencer.
  if (rep.kind === "influencer") return false;
