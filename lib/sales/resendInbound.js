...
export function receivedEmailToContract(email, event = {}) {
  ...
  return {
    ...
    // Who else the prospect copied — kept on the row for reply-all, and
    // never a token haystack (parseInboundEmail does not read it for one).
    cc: joinAddresses(e.cc) || header(headers, "cc"),
...
export function forwardHeaderLine({ filed, reason, prospect, attachmentsNote } = {}) {
  ...
  if (filed) {
    ...
    const why =
      {
        no_token: "the message carried no reply token and its sender is not one of your leads, so FieldQuo could not tell which prospect it belongs to",
        ambiguous_sender: "the message carried no reply token and its sender's address is on two of your leads, so FieldQuo could not choose",
