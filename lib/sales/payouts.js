 ...
 */
export async function closeWeekForRep({ salesRepId, start, end, earnerIds = null, prisma = db }) {
  if (!salesRepId || !start || !end) return null;

  // `earnerIds` — whose entries close into THIS batch. Absent, the rep's own.
  // Present, the payee's whole group (lib/sales/agency.js payeeGroups): a
  // call-centre agency's employees earn under their own ids — that is what
  // keeps "who are the best salespeople" answerable — and are PAID through
  // the agency's batch. The entries keep their salesRepId; only the batch
  // they land in is the agency's. The payee is always in its own group, so
  // an agency that earns on its own link is gathered too.
  const earners = Array.isArray(earnerIds) && earnerIds.length
    ? [...new Set([salesRepId, ...earnerIds.filter((id) => typeof id === "string" && id)])]
    : [salesRepId];

  const entries = await prisma.salesCommissionEntry.findMany({
    where: { salesRepId: { in: earners }, payoutBatchId: null },
