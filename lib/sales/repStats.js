 ...
 *
 * `linkToken` — the per-text token lib/sales/signupProgress.js mints when a
 * rep TEXTS the link to a lead, so the signup page can report its steps to
 * that rep's panel. The portal's copyable link and the platform's listing
 * carry none: they are the rep's link, not a link to one person.
 *
 * `code` is the rep's opaque SalesRep.referralToken — from
 * lib/sales/repLink.js ensureReferralToken, never SalesRep.code. The legacy
 * code is the rep's real name slugged; it still RESOLVES (old cards, old
 * texts) but no link is built from it any more.
 */
export function signupLinkFor(origin, code, { linkToken = null } = {}) {
  if (!origin || !code) return null;
  const base = `${String(origin).replace(/\/+$/, "")}/signup?sales=${encodeURIComponent(code)}`;
  return linkToken ? `${base}&link=${encodeURIComponent(linkToken)}` : base;
}
