 ...
 */
export function repOnShift({ activity = null, lastDialAt = null, portalSeenAt = null, now = new Date() } = {}) {
  ...
  // The keepalive (SalesRep.lastSeenAt) is presence on its own since
  // 2026-09-21 — a rep with a portal tab open and no ledger row is
  // Available by derivation (agentState.js), and is at their desk.
  const seenMs = portalSeenAt instanceof Date ? portalSeenAt.getTime() : Date.parse(portalSeenAt);
  if (Number.isFinite(seenMs) && nowMs - seenMs <= PRESENCE_STALE_MINUTES * 60 * 1000) return true;
...
export async function repStillWorking({ db, salesRepId, now = new Date() } = {}) {
  ...
  const last =
    typeof db?.salesCallAttempt?.findFirst === "function"
      ...
      : null;
  const rep =
    typeof db?.salesRep?.findUnique === "function"
      ? await db.salesRep.findUnique({ where: { id: salesRepId }, select: { lastSeenAt: true } }).catch(() => null)
      : null;
  return repOnShift({ activity, lastDialAt: last?.dialledAt || null, portalSeenAt: rep?.lastSeenAt || null, now });
}
