...
// here is `demoCompanyId`, pointable only at an isDemo company — a free pool
// demo while it is null, or a demo seeded for this very rep — and read by
...
export const DEMO_GATE_WRITES = [
  // The pointer — at a pool demo while it is null (the claim), or at one of
  // the rep's OWN demos (pointRepAtOwnDemo). Both in demoAssign.js.
  "salesRep.demoCompanyId",
  // The rep's own demo companies: created, renamed, retired — never deleted.
  // lib/sales/repDemo.js, which re-reads demoOwnerRepId on every write.
  "company (isDemo, demoOwnerRepId = this rep): create, demoRetiredAt, demoRepSlot, authOrgId",
  ...
  // The demo login: a Better Auth user at demo-<code>@fieldquo.com, minted
  // through lib/demo/demoLogin.js's mintDemoUser, and its owner Member rows on
  // the rep's own demos (active on the one they are driving). repDemo.js.
  "user (demo-<code>@fieldquo.com) via demoLogin.mintDemoUser; member rows on the rep's own demos",
