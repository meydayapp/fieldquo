...
export function holderOf(number = {}) {
  ...
  if (number.assignedAdminId) {
    return {
      ...
      // PlatformAdmin has NO name column — email is the only human-readable
      // field on it. A `name ||` fallback here would read as though one
      // existed, which is how a select for it got written and 500'd a page.
      name: number.assignedAdmin?.email || "an admin",
    };
  }
