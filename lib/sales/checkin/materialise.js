...
// — which runs the SAME planner on the same schedule, and whose rows say
// "demo" in their origin and in their key so the send path can simulate the
// send on the company row (store.js) rather than reach a carrier.
...
import { openTriageThreads } from "../messages/triageStore";
import { FIRST_CHECKIN_DAY } from "./signals";
import { ruleDraft } from "./draft";
import { nextWindowOpening } from "./schedule";
import { KEY_PREFIX_DEMO, checkInSummary, planCheckIn, resolveCompanyNumber } from "./plan";
import { insertDraftRow } from "./rows";
import { materialiseUnfinishedSignupsForRep } from "./unfinishedSignup";
import { materialiseLinkNoSignupForRep } from "./linkNoSignup";
...
export async function materialiseCheckInsForRep({
  ...
  const rep = await client.salesRep.findUnique({
    where: { id: salesRepId },
    select: { id: true, name: true, workName: true, commissionPlan: { select: { retentionDays: true } } },
  });
  ...
  // ── Unfinished signups: the link was opened and never finished ─────────
  //
  // lib/sales/checkin/unfinishedSignup.js — two drafts, +2 h and +24 h
  // after the OPEN, on the lead, with the same link. Its own pass, before
  // the company loop, because these rows have no company to loop over;
  // its own try/catch, because a failure here must not cost a rep their
  // day-1 drafts. The origin for the link comes from the deployment
  // (NEXT_PUBLIC_APP_URL / VERCEL_URL); with neither, the pass skips with
  // `no_origin` rather than texting a link to nowhere.
  out.signupDrafts = { created: [], skipped: [] };
  try {
    let origin = null;
    try {
      origin = getAppOrigin();
    ...
    out.signupDrafts = await materialiseUnfinishedSignupsForRep({ salesRepId: rep.id, client, now, origin, dryRun });
  } catch (err) {
    log?.(`[sales check-in] unfinished-signup drafts failed for rep ${rep.id}: ${err?.message}`);
  }

  // lib/sales/checkin/linkNoSignup.js — the link went and nothing came of
  // it: day 2, day 5, a last nudge before the claim lapses; stopped (and
  // its open drafts dismissed) on a signup, a reply, a STOP, a released
  // claim or an opened link. Same isolation as the pass above, same
  // reason. No origin needed: these drafts carry no link — it is already
  // in the thread.
  out.linkDrafts = { created: [], skipped: [], dismissed: [] };
  try {
    out.linkDrafts = await materialiseLinkNoSignupForRep({ salesRepId: rep.id, client, now, dryRun });
  } catch (err) {
    log?.(`[sales check-in] link-no-signup drafts failed for rep ${rep.id}: ${err?.message}`);
  }
  ...
  for (const c of companies) {
    ...
    if (plan.state === "due" || plan.state === "refresh") {
      ...
      const draftText = ruleDraft(plan.decision, { repName: repPublicName(rep) });
      const entry = {
        companyId: c.id,
        name: c.name,
...
/**
 * The check-ins on the rep's OWN demo company, on the real schedule.
 *
 * ══ What changed on 2026-09-14, and why ═══════════════════════════════════
 *
 * This used to write ONE row — the day-1 draft, keyed `demo:<id>:1`, with a
 * signup date of "yesterday" — and the send path refused it. The owner, on
 * his own demo thread: "the drafts that are created don't have a send
 * button … shouldn't the draft be for companies that signed up regardless
 * of the milestone?" A demo that shows a draft nobody can press, and never
 * a day-7 or milestone draft, teaches a rep the shape of the feature and
 * none of its behaviour. So the demo now runs the SAME planner over a
 * stand-in company, and the rows it writes are the same touchpoints —
 * day 1, day 7, the milestone approach — with the same refresh rule and the
 * same "sent before the next one" arithmetic, under the `demo:` prefix.
 *
 * ══ The stand-in's timeline ═══════════════════════════════════════════════
 *
 * The demo "signed up" the day before the rep first saw its draft: the
 * anchor is the EARLIEST demo row's createdAt minus FIRST_CHECKIN_DAY, and
 * `now - FIRST_CHECKIN_DAY` when there is none yet — which is why a new rep
 * still finds the day-1 draft waiting on their first morning. Anchored to a
 * row that already exists rather than to a new column, so the schedule
 * survives a reset of the demo's contents and needs nothing in the schema.
 * From there the clock runs: day 7 arrives six days later, the milestone
 * approach at retentionDays - RETENTION_NEAR_DAYS, exactly as for a customer.
 *
 * ══ Clearly a fixture, in three places ════════════════════════════════════
 *
 *   origin      "demo"                — the row says what it is
 *   dedupeKey   demo:<id>:<1|7|retention> — never the shape a real touchpoint uses
 *   toE164      a 555 number          — reserved for fiction, rings nobody
 *
 * and the send path re-reads the company row and, on isDemo, SIMULATES the
 * send (store.js sendCheckIn: the row becomes "sent", no carrier is called,
 * no SalesSmsMessage is written). signals.js would have suppressed this
 * company as `demo_company` and draftCheckIn() would have refused it as
 * `not_contactable`; both are right for a real thread, and this function
 * clears the flag on the stand-in only — the company row is never written.
 *
...
export async function materialiseDemoCheckIn({ salesRepId, client = db, now = new Date(), dryRun = false } = {}) {
  ...
  const rep = await client.salesRep.findUnique({
    where: { id: salesRepId },
    select: { id: true, name: true, workName: true, demoCompanyId: true, commissionPlan: { select: { retentionDays: true } } },
  });
  ...
  // Only the demo-keyed rows: a manual follow-up the rep parked on the demo
  // thread is theirs and is not a touchpoint.
  const rows = await client.salesCheckIn.findMany({
    where: { salesRepId: rep.id, companyId: company.id },
    select: { ...CHECKIN_ROW_SELECT, createdAt: true },
    orderBy: { createdAt: "asc" },
  });
  const existing = rows.filter((r) => r.origin === DEMO_ORIGIN && /^demo:/.test(String(r.dedupeKey || "")));

  const days = rep.commissionPlan?.retentionDays;
  const retentionDays = Number.isFinite(days) && days > 0 ? days : null;
  const DAY = 24 * 60 * 60 * 1000;
  const firstSeen = existing.map((r) => new Date(r.createdAt).getTime()).filter(Number.isFinite).sort((a, b) => a - b)[0];
  const signedUpAt = new Date((Number.isFinite(firstSeen) ? firstSeen : now.getTime()) - FIRST_CHECKIN_DAY * DAY);
  const lastCheckInAt =
    existing
      .filter((r) => r.status === "sent" && r.sentAt)
      .map((r) => r.sentAt)
  ...
  // The stand-in. See the docblock: the anchored signup, the demo flag
  // cleared, the demo's own setup facts. It exists to produce a plan.
  const standIn = {
    id: company.id,
    name: company.name,
    signedUpAt,
  ...
  const plan = planCheckIn({
    ...
    retentionDays,
    existing,
    timeZone: COMPANY_TIMEZONE_DEFAULT,
    keyPrefix: KEY_PREFIX_DEMO,
    now,
  });

  const base = { ok: true, companyId: company.id, touchpoint: plan.touchpoint, state: plan.state, dayInLife: plan.decision?.dayInLife ?? null };

  if (plan.state === "open") return { ...base, created: false, reason: "exists", id: plan.row?.id || null };
  if (plan.state !== "due" && plan.state !== "refresh") return { ...base, created: false, reason: plan.state };

  const draftText = ruleDraft(plan.decision, { repName: repPublicName(rep) });
  const dedupeKey = plan.dedupeKey;
  const scheduledFor = plan.scheduledFor || nextWindowOpening(now, COMPANY_TIMEZONE_DEFAULT);

  if (dryRun) return { ...base, created: false, dryRun: true, wouldCreate: { companyId: company.id, draftText, dedupeKey } };

  if (plan.state === "refresh") {
    // The same re-aim the real path does: the untouched day-1 draft becomes
    // the day-7 one rather than being joined by it.
    const { count } = await client.salesCheckIn.updateMany({
      where: { id: plan.row.id, salesRepId: rep.id, status: "draft", draftSource: "rule", sendingStartedAt: null },
      data: { draftText, reasonCode: plan.decision.primary?.code || null, dedupeKey, scheduledFor },
    });
    return { ...base, created: false, refreshed: count > 0, reason: count > 0 ? "refreshed" : "exists", id: plan.row.id, draftText };
  }
  ...
  return { ...base, created: !existed, reason: existed ? "exists" : "created", id: checkIn.id, draftText };
}
...
export async function materialiseCheckInsForAllReps({
  ...
  // openRoadblocks / openQuestions: replies the triage filed as a roadblock
  // or a question that no rep has answered yet (lib/sales/messages/triage.js),
  // summed across reps so the morning's report says how many contractors
  // are stuck. Counted, not acted on — this cron still sends nothing.
  const report = { reps: 0, considered: 0, created: 0, refreshed: 0, repaired: 0, leadsCreated: 0, demoCreated: 0, failed: 0, openRoadblocks: 0, openQuestions: 0, perRep: [] };
  for (const rep of reps) {
    ...
    try {
      ...
      // Null when the count could not be read — a digest must not print a
      // reassuring zero for a table it could not open.
      const openTriage = await openTriageThreads({ salesRepId: rep.id, client })
        .then((o) => o.counts)
        .catch(() => null);
      report.considered += r.considered;
      report.created += r.created.length + (r.signupDrafts?.created?.length || 0) + (r.linkDrafts?.created?.length || 0);
      ...
      if (openTriage) {
        report.openRoadblocks += openTriage.roadblock || 0;
        report.openQuestions += openTriage.question || 0;
      }
      report.perRep.push({
        repId: rep.id,
        name: rep.name,
        ...
        skipped: r.skipped,
        signupDrafts: r.signupDrafts,
        linkDrafts: r.linkDrafts,
        demo,
        openTriage,
      });
    } catch (err) {
