...
/**
 * The two prefixes a touchpoint key can carry.
 *
 * `scheduled:` is a real company's row. `demo:` is the SAME touchpoint on the
 * rep's own demo company — the same planner, the same day-1 / day-7 /
 * milestone rows, on the demo's stand-in timeline (materialise.js
 * materialiseDemoCheckIn) — kept under its own prefix so the send path can
 * tell a fixture from a customer by the key alone, and so the two engines
 * can never claim each other's rows.
 */
export const KEY_PREFIX_SCHEDULED = "scheduled";
export const KEY_PREFIX_DEMO = "demo";

/** The materialised backlog's key: one per company per touchpoint. */
export function scheduledDedupeKey({ companyId, touchpoint, prefix = KEY_PREFIX_SCHEDULED } = {}) {
  if (!companyId) return null;
  if (prefix !== KEY_PREFIX_SCHEDULED && prefix !== KEY_PREFIX_DEMO) return null;
  ...
  return `${prefix}:${companyId}:${tp}`;
}

/**
 * The touchpoint a `scheduled:` or `demo:` key names, or null for any other
 * key. Both prefixes, because the planner's refresh rule ("an untouched
 * draft for an EARLIER touchpoint is re-aimed") has to read a demo row's
 * touchpoint the same way it reads a real one — the demo shows the real
 * behaviour or it shows nothing worth showing.
 */
export function touchpointOfKey(dedupeKey) {
  const m = /^(?:scheduled|demo):[^:]+:(\d+|retention)$/.exec(String(dedupeKey || ""));
  if (!m) return null;
  return m[1] === TOUCHPOINT_RETENTION ? TOUCHPOINT_RETENTION : Number(m[1]);
}

/**
 * What KIND of draft a key names, for a screen that lists drafts of every
 * origin side by side (the Today card, the texts banner).
 *
 *   scheduled:<co>:<1|7|retention>  a real company's touchpoint
 *   demo:<co>:<1|7|retention>       the same touchpoint on the rep's demo
 *   signup:<progress>:<2h|24h>      an unfinished-signup nudge (unfinishedSignup.js)
 *   engine:<co>:<yyyy-mm-dd>        the thread's own on-open suggestion, adopted
 *   anything else / null            a follow-up the rep parked by hand
 *
 * Returns `{ kind, touchpoint }` — `touchpoint` a day number, "retention",
 * "2h" / "24h", or null. Parsed from the key, never from a second column, so
 * the label a screen prints cannot disagree with the row the engine wrote.
 */
export function describeDraftKey(dedupeKey) {
  const key = String(dedupeKey || "");
  const scheduled = /^(scheduled|demo):[^:]+:(\d+|retention)$/.exec(key);
  if (scheduled) {
    return {
      kind: scheduled[1],
      touchpoint: scheduled[2] === TOUCHPOINT_RETENTION ? TOUCHPOINT_RETENTION : Number(scheduled[2]),
    };
  }
  const signup = /^signup:[^:]+:(2h|24h)$/.exec(key);
  if (signup) return { kind: "signup", touchpoint: signup[1] };
  // The link-went-nowhere sequence (linkNoSignup.js): d2 / d5 / last.
  const linksent = /^linksent:[^:]+:(d2|d5|last)$/.exec(key);
  if (linksent) return { kind: "linksent", touchpoint: linksent[1] };
  if (/^engine:[^:]+:\d{4}-\d{2}-\d{2}$/.test(key)) return { kind: "engine", touchpoint: null };
  return { kind: "manual", touchpoint: null };
}
 ...
 * @param keyPrefix      KEY_PREFIX_SCHEDULED for a real company (the
 *                       default); KEY_PREFIX_DEMO for the rep's demo, whose
 *                       rows are the same touchpoints under a key the send
 *                       path recognises as a fixture.
...
export function planCheckIn({
  ...
  existing = [],
  timeZone = null,
  keyPrefix = KEY_PREFIX_SCHEDULED,
  now = new Date(),
} = {}) {
  ...
  const dedupeKey = scheduledDedupeKey({ companyId: company?.id, touchpoint, prefix: keyPrefix });
