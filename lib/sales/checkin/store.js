...
import { parseLinkNoSignupKey } from "./linkNoSignup";
 ...
 *   read does.
 */
export async function threadContext({ salesRepId, toE164, client = db, lead: resolved = undefined } = {}) {
  // A caller that has already resolved the business (businessResolve.js)
  // hands its lead in; `undefined` — not null — means "look it up here".
  const lead = resolved === undefined ? await leadForThread({ salesRepId, toE164, client }) : resolved;
  let company = null;
...
/**
 * @param numbers  every number of the business the thread is with, when the
 *   caller has resolved them (businessResolve.js) — a draft aimed at the
 *   shop line belongs in the conversation opened from the owner's cell.
 *   Defaults to `toE164` alone.
 */
function numberSet(toE164, numbers) {
  return [...new Set([toE164, ...(numbers || [])].map((n) => normalisePhone(n)).filter(Boolean))];
}

export async function openCheckIns({ salesRepId, toE164, numbers = null, client = db } = {}) {
  const set = numberSet(toE164, numbers);
  if (!set.length || !salesRepId) return [];
  const rows = await client.salesCheckIn.findMany({
    where: { salesRepId, toE164: set.length === 1 ? set[0] : { in: set }, status: "draft" },
  ...
  // "Link sent Tue — no signup, no reply": the sentence under a link-went-
  // nowhere draft names WHEN the link went, which is on the progress row
  // the key points at. One read for every such row on the thread; a row
  // whose progress could not be found keeps the sentence without the day.
  const linked = rows.map((r) => ({ r, key: parseLinkNoSignupKey(r.dedupeKey) })).filter((x) => x.key);
  let sentAtById = new Map();
  if (linked.length && typeof client.salesSignupProgress?.findMany === "function") {
    const progress = await client.salesSignupProgress
      .findMany({ where: { id: { in: [...new Set(linked.map((x) => x.key.progressId))] } }, select: { id: true, linkSentAt: true } })
      .catch(() => []);
    sentAtById = new Map(progress.map((p) => [p.id, p.linkSentAt]));
  }
  return rows.map(({ dedupeKey, ...r }) => {
    const key = parseLinkNoSignupKey(dedupeKey);
    return key ? { ...r, sequence: { kind: "linksent", touchpoint: key.touchpoint }, reasonParams: { sentAt: sentAtById.get(key.progressId) || null } } : r;
  });
}
...
export async function createCheckIn({
  ...
  if (!draftText) {
    ...
    const drafted = await draftCheckIn({
      decision,
      // Signs a text to the contractor: the public name (repIdentity.js).
 ...
 *
 * ══ One branch never reaches the carrier: the rep's demo ═══════════════════
 *
 * A draft on a company whose row says isDemo — re-read here, never taken
 * from the request — is sent by simulateDemoSend() below: the same claim,
 * the same status flip, no deliverReplySms, no Twilio, no SalesSmsMessage.
 * The result carries `demo: true` so the route and the screen can say so.
 *
...
export async function sendCheckIn({
  ...
  // on file"; a demo draft exists so a rep sees what a check-in looks like
  // on their walkthrough account. deliverReplySms would refuse the first
  // (lead_no_phone) but not the second — a fictional 555 number is North
  // American and on nobody's do-not-contact list — so the demo is decided
  // HERE, on the company row re-read now, before anything can leave.
  if (!row.toE164) {
    return {
      ok: false,
      status: 409,
  ...
  if (row.companyId || row.origin === "demo") {
    const owner = row.companyId
      ? await client.company.findUnique({ where: { id: row.companyId }, select: { isDemo: true } })
      : null;
    if (owner?.isDemo === true || row.origin === "demo") {
      return simulateDemoSend({ rep, row, client, now });
    }
  }
  ...
  // The lead by NUMBER first (the rep may have corrected its zone since the
  // draft was written), else the lead the ROW names. The second case is the
  // company that signed up straight from the link: the backlog aims its
  // draft at the company's own phone (plan.js resolves it from Company.phone
  // and the owner's Member.phone), which need not be the number on the
  // rep's lead — and a draft with a lead on it that the send could not see
  // was refused for "we don't know their time zone" when the row knew the
  // province all along. The row's number still wins over the lead's phone:
  // the text goes where the draft says, judged in the lead's zone.
  const byNumber = await leadForThread({ salesRepId: rep.id, toE164: row.toE164, client });
  const byRow =
    !byNumber && row.leadId
      ? await client.salesLead
          .findFirst({
            where: { id: row.leadId, salesRepId: rep.id },
            select: {
              id: true, businessName: true, contactName: true, phone: true, timeZone: true, status: true,
              convertedCompanyId: true, salesRepId: true, email: true, province: true, country: true,
              prospectId: true, prospect: { select: { country: true, province: true } },
            },
          })
          .catch(() => null)
      : null;
  const lead = byNumber || (byRow ? { ...byRow, phone: row.toE164 } : null);
  const send = deliver || deliverReplySms;

  const result = await send({
    rep,
    // A lead when the phone matches one or the row names one, otherwise the
    // bare number with a null zone — which the window check refuses, in
...
/**
 * The demo's Send: everything a real send does except the carrier.
 *
 * ══ Why this exists, in the owner's words ═════════════════════════════════
 *
 * "the drafts that are created don't have a send button" — on his demo
 * thread. The send path refused a demo draft outright, the screen hid the
 * button, and a rep learning the tool on the account built for learning it
 * never saw what pressing Send does. This is the pattern
 * lib/social/mockMetaGraphClient.js sets for Meta: the REAL orchestration
 * runs — the same read, the same compare-and-set claim, the same status
 * flip on the same row — and the one call that would leave the building is
 * answered here instead. The decision to come here is taken on
 * `Company.isDemo` re-read from the database a moment ago, never on a flag
 * the request could carry.
 *
 * ══ What is deliberately NOT written ══════════════════════════════════════
 *
 * No SalesSmsMessage. That table's one invariant — a row exists iff the
 * carrier accepted the message (its own schema comment, and every count of
 * work done reads it: the funnel, growth, the triage) — is worth more than a
 * convenient thread row. The thread and the list read the SENT demo check-in
 * itself and draw it as the outbound message (sentDemoCheckIns below), so
 * the rep sees the bubble, marked as a demo, and no report ever counts a
 * text that was never sent.
 *
 * No readiness gates either. The window, the opt-out list and the mailing
 * address are judged for a real contractor's number and zone; the demo's
 * 555 number has neither, so a demo that refused with "we don't know their
 * time zone" would demonstrate the refusal and nothing else. The gates are
 * exercised on real threads, and the check proves this path never reaches
 * deliverReplySms.
 */
async function simulateDemoSend({ rep, row, client, now }) {
  const claim = await client.salesCheckIn.updateMany({
    where: { id: row.id, salesRepId: rep.id, status: "draft", sendingStartedAt: null },
    data: { sendingStartedAt: now },
  });
  if (claim.count === 0) {
    return { ok: false, status: 409, error: "A send of this draft is already under way.", demo: true };
  }
  const checkIn = await client.salesCheckIn.update({
    where: { id: row.id },
    data: { status: "sent", sentAt: now, sentMessageId: null },
  });
  return { ok: true, checkIn, messageId: null, demo: true };
}

/**
 * The demo thread's "messages": check-ins that were sent from a demo.
 *
 * Read from SalesCheckIn, not SalesSmsMessage — see simulateDemoSend for why
 * no message row exists. Shaped like salesThread()'s rows, with `demo: true`
 * on each, so the screen draws them as outbound bubbles and labels them.
 */
export async function sentDemoCheckIns({ salesRepId, toE164, numbers = null, client = db } = {}) {
  const set = numberSet(toE164, numbers);
  if (!set.length || !salesRepId) return [];
  const rows = await client.salesCheckIn.findMany({
    where: { salesRepId, toE164: set.length === 1 ? set[0] : { in: set }, status: "sent", origin: "demo" },
    orderBy: { sentAt: "asc" },
    select: { id: true, draftText: true, sentAt: true, toE164: true, companyId: true, dedupeKey: true, origin: true },
  });
  return rows.map((r) => {
    // The same chip a real send gets — "Day 1 check-in" — decided by the
    // same function over the row's own key, so the demo teaches the real
    // thread's vocabulary.
    const kind = outboundKind({ direction: "out", body: r.draftText, checkIn: { dedupeKey: r.dedupeKey, origin: r.origin } });
    return {
      id: `demo:${r.id}`,
      direction: "out",
      body: r.draftText,
...
/**
 * The demo threads that have had a send and no longer hold a draft — so the
 * list keeps showing the conversation after the rep presses Send, the way a
 * real thread would move from "Drafts due" to "Waiting on them".
 *
 * @returns `Map<e164, { name, companyId, lastBody, lastAt }>`
 */
export async function sentDemoByThread({ salesRepId, client = db } = {}) {
  const rows = await client.salesCheckIn.findMany({
    where: { salesRepId, status: "sent", origin: "demo", toE164: { not: null } },
    orderBy: { sentAt: "desc" },
    select: { toE164: true, draftText: true, sentAt: true, companyId: true, company: { select: { name: true } } },
  });
  const byThread = new Map();
  for (const r of rows) {
    if (byThread.has(r.toE164)) continue;
    byThread.set(r.toE164, { name: r.company?.name || null, companyId: r.companyId, lastBody: r.draftText, lastAt: r.sentAt });
  }
  return byThread;
}
