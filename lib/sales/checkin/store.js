 ...
 */
export async function sentDemoCheckIns({ salesRepId, toE164, client = db } = {}) {
  const other = normalisePhone(toE164);
  if (!other || !salesRepId) return [];
  const rows = await client.salesCheckIn.findMany({
    where: { salesRepId, toE164: other, status: "sent", origin: "demo" },
    orderBy: { sentAt: "asc" },
    select: { id: true, draftText: true, sentAt: true, toE164: true, companyId: true },
  });
  return rows.map((r) => ({
    id: `demo:${r.id}`,
    direction: "out",
