...
export async function leadForThread({ salesRepId, toE164, client = db } = {}) {
  ...
  // wins. Both are the same conversation either way.
  return candidates.find((lead) => normalisePhone(lead.phone) === other) || null;
}
 ...
 *
 * The day is UTC rather than anybody's local one, deliberately: this key is
 * never shown to a human and never compared with a displayed date, so a stable
 * definition beats a friendly one. A key built in the rep's zone would let a
 * rep travelling east produce two drafts for one day.
 */
...
/** The open drafts on one thread, oldest first. */
/**
...
export async function openCheckInsByThread({ salesRepId, client = db } = {}) {
  const rows = await client.salesCheckIn.findMany({
    ...
    select: { toE164: true, scheduledFor: true },
  });
  const byThread = new Map();
  for (const r of rows) {
    const entry = byThread.get(r.toE164) || { count: 0, nextDue: null };
    entry.count += 1;
...
export async function createCheckIn({
  ...
  try {
    ...
    if (err?.code === "P2002" && dedupeKey) {
      ...
      if (existing) return { ok: true, checkIn: existing, existed: true };
    }
    throw err;
  }
}
