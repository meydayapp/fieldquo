 ...
 */
/**
 * The two draft sequences that are not company signals but still have a
 * "why" to print under the draft: the opened-and-unfinished signup nudge
 * (unfinishedSignup.js) and the link that went nowhere (linkNoSignup.js).
 * Listed here rather than in REASON_CODES because they are not decided by
 * decideCheckIn() and must not become company signals by accident.
 */
export const SEQUENCE_REASON_CODES = Object.freeze(["signup_unfinished", "link_no_signup"]);

export function checkinHeadlineKey(code) {
  return REASON_CODES.includes(code) || SEQUENCE_REASON_CODES.includes(code) ? `app.salesCheckin.reason.${code}` : null;
}

/** Every key checkinHeadlineKey() can return. Asserted against the catalogue. */
export const CHECKIN_HEADLINE_KEYS = [...REASON_CODES, ...SEQUENCE_REASON_CODES].map(
...
export function setupFacts(steps) {
  ...
  return {
    ...
    // A reworded title carries its figures in titleParams ("… ({n})").
    remainingTitles: remaining
      .slice(0, 2)
      .map((s) =>
        String(s.title || s.key || "").replace(/\{(\w+)\}/g, (m, k) =>
          s.titleParams && s.titleParams[k] != null ? String(s.titleParams[k]) : m,
        ),
      )
      .filter(Boolean),
