...
export async function materialiseUnfinishedSignupsForRep({ salesRepId, client, now = new Date(), origin = null, dryRun = false } = {}) {
  ...
  const rep = await client.salesRep.findUnique({ where: { id: salesRepId }, select: { id: true, name: true, workName: true, code: true, referralToken: true } });
  if (!rep) return out;
  ...
  for (const row of rows) {
    ...
    // The opaque referral token, minted on first use — never the legacy
    // name slug in SalesRep.code (lib/sales/repLink.js). Minted even on a
    // dry run: it is the rep's permanent link, not a draft, and a preview
    // built from the name slug would show a link that is never sent.
    if (origin && !rep.referralToken) rep.referralToken = await ensureReferralToken(rep, client);
    if (!origin || !rep.referralToken) {
      out.skipped.push({ progressId: row.id, reason: origin ? "no_rep_code" : "no_origin" });
      continue;
    }
    const link = signupLinkFor(origin, rep.referralToken, { linkToken: row.token });
    const language = unfinishedSignupLanguage(lead);
    const draftText = unfinishedSignupDraft({ language, repName: repPublicName(rep), businessName: lead.businessName, link, touchpoint: due.key });
