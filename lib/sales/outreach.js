...
import { bodyHtml } from "./emailFormat";
 ...
 */
/**
 * @param replyDomain  SALES_REPLY_DOMAIN, when set — a domain Resend RECEIVES
 *   for (docs/SALES-OUTREACH.md §5b). The Reply-To then keeps the rep's local
 *   part and swaps the domain: `emilio+fqs<token>@reply.fieldquo.com`. The
 *   From is untouched — it is still the rep's real address — so the prospect
 *   sees a person; only the reply is routed through us, filed, and forwarded
 *   on to that same real mailbox. This is the door that exists because the
 *   reps' mailbox provider cannot forward to a webhook; without it "plus"
 *   mode depends on the provider honouring sub-addressing, and with it that
 *   question goes away, since Resend receives for any address at the domain.
 *
 *   Unset, everything above about the rep's own mailbox still holds.
 */
export function replyToAddress(repEmail, replyToken, mode, replyDomain) {
  ...
  // A reply domain that is SET but not a domain produces no address at all,
  // never a quiet fall-back to the rep's own mailbox: that fall-back would
  // send every reply somewhere nothing files it, on a deployment whose
  // operator believes the opposite. Readiness refuses first; this is the
  // second lock.
  const wantedDomain = String(replyDomain || "").trim();
  const domain = wantedDomain ? emailDomain(`x@${wantedDomain}`) : email.slice(at + 1);
  if (!domain) return null;

  if (mode === "plain") return `${local}@${domain}`;
...
export function caslFooterLines({ rep, mailingAddress, replyToken }) {
  // The name the prospect knows the rep by — their work name, else their
  // first name — never the full real name (lib/sales/repIdentity.js).
  const name = sanitiseHeaderText(repPublicName(rep), 120);
...
export function buildOutboundEmail({ rep, subject, body, replyToken, mailingAddress }) {
  ...
  if (!rep?.email || !repPublicName(rep)) throw new Error("buildOutboundEmail needs the rep's name and email");
  ...
  // The HTML part is derived from the typed text by lib/sales/emailFormat.js
  // — paragraphs as before, plus the four marks a rep may use (bold, italic,
  // a bulleted list, a link). The text part stays the string the rep typed:
  // the marks read fine as text, and the stored copy IS that string.
  const paragraphs = bodyHtml(cleanBody);
...
export function visibleReplyText(body) {
  // One splitter for three readers — this, the inbox's collapsed quote, and
  // the reply that quotes a stored message. lib/sales/emailQuote.js.
  return splitQuoted(sanitiseBodyText(body)).visible;
}
...
export function parseInboundEmail(payload = {}) {
  ...
  // Cc is stored, not searched: it is NOT one of extractReplyToken's
  // haystacks, so a token planted in a Cc files nothing.
  const cc = sanitiseHeaderText(p.cc ?? p.Cc ?? p.CC, 2000);
...
/** The most a lead search string is allowed to be. Same ceiling as the inbox. */
export const LEAD_SEARCH_MAX = 120;

/** A search string, trimmed and bounded; "" for anything unusable. */
export function leadSearchTerm(q) {
  if (typeof q !== "string") return "";
  return q.replace(/\s+/g, " ").trim().slice(0, LEAD_SEARCH_MAX);
}

/** The digits of a search string, or "" — three digits is the least that
 *  reads as part of a phone number rather than a street number in a name. */
function searchDigits(term) {
  const digits = term.replace(/\D+/g, "");
  return digits.length >= 3 ? digits : "";
}

/**
 * The WHERE a lead search adds — the same fields the inbox picker matches
 * (lib/sales/emailInbox.js searchWhere): business name, contact name, email,
 * plus the phone and the city, which a rep scanning a pipeline of two hundred
 * roofers reaches for as often as a name. Case-insensitive `contains`; a rep's
 * book is hundreds of rows, not millions.
 *
 * Phones are matched on DIGITS. The E.164 columns (the prospect's number, the
 * lead's extra numbers) hold digits contiguously, so `contains` works there
 * directly. SalesLead.phone is free text the rep typed — "(514) 555-1234" — and
 * Postgres cannot strip its punctuation inside a Prisma where, so a query with
 * digits in it admits every lead that HAS a phone and leaves the digit match to
 * leadMatchesSearch() on the way out. The list is capped at 500 rows per rep;
 * the second pass is cheap, and it is the only way the formatted column can be
 * searched at all without a raw query — which no route under /api/sales may
 * build (scripts/check-sales-outreach.mjs, section 8).
 *
 * Returns null for a blank query so the caller spreads nothing.
 */
export function leadSearchWhere(q) {
  const term = leadSearchTerm(q);
  if (!term) return null;
  ...
  return {
    OR: [
      ...
      { email: contains },
      { phone: contains },
      { prospect: { city: contains } },
      { prospect: { businessName: contains } },
      ...(digits
        ? [
            { prospect: { phoneE164: { contains: digits } } },
            { contactNumbers: { some: { e164: { contains: digits } } } },
            { phone: { not: null } },
          ]
        : []),
    ],
...
/**
 * Does this lead match the search, as a person reading the row would judge it?
 * The second half of leadSearchWhere(): the same fields, with every phone
 * compared digit-for-digit. Pure, and what the route filters the page by.
 *
 * @param lead { businessName, contactName, email, phone,
 *               prospect?: { city, businessName, phoneE164 },
 *               contactNumbers?: [{ e164 }] }
 */
export function leadMatchesSearch(lead, q) {
  const term = leadSearchTerm(q);
  if (!term) return true;
  ...
  if (texts.some((v) => typeof v === "string" && v.toLowerCase().includes(needle))) return true;
  const digits = searchDigits(term);
  if (!digits) return false;
  const phones = [lead.phone, lead.prospect?.phoneE164]
    .concat((Array.isArray(lead.contactNumbers) ? lead.contactNumbers : []).map((n) => n?.e164))
...
/**
 * "Emailed, no reply": the rep (or the intro sender) has emailed this lead and
 * nothing has come back SINCE — the last word in the conversation is ours.
 *
 * Not "never replied": a lead who answered in June and was emailed again in
 * September with silence since is exactly the row a rep wants on this chip. So
 * the latest outbound (an intro email's sentAt, or a thread's latest "out"
 * message) is compared with the latest inbound (every thread's lastInboundAt),
 * and the lead qualifies when there is an outbound and no inbound after it.
 *
 * Pure; decided in JS after the query because Prisma cannot compare one
 * relation's timestamp with another's inside a where.
 *
 * @param lead { introEmails?: [{ sentAt }],
 *               threads?: [{ lastInboundAt, messages?: [{ sentAt }] }] }
 *              — `messages` holding the thread's latest OUTBOUND message only.
 */
export function leadEmailedNoReply(lead) {
  if (!lead || typeof lead !== "object") return false;
  const at = (v) => {
    const n = v instanceof Date ? v.getTime() : new Date(v || 0).getTime();
    return Number.isFinite(n) && n > 0 ? n : 0;
  };
  const intros = Array.isArray(lead.introEmails) ? lead.introEmails : [];
  const threads = Array.isArray(lead.threads) ? lead.threads : [];
  let lastOut = 0;
  let lastIn = 0;
  for (const intro of intros) lastOut = Math.max(lastOut, at(intro?.sentAt));
  for (const thread of threads) {
    lastIn = Math.max(lastIn, at(thread?.lastInboundAt));
    const outs = Array.isArray(thread?.messages) ? thread.messages : [];
    for (const m of outs) lastOut = Math.max(lastOut, at(m?.sentAt));
  }
  return lastOut > 0 && lastIn < lastOut;
}

/**
 * Every lead of this rep's, optionally at one status, optionally matching a
 * search. The search half is widened on purpose (see leadSearchWhere) and the
 * route narrows the rows it gets back with leadMatchesSearch().
 */
export function leadListWhere(salesRepId, status, q) {
