...
import { pointRepAtOwnDemo, unpointRepFromOwnDemo } from "./demoAssign";
...
export async function repDemoState(rep, client = db) {
  ...
  return {
    demos: demos.map((d) => ({
      ...
      // The discovery trade whose three selling points the card lists —
      // null for an industry with none, and the card draws no list.
      pitchTrade: INDUSTRIES[d.demoIndustry]?.pitchTrade || null,
...
export async function ensureRepDemo({
  ...
  for (let generation = priorCount + 1; generation <= priorCount + MAX_GENERATIONS; generation++) {
    ...
    try {
      company = await client.company.create({
        data: {
          name: repDemoName(key, repPublicName(rep)),
          slug,
          bookingSlug: slug,
  ...
  company = await client.company.update({
    where: { id: company.id },
    data: { name: repDemoName(key, repPublicName(rep)) },
    select: DEMO_SELECT,
  });
...
/**
 * Retire this demo and seed nothing in its place — the owner's "they should
 * have the ability to delete them if they are not using them". Deletes
 * nothing: the company row is kept, retired, exactly as a reset retires it;
 * the trade goes back on the "add a demo" list. If it was the demo the rep is
 * pointed at, the pointer moves to another live demo of theirs when one
 * exists, else clears.
 */
export async function retireRepDemo({ rep, companyId, client = db, now = new Date() } = {}) {
  const company = await client.company.findUnique({ where: { id: companyId }, select: DEMO_SELECT });
  if (!isOwnLiveDemo(company, rep?.id)) {
    return { ok: false, status: 404, error: "That is not one of your demo companies." };
  }
  ...
  if (rep.demoCompanyId === company.id) {
    const others = (await listRepDemos(rep.id, client)).filter((d) => d.id !== company.id);
    if (others.length) {
      const opened = await openRepDemo({ rep: { ...rep, demoCompanyId: null }, companyId: others[0].id, client });
      if (!opened.ok) return opened;
    } else {
      const cleared = await unpointRepFromOwnDemo({ repId: rep.id, companyId: company.id }, client);
      if (!cleared.ok) return cleared;
    }
  }
  return { ok: true, retired: company };
}
...
export async function ensureRepDemoLogin({ rep, password, client = db, signUp, createOrg } = {}) {
  ...
  const userId = await mintDemoUser({ email, name: repPublicName(rep) || rep.name, password, signUp });
