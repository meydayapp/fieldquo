...
//
// The rule itself lives in ./contactBasis.js, which imports nothing. Its
// header says why: this file reaches lib/db.js through toE164, and the screen
// that has to render "email is closed on this record" is a client component.
// Re-exported here so a caller that already imports the suppression rules gets
// the consent rule from the same place, and there is one definition of both.
import { contactBasisFor } from "./contactBasis";

export { CONTACT_BASIS_STATES, SOURCE_CONTACT_BASIS, contactBasisFor } from "./contactBasis";

/** Every channel a source's provenance closes outright. */
export function prohibitedChannelsFor(sourceProvider) {
  return SUPPRESSION_CHANNELS.filter((c) => contactBasisFor(sourceProvider, c).state === "prohibited");
}
 ...
 * @param sourceProvider  Prospect.sourceProvider, when the caller knows where
 *        this contact came from. Optional, and omitting it changes nothing —
 *        every existing caller predates this and none of them regressed. When
 *        it IS supplied, a source whose provenance prohibits the channel
 *        refuses here, before the list is even consulted, because no absence
 *        of an opt-out makes an unlawful message lawful.
 * @returns { suppressed, hit, reason, basis }
 */
export function suppressionVerdict({ rows = [], channel, sourceProvider = null } = {}) {
  ...
  // Provenance first. A prospect whose address came from a licence register
  // has not opted out — there was never anything to opt out OF — so the list
  // is silent about them, and a list-only verdict would answer "not
  // suppressed" to a message CASL does not permit at all.
  const basis = sourceProvider ? contactBasisFor(sourceProvider, channel) : null;
  if (basis?.state === "prohibited") {
    return { suppressed: true, hit: null, reason: basis.reason, basis };
  }

  const live = rows.filter(
    (r) => r && !r.removedAt && Array.isArray(r.channels) && r.channels.includes(channel),
  );
  if (!live.length) return { suppressed: false, hit: null, reason: null, basis };
  ...
  return {
    suppressed: true,
    hit,
    reason: describeSuppression(hit),
    ...suppressionNotice(hit),
    basis,
  };
}
...
// ── The same sentence, as something a screen can say in the rep's language ──
//
// describeSuppression() above is unchanged and still English, because two
// callers need it that way: scripts/check-sales-suppression.mjs asserts its
// exact wording, and `suppressionRefusal()` puts it in an API `error` body
// where there is no translator and no reader's language to consult.
//
// What a SCREEN needs is different. /sales/leads/[id] and /sales/threads/[id]
// render this verdict to a rep who may be reading the portal in any of nine
// languages, and until now they rendered the English. The route knows which of
// seven sources closed the channel — that is the whole reason the sentence is
// composed server-side rather than fixed on the screen — so what travels to
// the browser is that KNOWLEDGE, as a catalogue key plus its values, and the
// screen resolves it against the rep's own catalogue.
//
// ── Why a whole key per (kind × source) and not a stem plus a clause ───────
//
// Sixteen keys rather than one sentence with `{how}` slotted into it. The
// slotted version is the defect this repository has now shipped twice: an
// English fragment glued to a translated one, producing a sentence half in
// each. "Everyone at acme.com is on the list" and "bob@acme.com is on the
// list" are also not one sentence with a swapped subject — the verb agrees
// with the subject in five of these languages, and the domain form is plural
// in some and singular in others. The sentence is the unit.
//
// The DATE is a placeholder, and that is not the same thing: a date is a
// value, not a clause. A row with no requestedAt cannot be produced by
// suppress() — it always writes one — but the column is nullable, so the
// placeholder gets a translated "not recorded" token rather than the word
// "null" or an invented day.

/** The `how` half of the key, for a row whose source we may not recognise. */
const NOTICE_SOURCE = new Set(["reply", "call", "sms", "form", "manual", "import", "regulator"]);

/**
 * The suppression sentence as a catalogue key and its values.
 *
 * @returns {{reasonKey:string, reasonParams:{value:string, date:string|null}}}
 *          or `{}` for no row at all, so it spreads into a verdict cleanly.
 */
export function suppressionNotice(row) {
  if (!row) return {};
  const who = row.kind === "domain" ? "domain" : "person";
  const source = NOTICE_SOURCE.has(row.source) ? row.source : "unrecorded";
  const when = row.requestedAt ? new Date(row.requestedAt) : null;
  return {
    reasonKey: `app.salesSuppression.reason.${who}.${source}`,
    reasonParams: {
      value: String(row.value ?? ""),
      // ISO, not a localised date: this is a record date on a compliance
      // notice, and the one format that means the same thing in every
      // language is the one the row itself is stored in.
      date: when && !Number.isNaN(when.getTime()) ? when.toISOString().slice(0, 10) : null,
    },
  };
}

/** Every key suppressionNotice() can name, so a check can assert all of them. */
export const SUPPRESSION_NOTICE_KEYS = ["person", "domain"].flatMap((who) =>
  [...NOTICE_SOURCE, "unrecorded"].map((source) => `app.salesSuppression.reason.${who}.${source}`),
);
