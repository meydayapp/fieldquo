...
export async function loadRepFunnels({ repIds = null, monthKey = null, now = new Date() } = {}) {
  ...
  const [attempts, linkSends, attributions] = await Promise.all([
    db.salesCallAttempt.findMany({
      // Test dials (lib/sales/testLines.js) are not a stage of anybody's funnel.
      where: excludingTestDials({ salesRepId: { in: ids }, direction: "out", dialledAt: { gte: bounds.start, lt: bounds.end } }),
  ...
  const funnels = reps
    .map((rep, i) => (wanted && !wanted.has(rep.id) ? null : buildRepFunnel({ rep: { ...rep, agency: agencyOf(rep) }, counts: counts[i], monthKey: key, references })))
    .filter(Boolean);
