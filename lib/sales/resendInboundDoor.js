...
// also no_token / unknown_token — a prospect who wrote to a rep's reply
// address is a prospect who wrote to the rep, whatever happened to our
// routing label. It is skipped for own_outbound (the rep's own message) and
...
const FORWARDED_OUTCOMES = new Set(["filed", "no_token", "unknown_token"]);
...
export async function processReceivedEvent(event, deps) {
  ...
  const ingest = await ingestInboundEmail(db, contract, { source: "resend", recordError });
