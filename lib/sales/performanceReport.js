...
import { reconciliationLine } from "./calls/dialTable";
import { buildCallQuality } from "./callQuality";
import { agencyOf } from "./agencyLabel";
import { onlyProspectDials } from "./testLines";
 ...
 */
export function buildCallActivity({ reps = [], attempts = [], from = null, to = null, now = new Date(), carrierLegs = null, scoped = false } = {}) {
  const rows = teamCallRows({ reps, attempts, activity: [], presence: [], from, to, now });
  const clean = onlyProspectDials(attempts);
  const a = from ? new Date(from) : null;
  const b = to ? new Date(to) : null;
  const inPeriod = (row) => {
    const d = row?.dialledAt ? new Date(row.dialledAt) : null;
    if (!d || Number.isNaN(d.getTime())) return false;
    if (a && d < a) return false;
    if (b && d > b) return false;
    return true;
  };
  const periodRows = clean.filter(inPeriod);
  // The table the owner reads (lib/sales/calls/dialTable.js): one
  // denominator, by attempt joined to the carrier by the child sid. It is
  // repCallStats's own `table` — composed ONCE, in reporting.js, over the
  // rows the rep placed in the period — and only aliased to the row here,
  // because app/components/sales/CallPerformanceSections.js reads `r.table`
  // and the floor board reads `stats.table`, and the two must be the same
  // object rather than two calls that happen to agree today.
  for (const row of rows) row.table = row.stats.table;
  ...
  const agencyRows = [...agencies.values()].map((ag) => {
    const stats = repCallStats({ attempts: clean.filter((a) => ag.repIds.includes(a?.salesRepId)), from, to, now });
    return { id: ag.id, name: ag.name, reps: ag.repIds.length, stats, table: stats.table };
  });

  const everyone = periodRows.filter((x) => reps.some((r) => r.id === x?.salesRepId));
  const total = repCallStats({ attempts: clean.filter((a) => reps.some((r) => r.id === a?.salesRepId)), from, to, now });
  return {
    reps: rows,
    agencies: agencyRows,
    total,
    totalTable: total.table,
    // Twilio's count of prospect legs in the period against FieldQuo's
    // attempts. null when the carrier was not asked (the page says so),
    // never a line that pretends the two agree.
    // A SCOPED report (an agency's team) compares only the carrier legs its
    // own attempts join to: a Twilio leg carries no rep, so a leg no team
    // attempt claims cannot be said to be the team's — and the floor-wide
    // count of unjoined legs is another rep's activity, which an agency
    // must not see. The platform's unscoped view keeps the full comparison.
    reconciliation:
      carrierLegs && !carrierLegs.error
        ? reconciliationLine({
            legs: scoped ? carrierLegs.legs.filter((l) => everyone.some((a) => a?.providerCallSid === l?.sid)) : carrierLegs.legs,
            attempts: everyone,
            listedAll: carrierLegs.listedAll,
          })
        : null,
    carrierError: carrierLegs?.error || null,
...
export function buildPerformanceReport({
  ...
  now = new Date(),
  repIds = null,
  // { legs, listedAll, error } from lib/sales/calls/reconcileProvider.js
  // listProspectLegs, or null when the carrier was not asked.
  carrierLegs = null,
} = {}) {
  ...
  return {
    ...report,
    scope: { repIds: scope },
    calls: buildCallActivity({ reps: scopedReps, attempts: scopedAttempts, from, to, now, carrierLegs, scoped: scope !== null }),
