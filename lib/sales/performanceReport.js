...
import { dialTableRow, reconciliationLine } from "./calls/dialTable";
...
export function buildCallActivity({ reps = [], attempts = [], from = null, to = null, now = new Date(), carrierLegs = null, scoped = false } = {}) {
  ...
  // denominator, by attempt joined to the carrier by the child sid. Rides
  // beside repCallStats on every row so the floor board's figures and this
  // table come from the same rows.
  for (const row of rows) row.table = dialTableRow(periodRows.filter((x) => x?.salesRepId === row.id), { now });
  ...
  const agencyRows = [...agencies.values()].map((ag) => ({
    ...
    table: dialTableRow(periodRows.filter((x) => ag.repIds.includes(x?.salesRepId)), { now }),
  }));
  ...
  return {
    ...
    totalTable: dialTableRow(everyone, { now }),
