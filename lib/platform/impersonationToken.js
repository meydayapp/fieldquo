...
export async function verifyImpersonationToken(token) {
  ...
  try {
    ...
    return {
      ...
      mode: payload.mode,
      // Unix seconds, straight off the JWT. Dropping this is why the banner
      // counted down from a hardcoded 30:00 that never moved — QA polled the
      // status endpoint twice, three seconds apart, and got 1800 both times,
      // which reads as "this support session never expires" over a customer's
      // data. It does expire; jwtVerify above rejects it. Only the number
      // shown was a lie, and now it comes from the same claim that enforces it.
