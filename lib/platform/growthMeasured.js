...
//                a conversion, the rest are not — plus every card-free trial
//                that ran out with no plan (no Subscription row at all), which
//                ended and did not convert. Paying and trialing TODAY (the
//                starting stock) are the dashboard's buckets, from
...
// not a dial the growth model may learn from. Every attempt count below
// carries the exclusion — the raw SQL as a clause, the Prisma counts as a
// where fragment — so a night of the owner testing the line does not become
// a reach rate.
import { Prisma } from "@prisma/client";
import { TEST_DIAL_SQL_EXCLUSION, prospectDialsOnly } from "@/lib/sales/testLines";

const notTest = Prisma.raw(TEST_DIAL_SQL_EXCLUSION);
...
export async function measureGrowth({ assumptions = {}, now = new Date() } = {}) {
  ...
  const [
    ...
    db.salesCallAttempt.count({ where: prospectDialsOnly({ direction: "out", disposition: { not: null } }) }),
    db.salesCallAttempt.count({ where: prospectDialsOnly({ direction: "out", disposition: { in: reachedCodes } }) }),
    ...
    // Paying and trialing TODAY — the forecast's starting stock — are the
    // dashboard's buckets (lib/platform/trialCounting.js), not two queries of
    // their own. They were: paying required billingStartedAt, so the one
    // company Stripe was actually billing read as nobody, and trialing was
    // Stripe's trialing rows alone, so every card-free trial (38d3308d on)
    // was missing from the pipeline the forecast converts.
    loadSubscriberBook(db, { now }),
    db.salesRep.count({ where: { active: true } }),
    db.prospect.count(),
    db.salesCallAttempt.count({ where: prospectDialsOnly({ direction: "out", dialledAt: { gte: thirtyDaysAgo } }) }),
    db.company.findMany({
    ...
    Promise.all(months.map((m) => db.salesCallAttempt.count({ where: prospectDialsOnly({ direction: "out", dialledAt: { gte: m.start, lt: m.end } }) }))),
    Promise.all(months.map((m) => db.salesCallAttempt.count({ where: prospectDialsOnly({ direction: "out", disposition: { in: reachedCodes }, dialledAt: { gte: m.start, lt: m.end } }) }))),
    db.salesCallAttempt.count({ where: prospectDialsOnly({ direction: "out", dialledAt: { gte: current.start, lt: current.end } }) }),
    db.salesCallAttempt.count({ where: prospectDialsOnly({ direction: "out", disposition: { in: reachedCodes }, dialledAt: { gte: current.start, lt: current.end } }) }),
    ...
    db.$queryRaw`
      WITH d AS (
        ...
        WHERE direction = 'out' AND disposition IS NOT NULL AND "prospectId" IS NOT NULL AND ${notTest}
      )
    ...
    db.salesCallAttempt.findMany({
      where: prospectDialsOnly({ direction: "out", disposition: AGREED_CODE }),
  ...
  // The forecast's starting stock, from the dashboard's own buckets.
  const payingNow = book.tally.billed;
  const trialingNow = book.tally.trialing.total;
  // A card-free trial that ran out with no plan chosen is a trial that ENDED
  // and did not convert — the same outcome as a Stripe trial that ended
  // unbilled, and invisible to the subscription counts above because it has
  // no Subscription row. Left out, the conversion rate would be measured over
  // the card-backed trials alone and read high by exactly the people who
  // walked away.
  const cardFreeEnded = book.companies.filter(
    (c) => c.subscription === null && (c.bucket === "trial_ended" || c.bucket === "locked"),
  ).length;
  ...
  const rates = {
    ...
    conversion: rate({ hit: trialsConverted, of: trialsEnded + cardFreeEnded, floor: FLOORS.conversion, assumed: assumptions.conversion ?? null }),
    churn: monthsRate({
