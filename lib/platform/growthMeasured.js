...
//                signup path records today. A paid-ad signup carries no
//                campaign tag yet (SignupOrigin knows the door, not the
//                advert), so it is counted as ORGANIC, and the page says so
//                rather than letting the marketing row imply that ads are
//                being measured. "tester" codes are not marketing.
//
...
import { rate, monthsRate, monthlyCount, monthLabel, marketingAssumption, FLOORS } from "@/lib/platform/growthModel";
...
export async function measureGrowth({ assumptions = {}, now = new Date() } = {}) {
  ...
  const bucket = (m, dials, reachedCount) => {
    ...
    const marketing = created.filter((c) => !c.referredByCode && !viaRep.has(c.id) && viaCampaign.has(c.id)).length;
    const organic = created.length - referral - rep - marketing;
    ...
    return {
      ...
      signups: { rep, referral, marketing, organic, total: created.length },
