...
import { TEST_DIAL_SQL_EXCLUSION, excludingTestDials } from "@/lib/sales/testLines";
...
export async function measureGrowth({ assumptions = {}, now = new Date() } = {}) {
  ...
  const [
    ...
    db.salesCallAttempt.count({ where: excludingTestDials({ direction: "out", disposition: { not: null } }) }),
    db.salesCallAttempt.count({ where: excludingTestDials({ direction: "out", disposition: { in: reachedCodes } }) }),
    ...
    db.prospect.count(),
    db.salesCallAttempt.count({ where: excludingTestDials({ direction: "out", dialledAt: { gte: thirtyDaysAgo } }) }),
    db.company.findMany({
    ...
    Promise.all(months.map((m) => db.salesCallAttempt.count({ where: excludingTestDials({ direction: "out", dialledAt: { gte: m.start, lt: m.end } }) }))),
    Promise.all(months.map((m) => db.salesCallAttempt.count({ where: excludingTestDials({ direction: "out", disposition: { in: reachedCodes }, dialledAt: { gte: m.start, lt: m.end } }) }))),
    db.salesCallAttempt.count({ where: excludingTestDials({ direction: "out", dialledAt: { gte: current.start, lt: current.end } }) }),
    db.salesCallAttempt.count({ where: excludingTestDials({ direction: "out", disposition: { in: reachedCodes }, dialledAt: { gte: current.start, lt: current.end } }) }),
    ...
    db.salesCallAttempt.findMany({
      where: excludingTestDials({ direction: "out", disposition: AGREED_CODE }),
