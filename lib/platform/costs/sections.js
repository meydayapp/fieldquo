...
export function buildSections({ twilio, openai, openaiBilled, retell, places, localScrape, apify, neon, stripe, charged = { byKind: {} }, fixed = { byProvider: [] }, now = new Date() } = {}) {
  ...
  // The Places API line is history: the path was retired 2026-09-20 and the
  // caller passes `places` only for a period that holds its rows.
  if (places) {
    sales.push(line({
      key: "google_places",
      label: "Google Places API — retired 2026-09-20",
      cents: places.cents,
      provider: "google_places",
      sourceKind: "computed",
      source: places.source,
      asOf: places.asOf || now,
      note: `${Number(places.requests || 0).toLocaleString("en-CA")} requests at list price before the key was blocked · nothing calls it now`,
  ...
  if (localScrape) {
    sales.push(line({
      ...
      note: `${Number(localScrape.places || 0).toLocaleString("en-CA")} places at $0 — the same fields the Places API charged $35/1,000 for`,
  ...
  if (openai?.tenants) {
    ...
    // Everything the AI wallet is debited for: the priced image actions, and
    // since 2026-09-25 the AI employee's replies and front desk, charged per
    // call at cost × lib/ai/imageEconomics.js's multiplier.
    const imageCharged = chargedOf("image_generation", "image_vision", "material_list", "ai_employee_reply", "ai_employee_front_desk");
    companies.push(line({
      key: "openai:tenants",
      label: "OpenAI — companies' AI (AI employee, digests, reviews, images)",
      cents,
      provider: "openai",
      sourceKind: "computed",
      ...
      charged: { ...chargedBlock(imageCharged, "kinds image_generation + image_vision + material_list + ai_employee_reply + ai_employee_front_desk from the AI wallet; the other text features are inside the plan's monthly allowance and carry no per-call charge"), inPlan: true },
    }));
  }
