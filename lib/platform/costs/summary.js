...
//   Google Places — historical only
//             PlatformCostDaily rows the retired Places API path wrote
//             (provider google_places): 119 requests on 2026-09-18, at list
//             price, before Google blocked the key and the owner retired
//             the sweep (lib/sales/intel/places.js's header). Nothing
//             writes this provider any more; the rows are the record of
//             what it cost, so a period that contains them still prints
//             the line, and a period that does not prints nothing.
//   Google Maps — read from the Mac
//             PlatformCostDaily provider local_scrape: places the owner's
//             Mac read, at $0, metered by lib/sales/intel/listings.js. The
//             Google path now.
//
//   OpenAI, billed
...
import { tierForModel } from "@/lib/ai/provider";

/**
 * Which product area an AiUsage feature name belongs to.
 *
 * Feature names are the ones recordAiUsage was given — "ai_employee_reply",
 * "quote_review", "quote_review_photos", "voice_auto_draft", "copilot" — and
 * the area is a prefix match on the closed list below. Anything unmatched is
 * "other", printed as such rather than folded into a neighbour.
 */
export function aiAreaOf(feature) {
  const f = String(feature || "");
  if (f.startsWith("ai_employee")) return "employee";
  if (f.startsWith("quote_review") || f.startsWith("conversation_review") || f.startsWith("conversation_temperature") || f.startsWith("conversation_coach")) return "review";
  if (f.startsWith("voice") || f.startsWith("call_") || f.startsWith("monthly_digest_calls")) return "voice";
  if (f.startsWith("copilot")) return "copilot";
  if (f.startsWith("website") || f.startsWith("funnel") || f.startsWith("marketing")) return "marketing";
  if (f.startsWith("jennifer")) return "support";
  return "other";
}
...
const PLACES_SOURCE = "PlatformCostDaily google_places — Text Search requests the retired API path metered at list price (US$35/1,000, Enterprise SKU) before 2026-09-20; nothing writes this provider now, the rows are history";
...
export async function platformCostSummary({ from, to, granularity = "day", client = db, now = new Date() } = {}) {
  ...
  // ── Google Places — the retired API's rows, when the period has any ────
  ...
  // No rows in the period → no line. The sweep was retired 2026-09-20 and
  // a "$0.00 · 0 requests" line every month after would read as a live
  // provider that happened to be idle.
  const placesLine = placesRows.length ? places : null;
  ...
  const tenantAi = await client.$queryRaw`
    SELECT date_trunc('day', "createdAt") AS day, "feature", "model",
           COALESCE(SUM("costMicros"), 0)::bigint AS micros,
           COUNT(*)::int AS calls,
  ...
  // ── By TIER and by AREA, for the employee's spend beside the rest ───────
  //
  // The AI employee runs on the best model (lib/ai/provider.js's tier), the
  // review and the copilot on the mini. "What does the best tier cost us" and
  // "what does the employee cost against review and voice" are the two
  // questions the owner asked; both are derived from columns AiUsage already
  // carries — the tier from the model id, the area from the feature name —
  // so nothing had to be written at record time to answer them.
  const byTier = new Map();
  const tenantByArea = new Map();
  let tenantMicros = 0;
  for (const row of tenantAi) {
    ...
    const tier = tierForModel(row.model);
    const tierRow = byTier.get(tier) || { tier, micros: 0, calls: 0, models: new Set() };
    tierRow.micros += micros;
    tierRow.calls += calls;
    tierRow.models.add(row.model);
    byTier.set(tier, tierRow);

    const area = aiAreaOf(row.feature);
    const areaRow = tenantByArea.get(area) || { area, micros: 0, calls: 0 };
    areaRow.micros += micros;
    areaRow.calls += calls;
    tenantByArea.set(area, areaRow);
  ...
  const openai = {
    ...
    tenants: {
      ...
      byArea: [...tenantByArea.values()].sort((a, b) => b.micros - a.micros),
      micros: tenantMicros,
  ...
  // database; conversation.js reads the count off the row. The expression
  // is lib/sales/calls/contractorWordsSql.js's, shared with the performance
  // page. It used to be written inline here, and inside a template literal
  // its `'\s+'` cooked to `'s+'` — the count was of chunks between the
  // letter s, not between spaces. A shared fragment in a plain string is
  ...
  const attempts = await client.$queryRaw(Prisma.sql`
    SELECT a."id", a."salesRepId", a."direction", a."dialChannel", a."dialSource", a."dialledAt", a."endedAt",
           ...
           ${CONTRACTOR_WORDS_SQL} AS "contractorWords"
  ...
  const split = buildSections({ twilio, openai, openaiBilled, retell, places: placesLine, localScrape, apify, neon, stripe, charged, fixed, now });
  ...
  const totals = [
    ...
    ...(placesLine ? [{ key: "google_places", label: "Google Places API — retired 2026-09-20", cents: places.cents, source: PLACES_SOURCE, asOf: now, note: `${places.requests} requests at list price before the key was blocked · nothing calls it now` }] : []),
    { key: "local_scrape", label: "Google Maps — read from the Mac", cents: 0, source: LOCAL_SCRAPE_SOURCE, asOf: now, note: `${localScrape.places} places at $0 · the same fields the Places API charged $35/1,000 for` },
  ...
  return {
    ...
    providers: {
      ...
      // `optional`: the owner has decided not to hold an org admin key, so
      // this row is a settled state, not a to-do. See DAILY_PROVIDERS.
      openai: { label: "OpenAI (billed)", configured: providerConfig.openai.configured, optional: providerConfig.openai.optional, envVar: providerConfig.openai.envVar, lastPullAt: lastPulls.openai || null, cadence: "daily, three days back", kind: "api" },
