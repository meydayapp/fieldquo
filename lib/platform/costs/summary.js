...
export async function platformCostSummary({ from, to, granularity = "day", client = db, now = new Date() } = {}) {
  ...
  const allInCents = r4(
    (twilio.totalCents ?? twilio.linesCents ?? 0) + (platformMicros + tenantMicros) / 10000 + (retell.cents || 0),
  );
  ...
  const signupsBlock = {
    ...
    allIn: {
      ...
      definition: "Twilio account total + OpenAI (platform and tenants) + Retell calls — everything FieldQuo paid these providers, whoever it was for",
