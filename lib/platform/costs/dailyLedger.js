...
//
// One Twilio account carries two businesses: FieldQuo's own sales floor
// (the reps' calls, their recordings, their texts to prospects, their
// numbers) and the tenants' crew and client texts, which leave from the
// shared system number and the crew lines. Twilio's total says nothing
// about which is which, so the split is made from what the ledger and the
// store DO tag, category by category, and says how:
//
//   every call category, recordings, transcription   sales — tenant voice
//                                                    is Retell, never Twilio
//   sms-outbound / sms-inbound                        by COUNT: the sales
//                                                    texts in the store
//                                                    (SalesSmsMessage) against
//                                                    Twilio's own message
//                                                    count for the category;
//                                                    the remainder is tenants'
//   phonenumbers                                      by count of numbers held:
//                                                    the platform's sales
//                                                    numbers against the crew
//                                                    lines
//   anything else, and "other" beyond the lines       not attributed
//
// A share is never invented: with no Twilio count for a category the
// category is left unattributed and the statement says so.

/** Categories that are the sales floor's alone. */
export const TWILIO_SALES_ONLY_CATEGORIES = Object.freeze(["calls-outbound", "calls-inbound", "calls-client", "recordings", "recordingstorage", "transcriptions"]);

const r4 = (n) => Math.round(n * 10000) / 10000;

/**
 * @param summary   summariseTwilio() output
 * @param counts    { salesSmsOut, salesSmsIn, salesNumbers, tenantNumbers } — integers from the store
 * @returns { sales: { cents, lines }, tenants: { cents, lines }, unattributed: { cents, lines }, method: string }
 */
export function splitTwilioSides(summary, counts = {}) {
  const sales = { cents: 0, lines: [] };
  const tenants = { cents: 0, lines: [] };
  const unattributed = { cents: 0, lines: [] };
  const share = (line, salesShare, how) => {
    const s = r4(line.cents * salesShare);
    const t = r4(line.cents - s);
    sales.cents = r4(sales.cents + s);
    tenants.cents = r4(tenants.cents + t);
    sales.lines.push({ category: line.category, cents: s, how });
    tenants.lines.push({ category: line.category, cents: t, how });
  };
  for (const line of summary?.lines || []) {
    if (TWILIO_SALES_ONLY_CATEGORIES.includes(line.category)) {
      share(line, 1, "Twilio voice is the sales floor; tenant voice is Retell");
      continue;
    }
    if (line.category === "sms-outbound" || line.category === "sms-inbound") {
      const storeCount = line.category === "sms-outbound" ? num(counts.salesSmsOut) : num(counts.salesSmsIn);
      const twilioCount = num(line.count);
      if (storeCount === null || twilioCount === null || twilioCount <= 0) {
        unattributed.cents = r4(unattributed.cents + line.cents);
        unattributed.lines.push({ category: line.category, cents: line.cents, how: "no message count to split by" });
        continue;
      }
      const salesShare = Math.min(1, storeCount / twilioCount);
      share(line, salesShare, `${Math.min(storeCount, twilioCount)} of Twilio's ${twilioCount} messages are in the sales store`);
      continue;
    }
    if (line.category === "phonenumbers") {
      const a = num(counts.salesNumbers);
      const b = num(counts.tenantNumbers);
      if (a === null || b === null || a + b <= 0) {
        unattributed.cents = r4(unattributed.cents + line.cents);
        unattributed.lines.push({ category: line.category, cents: line.cents, how: "no number counts to split by" });
        continue;
      }
      share(line, a / (a + b), `${a} sales numbers against ${b} crew lines held today`);
      continue;
    }
    unattributed.cents = r4(unattributed.cents + line.cents);
    unattributed.lines.push({ category: line.category, cents: line.cents, how: "not a category this split names" });
  }
  if (summary?.otherCents !== null && summary?.otherCents !== undefined && summary.otherCents !== 0) {
    unattributed.cents = r4(unattributed.cents + summary.otherCents);
    unattributed.lines.push({ category: "other", cents: summary.otherCents, how: "in Twilio's total, under no pulled category" });
  }
  return {
    sales,
    tenants,
    unattributed,
    method:
      "Calls, recordings and transcription are the sales floor's (tenant voice is Retell). Texts are split by count — the sales store's messages against Twilio's count for the category — and number rent by the numbers held today. What cannot be split is listed as not attributed, never spread.",
  };
}
