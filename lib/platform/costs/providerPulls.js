...
/**
 * One info line a day per optional provider, not one an hour. Per instance,
 * deliberately: a Vercel lambda is short-lived, so the worst case is one line
 * per cold start rather than a guarantee of exactly one — which is the right
 * trade for something that writes nothing and blocks nothing.
 */
const optionalFailureLoggedAt = new Map();
function noteOptionalFailure(provider, now) {
  const at = optionalFailureLoggedAt.get(provider) || 0;
  if (now.getTime() - at < 24 * 60 * 60 * 1000) return false;
  optionalFailureLoggedAt.set(provider, now.getTime());
  return true;
}
...
export const DAILY_PROVIDERS = Object.freeze({
  // `optional`: OpenAI's Costs endpoint needs an ORG-level admin key, which is
  // a different credential from OPENAI_API_KEY and cannot be derived from it.
  // The owner has decided not to hold one (2026-09-22), so this pull can never
  // succeed as configured — and a daily PlatformErrorLog row for a decision
  // already taken is noise on the one page that exists to show real failures.
  // Nothing is lost: FieldQuo's own metered spend (lib/ai/usage.js) is
  // unchanged and is what the costs page shows. The only thing the admin key
  // would add is the comparison against OpenAI's own invoice.
  openai: { label: "OpenAI", envVar: "OPENAI_ADMIN_API_KEY", source: OPENAI_COSTS_SOURCE, configured: openaiAdminConfigured, pull: pullOpenaiCosts, optional: true },
...
export function providerConfiguration() {
  const out = {};
  for (const [key, p] of Object.entries(DAILY_PROVIDERS)) out[key] = { configured: p.configured(), envVar: p.envVar, label: p.label, optional: Boolean(p.optional) };
  return out;
}
...
export async function pullProvider(provider, { from, to, client = db, now = new Date(), deps = {} } = {}) {
  ...
  try {
    ...
    const message = `${p.label} costs for ${startKey}..${endKey} could not be pulled: ${err?.message || err}`;
    if (p.optional) {
      // An optional provider's refusal is a CONFIGURATION fact, not an
      // incident: the console says so in words on /platform/costs and the
      // errors page stays a list of things that are actually broken. Logged
      // at info, and at most once a day — the cron asks every hour.
      if (noteOptionalFailure(provider, now)) console.info(`[platform costs] ${message} — optional; ${p.envVar} is not held, so FieldQuo's metered figures stand alone.`);
    } else {
      await recordError({
        area: "platform_costs",
    ...
    return { ok: false, provider, reason: p.optional ? "refused" : "failed", optional: Boolean(p.optional), envVar: p.envVar, error: err?.message || String(err), from: startKey, to: endKey, records: 0, rows: 0, written: 0, dropped: 0 };
  }
}
