...
export const SPOKEN_FEATURE_LINES = Object.freeze({
  ...
  // Sold on what it is allowed NOT to do. Every competitor's chatbot promises
  // to answer; what a contractor is actually afraid of is one that invents a
  // price or talks over them mid-negotiation, so the limits are the pitch.
  ai_employee:
    "An assistant that answers customer messages for you, in the job you give " +
    "it — a receptionist that books the callback, or a closer that can put a " +
    "price together from your own rates. It reads the policies and manuals you " +
    "upload, it can never invent a price or a date, and it stops the moment you " +
    "start typing. By default it writes you a draft rather than sending.",
  // The 24-hour rule is named on the phone, not buried. A contractor who finds
  // out about it from a failed send blames the software; one who was told
  // treats it as how WhatsApp works, which is what it is.
  whatsapp_messaging:
    "Your WhatsApp Business number answered from the same inbox as everything " +
    "else, with each conversation sitting against that customer's quotes and " +
    "jobs. WhatsApp's own rule is that you can write freely for 24 hours after " +
    "a customer messages you, and after that only an approved template — the " +
    "reply box tells you which you're in, so nothing bounces silently.",
  // Sold on the month-end read, not on "an inbox" — every tool has an inbox,
  // and what a contractor cannot get anywhere else is knowing which of those
  // conversations turned into paid work.
  page_messaging:
    "The messages people send your Facebook page and your Instagram account " +
    "come into the same place as everything else, so you answer them without " +
    "switching apps. At the end of the month you can see which of those " +
    "conversations turned into a job and which ones nobody ever replied to.",
  ai_copilot:
  ...
  ai_vision:
    "A closer, paid look at a quote's own photos — flags things like water " +
    "damage, a hairline crack or a bigger load than it looked, so nobody finds " +
    "out on site.",
  marketing_designer:
    "AI-generated marketing images for your ads and posts, made from a prompt " +
    "or from one of your own site photos.",
  kpi_dashboard:
    "One screen for win rate, margin, on-time completion and what you're " +
    "owed, rolled up from the jobs and invoices already in the account — " +
    "nothing to set up separately.",
  // Sold as the team's own room, because the question a contractor asks is
  // whether a customer can see it. They cannot: these are Member and Job rows.
  team_chat:
    "A chat for your own team: one room for everyone, a room for each active " +
    "job with the crew booked on it and the office, and direct messages. Your " +
    "customers never see it.",
  // The cost is named rather than hidden, the same as ai_vision — the build
  // draws on AI credits, and the plain rebuild from the quote stays free.
  ai_material_list:
    "A job's material list built from its approved quote, grouped so nothing " +
    "gets forgotten, with the reason for each line. The AI build uses your AI " +
    "credits; rebuilding the list straight from the quote is free.",
});
  ...
  const [plans, featureMap] = await Promise.all([
    db.plan.findMany({
      ...
      select: {
        ...
        stripePriceId: true,
        isPublic: true,
        // Same hazard: isRetired reads a missing column as "not retired", and
        // the sales line must not read out a plan nobody can buy.
        retiredAt: true,
      },
    }),
