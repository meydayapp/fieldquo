...
export async function changeSubscriptionPlan({ subscription, plan, interval = DEFAULT_INTERVAL, currency }) {
  ...
  await db.subscription.update({
    ...
    data: { planId: plan.id, billingInterval: interval },
  });
    ...
    case "customer.subscription.deleted": {
      ...
      await db.subscription.updateMany({
        ...
        data: {
          ...
          pastDueSince: null,
          // A plan change booked for the 1st has nothing to land on once the
          // subscription is gone. Stripe cancels the schedule too and sends
          // subscription_schedule.canceled, but the two events race and a
          // cancelled account showing "Switching to Solo on the 1st" is a
          // control that lies.
          ...CLEAR_PENDING,
        },
      });
