...
export async function createBillingCheckoutSession({
  company,
  plan,
  // Same cadence contract as the trial checkout above. Its only caller (the
  // Account & Billing "Choose plan" / Team "Add licenses" upgrade) has no
  // annual control on screen yet, so it passes nothing and gets monthly —
  // which is what that path has always created. The parameter is here so the
  // two builders share one rule rather than one of them owning a literal.
  interval = DEFAULT_INTERVAL,
