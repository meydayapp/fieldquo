...
export async function syncSubscriptionFromStripeEvent(event) {
  ...
  switch (event.type) {
    ...
    case "invoice.payment_succeeded": {
      ...
      // ── Sales milestone 2 ─────────────────────────────────────────────
      //
      // The FIRST real payment, and the two conditions are both load-bearing.
      //
      // billing_reason === "subscription_create" is the same filter the
      // referral programme below already uses, for the same reason. The trap
      // sitting next to it is checkout.session.completed, which fires at TRIAL
      // START with nothing collected, creates the Subscription row and flips
      // onboardingStatus to "active" — it is the event that looks right.
      //
      // amount_paid > 0 is not defensive padding. The first month is free
      // (TRIAL_PRICE) and the referral programme grants further free months on
      // top, so without it a $0 invoice would pay a full commission on nothing
      // collected. qualifiesForFirstPayment holds both rules in one place so a
      // reader cannot satisfy one and miss the other.
      if (qualifiesForFirstPayment(obj)) {
        ...
        if (paying?.companyId) {
          await earnMilestone({
            ...
            // Stripe's own timestamp. This row is also the anchor the 60-day
            // retention sweep counts from, so a replay moving it would move a
            // future payout with it.
            occurredAt: obj.status_transitions?.paid_at
              ? new Date(obj.status_transitions.paid_at * 1000)
              : null,
          }).catch((err) => {
            console.error("[sales] first-payment milestone failed:", err?.message);
          });
        } else {
          ...
          console.error(
            `[sales] first payment for customer ${obj.customer} found no subscription row — milestone not recorded`,
          );
        }
      }
