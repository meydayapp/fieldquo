 ...
 * @param floor    the denominator below which the observation is not used
 ...
 * @returns {{ value, basis: "measured"|"assumed"|"none", sampleSize, floor,
 *             remaining, measured, assumed }}
 *
 * `measured` is returned even when the basis is `assumed`, so the page can
 * show "so far 3 of 12" beside the assumption — the owner should see the
 * number forming, not just the day it takes over.
 */
export function rate({ hit = 0, of = 0, floor, assumed = null, max = 1 } = {}) {
  ...
  if (o >= f && measured !== null) {
    return { value: measured, basis: "measured", sampleSize: o, floor: f, remaining: 0, measured, assumed: a };
  }
  if (a !== null) {
    return { value: a, basis: "assumed", sampleSize: o, floor: f, remaining: f - o, measured, assumed: a };
  }
  return { value: null, basis: "none", sampleSize: o, floor: f, remaining: f - o, measured, assumed: null };
}
 ...
 * referred signups / paying at start), but the floor is on how many months
 * qualified, because three cancellations in one month with 21 subscribers is
 * one observation, not twenty-one.
 */
export function monthsRate({ hit = 0, of = 0, months = 0, floorMonths, assumed = null, max = 1 } = {}) {
  ...
  const a = within(assumed, cap);
  const measured = o > 0 ? Math.min(cap, h / o) : null;
  if (m >= f && measured !== null) {
    return { value: measured, basis: "measured", sampleSize: m, floor: f, remaining: 0, measured, assumed: a };
  }
  if (a !== null) {
    return { value: a, basis: "assumed", sampleSize: m, floor: f, remaining: f - m, measured, assumed: a };
  }
  return { value: null, basis: "none", sampleSize: m, floor: f, remaining: f - m, measured, assumed: null };
}

/**
 * A monthly COUNT with its basis — organic signups. Measured as the mean over
 * the observed months once enough months exist; the owner's number until then.
