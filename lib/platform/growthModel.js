...
/** The rates without which nothing can be drawn — see project() for why referral is not one of them. */
...
export function project({
  ...
  const referral = finite(rates.referral?.value) && rates.referral.value >= 0 ? rates.referral.value : 0;
  const organicPerMonth = finite(organic?.value) && organic.value >= 0 ? organic.value : 0;
