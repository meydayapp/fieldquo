...
export const AUDIT_ACTIONS = {
  ...
  // /platform/ai-billing: an AI feature moved between FieldQuo's own budget
  // and the companies' allowances. Feature, from and to are in details.
  ai_payer_changed: { label: "Changed who pays for an AI feature", tone: "neutral" },
  ...
  onboarding_next_steps_email_updated: { label: "Changed the next-steps email (on/off, delay)", tone: "neutral" },
  onboarding_next_steps_email_sampled: { label: "Emailed themselves a sample of the next-steps letter", tone: "neutral" },
  ...
  // The rep's referral link, minted on first ask (never re-minted — a
  // printed link must keep working), and the name prospects hear. The work
  // name is its own row because "who is this person the prospect says they
  // spoke to" is asked of the log after the fact.
  sales_rep_link_minted: { label: "Minted a rep's referral link", tone: "neutral" },
  sales_rep_work_name_set: { label: "Set the name a rep uses with prospects", tone: "neutral" },
  // Floor-wide switches, danger like the supervision settings: the floor
  // settings change every rep's write-up window after every call, and the
  // outcome settings carry the samples and answering-machine detection that
  // are billed per call (docs/SALES-OUTCOMES.md).
  sales_floor_settings_updated: { label: "Changed the sales floor's after-call settings", tone: "danger" },
  sales_outcome_settings_updated: { label: "Changed the call-outcome lists and settings", tone: "danger" },
  // A superadmin's verdict on a logged outcome, beside the recording —
  // the rep reads it on the call (lib/sales/calls/dispositionAudit.js).
  sales_disposition_audited: { label: "Audited a rep's logged call outcome", tone: "neutral" },
  ...
  // "Reassign" on /platform/signups (lib/signup/assignment.js): one signup
  // row moved straight from the rep holding it to another, with both reps
  // and the prospect in the details.
  signup_reassigned: { label: "Moved a signup from one rep to another", tone: "neutral" },
  // "Remove from list" / "Restore" on /platform/signups (lib/signup/dismissal.js).
  // Neutral both ways: FieldQuo's own note about a row, nothing deleted and
  // nothing on the signup or the company changed — but the row stops being
  // mailed and stops being offered to reps, so who hid it is worth a line.
  signup_dismissed: { label: "Removed a signup from the signups list", tone: "neutral" },
  signup_restored: { label: "Restored a signup to the signups list", tone: "neutral" },
