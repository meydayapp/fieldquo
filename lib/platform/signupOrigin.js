...
export function shapeOrigin(o) {
  return {
    ...
    // The rep, and — when the rep works for a call-centre agency
    // (lib/sales/agency.js) — the agency, so the console says "via Ann Lee
    // (Northline Contact)" rather than a name the owner has to look up.
    rep: o.salesRep
      ? {
          id: o.salesRep.id,
          name: o.salesRep.name,
          code: o.salesRep.code,
          agency:
            o.salesRep.engagement === "agency" && o.salesRep.manager?.kind === "agency"
              ? { id: o.salesRep.manager.id, name: o.salesRep.manager.name }
              : null,
        }
      : null,
...
export const ORIGIN_SELECT = {
  ...
  acceptLanguage: true,
  via: true,
  salesRep: { select: { id: true, name: true, code: true, engagement: true, manager: { select: { id: true, kind: true, name: true } } } },
  referralCode: true,
  utmSource: true,
  utmMedium: true,
  utmCampaign: true,
 ...
 * @param {object|null} args.utm           { utm_source, utm_medium, utm_campaign } as the signup page posted them — readUtm() cleans them
 * @returns {Promise<{ ok: boolean, origin: object|null, flag: string, error?: string }>}
 */
export async function recordSignupOrigin(
  { companyId, request, via, salesRepId = null, referralCode = null, statedCountry = null, companyName = null, utm = null },
  deps = {},
) {
  ...
  try {
    ...
    const origin = await database.signupOrigin.create({
      data: {
        ...
        // The advert, when the link named one. Nothing is inferred from the
        // referrer header — a tag is a statement, a referrer is a guess.
        ...readUtm(utm),
        flag: decision.flag,
