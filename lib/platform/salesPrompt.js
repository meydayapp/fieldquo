 ...
 *
 * The fallback is deliberately a real, monitored route rather than "someone
 * will ring you back". Nothing on this call writes a record anywhere — there is
 * no lead table for FieldQuo's own enquiries and no tool that could reach one —
 * so a promised callback would be a promise nobody can keep. The contact form
 * at /contact emails FieldQuo directly, which is a thing that provably happens.
 */
function honestGaps({ canTransfer, contactUrl }) {
...
anywhere, so never say someone will ring them back — nobody would. If you
cannot answer and cannot put them through, send them to ${contactUrl}, which
goes straight to FieldQuo, and tell them that is the quickest way to get a
real answer.`,
  );
...
export function buildSalesPrompt({
  ...
  parts.push(...honestGaps({ canTransfer, contactUrl }));
