 ...
 *
 * @param companies [{ country, isDemo, trialEndsAt, subscription: { status,
 *                    billingInterval, plan: { priceMonthly, priceAnnual, currency } } }]
 */
export function tallyCompanies(companies, now = new Date()) {
  ...
  for (const c of Array.isArray(companies) ? companies : []) {
    if (!c || c.isDemo) continue;
    const status = c.subscription?.status;
    const paying = status === "active" || status === "past_due";
    const trialling =
      status === "trialing" ||
      (!c.subscription && c.trialEndsAt && new Date(c.trialEndsAt).getTime() > t);
