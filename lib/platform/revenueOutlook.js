...
//
// MRR was "sum of priceMonthly across active subscriptions". Every one of
// those subscriptions is attached to a Plan with stripePriceId = null, so not
// one of them can raise a charge. Nominal MRR is $1,335. Collectable MRR is
// zero, and the gap between the two is the single most useful number on the
// page: it is exactly the revenue that is one configuration fix away.
 ...
 *
 *   stripeSubscriptionId  Stripe has an object to bill against
 *   plan.stripePriceId    that object knows what to charge
 *
 * Today every FieldQuo subscription has the first and none has the second,
 * which is why "NOT BILLABLE: 0" on the subscriptions page was giving an
 * all-clear during a total billing outage — it only checked the first.
 */
export function isCollectable(sub) {
  if (!sub) return false;
  if (!sub.stripeSubscriptionId) return false;
  if (!sub.plan?.stripePriceId) return false;
