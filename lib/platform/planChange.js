...
/**
 * Where a plan sits on the ladder, custom sizes included, or null off it.
 *
 * A custom row ("custom-20") ranks above every published rung — its seat
 * count is at least eleven, past Scale's ten — and two custom sizes rank by
 * seats, so 20 → 30 seats is an upgrade taken today and 30 → 20 a downgrade
 * that waits. Seats are added to the ladder's length so no custom rank can
 * collide with a rung's.
 */
function ladderRank(plan) {
  const rung = TIER_RANK[plan?.tierKey];
  if (rung) return rung;
  const seats = customSeatsFromTierKey(plan?.tierKey);
  return seats ? SEAT_LADDER.length + seats : null;
}
...
export function comparePlanRank(currentPlan, nextPlan) {
  const ladder = [currentPlan, nextPlan].map(ladderRank);
