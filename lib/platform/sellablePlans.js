...
/** The one sentence every sell path answers with for a retired plan. A 409
 *  carrying this, not a 404: the plan exists, and "not found" would send the
 *  person checking their link. */
export const RETIRED_PLAN_ERROR = "This plan is no longer offered.";

/**
 * Has this plan been taken off sale for good — kept only for the companies
 * already on it?
 *
 * Different from private. `isPublic: false` hides a plan from the pricing page
 * and the picker but still honours a link carrying its id (the bespoke-rate
 * hand-off, /api/marketing/plans?plan=<id>), and /api/companies and the
 * change-plan checkout accept any id that exists. So the owner's
 * "Live test — $1" was buyable at $1 by anyone holding the link, and it could
 * not be deleted: a Subscription references it. Retired means no path sells
 * it — listed or by link — while Stripe keeps billing its existing
 * subscribers exactly as before.
 *
 * Truthy `retiredAt` rather than `!= null`: Prisma hands back a Date and a
 * JSON round-trip hands back a string, and both are retired. A row read with
 * a narrow select that OMITS the column reads as not retired, the same
 * hazard isPublic carries — so every narrow select feeding this must include
 * `retiredAt: true`, and scripts/check-platform-pricing-console.mjs greps
 * that they do.
 */
export function isRetired(plan) {
  return Boolean(plan?.retiredAt);
}
...
export function isSellable(plan) {
  ...
  // First, before the price and before isPublic: a retired plan is not for
  // sale however it is priced and whoever is asking.
  if (isRetired(plan)) return false;
 ...
 *
 *   retired       taken off sale for good. Above no_price on purpose: a
 *                 retired plan's blank price is not a fault anyone should
 *                 go and fix.
...
export function planStatus(plan) {
  if (isRetired(plan)) {
    return {
      ...
      tone: "note",
      text:
        "Retired — kept for the companies still on it, sold to nobody, " +
        "listed or by link. Un-retire to offer it again.",
    };
  }
