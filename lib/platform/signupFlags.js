...
/** The three utm tags a signup link may carry, and the column each lands in. */
export const UTM_KEYS = Object.freeze({ utm_source: "utmSource", utm_medium: "utmMedium", utm_campaign: "utmCampaign" });
export const UTM_MAX = 100;

/**
 * utm_medium values that mean "a paid advert", and utm_source values that are
 * ad platforms — either marks the signup as an AD trial for the growth model.
 * Lower-cased; matched exactly, never by substring, so "organic_social" is
 * not "social" and "cpc-ish" is nothing.
 */
export const PAID_UTM_MEDIUMS = Object.freeze(["cpc", "ppc", "paid", "paid_social", "paidsocial", "paid-social", "display", "paid_search", "lead_form", "leadform"]);
export const AD_UTM_SOURCES = Object.freeze(["meta", "facebook", "fb", "instagram", "ig", "google", "googleads", "google_ads", "adwords", "tiktok", "linkedin", "bing", "microsoft"]);

/**
 * The utm tags off a signup, as stored: lower-cased, trimmed, capped, null
 * when absent or not a string. Takes a plain object of the three raw values
 * (what the signup page posted), so the check can execute it on hostile
 * shapes — a number, an array, a 2,000-character source.
 */
export function readUtm(raw) {
  const out = { utmSource: null, utmMedium: null, utmCampaign: null };
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) return out;
  for (const [param, column] of Object.entries(UTM_KEYS)) {
    const v = raw[param] ?? raw[column];
    if (typeof v !== "string") continue;
    const clean = v.trim().toLowerCase().slice(0, UTM_MAX);
    out[column] = clean || null;
  }
  return out;
}

/** Is this signup an ad trial? True only on a stated paid medium or a known ad platform as source. Absent tags are NOT organic — they are unknown. */
export function isPaidAdSignup(row) {
  // A null row (a company with no SignupOrigin at all) is not paid and not
  // a crash — the growth measurement maps over rows it did not write.
  const utmMedium = row && typeof row === "object" ? row.utmMedium : null;
  const utmSource = row && typeof row === "object" ? row.utmSource : null;
  const medium = typeof utmMedium === "string" ? utmMedium.trim().toLowerCase() : "";
  const source = typeof utmSource === "string" ? utmSource.trim().toLowerCase() : "";
  return PAID_UTM_MEDIUMS.includes(medium) || AD_UTM_SOURCES.includes(source);
}
