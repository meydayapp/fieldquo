...
//
// Since 2026-09-15 an INFLUENCER code also carries a commission plan. The
// company that redeems it is enrolled in the influencer programme
// (lib/influencers): their referral link earns commission under that plan
// instead of the referrer's free month. The owner's decision was that an
// influencer gets the SAME trial as anyone — so the console defaults an
// influencer code to 0 reward months, and 0 is now a legal value here. The
// enrolment itself is the signup route's job, on the promo's `ok` result;
// this file only records what the code carries.
//
...
export async function generatePromoCode({
  ...
  const resolvedKind = kind === "tester" ? "tester" : "influencer";
  const data = {
    label: label?.trim() || null,
    notes: notes?.trim() || null,
    kind: resolvedKind,
    // A plan belongs on an influencer code only; a tester code carrying one
    // would enrol a tester nobody meant to pay.
    commissionPlanId: resolvedKind === "influencer" && commissionPlanId ? String(commissionPlanId) : null,
    // 0 is "no extra months" — the ordinary trial — and is what an influencer
    // code defaults to. A tester code is ONLY free months, so its floor stays
    // at one: a tester code worth nothing is a code nobody meant to mint.
    rewardMonths: clampInt(rewardMonths, resolvedKind === "influencer" ? 0 : 1, 24, 3),
