...
// Only the /16 of a device's last address is ever handed to the platform
// console — see the standing block below.
...
export async function companyHealth(companyId) {
  ...
  const [
    ...
    // Seat-sharing signals. Deliberately NOT part of the score below: the
    // score answers "is this account being used", and a suspicion about HOW
    // MANY people are behind one login is a different question with a
    // different remedy. Folding it in would have quietly turned a heuristic
    // into a number support reads as fact.
    db.accountAbuseStrike.findMany({
      where: { companyId, createdAt: { gte: since30 } },
      orderBy: { createdAt: "desc" },
      take: 10,
      select: { id: true, kind: true, detail: true, createdAt: true, userId: true },
    }),
  ]);

  if (!company) return null;

  // The devices behind a flag, fetched ONLY when there is a flag. Support has
  // to be able to disagree with the detector, and "seven devices" is not
  // something anyone can argue with — seven rows that are all the same browser
  // on consecutive minor versions is. Skipped entirely for healthy companies,
  // which is nearly all of them, so this costs the normal page nothing.
  let flaggedDevices = [];
  if (abuseStrikes.length || (company.accountStatus && company.accountStatus !== "active")) {
    const rows = await db.accountDevice.findMany({
      where: { companyId, lastSeenAt: { gte: since7 } },
      orderBy: { lastSeenAt: "desc" },
      take: 25,
      select: { id: true, userId: true, userAgent: true, lastIp: true, firstSeenAt: true, lastSeenAt: true },
    });
    const names = new Map(
      (
        await db.member.findMany({
          where: { companyId, userId: { in: [...new Set(rows.map((r) => r.userId))] } },
          select: { userId: true, user: { select: { name: true, email: true } } },
        })
      ).map((m) => [m.userId, m.user?.name || m.user?.email || null]),
    );
    flaggedDevices = rows.map((r) => ({
      id: r.id,
      userId: r.userId,
      who: names.get(r.userId) || null,
      // Truncated: the tail of a user-agent is build noise, and the point is
      // "which browser on what" at a glance.
      userAgent: r.userAgent ? r.userAgent.slice(0, 160) : null,
      // The /16 ONLY. Staff need to see that these are unrelated networks, not
      // which address a contractor's employee was sitting at.
      network: networkOf(r.lastIp),
  ...
  return {
    ...
    ageDays,
    signals,
    // Read-only, and separate from `score` and `signals` on purpose (see the
    // query above). `under_review` means a person should look, not that
    // anything has been done to the account — the platform console can view
    // everything and edit nothing (non-negotiable #3), so there is no action
    // attached to this and clearing it is a deliberate, manual act.
    standing: {
      status: company.accountStatus || "active",
      reviewedAt: company.accountReviewedAt,
      reason: company.accountReviewReason,
      strikes30: abuseStrikes.length,
