 ...
 */
export function buildHowToPay({ company, language = "en", reference = "", online = null } = {}) {
  ...
  return { language: COPY[language] ? language : "en", title: c.title, reference: reference || "", online: onlineBlock, methods };
}
...
export function howToPayFor(invoice, { company, language, online = null } = {}) {
  ...
  return buildHowToPay({ company, language, reference: invoice?.invoiceNumber || "", online });
}
