...
export const PAYMENT_METHOD_LABELS = {
  ...
  // A card taken on a terminal FieldQuo never saw — recorded for past jobs
  // entered after the fact. See the enum comment in prisma/schema.prisma.
  card_elsewhere: "Card (taken elsewhere)",
  // The US offline methods, and PayPal for either side — the words a
  // homeowner would use, so "Record payment → Zelle" prints "Zelle" on the
  // statement. `check` is the US spelling of `cheque`; the catalogue in
  // lib/payments/offlineMethods.js decides which one a company is offered.
  zelle: "Zelle",
  venmo: "Venmo",
  cash_app: "Cash App",
  check: "Check",
