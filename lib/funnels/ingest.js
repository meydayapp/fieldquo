...
export function buildLeadFromFunnel(funnel, payload = {}) {
  ...
  return {
    ...
    // Through the shared shape, like every other intake path. A funnel keys its
    // answers by the contractor's own question text, so it passes no address
    // argument — but if one of their questions IS worded "address", that answer
    // lands on the reserved key and convertLead seeds the client from it, which
    // is the right outcome and the reason the shape has reserved keys at all.
    intake: buildLeadIntake({ details: intake }),
    budgetBand,
    timeline,
  };
}
