...
export const PROCESSORS = [
  ...
  {
    // A MEMBER's own consent, not the company's, and the one processor whose
    // data flows in both directions on purpose. Written to satisfy Google's
    // "Limited Use" disclosure for the calendar scopes: what is read (busy
    // intervals only) and what is written (FieldQuo's own events) are stated
    // exactly, because the verification reviewer compares this paragraph to
    // the scopes on the consent screen.
    id: "google-calendar",
    name: "Google Calendar",
    role: "A staff member's own calendar, connected by that member from Settings → My calendar",
    dataShared:
      "Only when a staff member connects their own Google account. FieldQuo writes its own events to that member's calendar — the visits assigned to them, with the site address and a link back — and reads back only whether the member is busy at a given time, never the titles or details of their own events. The connection is revoked and every FieldQuo-created event removed when the member disconnects. Nothing from Google Calendar is used for advertising, sold, or shown to anyone but that member as busy time.",
    verify: {
      description: "lib/calendar/googleClient.js calls the Calendar API",
      pattern: /googleapis\.com\/calendar\/v3/,
      roots: ["lib/calendar/googleClient.js"],
    },
  },
  {
    id: "google-solar",
  ...
  {
    // ── Two Meta entries, on purpose ──────────────────────────────────────
    //
    // Same vendor, two genuinely different data flows, and collapsing them
    // into one entry would make the policy vaguer than the truth. One SENDS
    // a company's own advertisement TO Meta; the other RECEIVES that
    // company's own spend FROM Meta. A reader deciding whether they mind
    // needs to know which is which, and a company may have enabled one and
    // not the other.
    id: "meta-content-publishing",
    name: "Meta (Facebook & Instagram) — publishing",
    role: "Social media publishing — posting a company's own ad to its own Page or Instagram account",
    dataShared:
