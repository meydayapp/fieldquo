 ...
 */
// ── Why the seven English names are gone ────────────────────────────────────
//
// They were a hardcoded array, so /app/settings/availability and the opening-
// hours modal printed "Monday" to a Spanish user whose every other word was
// translated. The names now come from Intl via lib/format/localeDate.js, which
// has the full reasoning — the short version is that CLDR already ships this
// table in every language, including the ones nobody here has translated.
//
// `language` is optional and defaults to English so that a caller which has
// not been given the reader's language yet renders exactly what it rendered
// before — Intl's en names are byte-identical to the array this replaced. Pass
// the language and the day names follow the reader.
export function orderedWeekdays(weekStartsOn = 0, language = "en") {
  return orderedWeekdayNames(weekStartsOn, language, "long");
}
