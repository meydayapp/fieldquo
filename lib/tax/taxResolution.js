...
/**
 * Sources that are a STATED zero — a position, not an absence. "us_exempt"
 * is no longer produced (the US rung applies the full rate and leaves the
 * switch-off to the contractor — owner, 2026-09-19) but documents written
 * under it keep their stated zero, so it stays recognised here.
 */
...
export function recordTaxResolution(result) {
  ...
  return {
    ...
    // Retired with the one-tap job-kind question (owner, 2026-09-19): new
    // records carry null / false. Kept so older records still read.
    ...
    // Which address named the province — "site" (the job address),
    // "client", "client_address" (the client's address line) or "company"
    // — so the office copy keeps saying "from the job address" after the
    // addresses change. Null on records written before it existed.
    placeSource: result.place?.source || null,
    // The components behind a Canadian total ("GST 5% + QST 9.975%"), so
    // the line can name them without re-reading today's table.
    components: Array.isArray(d.components)
      ? d.components.map((c) => ({ name: c.name, rate: num(c.rate) })).filter((c) => c.name && c.rate != null)
      : null,
  };
}
