...
//                                   SENTENCE SAYING SO otherwise — then
//                                   passed through lib/tax/usTaxability.js,
//                                   which decides whether the quote is a
//                                   taxable sale at all in that state (most
//                                   often it is not, and the answer is a
//                                   stated 0%, not an invented one).
...
//
// ── The per-quote answer ─────────────────────────────────────────────────────
//
// Sixteen states tax some jobs and not others on a distinction only the
// estimator can make (capital improvement vs repair, residential vs
// commercial, building vs landscaping). `workType` carries their answer —
// one of lib/tax/usTaxability.js's US_JOB_OPTIONS — and until they answer,
// the state's stated default applies with `assumedAnswer: true` on the
// result, which every surface prints. The same argument carries the EU
// "renovation" flag; the two vocabularies do not overlap.
//
...
import { usJobTreatment, US_TAXABILITY } from "@/lib/tax/usTaxability";
...
export function resolveTaxRate({
  ...
  // ── The US rung: rate × taxability ──────────────────────────────────────
  //
  // Both "known" (ZIP or no-local-tax state) and "state_only" (the floor,
  // with its caution) go through lib/tax/usTaxability.js before a number
  // reaches the box. The treatment decides whether the quote is a taxable
  // sale in that state at all, and on what share of it.
  if (found.country === "US" && (found.status === "known" || found.status === "state_only")) {
    const treatment = usJobTreatment(found.region, workType);
    const scale = (r) => Math.round(r * 10000) / 10000;
    const detail = { ...found, treatment };
    switch (treatment?.applies) {
      case "none":
        return {
          rate: 0,
          source: "us_exempt",
          ...
          assumedAnswer: treatment.assumedAnswer,
        };
      case "share":
        return {
          rate: scale(found.rate * treatment.share),
      ...
      case "fixed":
        return {
          rate: Number(treatment.fixedRate),
          source: "jurisdiction_us",
          label: found.label,
          detail,
          cautionKey: null,
      ...
      default:
        // A state the rate table knows and the taxability table does not.
        // Both list all fifty plus DC, so this is a bug, not a jurisdiction;
        // the default rate and an unknown status are the honest outcome.
        return { ...fallback, source: "unknown_unknown_region", detail: found };
    }
  }
 ...
 *   whole quote — repair and maintenance work is taxable in New York.
 *   Rates table: September 2026.
 *
...
function usExplanation(result, lang, pct) {
  ...
  const where =
    tr.applies === "fixed"
      ? { key: "app.tax.us.where.fixed", params: { rate: pct(tr.fixedRate), state } }
      : d.precision === "zip"
  ...
  const reasonKey = `app.tax.us.reason.${tr.answer || tr.reason || tr.labour || "exempt"}`;
  const scope =
    tr.applies === "none"
      ? { key: "app.tax.us.scope.none", params: { state } }
      : tr.applies === "share"
        ? { key: "app.tax.us.scope.share", params: { share: pct(Math.round(tr.share * 100)), rate: pct(result.rate) } }
        : { key: "app.tax.us.scope.all", params: {} };
  return {
    key: tr.applies === "none" ? "app.tax.note.usNone" : "app.tax.note.usApplied",
    params: {},
    parts: {
      where,
      scope,
      reason: { key: reasonKey, params: { state } },
      month: usRatesMonth(d, lang),
      upTo: d.maxRate != null ? { key: "app.tax.us.upTo", params: { zip: d.zip, maxRate: pct(d.maxRate) } } : null,
      assumed:
        tr.assumedAnswer && tr.answer
          ? { key: "app.tax.us.assumed", params: { answer: { key: `app.tax.us.option.${tr.answer}`, params: {} } } }
          : null,
    },
  };
}
...
export function renderTaxNote(note, t) {
  ...
  return t(note.key, {
    ...
    reason: t(p.reason.key, p.reason.params),
