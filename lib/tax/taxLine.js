...
const pctIn = (lang) => (n) =>
  Number.isFinite(Number(n)) ? Number(n).toLocaleString(numberLocaleFor(lang), { maximumFractionDigits: 3 }) : "";
 ...
 *   jurisdiction_vat       VAT 21% (Netherlands)
  ...
  switch (r.source) {
    case "jurisdiction_ca": {
      const parts = (r.components || [])
        .filter((c) => c && c.name && Number.isFinite(Number(c.rate)))
        .map((c) => `${c.name} ${pct(c.rate)}%`);
    ...
    case "jurisdiction_vat":
      return { key: "app.tax.headline.vat", params: { rate: pct(r.rate), country: r.label || "" } };
...
export function taxLineUnresolvedHint(res, { place = null } = {}) {
  ...
  if (src === "unknown_vat_status_unknown") return { key: "app.tax.line.answerVat", params: {} };
  return place
    ? { key: "app.tax.line.unresolvedHintPlace", params: { place } }
