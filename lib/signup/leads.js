...
export function normaliseCapture(body = {}) {
  ...
  for (const k of ["firstName", "lastName", "companyName", "city", "province"]) {
 ...
 *
 *   locked      the row is finished business — a Company completed it, or it
 *               was promoted. A later capture at the same address is somebody
 *               starting over (or somebody else typing a stranger's email);
 *               either way the lead on the floor must not be rewritten under
 *               a rep. Nothing is written.
 *
...
export function planCaptureWrite({ existing = null, incoming, now = new Date() } = {}) {
  ...
  if (existing?.completedCompanyId || existing?.promotedAt) return { refusal: "locked" };
...
export function decideSignupLeadPromotion({
  ...
  if (lead.completedCompanyId) return { action: "skip", reason: "completed", final: true };
