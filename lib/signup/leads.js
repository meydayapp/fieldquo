...
import { prospectNotDismissedClauses } from "@/lib/signup/dismissal";
...
/**
 * skipReason for a row the SAME browser left behind on its way to finishing
 * the SAME business under another address — a half-typed email captured
 * mid-word (the owner's own signup, 2026-09-25, captured a shortened form
 * of the address he finished under), or an address the account step refused.
 * Stamped by recordSignupCompletion; it keeps the row off the promotion floor
 * (decideSignupLeadPromotion skips any skipReason) and out of the five-minute
 * letter (earlyNudgePersonFromLead reads it as completed). It is NOT
 * "finished" for signupLeadFinished: the row itself created nothing, and a
 * resume of it must not claim a company it does not have.
 */
export const FINISHED_SAME_VISITOR = "finished_same_visitor";
...
export function normaliseCapture(body = {}) {
  ...
  // The website answer. A yes with an address that is not one (still being
  // typed) is kept as the yes alone — the capture is a draft, and the form's
  // own validator is what refuses it at the step.
  const websiteRead = readWebsiteAnswer({ hasWebsite: b.hasWebsite, website: b.website });
  const hasWebsite = websiteRead.error ? true : websiteRead.hasWebsite;
  const website = websiteRead.error ? null : websiteRead.website;
...
/**
 * SignupLead.utm after a capture: the three tags the /signup link carried
 * (the newest capture's, else the stored ones — as before), plus `ft`, the
 * browser's FIRST AD TOUCH (lib/analytics/product/campaigns.js
 * firstTouchForVisitor). `ft` is set once and never replaced: a first touch
 * that moved would not be a first touch. It never comes from the request
 * body — normaliseCapture reads only the three tags — so a stranger cannot
 * post a campaign onto somebody's row; captureSignupLead reads it from the
 * browser's own landings on the server.
 */
export function utmWithFirstTouch(existingUtm, incomingUtm, firstTouch) {
  const tags = {};
  const pick = (src) => {
  ...
  if (!pick(incomingUtm)) pick(existingUtm);
  const storedFt = existingUtm && typeof existingUtm === "object" && !Array.isArray(existingUtm) && existingUtm.ft && typeof existingUtm.ft === "object" ? existingUtm.ft : null;
  const ft = storedFt || (firstTouch && typeof firstTouch === "object" && !Array.isArray(firstTouch) ? firstTouch : null);
  const out = ft ? { ...tags, ft } : tags;
...
export function planCaptureWrite({ existing = null, incoming, now = new Date() } = {}) {
  ...
  const data = {
    ...
    // An explicit answer replaces the stored one; unanswered keeps it. A
    // "no" clears the address — a URL beside "no website" is a contradiction.
    hasWebsite: keep("hasWebsite"),
    website: incoming.hasWebsite === false ? null : keep("website"),
    referrer: keep("referrer"),
    utm: utmWithFirstTouch(existing?.utm, incoming.utm, incoming.firstTouch),
 ...
 *             hour — the same grace the setup banner counts down) AND not on
 *             the card-free trial. Since 2026-09-24 signup takes no card at
 *             all, so a company on its free month with no plan chosen has
 *             done everything signup asked; lacking a card is not stalling.
 *             This leg now only fires for the old shape — no trial date, no
 *             row — which is a company that never finished (console-made,
 *             or a pre-trial-date signup). Before this, every new company
 *             was flipped to "Stalled — no card" an hour in (jaspedo,
 *             2026-09-25). A row the sweep already flipped is flipped back
 *             by the next sweep: this is a reading, not a stamp.
 *   no_quote  no quote SENT within STALLED_NO_QUOTE_DAYS of signup — for
 *             every company, trial or not. A trial that has sent nothing in
 *             a week is exactly who a welcome call should reach.
 *
 ...
 *
 * @param company { createdAt, subscription: {...}|null, trialEndsAt,
 *                firstQuoteSentAt } — `subscription` and `trialEndsAt` must
 *                be SELECTED: undefined throws, for the reason
 *                lib/signup/abandoned.js assertSelected gives.
 */
export function stalledDecision({ company, now = new Date(), checkoutGraceMs } = {}) {
  ...
  // isCardFreeTrial throws when trialEndsAt was not selected — the same
  // loud failure as the subscription check above.
  if (company.subscription === null && !isCardFreeTrial(company) && age > checkoutGraceMs) return { stalled: true, reason: "no_card" };
 ...
 * refuses.
 *
 * A row whose signup (lead or company) the owner removed from
 * /platform/signups is not one to place either (lib/signup/dismissal.js):
 * "Remove from list" there would otherwise leave the same junk row in the
 * folder one click away.
 *
 * NOT the whole rule on its own: FieldQuo's do-not-contact LIST
 * (SalesSuppression) cannot be joined in a WHERE, so every reader of the
 * folder goes through lib/signup/salesFloor.js unplacedSignupRows(), which
 * applies this and then drops every row the list closes. Counting this
 * WHERE directly would count a do-not-contact row as waiting for a call —
 * the contradiction the owner read on Luma Painting, 2026-09-24.
 */
export function unplacedSignupWhere(now = new Date()) {
  return {
    ...
    AND: prospectNotDismissedClauses(),
  };
}
