...
import { nudgeRecipient, NUDGE_WINDOW_DAYS } from "@/lib/signup/abandoned";
import { emailKeyOf, tradeKeyForIndustries } from "@/lib/signup/leads";
...
export function earlyNudgePersonFromLead(lead) {
  ...
  return {
    ...
    completed: Boolean(lead.completedCompanyId),
    isDemo: false,
 ...
 * and a company and stopped at or before the card. Their last activity is
 * the later of the company's creation and the linked SignupLead's lastSeenAt
 * (the lead is stamped "checkout" when the company is created, and keeps
 * moving if they come back to /signup).
 *
 * `company.subscription` must be SELECTED — the same rule isIncompleteSignup
 * gives: undefined would read as "no subscription" and mail a customer.
 */
export function earlyNudgePersonFromCompany(company, { ownerName = null } = {}) {
  ...
  return {
    ...
    // A trialEndsAt with no Subscription is a company that FINISHED signing
    // up: since 2026-09-24 signup opens no checkout (see decideSignupNudge in
    // abandoned.js, which already refuses the 24-hour letter on the same
    // stamp). Reading only the Subscription here sent jaspedo "you didn't
    // finish" six minutes after they had. Their letter is the next-steps one.
    completed: company.subscription !== null || Boolean(company.trialEndsAt),
 ...
 */
export function planEarlyNudges({ people = [], suppressedAddresses = new Set(), sentKeys = new Set(), heldKeys = new Set(), now = new Date() } = {}) {
  const byKey = new Map();
  const skipped = [];
