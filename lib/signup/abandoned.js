 ...
 *   person.
 *
 * ══ Overridden by the owner, 2026-09-21 — this is now the SECOND touch ═════
 *
 * The owner read the above and ruled that the FIRST letter goes out five
 * minutes after the person's last activity, with the free month and the
 * trade's benefits in it: "If they didn't complete the signup after 5
 * minutes they should get a follow-up email prompting them to complete the
 * signup so they can enjoy the free month." His reason is the population —
 * most people who stop never reached Stripe, so there is no open checkout
 * tab to wait out, only a form they closed, and the letter is worth most
 * while the thought is still warm. That touch is lib/signup/earlyNudge.js,
 * and it is measured from the last ACTIVITY, so somebody still moving through
 * the steps is not written to.
 *
 * This file's rule is unchanged and is now the second and last touch: a day
 * after a company was created with no card, once. The argument above still
 * holds for the people it is written to.
 */
...
/**
 * ══ 2026-09-24: "finished signing up" has two shapes now ════════════════
 *
 * Everything above was written when the card was the last step of signup,
 * so "has a Subscription row" and "finished signing up" were the same
 * sentence. Since 38d3308d signup ends at Services with NO checkout: the
 * company is created on its thirty-day trial (Company.trialEndsAt) and no
 * Subscription row exists until the owner picks a plan from the banner. So a
 * company FINISHED signing up when it has a Subscription row OR is on the
 * card-free trial — a trialEndsAt with no row.
 *
 * The card-free half is keyed on trialEndsAt alone, with no "created after
 * 2026-09-24" cutoff, because that is the rule every other reader of this
 * state already applies: lib/billing/access.js trialAccessFor gives such a
 * company full access, BillingBanner says "Free trial · N days left", the
 * trial-reminder cron writes to it, and decideSignupNudge below (38d3308d's
 * own fix) skips it. A cutoff here would make this file the one place that
 * disagrees with the banner the company is looking at. It does mean an
 * abandoned CARD checkout from before the change (trialEndsAt was stamped
 * before checkout on the old path too) now reads as a trial — which is what
 * the product is actually giving it.
 *
 * The header's reason for ignoring trialEndsAt ("stamped at signup, before
 * checkout") is exactly why it is the right signal now: the stamp IS the
 * end of signup. Demos and console-created companies have no trialEndsAt and
 * keep the old no-row rule.
 *
 * Every reader — the two nudges, the sales floor's stalled rule,
 * /platform/signups, /platform/companies, the analytics overview, the tenant
 * board — goes through the fragments and predicates below. One declaration;
 * the incident that forced it (jaspedo, 2026-09-25 03:50 UTC, "you didn't
 * finish" five minutes after finishing) was a reader that kept its own copy.
 */

/**
 * The Prisma fragment for "never finished signing up": no Subscription row
 * AND no trial date.
 *
...
export function incompleteSignupWhere() {
  return { subscription: { is: null }, trialEndsAt: null };
}

/**
 * Its complement: a company that finished signing up — reached the end of
 * Stripe Checkout, or is on the card-free trial.
 *
 * An OR, so a caller must never spread it beside an OR of its own (the
 * second key would silently replace the first). Every caller today spreads it
 * beside `isDemo` alone; the check asserts the shape.
 */
export function completedSignupWhere() {
  return { OR: [{ subscription: { isNot: null } }, { trialEndsAt: { not: null } }] };
}

/**
 * The card-free trial alone: signed up, no plan chosen yet. Whether the
 * thirty days have run out is the caller's question (trialAccessFor answers
 * it); this is the population /platform/billing/subscriptions lists as "Free
 * trials without a plan" and /platform/signups as "New companies on free
 * trial".
 */
export function cardFreeTrialWhere() {
  return { subscription: { is: null }, trialEndsAt: { not: null } };
}

/**
 * Both relation and column must have been SELECTED. `undefined` is not
 * `null`: a query that forgot `subscription` would read every paying customer
 * as "no card", and one that forgot `trialEndsAt` would read every new trial
 * as "didn't finish" — the second is exactly what mailed jaspedo.
 */
function assertSelected(company, fn) {
  if (company.subscription === undefined) {
    throw new Error(
      `${fn}: company.subscription was not selected — cannot tell ` +
        "'never completed checkout' from 'not loaded'",
    );
  }
  if (company.trialEndsAt === undefined) {
    throw new Error(
      `${fn}: company.trialEndsAt was not selected — cannot tell ` +
        "'card-free trial' from 'not loaded'",
    );
  }
}

/** A trial date that parses. An unparseable one is not a statement. */
function hasTrialDate(company) {
  if (!company?.trialEndsAt) return false;
  const d = new Date(company.trialEndsAt);
  return !Number.isNaN(d.getTime());
}

/**
 * On the card-free trial: no Subscription row, a trial date. Demos are
 * never on it (they have no trial date, and are refused first anyway).
 */
export function isCardFreeTrial(company) {
  if (!company || company.isDemo) return false;
  assertSelected(company, "isCardFreeTrial");
  return company.subscription === null && hasTrialDate(company);
}

/**
 * Finished signing up: a Subscription row, or the card-free trial. The one
 * predicate every in-memory reader asks. Demos answer false — they never
 * signed up at all, and every caller refuses them before asking.
 */
export function hasFinishedSignup(company) {
  if (!company || company.isDemo) return false;
  assertSelected(company, "hasFinishedSignup");
  return company.subscription !== null || hasTrialDate(company);
}
 ...
 * trialEndsAt }.
 *
 * Throws when either was not selected — see assertSelected. That mistake,
 * read as "incomplete", would mail every customer.
 */
export function isIncompleteSignup(company) {
  if (!company) return false;
  if (company.isDemo) return false;
  return !hasFinishedSignup(company);
}
...
/**
 * RFC 2606 reserves example.com/.net/.org, .test, .invalid and localhost for
 * documentation and tests: nobody real reads them, and a letter to one is a
 * bounce against FieldQuo's own sending domain. The signup form accepts them
 * (a test run through the funnel is a legitimate use, and the fixtures in
 * the check are all example.com), so the refusal is NOT part of the decision
 * rule — it lives in the cron, at the point of sending, for both touches.
 */
export function isReservedTestAddress(email) {
  const value = nudgeRecipient(email);
  if (!value) return false;
  return /@(?:[a-z0-9-]+\.)*(?:example\.(?:com|net|org)|invalid|test|localhost)$/.test(value);
}
 ...
 *                         trialEndsAt, signupNudgeSentAt, memberCount } —
 *                         `subscription` AND `trialEndsAt` must be selected
 *                         (assertSelected throws otherwise).
 ...
 * @param heldByRep      true when a sales rep currently holds this company's
 *                       signup row (Prospect.companyId → assignedRepId, lease
 *                       live — lib/signup/salesFloor.js). The rep's own intro
 *                       email and call replace the letter: two messages from
 *                       two senders about one unfinished signup is the
 *                       "being watched" failure NUDGE_WINDOW_DAYS guards.
 * @param dismissed      true when the owner removed this row from
 *                       /platform/signups (lib/signup/dismissal.js). A row he
 *                       called junk is not written to either — hiding it on
 *                       the screen while the cron keeps mailing it would be
...
export function decideSignupNudge({
  ...
  // ── A trial with no plan is not an abandoned checkout ────────────────────
  //
  // Since 2026-09-24 signup opens no checkout at all (the owner: "move the
  // credit card and plan selection out of the sign up"), so a company with no
  // Subscription row and a trialEndsAt is a company that FINISHED signing up
  // and is on its free month — lib/billing/access.js's trialAccessFor owns
  // it, and lib/billing/trialReminder.js writes to it. "We noticed you didn't
  // finish" to that person is the false sentence the header below warns
  // about, and it would have gone to every new company twenty-four hours in.
  // The header's own reason for ignoring trialEndsAt (stamped before checkout)
  // is exactly why it is the right signal now: the stamp IS the signup.
  //
  // Through isCardFreeTrial, not a bare `company.trialEndsAt` read: the
  // cron's query for this letter did not SELECT trialEndsAt when this branch
  // was added, so the bare read saw `undefined` and let every new trial
  // through to the send. The predicate throws on an unselected column.
  if (isCardFreeTrial(company)) return { send: false, reason: "trial_no_plan" };
  ...
  // Removed from /platform/signups by the owner. Never stamped (see the
  // STAMPABLE list below): a restore puts the row back exactly as it was.
  if (dismissed) return { send: false, reason: "dismissed" };
  ...
  // A rep is on it. Not stamped: if the rep gives the row back and nobody
  // else takes it, the letter is still owed inside the window.
 ...
 * @param heldCompanyIds      a Set of company ids whose signup row a rep holds
 * @param dismissedCompanyIds a Set of company ids removed from /platform/signups
 * @param now                 injectable clock
 *
...
export function planSignupNudges({
  ...
  heldCompanyIds = new Set(),
  dismissedCompanyIds = new Set(),
  now = new Date(),
} = {}) {
  const rows = Array.isArray(companies) ? companies : [];
  const held = heldCompanyIds instanceof Set ? heldCompanyIds : new Set(heldCompanyIds || []);
  const dismissedIds = dismissedCompanyIds instanceof Set ? dismissedCompanyIds : new Set(dismissedCompanyIds || []);
  ...
  for (const company of ordered) {
    ...
    const decision = decideSignupNudge({
      ...
      heldByRep: held.has(company?.id),
      dismissed: dismissedIds.has(company?.id),
      now,
    });
