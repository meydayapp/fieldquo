...
export const CAPTURE_STEPS = Object.freeze(["account", "business", "team", "goals", "industry", "services", "plan", "checkout"]);
...
export function captureBodyFor(form = {}, step = "account", extras = {}) {
  ...
  const body = {
    ...
    // "Do you have a website?" — kept on the draft row so a resumed signup
    // does not lose it. The boolean as-is (null = unanswered); the address
    // only with a yes. lib/signup/leads.js normaliseCapture re-reads both.
    hasWebsite: form.hasWebsite === true || form.hasWebsite === false ? form.hasWebsite : null,
    website: form.hasWebsite === true ? s(form.website, 300) : null,
    trades,
    serviceCategoryIds,
