...
import { DISMISSAL_SELECT, isDismissed, notDismissedWhere } from "@/lib/signup/dismissal";
import { findRepByLinkCode } from "@/lib/sales/repLink";
import { firstTouchForVisitor } from "@/lib/analytics/product/campaigns";
import { dncSentence, signupDoNotContact, signupSuppressions } from "@/lib/signup/assignment";
import {
  SIGNUP_KINDS,
  FINISHED_SAME_VISITOR,
  SIGNUP_SOURCE,
  SIGNUP_STATUS,
  STEP_LABELS,
 ...
 *
 * `session` is the caller's Better Auth session, when it has one — the
 * route reads it, never the body. The user id is attached to the row ONLY
 * when the session's address is the address being captured: a signed-in
 * person typing a different email into the form is not that email's owner,
 * and linking the row to them would hand them that stranger's prefill on
 * their next signed-in return (signupLeadForResume's user-id fallback).
 */
export async function captureSignupLead({ client, body, now = new Date(), session = null } = {}) {
  ...
  const sessionKey = session?.email ? emailKeyOf(session.email) : null;
  if (session?.userId && sessionKey && sessionKey === read.lead.emailKey) read.lead.authUserId = String(session.userId);
  ...
  const existing = await client.signupLead.findUnique({
    ...
    select: {
      ...
      country: true, province: true, city: true, trades: true, language: true, hasWebsite: true, website: true, referrer: true, utm: true,
      visitorId: true, salesCode: true, referralCode: true, stepReached: true, consentAt: true,
      completedCompanyId: true, promotedAt: true, skipReason: true, authUserId: true,
    },
  });
  // The browser's first ad touch, read once — while the row has none — from
  // its own marketing landings (the beacon's anonymous id, which this body
  // already carried). Only when the row would take a write at all.
  if (read.lead.visitorId && !existing?.utm?.ft && !signupLeadFinished(existing) && !existing?.promotedAt) {
    const ft = await firstTouchForVisitor(client, read.lead.visitorId, now);
    if (ft) read.lead.firstTouch = ft;
  }
  ...
  // read above and this write locks the row in the same instant. "Unfinished"
  // is signupLeadFinished's rule — a row the cron matched to some company on
  // the books is still this person's live signup and keeps taking captures.
  const updated = await client.signupLead.updateMany({
    where: { id: existing.id, promotedAt: null, ...unfinishedSignupLeadWhere() },
    data: plan.data,
  });
 ...
 * so the same read is allowed on it.
 *
 * Two more things learned from his SECOND return the same evening:
 *
 *   · `completed` is signupLeadFinished's answer, not `completedCompanyId`'s.
 *     The cron had matched his row by phone to another company of his and
 *     the page, told "completed", put nothing back. A row that finished is
 *     still returned with the flag, so the page can refuse to prefill — but
 *     only a row that actually finished.
 *   · the address is looked up by resumeEmailKeys (case, and gmail's dots
 *     and plus tags), and then by the Better Auth user id the capture
 *     stored (authUserId), so a person whose login address no longer spells
 *     the row's key still gets their own row back. `matchedBy` says which.
 */
export async function signupLeadForResume({ client, token, email, userId } = {}) {
  let row = null;
  let matchedBy = null;
  if (typeof token === "string" && token) {
    row = await client.signupLead.findUnique({ where: { resumeToken: token }, select: RESUME_SELECT });
    matchedBy = row ? "token" : null;
  } else {
    const keys = resumeEmailKeys(email);
    if (keys.length) {
      const candidates = await client.signupLead.findMany({ where: { emailKey: { in: keys } }, select: { ...RESUME_SELECT, emailKey: true } });
      // The exact key before a provider-collapsed one: both may exist, and
      // the one spelled the way they signed in is theirs beyond doubt.
      row = keys.map((k) => candidates.find((c) => c.emailKey === k)).find(Boolean) || null;
      matchedBy = row ? "email" : null;
    }
    if (!row && userId) {
      row = await client.signupLead.findFirst({ where: { authUserId: String(userId) }, select: RESUME_SELECT, orderBy: { lastSeenAt: "desc" } });
      matchedBy = row ? "user" : null;
    }
  }
  if (!row) return null;
  return {
    ...
    // true / false / null — a "no" is put back as much as a URL is.
    hasWebsite: row.hasWebsite ?? null,
    ...
    completed: signupLeadFinished(row),
  };
}
 ...
 *
 * The code is the rep's opaque referralToken on a link built since
 * 2026-09-25, or their legacy name slug on an older one — lib/sales/repLink.js
 * resolves both, token first, the same precedence attribution uses. Unknown
 * resolves to null: the lead is nobody's by referral.
 */
async function referredRepFor({ client, lead }) {
  if (!lead.salesCode) return null;
  const rep = await findRepByLinkCode(lead.salesCode, client, {
    id: true,
    name: true,
    active: true,
    endedAt: true,
...
export async function promoteSignupLeads({ client, now = new Date(), limit = 200 } = {}) {
  const rows = await client.signupLead.findMany({
    // A row the owner removed from /platform/signups is never promoted into
    // a hot lead (lib/signup/dismissal.js) — until he restores it.
    where: { completedCompanyId: null, promotedAt: null, skipReason: null, phoneE164: { not: null }, ...notDismissedWhere() },
...
export async function recordSignupCompletion({ client, company, ownerEmail = null, ownerName = null, referred = false, now = new Date() } = {}) {
  try {
    ...
    let lead = null;
    if (keys.length) {
      // A row the cron had matched to some OTHER company on the books
      // (skipReason company_exists) is still this signup's row, and the
      // company it just created is the one it completed: the guess is
      // replaced by the fact, and the stamp cleared with it.
      lead = await client.signupLead.findFirst({ where: { emailKey: { in: keys }, ...unfinishedSignupLeadWhere() }, select: { id: true, prospectId: true, promotedLeadId: true, visitorId: true, companyName: true } });
      if (lead) {
        await client.signupLead.updateMany({ where: { id: lead.id, ...unfinishedSignupLeadWhere() }, data: { completedCompanyId: company.id, skipReason: null, stepReached: "checkout", lastSeenAt: now } });
        await closeSameVisitorSiblings({ client, lead });
      }
    }
    if (referred) {
...
/**
 * The rows the SAME browser left for the SAME business on its way to this
 * finish — marked done, so nothing follows them up as an abandoned signup.
 *
 * The owner's own signup, 2026-09-25: one browser captured an address
 * half-typed (the capture fires mid-word) for his painting business, then
 * finished the same business under the full address. The finished row was
 * linked here; the half-typed one stayed open, and five minutes later the
 * cron mailed "your free month is waiting" to an address that may not even
 * be his. completedCompanyId is @unique, so a sibling cannot be linked to
 * the same company — it is stamped FINISHED_SAME_VISITOR instead, which the
 * promotion floor and the five-minute letter both read as closed.
 *
 * Narrow on purpose: the same visitorId AND the same business name. A rep
 * walking three different customers through signup on one laptop shares a
 * visitorId across businesses that are genuinely not finished, and none of
 * them may be silenced by the one that was. Never throws past its caller's
 * own try.
 */
async function closeSameVisitorSiblings({ client, lead }) {
  if (!lead?.visitorId || !lead?.companyName) return;
  await client.signupLead.updateMany({
    where: {
      id: { not: lead.id },
      visitorId: lead.visitorId,
      companyName: { equals: lead.companyName, mode: "insensitive" },
      completedCompanyId: null,
      skipReason: null,
      promotedAt: null,
    },
    data: { skipReason: FINISHED_SAME_VISITOR },
  });
}
...
const COMPANY_FACTS_SELECT = {
  ...
  // With the subscription, the card-free trial's date: stalledDecision reads
  // both (a trial is not "stalled — no card"), and throws if either is missing.
  trialEndsAt: true,
...
export function companyFactsOf(company) {
  ...
  return {
    ...
    // Passed through as loaded — never defaulted. A caller that did not
    // select it gets undefined, and stalledDecision refuses to guess.
...
export async function sweepSignupProspects({ client, now = new Date() } = {}) {
  ...
  const recent = await client.company.findMany({
    // Not a company the owner removed from /platform/signups: its welcome row
    // would put the same junk back in the folder the next run.
    where: { isDemo: false, createdAt: { gte: since }, salesAttribution: { is: null }, referredByCode: null, signupProspects: { none: {} }, ...notDismissedWhere() },
...
/**
 * The rows unplacedSignupWhere matches, minus every one FieldQuo's
 * do-not-contact list closes.
 *
 * ══ Why the list is asked here and not in the WHERE ═══════════════════════
 *
 * unplacedSignupWhere already refuses Prospect.doNotContactAt — the flag a
 * rep sets on the floor. The LIST (SalesSuppression) is a different table,
 * keyed on normalised addresses, numbers and domains with no relation to a
 * Prospect, so no Prisma WHERE can join it. Before this the folder never
 * asked it at all: Luma Painting unsubscribed from a signup letter (all
 * three channels closed) and was offered, and handed to a rep, the same
 * evening. So every reader of the folder goes through this one function —
 * the section's list, its count on the funnel — and the assign write asks
 * the list again, fresh, before it writes (assignSignupToRep).
 */
async function unplacedSignupRows({ client, now, hotOnly = false, take = 200, select }) {
  const rows = await client.prospect.findMany({
    ...
    select: {
      ...select,
      id: true, email: true, phoneE164: true,
      signupLead: { select: { ...(select.signupLead?.select || {}), email: true, phoneE164: true } },
      company: { select: { ...(select.company?.select || {}), email: true, phone: true } },
    },
  });
  const dnc = await signupSuppressions({
    ...
    contacts: rows.map((p) => ({
      key: p.id,
      emails: [p.email, p.signupLead?.email, p.company?.email],
      phones: [p.phoneE164, p.signupLead?.phoneE164, p.company?.phone],
    })),
  });
  return rows.filter((p) => !dnc.get(p.id)?.dnc);
}

/** How many rows the review folder's signup section would list — the funnel's "hot, nobody placed" tile. */
export async function countUnplacedSignups({ client, now = new Date(), hotOnly = false } = {}) {
  return (await unplacedSignupRows({ client, now, hotOnly, take: 1000, select: { id: true } })).length;
}

/**
 * The unplaced signup rows, newest first, hot first. `hotOnly` is the funnel
 * link's filter. Never a do-not-contact row (unplacedSignupRows).
 */
export async function listUnplacedSignups({ client, now = new Date(), hotOnly = false, take = 200 } = {}) {
  const rows = await unplacedSignupRows({
    client,
    now,
    hotOnly,
    take,
...
export async function assignSignupToRep({ client, admin, rep, prospectId, now = new Date(), notify = null, callback = false } = {}) {
  ...
  const row = await client.prospect.findUnique({
    where: { id: prospectId },
    select: {
      id: true, businessName: true, province: true, country: true, signupKind: true, doNotContactAt: true, mergedIntoId: true, assignedRepId: true, assignedAt: true, claimExpiresAt: true, tradeKey: true,
      // Every address and number the row's person is known by, for the list.
      email: true, phoneE164: true,
      signupLead: { select: { email: true, phoneE164: true } },
  ...
  // FieldQuo's do-not-contact list, read in THIS request: an opt-out that
  // arrived while the owner had the folder open wins. The Prospect's own
  // flag was judged just above; this is the list, which the flag is not.
  const dnc = await signupDoNotContact({
    client,
    emails: [row.email, row.signupLead?.email, row.company?.email],
    phones: [row.phoneE164, row.signupLead?.phoneE164, row.company?.phone],
  });
  if (dnc.dnc) return { error: dncSentence(dnc), refused: true, doNotContact: true };
...
export async function ensureSignupProspect({ client, leadId = null, companyId = null, now = new Date() } = {}) {
  if (leadId) {
    const lead = await client.signupLead.findUnique({ where: { id: leadId }, include: { signupDismissal: true } });
    if (!lead) return { error: "That signup no longer exists.", reason: "no_lead" };
    if (isDismissed(lead)) return { error: "Removed from the signups list — restore it first.", reason: "dismissed" };
    if (signupLeadFinished(lead)) return { error: "This person finished their signup — they are a company now.", reason: "completed" };
    ...
    if (result.action === "link_company" || result.reason === "company_exists") return { error: "This email or phone already belongs to a company on the books.", reason: "company_exists" };
  ...
  if (companyId) {
    const company = await client.company.findUnique({
      where: { id: companyId },
      select: {
        ...COMPANY_FACTS_SELECT, ...DISMISSAL_SELECT, phone: true, referredByCode: true,
    ...
    if (isDismissed(company)) return { error: "Removed from the signups list — restore it first.", reason: "dismissed" };
    // A paying (or card-backed) customer is not a signup to ring. A card-free
    // trial IS still placed: its welcome call is the whole point of the
    // "new" row, so hasFinishedSignup is deliberately not the test here.
