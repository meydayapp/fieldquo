 ...
 * no demos, and a company that signed up through the product — a
 * Subscription row, or (since signup stopped taking a card on 2026-09-24) a
 * trial date. A company the console created by hand has neither and is not a
 * peer of somebody who just signed up. The pure firstQuoteProof() decides
 * whether the list is big enough to print.
 *
...
export async function firstQuoteMinutesForTrade(tradeKey, { client = db } = {}) {
  ...
  const companies = await client.company.findMany({
    where: {
      isDemo: false,
      industries: { hasSome: slugs },
      OR: [{ subscription: { isNot: null } }, { trialEndsAt: { not: null } }],
    },
    select: {
      createdAt: true,
