 ...
 */
export const STEPS = ["account", "business", "team", "goals", "industry", "services", "plan"];

/**
 * The two rungs added on 2026-09-24 (the owner: "when they ask how many
 * employees the calendar changes to reflect that"):
 *
 *   team    how many people work with you, and years in business — the
 *           calendar/dispatch preview beside the form scales with the answer,
 *           and the answer starts the trial banner's plan recommendation.
 *   goals   what is top of mind, and how they heard of us.
 *
 * Both are SKIPPABLE and neither gates a later step: furthestStep never
 * demands an answer from them, so a resumed visitor with a company and no
 * trades still lands on "industry" exactly as before, and a skipped step
 * stores null rather than a default (AGENTS.md: absence of a statement is not
 * a statement). They sit before the trades step, not after services, so
 * "Start my free trial" stays where it was — the last button of the funnel.
 */
export const OPTIONAL_STEPS = ["team", "goals"];
