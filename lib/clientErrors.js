...
// The plan gate's refusal is not a message to show — it is a prompt to open.
// See the interception in reportResponseError below.
 ...
 *
 * One tone of the shared toast layer (lib/toast.js), not its own event: the
 * error toast used to have its own channel and its own listener, mounted in
 * /app only — so every showError() on /sales and /platform went to nobody.
 * app/components/ToastLayer.js is mounted on all three surfaces.
 */
...
export async function reportResponseError(res, fallbackOrSetter, maybeFallback) {
  ...
  // ── The one refusal that is a door rather than a message ─────────────────
  //
  // A send refused because the company never finished checkout carries a path
  // that fixes it (lib/signup/planGate.js). Toasting that sentence would be
  // the wall the owner asked us not to build — "maybe prompting them to
  // complete it" — and there are more call sites that report failures through
  // this function than there are screens anyone would remember to wire.
  //
  // So it is intercepted HERE, and the prompt is opened instead. The setter, if
  // there is one, is left alone deliberately: an inline red banner under a Send
  // button, saying the same thing the modal in front of it says, reads as two
  // separate problems.
  if (planRequiredFrom(res.status, data)) return message;
