...
export async function autoTranslateOnSave(
  ...
  try {
    ...
    if (!gate.allowed) {
      log.warn?.(`[autoTranslate] ${jobs.length} draft(s) for ${companyId} left pending — ${gate.code || gate.reason}`);
      return leavePending("budget");
    }
  } catch (err) {
  ...
  // Draft. Each call is recorded on the meter's ledger as it returns.
