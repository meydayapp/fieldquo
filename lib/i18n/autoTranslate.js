...
const PER_FIELD = new Set(["company", "serviceContent"]);
...
export async function autoTranslateOnSave(
  ...
  if (model !== "company" && !id) return empty("bad_args");
  ...
  let existing;
  if (model === "company") existing = await readCompanyExisting(db, companyId, [...work, ...tooLong].map((w) => w.field));
  else existing = await readJsonExisting(db, model, id, companyId);
  if (model !== "company" && existing === null) return { ...empty("row_missing"), languages: targetLanguages(assumed) };
  ...
  const entriesFor = (field) => {
    if (model === "company") return existing[field] || {};
  ...
  for (const u of units) {
    const langs = Object.values(u.entries)
      ...
      .map((e) => (model === "company" ? e.sourceLanguage : e.from))
...
export function autoTranslateSummary({ model, fields = {}, sourceLanguage = "en" }, { isAiConfigured = realIsAiConfigured } = {}) {
  const spec = MODELS[model];
  const keys = spec ? spec.fields.filter((f) => fieldText(fields[f])) : [];
