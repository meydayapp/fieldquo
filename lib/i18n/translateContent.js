...
// hand-written translations in app/data/standardAddOns.fr.js, the same
// approach TrueFinish uses for its service catalogue. Only content a company
// writes itself needs drafting.
//
...
//
// Not on complete()'s schema mode: the key set varies per item (a row may or
// may not carry a description), and the reply is sometimes a bare array —
// strict structured outputs require an object root and forbid optional
// properties. applyDraftBatch() below already treats every field as
// untrusted and is executed against hostile replies by
// scripts/check-translation-draft.mjs. See lib/ai/jsonSchema.js.
 ...
 * rest in later.
 */
export async function translateFields(fields, from = "en", targets = [], { onUsage } = {}) {
  ...
  try {
    const text = await complete({
      ...
      prompt,
      maxTokens: 2000,
      // Metered by the caller when it passes one — the text-block translate
      // route does; the product routes that predate metering here do not,
      // and their calls are still counted nowhere, which is a known gap and
      // not one this parameter widens.
      onUsage,
    });
    if (!text) return {};
...
export function resolveProductText(product, language, defaultLanguage = "en") {
  ...
  // The company's own language needs no translation — unless the product was
  // DETECTED as written in another one (lib/i18n/autoTranslate.js records
  // `from` on every draft), in which case the drafted entry for the default
  // language is the one that reads right. Without it, an English-default
  // company's French product name printed in French on English quotes.
  const draftedFromElsewhere = entry?.name && entry.from && entry.from !== language;
  if (!language || (language === defaultLanguage && !draftedFromElsewhere)) {
    return { ...source, missing: false };
  }
  // …and a document in the language it was detected in reads the source as
  // written, not as a missing translation.
  const all = product?.translations && typeof product.translations === "object" ? Object.values(product.translations) : [];
  if (!entry?.name && all.some((e) => e?.auto && e.from === language)) return { ...source, missing: false };
