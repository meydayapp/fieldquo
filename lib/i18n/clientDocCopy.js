...
const COPY = {
  en: {
    ...
    // ── /f/[companySlug]/[funnelSlug] — the public lead funnel ────────────
    // Only what the funnel says and the self-quote form does not: the contact
    // placeholders, the upload control and the validation are read from
    // selfQuote below (lib/i18n/funnelCopy.js), so the two lead forms never
    // drift apart. Everything else on a funnel is the company's own copy.
    funnel: {
      unavailable: "This page isn't available right now.",
      progress: (n, total) => `Step ${n} of ${total}`,
      getStarted: "Get started",
      submit: "Submit",
      thanks: "Thanks!",
      estimateFailed: "Couldn't work that out just now.",
      // Says a minimum exists, never what it is — the floor is a rate.
      minimumApplied: "This job comes in under our minimum charge, so the minimum applies.",
      estimateNote: (company) =>
        `This is an estimate from the details you gave us, not a final quote. ${company} will confirm it before anything is binding.`,
      perVisit: "per visit",
      // The submit route's refusal for an address that cannot be delivered to
      // (lib/validation.js emailRefusal, code "invalid_email"). Its own sentence
      // is English, so the runner branches on the code and says this instead.
      errEmail: "That email address doesn't look right. Check it and try again.",
    },
  ...
  fr: {
    ...
    funnel: {
      ...
      submit: "Envoyer",
      thanks: "Merci !",
      estimateFailed: "Impossible de faire ce calcul pour le moment.",
      minimumApplied: "Ce travail est sous notre montant minimum, c'est donc le minimum qui s'applique.",
      estimateNote: (company) =>
        `Ceci est une estimation fondée sur les renseignements que vous nous avez donnés, et non une soumission finale. ${company} la confirmera avant tout engagement.`,
      perVisit: "par visite",
      errEmail: "Cette adresse courriel semble incorrecte. Vérifiez-la et réessayez.",
    },
  ...
  es: {
    ...
    funnel: {
      unavailable: "Esta página no está disponible en este momento.",
      progress: (n, total) => `Paso ${n} de ${total}`,
      getStarted: "Empezar",
      submit: "Enviar",
      thanks: "¡Gracias!",
      estimateFailed: "No pudimos calcularlo en este momento.",
      minimumApplied: "Este trabajo queda por debajo de nuestro cargo mínimo, así que se aplica el mínimo.",
      estimateNote: (company) =>
        `Esta es una estimación basada en los datos que nos dio, no un presupuesto final. ${company} la confirmará antes de que nada sea vinculante.`,
      perVisit: "por visita",
      errEmail: "Esa dirección de correo no parece correcta. Revísela e inténtelo de nuevo.",
    },
  ...
  uk: {
    ...
    funnel: {
      unavailable: "Ця сторінка зараз недоступна.",
      progress: (n, total) => `Крок ${n} з ${total}`,
      getStarted: "Почати",
      submit: "Надіслати",
      thanks: "Дякуємо!",
      estimateFailed: "Зараз не вдалося це розрахувати.",
      minimumApplied: "Ця робота менша за нашу мінімальну вартість, тому застосовується мінімум.",
      estimateNote: (company) =>
        `Це попередня оцінка на основі наданих вами даних, а не остаточний кошторис. ${company} підтвердить її, перш ніж щось стане обов'язковим.`,
      perVisit: "за візит",
      errEmail: "Ця адреса електронної пошти виглядає неправильно. Перевірте її та спробуйте ще раз.",
    },
  ...
  pa: {
    ...
    funnel: {
      unavailable: "ਇਹ ਪੰਨਾ ਇਸ ਵੇਲੇ ਉਪਲਬਧ ਨਹੀਂ ਹੈ।",
      progress: (n, total) => `${total} ਵਿੱਚੋਂ ਕਦਮ ${n}`,
      getStarted: "ਸ਼ੁਰੂ ਕਰੋ",
      submit: "ਭੇਜੋ",
      thanks: "ਧੰਨਵਾਦ!",
      estimateFailed: "ਇਸ ਵੇਲੇ ਇਸਦਾ ਹਿਸਾਬ ਨਹੀਂ ਲਗਾਇਆ ਜਾ ਸਕਿਆ।",
      minimumApplied: "ਇਹ ਕੰਮ ਸਾਡੀ ਘੱਟੋ-ਘੱਟ ਕੀਮਤ ਤੋਂ ਘੱਟ ਹੈ, ਇਸ ਲਈ ਘੱਟੋ-ਘੱਟ ਕੀਮਤ ਲਾਗੂ ਹੁੰਦੀ ਹੈ।",
      estimateNote: (company) =>
        `ਇਹ ਤੁਹਾਡੇ ਦਿੱਤੇ ਵੇਰਵਿਆਂ ਤੋਂ ਬਣਿਆ ਅਨੁਮਾਨ ਹੈ, ਅੰਤਿਮ ਕੋਟ ਨਹੀਂ। ਕੁਝ ਵੀ ਪੱਕਾ ਹੋਣ ਤੋਂ ਪਹਿਲਾਂ ${company} ਇਸਦੀ ਪੁਸ਼ਟੀ ਕਰੇਗਾ।`,
      perVisit: "ਪ੍ਰਤੀ ਫੇਰੀ",
      errEmail: "ਇਹ ਈਮੇਲ ਪਤਾ ਠੀਕ ਨਹੀਂ ਲੱਗਦਾ। ਇਸਨੂੰ ਜਾਂਚੋ ਅਤੇ ਦੁਬਾਰਾ ਕੋਸ਼ਿਸ਼ ਕਰੋ।",
    },
  ...
  tl: {
    ...
    funnel: {
      unavailable: "Hindi available ang pahinang ito sa ngayon.",
      progress: (n, total) => `Hakbang ${n} ng ${total}`,
      getStarted: "Magsimula",
      submit: "Ipadala",
      thanks: "Salamat po!",
      estimateFailed: "Hindi po namin ito makalkula sa ngayon.",
      minimumApplied: "Mas mababa ang trabahong ito sa aming minimum na singil, kaya ang minimum ang ilalapat.",
      estimateNote: (company) =>
        `Estimate ito batay sa mga detalyeng ibinigay ninyo, hindi pinal na quote. Kukumpirmahin ito ng ${company} bago maging pinal ang anuman.`,
      perVisit: "bawat pagbisita",
      errEmail: "Mukhang mali po ang email address na ito. Pakisuri at subukang muli.",
    },
  ...
  de: {
    ...
    funnel: {
      ...
      submit: "Absenden",
      thanks: "Danke!",
      estimateFailed: "Das konnten wir gerade nicht berechnen.",
      minimumApplied: "Dieser Auftrag liegt unter unserem Mindestbetrag, daher gilt der Mindestbetrag.",
      estimateNote: (company) =>
        `Dies ist eine Schätzung auf Grundlage Ihrer Angaben, kein endgültiges Angebot. ${company} bestätigt sie, bevor etwas verbindlich wird.`,
      perVisit: "pro Besuch",
      errEmail: "Diese E-Mail-Adresse scheint nicht zu stimmen. Bitte prüfen Sie sie und versuchen Sie es erneut.",
    },
  ...
  it: {
    ...
    funnel: {
      unavailable: "Questa pagina non è disponibile al momento.",
      progress: (n, total) => `Passaggio ${n} di ${total}`,
      getStarted: "Inizia",
      submit: "Invia",
      thanks: "Grazie!",
      estimateFailed: "Non è stato possibile calcolarlo in questo momento.",
      minimumApplied: "Questo lavoro è al di sotto del nostro importo minimo, quindi si applica il minimo.",
      estimateNote: (company) =>
        `Questa è una stima basata sui dati che ci ha fornito, non un preventivo definitivo. ${company} la confermerà prima che qualcosa diventi vincolante.`,
      perVisit: "a visita",
      errEmail: "Questo indirizzo email non sembra corretto. Lo controlli e riprovi.",
    },
