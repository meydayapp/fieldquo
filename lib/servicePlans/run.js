...
import { planOfferCopy, formatPlanPercent } from "@/lib/servicePlans/offerCopy";
...
const COMPANY_SELECT = {
  ...
  // Currency, Stripe flags and the offline methods with their addresses —
  // a plan's invoice email prints "How to pay" like any other.
...
/**
 * "Maintenance plan: 15% off every visit — $22.50 saved on this visit." In the
 * plan's language and the company's currency, from the figures this very
 * invoice carries — never a second calculation of the saving.
 */
export function planDiscountNote(plan, company, amounts) {
  const language = plan?.language || "en";
  const fmt = documentFormatters(language, company?.currency);
  const copy = planOfferCopy(language);
  return copy.invoiceNote(formatPlanPercent(Number(plan?.discountPct), fmt.locale), fmt.money(amounts.discount));
}
...
async function raiseInvoice({ plan, company, occurrence, amounts }) {
  ...
  const invoice = await db.invoice.create({
    data: {
      ...
      lineItems: [
        {
          // A plan sold from a quote with no services named carries its own
          // name as the service (lib/servicePlans/templates.js); printing it
          // twice — "Monthly Plan — Monthly Plan" — would read as a mistake.
          description:
            plan.serviceName && plan.serviceName !== plan.name
              ? `${plan.name} — ${plan.serviceName}`
              : plan.name,
          quantity: 1,
      ...
      // The recurring discount, SAID on the invoice as well as subtracted.
      // A bare "Discount −$22.50" row does not tell a client it is the plan
      // they signed up for doing it; this does, in the plan's language, on
      // every surface that prints invoice notes (PDF, portal, email).
      ...(amounts.discountCents > 0 ? { notes: planDiscountNote(plan, company, amounts) } : {}),
    },
    include: { client: true },
  });
...
async function emailInvoice({ plan, company, invoice, client, kind }) {
  ...
  const planLanguage = resolveClientLanguage({ document: invoice, client, company });
  const { subject, html, text } = buildInvoiceEmail({
    invoice,
    client,
    // Payment terms in the plan's language when a translation of the
    // company's current wording exists (lib/i18n/companyText.js).
    company: await localisedCompany(db, company, { companyId: plan.companyId, language: planLanguage }),
    url: portalInvoiceUrl(token, invoice.id),
    canTakeCard,
    kind,
    language: planLanguage,
    // "paid" is the receipt wording; anything else the invoice's.
    wording: await loadDocumentWording(db, {
      companyId: plan.companyId,
      kind: kind === "paid" ? "receipt" : "invoice",
      language: planLanguage,
    }),
  });
...
async function collectAutomatically({ plan, company, invoice, occurrence, amounts }) {
  ...
  if (result.outcome === "succeeded") {
    await recordStripePayment(db, {
      ...
      // The fee the intent was created with, read back rather than trusted
      // from `amounts` — see lib/stripe/paymentIntentFee.js.
      fee: await settledFeeOrNull(result.paymentIntent),
    });
    await db.servicePlanOccurrence.update({
      where: { id: occurrence.id },
      data: {
...
export async function settleOccurrenceFromIntent(intent) {
  ...
  if (intent.status === "succeeded") {
    await recordStripePayment(db, {
      ...
      fee: await settledFeeOrNull(intent),
    });
    await db.servicePlanOccurrence.update({
      where: { id: occurrence.id },
