...
//     the same `transfer_data.destination`, `on_behalf_of` and
//     `application_fee_amount` (the published processing fee, passed through
//     to the contractor — lib/stripe/processingFee.js) the pay link already
//     uses, from the same destinationChargeParams.
//
...
import { acssTransactionType } from "@/lib/stripe/bankDebit";
    ...
    params.payment_method_options = {
      acss_debit: {
        ...
        mandate_options: {
          ...
          // Shared with the one-off invoice debit — lib/stripe/bankDebit.js.
...
export async function chargeOccurrenceOffSession({
  ...
  try {
    // Fee, destination, settlement merchant, any outstanding account-fee
    // recovery, and the metadata that lets settlement split them — plus the
    // caller's own metadata (invoice, plan, occurrence ids).
    const route = await destinationChargeParamsWithRecovery(
      {
        company,
        amountCents,
        currency: stripeCurrency(company.currency),
        method: authorisation.paymentMethodType,
        metadata,
      },
      deps,
    );
    const intent = await stripe.paymentIntents.create(
      {
        ...
        // to the connected account, the contractor as settlement merchant, and
        // the processing fee for THIS method deducted. The method is pinned
        // above, so unlike the pay link the fee is exact at creation — a
        // pre-authorized debit is charged the 1% + 40¢ (max $5) debit rate,
        // never the card rate.
        ...route,
      },
