 ...
 * @param google GoogleReview rows the company switched on: { comment,
 *               reviewerName, starRating }. Rendered after the company's own
 *               rows, with "· Google" on the byline — the attribution the
 *               Business Profile policies require, and the reason the words
 *               arrive untouched.
 * @param limit  hard cap on what a host page can be made to render.
 * @returns [{ quote, author }] — author may be an empty string.
 */
export function mergeReviews({ rows = [], site = null, google = [], limit = 6 } = {}) {
  const out = [];
  const seen = new Set();
  ...
  for (const g of Array.isArray(google) ? google : []) {
    const stars = Number.isInteger(g?.starRating) && g.starRating >= 1 && g.starRating <= 5 ? g.starRating : null;
    push(
      g?.comment,
      [g?.reviewerName, stars ? `${"★".repeat(stars)}${"☆".repeat(5 - stars)}` : null, "Google"]
        .filter((s) => typeof s === "string" && s.trim())
        .join(" · "),
    );
  }
