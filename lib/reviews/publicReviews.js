...
export async function loadPublicReviews(companyId, { limit = PUBLISHED_LIMIT } = {}) {
  ...
  const [rows, site, google] = await Promise.all([
    ...
    // The Google reviews the company switched on, exactly as fetched and
    // attributed — lib/reviews/googleBusiness/sync.js says why they are a
    // cache and never a Testimonial. A row with no words is a rating alone
    // and cannot be quoted, so only reviews with a comment are read.
    db.googleReview.findMany({
      where: { companyId, showOnSite: true, comment: { not: null } },
  ...
  return mergeReviews({ rows, site, google, limit });
}
