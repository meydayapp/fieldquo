...
import { ensurePaymentScheduleForJob } from "@/lib/paymentSchedule/run";
import { taskForAcceptedQuote } from "@/lib/tasks/autoCreate";
import { fileAcceptanceDocuments } from "@/lib/jobs/documentAutofile";
 ...
 * @param {object} [opts]
 * @param {string|null} [opts.createdById]  the staff member, on the back-office door
 * @param {Buffer|null} [opts.signedPdf]    the signed PDF the public route has
 *   already rendered for the client's and the owner's emails. Handed on so the
 *   contract filed on the job is byte-for-byte the one they were sent, and so
 *   the acceptance renders it once, not twice.
 */
export async function onQuoteAccepted(quoteId, { createdById = null, signedPdf = null } = {}) {
  // A past job entered after the fact already has its job and its paid
  // invoice, and its acceptedAt is the real date the company typed. Nothing
  // below applies: the job and invoice helpers would find their rows and
  // stop, but the payment schedule would raise stages and EMAIL a deposit
  // request, and the task would ask someone to schedule work done in 2024.
  // One read here, so a status flip on such a row stays inert.
  try {
    const flagged = await db.quote.findUnique({
      where: { id: quoteId },
      select: { historicalImportedAt: true },
    });
    if (flagged?.historicalImportedAt) return { job: null, invoice: null };
  } catch (err) {
    console.error("[quoteLifecycle] historical check:", err?.message);
  }
  ...
  // A company with a structured payment schedule (Settings → Company →
  // Payment schedule) gets its stages created here — the SAME invoice
  // above, requested in shares rather than all at once. A company with no
  // schedule (every company, before this feature existed) sees NOTHING
  // change: ensurePaymentScheduleForJob finds no template and returns
  // immediately. See lib/paymentSchedule/run.js's own header for why this
  // sits alongside ensureInvoiceForQuote rather than replacing it.
  if (job && invoice) {
    try {
      await ensurePaymentScheduleForJob({
        jobId: job.id,
        quoteId,
        invoiceId: invoice.id,
      });
    } catch (err) {
      console.error("[quoteLifecycle] payment schedule:", err?.message);
    }
  }

  // ── What the job is born holding ─────────────────────────────────────
  //
  // The quote as sent, the quote as signed, and the homeowner's own photos.
  // The owner's words: "the job should have the original quote, the
  // approved contract and invoice as part of the documents", and the
  // quote's pictures should "land in the job too". Filed here, on the one
  // path both acceptance doors take, so a phone approval and a signed one
  // leave the same job behind. Neither helper throws — a render failure is
  // a PlatformErrorLog row (area "job_documents") and a job with one
  // document fewer, never an approval that looks failed.
  if (job) {
    try {
      await fileAcceptanceDocuments({ quoteId, jobId: job.id, byUserId: createdById, signedPdf });
    } catch (err) {
      console.error("[quoteLifecycle] job documents:", err?.message);
    }
    try {
      await fileQuotePhotosOnJob({ quoteId, jobId: job.id });
    } catch (err) {
      console.error("[quoteLifecycle] job photos:", err?.message);
    }

    // The plan: one step per approved line and per ticked option, ordered
    // with dependencies (lib/jobs/plan.js). Here, on the one path both
    // acceptance doors take, so the phone approval and the signed one leave
    // the same plan behind. Idempotent by sourceKey — a retried approval
    // adds nothing — and it never throws.
    await ensurePlanForJob(job.id, { byUserId: createdById });
  }

  // A maintenance plan on the quote — included, or ticked by the client —
  // becomes that client's running ServicePlan, billed per visit by the plan
  // engine. Here, on the path both doors take, so a phone approval and a
  // signed one leave the same plan behind. Idempotent per offer and never
  // throws (lib/servicePlans/fromQuote.js).
  await ensureServicePlansForQuote(quoteId, { createdById });
