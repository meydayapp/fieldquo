...
export async function performImport({
  ...
  // (the client must not see who the sub is). Fall back to a neutral trade label.
  const tradeLabel = String(label || "Subcontracted work").slice(0, 120);
...
export async function materializeImportedCosts(db, { quoteId, jobId, companyId = null, createdById = null }) {
  ...
  const imports = await db.quoteImport.findMany({
    ...
    where: { targetQuoteId: quoteId, expenseId: null, ...(companyId ? { targetCompanyId: companyId } : {}) },
