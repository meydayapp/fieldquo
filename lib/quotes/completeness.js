...
export function completenessChecks(quote, items) {
  ...
  // Lines with nothing under the name. The name is not the scope of the line
  // — "Soft-close hinges × 24" tells the client what it is called, not what
  // is done — and the AI review is shown the detail beside the name on
  // purpose (writingPass in lib/ai/quoteReview.js). Stated plainly here, in
  // the same words for every line, so "the review thinks it's empty" is
  // never a guess about the model: it is this check, naming the lines. Low
  // severity, not blocking — a scope paragraph above can carry the work for
  // a whole group, and a one-line quote with a good paragraph is still ready.
  ...
  if (bare.length) {
    ...
    add(
      ...
      `${named}${bare.length > 3 ? ` and ${bare.length - 3} more` : ""} — only the name reaches the client. A sentence on what is done, per what unit, is what they judge the price against.`,
    );
  }
