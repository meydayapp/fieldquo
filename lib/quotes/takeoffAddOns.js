...
import { PAINT_OPTIONAL_EXTRAS_CAP } from "@/lib/pricing/paintTakeoff";
  ...
  for (const g of groups) {
    ...
    for (const e of extras) {
      rows.push({
        ...
        // Which room the offer belongs under, so the client's page can draw
        // it beside the room. A label, never an index: the takeoff is edited
        // and re-synced, and a label survives a room being reordered.
        areaLabel: e.areaLabel ? String(e.areaLabel).trim().slice(0, 120) : null,
      });
    }
  }

  // Capped, so a 30-room takeoff with every room optional cannot produce a
  // 30-item shopping list on a homeowner's page. The cap is the takeoff's own
  // (PAINT_OPTIONAL_EXTRAS_CAP): higher than the add-ons editor's 8 because
  // per-area options draw under their room, not in one list at the foot.
  const capped = rows
    .filter((r) => r.description && Number.isFinite(r.amount) && r.amount > 0)
    .slice(0, PAINT_OPTIONAL_EXTRAS_CAP);
