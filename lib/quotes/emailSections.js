...
export function assertSectionFieldsLoaded(company = {}, quote = null) {
  ...
  // The company's default "what happens next" wording, for the same reason
  // and by the same rule: the email prints `quote.processNotes ||
  // company.defaultProcessNotes` (lib/quotes/completeness.js
  // effectiveProcessNotes, which the PDF and the client page already use), so
  // a select that leaves the column out silently drops the section from the
  // covering email of every company whose wording lives in Settings. A
  // missing KEY is the signal; a null value is a company that has written
  // none, which is a fact and not a mistake.
  if (!("defaultProcessNotes" in company)) missing.push("company.defaultProcessNotes");
  if (missing.length) {
    throw new Error(
