...
/**
 * The rate to seed a line item with, for a trade that has no price book.
 *
 * The company's own number when they set one, otherwise the catalogue's
 * opening rate — the read-time fallback TRADE_DEFAULT_RATES was written to be.
 *
 * It has to be applied HERE because Settings > Services stopped writing that
 * fallback into CompanyServiceCategory.defaultRate: doing so pinned four
 * trades to today's number the first time anyone pressed Save, and fed
 * FieldQuo's own opening rates back into the peer benchmark as though real
 * companies had chosen them (tradePriceBooks.js:1887). With the column now
 * honestly null for a company that never typed a rate, every reader of it
 * needs the fallback of its own — this is the one that seeds a quote line, so
 * an electrician who has never opened the rates screen still opens at $80/hour
 * rather than at zero.
 */
function categoryRate(category) {
  if (category?.defaultRate != null) return category.defaultRate;
  return defaultTradeRate(category?.key)?.rate ?? null;
}
...
export function newScopeGroup(
  ...
  return {
    ...
    ...(unitPriced
      ? {
          baseUnitPrice:
            ...
            num(categoryRate(category)),
          complexityLevel: "standard",
          complexityUpcharge: 0,
          complexityReasons: [],
          // NO factor-model `complexity` object here, on purpose. It was
          // seeded (every question on its benign answer) on the reasoning
          // that Standard adds nothing — true of the factor model alone,
          // false beside the chips: finalUnitPrice consults the factor model
          // FIRST, so a seeded Standard answered before the chip the
          // estimator had actually pressed, and Moderate (+$20) priced
          // nothing on every new cabinet group (owner's report, 2026-09-22).
          // Nothing renders the factor picker for cabinets yet
          // (app/components/pricing/ComplexityPicker.js); until it does, the
          // chips are the control on screen and they have to be the price.
          color: "",
          sheen: "",
          doorStyle: "",
    ...
    //
    // A takeoff trade starts with none either. Its price IS the calculator's
    // output (takeoffLinesFor, flattened in at save), so a seeded line under
    // it was a second, unrelated number for the same work: "Roofing — $525"
    // from CompanyServiceCategory.defaultRate sitting beneath a roof surface,
    // pitch and material form that had already priced the job. Every one of
    // the fourteen takeoff trades has a price book, and for a book trade
    // defaultRate is null on any company that set rates through Settings, so
    // the line the estimator saw was "$0.00" on a real tenant and the demo
    // seeder's headline rate on a demo — redundant either way, and on a demo
    // actively wrong. The table stays for genuine extras, exactly as it does
    // for a cabinet group.
    lineItems:
      isTiered || unitPriced || hasTakeoff(category?.key)
        ? []
        : [
            {
              description: label,
              ...
              rate: num(categoryRate(category)),
              amount: num(categoryRate(category)),
            },
          ],
  };
}
 ...
 */
export function takeoffLinesFor(group, rateOverrides = null, language = null) {
  ...
  const lines = buildTradeLineItems(
    ...
    group.takeoff,
    rateOverrides,
    // The QUOTE's language and the group's own name, for the complexity reason
    // lines and for the Specialty line that replaces the prices when a job is
    // one somebody has to go and look at. Both optional; a caller that has no
    // language in hand gets English, which is what every other derived line in
    // this file does when it is not told.
    { language: language || "en", label: group.label },
  );
 ...
 */
export function cabinetAddOnLinesFor(group, rateOverrides = null, language = null) {
  if (!group || group.persisted) return [];
  ...
  const lines = cabinetAddOnLines(
    {
      ...
      ...group,
      // The QUOTE's language, so the generated lines are written in the
      // language of the document they land on — see cabinetAddOnText.
      language,
    },
 ...
 *
 * The base (below) plus the estimator's own complexity factors, priced in the
 * order lib/pricing/customFactors.js documents: built-in complexity is
 * already inside the base, then custom %, then custom fixed and hours. A
 * group with no custom factor returns the base untouched — the same number,
 * to the cent, that this function returned before custom factors existed.
 */
export function groupSubtotal(group, rateOverrides = null) {
  const base = groupBaseSubtotal(group, rateOverrides);
  if (!hasCustomFactors(group)) return base;
  return round2(base + priceCustomFactors({ base, factors: group.customFactors }).total);
}

/**
 * A group's price BEFORE any custom factor — what a custom percentage is a
 * percentage of.
 *
 ...
 * group is always just the sum of its lines (the builder has already split
 * its custom-factor lines out — splitCustomFactorLines).
 */
export function groupBaseSubtotal(group, rateOverrides = null) {
  if (!group) return 0;
  ...
  return round2(
    takeoffSum +
      (isUnitPriced(group.categoryKey)
        ? // The book is passed so a group on the factor complexity model reads
          // the company's OWN Moderate/High per-unit figures. A group on the
          // old chips never looks at it and prices exactly as before.
          num(
            unitPricingSubtotal(
              group,
              getPriceBook(group.categoryKey, rateOverrides),
...
function withCabinetAnswers(group) {
  ...
  // The factor complexity, travelling with the rest of the cabinet answers for
  // the same reason they do: it is an answer about THIS kitchen, a cabinet
  // group has no `takeoff` column to put it in, and without a home here the
  // ticked factors would vanish at save and reopen unticked on a quote that
  // has them — the screen-stating-something-untrue-about-its-own-document
  // failure the cabinet answers were folded in here to fix.
  if (group?.complexity && typeof group.complexity === "object") {
    intake.complexity = group.complexity;
  }
  return intake;
}

export function scopeGroupPayload(group, rateOverrides = null, language = null) {
  let lineItems = Array.isArray(group?.lineItems) ? group.lineItems : [];

  if (!group?.persisted) {
    if (isUnitPriced(group?.categoryKey)) {
      const book = getPriceBook(group?.categoryKey, rateOverrides);
      const units = num(groupUnits(group));
      const rate = num(finalUnitPrice(group, book));
      // The factor module, or null for a group still on the chips. Resolved
      // once here and used twice below, so the reasons the client reads and
      // the level the office reads can never come from two different reads of
      // the same answers.
      const resolved = resolveComplexity({
        trade: group?.categoryKey,
        // On the group while the builder is open; one level down once it has
        // been round-tripped, because a cabinet group has no takeoff column
        // and withCabinetAnswers persists it beside the door count. Same
        // two-place read as app/data/cabinetPricing.js, same reason.
        complexity: group?.complexity || group?.intakeValues?.complexity,
        book,
        language: language || "en",
      });
      // Specialty refuses to price. The whole base scope becomes ONE unpriced
      // line saying so — a $0.00 per door would be a number a homeowner could
      // hold the contractor to, and 25 of them is a $0 kitchen.
      if (resolved && !resolved.priced) {
        lineItems = [
          specialtyLine(resolved, group.label),
          ...lineItems,
        ];
      } else {
        const factorDetail = resolved
          ? complexityDetailText(resolved, language || "en")
          : "";
        const base = {
          ...
          // What the client reads under the line on the quote, the PDF and the
          // email. Only the reasons — never the level, never the multiplier.
          ...(factorDetail ? { detail: factorDetail } : {}),
          ...
          meta: {
            ...
            // The factor module's own read, for the office. `meta` reaches no
            // client-facing renderer — ScopeGroupsSection reads description,
            // detail and amount and nothing else.
            ...(resolved
              ? {
                  complexity: {
        ...
        lineItems = [
          base,
          ...cabinetAddOnLinesFor(group, rateOverrides, language),
          ...lineItems,
        ];
      }
    }
    ...
    if (hasTakeoff(group?.categoryKey) && group?.takeoff) {
      lineItems = [
        ...takeoffLinesFor(group, rateOverrides, language),
        ...lineItems,
      ];
    }
  }

  // ── The estimator's own complexity factors, LAST ────────────────────────
  //
  // After every derived and typed line, persisted group or not: a factor
  // added while reopening a saved quote is as real as one added on a new
  // one, and its percentage is of the lines the group bills as they stand
  // (lib/pricing/customFactors.js). Nothing is appended for a group without
  // factors, so its payload is byte-identical to what it was before this
  // existed — scripts/check-custom-factors.mjs holds the md5s.
  if (hasCustomFactors(group)) {
    lineItems = [
      ...lineItems,
      ...customFactorLines({
        base: groupBaseSubtotal(group, rateOverrides),
 ...
 *
 * The builder writes ONE base line in "unit"s; an instant estimate writes the
 * doors and the drawer fronts as two (lib/estimate/estimateLines.js), each
 * carrying the same meta. Both are the faces the client is billed for.
 */
export function billedUnitsOf(group) {
  const lines = Array.isArray(group?.lineItems) ? group.lineItems : [];
  const total = lines
    .filter((l) => l && l.meta && ["unit", "door", "drawer"].includes(l.unit))
    .reduce((s, l) => s + (Number(l.quantity) > 0 ? Number(l.quantity) : 0), 0);
