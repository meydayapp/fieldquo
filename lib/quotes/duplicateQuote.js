...
export function duplicateQuoteData(source, { quoteNumber, userId, member }) {
  ...
  // Maintenance plans on the quote are offers too (QuotePlanOffer): the
  // frozen terms are copied, the client's tick and the plan it became are
  // not. A copied first-visit date that has passed by approval falls back to
  // "one cadence step after approval" (lib/servicePlans/templates.js).
  const planOffers = Array.isArray(source.planOffers)
    ? source.planOffers.map((o, i) => ({
        ...
        pricePerVisit: amount(o.pricePerVisit),
        discountPct: Number(o.discountPct || 0),
        taxRatePct: o.taxRatePct === null || o.taxRatePct === undefined ? null : Number(o.taxRatePct),
        sortOrder: i,
        selected: false,
        selectedAt: null,
        servicePlanId: null,
  ...
  return {
    ...
    // The job's address is WORK, not history: a copy of a GC's quote is
    // almost always for the same site, and a copy of a homeowner's quote is
    // for their house. The offline-payment offer is also copied — the copy
    // is priced as the original was; the client's CHOICE of it is not.
    siteAddress: source.siteAddress || null,
    offlineDiscountPct: source.offlineDiscountPct ?? null,
    ...
    ...(planOffers.length && { planOffers: { create: planOffers } }),
