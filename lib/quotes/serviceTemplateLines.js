...
//                     over the staircases
//   room measure      flooring_install, tiling, drywall_install — the rooms
//                     measured with paint's geometry (lib/measure/
//                     reuseTakeoffs.js): floorSqft, wallSqft (net of the
//                     openings), ceilingSqft, linearFt for the surfaces the
//                     trade ticks, the trade's generic areaSqFt, and
//                     drywallSheets
//   fence line        fence_services — the Linear Feet box (typed, or traced
//                     on the aerial still) as edgingFt and perimeterLf, and
//                     fencePosts; the gate boxes by their own names below
//   slab measure      concrete — the Square Footage box (typed or traced) as
//                     areaSqft and areaSqFt, and concreteCuYd at the intake's
//                     thickness and waste
...
//
// A reuse takeoff may also STATE a waste per material (the header of
// lib/measure/reuseTakeoffs.js): the figure then carries `wastePct`, and on
// material lines keyed to it that waste REPLACES the template line's own —
// never adds to it. A figure with no stated waste carries none, and every
// other calculator's figures never do, so their lines expand exactly as
// before (docs/ROADMAP.md 2026-09-25: md5 of every seeded template expanded
// into every existing trade, unchanged).
//
...
import { expandTemplate, templatesFor, MAX_WASTE_PCT } from "@/lib/services/templates";
...
export const CALCULATORS = Object.freeze({
  ...
  // The reuse takeoffs (lib/measure/reuseTakeoffs.js) — they measure, they
  // do not price, so none of them appears in CALCULATOR_PRICED_KEYS.
  roomMeasure: { trades: ["flooring_install", "tiling", "drywall_install"] },
  fence: { trades: ["fence_services"] },
  concrete: { trades: ["concrete"] },
});
 ...
 */
// A reuse takeoff is listed AFTER the calculator that already filled a key,
// so a quote that has both still names the one it always named.
const KEY_CALCULATORS = Object.freeze({
  wallSqft: ["paint", "siding", "roomMeasure"],
  ceilingSqft: ["paint", "roomMeasure"],
  floorSqft: ["paint", "flooring", "roomMeasure"],
  linearFt: ["paint", "roomMeasure"],
  squares: ["roof"],
  areaSqft: ["roof", "concrete"],
  valleyFt: ["roof"],
  perimeterFt: ["roof"],
  ...
  lotSize: ["lot"],
  edgingFt: ["lot", "fence"],
  areaSqFt: ["roomMeasure", "concrete"],
  perimeterLf: ["fence"],
  drywallSheets: ["roomMeasure"],
  fencePosts: ["fence"],
  gateCount: ["fence"],
  driveGateCount: ["fence"],
  concreteCuYd: ["concrete"],
  bedrooms: ["cleaning"],
  bathrooms: ["cleaning"],
...
/**
 * The figures ONE group carries, as { key: { value, calc, wastePct? } } —
 * `wastePct` only where a reuse takeoff states one (see the header).
 */
export function groupMeasurements(group) {
  const out = {};
  if (!group || typeof group !== "object") return out;
  const put = (key, value, calc, { zeroIsFigure = false, wastePct } = {}) => {
    const v = Number(value);
    if (!Number.isFinite(v) || v < 0) return;
    if (v === 0 && !zeroIsFigure) return;
    if (!Object.hasOwn(out, key)) {
      out[key] = { value: Math.round(v * 100) / 100, calc, ...(typeof wastePct === "number" ? { wastePct } : {}) };
    }
  };
  const t = group.takeoff && typeof group.takeoff === "object" ? group.takeoff : null;
  const key = group.categoryKey;
  ...
  // The reuse takeoffs. Rooms live on the group's takeoff; the fence and the
  // slab are intake boxes the tracer fills (or the estimator typed).
  if (t && isRoomMeasureTrade(key)) {
    for (const [k, e] of Object.entries(roomMeasureMeasurements(t, key))) put(k, e.value, "roomMeasure", { wastePct: e.wastePct });
  }
  if (key === "fence_services") {
    for (const [k, e] of Object.entries(fenceMeasurements(group.intakeValues))) put(k, e.value, "fence", { wastePct: e.wastePct });
  }
  if (key === "concrete") {
    for (const [k, e] of Object.entries(concreteMeasurements(group.intakeValues))) put(k, e.value, "concrete", { wastePct: e.wastePct });
  }
  ...
  for (const g of ordered) {
    ...
    for (const [k, v] of Object.entries(m)) {
      ...
      sources[k] = {
        value: v.value,
        calc: v.calc,
        groupTempId: g.tempId ?? null,
        groupLabel: String(g.label || ""),
        ...(typeof v.wastePct === "number" ? { wastePct: v.wastePct } : {}),
      };
    }
  }
...
export function expandServiceTemplate(product, {
  ...
  const all = expandTemplate(withJobWaste(product, sources), {
    measurements: values,
    currency,
    fromCurrency,
    fx,
...
/** The waste a figure's takeoff states, or null (the template line's own applies). */
function jobWaste(sources, key) {
  const s = key && Object.hasOwn(sources || {}, key) ? sources[key] : null;
  const w = s && typeof s.wastePct === "number" ? s.wastePct : null;
  return w !== null && Number.isFinite(w) && w >= 0 ? w : null;
}

/**
 * The product with each material line's waste replaced by the waste its
 * figure's takeoff states — the same Product row, untouched, when no figure
 * states one (every calculator but the reuse takeoffs), so those expansions
 * are byte for byte what they were. expandTemplate's sanitiser clamps and
 * rounds the value like any template waste; 0 leaves no waste on the line.
 */
function withJobWaste(product, sources) {
  const lines = product?.templateLines;
  if (!Array.isArray(lines)) return product;
  if (!Object.values(sources || {}).some((s) => s && typeof s.wastePct === "number")) return product;
  return {
    ...product,
    templateLines: lines.map((l) => {
      if (!l || typeof l !== "object" || l.kind !== "material" || typeof l.measurementKey !== "string") return l;
      const w = jobWaste(sources, l.measurementKey);
      return w === null ? l : { ...l, wastePct: w };
    }),
  };
}
...
export function refillTemplateRun(items, runId, measurements) {
  ...
  return list(items).map((item) => {
    ...
    // The takeoff's stated waste when it states one (as at add time), else
    // the waste the line was added with.
    const stated = jobWaste(sources, m.measurementKey);
    const waste = m.lineKind === "material" ? (stated !== null ? Math.min(MAX_WASTE_PCT, stated) : num(m.wastePct)) : 0;
