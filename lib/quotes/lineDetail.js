...
export function lineFromProduct(product, { language = null, defaultLanguage = "en" } = {}) {
  ...
  // Product.costPrice was written by the products screen and the CSV import
  // and read by nothing on a quote; it is the line's opening unit cost now
  // (lib/costing/lineItemCost.js). Only when the row states one — a product
  // with no cost adds no key, so the line is exactly what it was.
  const unitCost = round2(product?.costPrice);
  return {
    ...
    ...(detail ? { detail } : {}),
    ...(unitCost > 0 ? { unitCost } : {}),
    // Which price-book item this line is, so its commission override
    // (Product.workedByPct/soldByPct, "the item beats the member") can find
    // it on the invoice — lib/commissions/compute.js#lineProductId. Office
    // only, like unitCost: no client renderer reads it, and a line typed by
    // hand simply has none. Only when the row has an id, so a fixture or a
    // not-yet-saved product adds no key.
