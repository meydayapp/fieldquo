...
export const SURFACES = Object.freeze(["funnel", "instant_quote"]);
...
export function stepRank(surface, stepKey, funnelSteps = []) {
  ...
  if (surface === "instant_quote") {
    const i = INSTANT_QUOTE_STEPS.indexOf(stepKey);
    return i >= 0 ? i : null;
  }
  if (surface === "funnel") {
...
export function reportSteps(surface, funnelSteps = []) {
  if (surface === "instant_quote") return INSTANT_QUOTE_STEPS.map((key, rank) => ({ key, rank }));
  const ids = funnelStepIds(funnelSteps);
