...
/**
 * One thread's score from its own messages, with the stored paid reading
 * re-applied — computed, NOT written.
 *
 * `thread` needs `quoteId`, `scoreReasons` and `messages` (direction, private,
 * body, sentAt, failedReason). The write path below and the coaching route
 * (app/api/messaging/threads/[id]/coach) both call this, so the likelihood the
 * coach panel shows is the same sum the stored column holds — never a second
 * scorer that could drift from the chip in the inbox.
 *
 * @returns {{ rule, scored, storedAi, context }}
 */
export async function scoreThreadFresh(companyId, thread, { db = defaultDb, now = new Date() } = {}) {
  ...
  return { rule, scored: applyAiRead(rule, previousAi), storedAi: previousAi, context };
}
...
export async function rescoreThread({ threadId, companyId, db = defaultDb, now = new Date() }) {
  ...
  const { scored } = await scoreThreadFresh(companyId, thread, { db, now });
