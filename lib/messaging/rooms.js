...
import { deliveryVerdict } from "@/lib/sms/deliveryStatus";
...
export function messageItem(m) {
  ...
  // A text's carrier receipt (SmsDelivery, attached by the thread route on an
  // SMS thread). A carrier failure turns the bubble exactly as a refused send
  // does — the homeowner did not get it either way — even before the status
  // callback has written failedReason onto this row. The screen translates
  // the code; `error` stays the stored sentence for everything else.
  const receipt = out && m.sms ? { verdict: deliveryVerdict(m.sms), errorCode: m.sms.errorCode ?? null } : null;
  return {
    id: m.id,
    kind: "message",
    ...
    status: m.failedReason || receipt?.verdict === "failed" ? "failed" : "sent",
