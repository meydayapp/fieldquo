...
import { listPageConversations, nextConversationCursor } from "@/lib/meta/client";
...
/** Conversations per Graph page. Meta's default is 25; kept explicit so the cursor loop is bounded. */
const PAGE_SIZE = 25;
 ...
 *                     platforms: string[], importedAt: Date|null }>}
 *
...
export async function pageImportState(companyId) {
  const none = { available: false, granted: { facebook: false, instagram: false }, platforms: [], importedAt: null };
  ...
  return { available: true, reason: null, granted, platforms, importedAt };
}
 ...
 *                (default: IMPORT_DEFAULT_SINCE_DAYS ago). The CONVERSATION
...
export async function importPageConversations({
  ...
  const sinceAt =
    since instanceof Date ? since : new Date(now.getTime() - IMPORT_DEFAULT_SINCE_DAYS * 24 * 60 * 60 * 1000);
  ...
  for (const channel of channels) {
    ...
    while (!stop && counts.conversations < limit) {
      const res = await fetchConversations({
        ...
        after,
        limit: Math.min(PAGE_SIZE, limit - counts.conversations),
      });
      if (!res?.ok) {
        counts.errors += 1;
        result.errors += 1;
 ...
 *
 * Guarded on the stamp — a company whose channels have EVER been imported is
 * not imported again by a reconnect (the manual refresh is for that, and it
 * says so). Never throws: the callers are the OAuth callback, the Page-picker
...
export async function importAfterConnect({ companyId } = {}) {
  try {
    const state = await pageImportState(companyId);
    if (!state.available || state.importedAt) return null;
    const result = await importPageConversations({ companyId });
    if (result.lastError) {
      await recordError({
        area: "webhook",
