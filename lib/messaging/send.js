...
import { sendWebChatMessage, sendSmsChatMessage } from "./ownSend";
 ...
 * @param lastInboundAt        MessageThread.lastInboundAt. The 24-hour window
 *                             on WhatsApp, Facebook and Instagram is computed
 *                             from it (Facebook and Instagram since
 *                             2026-09-22). Passed by every caller rather
...
export async function sendOnChannel({
  ...
  // ── FieldQuo's own two channels ─────────────────────────────────────────
  //
  // Before the WhatsApp branch, because "not Meta's Send API" used to mean
  // "WhatsApp" and now means one of three things. Both are text-only: a
  // template, a photo or a pin on either is refused by NAME below rather
  // than silently downgraded, the same rule the Messenger branch keeps.
  // ── An email conversation (lib/mailbox/) ────────────────────────────────
  //
  // Refused by name. A person's reply to an email thread is sent as an email
  // by the reply route itself (lib/mailbox/reply.js), which needs the thread's
  // subject and References that this router is not given; the only other
  // caller, the AI employee, must never answer email at all. Without this
  // branch an email thread would fall through to the Meta send below and fail
  // with a sentence about Facebook.
  if (platform === "email") {
    return {
      ok: false,
      reason: "email_not_via_channel",
      message: "An email conversation is answered from the inbox, as an email.",
    };
  }

  if (platform === "web" || platform === "sms") {
    if (kind !== "text") {
      return {
        ok: false,
        reason: `${kind}_unsupported`,
        message:
          platform === "web"
            ? "The website chat carries text only."
            : "A text message carries text only.",
      };
    }
    return platform === "web"
      ? sendWebChatMessage({ text })
      : sendSmsChatMessage({ channel, recipientExternalId, text });
  }
