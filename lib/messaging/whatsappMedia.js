...
/**
 * The message object a LOCATION send posts.
 *
 * ══ What Meta documents ════════════════════════════════════════════════════
 *
 * developers.facebook.com/docs/whatsapp/cloud-api/guides/send-messages, read
 * 2026-09-08: a location message is `{ type: "location", location: { … } }`
 * with `latitude` and `longitude` REQUIRED and `name` and `address` optional.
 * It is a service message like any other, so the 24-hour customer service
 * window applies to it exactly as it applies to a typed reply — which is why
 * nothing here checks the window: serviceWindowRefusal in
 * lib/messaging/whatsappSend.js has already refused it by then, with the same
 * `service_window_closed` reason a text send gets.
 *
 * ══ Why the coordinates never come from the browser ════════════════════════
 *
 * They are read from the company's own row, server-side, in the reply route.
 * The browser names the intent ("send our address") and the server resolves
 * the value — AGENTS.md non-negotiable #5 applied to something that is not
 * money: a request that could name arbitrary coordinates would be a way to
 * send a homeowner a pin on someone else's house from a company's own number.
 *
 * Strings, not numbers, because that is what Meta's own examples show and a
 * Decimal column stringifies without a rounding decision being made here.
 */
export function whatsAppLocationPayload({ latitude, longitude, name = null, address = null }) {
  const location = {
    latitude: String(latitude),
    longitude: String(longitude),
  };
  // Written as a list of what IS present rather than always sending the keys:
  // an empty `name` renders in WhatsApp as a blank line above the address.
  if (name) location.name = String(name).slice(0, 120);
  if (address) location.address = String(address).slice(0, 300);
