...
export function waitedLabel(waitingSince, now = new Date()) {
  ...
  // `n`, because that is the placeholder the nine catalogues carry ("{n}
  // min"). This returned `count` until 2026-09-12 and the badge read
  // "Waiting {n} min" on the live inbox — nothing executed this function
  // against the catalogue. scripts/check-app-messages-kit.mjs does now.
  if (minutes < 1) return { key: "app.messages.waiting.justNow", params: {} };
  if (minutes < 60) return { key: "app.messages.waiting.minutes", params: { n: Math.round(minutes) } };
  if (minutes < 60 * 24) {
    return { key: "app.messages.waiting.hours", params: { n: Math.round(minutes / 60) } };
  }
  return { key: "app.messages.waiting.days", params: { n: Math.round(minutes / (60 * 24)) } };
}
