...
import { whatsAppMediaPayload, whatsAppLocationPayload, uploadWhatsAppMedia } from "./whatsappMedia";
 ...
 * @param {string}  kind                 "text" | "template" | "media" |
 *                                       "location"
 * @param {object}  template             a WhatsAppTemplate row, for "template"
 * @param {object}  media                for "media": { type, mimeType, buffer,
 *                                       filename, caption }. The BYTES, not a
 *                                       link — this function uploads them to
 *                                       Meta itself, after the window refusal,
 *                                       and sends by the id that comes back
 * @param {object}  location             for "location": { latitude, longitude,
 *                                       name, address }, resolved by the route
 *                                       from the COMPANY's own row — never
 *                                       coordinates a browser named
...
export async function sendWhatsAppMessage({
  ...
  if (kind === "template") {
    ...
    // ── A picture obeys the window, and by the SAME name ──────────────────
    //
    // Nothing special happens here for that. `kind` is "media", not
    // "template", so serviceWindowRefusal ABOVE has already refused it with
    // `service_window_closed` and the same sentence a typed reply gets —
    // which is the point: a photo outside the window is refused for the one
    // reason it is actually refused for, not a second one invented beside it.
    // Meta agrees; a media send outside the window is the same 131047, and
    // classifyWhatsAppError maps it onto the same string.
    //
    // ── Why the UPLOAD happens here and not in the route ──────────────────
    //
    // Because it must happen AFTER the refusals. POST /{phone-number-id}/media
    // is a real Graph call with a real multi-megabyte body; doing it in the
    // route would mean a contractor's photo went up to Meta and was then
    // refused for a closed window, wasting the upload they waited on and
    // leaving an orphan media id behind. The order is the guard.
    if (!media?.type || !media?.buffer) {
      // Reachable only from a caller that handed over nothing. Named rather
      // than posted, because an empty media object is a Meta 400 that reads
      // like a broken connection.
      return {
        ok: false,
        reason: "media_missing",
        message: "There was no file to send.",
      };
    }
    ...
    // ── A pin obeys the window too, and by the SAME name ──────────────────
    //
    // `kind` is "location", not "template", so serviceWindowRefusal above has
    // already refused it with `service_window_closed` and the identical
    // sentence a typed reply gets. Nothing type-specific is invented here,
    // which is the point: one rule, one reason, one thing to read on the
    // failed bubble whichever kind it was.
    //
    // No upload and no Graph round trip before this point, so a refusal costs
    // nothing at all — the ordering guard the media branch above needs is
    // free here.
    if (!location || !Number.isFinite(Number(location.latitude)) || !Number.isFinite(Number(location.longitude))) {
      // Reachable only from a caller that handed over nothing usable. Named
      // rather than posted: Meta answers a malformed location with a generic
      // 400 that reads like a broken connection.
      return {
        ok: false,
        reason: "location_missing",
        message: "There was no place to send — this company has no address with map coordinates on file.",
      };
    }
    message = whatsAppLocationPayload(location);
  } else {
