...
import { isMessagingPlatform, META_PLATFORMS } from "./platforms";
...
export function publicChannelShape(channel) {
  ...
  return {
    ...
    // "embedded_signup" | "manual" | null. Null is "not recorded" — every row
    // written before the column existed — and the panel prints nothing for
    // it rather than guessing which door an old token came through.
    connectedVia: channel.connectedVia || null,
...
/** A company's live META channels — disconnected ones excluded, never
 *  deleted. Meta's three only: the web-chat and SMS rows the AI employee
 *  creates (lib/aiEmployee/ownChannel.js) are not "connected to Meta" and
 *  must not make messagingConnection() say a Page is. */
export async function listChannels(companyId) {
  return db.messagingChannel.findMany({
    where: { companyId, disconnectedAt: null, platform: { in: [...META_PLATFORMS] } },
...
export async function saveChannel({
  ...
  displayPhoneNumber,
  verifiedName,
  // Which door the credential came through — see the schema comment on
  // MessagingChannel.connectedVia. Undefined on a Page connect for the same
  // reason as the three above; a WhatsApp reconnect through the OTHER door
  // overwrites it, because the row now holds that door's token.
  connectedVia,
}) {
  if (!isMessagingPlatform(platform) || !META_PLATFORMS.includes(platform)) {
    // Refused rather than stored. Meta's three only: this function encrypts a
    // Meta token, and FieldQuo's own web/sms channels have none — they are
    // created by lib/aiEmployee/ownChannel.js, which never comes here. `platform` is a plain String column and the
