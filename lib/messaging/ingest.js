...
import { pushInboundMessage } from "./pushInbound";

/**
 * @param {object} event  one parsed event from lib/messaging/envelope.js, or
 *                        the identical shape lib/messaging/pageImport.js builds
 *                        from the Conversations API (with `imported: true`)
...
export async function ingestEvent(event) {
  ...
  //
  // And never for an IMPORTED message (lib/messaging/pageImport.js stamps
  // `imported: true` on every event it builds). A pull of the last thirty
  // days' conversations is history, not a homeowner writing now: drafting a
  // reply to a question from three weeks ago — and paying for it — would be
  // the employee answering the past. The question of whether those threads
  // still need a reply is answered by their status and the waiting badge,
  // which the same ingest sets exactly as for a live delivery.
  let ai = null;
  // `noAutoReply` is the shared-SMS-line case (lib/aiEmployee/smsChannel.js):
  // a phone on file at more than one company is stored at each and answered
  // by none, because no tenant can be sure the text was for them. The staff
  // push below still goes out — a person must see it.
  if (created && event.direction === "in" && !event.imported && !event.noAutoReply) {
    ai = await aiEmployeeOnInbound({
      companyId,
      threadId: thread.id,
      messageId: stored?.id || null,
      // The channel's own platform decides WHICH employee answers — "web"
      // and "sms" are FieldQuo's own channels and route to the employee
      // whose switch for them is on (lib/aiEmployee/employees.js); Meta's
      // three route to the Meta one. The visitor's language rides along for
      // the disclosure line.
      platform: channel.platform,
      language: event.language || null,
    }).catch(() => null);
    // The push copy — "New message from Maria Lopez" to every member who
    // can read the inbox, on the same "genuinely new, not imported" terms as
    // the employee above, and for the same reason: history is not a
    // homeowner writing now. Not awaited: lib/messaging/pushInbound.js.
    void pushInboundMessage({
      companyId,
      threadId: thread.id,
      participantName: event.participantName || thread.participantName || null,
      body: event.body || "",
    });
  }
  ...
  // `ai` is advisory, for FieldQuo's own channels: the web widget reads it
  // to decide between "here is the answer" and "someone will reply shortly".
  return { handled: true, created, threadId: thread.id, companyId, ai };
}
