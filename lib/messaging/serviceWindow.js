...
//
// The 24-hour customer service window on Meta's three platforms, as a pure
// function. Written for WhatsApp first (everything quoted below is WhatsApp's
// documentation); Facebook Messenger and Instagram were added on 2026-09-22 —
// see SERVICE_WINDOW_PLATFORMS in ./platforms.js for why, and for the one
// thing that differs: no template, so no way through once it closes.
//
...
import { SERVICE_WINDOW_PLATFORMS, offersTemplates } from "./platforms";
...
export function serviceWindowRefusal({ platform, lastInboundAt, kind = "text", now = new Date() }) {
  // Platforms with no window (FieldQuo's own web chat and SMS, a filed email)
  // are not refused by this function. Facebook and Instagram ARE, since
  // 2026-09-22: metaSend.js still sends `messaging_type: "RESPONSE"` and Meta
  // still has the last word, but a send Meta was always going to refuse is
  // now refused here first, by name, the way WhatsApp's always was.
  ...
  return {
    ...
    // Two sentences per platform, because the two reasons need two different
    // actions from the contractor. Each names what still works — a refusal
    // that only says no is a dead end with a polite face on it. On WhatsApp
    // that is a template; on Facebook and Instagram there is no way through
    // inside FieldQuo (no HUMAN_AGENT permission — see TEMPLATE_PLATFORMS),
    // so it is the customer writing again, or a call or an email.
    message: refusalSentence(platform, state.reason),
    detail: state.reason,
    closesAt: state.closesAt,
  };
}

function refusalSentence(platform, reason) {
  if (offersTemplates(platform)) {
  ...
  return reason === "never_opened"
    ? `${name} only lets businesses reply within 24 hours of the customer's last message, and this person has not messaged you there, so nothing was sent. If they write, you can reply; until then, call or email them.`
    : `${name} only lets businesses reply within 24 hours of the customer's last message, and more than 24 hours have passed, so nothing was sent. The conversation reopens when they write again; until then, call or email them.`;
}
 ...
 *             wayThrough: "template"|"contact"|null }}
 *
 * `wayThrough` is what the composer offers once the window has closed:
 * "template" (WhatsApp — the picker) or "contact" (Facebook, Instagram — the
 * customer writes again, or the contractor calls or emails; the composer
 * links the client record when there is one). Carried on the notice rather
 * than inferred by the page from the platform name, so the page draws what
 * the server decided and a new platform cannot inherit a template picker by
 * accident.
 *
...
export function serviceWindowNotice({ platform, lastInboundAt, now = new Date() }) {
  if (!needsServiceWindow(platform)) {
    return { open: true, closesAt: null, msRemaining: null, reason: null, blockKey: null, warnKey: null, hours: null, wayThrough: null };
  }

  const keys = WINDOW_KEYS[platform] || WINDOW_KEYS.whatsapp;
  const wayThrough = offersTemplates(platform) ? "template" : "contact";
  const state = serviceWindowState(lastInboundAt, now);
  if (!state.open) {
    return {
      ...state,
      blockKey: state.reason === "never_opened" ? keys.neverOpened : keys.closed,
      warnKey: null,
      hours: null,
      wayThrough,
    };
  ...
  return {
    ...state,
    blockKey: null,
    warnKey: hours < 1 ? keys.closingSoon : null,
    hours,
    wayThrough,
  };
}

// Each platform's three sentences. WhatsApp's name a template; Facebook's and
// Instagram's say why ("only lets businesses reply within 24 hours of the
// customer's last message") and what still works (they write again; a call
// or an email). Literal keys so scripts/check-translations.mjs and the checks
// see every one as a string that must exist in nine languages.
const WINDOW_KEYS = Object.freeze({
  whatsapp: {
    closed: "app.messages.window.closed",
    neverOpened: "app.messages.window.neverOpened",
    closingSoon: "app.messages.window.closingSoon",
  },
  facebook: {
    closed: "app.messages.window.facebook.closed",
    neverOpened: "app.messages.window.facebook.neverOpened",
    closingSoon: "app.messages.window.facebook.closingSoon",
  },
  instagram: {
    closed: "app.messages.window.instagram.closed",
    neverOpened: "app.messages.window.instagram.neverOpened",
    closingSoon: "app.messages.window.instagram.closingSoon",
  },
});
