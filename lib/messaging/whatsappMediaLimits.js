...
/**
 * What the composer may send.
 *
 * Sticker is absent deliberately: FieldQuo has no sticker picker, so offering
 * the type would be a control with nothing behind it. Audio is absent for the
 * same reason — there is no recorder in the composer, and an "attach an audio
 * file" affordance nobody asked for is not the gap this change closes.
 */
...
/** The `accept` attribute for the composer's file input. Kept in step with the
 *  sets above: a picker that offers a type the send refuses is a control that
 *  appears to work and doesn't. */
export const WHATSAPP_MEDIA_ACCEPT =
  "image/jpeg,image/png,image/heic,image/heif,image/webp,image/gif,video/mp4,video/3gpp,application/pdf";
...
export function classifyWhatsAppOutboundMedia(file = {}) {
  ...
  if (!isImage && !isVideo && !isDocument) {
    return {
      ...
      message:
        "WhatsApp accepts JPEG and PNG pictures (5 MB), MP4 or 3GP video (16 MB) and PDFs. " +
