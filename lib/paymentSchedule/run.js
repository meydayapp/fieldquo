...
const COMPANY_SELECT = {
  ...
  // the deposit email prints "How to pay" like any invoice email.
...
export async function requestStagePayment(stageId, deps = {}) {
  ...
  const stageLanguage = resolveClientLanguage({ document: stage.invoice, client, company });
  // The stage's name heads the email ("Deposit"). It is the company's
  // schedule wording, copied onto this job at acceptance and drafted into
  // the other languages when the schedule was saved (lib/i18n/phrases.js,
  // keyed by the text, so the copy finds it). No draft → as written.
  const trStage = await loadPhrases(prisma, stage.companyId, stageLanguage, [{ ns: "paymentStage", text: stage.label }]);
  const { subject, html, text } = buildInvoiceEmail({
    ...
    note: trStage("paymentStage", stage.label),
    language: stageLanguage,
    // The deposit / stage request wording, when the company customised it.
  ...
  // Checked, because sendEmail returns its failures where the old
  // `resend.emails.send()` threw them. Everything below records that the
  // client was asked for money — sentAt, status "sent", the stage marked
  // "requested" and therefore never retried. Writing that on a send that
  // didn't happen leaves a deposit nobody will ever chase.
  if (sent?.error || sent?.skipped) {
    return {
      fired: false,
      reason: "send_failed",
      error: sent?.skipped ? "email_not_configured" : sent.error,
    };
  }
...
export async function runPaymentSchedule({ now = new Date() } = {}) {
  const jobIds = await db.jobPaymentStage.findMany({
    // A past job entered after the fact never gets stages (onQuoteAccepted
    // skips it), but a stage that somehow points at one must not email a
    // deposit request for work paid in full two years ago.
    where: { status: "pending", job: { historicalImportedAt: null } },
    select: { jobId: true },
    distinct: ["jobId"],
  });
