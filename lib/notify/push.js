...
//
// Money never travels in a push — with one deliberate exception. A
// notification sits on a lock screen; the feed row's amount is withheld
// from readers without showPricing and the push says less than the row,
// never more. The exception is the rep's own pay: when a superadmin marks a
// payout batch paid, lib/sales/payoutProof.js pushes "Your week of … was
// paid — $X" to that rep alone, on the owner's instruction. It is the rep's
// own money on the rep's own device, and nobody else's figure is in it.
