...
export async function taskForJobCostReview(jobId) {
  ...
  try {
    const job = await db.job.findUnique({
      where: { id: jobId },
      select: { id: true, companyId: true, clientId: true, title: true },
    });
    if (!job) return null;
    return await createOnce({
      companyId: job.companyId,
      sourceKey: jobCostReviewKey(job.id),
      title: `Review what "${job.title}" actually cost`,
      description:
        "Approve the hours, tag the receipts and materials, and confirm the job's real cost — the estimate is only worth comparing against a complete actual.",
