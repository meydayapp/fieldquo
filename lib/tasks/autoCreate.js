 ...
 *
 * Exported for lib/jobs/buildPlan.js, whose steps are born on the same
 * session-less path (a client's approval) and need the same answer. `prisma`
 * is that file's injection seam, passed through.
 */
export async function fallbackAuthorId(companyId, prisma = db) {
  ...
  try {
    const job = await db.job.findUnique({
      ...
      select: {
        ...
        clientId: true,
        quoteId: true,
        // Excluded rows (a line the office removed from an AI-built list)
        // are neither to buy nor bought — see JobMaterial.excludedAt.
        materials: { where: { excludedAt: null }, select: { purchasedAt: true } },
      },
    });
