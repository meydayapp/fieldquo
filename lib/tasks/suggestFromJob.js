...
You return a JSON object with one key, "tasks": a list of
{"title":"...","because":"...","priority":"normal","dueInDays":null}.
An empty list is a correct and common answer.`;

/**
 * The suggestion shape, enforced at the vendor with `strict: true`.
 *
 * ── Why the list is wrapped in an object ──────────────────────────────────
 *
 * The prompt used to ask for a bare JSON array. Strict structured outputs
 * require the ROOT to be an object — not an array, not an anyOf — so the
 * reply is now `{ "tasks": [...] }` and the prompt says so. That is a real
 * behaviour change to a prompt, made deliberately and named here rather than
 * slipped in: it is the one schema rule that cannot be worked around.
 *
 * ── The one number, and why it is allowed to be one ───────────────────────
 *
 * `dueInDays` is the only numeric field in any schema in this codebase, and it
 * is worth saying why it does not break the rule that a schema must never make
 * the model the arithmetic. It is not a measurement, a price, a quantity or a
 * total; it is a PROPOSED offset attached to a suggestion a person reads and
 * accepts or discards, and it is range-checked below (whole days, 0-365)
 * because `minimum` and `maximum` are not in the strict subset. Nothing is
 * computed from it and nothing is shown to a client because of it. A field
 * that failed any of those tests would belong in code instead.
 */
  ...
  properties: {
    tasks: {
      ...
      items: {
        type: "object",
        properties: {
          title: { type: "string", description: "Short, imperative, under 60 characters." },
          because: { type: "string", description: "The exact phrase from the input this came from, verbatim." },
          priority: { type: "string", enum: ["urgent", "high", "normal", "low"] },
          dueInDays: {
            type: ["integer", "null"],
            description: "Whole days from today, or null when the notes imply no timing.",
          },
        },
        required: ["title", "because", "priority", "dueInDays"],
...
export async function suggestTasksForJob({ jobId, companyId, onUsage }) {
  ...
  const result = await complete({
    ...
    schemaName: "job_task_suggestions",
  });

  // "unavailable" and "error" were already two different reasons on this
  // screen, and they stay two: a deployment with no key is a configuration
  // fact the user can be told plainly, while anything else is a fault. Before
  // this, everything except a missing key arrived as the same empty string and
  // was reported as "ai_unavailable" — including a retired model ID, which is
  // the exact confusion provider.js's header warns about.
  if (!result.ok) {
    return {
      suggestions: [],
      reason: result.reason === "unconfigured" ? "ai_unavailable" : "ai_error",
    };
  }
  ...
  const suggestions = result.data.tasks
    .map((raw) => {
      ...
      // The schema carries this as an enum, which strict mode DOES enforce —
      // enum is one of the few constraints in the supported subset. Kept
      // anyway, because it is two lines and because the day a provider stops
      // honouring the enum is the day an unknown priority string reaches a
      // <select> and renders as a blank row.
