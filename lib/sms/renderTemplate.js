 ...
 * cron), `booking_confirmation` (lib/booking/finalizeBooking.js), and
 * `booking_moved` / `booking_cancelled` (lib/schedule/changeText.js, from
 * the office's move/cancel routes and the client's manage link) are all
 * wired to send, so all five are what the settings screen exposes. A type
...
export const SMS_TEMPLATE_TYPES = {
  ...
  // ── The two change texts ─────────────────────────────────────────────────
  //
  // Sent by lib/schedule/changeText.js when an appointment or a job visit is
  // moved or cancelled — by the office or by the client's own link. Governed
  // by the SAME switch as the confirmation (bookingTextOn): one switch for
  // booking texts, so a company that turned texts off is not surprised by a
  // second kind, and one that left them on is not asked twice. The settings
  // screen shows that state beside these two rather than a second toggle.
  booking_moved: {
    label: "Appointment moved",
    labelKey: "app.smsTemplates.type.booking_moved",
    editable: true,
    tokens: {
      company: { hint: "your business name", hintKey: "app.smsTemplates.token.company", sample: "Northside Painting" },
      service: { hint: "what they booked", hintKey: "app.smsTemplates.token.service", sample: "on-site estimate" },
      when: { hint: "the new appointment time", hintKey: "app.smsTemplates.token.newWhen", sample: "Thu, Aug 14 at 10:00 AM" },
      previous: { hint: "the time it was before", hintKey: "app.smsTemplates.token.previous", sample: "Tue, Aug 12 at 2:00 PM" },
      where: { hint: "the kind of appointment and where — a visit at their address, a call to their number", hintKey: "app.smsTemplates.token.where", sample: "On-site visit at 123 Oak St" },
      // Only a booking made on the booking page has one; a hand-booked
      // appointment or a crew visit leaves it empty and the tidy-up in
      // fillTemplate removes the hole.
      link: { hint: "their link to change or cancel, when they booked online", hintKey: "app.smsTemplates.token.link", sample: "https://app.fieldquo.com/visit/…" },
      phone: { hint: "your business phone", hintKey: "app.smsTemplates.token.phone", sample: "555-0100" },
    },
    fallback: (v) =>
      bookingMovedText({
  ...
  booking_cancelled: {
    ...
    labelKey: "app.smsTemplates.type.booking_cancelled",
    editable: true,
    tokens: {
      company: { hint: "your business name", hintKey: "app.smsTemplates.token.company", sample: "Northside Painting" },
      service: { hint: "what they booked", hintKey: "app.smsTemplates.token.service", sample: "on-site estimate" },
      when: { hint: "the time that was cancelled", hintKey: "app.smsTemplates.token.cancelledWhen", sample: "Tue, Aug 12 at 2:00 PM" },
      where: { hint: "the kind of appointment and where — a visit at their address, a call to their number", hintKey: "app.smsTemplates.token.where", sample: "On-site visit at 123 Oak St" },
      phone: { hint: "your business phone", hintKey: "app.smsTemplates.token.phone", sample: "555-0100" },
    },
    fallback: (v) =>
...
/** The message types the booking-text switch (lib/booking/bookingText.js) governs. */
export const BOOKING_TEXT_TYPES = Object.freeze(["booking_confirmation", "booking_moved", "booking_cancelled"]);
...
export function renderMessage({ type, templates, values = {}, language = "en", templateLanguage = null, translatedTemplates = null }) {
  ...
  //
  // Checked FIRST since 2026-09-25. `translatedTemplates` is built for this
  // reader's language (lib/i18n/companyText.js smsTemplateTranslations), and
  // a wording for the company's DEFAULT language is in it only when the
  // custom text was detected as written in another language — the French
  // wording of an English-default company, whose English reader must get the
  // English draft rather than the French original that `templateLanguage`
  // (the default) would otherwise wave through.
  const translated =
