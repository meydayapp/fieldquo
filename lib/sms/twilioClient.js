...
//
// `purpose`, `ref` and `clientId` are bookkeeping for the delivery receipt
// (model SmsDelivery): what the text was for, the record it was about
// ({ type: "appointment", id }) and the client it went to. They change nothing
// that is sent. A caller that omits them is still tracked, as purpose "other" —
// being able to see that a text never arrived matters more than its label.
export async function sendSms({ to, body, from, companyId, purpose, ref, clientId }) {
  ...
  if (companyId) {
    ...
    try {
      ...
      await recordError({
        ...
        // Same rule as lib/email/resend.js's twin of this row: the company goes
        // in the field the console resolves a name from, never into the
        // sentence as a raw id.
        message: `Refused to text ${formatted}: the sending account could not be read to check isDemo`,
        companyId,
  ...
  // ── Did it ARRIVE? ───────────────────────────────────────────────────────
  //
  // Twilio's 201 means queued, not delivered: a US carrier drops a text from
  // an unregistered (A2P 10DLC) number AFTER Twilio accepted it, and we used
  // to record the 201 and nothing else. So a row is opened before the send
  // (the callback URL names it) and Twilio is asked to post every status
  // change to /api/sms/status. When the callback can't be sent or never
  // arrives, /api/cron/sms-delivery-reconcile asks Twilio directly. Both
  // helpers swallow their own failures — see lib/sms/deliveryStore.js.
  const tracking = await openDelivery({ companyId, purpose, ref, clientId, to: formatted, from: fromNumber });
  const callback = statusCallbackUrl(tracking?.id);
  ...
  try {
    ...
    await recordSendAccepted(tracking, {
      sid: message.sid,
    ...
    await recordSendRefused(tracking, err);
