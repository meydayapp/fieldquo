...
export const TRIGGER_META = {
  lead_no_response: {
    label: "New enquiry, nobody replied",
    description: "Fires once an enquiry has sat at 'new' for the delay period with no quote and nobody marking it contacted.",
    defaultDelay: { value: 2, unit: "days" },
    entityType: "lead",
    // findLeadNoResponse: status must still be "new" and quoteId still null.
    // The lifecycle moves a lead to "contacted" the moment a quote goes out
    // (lib/quotes/quoteLifecycle.js), and any status a person sets by hand
    // takes it out of the query too.
    stopsWhen: "lead_answered",
    // TRANSACTIONAL, on the same reasoning as quote_no_response: the person
    // wrote in asking this company for a price, and this is the reply to
    // that request. CASL's inquiry carve-out covers a message responding to
    // a request the recipient themselves made. The settings copy for Meta
    // lead forms (app.setMetaLeads.whatHappens) promised "your own
    // follow-up rules apply" to an enquiry; until this trigger existed no
    // rule could, because every finder started at a quote.
    commercial: false,
  },
  quote_no_response: {
