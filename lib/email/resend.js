...
export function senderFor(company = {}, platformFrom) {
  ...
  // A platform domain is never a tenant's sending domain, whatever the row
  // says — the read-side of lib/email/platformDomains.js. A company whose row
  // claims fieldquo.com (a QA test once made one do exactly that) sends as if
  // it had no domain, from the platform sender, rather than as FieldQuo.
  const verified =
    company.emailDomainStatus === "verified" &&
    company.emailDomain &&
    !isPlatformEmailDomain(company.emailDomain);
 ...
 */
export async function sendEmail({ to, subject, html, text, replyTo, from, cc, attachments, headers, companyId, clientMail = false }) {
  ...
  if (companyId) {
    ...
    try {
      ...
      await recordError({
        ...
        // The company is carried in the field below, which /platform/errors
        // resolves into a linked name; the raw id in the sentence was a string
        // nobody could identify. The recipient stays — it is the one detail
        // that says WHICH send was refused.
        message: `Refused to send mail to ${to}: the sending account could not be read to check isDemo`,
        companyId,
  ...
  // ── The company's own mailbox, first, when it asked for that ─────────────
  //
  // Only CLIENT mail (the caller says so with `clientMail`) and only when a
  // company-scope mailbox has "Send client emails from this mailbox" on —
  // lib/mailbox/send.js. Any failure there falls through to the send below
  // for THIS message and is recorded on the mailbox row; a client email is
  // never dropped because the contractor's mailbox said no. Imported lazily
  // so the IMAP/SMTP code loads only for a send that could use it. With the
  // switch off (or `clientMail` absent) this block does nothing and the
  // Resend payload below is byte-for-byte what it was.
  if (companyId && clientMail) {
    const via = await import("@/lib/mailbox/send")
      .then(({ trySendThroughMailbox }) =>
        trySendThroughMailbox(null, { companyId, mail: { to, cc, subject, html, text, from, attachments, headers } }),
      )
      .catch((err) => ({ failed: true, code: "threw", error: err?.message }));
    if (via?.sent) return { id: via.messageId, via: "mailbox", mailboxAddress: via.address };
    if (via?.failed) {
      await recordError({
        area: "email",
        code: "mailbox_send_fallback",
        message: `Sending from the company's own mailbox failed (${via.code}); sent from the platform sender instead.`,
        companyId,
        detail: { to, subject, code: via.code, error: String(via.error || "").slice(0, 300) },
      }).catch(() => {});
    }
  }
  ...
  try {
    const result = await resend.emails.send({
      ...
      // Only the sales inbox's reply-all passes a Cc (lib/sales/outreachSender.js);
      // every recipient in it was validated against the lead's own record there.
    ...
    if (result.error) {
      ...
      await recordError({
        ...
        // Whose send this was. Null when the caller genuinely has no company —
        // a platform-level send — which the console prints as an unattributed
        // row rather than guessing. Without it, the commonest tenant-affecting
        // failure in the log could not be filtered to the company it happened
        // to, and support would be reading a to-do list of orphans.
        companyId,
    ...
    await recordError({
      ...
      // Same reason as the rejection above: traceable to a company when there
      // is one, honestly null when there is not.
      companyId,
      detail: errorDetail(err, { to, subject }),
    });
