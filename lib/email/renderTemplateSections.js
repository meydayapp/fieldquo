...
export function renderTemplateSections(sections = [], mergeData = {}, options = {}) {
  ...
  return emailShell({ theme, body, options });
}
...
${body}
            ...
            ${renderFooter(theme)}
            ${options.unsubscribe?.token ? unsubscribeFooterRow({ token: options.unsubscribe.token, request: options.unsubscribe.request, theme }) : ""}
          </table>
        </td>
      </tr>
    </table>
