...
// An unpriced text block (lib/quotes/textBlocks.js) has no amount cell —
// "Exclusions … $0.00" in the covering email would be the one place the
// document says a number the PDF and the page do not.
...
// toGroups — quotes carry scopeGroups, invoices a flat lineItems array — now
// lives in lib/quotes/scopeGroupDisplay.js, so the email template editor can
// group a preview without importing this file's catalogues. Re-exported so
// ScopeGroupsSection and every other caller keep importing it from here.
export { toGroups };
  ...
  const cards = groups
    .map((g, gi) => {
      const content = resolveServiceContent(
        ...
        g.override,
        g.takeoff,
        // The document's language (2026-09-25) — the email twin of the PDF
        // section; it was the one caller still printing the English prose.
        language,
      );
      ...
      // The scope paragraph, above the prices and above what's included.
      // Only on `detail` for the same reason the bullets are: an invoice email
      // is a covering note for work already agreed, and re-explaining the trade
      // to somebody who has already signed is noise.
      const scope =
        detail && content.description
          ? `
              <tr>
                <td style="padding:0 0 8px;font-family:${EMAIL_FONT};font-size:13px;line-height:1.5;color:${t.inkMuted};">${escapeHtml(content.description)}</td>
              </tr>`
          : "";
        ...
        .map(
          (item) => `
                      <tr>
                        ...
                        <td align="right" style="padding:3px 0;text-align:right;white-space:nowrap;font-family:${EMAIL_FONT};font-size:13px;color:${t.inkMuted};">${lineShowsAmount(item) ? money(item.amount) : ""}</td>
                      </tr>`,
        )
        .join("");
...
export function quoteSectionsText({
  ...
  if (groups.length) {
    ...
    for (const g of groups) {
      ...
      // Same order as the HTML twin: what the work is, then what it costs.
      // The text part carries the argument, not a stub.
      if (content.description) out.push(`  ${content.description}`);
      for (const item of g.items) {
        out.push(
          `  ${itemText(item)}${num(item.quantity) > 1 ? ` x${item.quantity}` : ""}${lineShowsAmount(item) ? `  ${money(item.amount)}` : ""}`,
        );
      }
