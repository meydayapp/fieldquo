...
const COPY = {
  en: {
    ...
    once: "The link works once.",
    expiry: (n, unit) =>
      unit === "hours"
        ? `It expires in ${n} ${n === 1 ? "hour" : "hours"}.`
        : `It expires in ${n} ${n === 1 ? "minute" : "minutes"}.`,
    fallback:
  ...
  fr: {
    ...
    expiry: (n, unit) =>
      unit === "hours"
        ? `Il expire dans ${n} ${n === 1 ? "heure" : "heures"}.`
        : `Il expire dans ${n} minute${n === 1 ? "" : "s"}.`,
    fallback:
  ...
  es: {
    ...
    expiry: (n, unit) =>
      unit === "hours"
        ? `Caduca en ${n} ${n === 1 ? "hora" : "horas"}.`
        : `Caduca en ${n} ${n === 1 ? "minuto" : "minutos"}.`,
  ...
  uk: {
    ...
    expiry: (n, unit) =>
      unit === "hours"
        ? `Воно діє ${n} ${ukPlural(n, "годину", "години", "годин")}.`
        : `Воно діє ${n} ${ukPlural(n, "хвилину", "хвилини", "хвилин")}.`,
  ...
  pa: {
    ...
    expiry: (n, unit) =>
      unit === "hours"
        ? `ਇਹ ${n} ਘੰਟੇ ਬਾਅਦ ਖ਼ਤਮ ਹੋ ਜਾਂਦਾ ਹੈ।`
        : `ਇਹ ${n} ਮਿੰਟ ਬਾਅਦ ਖ਼ਤਮ ਹੋ ਜਾਂਦਾ ਹੈ।`,
  ...
  tl: {
    ...
    expiry: (n, unit) =>
      unit === "hours"
        ? `Mag-e-expire ito sa loob ng ${n} oras.`
        : `Mag-e-expire ito sa loob ng ${n} minuto.`,
    fallback:
...
export function resetPasswordEmail({ url, userName, language, company, expiresMinutes } = {}) {
  ...
  const parts = {
    ...
    lines: [
      c.resetBody(org),
      [c.once, expiry ? c.expiry(expiry.n, expiry.unit) : ""].filter(Boolean).join(" "),
    ],
    cta: c.resetCta,
    ctaUrl: link,
