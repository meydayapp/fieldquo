...
export async function getDomain(id) {
  ...
  return {
    ...
    status: data.status,
    records: data.records || [],
    // `capabilities.sending` / `capabilities.receiving`, each "enabled" or
    // "disabled" — Resend's own words for whether the domain has receiving
    // switched on (the toggle on the domain page, plus its MX record). Read
    // by lib/sales/outreachSender.js to decide whether SALES_REPLY_DOMAIN can
    // actually take a prospect's reply. Passed through untouched; absent on
    // an older API response, which callers treat as "could not tell", never
    // as "no".
    capabilities:
      data.capabilities && typeof data.capabilities === "object" ? data.capabilities : null,
  };
}
