...
export function buildOnboardingNextStepsEmail({
  ...
  const moreRows = moreSteps.map((step) => {
    // A reworded title carries its figures (lib/setupSteps.js titleWhen).
    const label = fill(t(step.titleKey || `app.setup.step.${step.key}`, step.title || step.key), step.titleParams || {});
    let unlock = "";
