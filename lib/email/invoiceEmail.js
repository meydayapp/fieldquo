...
//     button.
//   * Under the button, HOW TO PAY: the online methods and every offline
//     method the company switched on with its address — the e-transfer
//     email, the Zelle number, who to make a cheque out to — in the
//     document's language, with the invoice number as the reference. The
//     block is the one the send route stored on the invoice (howToPay), so
//     a reminder six weeks later says the same address the first email did.
//     A bank account number never goes in an email body: that method says
//     "the details are on the invoice" here and prints in full on the PDF
//     and the portal (lib/payments/offlineMethods.js).
...
import { documentCustomFacts, customFactsHtml, customFactsText } from "@/lib/documentSections/customFacts";
import {
  documentEmailHtml,
  amountBlock,
  emailButton,
...
import { howToPayFor, onlineOptions } from "@/lib/payments/offlineMethods";
import { howToPayEmailHtml, howToPayEmailText } from "@/lib/payments/howToPayEmail";

// documentEmailHtml's card is 560 wide with 22px gutters — see quoteEmail.js.
 ...
 * @param wording   the company's wording copy for THIS kind of email (invoice,
 *                  reminder, receipt or deposit request), from chooseWording()
 *                  in lib/email/documentEmailWording.js, or null for the
 *                  original. Five slots only — the figure, the due date, the
 *                  button, the payment methods and the footer's tax line stay
 *                  derived from the invoice.
 * @param howToPay  the rendered "How to pay" block, when the caller already
 *                  has it (the send route builds and stores it in one go).
 *                  Otherwise read from invoice.howToPay, else built from the
 *                  company's live settings with `url` as the pay link.
 */
export function buildInvoiceEmail({
  ...
  // Attached by the send route as invoice.customFields; one formatter with
  // the PDF and the portal (lib/documentSections/customFacts.js).
  const customFacts = documentCustomFacts(invoice.customFields, { date, labels });
  ...
  // ── The five wording slots — see quoteEmail.js ───────────────────────────
  const slots = wording?.source === "copy" ? wording.slots : null;
  const tokens = {
    clientName,
    ...
    // The figure this email headlines, formatted: what was paid on a receipt,
    // otherwise what is being asked for.
  ...
  // "How to pay" belongs on a bill, not on a receipt for money that has
  // already been taken by a method the client chose.
  const block =
    kind === "paid"
      ? null
      : howToPay !== undefined
        ? howToPay
        : howToPayFor(invoice, { company, language, online: canTakeCard ? onlineOptions(company, url) : null });
  const methodsHtml = block ? howToPayEmailHtml(block, { theme: t, font: EMAIL_FONT }) : "";
  const methodsText = block ? howToPayEmailText(block) : [];
  ...
  const letter =
    canvasLetter && !canvasLetter.empty
      ? `
        <div style="margin:0 0 20px;">${canvasLetter.body}</div>`
      : `
        <p style="font-family:${EMAIL_FONT};font-size:15px;line-height:1.6;margin:0 0 14px;color:${t.ink};">${escapeHtml(greeting)}</p>
...
${emailButton({ url, label: cta, fill })}
  ...
  const text = [
    ...
    "",
    `${cta}: ${url}`,
    ...(methodsText.length ? ["", ...methodsText] : []),
    "",
    closing,
    signature,
  ]
