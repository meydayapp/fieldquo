...
export async function sendTeamInviteEmail({
  ...
  // The language the inviter chose on the New User page. Stored on
  // PendingTeamProfile, which is the only record that has it at this point —
  // Better Auth's invitation payload carries no language, so it is read here
  // rather than threaded through auth.js.
  //
  // Without this the picker was decorative: the code was saved, copied onto
  // the Member on accept, and read by nothing, so every invitation went out in
  // English however carefully it was chosen.
  let inviteLanguage = "en";
  try {
    const pending = await db.pendingTeamProfile.findFirst({
      where: { companyId: organizationId, email: String(email || "").trim().toLowerCase() },
      select: { invitationLanguage: true },
    });
    if (pending?.invitationLanguage) inviteLanguage = pending.invitationLanguage;
  } catch {
    // A lookup failure must not stop the invitation going out. English is the
    // documented default, and an invite in the wrong language beats no invite.
  }
  const outcome = await deliver({
    // organizationId IS the companyId — it is used as one three lines above.
    // An invitation goes to somebody who is not yet in the product, so this is
    // one of the few paths where a demo account can put mail in front of a
    // stranger with nothing but a typed address.
