...
export async function resolveSender(company = {}, companyId) {
  // Discovered from Resend rather than read from an env var — see
  // platformSender.js. Only used when the company has no verified domain of
  // its own, but resolving it here means every async send path gets it
  // without having to know that.
  const base = senderFor(company, await getPlatformFrom());
