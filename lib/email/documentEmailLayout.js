 ...
 *                titled box.
 */
export function preparedForBlock({ theme, label = "Prepared for", client = {}, jobAddress = null }) {
  if (!client.name && !client.address) return "";

  const contact = [client.email, client.phone].filter(Boolean).join(" · ");
  // { label, value } from lib/documentSections/ClientInfoSection.js
  // jobAddressLine — the site, when it is not the client's own address.
  const job =
    jobAddress?.value
      ? `<div style="margin-top:4px;font-size:10px;font-weight:700;letter-spacing:1px;color:${theme.inkFaint};">${escapeHtml(String(jobAddress.label || "Job address").toUpperCase())}</div>
      <div style="font-size:12px;color:${theme.inkMuted};">${escapeHtml(jobAddress.value)}</div>`
      : "";
...
export function documentEmailHtml({
  ...
  body = "",
  footerNote = "",
  // The name line in the footer. The company's name unless a wording copy
  // (lib/email/documentEmailWording.js) says otherwise — already escaped by
  // the caller when it is, exactly like footerNote.
  signature = null,
}) {
...
${body}
          ...
          <tr>
            <td bgcolor="${theme.accentWash}" style="background:${theme.accentWash};border-top:1px solid ${theme.borderSoft};padding:18px 22px;font-family:${EMAIL_FONT};font-size:12px;line-height:1.6;color:${theme.inkMutedOnWash};">
              ...
              <div style="font-size:13px;font-weight:700;color:${theme.inkOnWash};">${signature == null ? escapeHtml(company.name || "") : signature}</div>
