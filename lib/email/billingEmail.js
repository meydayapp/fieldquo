 ...
 * @param atPeriodEnd "cancelled" only — the plan is booked to end on
 *                 `periodEnd` and works until then (a paid plan cancelled
 *                 from the product). False: it has ended. See
 *                 lib/billing/notify.js's notifyCancellation for why the two
...
export function buildBillingEmail({
  ...
  // cancelled
  const ending = Boolean(atPeriodEnd && periodEnd);
  return {
    subject: ending
      ? `Your FieldQuo plan ends on ${periodEnd}`
      : "Your FieldQuo plan has been cancelled",
    html: shell({
      heading: ending ? "Your plan is ending" : "Your plan has ended",
      sub: companyName,
      body:
        p(
          ending
            ? `Your <strong>${escapeHtml(planName || "FieldQuo")}</strong> plan ends on <strong>${escapeHtml(periodEnd)}</strong>; nothing more will be charged. Everything keeps working until then, and you can resume any time before that date from Account &amp; Billing.`
        ...
        small(
          ending
            ? "Changed your mind? Press Resume on Account & Billing before that date and the plan simply continues."
            : "If you cancelled by mistake, press Resume on Account & Billing to pick up where you left off.",
        ),
      cta: billingUrl ? { url: billingUrl, label: ending ? "Resume my plan" : "Resume" } : null,
    }),
  };
}
