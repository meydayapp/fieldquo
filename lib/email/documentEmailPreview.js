...
function sampleQuote() {
  return {
    ...
    createdAt: new Date(),
    // QuoteLineItem's own shape — description and amount — because the
    // builder reads exactly those (quoteSections.js toGroups/itemText and
    // money(item.amount)). `name`/`total` rows rendered every line as $0.00
    // and the card subtotal as 0: a sample the owner could not tell from a
    // broken email. check:document-emails asserts the amounts appear.
    lineItems: [
      { description: "Cabinet doors & drawer fronts — spray refinish", quantity: 24, unitPrice: 125, amount: 3000 },
      { description: "Cabinet boxes — on-site refinish", quantity: 1, unitPrice: 750, amount: 750 },
      { description: "Premium hardware replacement", quantity: 24, unitPrice: 6.25, amount: 150 },
    ],
    processNotes: null,
...
function sampleInvoice(kind) {
  ...
  return {
    ...
    createdAt: new Date(),
    // Invoice.lineItems is the same JSON shape — createInvoiceFromQuote copies
    // description/amount across. buildInvoiceEmail reads no line items today
    // (the invoice email is a covering note, not an itemised copy); carried
    // so the sample stays the shape of a real invoice if it ever does.
...
export async function renderDocumentEmailPreview(db, { companyId, kind, language = "en", copy = null, request = null }) {
  ...
  if (meta.builder === "quote") {
    ...
    const scopeGroups = recent
      ...
      : [];
    const customFields = recent ? await loadDocumentCustomFields(db, companyId, "quote", quote.id, { language }) : [];
    const args = {
  ...
  const customFields = recent ? await loadDocumentCustomFields(db, companyId, "invoice", invoice.id, { language }) : [];
