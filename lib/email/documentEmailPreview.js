...
import { SAMPLE_LINE_ITEMS } from "@/lib/email/templateLineItems";
...
function sampleQuote() {
  return {
    ...
    createdAt: new Date(),
    // Stored-shape lines (description / quantity / rate / amount), shared
    // with the email template editor's sample. These were name / unitPrice /
    // total — a shape no stored quote has — and buildQuoteEmail reads
    // `amount`, so the sample quote's email priced every line at $0.00.
