...
export function mapBudget(raw) {
  ...
  if (nums.length) {
    ...
    // ── A "+" is a floor, not a ceiling ──────────────────────────────────
    //
    // Everything below buckets on the largest figure as though it were the TOP
    // of the range, which is right for "$3,500 – $6,000" and exactly backwards
    // for "$15,000+". That top band was landing in 5k_15k — the richest lead on
    // the board scored a tier below what they actually said, on every open-ended
    // band any company configures.
    //
    // A floor buckets by which band it falls INTO, lower-inclusive: "at least
    // $15,000" is 15k_plus, "at least $10,000" is 5k_15k because that is all
    // they committed to. Deliberately not promoted to the band above — they said
    // a minimum, and inventing headroom is the same invention as crediting an
    // unanswered budget.
    if (/\+\s*$/.test(s) || /\b(or more|and up|and over|at least|plus|over|above|minimum)\b/.test(s)) {
      if (amount >= 15000) return "15k_plus";
      if (amount >= 5000) return "5k_15k";
      if (amount >= 1000) return "1k_5k";
      return "under_1k";
    }
...
export function normaliseLeadRow(row) {
  ...
  // Every CRM export has one and this importer read none of them, so a bought
  // list arrived with the job's location in a column we threw away — and the
  // lead converted into a client with no address, the same loss the instant
  // quote had. Only the street line: `city`/`province` are NOT picked, because
  // a jurisdiction guessed from a spreadsheet column drives a tax rate on a
  // document, and a wrong one is worse than none.
  const address = pick(
    "address", "Address", "street", "Street", "street_address",
    "Street Address", "job address", "Job Address", "site address", "Site Address",
    "location", "Location",
  );
  return { name, email, phone, message, address, budgetBand, timeline };
}
