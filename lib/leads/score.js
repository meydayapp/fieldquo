...
// ── Every reason is a sentence AND a key ────────────────────────────────────
//
// `label` is English and is what gets STORED on LeadRequest.scoreReasons —
// lib/analytics/leadScoring.js groups win rates by it, and rows written before
// keys existed carry only it, so it stays exactly as it was. `key` (plus
// `values` for the counted ones) rides beside it so the leads screen can print
// the same reason in the reader's language: t(r.key, r.label, r.values). A row
// with no key falls back to its label. lib/messaging/conversationScore.js
// already works this way (labelKey) and called this file's English-only
// output "a wart"; this is the wart removed without touching a stored row.
const BUDGET_REASON = {
  "15k_plus": "Budget $15k+",
...
const BUDGET_KEY = {
  "15k_plus": "app.leads.reason.budget15kPlus",
  "5k_15k": "app.leads.reason.budget5k15k",
  "1k_5k": "app.leads.reason.budget1k5k",
  under_1k: "app.leads.reason.budgetUnder1k",
  unsure: "app.leads.reason.budgetUnsure",
};
...
const TIMELINE_KEY = {
  asap: "app.leads.reason.timelineAsap",
  "2_weeks": "app.leads.reason.timeline2Weeks",
  "1_3_months": "app.leads.reason.timeline1to3Months",
  exploring: "app.leads.reason.timelineExploring",
};
...
export function scoreLead(lead = {}, { unasked = [] } = {}) {
  const reasons = [];
  let score = 0;
  const add = (points, label, key, values) => {
    if (points > 0) {
      score += points;
      reasons.push({ label, weight: points, key, ...(values ? { values } : {}) });
    }
  };

  // 1 — Timeline
  const tPts = TIMELINE_POINTS[lead.timeline];
  if (tPts != null) add(tPts, TIMELINE_REASON[lead.timeline], TIMELINE_KEY[lead.timeline]);

  // 2 — Budget (0-point "unsure" still gets a reason so its absence is visible)
  const bPts = BUDGET_POINTS[lead.budgetBand];
  if (bPts != null) {
    if (bPts > 0) add(bPts, BUDGET_REASON[lead.budgetBand], BUDGET_KEY[lead.budgetBand]);
    else
      reasons.push({
        label: BUDGET_REASON[lead.budgetBand],
        weight: 0,
        key: BUDGET_KEY[lead.budgetBand],
      });
  }

  // 3 — Emergency (from structured intake flags)
  if (isEmergencyIntake(lead.intake)) add(20, "Flagged as an emergency", "app.leads.reason.emergency");

  // 4 — Contactability
  if (lead.phone) add(8, "Phone number provided", "app.leads.reason.phoneProvided");
  if (lead.email) add(4, "Email provided", "app.leads.reason.emailProvided");
  ...
  if (photoCount > 0) {
    add(
      Math.min(10, photoCount * 4),
      `${photoCount} photo${photoCount > 1 ? "s" : ""} attached`,
      "app.leads.reason.photosAttached",
      { value: photoCount },
    );
  }
  if (planCount > 0) {
    ...
    add(
      12,
      `Sent a plan (${planCount} PDF${planCount > 1 ? "s" : ""})`,
      "app.leads.reason.sentPlan",
      { value: planCount },
    );
  }
  if (lead.kitchenDesign) add(8, "Designed a kitchen layout", "app.leads.reason.kitchenLayout");
  if ((lead.message || "").trim().length >= 120)
    add(5, "Wrote a detailed description", "app.leads.reason.detailedDescription");
  ...
  if (withheld > 0) {
    reasons.push({
      label: "Scored without budget — the phone can't ask",
      weight: 0,
      key: "app.leads.reason.noBudgetPhone",
    });
  }
