 ...
 *
 * Only the phone, and only budget: the receptionist is FORBIDDEN to discuss
 * money (absolute rule 1 in lib/voice/prompt.js), so its 30 points come out of
 * the denominator instead of counting against the caller. Everything else on
 * the score is either observable (a number, an email, an address) or askable in
 * any channel.
 *
 * `phone_agent_recovered` is the SAME receptionist on the SAME call — the lead
 * dug back out of a transcript the webhook never delivered (lib/ai/
 * callLeadRecovery.js). It was missing here, so a recovered call was scored
 * against a budget question that call was forbidden to ask, and came out cold
 * for exactly the reason the live path had already been fixed for.
 *
 * NOT the same map as NOT_ASKED_BY_SOURCE in lib/leads/qualifiers.js, and
 * deliberately narrower. That one says which channels do not PUT a question —
 * which is what the leads screen may claim on the page. This one changes the
 * arithmetic, so it lists only the channel that COULD not ask. Widening it to
 * the other four (the instant quote has no timeline question, the kitchen
 * designer and the portal ask neither) would re-rank every lead board in
 * production and is an owner's call, not a side effect of a display fix.
 * check-lead-intake.mjs pins this map as a subset of that one.
 */
...
export async function createScoredLead(input) {
  ...
  // No actor: a homeowner filled in a form, and null is the honest answer.
  notifyEvent({
    companyId: input.companyId,
    type: "lead.created",
    entityId: lead.id,
