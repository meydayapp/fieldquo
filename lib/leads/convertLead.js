...
// the quote, links the two (so the lead reads "quoted" and the quote traces back
// to who asked), and flips the lead to converted. Idempotent: a lead already
// linked to a quote returns that quote instead of spawning a second.
//
...
async function findOrCreateClient(companyId, lead) {
  ...
  const address =
    lead.intake && typeof lead.intake === "object" ? lead.intake.address : null;
...
export async function convertLeadToQuote({ lead, member, company }) {
  ...
  await db.leadRequest.update({
    where: { id: lead.id },
    data: { status: "converted", quoteId: quote.id },
  });
