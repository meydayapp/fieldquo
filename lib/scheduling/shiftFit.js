...
/**
 * The UTC windows a worker's weekly rows open between two instants.
 *
 * The day board reads this to grey out the hours a person has NOT said they
 * are available for, and it has to be the same arithmetic the refusal above
 * uses — a cell drawn grey that the route would accept, or drawn white that it
 * would refuse, is a board lying about what happens on click. Same hour-step
 * over local days, same overnight handling, same "no rows means no verdict":
 * an empty list back means nothing is greyed, never that everything is.
 *
 * @returns [{from:Date, to:Date}] — empty when the worker has declared nothing
 */
export function availabilityWindowsBetween(rows, from, to, timezone = "America/Toronto") {
  const s = asDate(from);
  const e = asDate(to);
  const avail = Array.isArray(rows) ? rows.filter(Boolean) : [];
  if (!s || !e || e <= s || avail.length === 0) return [];
  const tz = avail[0].timezone || timezone;
  return localDaysTouched(s, e, tz)
    .flatMap((d) => windowsOn(avail, d, tz))
