...
//
//   AvailabilitySchedule  when this person CAN work. They set it. It is the
//                         outer bound, and scheduling outside it is scheduling
//                         someone who told you they are not there. BLOCKS.
//
//   LeaveRequest          approved time off. Same thing, only worse: it was
//                         asked for and granted. BLOCKS.
//
 ...
 * @returns {{ok:boolean, blocks:string[], warnings:string[], notes:string[]}}
 *          `ok` false means refuse. Warnings never make it false.
 */
export function shiftFit({
  ...
  if (!s || !e)
    return {
      ok: false,
      blocks: ["A shift needs a start and an end."],
      warnings,
      notes,
    };
  if (e <= s)
    return {
      ok: false,
      blocks: ["The shift's end must be after its start."],
      warnings,
      notes,
    };
  ...
  return { ok: blocks.length === 0, blocks, warnings, notes };
}
