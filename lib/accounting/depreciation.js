...
export function assetCharge(asset, asOf) {
  ...
  if (!life || !inService || base <= 0) {
    ...
    // A cost of zero is NOT a fully-salvaged item: it is a row whose price
    // nobody has entered — the fleet door lets a van in without one
    // (lib/assets/create.js). Saying "fully written down" about it would be
    // a claim nobody made, in the register's own words, so it gets its own
    // reason and a book value of nothing rather than of the salvage.
    if (num(asset?.cost) <= 0) return { ...empty("no_cost_recorded"), bookValue: 0 };
    // base === 0 with a good life, date and a real cost is legitimate: a
    // fully-salvaged item genuinely costs nothing to use up.
