...
export const TAX_SUMMARY_LIMITATIONS = [
  ...
  "Input tax credits are not netted off. Tax you PAID is shown separately, and only for purchases booked from a read receipt — expenses typed in by hand or imported from a bank file carry no tax figure — so your actual remittance will be lower than what you charged, by at least that amount.",
...
/**
 * Tax PAID on purchases, from the expenses that state it — the ones booked
 * from a read receipt (lib/receipts/record.js writes Expense.taxAmount and
 * taxBreakdown; nothing else does).
 *
 * Only those rows. An expense with no taxAmount is "we don't know", not "no
 * tax", so it is counted as unknown in `excludes` rather than as a zero —
 * a sum that quietly treated every hand-typed expense as tax-free would
 * understate what the contractor can claim with nothing saying so.
 *
 * Split by the printed label (GST / PST / HST / QST) where the receipt
 * printed them separately; a receipt with one tax line lands under "Tax".
 */
export function inputTaxFigure(expensesIn = []) {
  const stated = expensesIn.filter((e) => e?.taxAmount !== null && e?.taxAmount !== undefined);
  const unstated = expensesIn.length - stated.length;
  const label = "Tax paid on purchases, read off receipts";
  const includes = ["The tax printed on each receipt booked through the receipts book, dated in the period"];
  const excludes = [
    `${unstated} expense(s) in the period that state no tax — typed in by hand or imported — whose tax is unknown, not zero`,
    "Any judgement about how much of it is recoverable: that is your accountant's",
  ];
  if (!stated.length) return nothingRecorded(label, { includes, excludes });

  const byLabel = new Map();
  let total = 0;
  for (const e of stated) {
    const amount = num(e.taxAmount);
    total += amount;
    const parts = Array.isArray(e.taxBreakdown) ? e.taxBreakdown : null;
    if (parts && parts.length) {
      for (const p of parts) {
        const key = String(p?.label || "Tax").trim() || "Tax";
        byLabel.set(key, (byLabel.get(key) || 0) + num(p?.amount));
      }
    } else {
      byLabel.set("Tax", (byLabel.get("Tax") || 0) + amount);
    }
  }
  return figure(label, total, {
    count: stated.length,
    components: [...byLabel.entries()].map(([l, amount]) => ({ label: l, amount, count: null })),
    includes,
    excludes,
  });
}
...
export function buildFinancialStatements({
  ...
  /**
   * The date a document was issued, and which column that came from — the
   * one rule in lib/invoices/issueDate.js, which also dates a past job
   * entered after the fact by its real dates rather than the day it was typed.
   */
  const issueOf = (fam) => {
    const source = fam.root || fam.members[0];
    const { date, from } = invoiceIssueDate(source);
    const key = dayKey(date);
    return key ? { key, from } : { key: null, from: null };
  };
  ...
  // A draft has not been issued, and neither has a cancelled invoice. issueOf()
  // dates a family by createdAt when there is no sentAt — right for a legacy
  // sent invoice that was never stamped, and wrong for a draft, where that is
  // the day somebody started typing. Without this, three unpaid drafts sat in
  // the period's "invoiced" and its accrued tax as if they had gone out. The
  // receivables section already states the rule at its own loop ("the latest
  // version of every non-draft invoice"); this is the same rule, applied to
  // the other three statements, keyed on the LATEST version's status because
  // an amendment can take a sent invoice back to draft.
  const isIssued = (fam) => {
    const s = fam.latest?.status;
    return Boolean(s) && s !== "draft" && s !== "cancelled";
  };
  const issuedIn = families.filter((f) => isIssued(f) && inRange(issueOf(f).key, fromKey, toKey));
  ...
  const materials =
    jobExpenses.length === 0
      ...
      : figure("Materials, subcontractors and other job costs", jobExpenseTotal, {
          ...
          includes: [
            "Expenses tagged to a job, by the category they were entered under",
            "Sales tax paid on those purchases, inside each amount — nothing is netted off (receipt-read expenses also state their tax separately; see the sales-tax summary)",
          ],
          excludes: ["Overhead and general expenses"],
        });
  ...
  for (const fam of issuedIn) {
    ...
    const st = taxStatement({
      ...
      tax: l?.tax,
      // The invoice's own record, when it has one: a stated US zero is a
      // "none", not a hole to go and look at.
      stored: l?.taxResolution || null,
      company,
      taxRates,
  ...
  const salesTax = {
    ...
    paidOnPurchases: inputTaxFigure(expensesIn),
