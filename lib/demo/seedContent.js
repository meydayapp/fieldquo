...
/**
 * One line in the shape every reader of Quote.lineItems / Invoice.lineItems
 * uses: `description` is the line's name, `detail` the sentence under it,
 * `rate` × `quantity` = `amount`. See lib/quotes/builderPayload.js
 * (lineItemsFromStored / applyLineItemEdit) and lib/invoices/labourLine.js
 * (retotal).
 *
 * The seed used to write { name, unitPrice, total } — the Product/email-preview
 * vocabulary. Nothing on a document reads those keys: the invoice page, the
 * portal, the PDF and the covering email all print `money(item.amount)`, so
 * every demo line showed $0.00 beneath a correct total, and the edit screen's
 * legacy fallback was the only reader that could make sense of them.
 */
function line({ description, detail, quantity, unit, rate }) {
  return {
    description,
    ...(detail ? { detail } : {}),
    quantity,
...
function lineItems(ctx, picks) {
  return picks.map(({ i, qty, note }) => {
    const s = ctx.preset.services[i % ctx.preset.services.length];
    return line({ description: s.name, detail: note, quantity: qty, unit: s.unit, rate: Number(s.rate) });
  });
}

function totalsFor(ctx, items, { taxEnabled = true } = {}) {
  const subtotal = money(items.reduce((s, li) => s + li.amount, 0));
...
async function seedCompany(ctx) {
  ...
  await db.company.update({
    ...
    data: {
      ...
      // The offline methods for the demo's country, with the address each
      // needs so the demo invoice's "How to pay" block is the real thing.
      // Was ["card", "etransfer", "cheque"] — two spellings the catalogue
      // never had, printed verbatim on twelve live companies' invoices.
      paymentMethods: prof.country === "US" ? ["zelle", "check", "cash"] : ["e_transfer", "cheque", "cash"],
      paymentMethodDetails:
        prof.country === "US"
          ? { zelle: { address: `pay@${ctx.slug}.example.com` } }
          : { e_transfer: { address: `pay@${ctx.slug}.example.com`, autoDeposit: true } },
...
async function seedPipeline(ctx) {
  ...
  for (const st of STORIES) {
    ...
    const base = {
      ...
      // In the shape lib/ai/quoteReview.js actually returns and
      // app/components/quotes/SuggestAddOns.js actually reads: `checks`,
      // `pricing`, `addOns`, `rewrites`, `readiness`. The first fixture here
      // was written as { summary, issues, suggestedAddOns } — a shape nothing
      // renders — so the one demo quote seeded "already reviewed" opened to a
      // panel throwing on `review.checks.length`. The figures are the same
      // arithmetic the real review would do on this quote: two checks at the
      // real ids and severities, readiness = 100 − (10 + 4), and a pricing
      // verdict a company with this many accepted quotes really gets.
      ...(st.aiReview
        ? {
            aiReview: {
              generatedAt: ctx.day(-st.days, 15).toISOString(),
              quoteTotal: total,
              checks: [
                {
                  id: "no_detail",
                  severity: "medium",
                  title: "1 line has no description",
                  detail: `"${items[0].description}" — only the name reaches the client, and there is no scope paragraph above it to say what is done. A sentence on what is done, per what unit, is what they judge the price against.`,
                },
                {
                  id: "no_photos",
                  severity: "low",
                  title: "No photos",
                  detail: "A photo of the actual job shows you were there and looked properly. Optional, but it separates you from whoever quoted over the phone.",
                },
              ],
              pricing: {
                verdict: "in_range",
                detail: `In line with what you usually win at — your median accepted quote for this kind of work is $${Math.round(subtotal * 1.05).toLocaleString("en-CA")} across 4 quotes.`,
                sampleSize: 4,
                medianAccepted: money(subtotal * 1.05),
                medianDeclined: null,
              },
              addOns: [
                {
                  description: `Extended warranty — ${items[0].description.toLowerCase()}`,
                  detail: "Covers the finish against peeling or fading for five years, so the work stays looking new.",
                  amount: money(subtotal * 0.06),
                  priceSampleSize: 3,
                  frequency: 0.5,
                  note: null,
                  source: "history",
    ...
    if (st.tiered) {
      ...
      for (const [label, suffix, f] of tiers) {
        const tItems = items.map((li) => line({ ...li, rate: money(li.rate * f) }));
      ...
      if (res.created && st.addOns) {
        const addOns = [
          { description: `Extended warranty — ${items[0].description.toLowerCase()}`, amount: money(subtotal * 0.06), source: "ai", selected: st.status === "accepted" },
    ...
    const invItems = isDeposit
      ? [line({ description: `Deposit — 50% of ${quote.quoteNumber}`, detail: title, quantity: 1, unit: "ea", rate: money(subtotal / 2) })]
      : items;
  ...
  for (let seq = 1; seq <= 5; seq++) {
    ...
    if (paid) {
      ...
      const invoice = await ctx.create("invoice", {
        ...
        status: "paid",
        createdById: ctx.owner.user.id,
        lineItems: [line({ description: service.name, detail: "Maintenance plan — monthly", quantity: 1, unit: service.unit, rate: subtotal })],
        subtotal,
        discount,
        tax,
        total,
