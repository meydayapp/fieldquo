...
//
// Also consumed by lib/migrations/hosts.js, for the paid data-migration
// service's "book a call with a superadmin" step. That is deliberate reuse,
// not a coincidence: it's the same PlatformAdmin calendar (DemoHostAvailability)
// and the same slot math, and a migration call has to see a demo call already
// on the calendar or the two could double-book a superadmin. See that file's
...
/**
 * The hosts free for a booking `minutes` long starting at `iso` — every
 * consecutive slot of the span must be on the host's own grid, so a sixty-
 * minute walkthrough needs the slot AND the one after it, and a span that
 * runs past the end of a host's stated hours is not offered. A thirty-minute
 * ask is exactly hostsFreeAt.
 */
export function hostsFreeFor(hosts = [], iso, minutes = SLOT_MINUTES, now = new Date()) {
  const t = new Date(iso).getTime();
  if (!Number.isFinite(t)) return [];
  const slots = Math.max(1, Math.ceil(minutes / SLOT_MINUTES));
  const needed = Array.from({ length: slots }, (_, i) => t + i * SLOT_MINUTES * 60000);
  return hosts.filter((host) => {
    const starts = new Set(hostGrid(host, now).flatMap((g) => g.starts.map((s) => s.getTime())));
    return needed.every((n) => starts.has(n));
  });
}

/**
 * The slot starts, by day, from which a booking `minutes` long fits on at
 * least one host. availableSlotsByDay is this with one slot.
 */
export function availableSlotsByDayFor(hosts = [], minutes = SLOT_MINUTES, now = new Date()) {
  const days = availableSlotsByDay(hosts, now);
  if (minutes <= SLOT_MINUTES) return days;
  return days
    .map((d) => ({ day: d.day, slots: d.slots.filter((s) => hostsFreeFor(hosts, s.iso, minutes, now).length > 0) }))
...
export function assembleHosts(windows = [], bookings = []) {
  ...
  for (const b of Array.isArray(bookings) ? bookings : []) {
    ...
    // A booking blocks one slot unless the row says how long it is. A
    // sales walkthrough is sixty minutes (lib/sales/nextSteps.js), and
    // blocking only its first half would sell the second half to a
    // homepage demo. The loaders derive `minutes` from DemoBooking.source;
    // an ordinary demo carries none and is one slot, as before.
    const minutes = Number.isFinite(Number(b?.minutes)) && Number(b.minutes) > 0 ? Number(b.minutes) : SLOT_MINUTES;
    const range = { start, end: new Date(start.getTime() + minutes * 60000) };
    if (!b.hostAdminId) {
