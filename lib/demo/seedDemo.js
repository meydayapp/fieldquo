 ...
 *
 * Null for a trade with a price book, and that is the load-bearing part. A
 * real company can never hold a defaultRate on roofing, siding or any other
 * book trade: Settings > Services hides the single-rate box for exactly those
 * (app/app/settings/services/page.js) because the book already prices the
 * work per square, per door, per tread. This seeder used to write the
 * headline rate onto every category regardless, which put a demo in a state
 * no real tenant can reach — and the builder faithfully seeded "Roofing —
 * $525.00" under a roof takeoff that had already priced the job. The comment
 * that justified it ("so an instant quote in the demo produces a number") was
 * wrong: nothing on the instant-quote path reads defaultRate; it prices from
 * the book like everything else. A demo must cost and review a quote exactly
 * as a real company does, and the first condition of that is holding the same
 * rows a real company would.
 */
export function rateFor(preset, categoryKey) {
  if (hasPriceBook(categoryKey)) return null;
  return preset.services?.[0]?.rate ?? null;
}

export function unitFor(preset, categoryKey) {
  if (hasPriceBook(categoryKey)) return null;
    ...
    data: {
      ...
      serviceCategories: {
        create: categories.map((c) => ({
          categoryId: c.id,
          enabled: true,
          // The preset's rate becomes the category's default for a trade
          // quoted by a single rate (an electrician's hourly). A trade with a
          // price book gets null, as it would on a real company — see rateFor.
