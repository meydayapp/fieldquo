...
export async function seedDemoContent(companyId, preset, now = new Date()) {
  ...
  for (const spec of specs) {
    ...
    const quote = await db.quote.create({
      data: {
        ...
        // Seeded, not sold. A demo tenant's quotes must be distinguishable
        // from a real company's on any figure grouped by origin.
        createdVia: requireCreatedVia("demo"),
        ...
        // The EVENT dates, which are what the reports read. Win/loss dates a
        // quote by sentAt or, failing that, by its decision; the home page
        // counts accepted quotes by acceptedAt. A demo quote carrying only
        // createdAt was in no period at all — the demo showed a shop that
        // had accepted nothing this month while two accepted jobs sat on
        // its board. Sent the day it was written, accepted two days later.
        ...(spec.status === "sent" || spec.status === "accepted" ? { sentAt: at(-spec.daysAgo) } : {}),
        ...(spec.status === "accepted"
          ? { acceptedAt: at(-(spec.daysAgo - 2)), acceptedTotal: subtotal }
          : {}),
      },
    });
    ...
    if (done) {
      await db.invoice.create({
        data: {
          ...
          // Paid two days after it went out. "Revenue this month" reads
          // paidDate now, not updatedAt; a paid demo invoice without one is
          // revenue in no month.
          sentAt: at(-(spec.daysAgo - 6)), paidDate: at(-(spec.daysAgo - 8)),
        },
      });
    }
  }
  ...
  // ── The receptionist's call list, populated without a real line ever ringing ──
  //
  // A demo's number is simulated (see lib/voice/demoLine.js) precisely so
  // nobody can dial it — which means the receptionist screen's call list would
  // otherwise always be empty, on the one screen a sales call most wants to
  // show working. These are fixtures, exactly like the quotes and invoices
  // above: fake, wiped and re-seeded on every reset (wipeContent), and never
  // pretending to be a real call — see VoiceCall.recoveredAt and toE164, both
  // left null, which is the honest state for a row nothing at Retell produced.
  const callSpecs = [
    {
      client: 0, daysAgo: 6, durationSec: 143,
      disposition: "answered",
      summary: `Asked about ${(titles[0] || "a project").toLowerCase()} — took the address and said someone would follow up with a quote.`,
    },
    {
      client: 2, daysAgo: 3, durationSec: 96,
      disposition: "answered",
      summary: "Wanted to know if photos could be emailed in instead of a site visit — pointed them to the address on file.",
    },
    {
      client: 1, daysAgo: 1, durationSec: 58,
      disposition: "answered",
      summary: "Confirmed the upcoming visit time and asked for a reminder call the morning of.",
      needsReview: false,
    },
    {
      client: 3, daysAgo: 0, durationSec: 21,
      disposition: "voicemail",
      summary: "Caller hung up before leaving details.",
      needsReview: true,
    },
  ];
  let cn = 1;
  for (const spec of callSpecs) {
    ...
    const fromE164 = client?.phone ? `+1${String(client.phone).replace(/\D/g, "")}` : null;
    const startedAt = at(-spec.daysAgo);
    await db.voiceCall.create({
      data: {
        companyId,
        providerCallId: `demo-seed-${companyId}-${cn++}`,
        direction: "inbound",
        fromE164,
        // toE164 stays null: no VoicePhoneNumber exists yet for a freshly
        // seeded demo, and inventing one here would be a second, second-hand
        // copy of whatever number the demo eventually sets up in Settings.
        startedAt,
        endedAt: new Date(startedAt.getTime() + spec.durationSec * 1000),
  ...
  return { clients: clients.length, quotes: specs.length, calls: callSpecs.length };
}
