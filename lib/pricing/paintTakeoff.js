...
export function paintTakeoff(config, book, opts = {}) {
  ...
  for (let areaIndex = 0; areaIndex < rawAreas.length; areaIndex += 1) {
    ...
    const record = {
      ...
      // Internal — printed on the crew's work order (lib/workOrder/build.js)
      // and never on a client surface: ScopeGroupsSection and
      // lib/email/quoteSections read `description` and `amount` and nothing
      // else.
