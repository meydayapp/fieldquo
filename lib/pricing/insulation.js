...
/**
 * Ontario Building Code minimums, SB-12. A floor, not a recommendation.
 *
 * These are what a permit inspector checks, and they are a different kind of
 * number from ENERGY STAR's — one is a code requirement and the other is
 * advice. They are carried separately and labelled separately for that reason:
 * telling an Ontario homeowner that R20 in a basement is "recommended" when it
 * is in fact the legal minimum understates it, and telling them R60 in an attic
 * is "required" everywhere overstates it.
 *
 * Source: Konstruction Group (Toronto), quoting OBC minimums — basement walls
 * R-20, above-grade exterior walls R-22, ceilings and attics R-60. The attic
 * figure happens to agree with ENERGY STAR's Zone 6 recommendation; the wall
 * figure has no ENERGY STAR equivalent, which is exactly why it is here.
 */
export const OBC_MINIMUMS = {
  basement_wall: { label: "Basement wall", r: 20 },
  wall: { label: "Above-grade exterior wall", r: 22 },
  attic: { label: "Ceiling or attic", r: 60 },
};

/** The Ontario code minimum for an assembly, or null where the code is silent. */
export function codeMinimumR(assembly) {
  const row = own(OBC_MINIMUMS, assembly);
...
export function insulationTakeoff(config, material, rates) {
  ...
  // Explicit target wins, then the Ontario code minimum when the estimator has
  // asked for it, then ENERGY STAR. Three bases, resolved in that order, and
  // the takeoff shows which one produced the number — a target nobody can trace
  // is a target nobody can defend at the door.
  let targetBasis = "none";
  let target = positive(c.targetR);
  if (target > 0) {
    targetBasis = "manual";
  } else if (c.targetBasis === "code") {
    target = positive(codeMinimumR(c.assembly));
    if (target > 0) targetBasis = "code";
  }
  if (target <= 0) {
    target = positive(
      recommendedR(c.climateZone, c.assembly, c.existingDepthIn),
    );
    if (target > 0) targetBasis = "energy_star";
  }
  ...
  add(
    ...
    install,
    rated ? `${round1(sqft)} sqft at ${depth.inches}"` : `${round1(sqft)} sqft`,
  );
  ...
  return {
    ...
    targetBasis,
    // Open-cell foam and unfaced batt do not stop vapour; closed-cell is its
    // own barrier at the thicknesses used here. Reported so the builder can
    // price the barrier rather than leave an incomplete assembly on a quote.
    needsVapourBarrier: Boolean(material.needsVapourBarrier),
    rated,
    capped: depth.capped,
    hours: round2(hours),
...
export function insulationCrewDays(
  totalHours,
  ...
  return {
    ...
    hoursPerDay: perDay,
    days:
      hours > 0 ? Math.max(0.5, Math.round((crewHours / perDay) * 10) / 10) : 0,
  };
}
