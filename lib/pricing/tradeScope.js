 ...
 *          addOnUnits?:Record<string, number>}} config
...
export function cabinetAddOnLines(config, book) {
  ...
  if (config?.handleHoles && handleUnits > 0) {
    items.push(
      line("New handle holes drilled in the doors and drawer fronts", {
  ...
  if (config?.threeTone) {
    items.push(
      line("Three-colour finish", {
        ...
        detail: toneUnits > 0 ? `Third colour across ${toneUnits} pieces.` : "",
      }),
    );
    ...
    items.push(
      line("Two-tone finish", {
        ...
        detail: toneUnits > 0 ? `Second colour across ${toneUnits} pieces.` : "",
      }),
    );
  }
