...
function buildCabinets(config, book) {
  ...
  if (doors > 0) {
    items.push(line(`${book.label} — doors${suffix}`, { quantity: doors, unit: "door", rate: doorRate }));
  }
  if (drawers > 0) {
    items.push(line(`${book.label} — drawer fronts${suffix}`, { quantity: drawers, unit: "drawer", rate: drawerRate }));
  }
