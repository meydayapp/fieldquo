...
import { cabinetRunLabour } from "./cabinetLabour";
...
// `detail` is the scope text printed under the line on the client's copy — see
// lib/documentSections/ScopeGroupsSection.js. Omitted rather than emitted empty
// when there is nothing to say: a `detail: ""` on every generated line would
// put an empty paragraph into stored JSON on every quote, and the renderers
// would then have to distinguish "no scope written" from "scope deliberately
// blank" for a distinction nobody is making.
function line(description, { quantity = 1, unit = "flat", rate = 0, detail = "" } = {}) {
...
/**
 * What each cabinet upgrade line SAYS — its name and the scope sentence under
 * it — by document language.
 *
 * English and French are both hand-written here rather than machine-translated
 * at render time (AGENTS.md non-negotiable 6). The name is translated with the
 * sentence: a French detail under an English name is a half-translated line,
 * which reads worse than either language on its own. Nothing keys on these
 * names (the builder's checkbox labels in UnitPricingFields.js are their own
 * strings, and the price book is keyed by `handleHolesPerDoor` etc.), so a
 * French name on a French quote breaks no lookup.
 *
 * The two- and three-tone sentences take the piece count when there is one,
 * because "Two-tone finish" beside a number the client cannot reconstruct is
 * what invites the phone call; with no count the sentence still says what the
 * line is, rather than vanishing — which is what the old
 * `toneUnits > 0 ? … : ""` did.
 */
const CABINET_ADDON_TEXT = {
  en: {
    handleHoles: {
      name: "New handle holes drilled in the doors and drawer fronts",
      detail:
        "Measure, mark and drill new handle holes in the doors and drawer fronts, per piece.",
    },
    softCloseHinges: {
    ...
    drawerSlides: {
      name: "Drawer slides",
      detail: "Supply and fit soft-close full-extension drawer slides, per drawer.",
    },
    twoTone: {
      name: "Two-tone finish",
      detail: (n) =>
        n > 0
          ? `Second colour across ${n} pieces — separate masking, staging and spray cycles.`
          : "Second colour on part of the kitchen — separate masking, staging and spray cycles.",
    },
    threeTone: {
      name: "Three-colour finish",
      detail: (n) =>
        n > 0
          ? `Third colour across ${n} pieces — separate masking, staging and spray cycles for each colour.`
          : "Third colour on part of the kitchen — separate masking, staging and spray cycles for each colour.",
    },
  },
  fr: {
    handleHoles: {
      name: "Nouveaux trous de poignées percés dans les portes et façades de tiroirs",
      detail:
        "Mesure, marquage et perçage des nouveaux trous de poignées dans les portes et façades de tiroirs, par pièce.",
    },
    softCloseHinges: {
      name: "Charnières à fermeture douce",
      detail:
        "Fourniture et pose de charnières à fermeture douce en remplacement des charnières existantes, par porte.",
    },
    drawerSlides: {
      name: "Coulisses de tiroir",
      detail:
        "Fourniture et pose de coulisses de tiroir à extension complète et fermeture douce, par tiroir.",
    },
    twoTone: {
      name: "Fini deux tons",
      detail: (n) =>
        n > 0
          ? `Deuxième couleur sur ${n} pièces — masquage, mise en place et cycles de pulvérisation distincts.`
          : "Deuxième couleur sur une partie de la cuisine — masquage, mise en place et cycles de pulvérisation distincts.",
    },
    threeTone: {
      name: "Fini trois couleurs",
      detail: (n) =>
        n > 0
          ? `Troisième couleur sur ${n} pièces — masquage, mise en place et cycles de pulvérisation distincts pour chaque couleur.`
          : "Troisième couleur sur une partie de la cuisine — masquage, mise en place et cycles de pulvérisation distincts pour chaque couleur.",
    },
  },
};

/** French for "fr" and "fr-*" (the same test lib/documents/serviceContent.js
 *  makes), English for everything else including nothing. */
export function cabinetAddOnText(language) {
  const l = String(language || "").trim().toLowerCase();
  return l === "fr" || l.startsWith("fr-") ? CABINET_ADDON_TEXT.fr : CABINET_ADDON_TEXT.en;
}
 ...
 *
 * ── Counts default, they do not dictate ────────────────────────────────────
 *
 * Each upgrade derives its own quantity from the door and drawer counts,
 * because that is right on the overwhelming majority of jobs and typing it
 * twice is how the two come to disagree. But it is only a DEFAULT: a kitchen
 * where the client wants handles on two doors and nothing else is a real
 * kitchen, and pricing it at 32 units because that is how many pieces exist
 * would put a number on a document that the client did not ask for.
 *
 * So every derived count can be overridden by `addOnUnits[key]`. Absent means
 * "use the default"; a number means the estimator has counted. Zero is a
 * legitimate answer and drops the line, which is why the check below is for
 * a finite number rather than a truthy one.
 *
 * ── Every line says what it is ─────────────────────────────────────────────
 *
 * These are the "default add-ons" the owner meant when he said the AI review
 * thought they were empty: the lines were emitted as a name and a number, with
 * no `detail` under them, so the client's document printed "Soft-close hinges
 * × 24" and the review — shown the name and the detail, on purpose — saw a
 * name with nothing beneath it. Each line now carries its scope sentence, in
 * the quote's own language (`config.language`, fixed at creation; French gets
 * the French sentence, everything else the English). Only NEW lines get it: a
 * stored group is never re-derived (see scopeGroupPayload), so a quote sent
 * last month keeps saying what it said.
 *
 * @param {{doors:number, drawers:number, handleHoles?:boolean,
 *          softCloseHinges?:boolean, drawerSlides?:boolean,
 *          twoTone?:boolean, threeTone?:boolean,
 *          addOnUnits?:Record<string, number>, language?:string}} config
...
export function cabinetAddOnLines(config, book) {
  ...
  const doors = num(config?.doors);
  const drawers = num(config?.drawers);
  const say = cabinetAddOnText(config?.language);

  // The estimator's count when they gave one, the derived count otherwise.
  // Number.isFinite rather than `|| fallback`: 0 is a real answer ("they only
  // want it on the two by the sink" — then changed their mind), and `||` would
  // silently restore the full count on exactly the job where somebody had been
  // most deliberate.
  const override = (key) => {
    const v = Number(config?.addOnUnits?.[key]);
    return Number.isFinite(v) && v >= 0 ? v : null;
  };
  const countFor = (key, derived) => override(key) ?? derived;
  ...
  // Doors AND drawers. This counted doors only, which is wrong on any kitchen
  // built this century: a drawer front takes a handle exactly like a door does,
  // and a 24-door/8-drawer kitchen was quoted 24 holes for 32 pieces. The
  // price-book key is still `handleHolesPerDoor` — renaming a stored key to fix
  // a quantity would re-price every company's book on deploy — so the name now
  // reads as "per door-or-drawer-front", and the line says which.
  const handleUnits = countFor("handleHoles", doors + drawers);
  if (config?.handleHoles && handleUnits > 0) {
    items.push(
      line(say.handleHoles.name, {
        quantity: handleUnits,
        unit: "piece",
        rate: a.handleHolesPerDoor,
        detail: say.handleHoles.detail,
      }),
    );
  }
  // Hinges stay on doors. A drawer runs on slides, not hinges — the line below
  // is the drawer's equivalent, and adding drawers here would bill twice for
  // the same piece.
  const hingeUnits = countFor("softCloseHinges", doors);
  if (config?.softCloseHinges && hingeUnits > 0) {
    items.push(
      line(say.softCloseHinges.name, {
        quantity: hingeUnits,
        unit: "door",
        rate: a.softCloseHingesPerDoor,
        detail: say.softCloseHinges.detail,
      }),
    );
  }
  const slideUnits = countFor("drawerSlides", drawers);
  if (config?.drawerSlides && slideUnits > 0) {
    items.push(
      line(say.drawerSlides.name, {
        quantity: slideUnits,
        unit: "drawer",
        rate: a.drawerSlidesPerDrawer,
        detail: say.drawerSlides.detail,
      }),
    );
  }
  ...
  //
  // A second colour is almost never the whole kitchen — it is the island, or
  // the uppers. Priced across every piece by default because that is the
  // safe reading of an unanswered question, but the count is the one an
  // estimator most often needs to change, so it takes an override like the
  // rest. The line says how many pieces it covers, because "Two-tone finish"
  // beside a number the client cannot reconstruct is what invites the phone
  // call.
  const toneUnits = countFor("tone", doors + drawers);
  if (config?.threeTone) {
    items.push(
      line(say.threeTone.name, {
        quantity: 1,
        unit: "job",
        rate: num(a.threeToneFlat) + num(a.threeTonePerUnit) * toneUnits,
        detail: say.threeTone.detail(toneUnits),
      }),
    );
  } else if (config?.twoTone) {
    items.push(
      line(say.twoTone.name, {
        quantity: 1,
        unit: "job",
        rate: num(a.twoToneFlat) + num(a.twoTonePerUnit) * toneUnits,
        detail: say.twoTone.detail(toneUnits),
      }),
    );
  }
...
/**
 * Painting by area and substrate — the model the two painting books share.
 *
 * ── One line per substrate, quoted flat, with the measurement in the text ───
 *
 * The obvious shape is quantity 414, unit "sqft", rate $1.15 — and it is
 * wrong, because a substrate's price is labour PLUS paint and the blended
 * per-unit rate almost never survives rounding to cents: 414 sqft of wall
 * comes to $474.61, which is $1.146... a foot, and $1.15 x 414 is $476.10.
 * Every renderer in this codebase treats `amount` as the truth, so a rate that
 * disagrees with it by $1.49 is a number an estimator will one day have to
 * defend. Flat lines with "(414 sqft)" in the description say the same thing
 * and are exactly right.
 *
 * OPTIONAL areas and substrates are NOT here. They leave the priced scope
 * entirely and come back as QuoteAddOn rows the client ticks — see
 * paintOptionalExtras() in lib/pricing/paintTakeoff.js.
 */
function buildPaintAreas(config, book) {
  const result = paintTakeoff(config, book?.takeoff);
  const items = [];
  for (const area of result.areas) {
    // The area's client note prints under its first priced line — the same
    // `detail` a line item already carries to the PDF and the email. It used
    // to reach the document only when the area was OPTIONAL (as the offer's
    // detail); an included room's note was stored and printed nowhere.
    let note = String(area.clientNote || "").trim();
    for (const l of area.lines) {
      if (l.optional || l.amount <= 0) continue;
      const measure =
        l.kind === "prep"
          ? `${l.displayHours} h`
          : `${l.quantity} ${l.unit}`;
      items.push(
        line(`${area.label} — ${l.label} (${measure})`, {
          rate: l.amount,
          detail: note,
        }),
      );
...
function buildInteriorPaint(config, book) {
  // The discriminator, and the reason an existing quote never reprices: a
  // takeoff stored before this model landed has no `model` key and keeps
  // taking the complexity-grid path below, unchanged.
  if (config?.model === "area_substrate") return buildPaintAreas(config, book);
 ...
 * @param {object} [opts]
 * @param {string} [opts.language]  the DOCUMENT's language, for the complexity
 *   reason lines — a quote keeps the language it was created in
 *   (non-negotiable #6), so this is the quote's, never the estimator's.
 * @param {string} [opts.label]     the group's own name, used on the Specialty
 *   line so the client reads "Front staircase — on-site assessment required"
 *   rather than a bare sentence with nothing attached to it.
 * @returns {Array} line items in fieldquo's shape; [] for a trade with no book
 */
export function buildTradeLineItems(categoryKey, config, overrides, opts = {}) {
  ...
  // The factor module, or null on every takeoff written before it existed —
  // `model: "factors_v1"` is the discriminator and nothing else reaches here.
  ...
  // Specialty prices NOTHING. Not the elements, not a placeholder, not zero:
  // the group becomes one unpriced text line saying an assessment is needed,
  // and lineShowsAmount() keeps the amount column empty on the PDF, the email
  // and the builder table. A $0.00 beside a staircase is a number a homeowner
  // can hold a contractor to, which is the opposite of what "we need to see
  // it" means.
  if (resolved && !resolved.priced) {
    return [specialtyLine(resolved, opts.label || book.label || "")];
  }
  ...
  let items;
  try {
    items = builder(config, book).filter((i) => i && Number.isFinite(i.amount));
  } catch {
    return [];
  }

  return withComplexityReasons(items, resolved, opts.language || "en");
}

/**
 * Put the ticked factors on the client's copy, and the level on the office's.
 *
 * The reasons go into the FIRST line's `detail` — the field
 * lib/documentSections/ScopeGroupsSection.js prints under the line on the
 * quote, the PDF and the email. Appended to whatever detail that line already
 * carried rather than replacing it: a paint line's "two coats over primer" is
 * not less true because the staircase is also curved, and clobbering it would
 * be the destructive-operation-labelled-as-cosmetic failure.
 *
 * `meta.complexity` carries the level, the score and the multiplier. `meta` is
 * never rendered on a client-facing surface — ScopeGroupsSection reads
 * description, detail and amount and nothing else — so the office sees the
 * arithmetic and the homeowner sees the work, which is the split the owner
 * asked for.
 */
function withComplexityReasons(items, resolved, language) {
  if (!resolved || !items.length) return items;
  ...
  return [
    {
      ...first,
      ...(detail
        ? { detail: first.detail ? `${String(first.detail).trim()}\n${detail}` : detail }
        : {}),
      meta: { ...(first.meta || {}), complexity: meta },
    },
    ...rest,
  ];
}
 ...
 *
 * ── The complexity multiplier lands HERE, and only here ────────────────────
 *
 * This is the one function that answers "how long", so it is the one place a
 * complexity level is allowed to change a number. Materials never pass through
 * it — lib/costing/tradeMaterials.js counts bundles and gallons on its own —
 * which is exactly the separation the owner asked for: a Complex staircase is
 * 1.75× the crew's day and 1× the stain. Multiplying materials as well would
 * make the job-costing screen report a quantity nobody will ever buy, and the
 * close-out calibration would then "learn" from it.
 *
 * A takeoff with no `complexity` object, or one written before this module
 * existed, returns the hours unchanged — resolveComplexity() returns null and
 * applyComplexityToHours() passes its argument straight through.
 */
export function tradeLabourHours(categoryKey, config, overrides) {
  const book = getPriceBook(categoryKey, overrides);
  if (!book || !config || typeof config !== "object") return 0;
  const hours = baseTradeLabourHours(categoryKey, config, book);
  if (!hours) return hours;
  return applyComplexityToHours(
    hours,
    resolveComplexity({ trade: categoryKey, complexity: config.complexity, book }),
  );
}

/** The measured hours, before any complexity level is applied. */
function baseTradeLabourHours(categoryKey, config, book) {
  ...
  // Cabinets, because the hours ARE the model and this trade reported ZERO.
  // A per-door rate returns the same number for a greasy 1980s kitchen at three
  // coats with frame-mounted hinges as for a two-year-old IKEA install at one —
  // and cabinet groups carry no `takeoff` at all, so the costing had nothing to
  // read and every cabinet quote was scored labour-blind on this product's
  // highest-volume trade. See lib/pricing/cabinetLabour.js.
  if (isUnitPriced(categoryKey)) {
    const detail = cabinetRunLabour(config, book);
    return detail.incomplete ? 0 : detail.hours;
  }

  // Painting, because the hours are the whole model: a production rate IS an
  // hours figure, and until this landed the trade reported 0 predicted hours
  // and the cost panel scored every painting job labour-blind.
  //
  // These are the SAME hours the client-facing price was built from, and they
  // are deliberately returned bare. lib/costing/quoteCosting.js multiplies them
  // by the crew's burdened COST (FALLBACK_LABOUR_RATE = 35 with no crew);
  // buildPaintAreas multiplied them by the hourly SELL rate (85 interior, 80
  // exterior). One set of hours, two rates, and neither function can see the
  // other's — which is the only reason the margin figure means anything.
  if (categoryKey === "interior_painting" || categoryKey === "exterior_painting") {
    if (config?.model !== "area_substrate") return 0;
    return num(paintTakeoff(config, book?.takeoff).hours);
  }
...
export function tradeLabourDetail(categoryKey, config, overrides) {
  ...
  // The SAME multiplier tradeLabourHours applies, on the same numbers.
  //
  // Without this the two disagree. lib/proposal/load.js prefers the DETAIL and
  // only falls back to the total, so a Complex roof would print an
  // unmultiplied day-by-day plan on the homeowner's proposal while the cost
  // panel scored the job against the multiplied total — one roof, two answers
  // to "how long", on a document somebody signs.
  const scaled = (detail) =>
    scaleLabourDetail(
      detail,
      resolveComplexity({ trade: categoryKey, complexity: config.complexity, book }),
    );
  if (categoryKey === "roofing_service") return scaled(roofRunLabour(config, book));
  if (categoryKey === "paving") return scaled(paverLabour(config, book.labour));
  if (categoryKey === "insulation") return scaled(insulationRunLabour(config, book));
  if (
    (categoryKey === "interior_painting" ||
      categoryKey === "exterior_painting") &&
    config?.model === "area_substrate"
  ) {
    const r = paintTakeoff(config, book?.takeoff);
    return {
      hours: r.hours,
      // Area by area rather than substrate by substrate: the estimator's
      // question here is "which room is eating the day", and 40 substrate rows
      // answer it worse than 6 room rows.
      parts: r.areas.map((a) => ({
        name: a.label,
        hours: a.hours,
        displayHours: a.displayHours,
      })),
      // What to buy, and how many lines have paint with no price on it.
      purchase: r.purchase,
...
/**
 * An itemised labour detail, with the complexity level applied to every hour
 * in it and to its total.
 *
 * Every field that is hours gets the same factor, so the parts still add up to
 * the total — a breakdown that no longer sums is worse than no breakdown,
 * because the estimator checks it once, finds it wrong, and stops reading it.
 * `purchase`, `unpricedCount` and the material-shaped fields are passed
 * through untouched: this multiplies time, never stuff.
 *
 * Returns its argument unchanged for a takeoff not on the factor model, and
 * for an incomplete one — there is nothing to scale when the engine already
 * said it could not answer.
 */
function scaleLabourDetail(detail, resolved) {
  if (!detail || detail.incomplete || !resolved) return detail;
  const m = resolved.multiplier;
  if (m == null || !(m > 0) || m === 1) return detail;
  const scale = (h) => applyComplexityToHours(h, resolved);
  const out = { ...detail };
  if (Number.isFinite(detail.hours)) out.hours = scale(detail.hours);
  if (Number.isFinite(detail.labourHours)) out.labourHours = scale(detail.labourHours);
  if (Number.isFinite(detail.crewHours)) out.crewHours = scale(detail.crewHours);
  // Days follow the hours rather than being re-derived here: the engine that
  // produced them owns how a day is divided (its own hoursPerDay, its own
  // minimum half-day), and re-deriving would be a second opinion about it.
  if (Number.isFinite(detail.days)) out.days = Math.round(detail.days * m * 10) / 10;
  for (const key of ["parts", "breakdown", "steps"]) {
    if (Array.isArray(detail[key])) {
      out[key] = detail[key].map((row) => ({
        ...row,
        ...(Number.isFinite(row?.hours) ? { hours: scale(row.hours) } : {}),
        ...(Number.isFinite(row?.displayHours)
          ? { displayHours: scale(row.displayHours) }
          : {}),
      }));
    }
  }
...
/**
 * The optional scope a takeoff offers the client, as add-on rows.
 *
 * "Optional" is only real if unticking it moves the number on the document
 * they sign. The mechanism that already does that is QuoteAddOn: the browser
 * posts ids, the server reads amounts off its own rows and reprices (see
 * app/api/public/quotes/[token]/route.js). So an optional area or substrate is
 * left out of the priced scope above and returned here instead, and the quote
 * save route turns these into rows. Nothing renders an "optional" checkbox
 * that does not end up in this list.
 *
 * Each row also names the area it belongs under (`areaLabel`, `areaIndex`)
 * and what it is (`kind`), so the client's page can draw a room's options
 * beside the room. Neither carries a rate.
 *
 * @returns {Array<{description:string, detail:string|null, amount:number,
 *   areaLabel:string|null, areaIndex:number|null, kind:string}>}
 */
export function tradeOptionalExtras(categoryKey, config, overrides) {
  if (categoryKey !== "interior_painting" && categoryKey !== "exterior_painting")
    return [];
  if (!config || typeof config !== "object" || config.model !== "area_substrate")
    return [];
  const book = getPriceBook(categoryKey, overrides);
  if (!book) return [];
  try {
    return paintOptionalExtras(config, book.takeoff).filter(
      (e) => Number.isFinite(e.amount) && e.amount > 0,
    );
  } catch {
    // Same reasoning as buildTradeLineItems: a malformed stored takeoff must
    // not take the save down. No extras is recoverable and visibly wrong.
    return [];
  }
}
  ...
  switch (categoryKey) {
    ...
    case "stairs":
      return {
        ...
        basement: false,
        basementTreads: 0,
        // Every factor on its benign answer. That is a STATEMENT — "nothing
        // unusual about this staircase" — not a default padding an absence,
        // and it is the one the estimator changes rather than has to create.
        // Score 0, level Standard, multiplier 1, no reason lines: identical to
        // what a takeoff with no complexity object at all produces, which is
        // why seeding it cannot move an existing price.
        complexity: newComplexity("stairs"),
        notes: "",
      };
    case "flooring":
    ...
    case "roofing_service":
      return {
        ...
        crewSize: 2,
        measuredFrom: "",
        // Same reasoning as stairs above: the benign answer to each of the
        // seven roof conditions, which scores 0 and changes nothing until an
        // estimator says otherwise.
        complexity: newComplexity("roofing_service"),
        notes: "",
      };
    case "countertop":
      return {
    ...
    // Both painting trades open on the area/substrate model. `model` is what
    // every consumer branches on, so it is the one key that must be present:
    // a config without it is a quote written before this existed, and it keeps
    // pricing off the complexity grid forever. New ones never do.
    //
    // No seeded area. An estimator adds the first room and measures it — a
    // blank "Room 1" with 0 x 0 x 0 in it is a row of zeroes pretending to be
    // a measurement, and the old `defaultSqft` per room type is exactly the
    // habit this model exists to break.
    //
    // `estimateType` is the first question on the screen (Interior, Exterior,
    // Cabinets & millwork, Staining, Commercial — lib/pricing/paintTakeoff.js)
    // and is preselected from the trade the group was filed under; the
    // estimator changes it with one tap. A stored takeoff without one keeps
    // pricing exactly as it did — see estimateTypeOf().
    case "interior_painting":
      return { model: "area_substrate", estimateType: "interior", areas: [], notes: "" };
    case "exterior_painting":
      return {
        model: "area_substrate",
        estimateType: "exterior",
        areas: [newPaintArea("exterior", book?.takeoff, { estimateType: "exterior" })],
        notes: "",
      };
    case "snow_removal":
      return {
