...
function buildSnowRemoval(config, book) {
  ...
  const drive = book.driveways?.[config.drivewaySize];
  if (drive && num(drive.price) > 0) {
    items.push(line(`${drive.label}${suffix}`, { rate: drive.price }));
  }

  if (config.walkway && num(e.walkwayPrice) > 0) {
    items.push(line("Walkway clearing", { rate: e.walkwayPrice }));
  }
  if (config.frontSteps && num(e.frontStepsPrice) > 0) {
    items.push(line("Front steps", { rate: e.frontStepsPrice }));
  }
  if (
    config.salting &&
  ...
  // Per-visit work sits alongside a seasonal contract rather than replacing
  // it: a company can sell the season and still bill an extra clear-out.
  ...
  // A discount, so it must never invent money: it is capped at the total and
  // emitted as a negative line the client can see, not folded silently into
  // the rate above it.
