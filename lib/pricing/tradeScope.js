...
function buildStairs(config, book) {
  ...
  for (const section of asArray(config.sections)) {
    ...
    if (treads > 0) {
      items.push(line(`${title} — treads`, { quantity: treads, unit: "tread", rate: c.treadPrice }));
    }
    ...
    if (section.paintRisers && num(section.risers) > 0) {
      items.push(line(`${title} — risers`, { quantity: num(section.risers), unit: "riser", rate: c.riserPrice }));
    }
    if (section.paintBalusters && num(section.balusters) > 0) {
      items.push(line(`${title} — balusters`, { quantity: num(section.balusters), unit: "each", rate: c.balusterPrice }));
    }
    if (section.paintPosts && num(section.posts) > 0) {
      items.push(line(`${title} — newel posts`, { quantity: num(section.posts), unit: "each", rate: c.postPrice }));
    }
    if (num(section.handrailFt) > 0) {
      items.push(line(`${title} — handrail`, { quantity: num(section.handrailFt), unit: "lf", rate: c.handrailPricePerFt }));
    }
    if (num(section.landingSqft) > 0) {
      items.push(line(`${title} — landing`, { quantity: num(section.landingSqft), unit: "sqft", rate: c.landingPricePerSqft }));
    }
    if (section.twoTone) {
      items.push(line(`${title} — two-tone finish`, { rate: c.twoToneSurcharge }));
    }
  }
