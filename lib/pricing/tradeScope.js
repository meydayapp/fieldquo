...
function buildCabinets(config, book) {
  ...
  const doorRate = num(book.perDoor) + uplift;
  const drawerRate = num(book.perDrawer) + uplift;

  if (doors > 0) {
    items.push(line(`${book.label} — doors`, { quantity: doors, unit: "door", rate: doorRate }));
  }
  if (drawers > 0) {
    items.push(line(`${book.label} — drawer fronts`, { quantity: drawers, unit: "drawer", rate: drawerRate }));
  }

  const a = book.addOns || {};
  if (config.handleHoles && doors > 0) {
    items.push(line("New handle holes (drilling)", { quantity: doors, unit: "door", rate: a.handleHolesPerDoor }));
  }
  if (config.softCloseHinges && doors > 0) {
    items.push(line("Soft-close hinges", { quantity: doors, unit: "door", rate: a.softCloseHingesPerDoor }));
  }
  if (config.drawerSlides && drawers > 0) {
    items.push(line("Drawer slides", { quantity: drawers, unit: "drawer", rate: a.drawerSlidesPerDrawer }));
  }
  ...
  if (config.threeTone) {
    items.push(line("Three-colour finish", { rate: num(a.threeToneFlat) + num(a.threeTonePerUnit) * units }));
  } else if (config.twoTone) {
    items.push(line("Two-tone finish", { rate: num(a.twoToneFlat) + num(a.twoTonePerUnit) * units }));
  }
  ...
  if (items.length > 0 && minimum > 0 && runningTotal < minimum) {
    items.push(line("Job minimum adjustment", { rate: minimum - runningTotal }));
  }
...
function buildStairs(config, book) {
  ...
  for (const section of asArray(config.sections)) {
    ...
    if (treads > 0) {
      items.push(line(`${title} — treads`, { quantity: treads, unit: "tread", rate: rateFor("treadPrice") }));
    }
    ...
    if (section.paintRisers && num(section.risers) > 0) {
      items.push(line(`${title} — risers`, { quantity: num(section.risers), unit: "riser", rate: rateFor("riserPrice") }));
    }
    if (section.paintBalusters && num(section.balusters) > 0) {
      items.push(line(`${title} — balusters`, { quantity: num(section.balusters), unit: "each", rate: rateFor("balusterPrice") }));
    }
    if (section.paintPosts && num(section.posts) > 0) {
      items.push(line(`${title} — newel posts`, { quantity: num(section.posts), unit: "each", rate: rateFor("postPrice") }));
    }
    if (num(section.handrailFt) > 0) {
      items.push(line(`${title} — handrail`, { quantity: num(section.handrailFt), unit: "lf", rate: rateFor("handrailPricePerFt") }));
    }
    if (num(section.landingSqft) > 0) {
      items.push(line(`${title} — landing`, { quantity: num(section.landingSqft), unit: "sqft", rate: rateFor("landingPricePerSqft") }));
    }
    if (section.twoTone) {
      items.push(line(`${title} — two-tone finish`, { rate: rateFor("twoToneSurcharge") }));
    }
  }

  if (config.basement && num(config.basementTreads) > 0) {
    items.push(line("Basement stairs — treads", {
...
function buildFlooring(config, book) {
  ...
  for (const section of asArray(config.sections)) {
    ...
    const detail = [section.woodSpecies, section.finishType].filter(Boolean).join(" · ");
    items.push(line(detail ? `${title} — refinishing (${detail})` : `${title} — refinishing`, {
      quantity: sqft,
      unit: "sqft",
      rate: c.pricePerSqft,
    }));
    ...
    if (section.stainChange) {
      items.push(line(`${title} — stain colour change`, { quantity: sqft, unit: "sqft", rate: c.stainChangePricePerSqft }));
    }
    if (section.gapFilling) {
      items.push(line(`${title} — gap filling`, { quantity: sqft, unit: "sqft", rate: c.gapFillingPricePerSqft }));
    }
    if (section.waterDamageRepair) {
      items.push(line(`${title} — water damage repair`, { rate: c.waterDamagePrice }));
    }
    if (section.furnitureMoving) {
      items.push(line(`${title} — furniture moving`, { rate: c.furnitureMovingPrice }));
    }
    if (section.stairBlending) {
      items.push(line(`${title} — stair blending`, { rate: c.stairBlendingPrice }));
    }
  }
  return items;
}
...
function buildCountertop(config, book) {
  ...
  for (const item of asArray(config.items)) {
    ...
    const rate = clientPriceFromCost(item.supplierCost, markupPct, item.override);
    if (rate <= 0) continue;
...
function buildInteriorPaint(config, book) {
  ...
  for (const room of asArray(config.rooms)) {
    ...
    if (room.walls && sqft > 0) {
      items.push(line(`${title} — walls`, { quantity: sqft, unit: "sqft", rate: c.wallPricePerSqft }));
    }
    if (room.ceiling) items.push(line(`${title} — ceiling`, { rate: c.ceilingPrice }));
    if (room.trim) items.push(line(`${title} — trim`, { rate: c.trimPrice }));
    if (room.doors && num(room.doorsCount) > 0) {
      items.push(line(`${title} — doors`, { quantity: num(room.doorsCount), unit: "door", rate: c.doorPrice }));
    }
    if (room.closets && num(room.closetsCount) > 0) {
      items.push(line(`${title} — closets`, { quantity: num(room.closetsCount), unit: "each", rate: c.closetPrice }));
    }
    if (room.colorChange) items.push(line(`${title} — colour change`, { rate: c.colorChangeSurcharge }));
    if (room.drywallPrep) items.push(line(`${title} — drywall prep`, { rate: c.drywallPrepPrice }));
  }

  const g = book.global || {};
  if (config.popcornRemoval && num(config.popcornSqft) > 0) {
    items.push(line("Popcorn ceiling removal", {
      quantity: num(config.popcornSqft),
      unit: "sqft",
      rate: g.popcornRemovalPricePerSqft,
    }));
  }
  if (config.furnitureMoving) items.push(line("Furniture moving", { rate: g.furnitureMovingPrice }));
  return items;
}
...
function buildExteriorPaint(config, book) {
  ...
  for (const item of asArray(config.items)) {
    ...
    const baseRate =
      bookItem.priceType === "flat" ? num(bookItem.flatPrice) : num(c[bookItem.priceType]);
    const rate = num(item.override) > 0 ? num(item.override) : baseRate;
    if (rate <= 0) continue;
    items.push(line(bookItem.label || item.id, { quantity: qty, unit: bookItem.unit || "each", rate }));
  }

  const e = book.extras || {};
  if (config.pressureWashing) items.push(line("Pressure washing", { rate: e.pressureWashingPrice }));
  if (config.priming && num(config.primeSqft) > 0) {
    items.push(line("Priming", { quantity: num(config.primeSqft), unit: "sqft", rate: e.primePricePerSqft }));
  }
  return items;
}
...
export function tradeSubtotal(categoryKey, config, overrides) {
  return money(
    buildTradeLineItems(categoryKey, config, overrides).reduce((s, i) => s + i.amount, 0),
  );
}
...
export function createTradeConfig(categoryKey, overrides) {
  ...
  switch (categoryKey) {
    ...
    case "cabinet_refacing":
      return {
        doors: 0, drawers: 0, complexityLevel: "standard",
        handleHoles: false, softCloseHinges: false, drawerSlides: false,
        twoTone: false, threeTone: false, colour: "", sheen: "", notes: "",
      };
    case "stairs":
      return {
        sections: [newStairSection("Main Staircase")],
        basement: false, basementTreads: 0, notes: "",
      };
    case "flooring":
    ...
    case "interior_painting":
      return {
        ...
        popcornRemoval: false, popcornSqft: 0,
        furnitureMoving: false, notes: "",
      };
    case "exterior_painting":
      return {
        complexityLevel: "standard",
        pressureWashing: false, priming: false, primeSqft: 0, notes: "",
        items: (book.items || []).map((i) => ({
          id: i.id, enabled: false, quantity: 0, override: 0,
        })),
      };
    default:
      return null;
...
export function newStairSection(title = "Staircase") {
  return {
    title, complexityLevel: "standard",
    treads: 0, risers: 0, balusters: 0, posts: 0,
    handrailFt: 0, landingSqft: 0,
    paintRisers: false, paintBalusters: false, paintPosts: false,
    twoTone: false, stainColour: "", notes: "",
  };
}

export function newFloorSection(title = "Floor Area") {
  return {
    title, complexityLevel: "standard", sqft: 0, rooms: 1,
    woodSpecies: "", finishType: "",
    stainChange: false, waterDamageRepair: false, gapFilling: false,
    furnitureMoving: false, stairBlending: false, notes: "",
  };
}

export function newPaintRoom(title = "Room") {
  return {
    title, roomType: "bedroom", complexityLevel: "standard", sqft: 0,
    walls: true, ceiling: false, trim: false,
    doors: false, doorsCount: 1, closets: false, closetsCount: 1,
    colorChange: false, drywallPrep: false, notes: "",
  };
}
