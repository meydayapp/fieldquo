...
/**
 * The price tier a resolved level presses on the controls that already price
 * the job — the stair section's Standard / Moderate / High tiles and the
 * cabinet chips — or null when the level must not touch them.
 *
 * ONE mapping, the one app/data/cabinetPricing.js factorLevelUpcharge already
 * prices by (Complex takes the High figure), so the picker can never move a
 * tile to a tier the price path would not have chosen for the same answers.
 * Specialty is null: that group becomes an unpriced "on-site assessment" line
 * and no tier means anything under it. An unknown level is null rather than
 * Standard — a stale value must not quietly reset a tier somebody pressed.
 */
