 ...
 *
 * The FACTORS are the reference calculator's, unchanged: walkable 1.00,
 * moderate 1.30, steep 1.60. Keeping them identical means a company that has
 * been quoting off that calculator gets the same pitch answer here and can
 * see the difference is coming from tear-off and details, not from a number
 * moved under them.
 *
...
export const PITCH_BANDS = [
  ...
  { maxRise: 12, factor: 1.6, key: "steep", label: "Steep (10–12/12)" },
  {
    maxRise: Infinity,
    factor: 2.0,
    key: "very_steep",
...
export const ROOF_LABOUR_DEFAULTS = {
  ...
  //
  // WAS 0.7 and 0.45. Every crew figure found for a strip lands under that —
  // 15 sq, one layer, two people, 2–3 h (0.27–0.40 h/sq); 25 sq, two layers,
  // three people, 4–5 h (0.48–0.60 h/sq for BOTH layers); an eight-man crew
  // stripping and drying-in 25 sq in 2.5 h (0.8 h/sq, and that includes the
  // underlayment). 0.5 sits at the top of what a real one-layer strip
  // measures, which is where a default for a crew you have never met belongs.
  // Sources, with URLs, in docs/research/ROOFING-RATES-2026.md §1.
  tearOffFirstLayerPerSquare: 0.5,
  tearOffAdditionalLayerPerSquare: 0.3,
