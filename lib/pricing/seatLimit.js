...
export function seatCheck({ roster = [], plan = null, incoming = null } = {}) {
  ...
  // A custom row ("custom-20") is a rung too — read off its own key so the
  // refusal names "Your Custom plan" and the right counts.
  const customSeats = customSeatsFromTierKey(plan?.tierKey);
  const tier =
    SEAT_LADDER.find((t) => t.tierKey === plan?.tierKey) ||
    (customSeats ? customTier(customSeats) : null);
  // The smallest rung that fits them AFTER the person they are adding — the
  // same question tierFor() answers, asked through it so the two cannot
  // disagree. Past Scale that is a custom size; null means more than a
  // hundred people, which is a conversation rather than an upgrade button.
  const nextTier = tierFor({ seats: seatsAfter, crew: crewAfter });
...
export function seatLimitMessage(check) {
  ...
  // A custom size is named with its count — "Custom covers 12 seats" tells
  // the reader what the fifth card on the billing page will be set to.
  const upgrade = next
    ? ` Need more? ${next.custom ? `A custom plan at ${next.seats} seats` : next.label} covers ${next.seats} ${next.seats === 1 ? "seat" : "seats"} and ${next.crewSeats} crew.`
