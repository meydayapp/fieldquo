...
export const EMBED_HEIGHTS = {
  book: 640,
  quote: 640,
  // The instant estimate is a short question list that ends in a price. It is
  // the shortest of the three forms and the one most likely to sit halfway
  // down a marketing page, so it starts smaller.
  "instant-quote": 560,
  reviews: 220,
  // A funnel is one question per screen by design, so any single step is
  // short — but the steps differ in height and the box must not jump on every
  // tap. 520 holds a question, its options and the button without the frame
  // resizing for most steps.
  funnel: 520,
};

/**
 * The widgets the embed routes serve.
 *
 * `funnel` is the odd one and is excluded from this list on purpose: every
 * other widget is one-per-company, so the widget name is enough to address it.
 * A company has many funnels, so that one needs a second identifier and lives
 * at /embed/[companySlug]/funnel/[funnelSlug]. Callers ask for it by passing
 * `funnelSlug` to embedSnippet rather than by finding it here.
 */
export const EMBED_WIDGETS = Object.keys(EMBED_HEIGHTS).filter(
  (w) => w !== "funnel",
);
 ...
 */
export function embedSnippet({ origin, slug, widget, title, funnelSlug } = {}) {
  ...
  // A funnel needs a second identifier, and without one there is no snippet to
  // give — returning "" rather than a URL that 404s on the contractor's site.
  if (widget === "funnel" && !funnelSlug) return "";
  if (funnelSlug && /["'<>\s]/.test(funnelSlug)) return "";

  // Two funnels on one page must not share an element id, or the height
  // listener resizes whichever it finds first. Every other widget is
  // one-per-company so its name is already unique.
  const id = funnelSlug
    ? `fieldquo-funnel-${String(funnelSlug).replace(/[^a-zA-Z0-9_-]/g, "-")}`
    : `fieldquo-${widget}`;
  const src = funnelSlug
    ? `${origin}/embed/${encodeURIComponent(slug)}/funnel/${encodeURIComponent(funnelSlug)}`
    : `${origin}/embed/${encodeURIComponent(slug)}/${widget}`;
