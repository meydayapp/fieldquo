...
// root LanguageProvider, which was handed no language and fell back to the
// browser's own signals — at the time a PERMANENT localStorage key,
// "fieldquo-language", then navigator.language. That key was shared with the
// marketing site and written by anything that changed the language (the
// header's switcher, /app/settings/language, a contractor's booking page), so
// the answer depended on what that browser had last been told, not on the
// person — and an email app's in-app browser has a different store and a
// different navigator.language altogether.
//
// The key is gone now (same day, lib/i18n/statedLanguage.js): a browser signal
// is this tab's explicit switch (sessionStorage) or the device, and a
// signed-in browser learns the account's language even on a public page. The
// link still carries the language, because the person clicking it may be on a
