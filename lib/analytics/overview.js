...
export async function getAnalyticsOverview({ companyId }) {
  ...
  const [
    ...
    db.invoice.aggregate({
      where: { companyId, status: "paid", paidDate: { gte: startOfMonth } },
      _sum: { total: true },
      _count: true,
    }),
    ...
    db.invoice.aggregate({
      where: { companyId, status: "paid", paidDate: { gte: startOfYear } },
      _sum: { total: true },
    }),
    ...
    db.quote.count({
      where: {
        companyId,
        status: "accepted",
        acceptedAt: { gte: startOfLastMonth, lt: endOfLastMonth },
      },
    }),
    db.quote.count({
      where: {
