...
export function toOpportunities(quotes) {
  ...
  for (const siblings of groups.values()) {
    ...
    // When this opportunity enters a period. Normally the earliest send. A
    // quote accepted or declined without ever being stamped sent — marked
    // accepted by hand after a phone call, or imported — was still offered
    // and still decided, and its decision date is a recorded fact rather than
    // a guess. Dating it by that is not padding absent data (AGENTS.md
    // failure class 5); dropping it was the same failure pointed the other
    // way: two accepted quotes worth $16,633 with jobs and invoices behind
    // them were reported as "left draft" because nobody had pressed Send.
    // Only a quote with neither date belongs to no period. `datedBy` says
    // which date was used, so the report can say so too rather than letting
    // a decision date pass for a send date in the time-to-decision figure —
    // which keeps using `sentAt` alone.
    const datedAt = sentAt || decidedAt || null;
    const datedBy = sentAt ? "sent" : decidedAt ? "decision" : null;
 ...
 *   `sentAt`, no `acceptedAt` and no `declinedAt`, company-wide. They are in
 *   NO period and are reported as an exclusion so the totals never quietly
 *   disagree with the quote list. A quote with a decision date but no send
 *   date is NOT undated — it belongs to the period of its decision.
...
export function buildWinLoss({
  ...
  // An opportunity with no send date AND no decision date is not in this
  // period and not in any other. Counted, named, and left out of every
  // figure. One with a decision but no send date is in the period of its
  // decision — see toOpportunities on `datedAt` — and the report says how
  // many were dated that way, because "sent" in the counts below then means
  // "went out, or was decided", and a reader is owed that sentence.
  const undatedInRows = all.filter((o) => !o.datedAt).length;
  const inRange = all.filter(
    (o) => o.datedAt && o.datedAt.getTime() >= startTs && o.datedAt.getTime() <= endTs,
  );
  const datedByDecision = inRange.filter((o) => o.datedBy === "decision").length;
  ...
  if (datedByDecision > 0) {
    notes.push({ code: "dated_by_decision", count: datedByDecision });
  }
  ...
  return {
    ...
    // can see on the quotes list. `datedByDecision` is not an exclusion — those
    // ARE in the counts — but it sits here because it is the other half of the
    // same accounting: how many of the counted were placed by a decision date.
    excluded: { undated: Math.max(undatedCount, undatedInRows), datedByDecision },
    notes,
  };
}
