...
/** The issue date of a family, and which column it came from — lib/invoices/issueDate.js's rule. */
 ...
 * @param {object[]} [p.followUpLogs]  FollowUpLog rows for invoices
 *                              ({ entityId, sentAt }) — the automated
 *                              overdue reminders the cron has already sent
...
export function buildReceivables({
  ...
  // Automated reminders, latest per row. The cron logs against whichever
  // version it emailed, so a family's answer is the max over its members,
  // taken below with the same family-wide rule payments use.
  const autoReminderByRow = new Map();
  for (const log of followUpLogs) {
    if (!log?.entityId) continue;
    const when = log.sentAt instanceof Date ? log.sentAt : new Date(log.sentAt);
    if (Number.isNaN(when.getTime())) continue;
    const prior = autoReminderByRow.get(log.entityId);
    if (!prior || when > prior) autoReminderByRow.set(log.entityId, when);
  }
  ...
  for (const family of families) {
    ...
    // ── The chase trail is the FAMILY's, like the payments ──────────────────
    //
    // Amending an invoice mints a new row with chaseCount 0, and the cron logs
    // against whichever version was live when it fired. Read off the latest
    // row alone, a client chased three times and then sent a corrected figure
    // would show as never chased — the exact gap these fields exist to close.
    // So: newest lastChasedAt across versions, chases summed, and the newest
    // automated reminder logged against any member.
    let lastChasedAt = null;
    let chaseCount = 0;
    let autoReminderAt = null;
    for (const m of family.members) {
      const chased = m?.lastChasedAt ? new Date(m.lastChasedAt) : null;
      if (chased && !Number.isNaN(chased.getTime()) && (!lastChasedAt || chased > lastChasedAt)) {
        lastChasedAt = chased;
      }
      chaseCount += Math.max(0, Math.trunc(num(m?.chaseCount)));
      const auto = autoReminderByRow.get(m?.id);
      if (auto && (!autoReminderAt || auto > autoReminderAt)) autoReminderAt = auto;
    }

    // The job is the family's too: an explicit link is set on the row it was
    // made on, and a later version does not copy it. Newest version that
    // carries one wins, and none means none — never the client's other jobs.
    const jobId =
      [...family.members].reverse().find((m) => m?.jobId)?.jobId ?? null;
