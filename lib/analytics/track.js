...
// view of a session, the UTM tags of the landing, and — on the public
...
export function trackPageView(pathname, host) {
  ...
  track(
    "page_view",
    first ? { p: pattern, r: ctx.referrer, ...ctx.utm } : { p: pattern },
    surface,
  );
  return pattern;
}

/** A step of the signup funnel was shown. */
export function trackSignupStep(step) {
  track("signup_step", { p: step }, "marketing");
}

/** The page is handing off to Stripe. Flushed immediately: the navigation is next. */
export function trackCheckoutStarted() {
  track("checkout_started", { p: "signup" }, "marketing");
  flush();
}
