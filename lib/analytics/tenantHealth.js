...
//     a dashboard can print. Anything under MIN_SAMPLE reports the counts and
//     a null rate — the UI shows "—", not a percentage.
//
...
export function buildFunnel(quotes, jobs, invoices) {
  ...
  return {
    stages: [
      { key: "created", label: "Quotes created", count: created, rate: null },
      { key: "sent", label: "Sent to a client", count: sent, rate: pct(sent, created) },
      { key: "accepted", label: "Accepted", count: accepted, rate: pct(accepted, sent) },
      { key: "job", label: "Became a job", count: jobsCreated, rate: pct(jobsCreated, accepted) },
      { key: "completed", label: "Job completed", count: jobsCompleted, rate: pct(jobsCompleted, jobsCreated) },
      { key: "invoiced", label: "Invoiced", count: invoiced, rate: pct(invoiced, jobsCompleted) },
      { key: "paid", label: "Paid", count: paid, rate: pct(paid, invoiced) },
    ],
