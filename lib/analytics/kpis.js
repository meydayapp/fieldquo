...
export const REASONS = {
  below_floor: "Too few decisions in this period to draw a rate from.",
  none_decided_yet: "Nothing has been decided yet this period.",
  no_quotes_sent: "No quotes were sent in this period.",
  no_won_quotes: "No quotes were won in this period.",
  no_leads_in_period: "No leads came in during this period.",
  no_throughput_reference:
    "There is a backlog, but nothing finished this period to measure a weekly pace against.",
...
/** A rate over (numerator, denominator), floored at RATE_FLOOR, envelope-shaped. */
function rateKpi(numerator, denominator, { noneYetReason, belowFloorReason }) {
  if (denominator <= 0) {
    return kpi({ sampleSize: 0, reason: noneYetReason });
  }
...
function buildSalesFromWinLoss({ from, to, quotes, undatedCount }) {
  ...
  const winRate = report.hasData
    ? report.winRate.value === null
      ? kpi({
          ...
          reason:
            report.winRate.suppressed === "below_floor" ? "below_floor" : "none_decided_yet",
        })
      : kpi({ value: round1(report.winRate.value * 100), sampleSize: report.winRate.n })
    : kpi({ sampleSize: 0, reason: "no_quotes_sent" });
  ...
  const avgJobValue =
    !report.hasData || wonCount === 0
      ? kpi({ sampleSize: wonCount, reason: report.hasData ? "no_won_quotes" : "no_quotes_sent" })
      : wonCount < COUNT_FLOOR
        ? kpi({ sampleSize: wonCount, reason: "below_floor" })
        : kpi({
...
export function buildMarginRollup({ jobs = [], overheadPerJob = null, materialsTrap } = {}) {
  ...
  const grossMarginPct =
    sample < COUNT_FLOOR
      ? kpi({ sampleSize: sample, reason: "below_floor", raw: base })
      : kpi({
...
function buildLabourCostPct(rows, base) {
  ...
  if (sample < COUNT_FLOOR) {
    return kpi({ sampleSize: sample, reason: "below_floor", raw: base });
  }
...
// ═══════════════════════════════════════════════════════════════════════════
//
// Six metrics a KPI dashboard is normally expected to carry, and the reason
...
export const NOT_TRACKED = [
  ...
  {
    key: "reworkCallbackRate",
    label: "Rework / callback rate",
    reason:
      "No field on Job or Invoice marks a visit as a callback or a redo. This would need a new flag on the schedule, set by whoever books the return visit.",
  },
  {
    key: "csat",
    label: "Customer satisfaction",
    reason: "Nothing in the product asks a client to rate the work. There is no survey to summarise.",
  },
  {
    key: "safetyIncidentRate",
    label: "Safety incident rate",
    reason: "There is no incident log anywhere in the schema. This would need its own model, not a report over an existing one.",
  },
  {
    key: "equipmentUtilisation",
    label: "Equipment utilisation",
    reason:
      "Asset only records depreciation (cost, useful life, disposal) — there is no per-use log of when a piece of equipment was actually on a job.",
  },
  {
    key: "changeOrderRate",
    label: "Change-order rate",
    reason:
      "A quote is edited in place before it is sent; nothing distinguishes an original scope from a change agreed mid-job. This would need change orders modelled as their own record, not inferred from quote edits.",
  },
];
