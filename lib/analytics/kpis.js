 ...
 *
 * ── One opinion about what a change order is worth ─────────────────────────
 *
 * `totalPriceDelta` used to sum every row here, with its own loop. That was
 * fine while ChangeOrder had no status and this figure was read by nothing but
 * a test script. It is not fine now: the same money reaches the job page and
 * an invoice through lib/jobs/changeOrderValue.js, and a KPI counting rejected
 * and not-yet-agreed rows would be a second opinion about the same dollars —
 * exactly AGENTS.md failure class #4. So the summing lives there and this
 * calls it.
 *
 * Rows with no `status` at all still count as approved (the fixtures here
 * predate the column, and so does every row written before it); see that
 * file's header for why absent and unrecognised are treated differently.
 *
 * @param {object[]} jobs  completed jobs, each
 *   `{ id, changeOrders: [{ priceDelta: number, status?: string }] }`
 */
export function buildChangeOrderRate({ jobs = [] } = {}) {
  ...
  for (const job of jobs || []) {
    ...
    totalPriceDelta += changeOrderSummary(orders).approvedTotal;
  }
 ...
 *                                        period, across every channel, in
 *                                        the company's currency — rows
 *                                        flagged with another currency (see
 *                                        MarketingSpend.currency) must
 *                                        already have been converted at the
 *                                        pinned rate or excluded by the
 *                                        caller, the way
 *                                        lib/analytics/marketingRollup.js's
 *                                        totals.spend is built; the caller
 *                                        prints "≈" off its `approximate`.
...
export const NOT_TRACKED = [
  {
    key: "costPerLead",
    label: "Cost per lead",
    // Narrowed, not removed. lib/analytics/campaignRollup.js now computes a
    // real per-campaign cost per lead for the ONE path that carries an id on
    // both sides — a Meta-synced spend row and a Meta lead-form lead — and
    // the Spend page shows it. This dashboard still refuses a per-CHANNEL
    // figure, because every other channel's leads carry no attribution and
    // MarketingSpend.leads is still typed in by hand.
    reason:
      "MarketingSpend.leads is typed in by hand, and outside Meta lead forms no lead — LeadRequest — carries a campaign id or a UTM value. A per-channel cost-per-lead built on a hand-typed denominator and no source attribution would look precise and mean nothing. The per-campaign figure that IS honest lives on the Spend page, for Meta lead-form leads only.",
  },
