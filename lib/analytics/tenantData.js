...
// ── Who this board is about ────────────────────────────────────────────────
//
// Demo companies were already excluded, for the reason above. An ABANDONED
// SIGNUP is the same argument one step further: buildAdoption() divides by the
// number of companies, and ten of the thirty-two rows in the database are
// people who closed the Stripe tab and never gave a card. They will never adopt
// instant quotes or invoicing, so leaving them in the denominator quietly
// reported every adoption rate as about two-thirds of its real value, and put
// ten permanently "dormant" accounts at the top of a needs-attention list whose
// whole purpose is naming customers worth phoning about their usage. They are
// worth phoning — about finishing their signup, which is /platform/signups.
//
// Applied to the QUOTE, JOB and INVOICE queries too, not only to the company
// one. One of these ten went on to create a quote; scoping the companies
// without scoping their records would leave that quote in the funnel totals
// with no company row to attribute it to, which is a subtler version of the
// same wrong number.
const REAL_CUSTOMER = { isDemo: false, ...completedSignupWhere() };
...
export async function collectTenantAnalytics(db, { since } = {}) {
  ...
  const [quotes, jobs, invoices, companies, scopeGroups] = await Promise.all([
    db.quote.findMany({
      where: { company: REAL_CUSTOMER, ...(window && { createdAt: window }) },
      select: {
    ...
    db.quoteScopeGroup.findMany({
      where: { quote: { company: REAL_CUSTOMER, ...(window && { createdAt: window }) } },
      select: {
        subtotal: true,
