...
import { completedSignupWhere } from "@/lib/signup/abandoned";
import { funnelFromVisitors } from "./aggregate";
...
export async function signupVisitors({ start, end } = {}) {
  ...
  const rows = await db.$queryRaw`
    SELECT "visitorId",
           ...
           COALESCE(array_agg(DISTINCT path) FILTER (WHERE event = 'signup_step'), '{}') AS steps
    FROM "AnalyticsEvent"
    WHERE "createdAt" >= ${from} AND "createdAt" < ${end}
      AND "visitorId" IS NOT NULL AND "isDemo" = false
      AND ((event = 'page_view' AND path = '/signup') OR event = 'signup_step')
    GROUP BY "visitorId"`;
  const [linked, companiesFinished] = await Promise.all([
    trialVisitors({ start: from, end }),
    signupsCompleted({ start: from, end }),
  ]);
  const trialIds = new Set(linked.map((l) => l.visitorId));
  const seen = new Set();
  const visitors = rows.map((r) => {
    seen.add(r.visitorId);
    return { visited: Number(r.visited) === 1, steps: Array.isArray(r.steps) ? r.steps : [], trial: trialIds.has(r.visitorId) };
  });
  // A browser tied to a finished company whose beacons fell outside the
  // window (or never landed) is still a trial started in the range.
  for (const id of trialIds) if (!seen.has(id)) visitors.push({ visited: false, steps: [], trial: true });
  ...
  return {
    ...
    excluded: f.excluded,
    from,
    // The reconciliation line under the funnel: every company that finished
    // signup in the window (the shared classification), and how many of them
    // the funnel could tie to a browser.
    companiesFinished,
    companiesLinked: new Set(linked.map((l) => l.companyId)).size,
  };
}

/**
 * Browsers tied to a company that FINISHED signing up in [start, end):
 * SignupLead.visitorId (the anonymous id the signup capture already posts,
 * lib/signup/leadCapture.js) → completedCompanyId, for a completion that is
 * the person's own (signupLeadFinished: not the cron's "matches a company
 * already on the books"), demo companies excluded, "finished" by the one
 * rule every console screen uses (completedSignupWhere).
 *
 * No new storage and no new identifier: both columns were written by the
 * signup long before this read them.
 */
export async function trialVisitors({ start, end } = {}) {
  const rows = await db.signupLead.findMany({
    where: {
      visitorId: { not: null },
      completedCompanyId: { not: null },
      OR: [{ skipReason: null }, { skipReason: { not: "company_exists" } }],
      completedCompany: { is: { isDemo: false, createdAt: { gte: start, lt: end }, ...completedSignupWhere() } },
    },
    select: { visitorId: true, completedCompanyId: true },
  });
  return rows.map((r) => ({ visitorId: r.visitorId, companyId: r.completedCompanyId }));
}

/**
 * Companies that FINISHED signing up in [start, end), demo companies
 * excluded — one per company. This is the funnel's last bar on every range,
 * including the ones past the raw window.
 *
 * Since 38d3308d a signup finishes with no card and no Subscription row: the
 * company is created on its card-free trial, and the row only appears if and
 * when a plan is chosen, maybe weeks later. Counting Subscription rows made
 * every new signup invisible to this bar (three on 2026-09-24/25 alone) and
 * would later have counted each one in the month it CHOSE A PLAN.
 *
 * So: companies that have finished signing up by the rule every console
 * screen uses (lib/signup/abandoned.js completedSignupWhere — a
 * Subscription row, or the card-free trial), dated by the company's own
 * creation. For a company that went through the old checkout that is minutes
 * before its Subscription row; for a card-free trial it is the moment the
 * signup finished. One company, one date, whatever it does later.
 */
export async function signupsCompleted({ start, end } = {}) {
  return db.company.count({
    where: { isDemo: false, createdAt: { gte: start, lt: end }, ...completedSignupWhere() },
  });
}
...
/**
 * How many companies could have used a page: every company that finished
 * signing up (completedSignupWhere), demos only when asked for. It counted
 * every non-demo Company row, so an abandoned signup — never in the product —
 * sat in the denominator of every "N of M companies used this" and the
 * number disagreed with the dashboard's "companies" and the tenant board's.
 */
export async function companyDenominator({ includeDemo = false } = {}) {
  const real = { isDemo: false, ...completedSignupWhere() };
  return db.company.count({ where: includeDemo ? { OR: [{ isDemo: true }, real] } : real });
}
