...
import { DEFAULT_MARGIN_TARGET_PCT } from "@/lib/costing/marginTarget";
...
/**
 * The margin the floor assumes when the company has never set one. One
 * definition, shared with the quote's Cost & margin panel — the two used to
 * carry 20 and 30 respectively, against the same Settings box.
 */
export const DEFAULT_TARGET_MARGIN = DEFAULT_MARGIN_TARGET_PCT / 100;

export function normaliseTargetMargin(value, fallback = DEFAULT_TARGET_MARGIN) {
...
/**
 * Which margin the floor is worked out at.
 *
 * Pure, so scripts/check-overhead-arithmetic.mjs can drive it. Three sources
 * in strict order, and only the first that SAYS something wins:
 *
 *   requested  — an explicit value from the caller (the ?targetMargin= query
 *                on /api/analytics/minimum-price), a what-if that must not be
 *                written anywhere;
 *   saved      — ForecastSettings.targetMargin, what the owner set on
 *                Settings → Overhead;
 *   the default — 20%, which until the column existed was the ONLY margin the
 *                floor had ever used, on every company, with no way to change
 *                it. The screen printed "At a 20% target margin" as if it
 *                were the company's figure.
 *
 * `null`/`undefined`/"" mean "nothing said" at every level — 0 is a margin,
 * not an absence (normaliseTargetMargin's own note). A garbage value at one
 * level falls through to the NEXT level rather than to the default, so a
 * malformed query cannot erase a saved setting.
 */
export function resolveTargetMargin(requested, saved) {
  const absent = (v) => v === null || v === undefined || v === "";
  if (!absent(requested)) {
    const n = Number(requested);
    if (Number.isFinite(n) && n >= 0) return normaliseTargetMargin(n);
  }
  if (!absent(saved)) {
...
export async function calculateMinimumPrice({ companyId, targetMargin }) {
  ...
  // The company's own setting applies whenever the caller passed nothing —
  // which is every caller except the what-if query on the analytics route.
  const margin = resolveTargetMargin(targetMargin, forecast?.targetMargin);
  ...
  return priceFromBurn({ burn, capacity, targetMargin: margin });
}

/**
 * The arithmetic half of calculateMinimumPrice, split out so it can run
 * without a database — same reason lib/analytics/burnRate.js's
 * combineBurnRate exists. `margin` is expected already normalised (this is
 * called with one that already went through normaliseTargetMargin above).
 *
 * scripts/check-overhead-arithmetic.mjs feeds this the owner's own reported
 * numbers (2026-08-30) — capacity for ~4.3 jobs/month, a burn built from
 * $0/$736/$1,000/$1,196/$0 — and checks the $677 cost-per-job and $846
 * minimum price it prints are what this function, unmodified, actually
 * returns.
 */
export function priceFromBurn({ burn, capacity, targetMargin }) {
  const margin = normaliseTargetMargin(targetMargin);
  const jobsPerMonth = Number(capacity) * 4.33;
  ...
  // ── Cost, not cash ────────────────────────────────────────────────────────
  //
  // This was `totalMonthlyBurn`, which is what LEAVES THE BANK. The difference
  // is the truck: cash counts the whole loan payment and nothing for wear;
  // cost counts the wear and only the interest on the loan. A price floor
  // built on cash goes wrong twice — it double-charges the truck while the
  // loan runs, and then loses the truck entirely the month the loan ends,
  // which is the failure that silently drops a contractor below break-even.
  //
  // See lib/analytics/burnRate.js for why both numbers exist, and
  // lib/accounting/depreciation.js for the rule that separates them.
  const costPerJob = burn.totalMonthlyCost / jobsPerMonth;
  const minimumPrice = costPerJob / (1 - margin);

  return {
    monthlyFixedCosts: burn.totalMonthlyCost,
    // The cash figure alongside it, because they now differ and a contractor
    // comparing this screen with the runway KPI is owed the reason.
    monthlyCashOut: burn.totalMonthlyBurn,
    // Which loans are charged at interest, and whether an unlinked asset and
    // an unlinked loan are sitting side by side. The screen says so; the
    // server does not guess a pairing.
    doubleCountRisk: burn.doubleCountRisk,
    // Passed through rather than re-derived in the browser. The rule for which
    // loans are interest-only has three clauses (linked, the asset is still
    // charging, the debt is active) and a second copy in client JavaScript is
    // the copy that drifts — the screen would then annotate a row the floor
    // did not discount.
    interestOnlyDebtIds: burn.interestOnlyDebtIds,
...
export async function calculateHourlyFloor({
  ...
  // The cost basis, not the cash — same reason as the per-job floor above.
  const hourlyFloor = (burn.totalMonthlyCost + profit) / hours;

  return {
    monthlyFixedCosts: Math.round(burn.totalMonthlyCost * 100) / 100,
