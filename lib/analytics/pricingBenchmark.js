...
export async function getPricingBenchmark({ companyId }) {
  ...
  const categories = await Promise.all(
    enabledCategories.map(async ({ category }) => {
      ...
      // `_avg` comes back null when every row in the group has a null
      // subtotal — priced scope groups exist, but nobody put a number on
      // them. `|| 0` used to turn that into "$0.00 platform average", which
      // reads as "everyone else charges nothing for this" and, worse, made
      // the comparison column divide by it and print a confident "0%".
      // Absence stays absence; the screen decides how to say so.
      const avg = (v) => (v === null || v === undefined ? null : Math.round(Number(v)));
      return {
        categoryId: category.id,
        label: category.label,
        yourAvgPrice: avg(yourGroups._avg.subtotal),
        platformAvgPrice: avg(platformGroups._avg.subtotal),
