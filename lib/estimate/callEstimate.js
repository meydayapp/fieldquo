...
export const MEASURE_SHAPES = {
  ...
  // Gutters read nothing from the conversation but the address: the run and
  // the downspouts come off the roof model, and storeys are confirmed on
  // site. A measurement the model refuses (a shed, a strip mall) comes back
  // as needs_site_visit and the call books one instead of pricing.
  gutter_address: {
    required: [],
    reads: [],
    from: {},
  ...
  // Lawn care is sized from the address alone (parcel − roof − driveway, or
  // the minimum band where no parcel source exists — lib/measure/
  // lawnEstimate.js), so a call CAN price it: with no program named, the
  // estimator prices the cheapest one, which is the honest "starting at".
  lawn_address: {
    required: [],
    reads: [],
    from: {},
  ...
  // The generic trace (paving) is the same drawing under another name.
  area_polygon: { blocked: ESTIMATE_BLOCKED.NEEDS_MAP },
...
export async function draftEstimateFromForm({
  ...
  const quote = await createEstimateDraft({
    ...
    contact,
    // As priced — painting's scope is settled by priceOneMaterial against the
    // company's services, and the draft records the scope the figure used.
    measurement: priced.measurement || measured.measurement,
    materialKey: form.materialKey,
    ...
    clientId,
    sourceCallId,
    // The same distinction `source: "phone_call"` above draws, on the column
    // that reports it: a person rang, and the assistant priced them. The two
    // are not redundant — `source` says how the NUMBER was derived and would
    // read "manual" on most trades, and createdVia says which of this
    // function's two callers ran. See lib/quotes/createdVia.js.
    createdVia: "voice_call",
  });
