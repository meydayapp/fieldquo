...
export function budgetBands(thresholds, { currency = "CAD", language = "en" } = {}) {
  ...
  // Three languages, like every other sentence on the instant-quote page. The
  // symbol stays in front whatever the locale — "$3 500" is how a Québec
  // price is written and "3 500 $" is not what the owner's bands read as.
  const code = String(language || "en").slice(0, 2);
  const locale = code === "fr" ? "fr-CA" : code === "es" ? "es" : "en-CA";
  const under = code === "fr" ? "Moins de" : code === "es" ? "Menos de" : "Under";
  const n = (v) => `${s}${v.toLocaleString(locale)}`;
...
/**
 * The lead-scorer key for a band, chosen by POSITION rather than by dollars.
 *
 * ══ Why position, and not the amount ═══════════════════════════════════════
 *
 * The scorer's keys (lib/leads/score.js) are absolute: 15k_plus is worth 30
 * points, under_1k is worth 6. These bands are not absolute — the owner sets
 * the thresholds per trade, so they describe THIS company's range and nothing
 * wider. A cabinet shop whose top band is "$10,000+" has said that ten
 * thousand dollars is the top of its world; a homeowner who picks it has told
 * that contractor they can afford the biggest job the contractor does.
 *
 * Reading the label's dollars instead sent that lead through as 5k_15k and
 * docked it eight points for the crime of hiring a cabinet refinisher rather
 * than a roofer. The signal in the answer is "top of the range offered", and
 * the position is what carries it. A plumber whose bands top out at "$1,500+"
 * gets the same treatment, correctly: that is a big job for a plumber.
 *
 * Falls back to the generic thresholds' own positions, so a company that never
 * touched the setting scores exactly as it did before this existed.
 *
 * @param index 0-based band index from bandForIndex/budgetBands
 * @returns a key from lib/leads/score.js BUDGET_POINTS, or null when unanswered
 */
export function scoreKeyForBandIndex(index) {
  const KEYS = ["under_1k", "1k_5k", "5k_15k", "15k_plus"];
  if (!Number.isInteger(index) || index < 0 || index >= KEYS.length) return null;
  return KEYS[index];
}
