...
const INSTANT_ONLY_FIELDS = {
  ...
  // Gutters are an instant trade with no material rows: one profile (seamless
  // aluminium), priced by the foot and the downspout with a LOW and a HIGH
  // rate each, because the range on the public form is two real ends (one
  // storey / two storeys) rather than a band around a point. The gutter
  // price book (gutter_services) carries the builder's own per-foot rates by
  // profile and work type; these are the public instant estimator's, kept
  // apart for the same reason refacing's per-box rate is — the two screens
  // price different things and a shared field would be wrong on one of them.
  // Stairs: the tread rate is a material row (per complexity) and the
  // railing has its own box on the settings screen; these three price the
  // counts the step count and shape derive (lib/estimate/stairsFromSteps.js).
  // The stairs price book declares the same three, but under
  // `complexity.<tier>.…` — the instant config carries one rate each,
  // seeded from the book's standard tier, so they are declared here.
  stair: [
    { path: "riserPrice", label: "Riser", suffix: "$ / riser", step: 5 },
    { path: "balusterPrice", label: "Baluster", suffix: "$ / each", step: 5 },
    { path: "postPrice", label: "Newel post", suffix: "$ / each", step: 5 },
  ],
  gutters: [
    { path: "perFt.low", label: "Per linear ft — low", suffix: "$ / linear ft", step: 0.5 },
    { path: "perFt.high", label: "Per linear ft — high", suffix: "$ / linear ft", step: 0.5 },
    { path: "perDownspout.low", label: "Per downspout — low (one storey)", suffix: "$ / downspout", step: 10 },
    { path: "perDownspout.high", label: "Per downspout — high (two storeys)", suffix: "$ / downspout", step: 10 },
    { path: "minCharge.low", label: "Minimum job — low", suffix: "$", step: 50 },
    { path: "minCharge.high", label: "Minimum job — high", suffix: "$", step: 50 },
  ],
};
