...
//
// Trades whose builder has no structured takeoff (lawn mowing, junk removal,
// epoxy, parging, countertop's supplier-invoice model) get their measurement
...
export function costingInputsForInstantTrade(trade, materialKey, measurement, { categoryKey = null } = {}) {
  ...
  if (trade === "epoxy" || trade === "parging" || trade === "lawn_mowing") {
    const sqft = pos(m.areaSqft);
    if (!sqft) return { takeoff: null, intakeValues: null };
    return {
      takeoff: null,
