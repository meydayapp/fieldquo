...
export function costingInputsForInstantTrade(trade, materialKey, measurement, { categoryKey = null } = {}) {
  ...
  if (trade === "stair") {
    ...
    // The whole staircase from the step count and shape — the same rule the
    // public range was priced by (lib/estimate/stairsFromSteps.js), so the
    // takeoff the estimator opens carries the counts the homeowner was
    // quoted on, each switched ON so the builder prices them the way the
    // range did. The note says where every number came from; the estimator
    // adjusts after the photos and the figures are theirs from then on.
    const derived = stairsFromSteps({ steps: treads, shape: m.shape, railingFt: pos(m.railingFt) });
    if (!derived) return { takeoff: null, intakeValues: null };
    return {
      takeoff: {
        sections: [
          drop({
            ...
            paintBalusters: true,
            paintPosts: true,
            notes: `Risers, balusters, posts${derived.railingGiven ? "" : " and handrail"} ${stairsDerivationNote(derived)} — adjust after the photos.`,
          }),
        ],
      },
      intakeValues: drop({
        treads: derived.treads,
        railingFt: pos(m.railingFt),
        shape: derived.shape,
      }),
    };
  }
