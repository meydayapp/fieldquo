...
import { applyOfferedScope, paintingScopesOffered } from "@/lib/estimate/instantSeed";
...
async function offeredPaintingScopes(companyId) {
  return paintingScopesOffered(await companyEnabledCategoryKeys(companyId));
}
...
export async function loadCompanyInstantTrades(companySlug) {
  ...
  const paintingScopes = configs.some((r) => r.trade === "painting")
    ? await offeredPaintingScopes(company.id)
    : [];
 ...
 */
export async function priceAllMaterials({ companyId, trade, measurement: requested }) {
  const config = await loadEnabledConfig(companyId, trade);
  if (!config) return { ok: false, reason: "not_configured" };

  // Painting's scope is the company's services' call, not the browser's.
 ...
 */
export async function priceOneMaterial({ companyId, trade, materialKey, measurement: requested }) {
  ...
  // Same settlement as priceAllMaterials: this is the authoritative figure the
  // draft is written from, so it must not price a scope the company doesn't
  // sell just because a request body named one.
  const scoped = await withOfferedScope(companyId, trade, requested);
  if (!scoped.ok) return { ok: false, reason: scoped.reason };
  const measurement = scoped.measurement;
