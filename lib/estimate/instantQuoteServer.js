...
export async function priceAllMaterials({ companyId, trade, measurement }) {
  ...
  if (!spec.hasMaterials) {
    ...
    return { ok: true, visibility, options: [{ materialKey: null, label: null, low: est.low, high: est.high, point: est.point, unit: est.unit || null }] };
  }
  ...
  return { ok: true, visibility, options };
}
