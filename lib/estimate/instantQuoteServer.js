...
import { instantTradeLabel, instantTradeBlurb, junkItemLabel, junkJobTypeLabel } from "@/lib/i18n/instantQuoteCopy";
import { offeredInstantLanguages, resolveInstantLanguage } from "@/lib/estimate/instantQuoteLanguages";
...
export async function loadCompanyInstantTrades(companySlug, { language: requested = null } = {}) {
  const company = await db.company.findUnique({
    ...
    select: {
      ...
      // Whether the company has drawn a service area — the one fact that
      // locks the job address to "required" on every trade (lib/estimate/
      // formFields.js). The coordinates and the radius never leave the
      // server; only the verdict does, and only as a field state.
      latitude: true,
      longitude: true,
      serviceRadiusKm: true,
      servicePostalPrefixes: true,
      // The languages the company offers on this form (Settings › Instant
      // quotes). Empty = never chosen = all three; see
      // lib/estimate/instantQuoteLanguages.js.
      instantQuoteLanguages: true,
    },
  });
  if (!company) return null;
  const areaConfigured = serviceAreaConfigured(company);
  // The visitor's pick when the company offers it, else the company's own
  // language when offered, else the first offered — never a language the
  // company switched off, whatever ?lang= said.
  const lang = resolveInstantLanguage(company, requested);
  ...
  // The option names in `lang`, for every trade on the page, in one read
  // (see materialPhraseEntries below). The same effective config the map
  // below lists from, so the names looked up are the names printed.
  const trMaterial = await loadPhrases(
    db,
    company.id,
    lang,
    configs.flatMap((row) => materialPhraseEntries(effectiveInstantConfig(row.trade, row.config, enabledRows))),
  );
  ...
  const trades = configs
    .map((row) => {
      ...
      const materials = spec.hasMaterials
        ? (localiseMaterialLabels(sanitiseInstantConfig(config), trMaterial)?.materials || []).map((m) => ({ key: m.key, label: m.label }))
        : [];
      ...
      // Every label below is in `lang` (resolved above, once): the VISITOR's
      // language when the page asked for one the company offers, else the
      // company's. Switching the pills re-fetches this payload rather than
      // leaving "Sofa / couch" in the middle of a French junk-removal picker.
      ...
      return {
        ...
        // One line saying what the service IS, drawn under the card when the
        // company offers more than one. FieldQuo's own words from a closed
        // table — never the company's free text, which can carry a rate
        // (see instantTradeBlurb). Null draws nothing.
        description: instantTradeBlurb(row.trade, lang),
        measure: spec.measure,
        hasMaterials: spec.hasMaterials,
        materials,
        // Which qualifier questions this trade asks and which are required —
        // photos, budget, when, notes, phone, email, the job address — as
        // the owner set them, with the two locks applied (an address the
        // estimator measures from, or a service area to check, is required
        // whatever was saved). States only, never a rate; the request route
        // applies the same table, so the browser is a courtesy and the
        // server is the gate.
        fields: effectiveFormFields(config?.fields, { measure: spec.measure, serviceAreaConfigured: areaConfigured }).fields,
  ...
  return {
    ...
    language: lang,
    // The pills the form draws, in order — the company's choice, or all
    // three when it never made one.
    languages: offeredInstantLanguages(company),
    trades,
...
// ── Option names in the reader's language ───────────────────────────────────
//
// A trade's material/option names are the company's own words
// ("Architectural shingles", or whatever it renamed them to), drafted into
// the other languages when the trade is saved (PUT /api/settings/instant-
// quote, namespace materialLabel — lib/i18n/phrases.js). They are swapped in
// on the CONFIG, before anything lists or prices it, so the picker, the
// /measure option cards, the report's cards and the estimator's breakdown
// lines ("22 squares of …", which become the draft quote's lines — a
// document then fixed in that language) all read one translation from one
// place, without teaching fourteen estimators about phrases. Keys are left
// alone: pricing and the posted materialKey never look at a label. No draft
// yet → the name as the company wrote it.

/** The { ns, text } entries loadPhrases needs for one config's option names. */
export function materialPhraseEntries(config) {
  const materials = config && typeof config === "object" && Array.isArray(config.materials) ? config.materials : [];
  return materials
    .filter((m) => m && typeof m === "object" && typeof m.label === "string" && m.label.trim())
    .map((m) => ({ ns: "materialLabel", text: m.label }));
}

/** Pure: the config with each option name run through `tr` (loadPhrases' lookup). */
export function localiseMaterialLabels(config, tr) {
  if (!config || typeof config !== "object" || !Array.isArray(config.materials) || typeof tr !== "function") return config;
  return {
    ...config,
    materials: config.materials.map((m) =>
      m && typeof m === "object" && typeof m.label === "string" && m.label.trim()
        ? { ...m, label: tr("materialLabel", m.label) }
        : m,
    ),
  };
}
...
/**
 * The config with its option names in `language`. Only when a caller NAMED
 * the language: the phone and AI-employee paths price without one and have
 * always printed the names as written, and defaulting them to English drafts
 * would change a French company's call-drafted quote without anyone asking.
 */
async function withMaterialPhrases(config, companyId, language) {
  if (!config || !language) return config;
  const entries = materialPhraseEntries(config);
  if (!entries.length) return config;
  return localiseMaterialLabels(config, await loadPhrases(db, companyId, language, entries));
}

/**
 * The owner's saved `fields` block for one trade, raw (null when the trade
 * has no row or the row carries none). The request route runs it through
 * effectiveFormFields with the company's own area verdict, exactly as the
 * public payload above does, before it decides what a POST must carry.
 * Enabled-ness is not judged here: a switched-off trade is refused a few
 * lines later by the pricer, with the sentence that already exists for it.
 */
export async function loadInstantFormFields(companyId, trade) {
  if (!companyId || !trade) return null;
 ...
 */
export async function priceAllMaterials({ companyId, trade, measurement: requested, language: namedLanguage = null }) {
  const language = namedLanguage || "en";
  ...
  const config = await withMaterialPhrases(await loadEnabledConfig(companyId, trade), companyId, namedLanguage);
 ...
 */
export async function priceOneMaterial({ companyId, trade, materialKey, measurement: requested, language: namedLanguage = null }) {
  const language = namedLanguage || "en";
  ...
  const config = await withMaterialPhrases(sanitiseInstantConfig(await loadEnabledConfig(companyId, trade)), companyId, namedLanguage);
  if (!config) return { ok: false, reason: "not_configured" };
