...
import { estimateRange } from "./estimateMoney";
...
const COPY = {
  en: {
    ...
    bookNote: "Prefer to get it settled? Pick a time that suits you and we'll come and confirm the price on site.",
  },
  fr: {
    ...
    bookNote: "Vous préférez fixer les choses ? Choisissez un moment qui vous convient et nous viendrons confirmer le prix sur place.",
  },
  es: {
    subject: (c) => `Su estimación de ${c}`,
    greeting: (n) => (n ? `Hola ${n},` : "Hola,"),
    thanksRange: (c) => `Gracias por los detalles. Este es su rango estimado de ${c}:`,
    thanksGated: (c) => `Gracias por los detalles — ${c} está preparando su presupuesto.`,
    rangeLabel: "Rango estimado",
    beforeTax: "antes de impuestos",
    disclaimer: (c) =>
      `Esta es una estimación basada en lo que nos indicó. ${c} la revisará y confirmará el precio exacto antes de que nada sea vinculante — sin compromiso.`,
    reference: "Su referencia",
    next: "Nos pondremos en contacto en breve.",
    bookCta: "Reservar una visita",
    bookNote: "¿Prefiere dejarlo resuelto? Elija un momento que le convenga y vendremos a confirmar el precio en el sitio.",
  },
};
...
// The figure in the COMPANY's currency, grouped for the reader's language —
// the same formatter the public form uses (lib/estimate/estimateMoney.js).
// This used to be `"$" + n.toLocaleString("en-CA")` under a hard-coded "CAD"
// caption, which mailed a Michigan contractor's homeowner a Canadian-dollar
// estimate. The currency is the company's; only the digit grouping follows
// the language.
const rangeText = (low, high, currency, language) =>
  estimateRange(low, high, currency || "CAD", language === "fr" ? "fr-CA" : language === "es" ? "es" : "en-CA");
...
export function buildEstimateEmail({
  ...
  budgetGap = false,
  bookingUrl = null,
  // Sentences the estimator wrote for the homeowner, already in `language`
  // — gutters: "measured from aerial imagery of your roofline · imagery date
  // …" and "an estimate from aerial measurements, not a contract". Rendered
  // under the range, only when there is a range to sit under.
  notes = [],
}) {
  ...
  const rangeBlock = pub.show
    ...
    <div style="background:${accent}14;border:1px solid ${accent};border-radius:10px;padding:20px;text-align:center;margin:22px 0;">
      ...
      <div style="font-size:30px;font-weight:800;color:${accent};margin-top:4px;">${escapeHtml(rangeText(pub.low, pub.high, company.currency, language))}</div>
      <div style="font-size:12px;color:#6b7280;margin-top:4px;">${escapeHtml(company.currency || "CAD")} · ${escapeHtml(t.beforeTax)}</div>
    </div>${
      Array.isArray(notes) && notes.length
        ? notes
            .filter((n) => typeof n === "string" && n.trim())
            .map((n) => `<p style="margin:0 0 6px;font-size:13px;line-height:1.6;color:#6b7280;">${escapeHtml(n)}</p>`)
            .join("")
        : ""
    }`
    : "";

  // ── Where the financing block sits, and why that's the only thing that moves
  //
  // When the homeowner told us a budget and the estimate came in above its
  // ceiling, the one useful thing this email can say is "there's a way to
  // spread this" — and saying it below the disclaimer, four paragraphs down, is
  // saying it to someone who has already closed the tab. So it moves up, under
  // the number that caused the problem.
  //
  // POSITION IS ALL THAT CHANGES. No new sentence, no "we noticed this is over
  // your budget" (which tells a stranger we've been sizing them up), and above
  // all nothing when the company hasn't enabled financing — `financing` is null
  // then and this whole branch is dead. Inventing an offer because the budget
  // looked short would put a promise in the contractor's mouth that they never
  // made, which is the one thing lib/estimate/financing.js exists to prevent.
  const leadWithFinancing = Boolean(financing) && budgetGap && pub.show;
  ...
  // ── The one thing this email can DO ───────────────────────────────────────
  //
  // It is an acknowledgement, deliberately — nobody has priced this yet, so it
  // carries no approve button, no signature line and no PDF. But an
  // acknowledgement that ends at "we'll be in touch" leaves the next move to a
  // phone call that, often, neither side makes.
  //
  // Rendered ONLY when the caller passes a URL, and the caller only has one
  // when the company can actually take a booking (canBookVisit — an active
  // event type, and the mode still offered). No booking, no button: an email
  // is the worst possible place for a dead link, because it outlives the page
  // it was sent from and there is nobody there to explain it.
  //
  // Whatever modes the company offers — a home visit, a phone consultation, a
  // video call — the booking page presents them. So "book a call back" and
  // "book a visit" are the same door, and this does not need to guess which.
  const bookingBlock = bookingUrl
    ? `
    <div style="text-align:center;margin:22px 0 0;">
      <a href="${safeUrl(bookingUrl)}" style="display:inline-block;background:${accent};color:${accentInk};text-decoration:none;font-weight:700;padding:12px 22px;border-radius:8px;font-size:15px;">${escapeHtml(t.bookCta)}</a>
      <p style="font-size:12px;line-height:1.6;color:#6b7280;margin:10px 0 0;">${escapeHtml(t.bookNote)}</p>
    </div>`
    : "";
...
<div style="margin:0;padding:24px 12px;background:#f6f7f9;font-family:${theme.font || "system-ui,-apple-system,Segoe UI,sans-serif"};">
  <div style="max-width:520px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #eceff3;">
    ...
    <div style="padding:28px;">
      ...
      ${rangeBlock}
      ${leadWithFinancing ? financingBlock : ""}
