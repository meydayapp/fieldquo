...
export async function createEstimateDraft({
  ...
  const quote = await db.quote.create({
    data: {
      ...
      // PDF and the invoice raised later all explain the same figure. A US
      // draft carries the full rate for the address, as a Canadian one
      // carries its province's; the reviewer sees the state's rule as a
      // hint and switches tax off if it applies.
      taxResolution:
