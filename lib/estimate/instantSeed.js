...
// ── The paving mapping ──────────────────────────────────────────────────────
//
// The paving book (app/data/tradePriceBooks.js `paving`) prices three
// SURFACES — patio, walkway, driveway — each with an installed $/sqft per
// complexity tier, and that is the whole scope: excavation, compacted base,
// bedding, edge restraint and polymeric sand are inside the headline number,
// the book says so, and the quote builder's buildPaving prices exactly these
// three rates. So the instant "materials" a homeowner picks from are the
// surfaces, at the book's STANDARD tier — the cleanest job, the same reading
// painting makes of its book — and the driveway carries the book's driveway
// paver upcharge exactly as buildPaving adds it, so the two screens quote
// the same driveway.
//
// What the book cannot state is left out rather than approximated:
//
//   · the paver GRADE is a cost-side allowance (paverOptions vs
//     paverAllowancePerSqft — a stone at or under the allowance adds nothing,
//     and the standard interlock the allowance was set for is under it), and
//     the public form does not ask it, so a per-grade row would be a choice
//     that changes no number;
//   · the moderate / high tiers describe things a homeowner cannot judge
//     sight-unseen (base depth, clay, tight access), so they are the
//     reviewer's call on the draft, not a surcharge box on a stranger's form;
//   · a minimum charge: "not published by ANY of the thirty-odd sources", so
//     `minimumTotal` is 0 in the book and is NOT derived — deriving a zero
//     would nag a company that set a real floor to remove it. The floor is
//     the instant-only knob it is for painting (INSTANT_ESTIMATE_DEFAULTS).
//
// `assumesMinSqft` IS derived: the book states the published rates assume at
// least 500 sq ft, and the estimator says so on any smaller trace rather than
// pretending a 120 sq ft walkway costs 120 × the rate.
...
const PAVING_FIELDS = PAVING_SURFACES.map(([key, label]) => ({
  path: `materials.${key}.ratePerSqft`,
  label: `${label}, installed, per sqft`,
  kind: "money",
}));
...
const DERIVATIONS = {
  paving: {
    categories: ["paving"],
    derive({ books }) {
      const book = books?.paving;
      if (!book) return null;
      const upcharge = num(readField(book, "extras.drivewayPaverUpchargePerSqft"));
      const materials = [];
      for (const [key, label, rateKey] of PAVING_SURFACES) {
        let rate = num(readField(book, `complexity.standard.${rateKey}`));
        if (!(rate > 0)) continue; // a rate the company zeroed is "not sold", not free
        if (key === "driveway") rate += Math.max(0, upcharge);
        materials.push({ key, label, ratePerSqft: Math.round(rate * 100) / 100 });
      }
      if (!materials.length) return null;
      const assumesMinSqft = num(readField(book, "assumesMinSqft"));
      return {
        materials,
        ...(assumesMinSqft > 0 && { assumesMinSqft }),
      };
    },
    fields() {
      return PAVING_FIELDS;
    },
  },
  cabinet_refinishing: {
