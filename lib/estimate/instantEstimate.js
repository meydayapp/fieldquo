...
// zeroes a rate. defaultDerivedSeed reads the price books for SEED VALUES only
// (see the cabinet_refinishing and painting blocks below); nothing here prices
// off a book at runtime.
...
// Lawn-care programs are their own price model (a bundle of treatments, each
// priced by lawn-size band) and live in their own pure module; the seed is
// the competitor-observed card the owner collected. Re-exported through
// INSTANT_ESTIMATE_DEFAULTS / INSTANT_ESTIMATE_TRADES like every other trade.
import { estimateLawnCare } from "@/lib/estimate/lawnCare";
import { stairsFromSteps, stairsDerivationNote } from "@/lib/estimate/stairsFromSteps";
import { LAWN_CARE_SEED } from "@/lib/estimate/lawnCareSeed";
...
export const INSTANT_ESTIMATE_DEFAULTS = {
  ...
  //
  // The derivation itself lives in lib/estimate/instantSeed.js, because the
  // settings route runs the same one over the COMPANY's book (their per-door
  // rate under Services & Pricing, not ours) and reports when the saved row
  // has drifted from it. This is that derivation over the code book.
  cabinet_refinishing: {
    ...defaultDerivedSeed("cabinet_refinishing"),
    rangeBandPct: 0.15,
  },
  cabinet_refacing: {
    perDoor: 95,
  ...
  // $/sqft of surface, a scope surcharge (exterior carries height, weather and
  // heavier prep) and a condition surcharge (patching, priming, stains).
  //
  // Read out of the painting price books the same way refinishing is — the
  // base rate is the interior book's standard-tier wall rate, exterior is the
  // exterior book's siding rate over it, and the condition tiers are the
  // book's own moderate/high tiers over standard. lib/estimate/instantSeed.js
  // explains the mapping and what the book cannot state: it has no paint
  // GRADE, so the seed carries one ("Standard") and a company that sells
  // grades adds its own rows. The instant-only knobs below are literal because
  // no book states them: the range band is the estimator's own uncertainty,
  // and neither painting book carries a job minimum.
  painting: {
    ...defaultDerivedSeed("painting"),
    rangeBandPct: 0.15,
    minCharge: 400,
  },
  // Interlock paving, priced by the square foot of ground the homeowner
  // TRACES on the satellite map (measure `area_polygon` — the same trace lawn
  // mowing takes, lib/estimate/tracedArea.js). The materials are the three
  // surfaces the paving book prices — patio, walkway, driveway — at its
  // standard tier, read out of the book by lib/estimate/instantSeed.js so
  // the instant form and the quote builder quote the same driveway.
  //
  // `minCharge` is 0, and deliberately not a literal like painting's 400: the
  // book records that no published source states a paving minimum, and the
  // owner's rule is that absence of a statement is not a statement. The
  // company types theirs on the settings screen; until they do, a tiny trace
  // prices at the rate and the estimate says the rates assume 500 sq ft.
  paving: {
    ...defaultDerivedSeed("paving"),
    rangeBandPct: 0.15,
    minCharge: 0,
  },
  stair: {
    ...
    railingPerFt: 60,
    // The rest of the staircase, priced from the counts the step count and
    // shape derive (lib/estimate/stairsFromSteps.js). Seeded from the stairs
    // price book's standard tier (app/data/tradePriceBooks.js) so the instant
    // range and the builder's takeoff start from the same numbers; a company
    // that zeroes one drops that line from the range.
    riserPrice: 25,
    balusterPrice: 25,
    postPrice: 150,
    rangeBandPct: 0.15,
  ...
  // ── Gutters: seamless aluminium eavestrough, measured from the roof model ──
  //
  // Priced per linear foot PLUS per downspout, each with a LOW and a HIGH
  // rate, and a minimum job charge on each end. The range is explicit rather
  // than a band around a point, because the two ends mean different things:
  // the low end is a single-storey run with short downspouts, the high end
  // is what a two-storey hang with long drops comes to — and no roof model
  // says how many storeys a house has (storeys are confirmed on site; the
  // copy says so).
  //
  // Seeded from a competitor's published instant estimates the owner
  // collected in Ottawa/Gatineau, 2026-09 (CAD):
  //
  //   184 ft / 6 downspouts    4,161 – 5,629
  //   144 ft / 5 downspouts    3,909 – 5,289
  //    79 ft / 18 downspouts   8,173 – 13,622   (a mis-measured strip mall,
  //                                              but it pins the downspout rate)
  //     7 ft / 4 downspouts    1,000 – 1,250    (their minimum; the run is
  //                                              nonsense and is refused here)
  //
  // Fitting per-foot + per-downspout + a fixed base to the three real points
  // EXACTLY gives a NEGATIVE per-foot rate in both bands (−1.69 low, −6.69
  // high), so a base is not what they charge. Least squares on per-foot and
  // per-downspout alone fits all three within ±9% (low: 10.71/ft + 407/ds;
  // high: 8.92/ft + 718/ds) and hits the 79/18 point to 0.1%. Rounded to a
  // card an owner can read: the per-foot rate is a narrow band (profile,
  // gauge), the per-downspout rate is the wide one (storeys). Through the
  // seed, 184/6 prices 4,300 – 6,220 and 144/5 3,490 – 5,080.
  //
  // Currency-neutral: these are numbers in whatever the company bills in,
  // and the settings screen labels them in the company's currency. Never
  // published as-is (non-negotiable #4) — the company adopts or edits them.
  gutters: {
    perFt: { low: 10, high: 11 },
    perDownspout: { low: 410, high: 700 },
    minCharge: { low: 1000, high: 1250 },
  },
  // ── Lawn care: programs and add-ons by lawn-size band ────────────────────
  //
  // Not a range: a program has a price, and the competitor prints it to the
  // cent. See lib/estimate/lawnCare.js for the model and lawnCareSeed.js for
  // where every figure came from (and which one — the per-1,000 sq ft step
  // — is a seed estimate rather than an observation).
  lawn_care: {
    ...LAWN_CARE_SEED,
    rangeBandPct: 0,
  },
...
export function estimateStair(measurements, materialKey, config) {
  ...
  // ── The whole staircase, from the step count and the shape ──────────────
  //
  // This priced treads and whatever railing feet the homeowner typed, and
  // nothing else — no risers, no balusters, no newel posts — so a stair
  // refinish was quoted at roughly two-thirds of itself. The counts come from
  // the owner's rule (lib/estimate/stairsFromSteps.js): two balusters a step,
  // four posts on an L, five on a U, the handrail from the run plus a landing
  // per turn unless the homeowner measured it. Each line prices off the
  // company's own instant rates — riserPrice / balusterPrice / postPrice are
  // seeded from the stairs price book's standard tier and edited on the same
  // settings screen as the tread rate — and a rate the company zeroed drops
  // the line rather than charging nothing for it silently.
  const derived = stairsFromSteps({
    steps: treads,
    shape: measurements?.shape,
    railingFt: num(measurements?.railingFt),
  });
  if (!derived) return { ok: false, reason: "no_measurement" };

  const base = derived.treads * num(tier.ratePerTread);
  const breakdown = [{ label: `${derived.treads} treads — ${tier.label}`, amount: round(base) }];
  let point = base;

  const extras = [
    ["risers", derived.risers, num(config?.riserPrice), (n) => `${n} risers`],
    ["balusters", derived.balusters, num(config?.balusterPrice), (n) => `${n} balusters`],
    ["posts", derived.posts, num(config?.postPrice), (n) => `${n} newel ${n === 1 ? "post" : "posts"}`],
    ["handrail", derived.handrailFt, num(config?.railingPerFt), (n) => `Handrail (${n} ft)`],
  ];
  for (const [, quantity, rate, label] of extras) {
    const cost = quantity * rate;
    if (!(quantity > 0) || !(cost > 0)) continue;
    breakdown.push({ label: label(quantity), amount: round(cost) });
    point += cost;
  }

  return {
    ok: true,
    ...toRange(point, config),
    breakdown,
    // Carried onto the draft and the report so the estimator sees what was
    // assumed before adjusting it against the photos.
    derived: {
      shape: derived.shape,
      steps: derived.steps,
      ...
      railingGiven: derived.railingGiven,
    },
    assumptions: [
      `${derived.risers} risers, ${derived.balusters} balusters, ${derived.posts} newel posts` +
        (derived.railingGiven ? "" : ` and ~${derived.handrailFt} ft of handrail`) +
        ` — ${stairsDerivationNote(derived)}; confirmed from the photos`,
    ],
  };
}
...
// ── Paving (traced area × installed rate per surface) ────────────────────────
//
// The simplest area trade in the file: the traced square footage times the
// installed rate of the surface the homeowner picked. No surcharge map, on
// purpose — see the paving mapping in instantSeed.js for why the book's
// moderate/high tiers and paver grades stay the reviewer's call. The range
// band carries the sight-unseen uncertainty, as it does for every trade.
//
// One thing the book DOES state is said back: its rates assume a job of at
// least `assumesMinSqft` (500 sq ft in the reference book), and every source
// says the rate goes up below that. A smaller trace is still priced — the
// company's floor, if they set one, applies through toRange like everywhere
// else — but the assumption names it, so the homeowner and the reviewer both
// know this figure is the one that gets confirmed on site.
export function estimatePaving(measurements, materialKey, config) {
  const est = estimateAreaTrade({ measurements, materialKey, config });
  if (!est.ok) return est;
  const areaSqft = num(measurements?.areaSqft);
  const assumesMinSqft = num(config?.assumesMinSqft);
  return {
    ...est,
    assumptions: [
      `~${areaSqft} sqft traced on the map`,
      assumesMinSqft > 0 && areaSqft < assumesMinSqft
        ? `Rates assume at least ${assumesMinSqft} sqft; a smaller job is confirmed on site`
        : null,
    ].filter(Boolean),
  };
}
...
export function estimateCabinetRefinishing(measurements, config) {
  ...
  // ── Each entry is the homeowner's LABEL and the estimator's LINE ────────
  //
  // `label`/`amount` is what the public report prints. `line` is the row the
  // draft stores on its scope group, in the shape the quote builder writes
  // for this trade (lib/pricing/tradeScope.js buildCabinets: "<service> —
  // doors", 25 × $190/door, and the reasons meta the quote page explains a
  // price with). Before this the draft stored "25 doors refinished ×1 @
  // $4,750" — a one-liner an estimator could neither recount nor defend,
  // and the owner's "the instant quote doesn't have the same look and feel"
  // — while a hand-built quote of the same kitchen carried the count, the
  // per-door rate and the complexity level on every line.
  const meta = (base) => ({
    baseUnitPrice: base,
    complexityLevel: level,
    ...(uplift > 0 ? { complexityUpcharge: uplift } : {}),
    // Nobody ticked a reason on a public form; an empty list is the honest
    // record and the quote page says "No reasons were recorded".
    complexityReasons: [],
  });
  const breakdown = [];
  if (doors > 0)
    breakdown.push({
      ...
      line: { part: "doors", quantity: doors, unit: "door", rate: doorRate, meta: meta(num(config?.perDoor)) },
    });
  if (drawers > 0)
    breakdown.push({
      label: `${drawers} ${drawers === 1 ? "drawer front" : "drawer fronts"}`,
      amount: round(drawerCost),
      line: { part: "drawer fronts", quantity: drawers, unit: "drawer", rate: drawerRate, meta: meta(num(config?.perDrawer)) },
    });
  for (const l of addOnLines)
    breakdown.push({
      label: l.description,
      amount: round(l.amount),
      line: { description: l.description, quantity: l.quantity, unit: l.unit, rate: l.rate, ...(l.detail ? { detail: l.detail } : {}) },
    });
  if (range.minimumApplied) {
    ...
    const topUp = round(num(config?.minCharge) - point);
    breakdown.push({
      label: "Job minimum adjustment",
      amount: topUp,
      line: { description: "Job minimum adjustment", quantity: 1, unit: "flat", rate: topUp },
    });
  }
...
export function estimateCabinetRefacing(intake, materialKey, config) {
  ...
  // Same two shapes as refinishing above: the homeowner's label, and the
  // builder-shaped line the draft stores (count × per-unit rate, with the
  // reasons meta). Refacing's book has one standard tier and the public
  // form asks no complexity question, so the meta says "standard".
  const unitMeta = (base) => ({ baseUnitPrice: round(base * mult), complexityLevel: "standard", complexityReasons: [] });
  const breakdown = [
    {
      label: `${doors} ${doors === 1 ? "door" : "doors"} refaced`,
      amount: round(doorCost),
      line: { part: "doors", quantity: doors, unit: "door", rate: round(num(config?.perDoor) * mult), meta: unitMeta(num(config?.perDoor)) },
    },
    {
      label: `${drawers} ${drawers === 1 ? "drawer front" : "drawer fronts"}`,
      amount: round(drawerCost),
      line: { part: "drawer fronts", quantity: drawers, unit: "drawer", rate: round(num(config?.perDrawer) * mult), meta: unitMeta(num(config?.perDrawer)) },
    },
  ];
  if (boxCost > 0)
    breakdown.push({
      label: `${boxLf} lf box veneer`,
      amount: round(boxCost),
      line: { part: "box veneer", quantity: boxLf, unit: "lin ft", rate: round(num(config?.perBoxLinearFt) * mult) },
    });
...
// ── Gutters ─────────────────────────────────────────────────────────────────
//
// Two explicit ends rather than toRange(): see INSTANT_ESTIMATE_DEFAULTS.gutters.
// The measurement arrives from lib/measure/gutterMeasurement.js with its own
// verdict; an untrustworthy one never reaches here (measureForTrade refuses
// it first), but the check is repeated because a pure function should not
// trust its caller about whether 7 ft of gutter is a house.
export function estimateGutters(measurements, config, { language = "en" } = {}) {
  const ft = Math.round(num(measurements?.gutterFt));
  const ds = Math.max(0, Math.floor(num(measurements?.downspouts)));
  if (ft <= 0) return { ok: false, reason: "no_measurement" };
  if (measurements?.trustworthy === false) return { ok: false, reason: "needs_site_visit" };

  const perFtLow = num(config?.perFt?.low);
  const perFtHigh = num(config?.perFt?.high);
  if (!(perFtLow > 0) || !(perFtHigh > 0)) return { ok: false, reason: "material_not_configured" };
  const dsLow = Math.max(0, num(config?.perDownspout?.low));
  const dsHigh = Math.max(0, num(config?.perDownspout?.high));
  const minLow = Math.max(0, num(config?.minCharge?.low));
  const minHigh = Math.max(0, num(config?.minCharge?.high));

  const rawLow = ft * perFtLow + ds * dsLow;
  const rawHigh = ft * perFtHigh + ds * dsHigh;
  // The high end can never sit under the low end, whatever the card says: an
  // owner who types the two columns the wrong way round gets a range, not a
  // "$5,000 – $4,000".
  const low = Math.max(rawLow, minLow);
  const high = Math.max(rawHigh, minHigh, low);

  const t = gutterEstimateCopy(language);
  const breakdown = [
    { label: `${ft} ft of eavestrough`, amount: round((ft * perFtLow + ft * perFtHigh) / 2) },
  ];
  if (ds > 0) breakdown.push({ label: `${ds} downspout${ds === 1 ? "" : "s"}`, amount: round((ds * dsLow + ds * dsHigh) / 2) });
  ...
  return {
    ...
    minimumApplied: (minLow > 0 && rawLow < minLow) || (minHigh > 0 && rawHigh < minHigh),
    breakdown,
    assumptions: [
      t.gutterLine(ft, ds),
      t.measuredFrom(measurements?.imagery?.date || null),
      measurements?.basis === "perimeter" ? t.flatRoof : null,
      t.storeysUnknown,
      t.notAContract,
    ].filter(Boolean),
  };
...
export const INSTANT_ESTIMATE_TRADES = {
  ...
  // A drawn outline, like lawn mowing, under the GENERIC key: any trade
  // priced by the square foot of ground it can see from above traces the
  // same way (lib/estimate/tracedArea.js). lawn_polygon above is the same
  // path and keeps its own name because the settings copy, the review
  // screen's source label and the phone shapes all read the word "lawn"
  // off it. The materials are the paving book's three surfaces.
  paving: { measure: "area_polygon", fn: estimatePaving, hasMaterials: true },
  ...
  // An address, like roofing — the same Solar model, read for its eaves. Its
  // own measure key because the public form, the call shapes and the settings
  // screen all branch on this word, and "roof_address" measuring gutters is a
  // sentence somebody would later read as a bug. No materials: one profile,
  // priced by the foot and the downspout.
  gutters: { measure: "gutter_address", fn: estimateGutters, hasMaterials: false },
  // An address, sized without a trace (lib/measure/lawnEstimate.js: parcel −
  // roof − driveway where a city publishes parcels, the minimum band where
  // none does), with the drawn polygon as the correction. Its own measure
  // key: the form offers the trace as an option rather than requiring it,
  // and the phone can price it from an address where lawn_polygon cannot.
  // No materials — the homeowner picks a PROGRAM and add-ons, carried on the
  // measurement as keys and priced from the company's card.
  lawn_care: { measure: "lawn_address", fn: estimateLawnCare, hasMaterials: false },
};
 ...
 * page.
 */
export function computeInstantEstimate({ trade, measurements, materialKey, config, language = "en" }) {
  ...
  // The language is context, not a measurement: an estimator that writes its
  // assumptions for the homeowner (gutters) reads it; every other estimator
  // ignores the extra argument.
  if (spec.hasMaterials) return spec.fn(measurements, materialKey, config, { language });
  return spec.fn(measurements, config, { language });
}
