...
export async function extendAccessByMonths(companyId, months = 1) {
  ...
  // Stripe's reply, not our arithmetic: trialEndsAt is what Stripe now holds
  // (the same date, unless Stripe adjusted it), plus status and period end —
  // the shared mapping every Stripe-holding write uses.
  await db.subscription
    .update({ where: { companyId }, data: { trialEndsAt: until, ...subscriptionFieldsFromStripe(updated) } })
    .catch(() => {});
