...
export async function applySignupReferral({ company, code }) {
  ...
  try {
    ...
    //
    // When the referrer is an influencer, the NEW company still gets its
    // month — that promise is the link's, not the referrer's — but the
    // referrer's side is a SalesAttribution on their ledger row instead of a
    // month later. Written in the same transaction as the referee's grant,
    // through the same captureAttributionWithin() a rep's signup link uses:
    // self-dealing, an inactive ledger and an already-attributed company are
    // refused by the same decision, and nothing here invents a rule. The
    // verdict is returned so the signup route can log a miss the way it
    // logs one for a rep's code.
    ...
    return { referrer, trialEndsAt, attribution };
  } catch (err) {
