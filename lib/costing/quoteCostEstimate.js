...
export async function deriveQuoteCosting({ companyId, quote }) {
  ...
  const estimate = quoteCostSummary({
    ...
    // Nobody typed extra hours on a quote with no saved row — but a custom
    // complexity factor that SOLD extra hours is still time the crew spends
    // (lib/pricing/customFactors.js). 0 on every quote without one.
    addedLabourHours: customFactorHoursOf(quote.scopeGroups),
    addedMaterialCost: 0,
