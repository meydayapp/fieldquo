...
import { jobSubcontractCost, isSubcontractExpense } from "@/lib/subcontractors/money";
 ...
 *
 * ── Equipment, and the double count it must not create ─────────────────────
 *
 * The owner's ask was "a $9,000 spray rig depreciating over 5 years is a real
 * cost of every job it runs on, and none of it lands there." True, and also:
 * `overheadPerJob` (below) is ALREADY that cost, for EVERY asset the company
 * owns, spread EVENLY across its estimated job capacity — see
 * lib/accounting/depreciation.js's `assetOverhead()`, folded into
 * lib/analytics/burnRate.js's `totalMonthlyCost`, folded into
 * lib/analytics/minimumPrice.js's `costPerJob`, which is exactly the number
 * this route passes in as `overheadPerJob`. A company that has filled in
 * Settings → Overhead is ALREADY paying for the spray rig on every job,
 * whether or not that job used it.
 *
 * So `opts.equipment` (the pre-computed result of
 * lib/costing/equipmentUsage.js#equipmentCostForJob for this job's logged
 * AssetUseLog rows) is reported on every job that has any, but only ADDED to
 * `total` when `overheadPerJob` is null — i.e. when nothing else in this
 * calculation is capturing depreciation at all, because the company has never
 * set a jobs-per-week capacity. That is the one case where logging equipment
 * use makes a job's cost more honest rather than double-counted; the far more
 * common case (overhead known) reports the same figure purely as INFORMATION
 * — which asset, how many days, what it would cost in isolation — with
 * `includedInOverhead: true` telling the reader why it is not in the total.
 * See docs/SAFETY-AND-EQUIPMENT.md for the worked numbers.
 *
 * ── Overhead, and why it is an argument rather than a lookup ──────────────
 *
 * A quote's estimated cost has always included a share of overhead
 * (lib/costing/estimateJobCost.js: material + labour + overhead). This function
 * did not, so the two numbers a contractor sees for the same work were on
 * different bases: the job panel showed a GROSS margin and the variance against
 * the quote was structurally biased toward "under budget", because the estimate
 * carried a cost the actual never did.
 *
 * It arrives as an argument because this file is pure and its check drives
 * every branch without a database — and because it must be possible to ask for
 * the figure WITHOUT overhead. `null` is not "no overhead": it is "nobody has
 * told us what a job costs to run", which is the normal state for a company
 * that has not filled in the overhead screen, and it stays absent rather than
 * becoming zero.
 *
 * ── Subcontractors, and the OTHER double count ────────────────────────────
 *
 * `opts.subcontracts` is the job's JobSubcontractor rows. Their agreedAmount
 * (agreed/done/paid — never `quoted`) is its own line, `subcontracts`, and is
 * ADDED to the total: a company hired for a fixed price is a cost of the job
 * the way a crew's approved hours are, and it was nowhere on this screen.
 *
 * The same money also reaches the Expense table — every payment to a sub
 * writes an expense so the P&L sees it leave, and an imported quote is
 * materialised into one when the quote becomes a job. Those rows are dropped
 * from `expenses` here, by category and by id, so a sub is costed ONCE, at
 * the agreed amount. lib/subcontractors/money.js says which rows and why;
 * the short version is that the agreed amount is the liability and the
 * payments are how it is being settled, and counting both would cost a
 * $5,000 electrician at $10,000 the day the last cheque cleared.
 *
 * @param {object[]} expenses     rows tagged to this job: { id?, category, amount }
 * @param {object[]} timeEntries  rows for this job: { hours, status, worker }
 * @param {object}   opts         { overheadPerJob, overheadBasis, equipment, subcontracts }
 *                                `equipment` is the pre-computed result of
 *                                lib/costing/equipmentUsage.js#equipmentCostForJob
 *                                for this job, or null/omitted for a job with
 *                                no logged asset use — see the double-count
 *                                note above. `subcontracts` is the job's
 *                                JobSubcontractor rows ({ agreedAmount,
 *                                status, importExpenseId? }).
 */
export function actualJobCost(expenses = [], timeEntries = [], opts = {}) {
  const exp = Array.isArray(expenses) ? expenses : [];
  const entries = Array.isArray(timeEntries) ? timeEntries : [];

  // Costed before the expenses, because which expense rows are skipped
  // depends on which subcontract rows are counted — see the header.
  const subcontract = jobSubcontractCost(opts?.subcontracts);
  const adoptedExpenseIds = new Set(subcontract.excludedExpenseIds);

  const byCategory = new Map();
  let expenseTotal = 0;
  let subcontractExpensesSkipped = 0;
  for (const e of exp) {
    const amount = num(e?.amount);
    if (!amount) continue;
    // Already in `subcontracts` below, at the agreed amount. Counted here
    // for the panel's sentence, never for the total.
    if (isSubcontractExpense(e, adoptedExpenseIds)) {
      subcontractExpensesSkipped += amount;
      continue;
    }
  ...
  // Null-checked before Number(), the same way an hourly rate is: Number(null)
  // is 0, and "we don't know what this company's overhead is" must not render
  // as "this company has none".
  const rawOverhead = opts?.overheadPerJob;
  const overhead =
    rawOverhead === null || rawOverhead === undefined || rawOverhead === ""
      ? null
      : Number.isFinite(Number(rawOverhead))
        ? round2(Number(rawOverhead))
        : null;

  // A job with nothing logged against it must cost exactly what it cost
  // before this field existed — see the double-count note above and
  // scripts/check-job-costing.mjs's "no assets logged" regression assertion.
  const rawEquipment = opts?.equipment;
  const equipmentTotal =
    rawEquipment && Number.isFinite(Number(rawEquipment.total))
      ? round2(Number(rawEquipment.total))
      : 0;
  // Added to the total ONLY when nothing else here is already carrying
  // depreciation — see the file-level comment above `actualJobCost`.
  ...
  return {
    ...
    // The same share the quote was costed with, so the two are comparable.
    // Reported as its own line rather than folded silently into the total: a
    // margin that dropped because overhead arrived must be explainable, and a
    // contractor who cannot see why will assume the number is broken.
    overhead:
      overhead === null
        ? null
        : { amount: overhead, basis: opts?.overheadBasis || null },
    // Null when nothing was logged (equipmentTotal is 0 and rawEquipment was
    // never passed) — a job with no equipment logged says nothing here, the
    // same absence-is-not-zero rule `overhead` above follows, rather than
    // printing a $0 section that reads as "we checked and found nothing".
    equipment:
      rawEquipment == null
        ? null
        : {
            total: equipmentTotal,
            byAsset: Array.isArray(rawEquipment.byAsset) ? rawEquipment.byAsset : [],
            // Whether `total` above is already inside `overhead`'s figure.
            // True whenever overhead is known — see the double-count note on
            // this function. The UI must show this figure as informational,
            // never add it to a displayed total itself, when this is true.
            includedInOverhead: overhead !== null,
            addedToTotal: equipmentAddedToTotal > 0,
          },
    // Companies hired for a fixed price on this job, at the price agreed.
    // Null when the job has none, the same absence-is-not-zero rule as
    // `overhead` and `equipment`: a job with no subs says nothing here rather
    // than printing a $0 line that reads as "we checked and found none".
    subcontracts:
      subcontract.count === 0 && subcontract.byStatus.quoted === 0
        ? null
        : {
            total: subcontract.total,
            count: subcontract.count,
            byStatus: subcontract.byStatus,
            // What the same subs' payment and import expenses added up to,
            // and were NOT counted, so the panel can say "the $2,000 you have
            // paid so far is inside the $5,000 above" instead of leaving the
            // reader to wonder where their receipt went.
            expensesExcluded: round2(subcontractExpensesSkipped),
          },
    total: round2(
      expenseTotal + labourCost + (overhead ?? 0) + equipmentAddedToTotal + subcontract.total,
    ),
...
export const MAX_HOURS = 99_999_999.99;
...
/**
 * The browser → database boundary for a cost panel — invoice OR quote.
 *
 ...
 * "nobody has logged time for this person yet"; on a quote it means "take an
 * even share of the predicted pool". Both are different statements from "zero
 * hours", and crewLabourCost reads the two differently.
 *
 * Shared rather than copied because the crew rows are the same rows on both
 * documents. lib/costing/quoteCosting.js adds the fields only a quote has;
 * a second copy of this sanitiser is the one that would rot.
 */
export function normaliseCostingInputs(input) {
...
/**
 * The invoice's name for the boundary above. Kept because the invoice routes
 * read as invoice code, and because renaming an export three callers use to
 * save one line of indirection is churn without a benefit.
 */
export const normaliseInvoiceCosting = normaliseCostingInputs;
...
export function invoiceCostSummary({
  ...
  price = 0,
  marginTargetPct = 30,
  // The invoice's own lines' cost — quantity × unitCost per line, worked out
  // by the caller from the lines it is about to store (lib/costing/
  // lineItemCost.js). 0 when no line states a cost.
  lineItemCost = 0,
} = {}) {
