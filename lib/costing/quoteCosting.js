...
import { tradeLabourHours, estimateCabinetDoorCost } from "@/lib/pricing/tradeScope";
...
function takeoffDerived(group) {
  ...
  if (!categoryKey || !takeoff || typeof takeoff !== "object") {
    return { hours: 0, purchased: 0 };
  }
  let hours = 0;
  let purchased = 0;
  try {
    hours = num(tradeLabourHours(categoryKey, takeoff, rateOverrides || null));
  } catch {
    hours = 0;
  }
  try {
    const book = getPriceBook(categoryKey, rateOverrides || null) || {};
    purchased = num(estimateCabinetDoorCost(takeoff, book)?.total);
  } catch {
    purchased = 0;
  }
