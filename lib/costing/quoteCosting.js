...
/// The margin a quote is measured against when the company has set none.
/// Re-exported from lib/costing/marginTarget.js so the panel, the saved row
/// and the read endpoint cannot disagree about what "green" means — and so
/// it is the SAME default the Settings → Overhead box and the price floor
/// use. Callers that know the company pass its own target instead
/// (companyMarginTarget in lib/costing/quoteCostEstimate.js).
export const MARGIN_TARGET_PCT = DEFAULT_MARGIN_TARGET_PCT;
...
function takeoffDerived(group) {
  ...
  let hours = 0;
  let purchased = 0;
  // A group whose services' production rates answer "how long" takes those
  // hours instead (lib/services/productionRates.js — the owner's precedence
  // rule). They are handed to estimateQuoteCost on the group itself, so the
  // book's takeoff hours are left out here rather than counted twice. Absent
  // on every group without a rated service: this branch is not taken.
  // The same test estimateJobCost.js productionHoursOf applies, so the two
  // halves cannot disagree about whether a group is rated.
  const productionRated =
    group.productionHours !== null &&
    group.productionHours !== undefined &&
    group.productionHours !== "" &&
    typeof group.productionHours !== "boolean" &&
    Number.isFinite(Number(group.productionHours)) &&
    Number(group.productionHours) >= 0;
  try {
    hours = takeoff && !productionRated
...
export function quoteCostSummary({
  ...
  // The lines' own cost — see lib/costing/lineItemCost.js. Worked out by the
  // CALLER from the groups' stored lines (the save routes) or the screen's
  // (the builder), never re-read here: this function takes the cost inputs
  // and is deliberately blind to what a line is priced at.
  lineItemCost = 0,
} = {}) {
  ...
  const estimate = estimateQuoteCost({
    scopeGroups: groups.map((g, i) => ({
      ...
      // Only when the group's services answered — see takeoffDerived. An
      // absent key keeps the object estimateQuoteCost receives what it was.
      ...(g.productionHours !== null && g.productionHours !== undefined ? { productionHours: g.productionHours } : {}),
    })),
...
export function quoteCostingShape(source, { saved }) {
  ...
  return {
    ...
    // before the column existed — "no line stated a cost", which is what
    // every such quote's lines say.
    lineItemCost: round2(source.lineItemCost),
