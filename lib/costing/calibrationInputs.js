...
export async function calibrationInputs({ companyId, scopeGroups, frozenGroups }) {
  ...
  const labourGroups = resolvedGroups.map((g, i) => ({
    ...
    // A group whose hours came from its services' production rates
    // (lib/services/productionRates.js) was not predicted by the trade's
    // book, so it cannot calibrate the book: suggesting a new book rate off
    // hours the book never produced would teach it the service's number.
    // No basis = left out of the labour suggestion (labourCalibration.js
    // filters on it). Absent on every other group.
    basis: g.productionHours != null ? null : labourBasisFor({
      categoryKey: g.categoryKey,
      intake: g.intakeValues || {},
      takeoff: g.takeoff || null,
      rateOverrides: g.rateOverrides || null,
