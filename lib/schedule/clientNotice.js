// lib/schedule/clientNotice.js
//
// Telling the client when the OFFICE moves or cancels their visit.
//
// ── One sender for two kinds of row ────────────────────────────────────────
//
// An Appointment and a JobVisit reach the client's inbox through the same two
// letters the client's own "manage my visit" link sends — the moved letter and
// the cancelled letter in app/admin/lib/email/templates.js — with
// `initiatedBy: "office"` picking the sentence that is true. The letters are
// reused rather than re-described so a client who moved once and was moved
// once reads the same shape twice.
//
// ── The language is the client's, decided by the one rule ──────────────────
//
// lib/i18n/clientLanguage.js: the quote this visit is about, else the client's
// saved language, else the company default. A JobVisit's job came from a quote
// with a fixed language; an Appointment may name one. Either way the office's
// own language never leaks into a letter the client reads.
//
// ── Best-effort by contract ────────────────────────────────────────────────
//
// The row has already changed by the time either of these runs. A Resend
// hiccup is logged and swallowed; the caller must never turn it into "couldn't
// move that", because it did.

import {
  sendVisitRescheduledEmails,
  sendVisitCancelledEmails,
} from "@/app/admin/lib/email/templates";
import { db } from "@/lib/db";
import { resolveClientLanguage } from "@/lib/i18n/clientLanguage";
import { emailCopy } from "@/lib/i18n/emailCopy";
import { loadPhrases } from "@/lib/i18n/phrases";

/**
 * What to call the visit in the letter.
 *
 * A booking has an event type ("On-site estimate"); a job visit has the job's
 * title; a bare appointment has neither, and gets the catalogue's plain word
 * for a visit in the client's language rather than "undefined with Northline".
 *
 * `measure` is the one exception between the two: an Appointment scheduled
 * FROM A QUOTE (Appointment.quoteId — see lib/quotes/siteVisit.js) has no
 * event type and no job yet, but it is not a bare visit either; it is the
 * estimator coming to measure up, and the letter names it as such in the
 * client's language.
 */
export function serviceName({ eventTypeName, jobTitle, language, measure = false }) {
  const named = String(eventTypeName || jobTitle || "").trim();
  if (named) return named;
  const copy = emailCopy(language)?.visit || {};
  if (measure) return copy.measureService || emailCopy("en").visit.measureService;
  return copy.serviceFallback || emailCopy("en").visit.serviceFallback;
}

/**
 * serviceName, with a booking's appointment type in the CLIENT's language.
 *
 * The type's name is company text, drafted into the other document languages
 * when it was saved (lib/i18n/phrases.js, namespace eventTypeName); this reads
 * the stored draft for `language` and falls back to the name as typed. The
 * job title is not a phrase and is printed as written, as before.
 *
 * Async and separate from serviceName because it reads the database — the
 * letters below and the appointment route's text both call THIS, so the
 * letter and the text cannot name one visit two ways. `deps.db` is for a
 * check.
 */
export async function localServiceName({ company, eventTypeName, jobTitle, language, measure = false }, deps = {}) {
  const tr = await loadPhrases(deps.db || db, company?.id, language, [{ ns: "eventTypeName", text: eventTypeName }]);
  return serviceName({
    eventTypeName: typeof eventTypeName === "string" ? tr("eventTypeName", eventTypeName) : eventTypeName,
    jobTitle,
    language,
    measure,
  });
}

/**
 * "On-site estimate" is only true of a visit about a QUOTE. An appointment
 * linked to a job or an invoice carries `about` naming that record and is a
 * plain visit in the letter's first line; the record is named on the next.
 */
export function isMeasure(quote, about = null) {
  if (about?.kind === "job" || about?.kind === "invoice") return false;
  return Boolean(quote);
}

/**
 * The arrival window the client is promised, if any: the company's setting,
 * for an on-site visit only. A phone or video appointment has no drive to
 * absorb, so its time stays exact — the same rule visitView applies.
 */
export function windowFor(company, mode) {
  return mode && mode !== "visit" ? 0 : company?.arrivalWindowMinutes || 0;
}

/**
 * @param {object} args
 * @param {object} args.company     full Company row (sender, zone, window, default language)
 * @param {object} args.client      { name, email, language } — email absent → nothing sent
 * @param {object} [args.quote]     { language, quoteNumber } when the visit is about one
 * @param {object} [args.about]     aboutLabel() of the row — the job or invoice a
 *                                  hand-booked appointment is about, named in the
 *                                  letter the way the calendar card names it
 * @param {object} [args.document]  { language } of that record when it is not a
 *                                  quote (an invoice's own; a job's quote's), so
 *                                  non-negotiable #6 holds for those letters too
 * @param {string} [args.eventTypeName]
 * @param {string} [args.jobTitle]
 * @param {Date}   args.previousStartTime
 * @param {Date}   args.startTime
 * @param {string} [args.location]
 * @param {object} [args.where]     { mode, address, phone, email } — the booking's
 *                                  own facts, when there is a booking behind the
 *                                  row; the letter then names the kind of
 *                                  appointment in its own language
 *                                  (lib/booking/bookingModes.js) and `location`
 *                                  is only the fallback for a hand-booked one
 * @param {string} [args.mode]      "visit" | "call" | "video"
 * @param {string} [args.manageUrl] the client's own link, when the row has one
 * @returns {Promise<{ sent: boolean, language: string }>}
 */
export async function notifyClientMoved({
  company,
  client,
  quote = null,
  about = null,
  document = null,
  eventTypeName,
  jobTitle,
  previousStartTime,
  startTime,
  location,
  where = null,
  mode = "visit",
  manageUrl = null,
  // The re-issued calendar invite, for the client's copy only — built by
  // the caller from the booking row (lib/booking/bookingInvite.js).
  attachments = null,
}) {
  const language = resolveClientLanguage({ document: quote || document, client, company });
  const to = String(client?.email || "").trim();
  if (!to) return { sent: false, language };

  await sendVisitRescheduledEmails({
    company,
    clientName: client?.name || "",
    clientEmail: to,
    eventTypeName: await localServiceName({ company, eventTypeName, jobTitle, language, measure: isMeasure(quote, about) }),
    previousStartTime,
    startTime,
    location,
    where,
    timezone: company?.timezone,
    arrivalWindowMinutes: windowFor(company, where?.mode || mode),
    manageUrl,
    quoteNumber: quote?.quoteNumber || null,
    about,
    language,
    initiatedBy: "office",
    ...(attachments && { attachments }),
  }).catch((err) => console.error("[schedule] moved letter failed:", err?.message));

  return { sent: true, language };
}

/**
 * Same shape as notifyClientMoved. No refund is decided here: an office
 * cancelling a paid booking is a money decision the office makes on the
 * payment itself, and the letter's fee sentence says so honestly (the
 * `feeNotReturned` wording) rather than promising a refund nobody issued.
 */
export async function notifyClientCancelled({
  company,
  client,
  quote = null,
  about = null,
  document = null,
  eventTypeName,
  jobTitle,
  startTime,
  location,
  where = null,
  fee = null,
  attachments = null,
}) {
  const language = resolveClientLanguage({ document: quote || document, client, company });
  const to = String(client?.email || "").trim();
  if (!to) return { sent: false, language };

  await sendVisitCancelledEmails({
    company,
    clientName: client?.name || "",
    clientEmail: to,
    eventTypeName: await localServiceName({ company, eventTypeName, jobTitle, language, measure: isMeasure(quote, about) }),
    startTime,
    location,
    where,
    timezone: company?.timezone,
    quoteNumber: quote?.quoteNumber || null,
    about,
    refund: fee
      ? { refunded: false, amountCents: fee.amountCents, currency: fee.currency, reason: "office_cancelled" }
      : {},
    language,
    initiatedBy: "office",
    ...(attachments && { attachments }),
  }).catch((err) => console.error("[schedule] cancelled letter failed:", err?.message));

  return { sent: true, language };
}
