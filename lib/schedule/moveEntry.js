 ...
 * @param {Array<{startAt, endAt}>} [args.busy]  the assignee's PERSONAL busy
 *                                     blocks (their Google Calendar, via
 *                                     lib/calendar/googleBusy.js). A candidate
 *                                     that overlaps one is refused with
 *                                     "personal_busy" — no travel arithmetic,
 *                                     because a personal event is not a stop
 *                                     the van drives from; it is time the
 *                                     person said they do not have. The
 *                                     office may still force it.
...
export function planOfficeMove({
  ...
  // Personal busy time first: an overlap is a refusal on its own, before any
  // question of whether the drive fits. `force` overrides it the same way it
  // overrides a short drive — the office knows the person said yes.
  if (!force) {
    for (const b of Array.isArray(busy) ? busy : []) {
      ...
      if (bs < end.getTime() && be > start.getTime()) return deny(409, "personal_busy");
    }
  }
...
export function bracketStops(stops = [], start) {
  ...
  for (const s of Array.isArray(stops) ? stops : []) {
    ...
    // A personal busy block is not a stop: nothing drives from it. It is
    // handed to planOfficeMove as `busy` instead (see assigneeStopsAround).
    if (s.kind === "google_busy") continue;
...
export function moveReasonMessage(reason, travel) {
  switch (reason) {
    ...
    case "personal_busy":
      return "That overlaps something on their own calendar. Move it anyway if they have agreed.";
    case "travel_short":
...
export const MOVE_REASONS = Object.freeze(["no_time", "bad_time", "in_the_past", "travel_short", "personal_busy"]);
