...
export async function assigneeStopsAround({ companyId, assignedToId, start, exclude = {} }) {
  ...
  // The assignee's own Google Calendar, as opaque blocks. `kind: "google_busy"`
  // is what bracketStops skips and planOfficeMove refuses on — a personal
  // event has no point and no drive, so it is neither a previous nor a next
  // stop; it is time the person does not have. Empty when nothing is
  // connected or "use my busy time" is off (lib/calendar/googleBusy.js).
  const busy = await googleBusyRanges({ userId: assignedToId, companyId, from, to });
  ...
  return [
    ...
    ...busy.map((b) => ({ id: null, kind: "google_busy", startAt: b.start, endAt: b.end, point: null })),
  ];
}
