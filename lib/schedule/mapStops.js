...
export function buildMapStops(entries = [], { groupOf = null, people = [], groups = [] } = {}) {
  ...
  const stops = live.map((e) => {
    ...
    return {
      ...
      // Only a visit has an address on the map. A phone or video booking
      // (Booking.mode, carried on the feed entry as e.booking.mode) used to
      // fall through to the client's billing address and get a pin, which is
      // a van sent to a call. Those stops carry the mode instead, and the
      // popover names it from lib/booking/bookingModes.js.
      mode: e.booking?.mode || null,
      address:
        e.booking?.mode && e.booking.mode !== "visit"
          ? null
          : e.location || e.client?.address || null,
      ...about,
    };
  });
