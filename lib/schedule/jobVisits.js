...
export const VISIT_INCLUDE = {
  ...
  job: {
    select: {
      id: true,
      title: true,
      // The quote the job came from — the calendar card's "Open quote" link
      // (lib/schedule/entryCard.js). An id only; the link is offered on the
      // quotes dial and the quote page redacts its own money.
      quoteId: true,
      client: { select: { id: true, name: true, address: true } },
      // Where the site is — the geocode of Job.siteAddress
      // (lib/geo/geocodeJob.js), or the pair the address pick wrote. Read by
      // the day map so a visit can be a pin; carried below in the same
      // precedence lib/schedule/entryNeighbours.js measures travel by.
      latitude: true,
      longitude: true,
      siteLatitude: true,
      siteLongitude: true,
...
/** A Decimal, a number or a string as a finite number, else null. */
...
/**
 * The job's point as { latitude, longitude }, both or neither. The site pair
 * is taken only when BOTH halves are set — a latitude from one pair and a
 * longitude from the other is a place nobody has ever been.
 */
function jobPoint(job) {
  const site = { latitude: coord(job?.siteLatitude), longitude: coord(job?.siteLongitude) };
  if (site.latitude != null && site.longitude != null) return site;
  const geo = { latitude: coord(job?.latitude), longitude: coord(job?.longitude) };
...
export function toCalendarEntry(visit) {
  ...
  return {
    ...
    quoteId: visit.job?.quoteId ?? null,
    ...
    // supervisor flag, and the travel-leg code must see a null coordinate,
    // not a missing key it might treat as "not loaded yet".
    booking: null,
    requiresSupervisor: false,
    // When the row last changed. The subscribe feed (lib/calendar/feed.js)
    // turns this into SEQUENCE and LAST-MODIFIED, which is how a phone knows
    // the copy it holds is older than the one it just fetched. An appointment
    // spreads its whole row and carries the column for free; the two shaped
    // kinds must carry it on purpose. Null on a row that predates the column
    // — the feed then stamps it with "now", which only ever errs toward the
    // phone re-reading a row.
    updatedAt: visit.updatedAt ?? null,
    // The JOB's coordinates, since a visit happens at the job's site. The
    // site's own pair first, the siteAddress geocode second — the order
    // entryNeighbours.js already uses, so the map and the travel check agree
    // on where a visit is. Null when the job has neither: the map lists such
    // a visit as "no location on the map", and travelLegs says nothing about
    // the drive, which was true before as well.
    ...jobPoint(visit.job),
  };
}
  ...
  return {
    ...
    client: booking.clientName
      ? {
          ...
          // ── The number the booking was made with ──────────────────────
          //
          // Booking.clientPhone and clientEmail are captured at booking time
          // and were dropped here, so an unconverted booking reached the
          // calendar as a name and a time — the exact complaint, on the one
          // kind of row that has no client record to fall back on.
          //
          // Empty string to null: clientEmail is a required column and a phone
          // booking writes "" into it. "" would render as a mailto: link to
          // nowhere, where null renders as nothing.
    ...
    booking: booking.endTime
      ? {
          ...
          // Same shape the appointment branch emits, mode included — the
          // calendar reads one field for both kinds, and a badge that appeared
          // on converted bookings and vanished on identical unconverted ones
          // would read as a data problem rather than a rendering one.
          mode: booking.mode ?? null,
        }
      : null,
    requiresSupervisor: false,
    // Same reason as on a visit above: the feed's SEQUENCE.
    updatedAt: booking.updatedAt ?? null,
    latitude: booking.latitude ?? null,
    longitude: booking.longitude ?? null,
    // The Google Meet link the calendar sync minted for a video booking
    // (lib/calendar/googleSync.js), carried so a reconcile of an unconverted
    // booking sees it and never asks Google for a second room.
...
export function appointmentToCalendarEntry(appointment) {
  ...
  return {
    ...
    // ── Recorded for a reason, so carried ───────────────────────────────
    //
    // Each of these is stored on every appointment and reached no screen. A
    // column written and never read is the failure AGENTS.md opens with, and it
    // is the same one either way round: the product knew which quote this visit
    // was about, whether the client had been reminded, and where the place
    // actually is — and showed a name and a time.
    //
    // Nulls stay null. An appointment with no quote behind it is not an
    // appointment with a broken link.
    quoteId: appointment.quoteId || null,
    reminderSentAt: appointment.reminderSentAt || null,
    // For a directions link. Geocoded once at booking time so the scheduler can
    // work out travel; the same pair is what opens a map without re-geocoding
    // a street the crew is already standing near.
