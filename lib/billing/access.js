...
export async function accessForCompany(companyId, now = new Date()) {
  ...
  const sub = await db.subscription.findUnique({
    where: { companyId },
    select: { status: true, pastDueSince: true, canceledAt: true, cancelAtPeriodEnd: true, cancelAt: true, currentPeriodEnd: true, accessLockedAt: true, trialEndsAt: true },
  });
  if (!sub) {
    ...
    // the gate does.
    const company = await db.company.findUnique({ where: { id: companyId }, select: { createdAt: true, isDemo: true, trialEndsAt: true } });
