...
export function subscriptionFieldsFromStripe(obj, { now = new Date() } = {}) {
  ...
  // ── A cancellation that has not landed yet ───────────────────────────────
  //
  // A paying subscription is cancelled at the end of its paid period
  // (app/api/platform/billing/cancel/route.js), so Stripe carries
  // `cancel_at_period_end: true` and a `cancel_at` for weeks while the status
  // stays `active`. Written only when the object carries the key: the
  // synthetic `{ id, status: "canceled" }` objects the routes build for a
  // subscription Stripe no longer has must not turn "unknown" into "false".
  // Once the status IS canceled the pending end is over and both are cleared,
  // whatever the object says — Stripe leaves cancel_at_period_end true on the
  // deleted object, and a row reading "ends on Oct 13" under "Cancelled" is
  // two statements about one fact.
  if (status === "canceled") {
    fields.cancelAtPeriodEnd = false;
    fields.cancelAt = null;
  } else {
    if ("cancel_at_period_end" in obj) fields.cancelAtPeriodEnd = Boolean(obj.cancel_at_period_end);
    if ("cancel_at" in obj) fields.cancelAt = secondsToDate(obj.cancel_at);
  }
...
export const SYNCED_COLUMNS = Object.freeze([
  ...
  "billingInterval",
  "canceledAt",
  // The pending end of a paying subscription. In the drift set so a
  // cancellation scheduled from the Stripe dashboard or the customer portal
  // — neither passes through this app — reaches the banner within six hours.
  "cancelAtPeriodEnd",
  "cancelAt",
]);
