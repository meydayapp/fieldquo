...
//
// ══ Annual is the INTERVAL, and it now carries a discount ══════════════════
//
// It did not always. The owner's original decision was "the 1 yr commitment is
// just billed annually instead of the no commitment" — same rate, priceAnnual
// seeded at twelve times the monthly figure, annualSaving() returning 0. He
// then corrected it, for the reason annualSaving()'s own doc gives below: a
// commitment that saves nothing asks a customer to give up flexibility for
// nothing, and is therefore never taken. The ladder gives two months free.
//
// This paragraph said the old thing until 2026-09-03, three doc comments and a
// check script down the file all said the new one, and Account & Billing had
// been rendering "Save {amount} a year" for months. Whichever reader trusted
// the header was the one who got it wrong.
//
// No screen may claim a saving this file does not compute: if an operator types
// a different annual deal per tier into /platform/billing/plans, the number
...
/**
 * The cadence a LIVE Stripe subscription is actually billed on.
 *
 * The authority, and the reason Subscription.billingInterval is written from
 * here rather than from whatever the checkout screen asked for: a cadence can
 * change in the Stripe customer portal, which never passes through this app.
 *
 * Returns null when Stripe's object doesn't say — a one-off item, a deleted
 * price, a shape we don't recognise. Null must be treated as "leave what we
 * have", never as "monthly": overwriting a real yearly commitment with a guess
 * is how the two free months would go missing a second time.
 */
export function intervalFromStripeSubscription(sub) {
  const items = sub?.items?.data;
  if (!Array.isArray(items)) return null;
  for (const item of items) {
    const interval = item?.price?.recurring?.interval;
    if (isBillingInterval(interval)) return interval;
  }
  return null;
}

/**
 * The cadence a checkout may actually be created on, or a refusal.
 *
 * ══ Why this is a function and not four lines in the route ═════════════════
 *
 * It was four lines in the route, asserted by a regex looking for
 * `supportsInterval(plan, interval)` in the source. Mutation-testing that check
 * — putting `false &&` in front of the guard, so an unsellable cadence sailed
 * through — did not fail it. The regex still matched the dead condition. An
 * assertion that passes on broken code proves nothing, and this particular
 * broken code puts "1 year commitment" on the screen with a monthly charge on
 * the card.
 *
 * So the decision lives here, where a check script can EXECUTE it against a
 * plan that has no annual price and watch it refuse.
 *
 * An unrecognised or absent `requested` falls back to the default — that is a
 * caller who said nothing, and monthly is the option with no commitment
 * attached. A recognised cadence the PLAN cannot be sold on is a refusal, never
 * a fallback: silently downgrading a stated yearly choice to monthly is the
 * whole failure this file exists to prevent.
 *
 * @returns {{ interval: string }|{ error: string }}
 */
export function resolveCheckoutInterval(plan, requested) {
  const interval = isBillingInterval(requested) ? requested : DEFAULT_INTERVAL;
  if (supportsInterval(plan, interval)) return { interval };
  return {
    error:
      interval === "year"
        ? "That plan isn't sold on a yearly commitment. Choose monthly, or pick a different plan."
        : "That plan can't be billed monthly.",
  };
}
