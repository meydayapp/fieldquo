 ...
 * @param {Date}        [s.now]
 * @returns {{ mode: "uncancel"|"credited"|"charge_now"|"checkout"|"none",
...
export async function loadResumeState(companyId, { now = new Date() } = {}) {
  ...
  if (!row?.stripeSubscriptionId) {
    return { row, company, live: null, customer: null, priceId: null, paymentMethodId: null, decision: { mode: "checkout", reason: "no_subscription" } };
  }
  ...
  try {
    ...
    return { row, company, live: null, customer: null, priceId: null, paymentMethodId: null, decision: { mode: "checkout", reason: "stripe_missing" } };
  }
