...
/**
 * Stripe caps a coupon's `name` at 40 characters. "Retention 25% — <cuid>"
 * was 41, so every discount a company ever accepted failed with "Invalid
 * string … must be at most 40 characters" (reproduced in test mode
 * 2026-09-13). The name carries the tail of the id — enough to tell coupons
 * apart in the Stripe dashboard — and the full companyId lives in the
 * coupon's metadata, which is where a lookup reads it anyway.
 */
export const STRIPE_COUPON_NAME_MAX = 40;
export function retentionCouponName(companyId) {
  const tail = String(companyId || "").slice(-8);
  const name = `Retention ${DISCOUNT_PERCENT}% ${tail}`;
  if (name.length > STRIPE_COUPON_NAME_MAX) {
    throw new Error(`retentionCouponName: ${name.length} chars exceeds Stripe's ${STRIPE_COUPON_NAME_MAX}`);
  }
  return name;
}
 ...
 * @param perSeat       false on a flat ladder tier, where licences aren't billed
...
export function offersFor({
  ...
  //
  // And only on a plan that BILLS per licence. A ladder tier is a flat price
  // for a seat count — Solo is $99 whether one seat is filled or none — so
  // "removing 3 cuts the bill" would be false, and the apply path would set a
  // quantity on an item that is priced once. The presence of a crew allowance
  // is what marks a flat tier; legacy per-headcount plans have none.
  const unused = perSeat ? Math.max(0, seats - Math.max(1, activeMembers)) : 0;
  if (unused > 0) {
    offers.push({
      key: "reduce_licenses",
