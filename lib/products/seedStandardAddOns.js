...
export async function seedStandardAddOns({ companyId, categoryId, categoryKey }) {
  ...
  // In batches of ten, the way lib/products/seedServices.js writes its rows,
  // rather than one await per row. Each create is a nested write (the category
  // `connect`), so Prisma spends several round trips on it; serially that was
  // the single longest stretch of a painter's signup (measured 2026-09-25 —
  // docs/ROADMAP.md, "Signup progress screen"). The rows are independent:
  // `toCreate` was decided above, before any write, so the order they land in
  // changes nothing, and a partial failure leaves exactly the rows a re-run
  // (matched by name) picks up.
  const createOne = (a) =>
    db.product.create({
      data: {
        companyId,
  ...
  for (let i = 0; i < toCreate.length; i += 10) {
    await Promise.all(toCreate.slice(i, i + 10).map(createOne));
  }
