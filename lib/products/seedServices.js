...
import { planServiceSeeds, seedText, seedTemplateFor, serviceSeedsFor } from "@/lib/services/seeds";
...
export function productDataForSeed(service, { companyId, categoryId, language, currency }) {
  ...
  const { templateLines, defaultDiscount, imageUrl } = seedTemplateFor(service, { language, currency });
  return {
    companyId,
