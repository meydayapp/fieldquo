...
export const SCREENS_CHAPTER = {
  ...
  items: {
    ...
    "settings-team": { body: [
      ...
      "« Travailleurs » est un onglet de cet écran, pas une ligne des Paramètres, et seuls le propriétaire et les administrateurs le voient — « Tout le monde sur les registres — leur taux de rémunération, leur date d'embauche et leur statut de paiement. » Chaque ligne s'ouvre pour modifier le nom, le mobile d'où la personne envoie les photos de chantier, le taux horaire, la date d'embauche, à qui elle rend des comptes, chantier ou bureau, les heures garanties par semaine et si elle est active, avec « Connecter Stripe » pour les virements ; la page refuse tous les autres, Manager et Dispatcher compris, parce qu'un taux de rémunération relève de la paie où qu'on le saisisse, et le statut employé ou contractuel n'est volontairement pas modifiable ici.",
    ] },
...
export const ROLES_CHAPTER = {
  ...
  intro: [
    "L'entrepreneur ajoute une personne depuis Gérer l'équipe et choisit un niveau d'accès parmi cinq : Crew, Estimator, Dispatcher, Manager, ou son propre niveau de propriétaire. Les quatre premiers sont des préréglages — une grille remplie de onze domaines de permission et trois interrupteurs. Crew est figé : on le choisit et il n'y a aucun réglage à déplacer, parce que le niveau gratuit ne peut pas être remodelé — une grille relevée au-dessus du plafond Crew est un poste payant, quel que soit le chemin pris. Les trois préréglages payants — Estimator, Dispatcher, Manager — peuvent être remodelés ensuite, et déplacer n'importe quel réglage transforme le préréglage en « Personnalisé ». Les tableaux ci-dessous sont calculés à partir du code de permissions du produit lui-même, le jour où ce guide a été généré ; ils disent donc ce que le menu fait réellement.",
