...
19. `DONE — code + checks, commits f597ae77 + ac2af44f; verify on www.fieldquo.com after deploy` **Every Stripe fee passes through to the contractor** (owner: "we are not going to pay out of pocket … make sure that all of the fees are pass through") — every homeowner charge carries `application_fee_amount` from ONE rate table (`lib/stripe/processingFee.js`: cards 3% + 30¢ = Stripe's 2.9% + 30¢ + a 0.1% margin, PAD 1% + 40¢ max $5, ACH 0.8% max $5) plus `on_behalf_of`; at settlement the charge is TRUED UP to Stripe's actual fee (international card +0.8%, currency conversion +2%, Affirm) by a partial transfer reversal — a domestic card pays the published rate, never less; Stripe's Connect account fees (monthly active account, 0.25% + 25¢ per payout) are read by the daily `/api/cron/connect-fees` into `ConnectFeeRecovery` and ride on the company's next charge as their own line, capped and carried forward (never collected from a company that never pays again — shown as outstanding on /platform/billing/plans); new Express accounts request card_payments + transfers, older ones are repaired on the status poll; refunds keep the fee; disputes reverse the held amount + $15 fee and return the hold when won; `Payment.processingFeeCents/netCents/feeRateLabel/estimatedFeeCents/stripeFeeCents/accountFeeRecoveredCents` written at settlement and exported. Instant payouts ON with the 1% fee stated on the card and on a confirm step — **owner: Stripe → Settings → Connect → Platform pricing → Instant payouts = 1%, and Settings → Connect → Payouts → Allow debit cards = Yes**. Settings → Payments rate card (with surcharges), /platform read-only rates + Connect-fee totals, pricing-page sentence, Jobber bank-debit claim, sales guide EN/FR/ES. `check:processing-fee` (188 executed, in check:all). Not verified live: a real card payment on live keys showing the fee and a true-up on the invoice; the first cron pass against real `stripe_fee` descriptions (the account-id recognition is by description); the Express debit-card path for instant payouts.
