...
5. Sign in to /app and /sales in the Chrome with the Upwork tabs → product-guide screenshots (every sidebar + Settings item), RBAC section (Crew/Estimator/Dispatcher/Manager/Owner + Custom), live sales-portal captures (the manual no longer waits on them — fixture renders stand in until they land).
