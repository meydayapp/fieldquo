...
20. `IN PROGRESS — spine committed 4d37c958, articles landing per category; verify on www.fieldquo.com/help after deploy, help.fieldquo.com after the DNS step` **Public help centre, help.fieldquo.com** (owner: "like help.getjobber.com") — `middleware.js` rewrites the host to `/help/*` before every gate (a reader has no session); `help` moved to the "serves today" block of the reserved list; also served at `www.fieldquo.com/help/{en,fr,es}` today; the other six app languages get the chrome in their language and the English body with a notice. `lib/help/tree.js` = 14 categories + Only in FieldQuo + Videos, 311 slugs (Jobber's ~330 mapped one-for-one or marked n/a in `content/help/TREE.md`); one article per sidebar row and per Settings row (`helpArticle` on every row, `/app/help` links them), the four role articles from the permission grid, the fees/payouts article mirroring Jobber's outline with every number executed against `lib/stripe/processingFee.js`. Static pages (outline, figures from `docs/screens/**` only, FAQ, prev/next, related, was-this-helpful → additive `HelpFeedback`, rate-limited, no PII), client-side search over a generated index, sitemap with hreflang, canonical to help.fieldquo.com, light + dark, videos from `content/help/videos.json` (YouTube or a Cloudinary mp4; "video in English" notice). `check:help-centre` (in check:all): coverage of featureMatrix + registry + every sidebar row, EN/FR/ES parity per slug, English-leak heuristic, figure references, chrome in nine languages, middleware order, the feedback route executed through the db stub. **Owner steps:** docs/VERCEL.md → `help.fieldquo.com` (Vercel Domains + one CNAME); record videos and add them to `content/help/videos.json`. Not done: no live captures of the client-facing pages exist (the session is gone) — the harness renders what it can; the rest is said in the report.
