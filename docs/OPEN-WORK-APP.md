...
19. `DONE — code + checks, commit f597ae77; verify on www.fieldquo.com after deploy` **Stripe fees passed through to the contractor** (owner: "we are not going to pay out of pocket") — every homeowner charge now carries `application_fee_amount` from ONE rate table (`lib/stripe/processingFee.js`: cards 3% + 30¢ = Stripe's 2.9% + 30¢ + a 0.1% margin, PAD 1% + 40¢ max $5, ACH 0.8% max $5) plus `on_behalf_of`; new Express accounts request card_payments + transfers, older ones are repaired on the status poll; refunds keep the fee; `Payment.processingFeeCents/netCents/feeRateLabel` written at settlement and exported (Processing fee / Net deposited columns); disputes reverse the held amount + $15 fee from the contractor's transfer and return the hold when won; instant payouts (Settings → Payments, 30-day gate, `InstantPayout` rows) — **owner: set Connect → Platform pricing → Instant payouts to 1%**; Settings → Payments rate card, /platform/billing/plans read-only rates, pricing-page sentence, Jobber bank-debit claim, sales guide EN/FR/ES. `check:processing-fee` (135 executed, in check:all). Not verified live: a real card payment on live keys showing the fee on the invoice; the Express dashboard debit-card path for instant payouts.
