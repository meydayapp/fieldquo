...
Last updated: 10 September 2026 (/sales/messages reads like a phone. Bubbles group by sender inside a 300-second window, break at midnight and at a new speaker, are named and stamped once per group, and sit under day headings — the arithmetic in `lib/sales/messages/grouping.js`, pure, executed by `check:sales-messages` against 301 seconds, midnight, a shuffled thread and an empty one. Behind it, the check-in engine finally has a caller: `checkInSignals()` and `draftCheckIn()` had been written, tested against thirty branches and imported by NOTHING, so no rep could ever have seen a check-in. A due company's draft now appears IN the conversation, dashed and labelled Not sent, with Edit, When and Send — and nothing sends it but a press. Drafts live in their own `SalesCheckIn` table rather than as a status on SalesSmsMessage, whose one invariant is that a row exists only if the carrier accepted it. The rep sets the moment, or parks a manual follow-up from any conversation; both are refused outside the contractor's 08:00–21:00 and both are refused to anybody on the do-not-contact list — where the screen shows no compose box at all. The send takes a compare-and-set claim first, so two presses cannot put two texts on one phone, and hands it back if the carrier refuses. Escalation to tech support sits in the thread, and renders only where the company is one of the rep's own. One real bug fell out on the way: the thread's lead was looked up by "most recently updated lead with any phone" and then discarded on mismatch, so the prospect's TIME ZONE was almost always null — and a null zone refuses every send. **The `SalesCheckIn` table is NOT in the database yet: `prisma db push` fails with Neon's 512 MB project size limit. The screen says "drafts could not be read" rather than "you have none" until somebody with room runs it**)
