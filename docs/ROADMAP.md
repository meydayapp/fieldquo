...
Last updated: 20 September 2026 (a bank-debit payment above Stripe's $3,000.00 CAD pre-authorized-debit cap no longer 500s the client portal: the cap is measured and recorded in `lib/stripe/bankDebit.js`, the portal GET carries a per-invoice and per-stage offer so the bank button is absent above it with a sentence saying why in the eight portal languages, the pay route refuses in the client's language before Stripe is asked and turns any Stripe throw into a 400/502 sentence recorded under `stripe_checkout`, and the session builder refuses on its own — `npm run check:bank-debit-cap`)
...
the states that do not tax construction and asked a one-tap question in
the states that tax some jobs): "the contractor can charge the tax, I'm
not here to police them — they are responsible for what they charge. If
they charge the full tax then the system should allow them to, by default
— just like how we quote in Canada right now." Full write-up:
`docs/US-SALES-TAX.md`.

1. **The rate, by ZIP, on the whole quote.** `UsSalesTaxRate` — 25,337
   ZIPs across the 24 Streamlined Sales Tax member states plus
   Pennsylvania, from the states' own rate and boundary files
   (streamlinedsalestax.org/ratesandboundry, dated in the file name) and
   the Census ZCTA→county crosswalk. Loaded by `npm run us-tax:load`
   (idempotent upsert, never deletes; the boundary files are 300–400 MB
   each, so it runs monthly from the owner's Mac, not a cron).
   `lib/tax/usRates.js` hangs the row on the client object; the resolver
   stays pure and runs in the browser. No row → a state with no local
   sales tax answers with its rate; anywhere else the state floor is
   ...
   "addresses in this ZIP pay up to 7.9%". Destination-based: a Canadian
   company quoting Seattle gets Washington's rate.
2. **The state's rule, as a hint.** `lib/tax/usTaxability.js` (51 cited
   rows: 31 exempt, 4 taxable, 16 depends) no longer changes the number.
   It names the case in which the state leaves the customer untaxed and
   the builder prints it: "Note: residential work is not taxed to the
   customer in Texas; you pay tax on materials when you buy them — switch
   tax off on this quote if that applies to you; what you charge is your
   call." Nothing in WA, HI, NM. The one-tap job-kind chips are gone.
3. **Precedence.** A rate typed on the quote → a company rate named after
   ...
   this rate" / "collect nothing" — the company's own stated zero) → the
   tables → the company default.
4. **The record.** `Quote.taxResolution` holds what the line said (rate,
   ZIP, split, rates month) and is copied verbatim to any invoice raised
   from the quote; PDF, email, public quote page, portal and office pages
   print from it, never from today's table. Documents stored under the
   first version (`us_exempt`) keep printing their stated zero.
5. **Settings → Tax** shows a US card only to a company that touches the
   US: the rule in one paragraph, the rates-table month, the per-state
   overrides. The help article `tax-settings` (EN/FR/ES) carries the same.

`scripts/check-us-tax.mjs` (663 assertions) is in `check:all`.
...
- ZIP-level rates for TX, CA, NY, FL, IL, CO, AZ, AL, LA, MO, NM, SC, VA,
  ...
  its sentence apply and the estimator types the full rate. A live address
  lookup (CDTFA's API for CA, the Texas GIS locator) at quote time is the
  honest next step.
- The taxability rows not re-fetched this month should be read from their
  publication before their hint is relied on.
...
one generated by AI. but it should also have the option to be in English,
French and Spanish. just in case we find ourselves with a French speaker in
Quebec or a Latino contractor in the USA."

- **One `ProspectCallScript` row per prospect PER LANGUAGE.** Additive:
  `language String @default("en")`, `@@unique([prospectId, language])`
  replacing `prospectId @unique` (an index change, not a data one — the 148
  existing rows read as `en`; applied as three additive statements and one
  index drop because `prisma db push --accept-data-loss` is blocked in this
  environment, then a plain `prisma db push` reported the schema in sync).
  `PlatformAiUsage.meta Json?` carries `{ language, trigger }` on every
  `call_script` row.
- **The default rule** (`lib/sales/intel/callScript.js defaultScriptLanguage`):
  the lead's required language first (`requiredLanguageFor`: Quebec → `fr`;
  every other province → `en` since the 2026-09-12 rule; NB and no province →
  none), else the rep's portal language when it is one of the three, else
  `en`. So the rep-language branch reaches only NB and province-less rows —
  a Spanish-speaking rep on a Texas list opens on English and flips to
  Español; that is the lead-language rule outranking the portal, on purpose.
  The pipeline's `GENERATE_CALL_SCRIPT` writes the default only (no cost
  change for the backlog); `research.js`'s three-way staleness rule reads the
  default language's row and no other.
- **On demand, the other two.** `GET /api/sales/playbook?prospectId=&language=fr`
  — a row for that language missing or stale (same three-way rule) is written
  synchronously through the SAME `generateCallScript()` the stage uses (one
  prompt, one lint, one meter; the handler and the route share it), metered
  under `call_script` with `meta.language`, `meta.trigger: "on_demand"`, the
  asking rep's id and an `on_demand:` ref; rate-limited at
  `ON_DEMAND_PER_HOUR = 60` per rep counted off those ledger rows; a refused
  or failed generation answers with the default language's script and
  `scriptLanguage.fallback = { requested, shown, reason }` — never an empty
  panel. The route still never spends on an ordinary open: the default
  language is never generated there.
- **The prompt writes the body in the language, the quotes stay the site's.**
  `REGISTER`: Quebec French — "vous" (tutoiement not assumed), "soumission"
  not "devis", "cellulaire"; neutral Latin-American Spanish — "usted", no
  vosotros, "celular" not "móvil". The owner's English opener stays the one
  style example in every language, with "write the SAME register in …"
  around it. Citations are copied verbatim, never translated, so
  `findQuoteSource` still matches; across languages the lint keeps the
  verbatim-in-source half of rule 6 and drops the shared-content-word half,
  which cannot be measured across languages and is not pretended to be
  (`scriptVoice.js` header says what is lost).
- **The lint in three languages.** Finite-verb lists for French and Spanish
  (auxiliaries, modals, the persons a call uses, a suffix rule, a
  verb-position rule that errs toward accepting — documented as the opposite
  of the English list's fail-safe direction, because a Spanish script that
  failed every regular "-a" verb would fail every Spanish script), Unicode
  tokenising, "plutôt que" / "en lugar de" beside "rather than". **Rule 7,
  new and in every language:** a WORD-level guard on `closeAsk` — day names,
  tomorrow/demain/mañana-as-tomorrow, o'clock, "at ten", "à dix heures", "a
  las diez", half past / et demie / y media, midi / mediodía — which the
  coordinator noted was missing even in English ("tomorrow at ten" passed;
  "Tuesday at 2" was caught by the 2). Applied to the ask only: "closed
  Sundays" in whatWeSaw is a fact off the site. The old check fixture's ask
  ("on Thursday") is refused now and was rewritten.
- **The screen.** `CallPlaybook.js`: English · Français · Español (each
  language's own name) under the "Generated from what the crawler saw on …"
  line, in both layouts, current = the default, remembered per rep per
  language-of-lead in localStorage (`lib/sales/scriptLanguageMemory.js`;
  choosing the default forgets), a loading state while the other language is
  read, the fallback sentence when the route answered with the default. Three
  catalogue keys × nine languages; "Written in English from what was
  observed" became "Written from what was observed" in all nine.
  `CallPanel.js` re-reads with `&language=` and honours the memory on open.
  Both the console's Script tab and the lead page go through CallPanel, so
  both have it. `docs/screens/sales-console/desktop-script-fr.png` /
  `desktop-script-es.png` from the harness.
- **The brief is unchanged and stays in the portal language / English.** The
  Company card's phrased description and angles (`brief.js` phrasing,
  cached on the brief task) are model-phrased English and are NOT switched
  with the script — they are a different tab, a different cache and would be
  a second on-demand call; the "what we inferred" labels are catalogue
...
## The snapshot URL is configured once, and everything else is picked (9 September 2026)

The owner, about `/platform/sales/campaigns`: *"I thought you had already
fetched all the companies and saved it in cloudflare… where the fuck do I get
the snapshot URL… it should be just automated for me in a way that I can just
select few things and get the total number of trade."* He was right. The
extract had been run and uploaded — **80 files, 1,320,105 rows** in the R2
bucket `fieldquo-lead-storage` — and the form was still demanding a "Snapshot
URL (required)" per source and telling him to run a DuckDB extractor.

**One setting, once.** `/platform/sales/snapshots` (superadmin,
`PlatformSnapshotLibrary`, a singleton) holds the bucket's public base URL.
Saving it FETCHES: the route derives the URL of the smallest file in the
library, reads its first line, and refuses the save unless the header says what
the library says it should. Until it is set, the campaign form says so and
links there instead of creating campaigns that would read nothing.

**Nothing types a URL any more.** The create route builds each campaign's
snapshot URL from base + the object key of the file covering the chosen region,
and probes every one of them before the transaction. The campaign screen's
config box is gone too — it is a "Rebuild this URL from the snapshot library"
button, which re-derives and re-proves. `scripts/check-snapshot-campaigns.mjs`
(in `check:all`) pins all of it, mutation-tested.

**Country, region and trade are selects, filled from the data's own coverage.**
`lib/sales/discovery/snapshotLibraryData.js` is generated by
`scripts/build-snapshot-library.mjs` from the uploaded files themselves: per
file, the row count, the trade tally produced by running the repo's own
`tradeForCategories()` over every row, and per region the MEDIAN of its rows'
own coordinates — which is what lets the optional circle prefill a centre
without anybody looking up a city. So the form can say **"Painting, California
— 15,713 businesses"** before the button is pressed. Regenerate after any new
upload:
`node --import ./scripts/alias-loader.mjs scripts/build-snapshot-library.mjs`.

**"Not known" is never printed as a zero.** The RBQ register names no trade for
anybody, so Québec reads "343 painters, plus 54,275 rows whose trade is not
known" rather than pretending the register contains no painters.

**One selection can create several campaigns.** The extract is split at 50,000
rows a file and a campaign reads one file per source, so a region covered by
five files creates five campaigns — same territory, same trade — and the form
says how many before the button is pressed.

**The registration gate moved into the route.** A campaign whose territory is
in a jurisdiction FieldQuo is not registered to solicit in is created and
CANNOT BE STARTED (`lib/sales/discovery/campaignGate.js`), which is what a
creation script used to do by hand from somebody's laptop — with a hand-kept
list of eight states that had already drifted from the rules it was copied
from. The real list is read from `CALLING_JURISDICTIONS` and is **thirteen**:
Washington, Texas, New Jersey, Utah, Mississippi, Rhode Island, Ohio, Colorado,
Wisconsin, Alaska, Idaho, DC and Vermont. California requires no telemarketer
registration, so its 226,172 licences are not gated on one. The campaign screen
orders the outstanding list by the rows each would unlock.

**Blocked on the owner:** paste the bucket's public `r2.dev` URL at
`/platform/sales/snapshots`, and run `npx prisma db push` for
`PlatformSnapshotLibrary` — it is added to the schema and deliberately not
pushed.
