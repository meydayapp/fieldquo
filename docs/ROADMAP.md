...
Last updated: 19 September 2026 (the AI employee has a permission mode instead of a send switch, a proposals inbox for what the mode did not let it do alone, a website chat channel and an SMS channel on the one inbox, a real book_appointment through the self-booking engine, a face, a name and a voice, one row per role, and it runs on the best model at that model's real rate — see the section below)
