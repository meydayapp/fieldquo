...
| `FIELDQUO_SALES_HOLD_MUSIC_URL` | — | What an inbound caller hears while every rep is being rung (`/api/rep-dial/inbound`). Unset, the queue plays Twilio's own default hold music, so a caller is never left on a silent line either way — set it only to use your own recording. Must be a publicly reachable URL Twilio can fetch; a URL that 404s gives the caller silence, which is the one outcome the queue exists to prevent |
