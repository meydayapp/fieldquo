...
- Data deletion instructions URL: https://www.fieldquo.com/data-deletion. Data Deletion Request Callback URL: https://www.fieldquo.com/api/meta/data-deletion (Meta POSTs a `signed_request` here when a person removes the app; it answers with a confirmation code and a status URL on the page above — verified against the app secret, so META_APP_SECRET must be set in Vercel for it to accept anything). Privacy policy URL: https://www.fieldquo.com/privacy. Terms URL: https://www.fieldquo.com/terms. (If any of these 404s, tell me and stop.)
...
3. App Settings → Basic: fill Display name "FieldQuo", Contact email (ask me), Privacy Policy URL, Terms of Service URL, User data deletion → choose "Data deletion request callback URL" and enter https://www.fieldquo.com/api/meta/data-deletion; if the form only offers one of the two, use the callback URL and put the instructions URL (https://www.fieldquo.com/data-deletion) wherever it asks for a page — Category "Business and pages", App icon (tell me when to upload). Stop before Save and read the whole form back to me.
