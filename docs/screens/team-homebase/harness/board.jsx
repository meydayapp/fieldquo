...
function payload() {
  ...
  // The week's shifts for the labour line: today's, plus earlier days so
  // Marc is past forty hours and Ana is under.
  const weekShifts = [
    ...shifts.map((x) => ({ id: x.id, workerId: x.workerId, start: x.start, end: x.end, published: x.published, breaks: x.breaks })),
    ...[1, 2, 3, 4].map((d, i) => ({ id: `wk-m${i}`, workerId: "w1", start: at("07:00", d), end: at("17:30", d), published: true, breaks: [{ start: at("12:00", d), end: at("12:30", d), paid: false }] })),
    ...[1, 2, 3].map((d, i) => ({ id: `wk-a${i}`, workerId: "w2", start: at("08:30", d), end: at("16:30", d), published: true, breaks: [{ start: at("12:30", d), end: at("13:00", d), paid: false }] })),
  ];
  return {
    manager: true,
    shifts,
    ...
    live,
    businessHours,
    self: { id: "w1", name: "Marc Tremblay", title: "Foreman" },
    weekShifts,
    attendance: [
      { shiftId: "s1", status: "late", final: false, lateMinutes: 12, earlyMinutes: 0 },
      { shiftId: "s2", status: "on_time", final: false, lateMinutes: 0, earlyMinutes: 0 },
    ],
    payVisible: params.get("as") !== "dispatcher",
    rates: params.get("as") !== "dispatcher" ? { w1: 34, w2: 28, w3: 22, w4: null, w5: 26 } : null,
    otThresholdWeekly: 40,
    holidays: [{ key: "labourDay", name: "Labour Day", date: ymd, observed: ymd }],
    blackouts: [],
  };
}
