...
function callSectionsPort(fromIso, toIso) {
  ...
  const callRows = repIds.map((id) => {
    ...
    const conversations = mine.filter((a) => a.answeredAt && a.talkSeconds > 60).length;
    return { id, name: r?.name || id, agency: null, active: Boolean(r?.active), presence: null, stats: { ...base, dials: mine.length, measured: { ...base.measured, total: mine.length, bridged, measuredOf: talk.length, talkMs: talk.length ? talk.reduce((n, a) => n + a.talkSeconds, 0) * 1000 : null }, connect: { measured: bridged, connected, conversations, notConversations: connected - conversations, unknown: 0, awaitingTranscript: 0, unrecorded: 0, minContractorWords: 20, answerRate: rate(connected, bridged), conversationRate: rate(conversations, connected) }, callbacks: { booked: mine.filter((a) => a.callbackAt).length, upcoming: [], overdue: [] } } };
  }).sort((x, y) => y.stats.dials - x.stats.dials || x.name.localeCompare(y.name));
  const tb = now.filter((a) => a.dialChannel === "browser").length;
  const tc = now.filter((a) => a.answeredAt).length;
  const tconv = now.filter((a) => a.answeredAt && a.talkSeconds > 60).length;
  const totalStats = { ...repStats("rep_ana"), dials: now.length, measured: { total: now.length, bridged: tb, measuredOf: now.filter((a) => a.talkSeconds > 0).length, talkMs: now.filter((a) => a.talkSeconds > 0).reduce((n, a) => n + a.talkSeconds, 0) * 1000, holdMs: null, holdOf: 0, costCents: null, costOf: 0 }, connect: { measured: tb, connected: tc, conversations: tconv, notConversations: tc - tconv, unknown: 1, awaitingTranscript: 1, unrecorded: 0, minContractorWords: 20, answerRate: rate(tc, tb), conversationRate: rate(tconv, tc) }, callbacks: { booked: now.filter((a) => a.callbackAt).length, upcoming: [], overdue: [] } };
  return {
