...
await sleep(800);
for (let i = 0; i < 40; i++) {
  const r = await send("Runtime.evaluate", { expression: "document.documentElement.getAttribute('data-scene-ready') || (new URLSearchParams(location.search).get('do') ? '' : '1')", returnByValue: true });
