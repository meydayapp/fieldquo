...
| `desktop-demo-send.png` | 2026-09-14, the owner's demo thread ("Flooring Demo — Daniel", `?scenario=demoSend`): the day-1 draft WITH its Send — the send path simulates a demo send rather than refusing it — the footer "This is your demo company — Send here shows what happens, nothing leaves FieldQuo", and above the list the drafts-waiting banner: "3 drafts waiting — the engine wrote them for you; review and press Send. 1 of them on your demo company. 1 with no number on file — add one on My companies." with its "Show only drafts" press. |
