...
//
// The report laid out for a phone. Server component: every colour is a
// literal from lib/documents/theme.js (measured pairs — fillPair for the
// tiles and the starting-at figure, washPair for cards, never the raw brand
// hex under text), every sentence is from the model, and the only script on
// the page is the call-back form.
//
// Section order mirrors the PDF (lib/estimate/report/pdf.js) so the two
// documents read the same from top to bottom.
...
export default function ReportView({ report, company, token }) {
  ...
  return (
    <main className="min-h-dvh px-4 py-6 sm:py-10" style={{ backgroundColor: theme.page, color: theme.ink }}>
      <article
      ...
      >
        ...
        <div className="px-5 py-6 sm:px-8">
          ...
          {/* Questions */}
          <section className="mt-6" id="callback">
          ...
          <section className="mt-6">
            ...
            <div className="text-[11px] font-bold tracking-[0.12em] uppercase mt-4 mb-2" style={{ color: theme.accentText }}>
              {n.nextTitle}
            </div>
            <ol className="m-0 p-0 list-none space-y-2">
              {n.nextSteps.map((s, i) => (
                <li key={i} className="flex gap-3 text-sm" style={{ color: theme.ink }}>
                  <span
