// app/components/settings/BrandingForm.js
//
// The whole of Settings > Branding, as one component — the page renders it
// and so does the home page's "Add your logo and brand color" dialog
// (app/components/dashboard/stepPanels.js). One component, not a copy: the
// dialog uploads through the same handleFileSelect and saves through the same
// PATCH, so a logo added from the checklist is exactly a logo added from
// Settings, and there is no second form to drift.
//
// `compact` is what the dialog passes. It drops the page heading (the dialog
// has its own), the secondary and neutral colour rows, and the document
// preview — the step is "logo and brand colour", and a phone-height sheet
// with six sections in it is the page the dialog exists to spare people.
// Nothing compact hides is lost: the settings link at the foot of the dialog
// is the same page with everything on it.
//
// `onSaved` fires after a successful PATCH. The page passes nothing and keeps
// its "Saved ✓" flash; the dialog closes and re-reads the checklist.
"use client";
...
export default function BrandingForm({ compact = false, onSaved } = {}) {
  const { t } = useTranslation();
  ...
  // The outer padding and width are the page's; inside a dialog the frame
  // already has both.
  const frame = compact ? "space-y-6" : "p-4 sm:p-6 max-w-2xl mx-auto space-y-6";

  if (loading) {
    return (
      <div className={compact ? "" : "p-4 sm:p-6 max-w-2xl mx-auto"}>
