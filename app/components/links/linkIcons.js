...
// reads at 20px.
const FILLED = {
  // WhatsApp, for the messaging inbox's third platform badge
  // (app/app/messages). Drawn here and not imported, for the reason at the top
  // of this file: lucide ships no brand marks at all, and a `Whatsapp` import
  // is a build failure — the same one a `Facebook` import already caused in
  // this repo once. Not in lib/links/social.js's SOCIAL_PLATFORMS, and
  // deliberately: that list is the bio-link page's social row, where a
  // "profile URL" for a phone number is not a thing that exists. A glyph may
  // be used without being a social profile.
  whatsapp:
