...
export const MORE_GROUPS = [
  {
    ...
    items: [
      ...
      { key: "app.nav.clientTickets", href: "/app/tickets", icon: LifeBuoy, helpArticle: "client-tickets-queue" },
...
// dead taps). Request was the last exception — it opened the leads board,
// because there was no hand-entered lead form — and now opens one:
// /app/leads/new, which posts through the same createScoredLead every inbound
// channel uses and lands on the board with the new lead's drawer open.
// scripts/check-mobile-surfaces.mjs holds every href here to a real page file.
export const QUICK_ADD_ITEMS = [
  { key: "app.quickAdd.client", href: "/app/clients/new", icon: Users },
  { key: "app.quickAdd.request", href: "/app/leads/new", icon: ClipboardList },
