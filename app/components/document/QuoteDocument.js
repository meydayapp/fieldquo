  ...
  return (
    ...
    >
      ...
      <div className="flex h-[5px]" aria-hidden="true">
        <div className="flex-[7] bg-inverted" />
        <div className="flex-[3] bg-inverted opacity-55" />
      </div>
      {children}
    </article>
  );
    ...
    <header className="px-5 sm:px-7 pt-5 pb-4 border-b border-border" data-doc-masthead>
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="sm:text-right shrink-0">
          <div className="text-[11px] font-semibold tracking-[0.12em] uppercase text-brand-accent-text">
            {word}
          </div>
...
export function DocumentScopeGroup({
  ...
  return (
    ...
    >
      <div
        className={`flex items-center justify-between gap-3 px-3 py-2 ${accent ? "" : "bg-muted"}`}
      ...
      <div className="px-3">
        {lines.map((item, i) => (
          <div
            key={i}
            className="flex justify-between gap-4 text-[13px] text-foreground py-[5px] border-t border-border first:border-t-0"
            data-doc-line
          >
...
export function DocumentTotals({ rows = [], total, after = null, edit = {}, children }) {
  return (
    <div className="px-5 sm:px-7 py-5 border-t border-border" data-doc-totals>
      ...
      <div className="sm:max-w-[360px] sm:ml-auto space-y-0.5 text-[13px]">
        {rows.map((row) => (
        ...
        {total && (
          <div className="flex items-center justify-between gap-3 rounded-md bg-inverted text-inverted-foreground px-2.5 py-2 mt-1 font-bold" data-doc-total-band>
            <span className="text-xs uppercase tracking-wide">{total.label}</span>
            <span className="text-lg tabular-nums">{total.value}</span>
          </div>
        )}
        {after}
      </div>
