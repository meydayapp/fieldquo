...
export default function CheckInDraft({
  ...
  const [aimedBefore, aimedAfter] = sentenceAround(t("app.salesText.draftAimedAt"), "{when}");

  return (
    <div className="flex flex-col items-end py-2">
      <p className="px-1 pb-1 text-xs text-muted-foreground break-words text-right max-w-[95%]">
        <span className="font-semibold text-foreground">
          {suggestion ? t("app.salesText.suggestedCheckIn") : t("app.salesText.draftBadge")}
        </span>
        {reason ? ` · ${reason}` : null}
      </p>
      ...
      <div className="w-full max-w-[95%] rounded-2xl rounded-br-md border-2 border-dashed border-primary/50 bg-card p-3 space-y-3">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="inline-flex items-center gap-1 rounded-full bg-primary/10 px-2 py-1 text-xs font-semibold text-primary">
            ...
            {t("app.salesText.notSent")}
          </span>
          {draft.scheduledFor ? (
            <span className="text-xs text-muted-foreground">
              {aimedBefore}
              {scheduleLabel(draft.scheduledFor)}
              {aimedAfter}
            </span>
          ) : null}
          {draft.degraded && draft.draftSource === "rule" ? (
            ...
            <span className="text-xs text-muted-foreground">{t("app.salesText.plainWording")}</span>
          ) : null}
        </div>
        ...
        {picking ? (
          <div className="space-y-2 rounded-lg bg-muted p-3">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                {t("app.salesText.noParticularTime")}
              </button>
              <button
                type="button"
          ...
          {suggestion ? (
            ...
            <>
              <button
                ...
                className={`${BTN} border border-border text-foreground`}
              >
                <PencilLine size={15} aria-hidden="true" /> {t("app.salesText.edit")}
              </button>
              <button
                type="button"
                disabled={busy}
                ...
                className={`${BTN} border border-border text-foreground`}
              >
                <CalendarClock size={15} aria-hidden="true" /> {t("app.salesText.whenButton")}
              </button>
              <button
                type="button"
                disabled={busy}
                ...
                className={`${BTN} text-muted-foreground`}
              >
                <X size={15} aria-hidden="true" /> {t("app.salesText.putAway")}
              </button>
