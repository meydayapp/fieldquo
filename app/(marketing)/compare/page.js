 ...
 * data as it stands today: Housecall Pro has amounts whose currency their own
 * footer states, ServiceTitan has only request-a-price tiers, Projul has three
 * amounts whose currency their page never names, QuoteIQ has ten, and Jobber
 * has both amounts and a large withheld pile.
 *
 * ══ Why the currency line is separate ══════════════════════════════════════
 *
 * The first sentence used to end "in the currency they print it in", and it
 * was true of every competitor on this index when it was written. It stopped
 * being true the day Projul's three amounts began publishing: their page
 * prints no currency at all and the one beside those figures is the owner's
 * signed assertion. A card that keeps the old ending says Projul printed
 * something they did not — a false statement about a competitor, generated by
 * a sentence nobody edited, which is the exact failure this file's header
 * warns about one paragraph up. So the clause is conditional on the figures'
 * own `currencySourcing`, and where it does not hold the card says what
 * actually happened instead of going quiet.
 */
export function comparisonSummary(competitor, asOf) {
  ...
  // Absence is not read as "off their page" here either: withholdReason
  // already refuses a figure that records no provenance for its currency, so
  // anything that is not SOURCED_PUBLISHER got there by somebody signing for it.
  const asserted = amounts.filter((f) => f.price?.currencySourcing !== SOURCED_PUBLISHER);

  const lines = [];
  if (amounts.length > 0) {
    lines.push(
      `${amounts.length} of their published ${amounts.length === 1 ? "price" : "prices"} can be set beside ours${
        asserted.length === 0 ? ", in the currency they print it in." : "."
      }`,
    );
  }
  if (asserted.length > 0) {
    lines.push(
      `${asserted.length} of those name no currency on their own page, so the comparison says whose judgement the currency is rather than printing it as theirs.`,
    );
  }
