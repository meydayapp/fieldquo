// app/app/settings/material-costs/MaterialCostsEditor.js
//
...
// Exported, not just the page's: the home page's "Review cost and material
// recipes" dialog renders this same editor (app/components/dashboard/
// stepPanels.js). `compact` drops the page heading, the "Back to home" link
// and the cost-revision card — the dialog is about the recipes — and
// `onChanged` fires after a recipe is saved or reset so the checklist can
// re-read itself. The page passes neither.
export default function MaterialCostsEditor({ compact = false, onChanged } = {}) {
  ...
  if (loading) {
    return (
      <div className={`${compact ? "" : "p-4 sm:p-6 max-w-3xl mx-auto"} animate-pulse space-y-4`}>
