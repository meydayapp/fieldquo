...
//
// The whole of Settings > Availability as one component. The page renders
// it, and so does the home page's "Check your availability for bookings"
// dialog (app/components/dashboard/stepPanels.js) — the same week editor
// writing the same rows, so hours set from the checklist are exactly hours
// set from Settings.
//
// `compact` is what the dialog passes: your own hours only (no person
// picker), the bookable week only, and a plain Save button instead of the
// bottom dock — the dialog is a sheet with its own frame. Compact still
// READS the shift, because the mismatch warning under the bookable week is
// about the two disagreeing; it just does not edit it, and so its save
// writes the bookable rows alone. `onSaved` fires after that save.
//
...
export default function AvailabilityEditor({ compact = false, onSaved } = {}) {
  const { t, language } = useTranslation();
  ...
  async function handleSave() {
    ...
    try {
      await Promise.all([
        ...
        // The dialog does not edit the shift, so it does not write it: a PUT
        // that replaces the week with the rows it just read is a write for
        // nothing, on a route whose failure mode is a deleted calendar.
        ...(compact
          ? []
          : [
  ...
  if (loading) {
    return <div className={`${compact ? "" : "p-4 sm:p-6 max-w-2xl mx-auto"} animate-pulse h-64 bg-accent rounded-xl`} />;
  }

  if (loadFailed) {
    return (
      <div className={compact ? "space-y-4" : "p-4 sm:p-6 max-w-2xl mx-auto space-y-4"}>
        {!compact && (
  ...
  return (
    <div className={compact ? "space-y-8" : "p-4 sm:p-6 max-w-2xl mx-auto space-y-8"}>
      {!compact && (
      <div>
        <h1 className="text-2xl font-bold text-foreground">
      ...
      {/* The dock is the page's; the dialog's sheet has its own frame, so
          there the same controls sit in flow under the week. */}
      {compact ? (
