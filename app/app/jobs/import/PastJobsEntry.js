// app/app/jobs/import/PastJobsEntry.js
//
// The whole of the Past jobs screen as one component. The page renders it,
// and so does the home page's "Import older jobs" dialog
// (app/components/dashboard/stepPanels.js): the same one-job form, the same
// CSV review, the same POST — and the same promise, that nothing goes to the
// client. `compact` drops the page heading and the two links back; `onChanged`
// fires after a job (or a file of them) is written so the checklist can
// re-read itself. The page passes neither.
//
...
function SingleJobForm({ clients, categories, defaultTaxApplied, onChanged }) {
  const { t } = useTranslation();
...
export default function PastJobsEntry({ compact = false, onChanged } = {}) {
  const { t } = useTranslation();
  ...
  return (
    <div className={compact ? "space-y-4" : "p-4 sm:p-6 max-w-4xl mx-auto space-y-4"}>
      {!compact && <BackToHome />}
      <div>
        {!compact && (
        ...
        {/* The promise stays in both places: nothing goes to the client. */}
        <p className={`text-sm text-muted-foreground ${compact ? "" : "mt-1"}`}>
      ...
      <SingleJobForm clients={clients} categories={categories} defaultTaxApplied={defaultTaxApplied} onChanged={onChanged} />
      <CsvSection onChanged={onChanged} />
    </div>
  );
}
