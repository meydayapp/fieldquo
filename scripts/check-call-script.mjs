...

{
  ...
  ok("…drawn only when the route returned one", /\{data\.callScript \? \(\s*layout === "console" \? \(\s*<ConsoleScript script=\{data\.callScript\}[\s\S]{0,240}<AiScript script=\{data\.callScript\}/.test(draw));
...
// and the voice lint refuses a banned move in a generated sentence.
{
  ...
  for (const [what, re] of [
    ...
    ["…and treats the person who writes the quotes as the right person, said to them as speech", /If the person who answered writes the quotes up, they are the right person — say so TO THEM, as speech/],
    ["…never as a note to the rep", /never a note to the rep such as "if they are the one who writes the quotes, carry on"/],
