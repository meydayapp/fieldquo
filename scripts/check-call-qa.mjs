...
// ═══════════════════════════════════════════════════════════════════════════
{
  ...
  ok("a scorer that throws cannot fail the transcription", /score\(attempt\.id, \{ client, sample \}\)\.catch\(/.test(src));
  ok("…and the on-demand flag (sample: false) reaches the scorer, so a superadmin's ask is scored whatever the review sample says", /score\(attempt\.id, \{ client, sample \}\)/.test(src));
