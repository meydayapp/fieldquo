// Executes lib/gallery/stages.js + albums.js — stage inference and public
// selection — plus lib/gallery/tags.js — company-defined job-photo tags —
// and proves the tag axis can't reach the stage-driven privacy/pairing logic
...
ok("featuredUrls() does not reference a tag table or field — tags can't participate in this gate",
...
// ════════════════════════════════════════════════════════════════════════
// Source-scan: the API route's tagIds sync can't have drifted either
// ════════════════════════════════════════════════════════════════════════
//
// tagIds is a second write path alongside the scalar `data` object PATCH
// already builds (stage/featured/caption). This pins that it stays a
// SEPARATE axis, never folded into `stage`, and that the "issue" privacy
// refusal (proven above against featuredUrlsBody) still keys off the
// stage-driven fields, never tagIds. Reuses stripComments/readFileSync from
// the source-scan block above rather than re-deriving them.
